<? 
	...
	if($ecom_siteid==111 or $ecom_siteid==105)
	{
	...
	{
			global $productPageUrlHash, $db, $ecom_siteid, $ecom_hostname,$ecom_advancedseo,$ecom_selfhttp;		
				...
				if($value_id > 0 && $value_name!='' )
				{
					...
					$productPageUrlHash = $ecom_selfhttp.$ecom_hostname."/".$prodName."-".$value_name."-p$prodId-x$value_id.html";
				}
				else
				{
					...
					{
						$productPageUrlHash = $ecom_selfhttp.$ecom_hostname."/p$prodId/".$prodName.".html";
					}	
				}
	...
	// Get the delivery id for current website
		...
		{
			//if($ecom_siteid!=61)
			if($ecom_siteid!=61 and $ecom_siteid!=105 and $ecom_siteid!=77)
			{
				$str4 = "\tshipping";		
	...
	if($ecom_siteid==80)
	{
	print "id".$str."\tcondition\tdescription\texpiration_date\tc:bshopid\timage_link".$str1.$str2.$str3."\tlink\tprice\tproduct_type\tquantity\tweight\ttitle\tidentifier_exists".$str4.$str5."\n";
	}
	else
	{
	...
	while($row_prod = $db->fetch_array($ret_prod)) 
	{
		...
		//amount section start 
			...
			/*if($ecom_siteid==105) // only for puregusto
			{
				$curprice = $curprice - .003;
			}*/
		...
		//Google feed image fetching section
		 $sql_imgfeed = "SELECT a.image_id, a.image_bigpath, a.image_thumbpath FROM images a,images_googlefeed_product b 
					WHERE b.products_product_id = ".$row_prod[product_id]." AND a.image_id=b.images_image_id
					ORDER BY b.image_order ";
		$ret_imgfeed = $db->query($sql_imgfeed);
		if ($db->num_rows($ret_imgfeed)>0)
		{ 		$bigimage_path_arr = array();

			$cnt = 0;
			while ($row_imgfeed = $db->fetch_array($ret_imgfeed))
			{
				//$row_img 		= $db->fetch_array($ret_img);
				if($cnt == 0)
				{
					$images_id		= $row_imgfeed['image_id'];
					$bigimage_path	= $row_imgfeed['image_bigpath'];
					$thumb_path		= $row_imgfeed['image_thumbpath'];
				}
				else
				{
					if($cnt <= 10)
					{
						$bigimage_path_arr[]	= $row_imgfeed['image_bigpath'];
					}
				}
		...
		{		
			if($proceed_with_valueURL == true)
			{
				...
				if($db->num_rows($ret_varchck)==1) // just one variable only
				{
					if($db->num_rows($ret_varchck)>0)
					{
						$row_varchck = $db->fetch_array($ret_varchck);
					}		
				   $sql_var_data = "SELECT var_value_id,var_value,google_feed_prod_title FROM product_variable_data WHERE product_variables_var_id=".$row_varchck['var_id']." ORDER BY var_order LIMIT 1";
			...
			// id
						...
						if($ecom_siteid==105) // only for puregusto
						{
							if($row_var_data['var_value_id']==702054)
							{
								//$curprice = $curprice - .003;
							}		
						}
			...
			if(file_exists($big_image_path) and $bigimage_path!='')
			{
				...
				$big_image_url = $ecom_selfhttp.$ecom_hostname."/images/".$ecom_hostname."/" . $bigimage_path;
				print $big_image_url . "\t";
			}
				...
				if($bigimage_path_arr)
				{
					foreach($bigimage_path_arr as $bigimg_path)
					{
						...
						if(file_exists($big_img_path) and $bigimg_path!='')
						{
							...
							$add_image_urls .= $ecom_selfhttp.$ecom_hostname."/images/".$ecom_hostname."/" . $bigimg_path ;
						}
					}
				}
			...
			$ret_link = url_product_values($row_prod['product_id'],$row_prod['product_name'],1,0,'');
			print $ret_link."\t";
			...
			//weight
				...
				$web_weight		= trim($row_prod['product_weight']);
			
			    print  $web_weight."\t";
			}
			//title
			if($row_var_data['google_feed_prod_title']!='')
			{
			  $title = $row_var_data['google_feed_prod_title'];
			}
			elseif($row_prod['prod_googlefeed_name']!='')
			{
			...
			print "\n";
			   ...
			   elseif($db->num_rows($ret_varchck)>1) // more than one variable
				{
					$sql_varchck = "SELECT var_id ,var_name FROM product_variables WHERE var_value_exists=1 AND var_hide=0 AND products_product_id=".$row_prod['product_id']." ORDER BY var_order";		
					$ret_varchck = $db->query($sql_varchck);
					$varvalnew_arr = array();
					$varvalnew_str = '';
					if($db->num_rows($ret_varchck))
					{
						while ($row_varchk = $db->fetch_array($ret_varchck))
						{
							// get the id of first values of each of these variables
							$sql_valchk = "SELECT var_value_id 
												FROM 
													product_variable_data 
												WHERE 
														product_variables_var_id = ".$row_varchk['var_id']." 
												ORDER BY 
													var_order 
												LIMIT 
													1";
							$ret_valchk = $db->query($sql_valchk);
							if($db->num_rows($ret_valchk))
							{
								$row_valchk = $db->fetch_array($ret_valchk);
								if($row_valchk['var_value_id'])
								{
									$varvalnew_arr[] = 	$row_valchk['var_value_id'];
								}
							}
						}
						$varvalnew_str = implode(',',$varvalnew_arr);
						$row_prod['cur_var_value_id'] = $varvalnew_str;
					}	
	...
	function calculat_special_price_new($prod_arr)
	{
		...
		{
			$varvalueid_arr = explode(',',$prod_arr['cur_var_value_id']);
			foreach($varvalueid_arr as $vkey=>$vval)
			{
				$vval = trim($vval);
				if($vval>0)
				{
					// Check whether additional price is set for this variable value
					$sql_newvarval = "SELECT var_addprice FROM product_variable_data WHERE var_value_id = ".$vval." LIMIT 1";
