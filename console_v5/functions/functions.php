...
{ 
	...
	//echo $old."**".$new."ge--".$geometry; 
	$Geoarr = explode("x",$geometry);
	$width_rz = $Geoarr[0];
	$height_rz = $Geoarr[1];
	//$old = $new.'\new';
	...
	{ 
		...
		$resize = new ResizeImage($old);
		if($height_rz!='' AND $width_rz!='')
		{
			//echo "$new%exact%";
		$resize->resizeTo($width_rz, $height_rz,'exact');
		}
		else if($width_rz!='' AND $height_rz='')
		{
			//echo "$new%maxWidth%";
		   $resize->resizeTo($width_rz, $height_rz,'maxWidth');
		}
		else
		{
			//echo "$new%no%";
		    $resize->resizeTo($width_rz, $height_rz);
		}
		$resize->saveImage($image_path.'/'.$new);
