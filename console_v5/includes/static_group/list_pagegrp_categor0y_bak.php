<?php
	...
	# Description 	: Page for listing Product Categories for assigning to the static page groups
	# Coded by 		: Sny
	# Created on	: 22-Jun-2007
	# Modified by	: ANU
	# Modified On	: 03-Jun-2007
	#################################################################*/
...
{}*/
	...
	$query_string = "request=stat_group&fpurpose=assign_category&group_id=".$_REQUEST['group_id'];
	...
	function call_ajax_assignpagegroup(mod,cname,sortby,sortorder,recs,start,pg,group_id)
	{
		var atleastone 			= 0;
		var curid				= 0;
		var cat_ids 			= '';
		...
		//document.frm_prodcat.fpurpose.value = mod;
			
		var qrystr				= 'catname='+cname+'&sort_by='+sortby+'&sort_order='+sortorder+'&records_per_page='+recs+'&start='+start+'&pg='+pg;
		...
		{
			var msg = 'Assign the selected product categories from the selected product page group?';
			if(confirm(msg))
			{
					//show_processing();
					alert('fpurpose='+mod+'&'+qrystr+'&catids='+cat_ids);
					Handlewith_Ajax('services/static_group.php','fpurpose='+mod+'&'+qrystr+'&catids='+cat_ids+'&group_id='+group_id);
			}	
		}	
	}
	...
	function handle_expansion(imgobj,mod)
{
	...
	switch(mod)
	{
		case 'category':
			if (retindx!=-1)
			{
				...
				document.getElementById('cattr_details').style.display = '';
				if(document.getElementById=='cattr_norec'){
				document.getElementById('cattr_norec').style.display = '';
				}
			...
			else
			{
				...
				document.getElementById('cattr_details').style.display = 'none';
				if(document.getElementById=='cattr_norec'){
				document.getElementById('cattr_norec').style.display = 'none';
				}
			}	
		break;
	};
	...
	<input type="hidden" name="fpurpose" value="assign_category" />
	<input type="hidden" name="search_click" value="" />
	<input type="hidden" name="group_id" value="<?=$_REQUEST['group_id']?>" />
	<table border="0" cellpadding="0" cellspacing="0" class="maintable">
		<tr>
		  <td colspan="3" align="left" valign="middle" class="treemenutd"><a href="home.php?request=stat_group&fpurpose=edit&group_id=<?=$_REQUEST['group_id']?>&records_per_page=<?=$records_per_page?>&search_name=<?=$_REQUEST['search_name']?>&start=<?=$start?>&pg=<?=$pg?>&sort_by<?=$sort_by?>&sort_order=<?=$sort_order?>">Edit Static Page Group</a> >> List Product Categories</td>
		</tr>
		<tr>
		...
		<tr>
		  ...
		  <td width="350" align="center" class="listeditd">
		  	<?php 
			...
			?></td>
		  <td width="338" align="right" class="listeditd"> <?php
				if ($db->num_rows($ret_qry))
				{
			?>
		  Assign to Page Group&nbsp;
          <?php
			  	$top_group_arr = array();
				$sql_group = "SELECT catgroup_id,catgroup_name,catgroup_hide FROM product_categorygroup WHERE sites_site_id=$ecom_siteid 
								ORDER BY catgroup_name";
			  ...
			  ?>
          <input name="assign_catgroup" type="button" class="red" id="assign_catgroup" value="Assign" onclick="call_ajax_assignpagegroup('pagegroup_assign','<?php echo $_REQUEST['search_name']?>','<?php echo $pass_sort_by?>','<?php echo $pass_sort_order?>','<?php echo $pass_records_per_page?>','<?php echo $pass_start?>','<?php echo $pass_pg?>',<?=$group_id?>)" />
