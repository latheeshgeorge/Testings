...
$sql="SELECT newsletter_id, newsletter_template_id, newsletter_title,preview_title, preview_contents, newsletter_contents,newsletter_createdate,newsletter_lastupdate 
			FROM newsletters 
				WHERE sites_site_id=$ecom_siteid AND newsletter_id=".$newsletter_id. " AND sites_site_id='".$ecom_siteid."'";
$res=$db->query($sql);
...
<form name='frmEditNewsletter' action='home.php?request=newsletter'  method="post" >
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td colspan="5" align="left" valign="middle" class="treemenutd"><div class="treemenutd_div"><a href="home.php?request=newsletter&sort_by=<?=$_REQUEST['pass_sort_by']?>&sort_order=<?=$_REQUEST['pass_sort_order']?>&records_per_page=<?=$_REQUEST['pass_records_per_page']?>&search_name=<?=$_REQUEST['pass_search_name']?>&start=<?=$_REQUEST['pass_start']?>&pg=<?=$_REQUEST['pass_pg']?>">List Newsletter</a><span> Edit News lettter</span></div></td>
        </tr>
        <tr>
		...
		<tr>
          <td colspan="5" align="left" valign="middle" > <?PHP echo newsletter_tabs('main_tab_td',$newsletter_id) ?></td>
        </tr>
		<?php 
		if($alert)
		{			
		...
		?>
		 <tr id="tmpcheck">
          ...
          <td align="left" valign="middle" class="tdcolorgray" ><?PHP echo $templatename; ?>&nbsp;<a href="#" onmouseover ="ddrivetip('<?=get_help_messages('EDIT_NEWSLETTER_SLECT_TEMPLATE')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a></td>
		  ...
		  <input type="checkbox" name="chk_temp" value="1" onclick="temp_check(document.frmEditNewsletter)" />	<a href="#" onmouseover ="ddrivetip('<?=get_help_messages('EDIT_NEWSLETTER_CHANGE_TEMPLATE')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a></td>
	    </tr>
		<?  
		}
