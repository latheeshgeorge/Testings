...
else // case if product category group exists
{
	...
	$pass_cat_id=($_REQUEST['pass_cat_id']?$_REQUEST['pass_cat_id']:'0');
	$help_msg = get_help_messages('ADD_PROD_CAT_GROUP_SELCAT_MESS1');
	...
	$query_string = "request=prod_cat&fpurpose=subcategoryAssign";
	...
	$sql_assigned="SELECT category_id FROM product_categories WHERE sites_site_id=$ecom_siteid AND parent_id=".$pass_cat_id;
	...
	function call_save_selected(cname,sortby,sortorder,recs,start,pg,group_id) 
    {
		...
		else
		{
			...
			{
				show_processing();
				document.frm_prodcat.fpurpose.value='save_subcategoryAssign';
				document.frm_prodcat.submit();
			}	
		}	
	...
	<input type="hidden" name="fpurpose" value="subcategoryAssign" />
...
?>
	<table border="0" cellpadding="0" cellspacing="0" class="maintable" width="100%">
		...
		<tr>
		  <td colspan="<?php echo $colspan?>" align="right" valign="middle" class="listeditd"> <?php 
				if ($db->num_rows($ret_qry))
				{
