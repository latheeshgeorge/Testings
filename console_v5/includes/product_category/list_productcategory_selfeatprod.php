...
$page_type='Products';
$help_msg =get_help_messages('EDIT_CAT_ASSFEATPROD_MAIN_SEL');
...
$sql_assigned_products = "SELECT products_product_id FROM category_featuredprod_map where product_categories_category_id =".$group_id;
...
//#Search terms.
$query_string .= "request=prod_cat&fpurpose=featprod_sel";
...
$where_conditions = "WHERE p.sites_site_id=$ecom_siteid AND p.product_id=pcm.products_product_id AND product_categories_category_id =$category_id";
...
$where_conditions = "WHERE p.sites_site_id=$ecom_siteid ";
}
if($assigned_products_str!=''){
...
{ 
	...
	if (atleastone==0) 
	{
		alert('Please select the Products to assign to the current Category as featured products');
		return false;
	}
	else
	{
		if(confirm('Assign Selected Product(s) to current Category as featured products?'))
		{
				show_processing();
				...
				document.frmlistProducts.fpurpose.value='assign_featproducts';
...
<input type="hidden" name="fpurpose" value="featprod_sel" />
...
?>
  <table border="0" cellpadding="0" cellspacing="0" class="maintable" width="100%">
    <tr>
      <td colspan="3" align="left" valign="middle" class="treemenutd"><div class="treemenutd_div"><a href="home.php?request=prod_cat&catname=<?=$_REQUEST['pass_catname']?>&catgroupid=<?php echo $_REQUEST['pass_catgroupid']?>&parentid=<?php echo $_REQUEST['pass_parentid']?>&sort_by=<?=$_REQUEST['pass_sort_by']?>&sort_order=<?=$_REQUEST['pass_sort_order']?>&records_per_page=<?=$_REQUEST['pass_records_per_page']?>&start=<?=$_REQUEST['pass_start']?>&pg=<?=$_REQUEST['pass_pg']?>&curtab=featured_tab_td">List Product Categories</a> <a href="home.php?request=prod_cat&fpurpose=edit&checkbox[0]=<?php echo $pass_cat_id?>&catname=<?=$_REQUEST['pass_catname']?>&catgroupid=<?php echo $_REQUEST['pass_catgroupid']?>&parentid=<?php echo $_REQUEST['pass_parentid']?>&sort_by=<?=$_REQUEST['pass_sort_by']?>&sort_order=<?=$_REQUEST['pass_sort_order']?>&records_per_page=<?=$_REQUEST['pass_records_per_page']?>&start=<?=$_REQUEST['pass_start']?>&pg=<?=$_REQUEST['pass_pg']?>&curtab=featured_tab_td"> Edit Product Category</a><span> Assign Featured Products for '<? echo $show_catname;?>'</span></div></td>
    </tr>
	 <tr>
	...
	<tr>
      ...
      <table width="18%" border="0" cellpadding="0" cellspacing="0" class="sorttableright">
        ...
        <tr>
          ...
          <td align="left">&nbsp;&nbsp;&nbsp;&nbsp;<input name="button5" type="submit" class="red" id="button5" value="Go" /> <?php /*<a href="#" onmouseover ="ddrivetip('<?=get_help_messages('LIST_PROD_ASS_ADVERT_GO')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a>*/?></td>
        </tr>
      </table>
	  </div>
	  </td>
    ...
    <tr>
	  ...
	  ?>
		...
		<input name="assign_products" type="button" class="red" id="assign_products" value="Assign Products" onclick="return assignproducts();" />
		  <?php /*<a href="#" onmouseover ="ddrivetip('<?=get_help_messages('LIST_PROD_ASS_ADVERT_ASSIGN')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a> */ ?>
		<?
		}
		?>
   	   </td>
