<?php
	...
	{
		global $db,$ecom_siteid,$ecom_hostname,$ecom_themeid,$ecom_site_mobile_api,$ecom_enable_searchrefine_category;
		if($editid)
		{
	...
	<td align="left" valign="top" class="tdcolorgray_url">:<a href="<?php url_category($row_category['category_id'],$row_category['category_name'],-1);?>" title="Click to view the category in website" target="_blank"><?php url_category($row_category['category_id'],$row_category['category_name'],-1);?></a></td>
	</tr>
	</table>
	</div>
	</td>
...
<?php
	...
	{
		global $db,$ecom_siteid,$ecom_hostname,$ecom_themeid,$ecom_gridenable;		
		...
		if($editid)
		{
			...
			if($db->num_rows($ret_category))
			{
				$row_category = $db->fetch_array($ret_category);//echo "<pre>";print_r($row_category);
			}
...
Product  Bonus Points </td>
		...
		$sql_site = "SELECT site_custgroup_special_display_enable FROM sites WHERE site_id = $ecom_siteid LIMIT 1";
				...
				if($row_site['site_custgroup_special_display_enable']==1)
			{
				?>
         <tr>
		  <td align="left" valign="middle" class="tdcolorgray">Display to guests?</td>
		  <td align="left" valign="middle" class="tdcolorgray"><input type="checkbox" name="display_to_guest" id="display_to_guest" value="1" <?php echo ($row_category['display_to_guest']==1)?'checked="checked"':''?>  /></td>
         </tr>
        <?php
	        }
		  ...
		  ?>
		<tr>
		  <td align="left" valign="middle" class="tdcolorgray">Enable Grid Display?</td>
		  <td align="left" valign="middle" class="tdcolorgray"><input type="checkbox" name="enable_grid_display" id="enable_grid_display" value="1" <?php echo ($row_category['enable_grid_display']==1)?'checked="checked"':''?> onclick="javascript: showGroupList();" /></td>
		  <td colspan="2" align="left" valign="middle" class="tdcolorgray">
          
          <?php
		  	if($row_category['enable_grid_display']==1)
			{
		  ...
		  ?>
          <div id="show_group_list" style=" <?php echo $dispStyle; ?>">
          Select Product Variable Group &nbsp;
            <select name="product_variables_group_id" >
         ...
         <?php
			if ($db->num_rows($ret_prdt_var_grp))
			{
				...
				{
					if($row_prdt_var_grp['var_group_id'] == $row_category['product_variables_group_id'])
					{	$selectGroupId	=	' selected="selected"';	}
					else
					{	$selectGroupId	=	'';	}
					echo '<option value="'.$row_prdt_var_grp['var_group_id'].'" '.$selectGroupId.'>'.$row_prdt_var_grp['var_group_name'].'</option>';
				}
			}
		 ?>
          </select>
          <br /><br />          
           ...
           Number of columns &nbsp;
           <input type="text" name="grid_column_cnt" id="grid_column_cnt" value="<?php echo $row_category['grid_column_cnt'];?>" />
          </div></td>
		  </tr>
          <?php
			}
...
?>
	...
	function list_googlebase_cate($faction)
	{
		...
		$sql_grp = "SELECT google_taxonomy_id,google_taxonomy_keyword,is_active FROM google_productcategory_taxonomy ORDER BY google_taxonomy_keyword";
		$ret_grp = $db->query($sql_grp);
		
?>		<div>
		<span><strong>Select from the list</strong></span>
		<select name="<?php echo $selName;?>" <?php echo $enable_script;?> style="width:700px">
			<option value="0">--- Select ---</option>
<?php	if($db->num_rows($ret_grp))
		{
			while ($row_grp = $db->fetch_array($ret_grp))
			{
				if($row_grp['is_active']==1)
				{
					$exts = ' (*)';
				}	
				else
...
?>			<option value="<?php echo $row_grp['google_taxonomy_id'];?>"><?php echo $row_grp['google_taxonomy_keyword'].$exts;?></option>
<?php		}
		}
?>		</select>
</div>
<div class="assign_catclass"><a href="#moveto_googlecategory_div" style="text-decoration:none">
<input class="red" onclick="javascript:show_googlecategorypopup()" value="Lookup Category" name="Addmorecat_tab" id="Addmorecat_tab" type="button">
</a>
...
<?php
	...
	function show_featuredprod_list($cat_id,$alert='')
	{
		global $db,$ecom_siteid ;
		// Get the list of shops under current shop
		$sql_prod = "SELECT b.map_id,a.product_id,a.product_name,a.product_hide,b.prod_order  
					FROM products a,category_featuredprod_map b 
					WHERE a.sites_site_id=$ecom_siteid AND 
					b.product_categories_category_id = $cat_id 
					AND a.product_id = b.products_product_id 	  
					ORDER BY b.prod_order";
		$ret_prod = $db->query($sql_prod);
	  ?>
	  <div class="editarea_div">
		<table width="100%" cellpadding="0" cellspacing="1" border="0" class="fieldtable">
		<tr>
		  <td align="left" colspan="7" class="helpmsg_divcls">
		 <?php echo get_help_messages('EDIT_CAT_ASSFEATPROD_MAIN');?>	
		  </td>
		  </tr>
		  <tr>
		  <td align="right" colspan="5" class="tdcolorgray_buttons">
			<input name="Assignmore" type="button" class="red" id="Assignmore" value="Assign More" onclick="normal_assign_featprod('<?php echo $_REQUEST['catname']?>','<?php echo $_REQUEST['catgroupid']?>','<?php echo $_REQUEST['parentid']?>','<?php echo $_REQUEST['sort_by']?>','<?php echo $_REQUEST['sort_order']?>','<?php echo $_REQUEST['records_per_page']?>','<?php echo $_REQUEST['start']?>','<?php echo $_REQUEST['pg']?>','<?php echo $cat_id?>');" />
			<a href="#" onmouseover ="ddrivetip('<?=get_help_messages('EDIT_CAT_ASSFEATPROD_SHOP_BRND')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.gif" width="17" height="13" border="0" /></a>&nbsp;&nbsp;
			<?php
			if ($db->num_rows($ret_prod))
			{
			?>
			<div id="featprod_unassign_div" class="unassign_div" >
			<input name="Unassign" type="button" class="red" id="Unassign" value="Unassign" onclick="call_ajax_deleteall('featprod','checkboxshop[]')" />
			<a href="#" onmouseover ="ddrivetip('<?=get_help_messages('EDIT_CAT_UNASSFEATPROD_SHOP_BRND')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.gif" width="17" height="13" border="0" /></a></div>	
			<?php
			}
			?>
			</td>
		...
		?>
				...
				<tr>
				<td width="5%"  align="center" class="<?php echo $cls?>"><input type="checkbox" name="checkboxshop[]" value="<?php echo $row_prod['map_id'];?>" /></td>
				<td width="5%" align="left" class="<?php echo $cls?>"><?php echo $cnt++?>.</td>
				<td class="<?php echo $cls?>" align="left"><a href="home.php?request=products&fpurpose=edit&checkbox[0]=<?php echo $row_prod['product_id'];?>" title="Edit Product" class="edittextlink"><?php echo stripslashes($row_prod['product_name']);?></a></td>
				<td class="<?php echo $cls?>" align="center"><input type="text" name="featprod_sort_<?php echo $row_prod['map_id']?>" id="featprod_sort_<?php echo $row_prod['map_id']?>" value="<?php echo $row_prod['prod_order']?>" size="3" style="text-align:center" /></td>
				<td class="<?php echo $cls?>" align="center"><?php echo ($row_prod['product_hide']=='Y')?'Yes':'No'?></td>
				</tr>
		<?php
			}
			if ($exists)
			...
			<tr>
			<td align="center" colspan="5" class="tdcolorgray_buttons">
			<input name="Saveorder" type="button" class="red" id="Saveorder" value="Save Order" onclick="call_save_order('featprod','checkboxshop[]')" />
			<a href="#" onmouseover ="ddrivetip('<?=get_help_messages('EDIT_SHOP_BRAND_SAVEORDER_FEATPRODSHOP_BRND_GROUP')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.gif" width="17" height="13" border="0" /></a>
			</td>
			</tr>
			<?
			}
		...
		?>
		   <tr>
			 <td colspan="<?php echo $colspan?>" align="center" valign="middle" class="norecordredtext_small">
						  <input type="hidden" name="shop_norec" id="shop_norec" value="1" />
						  No Featured Products Assigned for this category.
			 </td>
		  </tr>
		<?
		}
...
<?php
	...
	/* Search Refine variables tab starts here */
	function show_category_variables($cat_id,$alert='',$getArr)
	{
		global $db,$ecom_siteid,$ecom_enable_searchrefine_category;
		
		if($ecom_enable_searchrefine_category == 1)
		{
			$sql_enablevar = "SELECT enable_searchrefine FROM product_categories WHERE sites_site_id = $ecom_siteid AND category_id = $cat_id ";
			$ret_enablevar = $db->fetch_one_row($sql_enablevar);
		}
		
		
		// Get the list of variables under current category
		$sql_variables = "SELECT * FROM product_category_searchrefine_keyword WHERE sites_site_id = $ecom_siteid AND product_categories_category_id = $cat_id ORDER BY refine_order ASC";
		$ret_variables = $db->query($sql_variables);
	  ?>
      <div class="editarea_div">
		<table width="100%" cellpadding="0" cellspacing="1" border="0" class="fieldtable">
		<tr>
		  <td align="left" colspan="7" class="helpmsg_divcls">
		 <?php echo get_help_messages('EDIT_CATEG_VAR_SUBMSG')?>	
		  </td>
		  </tr>
          <?php
			if($ecom_enable_searchrefine_category == 1)
			{
				?>
          <tr>
				<td colspan="7" align="left" class="tdcolorgray">Enable Search Refine?			    
			    <input name="enable_searchrefine" type="checkbox" id="enable_searchrefine" value="1" <?php if($ret_enablevar['enable_searchrefine'] == 1) { echo ' checked="checked"';}?> />
            <input type="button" name="SaveSetting" value="Save Setting" class="red" onclick="save_varsetting('<?php echo $cat_id;?>')" /></td>
		  </tr>
          <?php
			}
		  ?>
		  <tr>
		  <td align="right" colspan="5" class="tdcolorgray_buttons">
			<input name="Add" type="button" class="red" id="Add" value="Add Search Refine Variable" onclick="add_variable('<?php echo $getArr['catname']?>','<?php echo $getArr['catgroupid']?>','<?php echo $getArr['parentid']?>','<?php echo $getArr['sort_by']?>','<?php echo $getArr['sort_order']?>','<?php echo $getArr['records_per_page']?>','<?php echo $getArr['start']?>','<?php echo $getArr['pg']?>','<?php echo $cat_id?>');" />
			<a href="#" onmouseover ="ddrivetip('<?=get_help_messages('EDIT_CAT_ADD_VAR')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.gif" width="17" height="13" border="0" /></a>&nbsp;&nbsp;
			<?php
			if ($db->num_rows($ret_variables))
			{
			?>
			<div id="shop_unassign_div" class="unassign_div" >
			<input name="Delete" type="button" class="red" id="Delete" value="Delete" onclick="call_ajax_deleteall('variables','checkboxvariables[]')" />
			<a href="#" onmouseover ="ddrivetip('<?=get_help_messages('EDIT_CAT_DELETE_VAR')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.gif" width="17" height="13" border="0" /></a></div>	
			<?php
			}
			?>		  </td>
	</tr>
		...
		if ($db->num_rows($ret_variables))
		{
			$table_headers = array('<img src="images/checkbox.gif" border="0" onclick="select_all(document.frmEditProductCategory,\'checkboxshop[]\')"/><img src="images/uncheckbox.gif" border="0" onclick="select_none(document.frmEditProductCategory,\'checkboxshop[]\')"/>','Slno.','Variable Name','Variable Type','Hidden');
			$header_positions=array('center','center','left','center','center');
			$colspan = count($table_headers);
			echo table_header($table_headers,$header_positions); 
			$cnt = 1;
		...
		?>
				...
				<tr>
				<td width="5%"  align="center" class="<?php echo $cls?>"><input type="checkbox" name="checkboxvariables[]" value="<?php echo $row_variables['refine_id'];?>" /></td>
				<td width="5%" align="left" class="<?php echo $cls?>"><?php echo $cnt++?>.</td>
				<td class="<?php echo $cls?>" align="left"><a href="home.php?request=prod_cat&fpurpose=edit_variables&checkbox[0]=<?php echo $cat_id;?>&cur_catid=<?php echo $cat_id;?>&catname=<?=$getArr['catname'];?>&parentid=<?=$getArr['parentid'];?>&catgroupid=<?=$getArr['catgroupid'];?>&start=<?php echo $getArr['start'];?>&pg=<?php echo $getArr['pg'];?>&records_per_page=<?php echo $getArr['records_per_page'];?>&sort_by=<?php echo $getArr['sort_by'];?>&sort_order=<?php echo $getArr['sort_order'];?>&varID=<?php echo $row_variables['refine_id'];?>&curtab=variables_tab_td" title="edit" class="edittextlink"><?php echo stripslashes($row_variables['refine_caption']);?></a></td>
				<td class="<?php echo $cls?>" align="center"><?php echo ($row_variables['refine_display_style']=='BOX')?'COLOUR':$row_variables['refine_display_style'];?></td>
				<td class="<?php echo $cls?>" align="center"><?php echo ($row_variables['refine_hidden']==1)?'Yes':'No'?></td>
				</tr>
		<?php
			}
...
<?php
	...
	/*function show_edit_category_variable($getArr,$alert='')
	{
		global $db,$ecom_siteid,$ecom_enable_searchrefine_category;
	}*/
	/* Search Refine variables tab ends here */
?>
