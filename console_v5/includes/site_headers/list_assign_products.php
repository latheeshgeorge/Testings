...
$table_name='products';
$page_type='Products';
$help_msg = get_help_messages('LIST_ASS_PROD_SITE_HEAD_MESS1');
$table_headers = array('<img src="images/checkbox.gif" border="0" onclick="select_all(document.frmlistProducts,\'checkbox[]\')"/><img src="images/uncheckbox.gif" border="0" onclick="select_none(document.frmlistProducts,\'checkbox[]\')"/>','Slno.','Product Name','Site Headers already assigned for the product','Category','Active');
...
/////////////////////////////////////////////////////////////////////////////////////
$query_string .= "request=site_headers&fpurpose=list_assign_products&sort_by=$sort_by&sort_order=$sort_order&records_per_page=$records_per_page&search_name=".$_REQUEST['search_name']."&start=$start&header_id=".$_REQUEST['header_id']."&header_title=".$_REQUEST['header_title']."&pass_sort_by=".$_REQUEST['pass_sort_by']."&pass_sort_order=".$_REQUEST['pass_sort_order']."&pass_records_per_page=".$_REQUEST['pass_records_per_page']."&pass_search_name=".$_REQUEST['pass_search_name']."&categoryid=".$_REQUEST['categoryid']."&pass_pg=".$_REQUEST['pass_pg']."&pass_start=".$_REQUEST['pass_start'];
...
{ 
	...
	if (atleastone==0) 
	{
		alert('Please select the Products to assign to the Site Headers');
		return false;
	}
	else
	{
		if(confirm('Assign Products to the Site Headers?'))
		{
				show_processing();
...
<form name="frmlistProducts" action="home.php?request=site_headers" method="post" >	
...
<input type="hidden" name="request" value="site_headers" />
  ...
  <table border="0" cellpadding="0" cellspacing="0" class="maintable" width="100%">
    <tr>
      <td colspan="2" align="left" valign="middle" class="treemenutd"><div class="treemenutd_div"><a href="home.php?request=site_headers&sort_by=<?=$_REQUEST['pass_sort_by']?>&sort_order=<?=$_REQUEST['pass_sort_order']?>&records_per_page=<?=$_REQUEST['pass_records_per_page']?>&search_name=<?=$_REQUEST['pass_search_name']?>&start=<?=$_REQUEST['pass_start']?>&pg=<?=$_REQUEST['pass_pg']?>&group_id=<?=$_REQUEST['group_id']?>">List Site Headers</a> <a href="home.php?request=site_headers&fpurpose=edit&header_id=<?=$_REQUEST['header_id']?>&sort_by=<?=$_REQUEST['sort_by']?>&sort_order=<?=$_REQUEST['sort_order']?>&records_per_page=<?=$_REQUEST['records_per_page']?>&search_name=<?=$_REQUEST['search_name']?>&start=<?=$_REQUEST['start']?>&pg=<?=$_REQUEST['pg']?>&pass_sort_by=<?=$_REQUEST['pass_sort_by']?>&pass_sort_order=<?=$_REQUEST['pass_sort_order']?>&pass_records_per_page=<?=$_REQUEST['pass_records_per_page']?>&pass_search_name=<?=$_REQUEST['pass_search_name']?>&pass_start=<?=$_REQUEST['pass_start']?>&pass_pg=<?=$_REQUEST['pass_pg']?>&curtab=prodmenu_tab_td">Edit Site Headers</a><span> Assign Products to the Site Headers : <b>  '<?=$row_header['header_title']?>' </b></span></div></td>
    </tr>
	 <tr>
      ...
      <td height="48" class="sorttd" colspan="2" >
	   <div class="editarea_div">
        ...
        <tr>
          ...
          <td width="24%" height="30" align="left" valign="middle"><?php
          ...
          <td width="7%" height="30" align="right" valign="middle"><input name="button5" type="submit" class="red" id="button5" value="Go" />
            <a href="#" onmouseover ="ddrivetip('<?=get_help_messages('LIST_ASSPROD_SITE_HEAD_GO')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a></td>
          </tr>	
      </table>
	  </div>
      </td>
    ...
    <tr>
	  ...
	  ?>
		<tr>
		  <td align="right" valign="middle" class="listeditd" colspan="6" >
		  <input name="assign_products" type="button" class="red" id="assign_products" value="Assign Products" onclick="return assignproducts();" />
		  <a href="#" onmouseover ="ddrivetip('<?=get_help_messages('LIST_ASSPROD_ASSPROD')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a>		  
		  </td>
		</tr>
		<?php
			}
	  ...
	  $sql_products = "SELECT product_id,product_name FROM $table_name $where_conditions ORDER BY $sort_by $sort_order LIMIT $start,$records_per_page ";
	   ...
	   ?>
        <tr >
		 ...
		 <?
		 $sql_header="SELECT header_title FROM site_headers a,header_display_product b WHERE b.products_product_id=".$row['product_id']." AND a.header_id=b.site_headers_header_id";
		 $res_header=$db->query($sql_header);
		 $num_header=$db->num_rows($res_header);
		  if($num_header) {
		  ?>
		  <select name="site_headers">
		  <?
		  while($row_header=$db->fetch_array($res_header))
		  {
		  	echo "<option value=$row_header[header_title]>$row_header[header_title]</option>";
		  }
		  ?>
		  </select>
		 <?
	...
	<?php
	  ...
	  ?>
		<tr>
		  <td align="right" valign="middle" class="listing_bottom_paging" colspan="6" ><?php paging_footer($query_string,$numcount,$pg,$pages,$page_type,$colspan);  ?>
		  </td>
		</tr>
		<?php
			}
