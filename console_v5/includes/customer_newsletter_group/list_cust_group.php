<?php
	/*#################################################################
	# Script Name 	: list_cust_group.php
	# Description 	: Page for listing Customer Group
	# Coded by 		: SKR
...
$table_name='customer_newsletter_group';
$page_type='Customer Newsletter Group';
$help_msg = get_help_messages('LIST_CUST_GROUP_MESS1');
$table_headers = array('<img src="images/checkbox.gif" border="0" onclick="select_all(document.frmlistCustomerGroup,\'checkbox[]\')"/><img src="images/uncheckbox.gif" border="0" onclick="select_none(document.frmlistCustomerGroup,\'checkbox[]\')"/>','Slno.','Group Name','No. of Customers','Hide'); //'Discount',
$header_positions=array('left','left','left','left','left','left');
$colspan = count($table_headers);

//#Search terms
$search_fields = array('custgroup_name');
foreach($search_fields as $v) {
	$query_string .= "$v=".$_REQUEST[$v]."&";#For passing searh terms to javascript for passing to different pages.
}
	
//#Sort
$sort_by = (!$_REQUEST['sort_by'])?'custgroup_name':$_REQUEST['sort_by'];
$sort_order = (!$_REQUEST['sort_order'])?'ASC':$_REQUEST['sort_order'];
$sort_options = array('custgroup_name' => 'Newsletter Group Name');
...
if($_REQUEST['search_name']) {
	$where_conditions .= "AND ( custgroup_name LIKE '%".add_slash($_REQUEST['search_name'])."%')";
}
...
/////////////////////////////////////////////////////////////////////////////////////
$query_string .= "sort_by=$sort_by&sort_order=$sort_order&request=cust_group&records_per_page=$records_per_page&search_name=".$_REQUEST['search_name']."&start=$start";
?>
...
function call_ajax_changestatus(search_name,sortby,sortorder,recs,start,pg)
{
	...
	else
	{
		...
		{
				show_processing();
				Handlewith_Ajax('services/customer_newsletter_group.php','fpurpose=change_hide&'+qrystr+'&group_ids='+group_ids);
		}	
	}	
}
...
function call_ajax_delete(search_name,sortby,sortorder,recs,start,pg)
{
	...
	else
	{
		...
		{
			show_processing();
			Handlewith_Ajax('services/customer_newsletter_group.php','fpurpose=delete&del_ids='+del_ids+'&'+qrystr);
		}	
	}	
}
...
<input type="hidden" name="pg" value="<?=$pg?>" />
  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="maintable">
    <tr>
      <td colspan="3" align="left" valign="middle" class="treemenutd"><div class="treemenutd_div"><span> List Customer Newsletter Groups</span></div></td>
    </tr>
	<tr>
	...
	<tr>
	  ...
	  <div class="sorttd_div">
        ...
        <tr>
          <td width="6%" height="30" align="left" valign="middle">Group Name </td>
          ...
          <td width="24%" height="30" align="right" valign="middle">&nbsp;
            <input name="button5" type="submit" class="red" id="button5" value="Go" />
            <a href="#" onmouseover ="ddrivetip('<?=get_help_messages('LIST_CUST_GROUP_GO')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a></td>
          </tr>
      </table>
	  </div>
	  </td>
    ...
    <tr>
      <td class="listeditd" align="left" valign="middle" colspan="3"><a href="home.php?sort_by=<?=$sort_by?>&sort_order=<?=$sort_order?>&request=cust_group&fpurpose=add&records_per_page=<?=$records_per_page?>&search_name=<?=$_REQUEST['search_name']?>&start=<?=$start?>&pg=<?=$pg?>" class="addlist" onclick="show_processing()">Add</a> 
	  <?
	  if($numcount)
	  {
	  ?>
	  ...
	  ?>
		...
		Change Status
		  ...
		  <a href="#" onmouseover ="ddrivetip('<?=get_help_messages('LIST_CUST_GROUP_CHSTATUS')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a>&nbsp;&nbsp;
		<?
		}
		?>
   	   </td>
	   ...
	   if($numcount)
	   {
	   $sql_user = "SELECT custgroup_id,custgroup_name,custgroup_active FROM $table_name $where_conditions ORDER BY $sort_by $sort_order LIMIT $start,$records_per_page ";
	   ...
	   ?>
        <tr >
          <td align="left" valign="middle" class="<?=$class_val;?>"  width="6%"><input name="checkbox[]" value="<? echo $row['custgroup_id']?>" type="checkbox"></td>
		  <td align="left" valign="middle" class="<?=$class_val;?>"  width="2%"><?php echo $srno++?></td>
          <td align="left" valign="middle" class="<?=$class_val;?>"><a href="home.php?request=cust_group&fpurpose=edit&checkbox[0]=<?php echo $row['custgroup_id']?>&search_name=<?php echo $_REQUEST['search_name']?>&start=<?php echo $start?>&pg=<?php echo $pg?>&records_per_page=<?php echo $records_per_page?>&sort_by=<?=$_REQUEST['sort_by']?>&sort_order=<?=$_REQUEST['sort_order']?>" title="<? echo $row['custgroup_name']?>" class="edittextlink" onclick="show_processing()"><? echo $row['custgroup_name']?></a></td>
          
		<!--  <td align="left" valign="middle" class="<?=$class_val;?>"><?php echo $row['custgroup_discount']; ?></td> -->
		<td align="left" valign="middle" class="<?=$class_val;?>">
		<?php 
			$sql_cnts = "SELECT count(map_id) as cnts FROM customer_newsletter_group_customers_map 
							WHERE 
								custgroup_id = ".$row['custgroup_id'];
			$ret_cnts = $db->query($sql_cnts);
			list($cust_cnts) = $db->fetch_array($ret_cnts);
		   echo $cust_cnts;
		   ?>
		
		</td>
		  <td align="left" valign="middle" class="<?=$class_val;?>"><?php echo ($row['custgroup_active'] == 0)?'Yes':'No'; ?></td>
	  ...
	  <tr>
				  <td align="center" valign="middle" class="norecordredtext" colspan="5">
				  	No Customer Group exists.				  </td>
		  </tr>
		<?
		}
		?>	
		<tr>
      <td class="listeditd" align="right" valign="middle" colspan="3"><a href="home.php?sort_by=<?=$sort_by?>&sort_order=<?=$sort_order?>&request=cust_group&fpurpose=add&records_per_page=<?=$records_per_page?>&search_name=<?=$_REQUEST['search_name']?>&start=<?=$start?>&pg=<?=$pg?>" class="addlist" onclick="show_processing()">Add</a> 
	  <?
	  if($numcount)
	  {
	  ?>
