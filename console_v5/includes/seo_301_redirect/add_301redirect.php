...
$help_msg = get_help_messages('ADD_REDIRECT_MESS1');
?>	
...
<form name='frmaddredirect' action='home.php?request=seo_301redirect' method="post" onsubmit="return valform(this);">
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td colspan="2" align="left" valign="middle" class="treemenutd"><div class="treemenutd_div"><a href="home.php?request=seo_301redirect&sort_by=<?=$_REQUEST['sort_by']?>&sort_order=<?=$_REQUEST['sort_order']?>&records_per_page=<?=$_REQUEST['records_per_page']?>&redirect_old_url=<?=$_REQUEST['redirect_old_url']?>&redirect_new_url=<?=$_REQUEST['redirect_new_url']?>&srch_access_startdate=<?=$_REQUEST['srch_access_startdate']?>&srch_access_enddate=<?=$_REQUEST['srch_access_enddate']?>&start=<?=$_REQUEST['start']?>&pg=<?=$_REQUEST['pg']?>">List 301 Redirect URLs</a><span> Add 301 Redirect URLs</span></div></td>
        </tr>
		<tr>
        ...
        <tr>
          ...
          <td align="left" valign="middle" class="tdcolorgray"><input name="old_url" type="text" class="input" id="old_url" value="<? echo stripslashes($_REQUEST['old_url'])?>" size="80" />&nbsp;&nbsp; (e.g. Address = "http://example.com/oldURL.html)</td>
        </tr>
        <tr>
          <td align="left" valign="top" class="tdcolorgray" >New URL<span class="redtext">*</span></td>
          <td align="left" valign="middle" class="tdcolorgray"><input name="new_url" type="text" class="input" id="new_url" value="<? echo stripslashes($_REQUEST['new_url'])?>" size="80" />&nbsp;&nbsp; (e.g. Address = "http://example.com/newURL.html)</td>
        </tr>
        <tr>
