...
$tab_id=($_REQUEST['pass_tab_id']?$_REQUEST['pass_tab_id']:'0');

$tabale = "product_common_tabs";
$where  = "common_tab_id=".$tab_id;
...
$query_string = "request=common_prod_tab&fpurpose=ProdAssign";
...
$query_string .="&pass_tab_id=".$tab_id."";	
//#Sort
...
$sql_assigned="SELECT products_product_id FROM product_tabs WHERE  product_common_tabs_common_tab_id=".$tab_id;
...
<input type="hidden" name="pass_tab_id" value="<?=$tab_id?>" />
...
<table border="0" cellpadding="0" cellspacing="0" class="maintable" width="100%">
    <tr>
      <td colspan="2" align="left" valign="middle" class="treemenutd"><div class="treemenutd_div"><a href="home.php?request=common_prod_tab&search_name=<?=$_REQUEST['pass_searchname']?>&sort_by=<?=$_REQUEST['pass_sortby']?>&sort_order=<?=$_REQUEST['pass_sortorder']?>&records_per_page=<?=$_REQUEST['pass_records_per_page']?>&start=<?=$_REQUEST['pass_start']?>&pg=<?=$_REQUEST['pass_pg']?>">List Common Product Tabs </a> <a href="home.php?request=common_prod_tab&fpurpose=edit&checkbox[0]=<?=$tab_id?>&pass_search_name=<?=$_REQUEST['pass_searchname']?>&pass_sort_by=<?=$_REQUEST['pass_sortby']?>&pass_sort_order=<?=$_REQUEST['pass_sortorder']?>&pass_records_per_page=<?=$_REQUEST['pass_records_per_page']?>&pass_start=<?=$_REQUEST['pass_start']?>&pass_pg=<?=$_REQUEST['pass_pg']?>&curtab=products_tab_td">Edit Common Product Tabs </a><span> Assign Products with '<? echo $row_tab['tab_title'];?>'</span></div></td>
    </tr>
	<tr>
    ...
    <tr>
	  ...
	  <tr>
      <td valign="middle" colspan="<?php echo $colspan?>" align="right" class="listeditd">
