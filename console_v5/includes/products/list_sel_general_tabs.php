<?php
	/*#################################################################
	# Script Name 	: list_sel_general_tabs.php
	# Description 	: Page for listing product general tabs
	# Coded by 		: Sny
...
$sql_linkexist = "SELECT product_common_tabs_common_tab_id FROM product_tabs WHERE products_product_id=".$_REQUEST['checkbox'][0];
$ret_linkexist = $db->query($sql_linkexist);
if ($db->num_rows($ret_linkexist))
{
	while ($row_linkexist = $db->fetch_array($ret_linkexist))
	{
		$ext_arr[] = $row_linkexist['product_common_tabs_common_tab_id'];
	}
}
//Define constants for this page
$table_name='product_common_tabs';
$page_type='Product Common Tabs';
$help_msg = get_help_messages('EDIT_PROD_LINKED_ASSMORE_TAB');
$help_msg = str_replace("[productname]",$showprodname,$help_msg);
$table_headers = array('<img src="images/checkbox.gif" border="0" onclick="select_all(document.frm_gentab_link,\'checkbox_link[]\')"/><img src="images/uncheckbox.gif" border="0" onclick="select_none(document.frm_gentab_link,\'checkbox_link[]\')"/>','Slno.','Common Tab Title','Hidden');
...
$query_string = "request=products&fpurpose=add_prodgentab&prod_dontsave=1&checkbox[0]=".$_REQUEST['checkbox'][0]."&curtab=".$_REQUEST['curtab']."&records_per_page_link=".$_REQUEST['records_per_page_link']."&pss_records_per_page=".$_REQUEST['pss_records_per_page']."&pss_pg=".$_REQUEST['pss_pg']."&pss_start=".$_REQUEST['pss_start'];
foreach($search_fields as $v) {
	$query_string .= "&$v=".$_REQUEST[$v]."";//#For passing searh terms to javascript for passing to different pages.
}
	
//#Sort
$sort_by_link = (!$_REQUEST['sort_by_link'])?'tab_title':$_REQUEST['sort_by_link'];
...
//#Search Options
$where_conditions = "WHERE sites_site_id=$ecom_siteid AND common_tab_id NOT IN (".implode(",",$ext_arr).") ";

// Product Name Condition
if($_REQUEST['gen_title'])
{
	$where_conditions .= " AND ( tab_title LIKE '%".add_slash($_REQUEST['gen_title'])."%') ";
}
...
$sql_qry = "SELECT * FROM product_common_tabs $where_conditions ORDER BY $sort_by_link $sort_order_link LIMIT $start,$records_per_page ";
...
function val_form()
{
	...
	if (atleast==false)
	{
		alert('Select the tabs(s) to be linked');
		return false;
	}
	else
	{
		if(confirm('Are you sure you want to assign selected Common Product Tabs to current product?'))
		{
			document.frm_gentab_link.fpurpose.value='assig_prodgentab';
...
<input type="hidden" name="fpurpose" value="add_prodgentab" />
...
<div class="sorttd_div">
        ...
        <tr>
		  ...
		  <table width="100%" border="0" cellspacing="1" cellpadding="1">
            <tr>
              ...
              <td width="32%" align="left"><input name="gen_title" type="text" class="textfeild" id="gen_title" value="<?php echo $_REQUEST['gen_title']?>" /></td>
              <td width="64%" align="left"><table width="100%"  border="0" cellpadding="0" cellspacing="0">
                <tr>
                  <td width="65%" align="left"><div align="right">Show <input name="records_per_page_link" type="text" class="textfeild" id="records_per_page_link" size="3" maxlength="3" value="<?php echo $records_per_page?>" />
                  ...
                  <td width="35%" align="right"><input name="Search_go" type="submit" class="red" id="Search_go" value="Go" onclick="document.frm_gentab_link.search_click.value=1" />
                    <a href="#" onmouseover ="ddrivetip('<?=get_help_messages('EDIT_PROD_LINKED_ASSGO_TAB')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a></td>
                </tr>
              </table></td>
              </tr>
          </table>            </td>
    ...
    <tr>
	 ...
	 ?>
				  ...
				  <input type="checkbox" name="checkbox_link[]" id="checkbox_link[]" value="<?php echo $row_qry['common_tab_id']?>" /></td>
				  <td align="left" valign="middle" class="<?php echo $cls?>" width="1%"><?php echo $srno++?></td>
				  <td align="left" valign="middle" class="<?php echo $cls?>" width="50%"><a href="home.php?request=common_prod_tab&fpurpose=edit&checkbox[0]=<?php echo $row_qry['common_tab_id']?>&productname=<?php echo $_REQUEST['productname']?>&manufactureid=<?php echo $_REQUEST['manufactureid']?>&categoryid=<?php echo $_REQUEST['categoryid']?>&vendorid=<?php echo $_REQUEST['vendorid']?>&rprice_from=<?php echo $_REQUEST['rprice_from']?>&rprice_to=<?php echo $_REQUEST['rprice_to']?>&cprice_from=<?php echo $_REQUEST['cprice_from']?>&cprice_to=<?php echo $_REQUEST['cprice_to']?>&discount=<?php echo $_REQUEST['discount']?>&discountas=<?php echo $_REQUEST['discountas']?>&bulkdiscount=<?php echo $_REQUEST['bulkdiscount']?>&stockatleast=<?php echo $_REQUEST['stockatleast']?>&preorder=<?php echo $_REQUEST['preorder']?>&prodhidden=<?php echo $_REQUEST['prodhidden']?>&start=<?php echo $start?>&pg=<?php echo $pg?>&records_per_page=<?php echo $records_per_page?>" title="edit" class="edittextlink"><?php echo stripslashes($row_qry['tab_title'])?></a></td>
				  <td align="center" valign="middle" class="<?php echo $cls?>">
				  	<?php
				  		echo ($row_qry['tab_hide']==1)?'Yes':'No';	
					?>
					</td>
				</tr>
	<?php
	...
	?>	
			<tr>
				<td align="center" valign="middle" class="norecordredtext" colspan="<?php echo $colspan?>">
				  	No unassigned Common Product Tabs found.
				</td>
			</tr>	  
	<?php
		}
