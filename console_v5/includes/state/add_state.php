<?php
	/*#################################################################
	# Script Name 	: add_state.php
	# Description 	: Page for adding Site State
	# Coded by 		: SKR
...
$page_type = 'State';
$help_msg = get_help_messages('ADD_STATE_MESS1');
...
<form name='frmAddUser' action='home.php?request=general_settings_state' method="post" onsubmit="return valform(this);">
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td colspan="2" align="left" valign="middle" class="treemenutd"><div class="treemenutd_div"><a href="home.php?request=general_settings">General Settings</a>  <a href="home.php?request=general_settings_state&sort_by=<?=$_REQUEST['sort_by']?>&sort_order=<?=$_REQUEST['sort_order']?>&records_per_page=<?=$_REQUEST['records_per_page']?>&search_name=<?=$_REQUEST['search_name']?>&start=<?=$_REQUEST['start']?>&pg=<?=$_REQUEST['pg']?>">List State</a><span> Add State</span></div></td>
        </tr>
       <tr>
		...
		echo generateselectbox('general_settings_site_country_country_id',$countrylist,$_REQUEST['general_settings_site_country_country_id']); ?>
        ...
        <tr>
          ...
          <td width="85%" align="left" valign="middle" class="tdcolorgray">
		  <input class="input" type="text" name="state_name" value="<?=$_REQUEST['state_name']?>"  />
		  </td>
        </tr>
       
        <tr>
          <td align="left" valign="middle" class="tdcolorgray" >Active</td>
          <td align="left" valign="middle" class="tdcolorgray"><input type="radio" name="state_hide" value="1" <? if($_REQUEST['state_hide']==1) echo "checked"; ?> />Yes<input type="radio" name="state_hide" value="0" <? if($_REQUEST['state_hide']==0) echo "checked"; ?> />No
		  &nbsp;<a href="#" onmouseover ="ddrivetip('<?=get_help_messages('ADD_STATE_HIDE')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a></td>
        </tr>
		</table>
		</div>
		</td>
