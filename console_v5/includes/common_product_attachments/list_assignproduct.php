<?php
	/*#################################################################
	# Script Name 	: list_assignproduct.php
...
$table_name='products';
$page_type='Products';
$help_msg = get_help_messages('LIST_PROD_WHEN_VIEW_COM_ATT');
$attach_id=($_REQUEST['pass_attach_id']?$_REQUEST['pass_attach_id']:'0');

$tabale = "product_common_attachments";
$where  = "common_attachment_id=".$attach_id;
...
//#Search terms
$search_fields = array('productname','manufactureid','categoryid','vendorid','rprice_from','rprice_to','cprice_from','cprice_to','discount','discountas','bulkdiscount','stockatleast','preorder','stock_from','stock_to','prodhidden','sort_by','sort_order');

$query_string = "request=common_prod_attachment&fpurpose=ProdAssign";
...
$query_string .="&pass_attach_id=".$attach_id."";	
//#Sort
...
$sql_assigned="SELECT products_product_id FROM product_attachments WHERE  product_common_attachments_common_attachment_id=".$attach_id;
...
function call_save_selected(cname,sortby,sortorder,recs,start,pg,group_id) 
{
	...
	else
	{
		...
		{
			show_processing();
			document.frm_products.fpurpose.value='save_ProdAssign';
...
<input type="hidden" name="fpurpose" value="ProdAssign" />
<input type="hidden" name="search_click" value="" />
<input type="hidden" name="pass_attach_id" value="<?=$attach_id?>" />
...
<? $sql_attachment="SELECT attachment_title 
				FROM 
				...
				LIMIT 
					1";
$ret_attachment = $db->query($sql_attachment);
$row_attachment = $db->fetch_array($ret_attachment);
?>


<table border="0" cellpadding="0" cellspacing="0" class="maintable" width="100%">
    <tr>
      <td colspan="2" align="left" valign="middle" class="treemenutd"><div class="treemenutd_div"><a href="home.php?request=common_prod_attachment&search_name=<?=$_REQUEST['pass_searchname']?>&sort_by=<?=$_REQUEST['pass_sortby']?>&sort_order=<?=$_REQUEST['pass_sortorder']?>&records_per_page=<?=$_REQUEST['pass_records_per_page']?>&start=<?=$_REQUEST['pass_start']?>&pg=<?=$_REQUEST['pass_pg']?>">List Common Product Attachments </a> <a href="home.php?request=common_prod_attachment&fpurpose=edit&checkbox[0]=<?=$attach_id?>&search_name=<?=$_REQUEST['pass_searchname']?>&sort_by=<?=$_REQUEST['pass_sortby']?>&sort_order=<?=$_REQUEST['pass_sortorder']?>&records_per_page=<?=$_REQUEST['pass_records_per_page']?>&start=<?=$_REQUEST['pass_start']?>&pg=<?=$_REQUEST['pass_pg']?>&curtab=products_tab_td">Edit Common Product Attachment </a><span> Assign Products with '<? echo $row_attachment['attachment_title'];?>'</span></div></td>
    </tr>
	<tr>
		  ...
		  <table width="100%" height="56" border="0" cellpadding="0" cellspacing="0">
            ...
            <tr>
              ...
              <td align="left"><input name="Search_go" type="submit" class="red" id="Search_go" value="Go" onclick="document.frm_products.search_click.value=1" />
                <a href="#" onmouseover ="ddrivetip('<?=get_help_messages('LIST_PROD_WHEN_VIEW_GO')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a></td>
            </tr>
          </table>		  </td>
        </tr>
      </table></div>      </td>
    ...
    <tr>
	  ...
	  &nbsp;<a href="#" onmouseover ="ddrivetip('<?=get_help_messages('LIST_PROD_WHEN_VIEW_ASSIGN_COMM_ATT')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a></td>
    </tr>
	 <?php  
