...
if(!$_REQUEST['layoutcode'])
{
	...
	else
	{
		...
		$sql_lay = "SELECT layout_code FROM themes_layouts WHERE themes_theme_id = $ecom_themeid  ORDER BY layout_order LIMIT 1";
...
$sql_layouts = "SELECT layout_id,layout_code,layout_name,layout_positions FROM themes_layouts WHERE themes_theme_id=$ecom_themeid AND layout_code='$layoutcode'";
...
$sql_theme = "SELECT themename,themes_support_allowed_positions FROM themes WHERE theme_id=$ecom_themeid";
...
function show_legends()
{
    ...
    {
?>
		<div class="themetdcontent_divcls_leg"> 
...
$dispfeat_arr['mod_shelfgroup']              = array  (
                                                'mode' => 'edit',
                                                'url'  => 'home.php?request=shelfgroup&fpurpose=edit&checkbox[0]=[rep_id]'
                                                );                                                
...
{
    echo "<a href=\"javascript:remove_me('".$parentId."','".$myId."','".$Msg."')\"><img src='images/delete_comp-c.png'  alt='Click to remove from this position' border='0' title='Click to remove from this position'/></a>";
}
        ...
        if(array_key_exists($module_name,$special_comp)) // case of special modules defined in table themes_layouts_feature_special_position_components
        {
            if($default_layout_pos[$feature_id]!='')
            {
                ...
                switch($default_layout_pos[$feature_id]['module'])
                {
                    ...
                    case 'mod_shelfgroup':
                        // Get the value of shelf_currentstyle from product_shelf table
                        $sql_qry = "SELECT shelf_showinall as showall  
                                        FROM 
                                            shelf_group 
                                        WHERE 
                                            id = $component_id 
		...
		*/
		...
		if(dragObj!=null){
			...
			if(iMouseDown && !lMouseState){
				...
				dragHelper.className = dragClass;
				dragHelper.style.background='red'

				// set the class on our helper DIV if necessary
				var dragClass = curTarget.getAttribute('dragClass');
				if(dragClass){
					dragHelper.firstChild.className = dragClass;
						dragHelper.firstChild.style.background='#f3f3f3';
					dragHelper.firstChild.style.display = 'block';
...
.DragDragBox {
	background:#C90 url(images/drag-arrow-a.png) 93% 50% no-repeat;
...
<form name='frmComponentPosition_move' id='frmComponentPosition_move' action='home.php?request=comp_pos' method="post" onsubmit="return valforms(this);" >
...
<table width="100%" border="0" cellpadding="0" cellspacing="1">
	...
	<tr>
      <td class="tdcolorgraynormal" align="left"> <div class="themetdcontent_divcls_top">
		Select Layout
	<?php
		
		$sql_lay = "SELECT layout_id,layout_code,layout_name,layout_positions FROM themes_layouts WHERE themes_theme_id=$ecom_themeid ORDER BY layout_order";
		$ret_lay = $db->query($sql_lay);
		if($db->num_rows($ret_lay))
		{
		?>
			<select name="cbo_sellayout" id="cbo_sellayout" onchange="window.location='home.php?request=comp_pos&layoutcode='+this.value">
			<?php
	...
	?>	<a href="#" style="cursor:pointer;" onmouseover="return overlib('<?=get_help_messages('MAN_COMPO_POS_LAYOUT')?>',100,VAUTO);" onmouseout="return nd();"><img src="images/helpicon.png" border="0" alt="" /></a></div>
	</td>
	</tr>
	
	<tr>
          <td align="right" valign="middle">
         <div class="themetdcontent_divcls_leg">  <?php show_legends();?></div>
        </td>
          </tr>
	  <tr>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
		  	<tr>
			<td width="19%" valign="top" align="left" class="themeleft" > <div class="themetdcontent_divcls_l">
			...
			<?php
				...
				$sql_sitemenu = "SELECT a.menu_id,b.feature_name,b.feature_ordering,b.feature_id,b.feature_modulename FROM site_menu a,features b WHERE 
								a.sites_site_id=$ecom_siteid AND b.feature_showincomponentposition=1 AND a.features_feature_id=b.feature_id 
								ORDER BY b.feature_ordering";
				$ret_sitemenu = $db->query($sql_sitemenu);
				?>
					<div dropobj="0" class="DragContainer_s" id="sitemenu" overclass="OverDragContainer">
					<?php
