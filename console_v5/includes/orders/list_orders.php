...
$table_name			= 'orders';
$page_type			= 'Orders';
$help_msg 			= get_help_messages('PROD_ORDER_MESS1');
...
$class_arr = array 
					(
						...
						'INPROGRESS'=>'orderlisting_inprogress',
...
if($_REQUEST['ord_status']!='CANCELLED' and $_REQUEST['ord_sort_by']=='order_cancelled_on')
{
	$_REQUEST['ord_sort_by'] = '';
}
...
//if($_REQUEST['ord_status']=='CANCELLED')
{
	//$sort_options 		= array('order_date' => 'Order Date','order_cancelled_on'=>'Cancelled Date','order_status'=>'Order Status','order_custfname'=>'Customer Name','order_custemail'=>'Email','order_pre_order'=>'Preorder','order_totalprice'=>'Order Total','order_refundamt'=>'Refund Amount');
...
if($_REQUEST['ord_status'])
{
	if ($_REQUEST['ord_status']==-1) // Done to handle the case of selected to show all orders
	{
		...
		$where_conditions .= " AND order_status NOT IN ('CANCELLED')"; // This is done to avoid the cancelled orders while asked to show all orders
	}
	...
	elseif($_REQUEST['ord_status']=='UNSHIPPED')
	{
		$where_conditions .= " AND order_status NOT IN ('DESPATCHED','CANCELLED')";
	}
...
/////////////////////////////////////////////////////////////////////////////////////
 ...
 $sql_qry = "SELECT 
				...
				order_cancelled_on,order_cancelled_from,order_custcompany,
				CASE order_status 
...
/*	var qrystr				= 'sort_by='+sortby+'&sort_order='+sortorder+'&records_per_page='+recs+'&start='+start+'&pg='+pg;      */
	...
	if (atleastone==0)
	{
		alert('Please select the order(s) to move to archive');
	}
	else
	{
			if(confirm('Are you sure want to move the selected order(s) to archive section?'))
			{
				show_processing();
				<?php /*waiting_for_despatch_return = 0;
				Handlewith_Ajax('services/orders.php','fpurpose=archive&archive_order_ids='+archive_order_ids+'&'+qrystr);*/?>
				document.frm_orders.fpurpose.value 		= 'archive';
				document.frm_orders.archive_order_ids.value = archive_order_ids;
				document.frm_orders.submit();
...
function handle_orderstatus_change()
{
	...
	switch(document.frm_orders.cbo_orderstatus.value)
	{
		...
		break;
		case 'INPROGRESS':
			msg = 'Are you sure you want to change the order status of selected orders to "In Progress"?';
		break;
		case 'ONHOLD':
		...
		break;
		case 'CANCELLED':
			msg = 'Are you sure  you want to change the order status of selected orders to "Cancelled"?\n\n No other operation will be allowed on cancelled orders.';
		break;	
		default:
...
<input type="hidden" name="archive_order_ids" value="" />
...
<table border="0" cellpadding="0" cellspacing="0" class="maintable">
    <tr>
      <td colspan="3" align="left" valign="middle" class="treemenutd"><div class="treemenutd_div"><span> List Orders</span></div></td>
    </tr>
	<tr>
...
<td align="left">
	...
	<td>
		...
		<td align="left">
			...
			<td width="5%"><?php
				...
				{
				?>
					<input type="button" name="old_bshop" value="View Old Site" onclick="handle_oldwebsite()" class="red" />
				<?php
				}
				?></td>
			</tr>
			...
			<td width="2%" align="left"><?php /*?><a href="javascript:show_calendar('frm_orders.ord_fromdate');" onMouseOver="window.status='Date Picker';return true;" onMouseOut="window.status='';return true;"><img src="images/show-calendar.gif" width="24" height="22" border="0" /></a><?php */?>
			<td width="4%" align="center">and</td>
    ...
    <tr>
		...
		<table width="100%" cellpadding="1" cellspacing="0" border="0">
			<tr>
			<td>
			<a href="#" onclick="edit_order(-1)" class="ord_list_cls" title="View Order Details">Details</a>
			</td>
			<td align="left">
			<a href="#" class="archivelist_new" onclick="go_archive('<?php echo $sort_by?>','<?php echo $sort_order?>','<?php echo $records_per_page?>','<?php echo $start?>','<?php echo $pg?>','<?=$_REQUEST['categoryid']?>')">Archive</a>
			</td>
			<td align="right">
				<?php
				if($ecom_siteid!=105 AND $ecom_siteid!=104)
				{
				?>
			<a href="#" onclick="delete_order()" class="deletelist">Delete</a>
			<?php
				}
			?>
			</td>
    ...
    ?>
      ...
      <td colspan="<?php echo round($colspan/2)?>" align="right" valign="middle" class="listeditd"><div id='show_legenddiv_big' onclick="handle_showlegenddiv()" title="Click here">More Actions<img src="images/right_arr.gif" /></div>	  </td>
    </tr>
	<tr id="more_operations_tr" style="display:none">
      <td colspan="<?php echo $colspan?>" class="listeditd" >
	  <div class="sorttd_div">
	  <table width="100%" cellpadding="1" cellspacing="0" border="0">
	  <tr>
	  <td align="left">
	  ...
	  echo generateselectbox('cbo_orderstatus',$orderstat_array,'','','');
	  ?>
	    <input type="button" name="change_ordstatus" id="change_ordstatus" value="Change Order Status" onclick="handle_orderstatus_change()" class="red" />	    &nbsp;&nbsp;
		 <?php
	  if($ecom_allpricewithtax==1)
	  {
	  ?>
	 	<div style="float:right"><input type="button" name="taxreport_Submit" value="Tax Report" class="red" onclick="window.location='home.php?request=order_tax_report'" />	</div>	
	  <?php
	  }
	  ?>
	  <div style="float:right"><input type="button" name="bulkdespatch_Submit" value="Bulk Despatch" class="red" onclick="handle_despatch_bulk()" />
	  <a href="#" onmouseover ="ddrivetip('<? echo "You can use this button to despatch mutiple orders.<br> Tick the orders to be despatched and then click on the <strong>Bulk Despatch</strong> button."?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a>
	  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	  <?php
	  	if($ecom_siteid == 66) // case of stoney brook
		{
		?>
			<input type="button" name="receipt_Submit" value="Print Receipt?" class="red" onclick="print_receipt()" />
		...
		{
	  ?>
	  		<input type="button" name="packingslip_Submit" value="Print Packing Slip?" class="red" onclick="print_packing()" />
        	<a href="#" onmouseover ="ddrivetip('<?=get_help_messages('LIST_PROD_ORDER_PACKING')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a>        
	  <?php
		}
	  ?>
	  <input type="button" name="printerfriendly_Submit" value="Printer Friendly?" class="red" onclick="printer_friendly()" />
        <a href="#" onmouseover ="ddrivetip('<?=get_help_messages('LIST_PROD_ORDER_FRIENDLY')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a>
	  </div>
	  <br /><br />Change Payment Status
      <?php
	  $orderpaystat_array = array	(
		...
		echo generateselectbox('cbo_paymentstatus',$orderpaystat_array,'','','');							
	  ?>
	  <input type="button" name="change_paystatus" id="change_paystatus" value="Change Payment Status" onclick="handle_orderpaystatus_change()" class="red" />	  &nbsp;&nbsp;
     <?php 
     if($ecom_siteid==105) 
     {
	 ?>
      <div style="float:right">
		<input type="button" value="Generate Sales Report" onclick="window.location='home.php?request=orders&fpurpose=salesrepshow'" class="red"/>  
	  </div>	  
     <?php
 }
     ?> 
	  ...
	  <table width="97%" border="0" align="center" cellpadding="1" cellspacing="0">
        <tr>
          ...
          <td align="left" width="12%">Pending Order</td>
           <td width="1%"><div style="width:10px; height:10px; border:solid 1px #550000;vertical-align:middle; text-align:center" class="orderlisting_inprogress">O</div></td>
           <td align="left" width="15%">In Progress</td>
          ...
          <td align="left" width="18%">Cancelled Orders</td>
        </tr>
      </table></td>
    </tr>
     <?php  
	 ...
	 ?>
        <tr style="cursor:pointer" id="mainordertd_<?php echo $row_order['order_id']?>">
		 ...
		 <a class="edittextlink" href="home.php?request=orders&fpurpose=ord_details&edit_id=<?php echo $row_order['order_id']?>&ser_ord_id=<?php echo $_REQUEST['ord_id']?>&ser_ord_status=<?php echo $_REQUEST['ord_status']?>&order_placed_from=<?php echo $_REQUEST['order_placed_from']?>&ser_ord_name=<?php echo $_REQUEST['ord_name']?>&ser_ord_email=<?php echo $_REQUEST['ord_email']?>&ser_ord_fromdate=<?php echo $_REQUEST['ord_fromdate']?>&ser_ord_todate=<?php echo $_REQUEST['ord_todate']?>&ser_ord_stores=<?php echo $_REQUEST['ord_stores']?>&ser_ord_sort_by=<?php echo $sort_by?>&ser_ord_sort_order=<?php echo $sort_order?>&ser_pg=<?php echo $pg?>&ser_start=<?php echo $start?>&ser_records_per_page=<?php echo $_REQUEST['records_per_page']?>"> <?php echo $row_order['order_id']?></a></td>
		  <td align="center" valign="middle" class="<?php echo $cls?>" width="8%" title="<?php echo $hover_title?>" onclick="edit_order('<?php echo $row_order['order_id']?>')">
		  <?php 
		  		echo dateFormat($row_order['order_date'],'datetime_break');
		  ?>		  </td>
          <td align="left" valign="middle" class="<?php echo $cls?>" width="15%" onclick="edit_order('<?php echo $row_order['order_id']?>')">
          ...
          if(trim($row_order['order_custcompany'])!='' and $ecom_siteid==105)
          {
			echo '<br/>( '.stripslashes($row_order['order_custcompany']).' )';
		  }
		  else
		  {
			  //echo '<br/><br/>';
		  ...
		  <td align="left" valign="middle" class="<?php echo $cls?>" width="15%" title="<?php echo $hover_title?>" onclick="edit_order('<?php echo $row_order['order_id']?>')">
		  <?php echo mask_emails(stripslashes($row_order['order_custemail']))?></td>
		  <?php /*?><td align="center" valign="middle" class="<?php echo $cls?>" width="1%" title="<?php echo $hover_title?>" onclick="edit_order('<?php echo $row_order['order_id']?>')">
		  <?php echo ucwords($row_order['order_pre_order'])?></td><?php */?>
		  <td align="left" valign="middle" class="<?php echo $cls?>" width="10%" onclick="edit_order('<?php echo $row_order['order_id']?>')">
		  <div id="list_orderstatus_div_<?php echo $row_order['order_id']?>">
		  <?php 
		  	echo getorderstatus_Name($row_order['order_status']);
		  	if($_REQUEST['ord_status']=='CANCELLED')
		  	{
				//if($row_order['order_cancelled_by']!=0)
				{
					echo "<br><center>".dateFormat($row_order['order_cancelled_on'],'datetime_break')."</center>";
				}	
			}
		  ?>
		  </div>
		   </td>
		  <td align="right" valign="middle" class="<?php echo $cls?>" width="8%" title="<?php echo $hover_title?>" onclick="edit_order('<?php echo $row_order['order_id']?>')">
		  ...
		  <td align="left" valign="middle" class="<?php echo $cls?>" width="7%" title="<?php echo $hover_title?>" onclick="edit_order('<?php echo $row_order['order_id']?>')">
