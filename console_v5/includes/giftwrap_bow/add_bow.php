...
$page_type = 'Bows';
$help_msg =get_help_messages('ADD_GIFTWRAP_BOW_MESS1');
...
<form name='frmAddUser' action='home.php?request=giftwrap_bows' method="post" onsubmit="return valform(this);">
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td colspan="2" align="left" valign="middle" class="treemenutd"><div class="treemenutd_div"><a href="home.php?request=giftwrap_bows&sort_by=<?=$_REQUEST['sort_by']?>&sort_order=<?=$_REQUEST['sort_order']?>&records_per_page=<?=$_REQUEST['records_per_page']?>&search_name=<?=$_REQUEST['search_name']?>&start=<?=$_REQUEST['start']?>&pg=<?=$_REQUEST['pg']?>">List Giftwrap Bows</a><span> Add Bow</span></div></td>
        </tr>
       <tr>
        ...
        <tr>
		  ...
		  <input class="input" type="text" name="bow_name"  value="<?=$_REQUEST['bow_name']?>" />
		  </td>
        </tr>
		 <tr>
          <td width="15%" align="left" valign="middle" class="tdcolorgray" >Extra Price (<?php echo  display_curr_symbol()?>)</td>
          <td width="85%" align="left" valign="middle" class="tdcolorgray">
		  <input class="input" type="text" name="bow_extraprice" size="3"  value="<?=$_REQUEST['bow_extraprice']?>" />
		  </td>
        </tr>
		 <tr>
          <td width="15%" align="left" valign="middle" class="tdcolorgray" >Order</td>
          <td width="85%" align="left" valign="middle" class="tdcolorgray">
		  <input class="input" type="text" name="bow_order" size="3"  value="<?=$_REQUEST['bow_order']?>" />
		  <a href="#" onmouseover ="ddrivetip('<?=get_help_messages('ADD_GIFTWRAP_BOW_ORDER')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a></td>
        </tr>
       
        <tr>
          <td align="left" valign="middle" class="tdcolorgray" >Hidden</td>
          <td align="left" valign="middle" class="tdcolorgray"><input type="radio" name="bow_active" value="0" <? if($_REQUEST['bow_active']==0 && $_REQUEST['bow_active']!='') echo "checked";?> />Yes<input type="radio" name="bow_active" value="1" <? if($_REQUEST['bow_active']==1 || $_REQUEST['bow_active']=='') echo "checked";?>  />No
		  <a href="#" onmouseover ="ddrivetip('<?=get_help_messages('ADD_GIFTWRAP_BOW_HIDE')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a></td>
        </tr>
		</table>
		</div>
		</td>
