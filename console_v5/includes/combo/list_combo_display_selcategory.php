...
else // case if product category group exists
{
	...
	$query_string = "request=combo&fpurpose=displayCategoryComboAssign&pass_combo_id=".$combo_id."";
	...
	$sql_assigned="SELECT product_categories_category_id FROM combo_display_category WHERE sites_site_id=$ecom_siteid AND combo_id=".$combo_id;
	...
	function call_save_selected(cname,sortby,sortorder,recs,start,pg,shelf_id) 
    {
		...
		else
		{
			...
			{
				show_processing();
				document.frm_prodcat.fpurpose.value='save_displayCategoryComboAssign';
				document.frm_prodcat.submit();
			}	
		}	
	...
	<input type="hidden" name="fpurpose" value="displayCategoryComboAssign" />
...
?>
	...
	<table border="0" cellpadding="0" cellspacing="0" class="maintable" width="100%">
		<tr>
		  <td align="left" valign="middle" class="treemenutd"><div class="treemenutd_div"><a href="home.php?request=combo&search_name=<?=$_REQUEST['pass_searchname']?>&sort_by=<?=$_REQUEST['pass_sortby']?>&sort_order=<?=$_REQUEST['pass_sortorder']?>&records_per_page=<?=$_REQUEST['pass_records_per_page']?>&start=<?=$_REQUEST['pass_start']?>&pg=<?=$_REQUEST['pass_pg']?>">List Combo </a> <a href="home.php?request=combo&fpurpose=edit&checkbox[0]=<?=$combo_id?>&search_name=<?=$_REQUEST['pass_searchname']?>&sort_by=<?=$_REQUEST['pass_sortby']?>&sort_order=<?=$_REQUEST['pass_sortorder']?>&records_per_page=<?=$_REQUEST['pass_records_per_page']?>&start=<?=$_REQUEST['pass_start']?>&pg=<?=$_REQUEST['pass_pg']?>&curtab=categmenu_tab_td">Edit Combo </a><span>Assign Display Category for '<? echo $row_combo['combo_name'];?>'</span></div></td>
		</tr>
		<tr>
		...
		<tr>
	     ...
	     <table width="100%" border="0" cellpadding="0" cellspacing="0" class="sorttabletop">
			<tr>
			  ...
			  <td width="12%" height="30"   align="left" valign="middle"><input name="catname" type="text" class="textfeild" id="catname" value="<?php echo $_REQUEST['catname']?>" /></td>
			  ...
			  ?></td>
		      <td width="8%" height="30"   align="left" valign="middle">Category Group </td>
		      <td width="14%" height="30"   align="left" valign="middle"><?php
