...
if($_REQUEST['rb_submit'] and (!$_REQUEST['emt_id']))
{
	...
	if($flag_name)
	{
		$sql_insert = "INSERT INTO elements SET site_id='$ecom_siteid', element_label='$element_label', element_name='$element_name', element_valign='".$_REQUEST['valign']."', element_align='".$_REQUEST['align']."', element_type='radio', mandatory='$mandatory', error_msg='$error_msg', sort_no='".$row['cnt']."', section_id=".$_REQUEST['section_id'];
		$db->query($sql_insert);
...
if($_REQUEST['rb_submit'] and $_REQUEST['emt_id'])
{
	...
	if($flag_name)
	{
		...
		$db->query($sql_insert);
		$db->query("DELETE FROM element_value WHERE element_id=$emt_id");
...
<table width="100%" align="center">
	<tr class="subhead">
		<td colspan="2" align="center">
			<h1><?php if($_REQUEST['emt_id']) echo "Edit Radio Button"; else echo "Add Radio Button"; ?></h1>
		</td>
	</tr>
	<tr class="subhead">
		<td>
	...
	<tr>
		<td colspan="2" align="center">
			...
			{
		?>
			<input type="submit" name="rb_submit" value="Update" class="smallsubmit">
			<?php
			}
			else
			{
			?>
			<input type="submit" name="rb_submit" value="Create" class="smallsubmit">
			<?php
			}
			?>
		</td>
