<?php
	...
	# Description 	: Page for listing Products for assiging into the Survey
	# Coded by 		: ANU
...
$table_name='products';
$page_type='Products';
$help_msg = get_help_messages('EDIT_SURVAY_ASS_PROD_MESS1');
$table_headers = array('<img src="images/checkbox.gif" border="0" onclick="select_all(document.frmlistProducts,\'checkbox[]\')"/><img src="images/uncheckbox.gif" border="0" onclick="select_none(document.frmlistProducts,\'checkbox[]\')"/>','Slno.','Product Name','Surveys assigned for the product','Category','Active');
...
$sql_assigned_products = "SELECT products_product_id FROM survey_display_product where survey_survey_id =".$survey_id." AND sites_site_id=".$ecom_siteid;
...
$where_conditions = "WHERE sites_site_id=$ecom_siteid AND p.product_id=pcm.products_product_id 
							AND product_categories_category_id =$category_id";
...
/////////////////////////////////////////////////////////////////////////////////////
$query_string .= "request=survey&fpurpose=list_assign_products&sort_by=$sort_by&sort_order=$sort_order&records_per_page=$records_per_page&search_name=".$_REQUEST['search_name']."&start=$start&survey_id=".$_REQUEST['survey_id']."&survey_title=".$_REQUEST['survey_title']."&pass_sort_by=".$_REQUEST['pass_sort_by']."&pass_sort_order=".$_REQUEST['pass_sort_order']."&pass_records_per_page=".$_REQUEST['pass_records_per_page']."&pass_search_name=".$_REQUEST['pass_search_name']."&pass_survey_id=".$_REQUEST['pass_survey_id']."&status=".$_REQUEST['status']."&pass_start=".$_REQUEST['pass_start']."&pass_pg=".$_REQUEST['pass_pg'];
...
{ 
	...
	if (atleastone==0) 
	{
		alert('Please select the Products to assign to the Survey');
		return false;
	}
	else
	{
		if(confirm('Assign Products to the Survey?'))
		{
				show_processing();
...
<form name="frmlistProducts" action="home.php?request=Survey" method="post" >	
...
<input type="hidden" name="request" value="survey" />
  ...
  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="maintable">
    <tr>
      <td colspan="2" align="left" valign="middle" class="treemenutd"><div class="treemenutd_div"><a href="home.php?request=survey&sort_by=<?=$_REQUEST['pass_sort_by']?>&sort_order=<?=$_REQUEST['pass_sort_order']?>&records_per_page=<?=$_REQUEST['pass_records_per_page']?>&search_name=<?=$_REQUEST['pass_search_name']?>&start=<?=$_REQUEST['pass_start']?>&pg=<?=$_REQUEST['pass_pg']?>&status=<?=$_REQUEST['status']?>">List Survey</a> <a href="home.php?request=survey&fpurpose=edit&survey_id=<?=$survey_id?>&sort_by=<?=$_REQUEST['sort_by']?>&sort_order=<?=$_REQUEST['sort_order']?>&records_per_page=<?=$_REQUEST['records_per_page']?>&search_name=<?=$_REQUEST['search_name']?>&start=<?=$_REQUEST['start']?>&pg=<?=$_REQUEST['pg']?>&pass_sort_by=<?=$_REQUEST['pass_sort_by']?>&pass_sort_order=<?=$_REQUEST['pass_sort_order']?>&pass_records_per_page=<?=$_REQUEST['pass_records_per_page']?>&pass_search_name=<?=$_REQUEST['pass_search_name']?>&pass_start=<?=$_REQUEST['pass_start']?>&pass_pg=<?=$_REQUEST['pass_pg']?>&status=<?=$_REQUEST['status']?>&curtab=survyprod_tab_td">Edit Survey</a><span> Assign Products to the Survey : <b>  '<?=$_REQUEST['survey_title']?>' </b></span></div></td>
    </tr>
	<tr>
	...
	<tr>
      ...
      <table width="18%" border="0" cellpadding="0" cellspacing="0" class="sorttableright">
        <tr>
          <td width="29%" height="30" align="left" valign="middle">Records Per Page </td>
          <td width="30%" height="30" align="left" valign="middle"><input name="records_per_page" type="text" class="textfeild" id="records_per_page" size="3" maxlength="3"  value="<?=$records_per_page?>"/></td>
        ...
        <tr>
          ...
          <td height="30" align="left" valign="middle">&nbsp;&nbsp;&nbsp;&nbsp;
            <input name="button5" type="submit" class="red" id="button5" value="Go" /> <a href="#" onmouseover ="ddrivetip('<?=get_help_messages('EDIT_SURVAY_ASS_PROD_GO')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a></td>
        </tr>
      </table>
	  </div></td>
    </tr>
    ...
    <tr>
	  ...
	  ?>
		...
		<input name="assign_products" type="button" class="red" id="assign_products" value="Assign Products" onclick="return assignproducts();" />
		  <a href="#" onmouseover ="ddrivetip('<?=get_help_messages('EDIT_SURVAY_ASS_PROD_ASSPROD')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a>
		<?
		}
		?>      </td>
    </tr>
	  ...
	  $sql_products = "SELECT product_id,product_name FROM $table_name $where_conditions ORDER BY $sort_by $sort_order LIMIT $start,$records_per_page ";
	   ...
	   ?>
        <tr >
          <td align="left" valign="middle" class="<?=$class_val;?>"  width="7%"><input name="checkbox[]" value="<? echo $row['product_id']?>" type="checkbox"></td>
		 ...
		 <?
		 $sql_survey="SELECT survey_title FROM survey a,survey_display_product b WHERE b.products_product_id=".$row['product_id']." AND a.survey_id=b.survey_survey_id";
	...
	<tr> <td align="right" valign="middle" colspan="2" class="listing_bottom_paging">	  
	    <?
	  if($numcount) {
