<?php
	...
	$table_headers = array('Slno.','Credit Card Name','<img src="images/checkbox.gif" border="0" onclick="select_all(document.frm_creditcards,\'cardtype_id[]\')"/><img src="images/uncheckbox.gif" border="0" onclick="select_none(document.frm_creditcards,\'cardtype_id[]\')"/>','Display Order','');
...
function call_ajax_select_paymentcards(sortby,sortorder)
{
	...
	for(i=0;i<document.frm_creditcards.elements.length;i++)
	{
		if (document.frm_creditcards.elements[i].type =='checkbox' && document.frm_creditcards.elements[i].name=='cardtype_id[]')
		{
			...
			{
				var temp 		= 'cardtype_order_'+document.frm_creditcards.elements[i].value;
	...
	var qrystr = '&sort_by='+sortby+'&sort_order='+sortorder+'&card_ids='+enabled_ids+'&sort_str='+sort_str;
	...
	<input type="hidden" name="search_click" value="" />
		...
		<tr>
		  <td colspan="3" align="left" valign="middle" class="treemenutd"><a href="home.php?request=payment_types&sort_order=<? echo $_REQUEST['pass_sort_order']?>&sort_by=<? echo $_REQUEST['pass_sort_by']?>">List Payment types</a> &gt;&gt; Select Credit Card Types </td>
		</tr>
		<tr>
		  ...
		  <td height="48" colspan="3" class="sorttd">
			...
			<tr>
			  ...
			  <td  align="left"><input name="Search_go" type="submit" class="red" id="Search_go" value="Go" onclick="document.frm_creditcards.search_click.value=1" />
				<a href="#" onmouseover ="ddrivetip('<?=get_help_messages('LIST_PAYMENT_TYPES_CARD_GO')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a></td>
			</tr>
		  </table>
		  	  </td>
		</tr>
