<?php
	/*#################################################################
	# Script Name 	: list_shelfgroup.php
...
$table_name='shelf_group';
$page_type='Shelf Menu';
$help_msg = get_help_messages('LIST_SHEF_MENUS_MESS1');
$table_headers = array('<img src="images/checkbox.gif" border="0" onclick="select_all(document.frmlistShelf,\'checkbox[]\')"/><img src="images/uncheckbox.gif" border="0" onclick="select_none(document.frmlistShelf,\'checkbox[]\')"/>','Slno.','Shelf Menu Name','Layout (position)(order)','Hidden'	);
...
//#Sort
$sort_by = (!$_REQUEST['sort_by'])?'name':$_REQUEST['sort_by'];
$sort_order = (!$_REQUEST['sort_order'])?'ASC':$_REQUEST['sort_order'];
$sort_options = array('name' => 'Shelf Menu Name');
	...
	for(i=0;i<document.frmlistShelf.elements.length;i++)
	{
		if (document.frmlistShelf.elements[i].type =='checkbox' && document.frmlistShelf.elements[i].name=='checkbox[]')
		{
			...
			if (document.frmlistShelf.elements[i].checked==true)
			{
				...
				if (ids!='')
					ids += '~';
				 ids += document.frmlistShelf.elements[i].value;
			}	
		}
	}
	if (atleastone==0) 
	{
		alert('Please select the shelf menus to change the hide status');
	}
	else
	{
		if(confirm('Change Hide Status of Seleted Shelf Menu(s)?'))
		{
				show_processing();
				Handlewith_Ajax('services/shelfgroup.php','fpurpose=change_hide&'+qrystr+'&ids='+ids);
		}	
	}	
}
...
function edit_selected()
{
	...
	if(cnt==0) {
		alert('Please select atleast one shelf menu');
	}
	else if(cnt>1 ){
		alert('Please select only one shelf menu to edit');
	}
	else
	{
		show_processing();
...
function call_ajax_delete(search_name,sortby,sortorder,recs,start,pg)
{
	...
	if (atleastone==0)
	{
		alert('Please select shelf menu to delete');
	}
	else
	{
		if(confirm('Are you sure you want to delete selected Shelf Menu?'))
		{
			show_processing();
			Handlewith_Ajax('services/shelfgroup.php','fpurpose=delete&del_ids='+del_ids+'&'+qrystr);
		}	
	}	
}
function call_ajax_saveorder(search_name,sortby,sortorder,recs,start,pg)
{
	...
	if(confirm('Save Sort Order Of Shelf Menus?'))
		{
				show_processing();
				Handlewith_Ajax('services/shelfgroup.php','fpurpose=save_shelf_order&'+qrystr+'&Idstr='+Idstr+'&OrderStr='+OrderStr);
		}
...
<input type="hidden" name="pg" value="<?=$pg?>" />
  <table border="0" cellpadding="0" cellspacing="0" class="maintable" width="100%">
    <tr>
      <td colspan="3" align="left" valign="middle" class="treemenutd"><div class="treemenutd_div"><span> List Shelf Menus</span></div></td>
    </tr>
	<tr>
	...
	<tr>
	  ...
	  <div class="sorttd_div">
        ...
        <tr>
          <td width="13%" height="30" align="left" valign="middle">Shelf Menu Name </td>
          ...
          <td width="11%" height="30" align="right" valign="middle"><input name="button5" type="submit" class="red" id="button5" value="Go" />
            <a href="#" onmouseover ="ddrivetip('<?=get_help_messages('LIST_SHELF_MENUS_GO')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a></td>
          </tr>
      </table>
	  </div></td>
    </tr>     
    <tr>
	  ...
	  <tr>
      <td class="listeditd" align="left" valign="middle" colspan="3"><a href="home.php?sort_by=<?=$sort_by?>&sort_order=<?=$sort_order?>&request=shelfgroup&fpurpose=add&records_per_page=<?=$records_per_page?>&search_name=<?=$_REQUEST['search_name']?>&start=<?=$start?>&pg=<?=$pg?>" class="addlist" onclick="show_processing()">Add</a>
	  <?
	  if($numcount)
	  {
	  ?>
	  ...
	  ?>
		...
		Change Status
		  ...
		  <a href="#" onmouseover ="ddrivetip('<?=get_help_messages('LIST_SHELF_MENUS_CHSTATUS')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a>&nbsp;&nbsp;
		<?
		}
		?>
   	   </td>
	   ...
	   if($numcount)
	   {
	   $sql_shelf = "SELECT id,name,hide FROM $table_name $where_conditions ORDER BY $sort_by $sort_order LIMIT $start,$records_per_page ";
	   ...
	   ?>
        <tr >
          <td align="left" valign="middle" class="<?=$class_val;?>"  width="6%"><input name="checkbox[]" value="<? echo $row['id']?>" type="checkbox"></td>
		  <td align="left" valign="middle" class="<?=$class_val;?>"  width="2%"><?php echo $srno++?></td>
          <td align="left" valign="middle" class="<?=$class_val;?>"><a href="home.php?request=shelfgroup&fpurpose=edit&checkbox[0]=<?php echo $row['id']?>&search_name=<?php echo $_REQUEST['search_name']?>&start=<?php echo $start?>&pg=<?php echo $pg?>&records_per_page=<?php echo $records_per_page?>&sort_by=<?php echo $sort_by?>&sort_order=<?php echo $sort_order?>" title="<? echo $row['name']?>" class="edittextlink" onclick="show_processing()"><? echo $row['name']?></a></td>
	  ...
	  ?>
	  <tr>
		  <td align="center" valign="middle" class="norecordredtext" colspan="6" >No Shelf Menu exists. </td>
	  </tr>
		<?
		}
		?>	
		<tr>
		  <td align="left" valign="middle" class="listeditd" colspan="3" >
		  <a href="home.php?sort_by=<?=$sort_by?>&sort_order=<?=$sort_order?>&request=shelfgroup&fpurpose=add&records_per_page=<?=$records_per_page?>&search_name=<?=$_REQUEST['search_name']?>&start=<?=$start?>&pg=<?=$pg?>" class="addlist" onclick="show_processing()">Add</a>
		  <?
		  if($numcount)
		  {
		  ?>
