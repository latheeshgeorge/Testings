<?php
	/*#################################################################
	# Script Name 	: list_product_catgroup_selproduct.php
...
$table_name='products';
$page_type='Products';
$help_msg 	= get_help_messages('LIST_PROD_CAT_GROUP_MESS1');
/*$help_msg = 'This section lists the Products available on the site. Here there is provision for selecting a Product & assigning it to the current group.';*/
...
//#Search terms
$search_fields = array('productname','manufactureid','categoryid','vendorid','rprice_from','rprice_to','cprice_from','cprice_to','discount','discountas','bulkdiscount','stockatleast','preorder','stock_from','stock_to','prodhidden','sort_order','sort_by');

$query_string = "request=prod_cat_group&fpurpose=prodGroupAssign&records_per_page=$records_per_page&productname=".$_REQUEST['productname']."&manufactureid=".$_REQUEST['manufactureid']."&categoryid=".$_REQUEST['categoryid']."&vendor_id=".$_REQUEST['vendor_id']."&start=$start"."&pass_catgroupname=".$_REQUEST['pass_catgroupname']."&pass_sort_by=".$_REQUEST['pass_sort_by']."&pass_sort_order=".$_REQUEST['pass_sort_order']."&pass_records_per_page=".$_REQUEST['pass_records_per_page']."&pass_start=".$_REQUEST['pass_start']."&pass_pg=".$_REQUEST['pass_pg']."&pass_group_id=".$_REQUEST['pass_group_id'];
...
$sql_assigned="SELECT products_product_id FROM product_categorygroup_display_products WHERE sites_site_id=$ecom_siteid AND product_categorygroup_catgroup_id=".$group_id;
...
function call_save_selected(cname,sortby,sortorder,recs,start,pg,group_id) 
{
	...
	else
	{
		...
		{
			show_processing();
			document.frm_products.fpurpose.value='save_prodGroupAssign';
...
<input type="hidden" name="fpurpose" value="prodGroupAssign" />
...
<table border="0" cellpadding="0" cellspacing="0" class="maintable">
        ...
        <tr>
		  ...
		  <table width="100%" border="0" cellspacing="1" cellpadding="1">
            <tr>
			  ...
			  <?php
				...
				$selected_category = $_REQUEST['categoryid'];
				}else{
				$selected_category = 0;
				}
				if(is_array($cat_arr))
				{
					echo generateselectbox('categoryid',$cat_arr,$selected_category);
				}
			  ?>			  </td>
            </tr>
            <tr>
			  ...
			  <?php
				...
				$selected_vendor = $_REQUEST['vendorid'];
				}else{
				$selected_vendor = 0;
				}
				if(is_array($vendor_arr))
				{
					echo generateselectbox('vendorid',$vendor_arr,$selected_vendor);
				}
			  ?>			  </td>
            </tr>
            <tr>
		  ...
		  <table width="100%" height="56" border="0" cellpadding="0" cellspacing="0">
            ...
            <tr>
              ...
              <td align="right"><input name="Search_go" type="submit" class="red" id="Search_go" value="Go" onclick="document.frm_products.search_click.value=1" />
                <a href="#" onmouseover ="ddrivetip('<? echo get_help_messages('LIST_PROD_CAT_GROUP_GO') ?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a></td>
            </tr>
          </table>		  </td>
        </tr>
      </table></div></td>
    ...
    <tr>
	  ...
	  <a href="#" onmouseover ="ddrivetip('<? echo get_help_messages('LIST_PROD_CAT_GROUP_ASSIGNPROD') ?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a>	  </td>
    </tr>
	 <?php  
