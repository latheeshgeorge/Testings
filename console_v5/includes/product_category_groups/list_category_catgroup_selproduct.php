<?php
	/*#################################################################
	# Script Name 	: list_category_catgroup_selproduct.php
...
?>
	<br />
	<span class='errormsg'>Category Menus not added yet. Please add  Category Menus First.</span><br />
	<br /><br />
	<a class="smalllink" href="home.php?request=prod_cat&catgroupname=<?=$_REQUEST['catgroupname']?>&sort_by=<?=$_REQUEST['sort_by']?>&sort_order=<?=$_REQUEST['sort_order']?>&records_per_page=<?=$_REQUEST['records_per_page']?>&start=<?=$_REQUEST['start']?>&pg=<?=$_REQUEST['pg']?>">Go to the Category Menu Listing page</a><br />
	<br />
	<a class="smalllink" href="home.php?request=prod_cat&fpurpose=add&catgroupname=<?=$_REQUEST['catgroupname']?>&sort_by=<?=$_REQUEST['sort_by']?>&sort_order=<?=$_REQUEST['sort_order']?>&records_per_page=<?=$_REQUEST['records_per_page']?>&start=<?=$_REQUEST['start']?>&pg=<?=$_REQUEST['pg']?>&curtab=category_tab_td">Go to Add Category Menu</a>
<?php
}
else // case if product category group exists
{
	...
	$help_msg 	= get_help_messages('LIST_CATE_ASS_SEL_CAT_GROUP_MESS1');
	...
	//#Search terms
	$search_fields = array('catname','sort_order','sort_by','pass_group_id','parentid');
	
	$query_string = "request=prod_cat_group&fpurpose=categoryGroupAssign&pass_catgroupname=".$_REQUEST['pass_catgroupname']."&pass_sort_by=".$_REQUEST['pass_sort_by']."&pass_sort_order=".$_REQUEST['pass_sort_order']."&pass_records_per_page=".$_REQUEST['pass_records_per_page']."&pass_start=".$_REQUEST['pass_start']."&pass_pg=".$_REQUEST['pass_pg']."&catgroupid=".$_REQUEST['catgroupid'];
	...
	$sql_assigned="SELECT product_categories_category_id FROM product_categorygroup_display_category WHERE sites_site_id=$ecom_siteid AND product_categorygroup_catgroup_id=".$group_id;
	...
	<input type="hidden" name="catgroupid" value="<?=$_REQUEST['catgroupid']?>" />
<? 
...
?>
	<table border="0" cellpadding="0" cellspacing="0" class="maintable" width="100%">
		...
		?>
		<tr>
		  <td  colspan="3" class="sorttd" width="100%">
		  <div class="sorttd_div" >
		<table width="100%" border="0" cellpadding="0" cellspacing="1">
			<tr>
			  <td width="18%"  align="left" valign="middle">Category Name </td>
			  <td width="31%"  align="left" valign="middle"><input name="catname" type="text" class="textfeild" id="catname" value="<?php echo $_REQUEST['catname']?>" /></td>
			  <td width="14%"  align="left" valign="middle">Category Menu </td>
			...
			<td  align="left" valign="middle">Records Per Page</td>
			    <td   align="left" valign="middle"><input name="records_per_page" type="text" class="textfeild" id="records_per_page" size="3" maxlength="3" value="<?php echo $records_per_page?>" /></td>
			    <td   align="left" valign="middle">Sort By</td>
			    <td width="31%"   align="left" valign="middle"><?php echo $sort_option_txt;?> in <?php echo $sort_by_txt?> </td>
			    <td width="6%"   align="right" valign="middle"><input name="Search_go" type="submit" class="red" id="Search_go" value="Go" onclick="document.frm_prodcat.search_click.value=1" />
                  <a href="#" onmouseover ="ddrivetip('<? echo get_help_messages('LIST_CAT_ASS_SEL_CAT_GO')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a></td>
			</tr>
		  </table>
          </div></td>
	  </tr>	 		
		...
		<tr>
		  <td align="right" valign="middle" colspan="<?php echo $colspan?>" class="listeditd"><?php 
				if ($db->num_rows($ret_qry))
				{
