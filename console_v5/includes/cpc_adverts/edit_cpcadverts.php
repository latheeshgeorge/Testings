...
$page_type = 'Adverts Location';
$help_msg =get_help_messages('ADD_CPADVERTS_MESS1');


$advertplace_id=($_REQUEST['advertplace_id']?$_REQUEST['advertplace_id']:$_REQUEST['checkbox'][0]);

$sql_advert="SELECT advertplace_id,advertplace_name 
					FROM costperclick_advertplacedon  
						WHERE sites_site_id=$ecom_siteid AND advertplace_id=".$advertplace_id;
$res_advert= $db->query($sql_advert);
if($db->num_rows($res_advert)==0) { echo " <font color='red'> You Are Not Authorised  </a>"; exit; }
$row_advert = $db->fetch_array($res_advert);
?>	
		...
		if (document.frmEditAdverts.elements[i].type =='checkbox' && document.frmEditAdverts.elements[i].name== checkboxname)
		{
			...
			if (document.frmEditAdverts.elements[i].checked==true)
			{
				...
				if (ch_ids!='')
				 ...
				 obj1 = eval("document.getElementById('img_ord_"+document.frmEditAdverts.elements[i].value+"')");
				 obj2 = eval("document.getElementById('img_title_"+document.frmEditAdverts.elements[i].value+"')");
...
<form name='frmEditAdverts' action='home.php?request=costperclick_adverts' method="post" onsubmit="return valform(this);">
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td colspan="2" align="left" valign="middle" class="treemenutd"><a href="home.php?request=costperclick_adverts&sort_by=<?=$_REQUEST['sort_by']?>&sort_order=<?=$_REQUEST['sort_order']?>&records_per_page=<?=$_REQUEST['records_per_page']?>&search_name=<?=$_REQUEST['search_name']?>&start=<?=$_REQUEST['start']?>&pg=<?=$_REQUEST['pg']?>">List Cost Per Click Adverts Location</a> &gt;&gt; Edit Cost Per Click Adverts Location</td>
        </tr>
        <tr>
		...
		?>  
         <tr>
		  ...
		  <input class="input" type="text" name="advertplace_name" value="<?=$row_advert['advertplace_name']?>"  />		  </td>
        </tr>
        ...
        <tr>
		  ...
		  <input type="hidden" name="advertplace_id" id="advertplace_id" value="<?=$advertplace_id?>" />
		  ...
		  <input type="hidden" name="checkbox[0]" id="checkbox[0]" value="<?php echo $advertplace_id?>" />
		  <input type="hidden" name="src_page" id="src_page" value="gift_bow" />
		  <input type="hidden" name="src_id" id="src_id" value="<?php echo $advertplace_id?>" />
