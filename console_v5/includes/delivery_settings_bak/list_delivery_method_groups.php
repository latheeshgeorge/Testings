...
<input type="hidden" name="search_click" value="" />
  ...
  <table border="0" cellpadding="0" cellspacing="0" class="maintable" width="100%">
    <tr>
      <td colspan="3" align="left" valign="middle" class="treemenutd"> <a href="home.php?request=delivery_settings">Delivery Settings</a> &gt;&gt; List Delivery Method Groups</td>
    </tr>
	<tr>
    ...
    <tr>
      <td height="48" colspan="3" class="sorttd">
      	<table  border="0" cellpadding="0" cellspacing="0" class="sorttableleft" width="100%">
        <tr>
          <td width="32%"  align="left" valign="middle">Group Name Like </td>
          <td width="68%"  align="left" valign="middle"><input name="group_name" type="text" class="textfeild" id="group_name" value="<?=$_REQUEST['group_name']?>" /></td>
          </tr>
      </table>
      <table border="0" cellpadding="0" cellspacing="0" class="sorttableright" width="18%" >
        <tr>
          <td   align="left">Show</td>
          <td   align="left">
<input name="records_per_page" class="textfeild" type="text"  id="records_per_page" size="2" value="<?=$records_per_page?>"/>
        ...
        <tr>
          ...
          <td align="left"><?=$sort_option_txt?>
            in
           	<?=$sort_by_txt?>&nbsp;&nbsp;&nbsp;
           	<input name="go_button" type="button" class="red" id="go_button" value="Go" onclick="document.frmListDeliveryMethod.search_click.value=1;getSelected('list_delmethod_groups')" /> <a href="#" onmouseover ="ddrivetip('<?=get_help_messages('LIST_DELIVERY_METHOD_GROUP_GO')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a></td>
          </tr>
      </table>	  </td>
    </tr>
    <tr>
      <td width="224"    class="listeditd"><a href="home.php?request=delivery_settings&fpurpose=add_methodgroups&group_name=<?=$_REQUEST['group_name']?>&settings_section=<?=$settings_section?>&sort_by=<?=$sort_by?>&sort_order=<?=$sort_order?>&records_per_page=<?=$records_per_page?>&pg=<?=$pg?>" class="addlist">Add</a><? if($numcount){?> <a href="#" onclick="return edit_selected('edit_methodgroups')" class="editlist">Edit</a> <a href="#" onclick="call_delete()" class="deletelist">Delete</a><? }?></td>
      <td width="176" align="center"  class="listeditd"><? if($numcount){  paging_footer($query_string,$numcount,$pg,$pages,$page_type,$colspan); }?></td>
      <td width="402"   align="right"  class="listeditd"><? if($numcount){?>ChangeHideStatus <?php echo generateselectbox('change_status',array(0=>'No',1=>'Yes'),$_REQUEST['change_status']);?>&nbsp;&nbsp;		<input name="change_hide" type="button" class="red" id="change_hide" value="Change" onclick="call_changestatus()" />
	...
	?>
	 ...
	 <tr>
      <td   class="listeditd"><a href="home.php?request=delivery_settings&fpurpose=add_methodgroups&group_name=<?=$_REQUEST['group_name']?>&sort_by=<?=$sort_by?>&sort_order=<?=$sort_order?>&records_per_page=<?=$records_per_page?>&pg=<?=$pg?>" class="addlist">Add</a><? if($numcount){?> <a href="#" onclick="return edit_selected('edit_methodgroups')" class="editlist">Edit</a> <a href="#" onclick="call_delete()" class="deletelist">Delete</a> <? }?></td>
      <td  align="center"  class="listeditd"><?php if($numcount){ paging_footer($query_string,$numcount,$pg,$pages,$page_type,$colspan); }?></td>
