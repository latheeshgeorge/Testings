...
$table_headers = array('<img src="images/checkbox.gif" border="0" onclick="select_all(document.frmlistSettingsCurrencies,\'currency_id[]\')"/><img src="images/uncheckbox.gif" border="0" onclick="select_none(document.frmlistSettingsCurrencies,\'currency_id[]\')"/>','Slno.','Currency Name','Currency Sign','Currency Code','Current Rate','Margin for Rate','Numeric Code','Default Currency');
...
function call_ajax_ChangeDefault(currency_name,sortby,sortorder,recs,start,pg)
{
	...
	if (pick_automatic==0)
	{
		for(i=0;i<document.frmlistSettingsCurrencies.elements.length;i++)
		{
			...
			else if(document.frmlistSettingsCurrencies.elements[i].type=='text' && document.frmlistSettingsCurrencies.elements[i].name.substr(0,8)=='txtrate_')
			{
				if(rate_arr!='')
					rate_arr += '~';
		...
		var qrystr 	= 'rate_str='+rate_arr+'&currency_name='+currency_name+'&curr_default='+new_default_id+'&sort_by='+sortby+'&sort_order='+sortorder+'&records_per_page='+recs+'&start='+start+'&pg='+pg;
...
<input type="hidden" name="save_det_clicked" id="save_det_clicked" value="" />
  ...
  <table border="0" cellpadding="0" cellspacing="0" class="maintable">
    <tr>
      <td colspan="3" align="left" valign="middle" class="treemenutd">List Settings Curencies</td>
    </tr>
	<tr>
	...
	<?php 
		...
		?>
        		<tr>
          			<td  colspan="7" align="center" valign="middle" class="errormsg" id="mainalert_tr"><?php echo $alert?></td>
          		</tr>
		 <?php
		 	}
		 ?> 
    <tr>
      <td height="48" colspan="3" class="sorttd">
        ...
        <tr>
          <td width="23%"   align="left" valign="middle">Currency Name </td>
          <td width="77%" align="left" valign="middle"><input name="currency_name" type="text" class="textfeild" id="currency_name" value="<?=$_REQUEST['currency_name']?>" /></td>
          </tr>
      </table>
      <table width="18%" border="0" cellpadding="0" cellspacing="0" class="sorttableright">
        <tr>
          <td  align="left">Show</td>
          <td align="left"><input name="records_per_page" type="text" class="textfeild" id="records_per_page" size="3" value="<?=$records_per_page?>" />&nbsp;&nbsp;
            <?=$page_type?> Per Page            </td>
          </tr>
        <tr>
          <td align="left">Sort By</td>
          <td align="left"><?=$sort_option_txt?>&nbsp;&nbsp;
            in   
           	<?=$sort_by_txt?>	&nbsp;&nbsp;&nbsp;	   <input name="go_button" type="button" class="red" id="go_button" value="Go" onclick="document.frmlistSettingsCurrencies.search_click.value=1;getSelected('')" /> <a href="#" onmouseover ="ddrivetip('<?=get_help_messages('LIST_CURENCY_GO')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a></td>
          </tr>
      </table>	  </td>
    </tr>
    <tr>
      <td width="162" class="listeditd" align=""><a href="home.php?request=general_settings_currency&fpurpose=add_currency&currency_name=<?=$_REQUEST['currency_name']?>&sort_by=<?=$sort_by?>&sort_order=<?=$sort_order?>&records_per_page=<?=$records_per_page?>&pg=<?=$pg?>" class="addlist">Add</a> <a href="#" onclick="return edit_selected('edit_currency')" class="editlist">Edit</a><a href="#" onclick="call_ajax_delete('<?php echo $_REQUEST['currency_name']?>','<?php echo $sort_by?>','<?php echo $sort_order?>','<?php echo $records_per_page?>','<?php echo $start?>','<?php echo $pg?>')" class="deletelist">Delete</a> </td>
      <td width="225" align="center"  class="listeditd"><?php paging_footer($query_string,$numcount,$pg,$pages,$page_type,$colspan); ?>		</td>
      <td width="258" class="listeditd" align="right"> 
	  <?php
	  	if($pick_automatically['pick_currency_rate_automatically']==1)
		{
		?>
	  		<input name="Change_Default" type="button" class="red" id="save_Det" value="Save Details" onclick="document.getElementById('save_det_clicked').value=1;call_ajax_ChangeDefault('<?=$currency_name?>','<?=$sort_by?>','<?=$sort_order?>','<?=$records_per_page?>','<?=$start?>',<?=$pg?>)"/>
			 &nbsp;&nbsp;<a href="#" onmouseover ="ddrivetip('<?=get_help_messages('LIST_CURENCY_SAVE_MARGIN');?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a>
	  <?php
	  	}
	  ?>
	  ...
	  <input name="Change_Default" type="button" class="red" id="Change_Default" value="<?php if($pick_automatically['pick_currency_rate_automatically']==0) { echo 'Save Details';} else { echo 'Get Live Currency Rates';}?>" onclick="call_ajax_ChangeDefault('<?=$currency_name?>','<?=$sort_by?>','<?=$sort_order?>','<?=$records_per_page?>','<?=$start?>',<?=$pg?>)"/>
      &nbsp;&nbsp;<a href="#" onmouseover ="ddrivetip('<?=$rate_msg?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a></td>
    </tr>
    <tr>
    ...
    <tr>
      <td class="listeditd"><a href="home.php?request=general_settings_currency&fpurpose=add_currency&currency_name=<?=$_REQUEST['currency_name']?>&sort_by=<?=$sort_by?>&sort_order=<?=$sort_order?>&records_per_page=<?=$records_per_page?>&pg=<?=$pg?>" class="addlist">Add</a> <a href="#" onclick="return edit_selected('edit_currency')" class="editlist">Edit</a> <a href="#" onclick="call_ajax_delete('<?php echo $_REQUEST['currency_name']?>','<?php echo $sort_by?>','<?php echo $sort_order?>','<?php echo $records_per_page?>','<?php echo $start?>','<?php echo $pg?>')" class="deletelist">Delete</a></td>
