...
{
	tab_arr 									= new Array('summary_tab_td','payment_tab_td','despatch_tab_td','refund_tab_td','returns_tab_td','notes_tab_td','query_tab_td','other_tab_td');
	var atleastone 						= 0;
		...
		case 'operation_despatched_do': 	// case of despatch done clicked
			...
			else
			{
				if (confirm('Are you sure you want to despatch the selected products in current order?'))
				{
					...
					qrystr		= 'id_str='+id_str+'&refno='+refno+'&note='+note;
...
<form name='frmOrderDetails' action='home.php?request=order_tax_report' method="post">
 		<table width="100%" border="0" cellspacing="1" cellpadding="1">
        <tr>
          <td colspan="5" align="left" valign="middle" class="treemenutd"><div class="treemenutd_div"><a href="#" onclick="goback_order()">List Order Tax Report </a> <span> Order Tax Details</span></div> </td>
        </tr>
        <tr>
	  ...
	  ?>
		...
		<input type="hidden" name="paid_only" value="<?php echo ($_REQUEST['paid_only'])?$_REQUEST['paid_only']:$_REQUEST['paid_only']?>" />
