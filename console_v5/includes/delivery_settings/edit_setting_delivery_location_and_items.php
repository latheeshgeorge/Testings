<?php
	...
	$page_type		='Delivery Charges';
   	$help_msg 			= get_help_messages('ADD_SETTINGS_DELIVERY_EDIT_LOCATION_ITEMS_MESS1');
	$delivery_id	= $_REQUEST['deliveryid'];
	$sqllocation	= "SELECT * FROM delivery_site_location WHERE delivery_methods_deliverymethod_id=".$delivery_id."  AND sites_site_id=$ecom_siteid order by location_order";
...
{
	document.frm_delivery_list.add_fpurpose.value='save_order';
	document.frm_delivery_list.submit();
}
</script>
<form method="post" action="" id="frm_delivery_list" name="frm_delivery_list">
<input type='hidden' name='add_fpurpose' value=''>
 <table width="100%" border="0" cellspacing="0" cellpadding="0">
       ...
       <tr>
		...
		?>
		<tr>
          <td colspan="3" align="center" valign="middle" class="errormsg" ><?=$order_alert ?></td>
        </tr>
        <?php
		}
        ?>
		...
		<tr>
		<td colspan="4" align="left" valign="middle" width="100%"  >
		<table border="0" cellpadding="4" cellspacing="0" width="100%">
	<tr>
		<td colspan="3" class="treemenutd"><b>Location</b>&nbsp;&nbsp;&nbsp;&nbsp;<b><u><a href="home.php?request=delivery_settings&fpurpose=editdelivery&deliveryid=<? echo $delivery_id;?>"><font color="#CC0000" style="font-weight:normal">(Please Click here to add more Locations)</font></a></u></b><a href="#" onmouseover ="ddrivetip('<?=get_help_messages('ADD_DELIVERY_SETTINGS_LOCATION_IEMTS_ADD')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a></td>

	</tr>
	<?php
	if($db->num_rows($reslocation))
	{
	?>
		<tr>
		<td width="20%" class="listingtableheader">Name</td>
		<td width="20%" class="listingtableheader">Sort Order</td>
		<td align='left' class="listingtableheader"><input type='button' name='save_order' value='Save Order' class='red' onclick='save_sortorder()'></td>
	<?php	
	}
	?>		
			...
			?>
				<tr>
				<td width="20%" class="tdcolorgray"><? echo $rowlocation['location_name']?></td>
				<td width="20%" class="tdcolorgray"><input type='text' name='sort_order_<?=$rowlocation['location_id']?>' size='2' value='<? echo $rowlocation['location_order']?>'></td>
