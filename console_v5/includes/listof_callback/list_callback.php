...
$table_name='customer_productcallback_details';
$page_type='Product Callback';
$help_msg = get_help_messages('LIST_PRODCALLBACK_MESS1');
$table_headers = array('<img src="images/checkbox.gif" border="0" onclick="select_all(document.frmlistQuote,\'checkbox[]\')"/><img src="images/uncheckbox.gif" border="0" onclick="select_none(document.frmlistQuote,\'checkbox[]\')"/>','Slno.','Date Added','Name','Email Address','Product','Status','View');
...
$sort_options = array('name' => 'Name','contact_email' => 'Email','status' => 'Status','date_added' => 'Date Added' );
...
if($_REQUEST['search_name']) {
	$where_conditions .= "AND ( name LIKE '%".add_slash($_REQUEST['search_name'])."%')";
}
...
/////////////////////////////////////////////////////////////////////////////////////
 $query_string .= "sort_by=$sort_by&sort_order=$sort_order&request=listofcallback&records_per_page=$records_per_page&search_name=".$_REQUEST['search_name']."&search_email=".$_REQUEST['search_email']."&search_status=".$_REQUEST['search_status']."&start=$start&srch_review_startdate=".$_REQUEST['srch_review_startdate']."&srch_review_enddate=".$_REQUEST['srch_review_enddate']."";
...
function call_ajax_changestatus(search_name,sortby,sortorder,recs,start,pg)
{
	...
	if (atleastone==0) 
	{
		alert('Please select the callback to change the status');
	}
	else
	{
		if(confirm('Change Status of Seleted Callback(s)?'))
		{
				show_processing();
				Handlewith_Ajax('services/list_callback.php','fpurpose=change_status&'+qrystr+'&enquire_ids='+E_ids);
		}	
	}	
}
...
function call_ajax_delete(search_name,sortby,sortorder,recs,start,pg)
{
	...
	else
	{
		...
		{
			show_processing();
			Handlewith_Ajax('services/list_callback.php','fpurpose=delete&del_ids='+del_ids+'&'+qrystr);
		}	
	}	
}
...
<input type="hidden" name="request" value="listofcallback" />
<input type="hidden" name="start" value="<?=$start?>" />
<input type="hidden" name="pg" value="<?=$pg?>" />
  <table width="973" border="0" cellpadding="0" cellspacing="0" class="maintable">
    <tr>
      <td colspan="5" align="left" valign="middle" class="treemenutd">List Callbacks</td>
    </tr>
	<tr>
    ...
    <tr>
      <td height="48"  colspan="10" class="listeditd" >
        ...
        <tr>
          <td align="left" valign="middle" colspan="6" > Name </td>
    ...
    <tr>
	  ...
	  ?>
		...
		Change Status
		  ...
		  <a href="#" onmouseover ="ddrivetip('<?=get_help_messages('LIST_PROD_CALLBACK_CHSTATUS')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.gif" width="17" height="13" border="0" /></a>&nbsp;&nbsp;
		<?
		}
		?>
      </td>
    ...
    <tr>
      ...
      <table  border="0" cellpadding="0" cellspacing="0" class="listingtable">
	   ...
	   if($numcount)
	   {
	   $sql_user = " SELECT  prodcallback_id,sites_site_id,name,surname,mobile,contact_email,university,fullgroup,timetocall,howcontact,status,date_format(date_added,'%d-%b-%Y') as added_date
,products_product_id,date_format(status_changed_date,'%d-%b-%Y') as status_date,status_changed_by FROM $table_name $where_conditions ORDER BY $sort_by $sort_order LIMIT $start,$records_per_page  ";
	   $res = $db->query($sql_user);
	    $srno = getStartOfPageno($records_per_page,$pg); 
	   while($row = $db->fetch_array($res))
	   {
		   if($row['products_product_id']>0)
		   {
		   $sql_product = "SELECT product_name FROM products WHERE product_id=".$row['products_product_id']." AND sites_site_id=".$row['sites_site_id']." LIMIT 1";
	   ...
	   ?>
        <tr >
          <td align="left" valign="middle" class="<?=$class_val;?>"  width="6%"><input name="checkbox[]" value="<? echo $row['prodcallback_id']?>" type="checkbox"></td>
		  <td align="left" valign="middle" class="<?=$class_val;?>"  width="2%"><?php echo $srno++?></td>
		   <td align="left" valign="middle" class="<?=$class_val;?>"><? echo $row['added_date']?></td>
          <td align="left" valign="middle" class="<?=$class_val;?>"><? echo $row['name'].' '.$row['surname']?></td>
		  <td align="left" valign="middle" class="<?=$class_val;?>"><?php echo $row['contact_email']; ?></td>
		  <td align="left" valign="middle" class="<?=$class_val;?>"><?php echo $row_product['product_name']; ?></td>
		  <td align="left" valign="middle" class="<?=$class_val;?>"><?php echo $row['status']; ?> </td>
		  <td align="left" valign="middle" class="<?=$class_val;?>"><img id="imgexp_<?php echo $row['prodcallback_id']?>" src="images/plus.gif" onclick="handle_expansionall('<? echo $row['prodcallback_id']?>')" title="Click" border="0"></td>
		   </tr>
		   <tr id="<?php echo $row['prodcallback_id']?>" style="display:none">
		   <td colspan="8" class="listingtablestyleA">
		   <table cellspacing="0" cellpadding="2" border="0" width="100%">
		   <tr >
		   <td class="listingtablestyleA" valign="top" width="22%"><strong>Mobile</strong></td><td valign="top" class="listingtablestyleA" >: <?php echo $row['mobile']?></td>
		   <td class="listingtablestyleA" valign="top" width="22%"><strong>University</strong></td><td valign="top" class="listingtablestyleA" >: <?php echo $row['university']?></td>
		   </tr>
		    <tr>
		    <td class="listingtablestyleA" valign="top"><strong>Do you have a full group ready?</strong></td><td valign="top" class="listingtablestyleA">: <?php echo $row['fullgroup']?></td>
		    <td class="listingtablestyleA" valign="top"><strong>When is the best time to call you?</strong></td><td valign="top" class="listingtablestyleA">: <?php echo $row['timetocall']?></td>
		    </tr>
		    <tr>
		    <td class="listingtablestyleA" valign="top"><strong>How Do You Want to Be Contacted?</strong></td><td valign="top" class="listingtablestyleA">: <?php echo $row['howcontact']?></td>
		    <td class="listingtablestyleA" valign="top">&nbsp;</td><td valign="top" class="listingtablestyleA">&nbsp;</td>
		    		   </tr>
				<?php
	  ...
	  ?>
	  <tr>
				  <td align="center" valign="middle" class="norecordredtext" colspan="9" >No Callbacks exists.</td>
		  </tr>
		<?
		}
		?>	
