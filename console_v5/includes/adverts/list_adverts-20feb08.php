...
$table_headers = array('<img src="images/checkbox.gif" border="0" onclick="select_all(document.frmlistAdverts,\'checkbox[]\')"/><img src="images/uncheckbox.gif" border="0" onclick="select_none(document.frmlistAdverts,\'checkbox[]\')"/>','Slno.','Adverts','Layout (position)(order)','Hidden');
...
$sort_options = array('advert_title' => 'Adverts','advert_order' => 'Adverts Order');
...
function edit_selected()
{
	...
	if(cnt==0) {
		...
		alert('Please select only one Advert to edit');
	}
	else
	{
		show_processing();
...
<input  type="hidden" name="pass_search_name" id="pass_search_name" value="<?=$_REQUEST['search_name']?>"  />
  <table border="0" cellpadding="0" cellspacing="0" class="maintable">
    <tr>
      <td colspan="3" align="left" valign="middle" class="treemenutd">Adverts
	   &gt;&gt; List Adverts </td>
    </tr>
	<tr>
    ...
    <tr>
      <td height="48" class="sorttd" colspan="3" >
        ...
        <tr>
          <td width="22%" align="left" valign="middle">Advert title </td>
      ...
      <table width="18%" border="0" cellpadding="0" cellspacing="0" class="sorttableright">
        ...
        <tr>
          ...
          <td align="left">&nbsp;&nbsp;&nbsp;&nbsp;<input name="button5" type="submit" class="red" id="button5" value="Go" /> <a href="#" onmouseover ="ddrivetip('<?=get_help_messages('LIST_ADVERTS_GO')?>')"; onmouseout="hideddrivetip()"><img src="images/helpicon.png" width="17" height="13" border="0" /></a></td>
        </tr>
      </table>
	  </td>
    </tr>
    <tr>
      <td class="listeditd"><a href="home.php?request=adverts&fpurpose=add&pass_sort_by=<?=$sort_by?>&pass_sort_order=<?=$sort_order?>&pass_records_per_page=<?=$records_per_page?>&pass_search_name=<?=$_REQUEST['search_name']?>&pass_start=<?=$start?>&pass_pg=<?=$pg?>" class="addlist" onclick="show_processing()">Add</a> 
	  <?
	  if($numcount)
	  {
	  ?>
    ...
    <tr>
      ...
      <table  border="0" cellpadding="0" cellspacing="0" class="listingtable">
	   ...
	   if($numcount)
	   {
	   $sql_group = "SELECT advert_id,advert_title,advert_order,advert_hide FROM $table_name $where_conditions ORDER BY $sort_by $sort_order LIMIT $start,$records_per_page ";
	   ...
	   ?>
        <tr >
          ...
          <td align="left" valign="middle" class="<?=$class_val;?>"><a href="home.php?request=adverts&fpurpose=edit&checkbox[0]=<?php echo $row['advert_id']?>&pass_search_name=<?php echo $_REQUEST['search_name']?>&pass_start=<?php echo $start?>&pass_pg=<?php echo $pg?>&pass_records_per_page=<?php echo $records_per_page?>&pass_sort_by=<?=$_REQUEST['sort_by']?>&pass_sort_order=<?=$_REQUEST['sort_order']?>" title="<? echo $row['advert_title']?>" class="edittextlink" onclick="show_processing()"><? echo $row['advert_title']?></a></td>
