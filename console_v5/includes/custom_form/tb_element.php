...
if($_REQUEST['tb_submit'] and (!$_REQUEST['emt_id']))
{
	...
	$sql_check =  "SELECT element_id FROM elements WHERE  sites_site_id=$ecom_siteid AND element_sections_section_id=".$_REQUEST['section_id']." AND element_label='".$element_label."' AND element_type='text' LIMIT 1";
	...
	{   
		...
		VALUES(".$ecom_siteid.",".$_REQUEST['section_id'].",'".$element_name."','text','".$element_label."','".$_REQUEST['element_valign']."','".$_REQUEST['element_align']."','".addslashes($_REQUEST['element_size'])."',".$sort_no.",'".$mandatory."','".$error_msg."','".$_REQUEST['maxlength']."')";
		$db->query($sql_insert);
				?>
...
if($_REQUEST['tb_submit'] and $_REQUEST['emt_id'])
{
	...
	$sql_check =  "SELECT element_id FROM elements WHERE  sites_site_id=$ecom_siteid AND element_sections_section_id=".$_REQUEST['section_id']." AND element_label='".$element_label."' AND element_type='text' AND element_id<>".$_REQUEST['emt_id']." LIMIT 1";
	$ret_check =  $db->query($sql_check);
	if($db->num_rows($ret_check)>0)
	{
	  $flag_name= false;
...
<table width="100%" align="center" cellpadding="0" cellspacing="0">  
	<tr >
		<td colspan="2" align="left" class="listingtableheader">
			<?php if($_REQUEST['emt_id']) echo "Edit Text Box"; else echo "Add Text Box"; ?>		</td>
	</tr>
	<tr >
		...
		<td class="tdcolorgray">
			<input type="text" class="input" name="element_label" value="<?php if($element_label) echo $element_label; else echo $row['element_label']; ?>" size="35">
		</td>
	</tr>
	<tr >
		<td class="tdcolorgray">
	...
	<tr >
		...
		<td class="tdcolorgray">
			<input type="text" name="element_size" value="<?php if($size) echo $size; else if($row['element_size']) echo $row['element_size']; else echo "20"; ?>" size="2">
		</td>
	</tr>
	<tr >
		<td class="tdcolorgray">
		...
		<td class="tdcolorgray">
			<input type="text" name="maxlength" value="<?php if($tb_maxlength) echo $tb_maxlength; else if($row['maxlength']) echo $row['maxlength']; else echo "0"; ?>" size="2"> No limit if set to zero.
		</td>
	</tr>
	<tr>
		<td class="tdcolorgray">
	...
	<tr>
		...
		<?php
			...
			{
		?>
			<input type="submit" name="tb_submit" value="Update" class="red">
			<?php
			}
			else
			{
			?>
			<input type="submit" name="tb_submit" value="Create" class="red">
			<?php
			}
			?>
		</td>
