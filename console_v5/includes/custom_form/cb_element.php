...
if($_REQUEST['cb_submit'] and (!$_REQUEST['emt_id']) and $_REQUEST['element_label'] ) // to add new value
{
	...
	$sql_check =  "SELECT element_id FROM elements WHERE  sites_site_id=$ecom_siteid AND element_sections_section_id=".$_REQUEST['section_id']." AND element_label='".$element_label."' AND element_type='checkbox' LIMIT 1";
	...
	if($flag_name)
	{
		$sql_insert = "INSERT INTO elements SET sites_site_id='$ecom_siteid', element_label='$element_label', element_name='$element_name', element_valign='".$_REQUEST['valign']."', element_align='".$_REQUEST['align']."', element_type='checkbox', mandatory='$mandatory', error_msg='$error_msg', sort_no='".$row['cnt']."', element_sections_section_id=".$_REQUEST['section_id'];
		$db->query($sql_insert);
  ...
  {
    $alert = "Add Values for Check Box";
  }
}
if($_REQUEST['cb_submit'] and $_REQUEST['emt_id'] )
{
   if(count($_SESSION['element_values']))
	{
	$flag_name = true;
	...
	$sql_check =  "SELECT element_id FROM elements WHERE  sites_site_id=$ecom_siteid AND element_sections_section_id=".$_REQUEST['section_id']." AND element_label='".$element_label."' AND element_id<>".$_REQUEST['emt_id']." AND element_type='checkbox' LIMIT 1";
	$ret_check =  $db->query($sql_check);
	if($db->num_rows($ret_check)>0)
	{
	  $flag_name= false;
...
<form name="frmtb" action="add_elements.php" method="POST" class="frmcls" onsubmit="return valform(this);">
<table width="100%" align="center" cellpadding="0" cellspacing="0">
	<tr class="content_header">
		<td colspan="2" align="left" class="listingtableheader">
			<?php if($_REQUEST['emt_id']) echo "Edit Check Box"; else echo "Add Check Box"; ?>		</td>
	</tr>
	<tr>
	...
	<tr>
		<td colspan="2" align="center" class="tdcolorgray">
			...
			{
		?>
			<input type="submit" name="cb_submit" value="Update" class="red">
			<?php
			}
			else
			{
			?>
			<input type="submit" name="cb_submit" value="Create" class="red">
			<?php
			}
			?>
		</td>
