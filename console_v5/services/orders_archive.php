...
/* Function to decide which tab funciton is to be displayed */
	...
	if($_REQUEST['fpurpose']=='')
	{
		...
		include ('includes/orders_archive/list_orders.php');
	}
	elseif($_REQUEST['fpurpose']=='ord_details') //  View details page
	{
		...
		include ('includes/orders_archive/ajax/order_ajax_functions.php');
		include ('includes/orders_archive/order_details.php');
	}
	elseif($_REQUEST['fpurpose']=='do_archive_backtoorder') //  View details page
	{
		if(count($_REQUEST['checkbox']))
		{
			...
			{
				move_archive_to_order($v,$ecom_siteid,'back');
			}
			$alert = 'Selected archived order(s) moved back to My Orders Successfully';
		}	
				...
				else
				{
					...
					if ($baseStatus == "OK")
					{
						...
						if($row_ord['order_totalauthorizeamt']>0) // if authorize amount is >0
						{
							...
							{
								$order_fields = "order_paystatus = 'REFUNDED',order_status='CANCELLED'";
	...
	function do_changeorderstatus_operation($order_id,$ch_stat,$alt_prods,$stat_arr,$note,$src='det')
	{
		...
		if ($db->num_rows($ret_ord))
		{
			...
			if ($src!='det')
			{
				$sr_arr = array ('NEW','PENDING','ONHOLD','BACK');
...
function move_archive_to_order($order_id,$siteid,$mov)
{
	global $db;
	$sql_ord = "SELECT * FROM orders_archieve WHERE order_id = $order_id AND sites_site_id = $siteid LIMIT 1";
	$ret_ord = $db->query($sql_ord);
	if($db->num_rows($ret_ord))
	{
		$row_ord = $db->fetch_array($ret_ord);
		// Check whether an entry already exists in the archive table
		$sql_check = "SELECT order_id FROM orders WHERE order_id = $order_id and sites_site_id = $siteid LIMIT 1";
		$ret_check = $db->query($sql_check);
		if($db->num_rows($ret_check)==0)
		{
			$insert_array												= array();
			...
			$sql_del = "DELETE FROM orders_archieve WHERE order_id = $order_id AND sites_site_id = $siteid LIMIT 1";
			$db->query($sql_del);
		}
	}
}
