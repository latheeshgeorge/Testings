<?php
	 ...
	 *********************************************************/
	 $ecom_siteid 		= 74;//dmresponsive temp
	...
	if($db->num_rows($ret_images)>0)
	{
		while($row_images = $db->fetch_array($ret_images))
		{
           ...
           $srcfile = $image_path."/".$extralargename;
           	resize_image($srcfile, "big/".$bigname, $geometry["big"]);
            resize_image($srcfile, "thumb/".$thumbname, $geometry["thumb"]);
			resize_image($srcfile, "category/".$categoryname, $geometry["cat"]);
			resize_image($srcfile, "category_thumb/".$thumbcategoryname, $geometry["catthumb"]);
			resize_image($srcfile, "gallerythumb/".$gallerythumbname, '90>');
			resize_image($srcfile, "icon/".$iconname, $geometry["icon"]);
