...
// Source Site Details
// ###############################################################################
$src_siteid			= 229; //www.asll.co.uk on old domain
$src_host 			= 'localhost';  
$src_user 			= 'newserver';
$src_pass 			= 'yuj*dff';
$src_db 			= 'bshop_3_1';
...
// ###############################################################################
$dest_siteid		= 83; // asll.bshop4.co.uk on new domain
$dest_host			= 'localhost';
$dest_user			= 'bshop_am4';
$dest_pass			= 'b$H0pF@Ur';
...
$process_div = '<div id="import_processing_div" style="display:none; color:#FF0000" align="center">
	...
	$sql_cust = "SELECT customer_id,corp_dept_id,activated,accountType,title,firstname,middlename,surname,postion,companyName,companyType,
					...
					WHERE 
						site_id = $src_siteid 
						AND customer_id > $last_existing_id";
	...
	if (mysql_num_rows($ret_cust))
	{
		while ($row_cust = mysql_fetch_array($ret_cust))
		{
			...
			$sql_dest_check = "SELECT customer_id 
							FROM 
								customers 
							WHERE 
