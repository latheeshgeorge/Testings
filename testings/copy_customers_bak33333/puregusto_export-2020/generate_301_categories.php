<?php
	...
	$filename ='puregusto_category_301.csv';
	$fp = fopen ($filename,'w');
	
	fwrite($fp,'"Category Id","Category Name","URL"'."\n");
	// Check whether the product Exists in old website
	$sql_custsrc = "SELECT category_id,category_name FROM product_categories WHERE sites_site_id = $siteid";
	...
	if($db->num_rows($ret_custsrc))
	{
		while ($row_custsrc = $db->fetch_array($ret_custsrc))
		{
				...
				$newname = $row_custsrc['category_name'];
				$newurl = "https://".$source_domain."/".strip_url($newname)."-c$newid.html";	
			  fwrite($fp,add_qts($newid).','.add_qts($newname).','.add_qts($newurl)."\n");
			//$i++;
		}
	}	
