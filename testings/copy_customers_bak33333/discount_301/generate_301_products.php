<?php
	...
	$sql_custsrc = "SELECT * FROM products WHERE sites_site_id = $des_siteid";
	...
	if($db->num_rows($ret_custsrc))
	{
		...
		{
			$err_msg 	= '';
			$name		= stripslashes($row_custsrc['product_name']);
			$type		= 'Product';
			
			
			
			// Check whether the product Exists in old website
			$sql_prodloc = "SELECT product_id,product_name FROM products  
								WHERE 
									sites_site_id = $src_siteid 
									AND product_name='".addslashes($row_custsrc['product_name'])."' LIMIT 1";
			$ret_prodloc = $db->query($sql_prodloc);
			if($db->num_rows($ret_prodloc))
			{
				$row_prodloc = $db->fetch_array($ret_prodloc);
				$oldid = $row_prodloc['product_id'];
				$newid = $row_custsrc['product_id'];
				$oldurl = "http://".$dest_domain."/p$oldid/".strip_url($row_custsrc['product_name']).".html";
				$newurl = "http://".$dest_domain."/p$newid/".strip_url($row_custsrc['product_name']).".html";
