<?php
	...
	$db->select_db();
		...
		while($row_siteid = $db->fetch_array($ret_site))
		{
		   ...
		   write_giftemails($site_idd);
		}
	    function write_giftemails($site_id)
		{
			global $db;
			...
			$sql_email = " SELECT b.email_id,b.email_message,email_messagepath FROM gift_vouchers a,gift_voucher_emails b WHERE a.sites_site_id = $site_id AND a.voucher_id = b.gift_vouchers_voucher_id ";
			$ret_sql = $db->query($sql_email);
			if($db->num_rows($ret_sql))
			{	
				$flv_path = $email_path.'/gift_voucher_emails'; 
				if(!file_exists($flv_path)) mkdir($flv_path, 0777);
				while($row_email= $db->fetch_array($ret_sql))
				{
                                    if($row_email['email_message']!='')
                                    {
                                      ...
                                      fclose($fh);
                                      $update_array = array();
                                      $update_array['email_messagepath'] = 'email_messages/gift_voucher_emails/'.$row_email['email_id'].".txt";
                                      $db->update_from_array($update_array, 'gift_voucher_emails', 'email_id', $row_email['email_id']);
                                    }
				}
			}
			echo "Voucher emails write successfully for ".$ecom_hostname." <br />";
		  }
		  else
		  {
		    echo "Error!!";
