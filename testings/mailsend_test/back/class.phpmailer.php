 ...
 class PHPMailer
    {
       ...
       //     (e.g. "smtp1.example.com:25;smtp2.example.com").
        var $Host = "87.106.34.121";//"87.106.208.127";
        var $Port = 25;
        var $Helo = "";
        var $SMTPAuth = true;

        // Sets SMTP username.
        var $Username = "info-1@webclinicmailer.co.uk";
        ...
        */
        function SmtpSend($header, $body) {
            if (!$this->PluginDir) $this->PluginDir =
                str_replace('\\','/',dirname(__FILE__)).chr(47);
            ...
            $mime[] = sprintf("Content-Type: %s; name=\"%s\"%s", $type, $name,
                $this->LE);
            $mime[] = sprintf("Content-Transfer-Encoding: %s%s", $encoding,
                $this->LE);
        ...
        case 'text':
        default:
        $x += preg_match_all('/[\000-\010\013\014\016-\037\177-\377]/', $str,
            $matches);
        break;
    }
    if ($x == 0)
...
function Received() {
    if ($this->ServerVar('SERVER_NAME') != '') {
        $protocol = ($this->ServerVar('HTTPS') == 'on') ? 'HTTPS' :
        'HTTP';
