		...
		?>	
		  ...
		  else
		  {
			  ...
			  ?>
				  <tr>
					...
					<div class="updatediv" align="center"><a href="#" onclick="document.frm_cart.remcart_id.value='<?php echo $products_arr['cart_id']?>';handle_form_submit(document.frm_cart,'Update_qty','#upd')" title="<?php echo $Captions_arr['CART']['CART_UPDATE']?>"><img src='<?php url_site_image('cart_update.gif')?>' border="0" alt="<?php echo $Captions_arr['CART']['CART_UPDATE']?>"></a>  <a href="#" class="update_link" onclick="if (confirm_message('<?php echo $Captions_arr['CART']['CART_ITEM_REM_MSG']?>')) {document.frm_cart.remcart_id.value='<?php echo $products_arr['cart_id']?>';handle_form_submit(document.frm_cart,'Remove_qty','#rem')}" title="<?php echo $Captions_arr['CART']['CART_REMOVE']?>"><img src='<?php url_site_image('cart_delete.gif')?>' border="0" alt="<?php echo $Captions_arr['CART']['CART_REMOVE']?>"></a></div></td>
					<td align="right" valign="middle" class="<?php echo $tdpriceAclass?>"><?php echo print_price($products_arr['final_price'])?></td>
				  </tr>
				  <?php
		  ...
		  ?>
		   ...
		   // Section to show the tax details
			...
			/* if ($cartData["savings"]["customer"] or $cartData["savings"]["corporate"])
			 {
			 ?>
				<tr>
					...
					<?php
						...
						{
							if ($cartData['discounts']["customer_type"]	== 'Disc_Group') // check whether discount is due to customer group
								echo 'Customer Group Discount ('.$cartData["discounts"]["customer"].'%)';
							else // case if discount is due to discount set for current customer
								echo 'Customer Discount ('.$cartData["discounts"]["customer"].'%)';
						}	
						elseif ($cartData["savings"]["corporate"])
							echo 'Corporate Discount ('.$cartData["discounts"]["corporate"].'%)';
					?></td>
					<td align="right" valign="middle" class="shoppingcartpriceC"><?php 
						if ($cartData["savings"]["customer"])
						{
							echo print_price($cartData["savings"]["customer"]);
						}
						elseif ($cartData["savings"]["corporate"])
						{
							echo print_price($cartData["savings"]["corporate"]);
						}
						?></td>
				</tr>
			 <?php
		 ...
		 <?php
			  ...
			  ?>
				  <tr>
					...
					<td align="left" valign="middle" class="<?php echo $tdpriceBclass?>"><?php echo print_price($row_orddet['order_retailprice'])?></td>
					<td align="center" valign="middle" class="<?php echo $trmainclass?>">
					<?php
						// Calling the function to check whether the current product is in preorder or not
					...
					<td align="right" valign="middle" class="<?php echo $tdpriceAclass?>"><?php echo print_price($row_orddet['order_rowtotal'])?></td>
				  </tr>
				  <?php
				  	// If variables exists for current product, show it in the following section
					if ($vars_exists) 
