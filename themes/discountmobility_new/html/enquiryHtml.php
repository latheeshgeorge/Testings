	  ...
	  function enquiry_Showform()
	  {
		...
		//print_r($prod_array );
				  ...
				  <td colspan="4" align="left" valign="middle" class="cartlogin_msg">
					...
					if ($cust_id) // Case logged in 
					{
					  echo $Captions_arr['CART']['CART_LOGGED_IN_AS']; echo '&nbsp;'.$cartData['customer']['customer_fname']." ".$cartData['customer']['customer_mname']." ".$cartData['customer']['customer_surame'];?>
						<br />
						<? echo $Captions_arr['CART']['CART_IF_YOU_NOT_LOG'];?> <a href="<?php echo $ecom_selfhttp.$ecom_hostname?>/logout.html?rets=2" title="Logout" class="cartlogin_link"><? echo $Captions_arr['CART']['CART_HERE'];?></a><? echo "&nbsp;". $Captions_arr['CART']['CART_TO_LOGOUT'];?> 
					<?php
					}
          ...
          <tr>
            ...
            <td align="center" valign="middle" class="shoppingcartheaderA"><?php echo $Captions_arr['ENQUIRE']['ENQUIRE_PRICE']?></td>
		...
		while($row_select_enq = $db->fetch_array($ret_select_enq))
		{
		 ...
		 $prod_sql_det = "SELECT product_id,product_name,product_default_category_id,product_webprice,
		  ...
		  ?>
			  <tr>
				...
				<?php 
					...
					echo show_Price($row_prod_det,$price_class_arr,'other_3');					?></td>
            ...
            <td colspan="4" align="right" valign="middle" class="shoppingcartcontent">
			  ...
			  ?>
				...
				<?php
					...
					{
				?>
				  <input name="continue_submit" type="button" class="buttonred_cart_continue" id="continue_submit" value="Continue Shopping" onclick="show_wait_button(this,'Please Wait...');window.location='<?php echo $ps_url?>'" />
				<?php
					}
					else
						echo '&nbsp;';
		  ...
		  <?php /*?><tr><td colspan="6" align="right" valign="middle"><?php */?>
 		  ...
 		  <tr>
		       <td valign="top" class="cust_regmiddle_td_left">
				  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="cust_regmiddle">
			<tr>
				<td  align="left" valign="middle" class="logmsg_header"><?php echo $Captions_arr['CUST_LOGIN']['NEW_CUST']?></td>
			  </tr>
              <tr>
          	<td colspan="2" align="left" valign="top" class="cart_login_td">
            	<table cellpadding="0" cellspacing="0" class="cart_login_tbl">
			    <tr>
          ...
          <tr>
            <td colspan="2" align="left" valign="middle" class="logmsg_header">&nbsp;<?php echo $Captions_arr['CART']['CART_LOGIN']?></td>
          </tr>
           <tr>
          	<td colspan="2" align="left" valign="top" class="cart_login_td1">
            	<table cellpadding="0" cellspacing="0" class="cart_login_tbl">
          <tr>
...
/* Function to validate the Enquie Details*/
			...
			{
				//alert(feildmsg);
				fieldRequired 		= Array('cbo_enqtitle','txt_enqfname','txt_enqlname','txt_enqaddress','cbo_enqstate','txt_enqpostcode','txt_enqphone','txt_enqemail');
				fieldDescription 	= Array('<?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_TITLE']?>','<?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_FNAME']?>','<?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_LNAME']?>','<?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_ADDRESS']?>','<?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_STATE']?>','<?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_PCODE']?>','<?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_PHONE']?>','<?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_EMAIL']?>');
       ...
       <tr>
		...
		?>
                  <tr>
                   ...
                   <td width="57%" align="left" valign="top" ><select name="cbo_enqtitle" id="cbo_enqtitle"><option value="" >Select</option><option value="Mr." <? if($row_cust['customer_title']=='Mr.') echo "selected"; else echo '';?>>Mr.</option><option value="Ms." <? if($row_cust['customer_title']=='Ms.') echo "selected"; else echo '';?>>Ms.</option><option value="Mrs." <? if($row_cust['customer_title']=='Mrs.') echo "selected"; else echo '';?>>Mrs.</option>
                  ...
                  <tr>
                    <td class="regiconent" align="left" valign="top"><?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_ADDRESS']?><span class="redtext">*</span></td>
                    <td align="left" valign="top"><textarea name="txt_enqaddress" cols="25" rows="4"><?=$_REQUEST['txt_enqaddress']?></textarea></td>
                  </tr>
                 <tr>
                    <td class="regiconent" align="left" valign="top"><?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_STATE']?> <span class="redtext">*</span> </td>
