	...
	{		
		...
		function show_Variable_main_image($product_id,$var_arr=array())
		{
			...
			if ($comb_arr['combid']>0)
			{
				...
				{
				?>
				<div class="prod_det_zoom"><a href="<?php url_root_image($prodimg_arr[0]['image_extralargepath'])?>"
					rel='lightbox[gallery]' title="<?=$row_prod['product_name']?>">Click to Enlarge</a></div>
				<?php
				}*/ 			
				}
				else
				...
				<div class="ajx_det_main_midA">
					  <table width="100%" border="0" cellpadding="0" cellspacing="3" class="productdethumbtable">
						...
						foreach ($prodimg_arr as $k=>$v)
						{
							...
							if ($img_det!='')
								$img_det .= '~';
								$imgs_arr = explode('category_thumb/',$v['image_thumbcategorypath']);
							$img_det .= $imgs_arr[1];
						...
						?>
							...
							<?php
								 show_image(url_root_image($v['image_thumbcategorypath'],1),$title,$title,'preview',$cur_moreimg_id);
								$i++;
							?>
							</a>
							</li>
