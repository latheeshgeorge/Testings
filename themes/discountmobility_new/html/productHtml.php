...
function Show_ProductDetails($row_prod)
{
		global $Captions_arr,$ecom_siteid,$db,$ecom_hostname,$ecom_themename,
			$ecom_themeid,$default_layout,$inlineSiteComponents,$Settings_arr,$meta_arr,$ecom_twitteraccountId,$mobile_browser,$cookieval;
		...
		?>
		<script type="text/javascript" src="<? echo $ecom_selfhttp.$ecom_hostname?>/images/<? echo $ecom_hostname?>/scripts/tinybox.js"></script>
		...
		*/ 	
		...
		?>
		<table width="790" border="0" cellpadding="0" cellspacing="0" class="prod_details_table" itemscope itemtype="http://schema.org/Product">
		<?php
		if($alert)
		{
			?>
		...
		echo "
					<script  type=\"text/javascript\" src=\"".url_head_link("images/".$ecom_hostname."/scripts/zoom/jquery.elevatezoom.js",1)."\" ></script>";
		?>
		<?php
		...
		<?php	echo '<h1  itemprop="name">'.stripslashes($row_prod['product_name']).'</h1>';
       
		if($row_prod['product_discontinue']==1)
		{
			?>
			<div class="alert_innerD"> - This product has been discontinued  - </div>
			<?php
		}
		else
		{
...
<table width="100%" cellpadding="0" cellspacing="0" class="prod_details_tbl_l">
		...
		if($prod_img_show_type=='FLASH' or $prod_img_show_type=='FLASH_ROTATE')
		{
			...
			*/	
			...
			elseif($prod_img_show_type=='FLASH_ROTATE') // Case of showing flash which rotates the given images
			{
				...
				document.write('<param name="movie" value="<?php echo $ecom_selfhttp.$ecom_hostname?>/images/<?php echo $ecom_hostname?>/swf/rotate_flash.swf?filepath=<?php echo $ecom_selfhttp.$ecom_hostname?>/getflashrotate_images.php?p_id=<?php echo $_REQUEST['product_id']?>" />');
				document.write('<param name="quality" value="high" />');
		...
		else
		{
            ...
            /*$j("#zoom_03").elevateZoom({gallery:'gallery_01',zoomType:'lens', cursor: 'pointer', galleryActiveClass: 'active',tint:true, tintColour:'#F90', tintOpacity:0.5, imageCrossfade: true, loadingIcon: 'http://www.elevateweb.co.uk/spinner.gif'}); */
            $j("#zoom_03").elevateZoom({constrainType:"height", <?php echo $zoomtype?> containLensZoom: true,constrainSize:274, gallery:'gallery_01', cursor: 'pointer', galleryActiveClass: "active"}); 
            //pass the images to Fancybox
            $j("#zoom_03").bind("click", function(e) {  
            var ez =   $j('#zoom_03').data('elevateZoom');	
		...
		if($Settings_arr['show_bookmarks'])
		{
			...
			<?php		$this->bookmarks_disc(url_product($row_prod['product_id'],$row_prod['product_name'],1),htmlspecialchars($meta_arr['title']),$ecom_twitteraccountId); 
			?>	</div>
			<?php	
		}  
		?>
...
<?php
$sql_prod_vid = "SELECT product_id,product_video_embed FROM products 
                        WHERE product_id=".$row_prod['product_id']."  
								AND sites_site_id=$ecom_siteid LIMIT 1";
$ret_prod_vid = $db->query($sql_prod_vid);								
$row_prod_vid = $db->fetch_array($ret_prod_vid);								
if($row_prod_vid['product_video_embed']!='')
{
?>
   <div class="pdleftvideo"><?php echo  $row_prod_vid['product_video_embed'];?></div>
  <?php
}
...
if($db->num_rows($ret_prodreview))
{
	...
	while ($row_prodreview = $db->fetch_array($ret_prodreview))
	{
		...
		$review_author 	= '<div class="review_auth_name">'.stripslashes($row_prodreview['review_author']).' ('.$row_prodreview['rvdate'].')'.'</div>';
 ...
 ?>
		...
		<div class="reivew_add">
		<a href='<?php url_link('writeproductreview'.$_REQUEST['product_id'].'.html')?>'><?=stripslash_normal($Captions_arr['PRODUCT_REVIEWS']['READ_PRODUCT_ADDREVIEW_LINK'])?></a>
		</div>
<?php
		echo $review_content;
?>															
  ...
  <div class="pdleftdetails">
  <?php 

	if($row_prod['product_show_pricepromise']==1)
	{
	?>
		<div class="pdbannerWrap"><img src="<?php url_site_image('price_promise_banner.png')?>" alt="" border="0"></div>
	<?php
	}
	?>
    <div class="pdPriceDetailswrap">
		<?php 
...
<div class="pdPriceDetailLeft"  itemprop="offers" itemscope itemtype="http://schema.org/Offer">
...
$price_class_arr['ul_class'] 		= 'prodeulprice';
		...
		$price_bal = show_Price($row_prod,$price_class_arr,'prod_detail');
		echo $price_bal;
		
		$price_class_arr['from_detailspage'] 	= 0;
				show_excluding_vat_msg($row_prod,'vat_div');// show excluding VAT msg
...
?></div>
	...
	$("#monthly_repayment").html("&pound;"+(my_fd_obj.m_inst).toFixed(2));
	$("#finpermonth").html("&pound;"+(my_fd_obj.m_inst).toFixed(2));
	$("#fin_purchasepr").html("&pound;"+(my_fd_obj.goods_val).toFixed(2));
	$("#dpamount").html("&pound;"+(my_fd_obj.d_amount).toFixed(2));
	$("#loanamount").html("&pound;"+(my_fd_obj.l_amount).toFixed(2));
	$("#totrepayable").html("&pound;"+(my_fd_obj.l_repay).toFixed(2));
		}
...
if($price_bal!='')
		{
		?>
		<div class="no_credits"><img src="<?php url_site_image('no-credit.png')?>" alt="" border="0"><span class="nocc_span">No Credit Card Charges</span></div>
		<?php
		}
?>
<script type="text/javascript">
			T$('testclick1').onclick = function(){TINY.box.show('<?php echo $ecom_selfhttp.$ecom_hostname?>/images/<?php echo $ecom_hostname?>/otherfiles/vat-relief.html',1,600,400,1)}
		</script>

<div class="pdPriceDetailRight">
<?php 
$this->show_offer_buttons($row_prod);	
?>
</div>
...
{
?>
<div class="class_short_new">
<div class='bottom_det_prod_title'><?php echo stripslashes($row_prod['product_name'])?></div>
<?php
echo $row_prod['product_shortdesc']. ' <a href="#overview_tabs" class="short_desc_readmore">'.$Captions_arr['PROD_DETAILS']['SHORT_READ_MORE'].'</a>';
?>
</div>
<?php
}
?>
    ...
    if ($cookieval)	
	{					
		$this->Show_Recently_Product($cookieval,$display_title);
...
function Show_Recently_Product($cookval,$title)
{
		...
		$prod_compare_enabled = isProductCompareEnabled();
					...
					<table border="0" cellpadding="0" cellspacing="0" class="shelfBtable">
					<tr>
					<td colspan="3" class="pro_de_shelfBheader" align="left">Recently Viewed Products</td>
					</tr>
					<tr>
					<td colspan="3">
...
function show_ProductVariables($row_prod,$pos='column')
{
				...
				?>
				<script type="text/javascript">
				function handle_show_value_link(prodid,docroot,var_id,loading_gif,retdivid)
				{ 	
					var varstr 	= '';
					var valid	= '';
					var qrystr = '';
					var val_idst = 'var_'+var_id;
					obj = eval("document.getElementById('"+val_idst+"')");
					valid = obj.value;
					fpurpose										= 'ajax_show_variable_link';

					document.getElementById(retdivid).innerHTML = '<img src="<?php echo url_site_image('proddet_loading.gif',1)?>" alt="loading..." width="43px" height="11px;align:left">';
					//var retdivid									= 'variable_linka';
					document.getElementById('ajax_div_holder').value = retdivid;
						retobj 											= eval("document.getElementById('"+retdivid+"')");
						/*show_loading('variable_linka_loading_div');*/
					pass_docroot = docroot+'/includes/base_files/products.php';
					Handlewith_Ajax(pass_docroot,'ajax_fpurpose='+fpurpose+'&'+qrystr+'&prod_id='+prodid+'&pass_var='+var_id+'&pass_valid='+valid);
				}
			</script>
				<?php
				// ######################################################
								...
								while ($row_var = $db->fetch_array($ret_var))
								{
										...
										if ($var_Proceed)// Show the variable if it is valid to show
										{
											...
											<?php
													...
													{	      $proc_link = false;
														      if($ecom_siteid==	106 or $ecom_siteid== 104 or $ecom_siteid== 113)
																{ 
																	$var_namea = strtolower(trim($row_var['var_name']));
																	if($var_namea=='insurance options' or $var_namea=='insurance policy term')
																	{
																		$proc_link = true;
																		$onchange_function = "onchange = 'handle_show_value_link(\"".$_REQUEST['product_id']."\",\"".SITE_URL."\",\"".$row_var['var_id']."\",\"".url_site_image('loading.gif',1)."\",\"variable_linka\")' ";
																	
																	}
																	else if ($var_namea=='warranty')
																	{
																	   $onchange_function = "onchange = 'handle_show_value_link(\"".$_REQUEST['product_id']."\",\"".SITE_URL."\",\"".$row_var['var_id']."\",\"".url_site_image('loading.gif',1)."\",\"variable_linkb\")' ";
														...
														{
														?>	
                                                        &nbsp;<a class="facts_class" href="javascript:show_pdf_new(&quot;http://docs.markbatesltd.com/latest/keyfacts/3_Month_Mobility_Insurance_Key_Facts.pdf&quot;)">Key Facts</a>
                                                        &nbsp;<a class="facts_class" href="javascript:show_pdf_new(&quot;http://dealer.premiercare.info/introducer/12771/TOB12771.pdf&quot;)">TOB</a>
                                                        <?php
														}
														elseif($var_namea=='insurance policy term')	
														{
														?>
														&nbsp;<a class="facts_class" href="javascript:show_pdf_new(&quot;http://docs.markbatesltd.com/latest/keyfacts/Mobility_Insurance_&_Warranty_Key%20Facts.pdf&quot;)">Key Facts</a>
                                                        &nbsp;<a class="facts_class" href="javascript:show_pdf_new(&quot;http://dealer.premiercare.info/introducer/12771/IDD12771.pdf&quot;)">TOB</a>
														<?php	
														}
														...
														if($var_namea=='warranty')	
														{
															?>
															<div class="variable_linkb" id="variable_linkb" style="padding-top:5px;">

															&nbsp;<a class="facts_class trigger_popup_fricc">Warranty</a>
															</div>
															<?php
														}
								...
								while ($row_msg = $db->fetch_array($ret_msg))
									{
										...
										$clssA = ($ui%2==0)?'productvariabletdB':'productvariabletdB';
										$clssB = ($ui%2==0)?'productvariabletdAB':'productvariabletdBB';
										
										$var_exists = true;
										?>
										<tr>
										<td align="left" valign="middle" class="<?php echo $clssA?>"><?php echo stripslashes($row_msg['message_title'])?></td>
										<td align="left" valign="middle" class="<?php echo $clssB?>">
										<?php
				...
				$ret_var_checkbox = $db->query($sql_var_checkbox);
									...
									<table width="100%" border="0" cellpadding="0" cellspacing="0" 	class="variabletable" id="proddet_var_table">
												<tr>
									<td align="left" valign="middle" class="pdvartd" colspan="2">
									<?php
									?>
...
function show_offer_buttons($row_prod)
{
					...
					if($ecom_siteid==106 or $ecom_siteid==104 or  $ecom_siteid==113)
					{
						...
						<?php
						if($row_prod['product_freedelivery']==11)
						{
						?>	
						<div class="pdIconWrap"><a href="<?php url_link('freedelivery'.$row_prod['product_id'].'.html')?>" title="Free Delivery"><div class="free_del_logo"></div></a></div>
						<?php
						}
						?>
						...
						*/ 
						...
						{
						?>
						<div class="pdIconWrap"><a href="<?php url_link('showallbundle'.$row_prod['product_id'].'.html')?>" title=""><div class="combo_del_logo"></div></a></div>
						<?php
						}
						...
						<div class="pdIconWrap"><img src="<?php url_site_image('icon_insurance.png')?>" alt="" border="0"></div>
						<?php
						}
						?>

						
						<div class="pdIconWrap"><img src="<?php url_site_image('engeneering_icon.png')?>" alt="" border="0"></div>
						<div class="pdIconWrap"><img src="<?php url_site_image('vat.png')?>" alt="" border="0"></div>
						<?php
					}
					else
					{
...
else
{
	$ecom_selfhttp = "http://";
					...
					if ($caption_key!='') // Check whether the buynow or preorder button is to be displayed
					{
							...
							<tr>
							<?php
							if($showqty==1 AND $row_prod['product_discontinue']==0)// this decision is made in the main shop settings
							{
					...
					<?php
					if ($row_prod['product_show_enquirelink']==1 || $Settings_arr['proddet_showwishlist'] || $row_prod['product_discontinue']==1)
					{
								?>
								...
								{
								?>
										<span class="discontinus"><?php echo $Captions_arr['PROD_DETAILS'][$caption_key];?></span>
										<?php	
								}
					...
					if(count($prodimg_arr)>0)// Case if more than one image assigned to current product
					{
						...
						{ 
							...
							if($sml_width and $sml_height and $big_width and $big_height)
							{
								...
								{
									$extralarge_img = url_root_image($v['image_bigcategorypath'],1);
								}
								else
							...
							*/?>
							<a href="#" data-image="<?php echo url_root_image($v['image_bigcategorypath'],1);?>" data-zoom-image="<?php echo $extralarge_img?>">
...
global $ecom_hostname,$ecom_themename,$db;
					...
					src="<?php echo $ecom_selfhttp.$ecom_hostname?>/images/<?php echo $ecom_hostname?>/scripts/swfobject.js"></script>
					<script type="text/javascript">
...
function Show_Image_Normal($row_prod)
					...
					*/ 
					...
					if($sml_width and $sml_height and $big_width and $big_height)
					{
						if ($big_width<$sml_width or $big_height<$sml_height)
						{
							$extralarge_img = url_root_image($prodimg_arr[0]['image_bigcategorypath'],1);
						}
						else
						...
						$extralarge_img = url_root_image($prodimg_arr[0]['image_extralargepath'],1);
					?>
					<img id="zoom_03" src="<?php echo url_root_image($prodimg_arr[0]['image_bigcategorypath'],1)?>" data-zoom-image="<?php echo $extralarge_img?>"/>
...
function bookmarks_disc($url, $title, $tweeter='',$ret=-1)
{
	$HTML_image ='';
	if(!$tweeter) {
	$tweeter = 'tweetmeme';
	...
	<a target="_blank" href="http://twitter.com/home?status=<?php echo urlencode($title." ".$url);?>" title="Spread the word on Twitter!"><img src="<? url_site_image('social_twitter_24.png');?>" alt="Tweet This" border="0" style="width:16px;height:16px;"></a>&nbsp;
	<?php /*<a target="_blank" href="http://del.icio.us/post?url=<?php echo $url; ?>&amp;title=<?php echo $title;?>" title="Bookmark with Del.icio.us"><img src="<? url_site_image('delicious.gif');?>" alt="Bookmark with Del.icio.us" border="0" style="width:16px;height:16px;"></a>&nbsp;*/ ?>
	<a target="_blank" href="https://del.icio.us/login?log=out&url=<?php echo $url; ?>&title=<?php echo $title;?>" title="Bookmark with Del.icio.us"><img src="<? url_site_image('delicious.gif');?>" alt="Bookmark with Del.icio.us" border="0" style="width:16px;height:16px;"></a>&nbsp;
	
	<a target="_blank" href="https://digg.com/welcome?url=<?php echo $url; ?>&amp;title=<?php echo $title;?>" title="Digg This!"><img src="<? url_site_image('digg.gif');?>" alt="Digg This!" border="0" style="width:18px;height:16px;"></a>&nbsp;
	<a target="_blank" href="http://reddit.com/submit?url=<?php echo $url; ?>&amp;title=<?php echo $title;?>" title="Post to Reddit"><img src="<? url_site_image('reddit.gif');?>" alt="Post to Reddit" border="0" style="width:17px;height:16px"></a>&nbsp;
	<a target="_blank" href="http://www.facebook.com/sharer.php?u=<?php echo $url; ?>" title="Share on Facebook"><img src="<? url_site_image('facebook.gif');?>" alt="Share on Facebook" border="0" style="width:16px;height:16px;"></a>&nbsp;
	<a target="_blank" href="https://mix.com/?url=<?php echo $url; ?>&amp;title=<?php echo $title;?>" title="Post to mix"><img src="<? url_site_image('mix.png');?>" alt="Post to mix" border="0" style="width:16px;height:16px"></a>&nbsp;
	<?php
	/*<a target="_blank" href="http://www.kaboodle.com/za/additem?get=1&amp;url=<?php echo $url; ?>&amp;title=<?php echo $title;?>" title="Post to Kaboodle"><img src="<? url_site_image('kaboodle.gif');?>" alt="Post to Kaboodle" border="0" style="width:16px;height:16px;"></a>&nbsp;
	<a target="_blank" href="http://myweb2.search.yahoo.com/myresults/bookmarklet?t=<?php echo $title;?>&amp;u=<?php echo $url; ?>" title="Bookmark with Yahoo"><img src="<? url_site_image('yahoo.gif');?>" alt="Bookmark with Yahoo" border="0" style="width:16px;height:16px"></a>&nbsp;*/?>
	<a target="_blank" href="http://www.google.com/bookmarks/mark?op=add&amp;bkmk=<?php echo $url; ?>&amp;title=<?php echo $title;?>" title="Bookmark with Google"><img src="<? url_site_image('google.gif');?>" alt="Bookmark with Google" border="0" style="width:16px;height:16px"></a>
	<?php
	if($HTML_image!='')
	{
	?>
	<a href="http://pinterest.com/pin/create/button/?url=<?php echo urlencode($url);?>&media=<?php echo $HTML_image?>&description=<?php echo $title?>" class="pin-it-button" count-layout="horizontal" target="_blank"><img border="0" src="//assets.pinterest.com/images/PinExt.png" title="Pin It" style="width:43px;height:21px"/></a>
	<?php
	}
