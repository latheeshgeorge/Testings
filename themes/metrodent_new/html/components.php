...
/*function showSubCateImg(imgVal)
{
	if(imgVal != '')
	{
		var imgePath	=	'<img src="<?php echo SITE_URL?>/images/<?php echo $ecom_hostname;?>/'+imgVal+'" border="0">';
		alert(imgePath);
		document.getElementById('cate_menu_img').innerHTML = ''
		document.getElementById('cate_menu_img').innerHTML = imgePath;
	}
	else
	{
		document.getElementById('cate_menu_img').innerHTML = '';
...
function showSubSubCateImg(showSubSubCateImg)
{
	if(imgVal != '')
	{
		var imgePath	=	'<img src="<?php echo SITE_URL?>/images/<?php echo $ecom_hostname;?>/'+imgVal+'" border="0">';
			  ...
			  foreach ($advert_arr as $d=>$k)
					{
						...
						switch ($k['advert_type'])
						{
							...
							?>
                            	<div class="htmladswrap_side">
                                    <div class="advert_header_side"><?php echo $title?></div>
                                    <div class="htmlads_bg_side">
                                        <div class="htmladscontent_side">
		...
		?>
			<tr>
			  <td colspan="3" class="userloginmenuytop" >
				...
				<li>
					<h2>
					<a href="http://<?=$ecom_hostname?>/contact-us-pg50051.html" class="userloginmenuytoplink">
					Contact Metrodent					
					</a>
					</h2>
				  </li>
...
<?php
				...
				?>
				  ...
				  <?php
					...
					<a href="http://<?=$ecom_hostname?>/contact-us-pg50051.html" class="seo_links">
					Contact Metrodent					
					</a>
...
<?
		  ...
		  function mod_featured($ret_featured,$title)   // Function to show the payonaccount banner
		 { 
			       global $ecom_siteid,$db,$ecom_hostname,$Settings_arr,$Captions_arr,$position;
					if($_REQUEST['req']=='' && $position=='middle')
					{
					...
					if($title!='')
					{
					$HTML_comptitle = '<div class="featured_top"><div class="featured_title_text">'.$title.'</div></div>';
					}
					// Title
					...
					if(count($img_arr))
					{
					//$HTML_image .= show_image(url_root_image($img_arr[0][$fld_name],1),'','','','',1);
					$HTML_image .=show_image(url_root_image($img_arr[0][$fld_name],1),$row_featured['product_name'],$row_prod['product_name'],'','featsp_dental',1);
					}
					else
					{
					...
					if ($no_img)
					{
						$HTML_image .=  show_image($no_img,$row_featured['product_name'],$row_featured['product_name'],'','featsp_dental',1);
					}       
					}       
					$HTML_image .=  '</a></div>';
					}
					...
					<?php
					 if($price_arr['disc'])
						...
						?>
							...
							<div>
							<?php
							echo $Captions_arr['COMMON']['FEAT_OFF']." ".$price_arr['disc'];
							?>
							</div>
							</div>
							</div>
						...
						{
						?>
							<div class="featured_offA">
							<div class="featured_off_inA">
					...
					<?php echo $HTML_desc;?>
						<div class="featured_buy">
							<div class="featured_buy_otr">
								...
								<table style="width:500px" cellpadding="2" cellspacing="2" border="0" style="float:right">
								<tr>
								<td width="70%" align="right">
								<?php
									$class_arr 					= array();
									$class_arr['ADD_TO_CART']	= 'cupid-green_new';
									$class_arr['PREORDER']		= 'cupid-green_new';
									$class_arr['ENQUIRE']		= 'cupid-green_new';
									show_addtocart($row_featured,$class_arr,$frm_name,false,'','','',1)
								?>
								</td>
								<td width="30%" align="right">
								
								<a class='cupid-green_new' href="<?php url_product($row_featured['product_id'],$row_featured['product_name'],-1)?>" title=""><?php echo $Captions_arr['COMMON']['NEW_MOREINFO']?> &#187&#187</a>
								</td>
								</tr>
								</table>	
