		...
		?>
			...
			<?php
		        ...
		        $fetch_arr_admin 	= $db->fetch_array($res_admin);
					if($fetch_arr_admin['general_savedsearch_topcontent']!='')
					{
						?>
						<tr>
								<td align="left" valign="top"  class="shelfBproddes" ><?php echo $fetch_arr_admin['general_savedsearch_topcontent']?>
								</td>
						</tr>			
						<?php
					}
			...
			$count_sav=1;
				while ($row_search = $db->fetch_array($ret_search))
				{
				  $count_sav++;						
			?>					
					<td valign="middle" align="left" class="savedsearch_td"> 
					<h3><a href="<?php url_savedsearch($row_search['search_id'],$row_search['search_keyword'])?>" title="<?=$row_search['search_keyword']?>" class="link"><?=$row_search['search_keyword']?></a></h3> </td>
			<?php
			...
			if($fetch_arr_admin['general_savedsearch_bottomcontent']!='')
			{
				?>
				<tr>
					<td align="left" valign="top"  class="shelfBproddes" ><?php echo $fetch_arr_admin['general_savedsearch_bottomcontent']?>
					</td>
				</tr>			
				<?php
			}
