    ...
    function show_combo_product_details($row_prod,$keepselected=false,$disp_type='',$cur_col,$max_col)
	{
		...
		switch($disp_type)
		{
			...
			<td class="shelfAtabletd" align="left" valign="top" onmouseover="this.className='normalshelf_hover'" onmouseout="this.className='shelfAtabletd'">
				<div><h1 class="shelfAprodname_combo"><a href="<?php url_product($row_prod['product_id'],$row_prod['product_name'],-1)?>" title="<?php echo stripslashes($row_prod['product_name'])?>"><?php echo stripslashes($row_prod['product_name'])?></a></h1></div>
					<div class="selected_li">
						<?php
					...
					<div class="shelfAnormalprice_combo">
						<?php 
							echo "Deal price ".print_price($row_prod['combo_discount'],true,false);
						?>
					</div>
				<div class="shelfimg_combo">
							...
							while ($row_comb_det = $db->fetch_array($ret_comb_det))
							{
								...
								<tr>
									...
									<td align="left" valign="middle" class="mid_comboshelfBtabletd">
										...
										{
												echo ': ';?>
												<img  src="<?php url_site_image('selected_img_combo.gif')?>" border="0" />
											<?php
											if($comb_id==0)
											{
												?>
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="3">
							...
							<td align="center" valign="middle" class="shelfBtabletd_combo">
								...
								<td align="left" valign="middle" class="shelfBtabletd_combo">
									<div class="shelfBnormalprice_combo">
									<?php 
												echo "Deal price ".print_price($row_prod['combo_discount'],true,false);
									?>
									</div>
									...
									$ret_comb_det = $db->query($sql_comb_det);
									?>
									<div class="var1row_combo"><table width="100%" border="0" cellpadding="0" cellspacing="3">
									<?php
									// Case of variables
										if ($db->num_rows($ret_comb_det) )
										{
	...
	function Show_Combo_Multiple($ret_combos,$combprod_id,$deal_mod)
	{
		...
		?>
			...
			$fetch_arr_admin 	= $db->fetch_array($res_admin);
				if($fetch_arr_admin['general_comboall_topcontent']!='')
				{
					?>
					 <tr>
							<td align="left" valign="top"  class="shelfBproddes" colspan="3" ><?php echo $fetch_arr_admin['general_comboall_topcontent']?>
							</td>
					</tr>			
					<?php
				}
		...
		while ($row_combos = $db->fetch_array($ret_combos))
		{
			...
			$sql_prod = "SELECT a.product_id,a.product_name,a.product_variablestock_allowed,a.product_show_cartlink,
				...
				$cur_tr++;
				?>
						<td class="combo_multi_td" valign="top">
						<div class="lf_combodealA">
							<?php
		...
		echo "<td colspan='".$colsp."' class='combo_multi_td' valign='top'>&nbsp;</td></tr>";
		}
		if($fetch_arr_admin['general_comboall_bottomcontent']!='')
		{
			?>
			<tr>
				<td align="left" valign="top"  class="shelfBproddes"  colspan="3"><?php echo $fetch_arr_admin['general_comboall_bottomcontent']?>
				</td>
			</tr>			
			<?php
		}
