<?php
  ...
  <span class="breadcrumb-item active">'.stripslash_normal($Captions_arr['ENQUIRE']['ENQUIRE_MAINHEADING']).'</span>
</nav></div>';
  ...
  echo $HTML_treemenu;	
		...
		while($row_select_enq = $db->fetch_array($ret_select_enq))
		{
		 ...
		 $prod_sql_det = "SELECT product_id,product_name,product_default_category_id,product_webprice,
		  ...
		  ?>
			  ...
			  <tr>
			 <td  align="left" valign="middle"   class="<?=$trmainclass?>" >Add Notes:<br /><textarea  name="enquirenote_<?=$row_select_enq['enquiry_id']?>" rows="5" cols="30" id="note_text_<?=$row_select_enq['enquiry_id']?>" class="add_notearea"><? if($_REQUEST['enquirenote_'.$row_select_enq['enquiry_id']]) echo $_REQUEST['enquirenote_'.$row_select_enq['enquiry_id']];else echo $row_select_enq['enquiry_note'];?></textarea>&nbsp; <input name="buttonenqsaveone_<?=$row_select_enq['enquiry_id']?>" type="submit" class="btn btn-lg tn-block regbt" id="buttonenqsaveone_<?=$row_select_enq['enquiry_id']?>" value="Save Note" /></td>
			 </tr></table></td>
			  </tr>
		  <?php
		...
		<?php
				...
				?>  
             		...
             		<input name="buttonenq1" type="button" class="btn btn-lg redbutton" id="buttonenq1" value="Submit Enquiry" onclick="location.href='<?php url_link('fillenquiry.html')?>'"/>
					</div> 
	...
	function enquiry_Fillform($alert,$cust_id,$prod_cart_arr)
	{
		  ...
		  ?>
			...
			$HTML_treemenu = '	<div class="breadcrump"><nav class="breadcrumb">
  <a class="breadcrumb-item" href="'.url_link('',1).'" title="'.stripslash_normal($Captions_arr['COMMON']['TREE_MENU_HOME_LINK']).'">'.stripslash_normal($Captions_arr['COMMON']['TREE_MENU_HOME_LINK']).'</a>
  <a class="breadcrumb-item" href="'.url_link('enquiry.html',1).'" >'.stripslash_normal($Captions_arr['ENQUIRE']['ENQUIRE_MAINHEADING']).'</a>

  <span class="breadcrumb-item active">'.stripslash_normal($Captions_arr['ENQUIRE']['ENQUIRE_DET_MAIN']).'</span>
</nav></div>';
			?>
			  		 <div class="container-fluid">
			...
			?>
			  ...
			  */?> 
			  <div class="form-group">
																<label class="sr-only" for="form-txt_enqfname"><?php echo $row_cust['customer_fname']?></label>
...
<input name="txt_enqlname" class="form-control" value="<?=$row_cust['customer_surname']?>" type="text" maxlength="<?=$short?>" placeholder="<?=stripslash_normal($Captions_arr['ENQUIRE']['ENQUIRE_DET_LNAME'])?>">
			  ...
			  */
			  ...
			  ?> 			
<div class="form-group">
<label class="sr-only" for="form-txt_enqfname"><?php echo $row_cust['customer_phone']?></label>
<input name="txt_enqphone" class="form-control" value="<?=$row_cust['customer_phone']?>" type="text" maxlength="<?=$short?>" placeholder="<?=stripslash_normal($Captions_arr['ENQUIRE']['ENQUIRE_DET_PHONE'])?>">
			  ...
			  */
			  ?> 			  
			  <div class="form-group">
<label class="sr-only" for="form-txt_enqfname"><?php echo $row_cust['customer_email_7503']?></label>
<input name="txt_enqemail" class="form-control" value="<?=$row_cust['customer_email_7503']?>" type="text" maxlength="<?=$short?>" placeholder="<?=stripslash_normal($Captions_arr['ENQUIRE']['ENQUIRE_DET_EMAIL'])?>">
			...
			$table_class            = 'reg_table';			
				...
				include 'show_dynamic_fields.php';
			?>
			<input name="enquiry_submit" type="submit" class="btn-primary-bt topcart-bt" id="enquiry_submit" value="Send" />
			 </div>
			<?php	
				$cur_pos 				= 'Bottom';
				$section_typ			= 'enquire'; 
...
<?php	
  echo $HTML_treemenu;
	    ...
	    <div class="row-container">
				...
				<tr>
				<td align="left" valign="middle" class="regicontentAA"><?php echo $message; ?></td>
				</tr>
				</table>
			</div>
			</div>
  ...
  <span class="breadcrumb-item active">'.stripslash_normal($Captions_arr['ENQUIRE']['ENQUIRE_MYENQUIRYHEADING']).'</span>
</nav></div>';
  echo $HTML_treemenu;	
				?>
				<?php
		    ...
		    document.getElementById("datetrtd").className += " ordoptionsA";
				...
				<div class="ord_list_main">
					...
					<div class="ord_listing">
						<table width="100%" class="table search_areaTable table">
							<tr>
								<td class="ord_bet_date">Date between</td>
								...
								<td class="ord_sort_option"><?php echo $sort_option_txt;?></td>
								<td class="ord_sort_by"><?php echo $sort_by_txt?></td>
						...
						<table width="100%" class="table table-bordered table-striped cart-page">
							<tr>
								<td>#</td>
								<td><?php echo stripslash_normal($Captions_arr['ENQUIRE']['ENQUIRE_DATE'])?></td>
								<td><?php echo stripslash_normal($Captions_arr['ENQUIRE']['ENQUIRE_STATUS'])?></td>
							</tr>
							<?php
							if($db->num_rows($ret_select_enq))
							{  
								...
								{?>
									<tr onclick="document.my_enquire.enq_mod.value='details_my_enquiry';document.my_enquire.enquiry_id.value='<?=$row_select_enq['enquiry_id']?>';document.my_enquire.submit();" style="cursor:pointer" class="edithreflink_tronmouse" title="View Details" onmouseover="this.className='edithreflink_trmouseout'" onmouseout="this.className='edithreflink_tronmouse'">
										<td class="ordertabletdcolorB" ><?php echo $srno++?></td>
										<td class="ordertabletdcolorB" ><?=$row_select_enq['added_date']?></td>
										<td class="ordertabletdcolorB"><?=$row_select_enq['enquiry_status']?></td>
									</tr>
									<?php 
								}
								$pg_variable				= 'search_pg';
								...
								if ($tot_cnt>0)
								{
									if($paging['total_cnt'])
									{
										echo '<tr><td colspan="3">'.$paging['total_cnt'].'</td></tr>';				
									}
								?>
									<tr>
										<td  colspan="3">
...
<?php
  echo $HTML_treemenu;	
			...
			<table width="100%" border="0" class="table">
				<tr>
					<td>Date:</td>
					<td><?=$row_enq['added_date']?></td>
				</tr>
				<tr>
					<td>Status:</td>
					<td><?=$row_enq['enquiry_status']?></td>
				</tr>
				
				<?php if($row_enq['enquiry_status']=='NEW' || $row_enq['enquiry_status']=='PENDING')
				{
				?>
				<tr>
					<td>
						<input name="clearenq_button" type="button" class="redbutton" id="clearenq_button" value="<?php echo stripslash_normal($Captions_arr['ENQUIRE']['ENQUIRE_CANCEL'])?>" onclick="if(confirm_message('Are you sure you want to cancel the Enquiry?')){document.my_enquire_det.enq_mod.value='cancel_enquiry';document.my_enquire_det.enquiry_id.value='<?=$row_enq['enquiry_id']?>';document.my_enquire_det.submit();}" />
					</td>
			...
			<table width="100%" border="0" class="table table-bordered table-striped cart-page">
				<tr>
					<td><?php echo stripslash_normal($Captions_arr['ENQUIRE']['ENQUIRE_ITEM'])?></td>
					<td><?php echo stripslash_normal($Captions_arr['ENQUIRE']['ENQUIRE_DISC'])?></td>
				</tr>
			
			
			<?php if($db->num_rows($resp))
			{ 
				while($rowp = $db->fetch_array($resp))
				{
					...
					<tr onclick="window.location='<?php echo url_product($rowpr['product_id'],$rowpr['product_name'])?>'">
						<td class="<?=$trmainclass?>"> 
							<a href="<?php echo url_product($rowpr['product_id'],$rowpr['product_name'])?>" title="<?php echo $row_prod_det['product_name']?>" class="favoriteprodlink"><? echo $rowpr['product_name']; ?></a>
						<?php
							if($rowp['product_text']!='')
							{
							?>
							<br/><?= nl2br($rowp['product_text'])?>
							<?
							}
						?>
						<?php if($vars_exists){ 
						...
						if($db->num_rows($resv)){
							while($rowv = $db->fetch_array($resv))
							{
								...
								if (trim($rowv['variable_value'])!='')
												print "<br/><span class='cartvariable'>".stripslash_normal($rowv['variable_name']).": ". stripslash_normal($rowv['variable_value'])."</span><br />"; 
											else
												print "<br/><span class='cartvariable'>".stripslash_normal($rowv['variable_name'])."</span>"; 
						...
						if ($db->num_rows($ret_msgs))
						{
							while($row_msgs = $db->fetch_array($ret_msgs))
							{
								if($row_msgs['message_value']!='') {
								  print "<br/><span class='cartvariable'>".stripslash_normal($row_msgs['message_caption']).": ". stripslash_normal($row_msgs['message_value'])."</span><br />"; 
								}
							}
						}?>
						...
						<td class="<?=$trmainclass?>" ><?php echo $rowpr['product_discount']?></td>
					</tr> 
