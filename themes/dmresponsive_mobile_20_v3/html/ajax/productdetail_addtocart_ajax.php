...
{ 	
	global $Captions_arr,$ecom_siteid,$db;
		...
		if($db->num_rows($ret_select_enq)>0)
		{
          ...
          <h2 >My Enquiry(<?php echo  ($row_select_enq['cnt'])?$row_select_enq['cnt']:0?>)</h2>
        							<div class="alert alert-success fade in red_alert" id="red_alert" style="display:none;" ><span  >Item added to the Enquiry</span></div>

			<p><a href="javascript:void(0)" class="checkout-btn">Continue Shopping</a>
			</p>
			<p class="cd-go-to-cart"><a href="<?php echo SITE_URL?>/enquiry.html"><input name="continue_submit" class="btn btn-add-to-cart btn-lg sharp" id="continue_submit" value="View Enquiry"  type="button" onclick="window.location='<?php echo SITE_URL?>/enquiry.html'"></a>
			</p>			
			</div>        
		  <?php
		  break;
