<?php
	function show_BulkDiscounts_prod_detail($product_id,$var_arr=array())
	{
		...
		if ($db->num_rows($ret_prod))
		{
			...
			<?php if (count($bulkdisc_details['qty']))
			{
				
			?>		<div class="product-table table-responsive"><table class="qty-discount-table">
			<thead>
				<tr><th>Qty</th>
		<?php
	...
	function show_Variable_price_detail($product_id,$var_arr=array())
	{
		...
		function show_Variable_main_image($product_id,$var_arr=array())
		{
			...
			{
				$sql_prod = "SELECT product_name,product_id,product_variablecombocommon_image_allowed,default_comb_id     
							FROM 
								products 
							WHERE 
								sites_site_id = $ecom_siteid 
				...
				{
					$exclude_tabid = $prodimg_arr[0]['image_id']; 		// exclude id in case of multi images for products
				...
				show_Variable_more_images($row_prod['product_id'],0,$var_arr);
				?>
				</div>	
    ...
    function show_Normal_price_detail($product_id,$var_arr=array(),$cur_qty=1)
	{
		...
		if ($db->num_rows($ret_prod))
		{
			...
			$price_arr = show_Price_Productdetail_Normal($row_prod,$var_arr,$price_class_arr,$cur_qty);
			...
			if($price_arr['price_only']['discounted_price'])
			{
				...
				$cur_price = printcurr(($price_arr['price_only']['discounted_price'] * $cur_qty),$curr_sign);
				//$sav_price = $price_arr['price_only']['disc_percent'];
			}
			else
			{
				//$was_price = '';
				...
				$cur_price = printcurr(($price_arr['price_only']['base_price'] * $cur_qty),$curr_sign);
				//$sav_price = '';
			}
			
			$normal_prefix 	= 'Total Price';//($row_prod['price_fromprefix'])?$row_prod['price_fromprefix']:$PriceSettings_arr['price_fromprefix'];
			
			$HTML_price = '<div class="deat_price">';
			if($was_price)
				$HTML_price .= '<div class="deat_priceA">'.$normal_prefix.' '.$was_price.' + VAT</div>';
			if($cur_price)
				$HTML_price .= '<div class="deat_priceB">'.$normal_prefix.' '.$cur_price.' + VAT</div>';	
			if($sav_price)
		...
		function show_ajax_product_barcode($product_id,$var_arr=array())
	{
		global $db,$ecom_siteid,$Captions_arr;
		$Captions_arr['PROD_DETAILS'] 	= getCaptions('PROD_DETAILS');
		$barcode = '';
	...
	function show_pricereload_detail($product_id,$var_arr=array())
	{
		global $db,$ecom_siteid,$Captions_arr;
		$Captions_arr['PROD_DETAILS'] 	= getCaptions('PROD_DETAILS');
		$sql_prod = "SELECT product_id,product_name,product_variablestock_allowed,product_show_cartlink,product_show_enquirelink,
		...
		if ($db->num_rows($ret_prod))
		{
			...
			if(count($var_arr))
			{
				foreach ($var_arr as $kkk=>$vvv)
				{
					$validpass_arr[] = $vvv;
				}
				$validpass_str = implode(',',$validpass_arr);
				
				//echo $validpass_str;
			
				$row_prod['cur_var_value_id'] = $validpass_str;
			...
			*/
			...
			$save_price = str_replace('+ VAT','',$price_arr['prince_without_captions']['yousave_price']);
										...
										$curprice_tax_cap = curprice_tax($price_arr,$row_prod);
										  ?>
										<p class="price-details"><?php
			echo '<div id="price_holder1">';
