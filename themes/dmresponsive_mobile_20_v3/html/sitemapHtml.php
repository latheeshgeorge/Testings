	...
	class sitemap_Html
	{
		...
		$HTML_treemenu = '	<div class="breadcrump">
					<nav class="breadcrumb">
						 ...
						 <span class="breadcrumb-item active">'.stripslash_normal($Captions_arr['SITEMAP']['SITEMAP_TREEMENU_TITLE']).'</span>					</nav>
		...
		if(is_array($_SESSION['site_map_links']))
		{
			...
			?>
				...
				<?php
				 ...
				 {
				?>
					<a href="<?php echo $ecom_selfhttp.$ecom_hostname?>/sitemap.html?start_key=<?=$prev_start?>" title="Prev"> &laquo; Prev  </a>&nbsp;&nbsp;
				<? 
				}
				if(is_array($_SESSION['site_map_links'][$next]))
				{
				?>
					<a href="<?php echo $ecom_selfhttp.$ecom_hostname?>/sitemap.html?start_key=<?=$link_key?>" title="Next"> Next &raquo;</a> 
				<? 
				}
				?>
				</td>
