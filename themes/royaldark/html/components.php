                                           ...
                                           <?php
													while ($row_cat = $db->fetch_array($ret_cat))
													{
														...
														if ($cat_desc!='')
														{
															...
															echo $HTML_catdesc;
														?>
														<div class="sub_cate_mid_more"> <a href="<?php url_category($row_cat['category_id'],$row_cat['category_name'],-1)?>" class="cate_mid_ul_a" title="<?php echo stripslash_normal($row_cat['category_name'])?>"><img src="<?php url_site_image('ca-more.gif')?>" width="49" height="12" /></a></div>
														</div>
														<?php
															$cnt++;
														   if($cnt>=$max_cnt)
				  ...
				  <div class="top_news_con_in">
					...
					?>
						<div class="top_news_con_left">
						  	<a href="<?php url_link('custlogin.html')?>" title="<?php echo stripslash_normal($Captions_arr['CUST_LOGIN']['LOGIN'])?>">
								<?=stripslash_normal($Captions_arr['CUST_LOGIN']['LOGIN'])?>
							</a>
					<!--FB Login Button -->
					<?php if($ecom_fb_enable == 1) { ?>
                    <div class="fb_login_btn"><a href="<?php echo $loginUrl;?>" class="fb_login_link">Login with FB</a></div>
                    <?php } ?>
						</div>	
                        <?
                        }
						...
						if($cache_exists!=true)
						{
							...
							{	
								if ($position == 'right-middle-band') // ############## Left ##############
								{
									...
									if ($groupData['shopbrandgroup_listtype'] == 'Menu1')// case if shops are to be shown in list menu
									{
										// Do the following only if caching is not enabled or cache does not exists
										if ($cache_exists==false)
										{
...
<?php 
  ...
  <?php	
                           ...
                           <?php
							$path = url_root_image('adverts/'.$k['advert_source'],1);
							$link = $k['advert_link'];
							$flash_path =  '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,28,0" width="930" height="116">
							<param name="movie" value='.$path.'  >
    						<param name="quality" value="high" >
							<param name="BGCOLOR" value="#D6D8CB"><embed src='.$path.' type=application/x-shockwave-flash width = 930 height=116> </object>';
							$img_link=  '';
							echo  $flash_path ;
							?>
                            </div>
			...
			<tr>
			  <td colspan="3" class="userloginmenuytop" >
				...
				<?php
					...
					{
				?>
				<li><h2><a href="<?php url_link('logout.html')?>" class="userloginmenuytoplink"><?=$Captions_arr['LOGIN_MENU']['LOGOUT']?></a></h1></li>
				<?php
					}
				?>
				   <li>
