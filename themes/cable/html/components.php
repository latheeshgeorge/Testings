						...
						<ul id="main_navigation">
							...
							if(count($grp_array))
							{
								...
								while ($row_pg = $db->fetch_array($ret_pg))
								{
									$target = ($row_pg['page_link_newwindow']==1 and $row_pg['page_type']!='Page')?'target="_blank"':'';
								?>
									<li class="item1"><h2><a href="<?php if ($row_pg['page_type']=='Page') url_static_page($row_pg['page_id'],$row_pg['title']); else echo $row_pg['page_link'];?>" class="category_menu" title="<?php echo stripslash_normal($row_pg['title'])?>" <?php echo $target?>><span><?php echo stripslash_normal($row_pg['title']);?></span></a></h2></li>
								<?php
								}
								if ($grpData['group_showxmlsitemaplink']==1 )
								{
