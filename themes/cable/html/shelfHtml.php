				...
				foreach ($shelf_arr as $k=>$shelfData)
				{
					...
					if ($proceed)
					{
						...
						if ($_REQUEST['req']!='')// LIMIT for products is applied only if not displayed in home page
						{
							...
							if($shelf_for_inner==true) /* Case if call is to display shelf at the bottom on inner pages*/
							{
							 if($shelfData['shelf_displaytype']=='3row')
							 $Limit = ' LIMIT 3';
							 else if($shelfData['shelf_displaytype']=='4row')
							 $Limit = ' LIMIT 4';
							}
							else
						...
						{
							if($shelfData['shelf_currentstyle']=='nor' and $shelfData['shelf_displaytype']=='3row')
								$Limit = ' LIMIT 4';
							elseif($shelfData['shelf_currentstyle']=='nor' and $shelfData['shelf_displaytype']=='4row')
								$Limit = ' LIMIT 4';
							elseif($shelfData['shelf_currentstyle']=='sp1' and $shelfData['shelf_displaytype']=='3row')
								$Limit = ' LIMIT 1';
							elseif($shelfData['shelf_currentstyle']=='sp1' and $shelfData['shelf_displaytype']=='4row')
								$Limit = ' LIMIT 3';	
                        }                       
								...
								switch($shelfData['shelf_displaytype'])
								{
									...
									?>
										...
										while($row_prod = $db->fetch_array($ret_prod))
										{
											...
											{
											   $HTML_free ='<div class="normal_shlfA_freeC"></div>';
											}
										...
										?>
											...
											<div class="normal_shlfA_freeC"></div>
											<?php
											//echo $HTML_free;
											?>
						...
						<?php			$class_arr['ADD_TO_CART']       = '';
								...
								<?php		
									...
									case '4row': // case of three in a row for normal
										$HTML_comptitle = $HTML_maindesc = $HTML_paging = $HTML_showall = $HTML_free = '';
										if($cur_title!='')
										{
									...
									?>
										...
										while($row_prod = $db->fetch_array($ret_prod))
										{
											...
											{
												$HTML_bonus = '<div class="normal_shlfA_pdt_bonus"><span>Bonus: '.$row_prod['product_bonuspoints'].'</span></div>';
											}
											else
											{
												$HTML_bonus = '&nbsp;';
										...
										?>
											...
											echo $HTML_title;
											?>
											<div class="normal_shlfA_pdt_des"><?php echo $HTML_desc?></div>
											<?php /* 
											...
											*/ ?> 
											...
											<div class="normal_shlfA_pdt_bottom"></div>
											</div> 
											<?
											$cur_col++;
											if($cur_col>=$max_col)
							...
							elseif($shelfData['shelf_currentstyle']=='inner_listing') // case of shelf to be displayed in inner pages
							{
								switch($shelfData['shelf_displaytype'])
								{
									case '3row': // case of three in a row for inner pages
										$HTML_comptitle = $HTML_maindesc = $HTML_paging = $HTML_showall = '';
										if($cur_title!='')
										{
											   $HTML_comptitle ='  <div class="normal_shlfB_header">
										...
										{
											 $desc = stripslashes($desc);
											 $HTML_maindesc = '<div class="normal_shlfB_desc_outr_n">'.$desc.'</div>';
										}
										...
										if ($tot_cnt>0 and ($_REQUEST['req']!='') and $tot_cnt>$prodperpage and $shelf_for_inner==false)
										{
											$paging 		= paging_footer_advanced($path,$query_string,$tot_cnt,$start_var['pg'],$start_var['pages'],'',$pg_variable,'Products',$pageclass_arr);
											$HTML_paging	= '	
											<div class="subcat_nav_pdt_no_shelf"><span>'.$paging['total_cnt'].'</span></div>
                                        ...
                                        if($row_prod['product_bonuspoints']>0 and $shelfData['shelf_showbonuspoints']==1)// Check whether description is to be displayed)
                                        {
						...
						<?php
									...
									case '4row': // case of three in a row for normal
										...
										if($tot_cnt>0 and $_REQUEST['req']=='') // case of show all link
										{
										   $HTML_showall = "<div class='normal_midA_showall'><a href='".url_shelf_all($shelfData['shelf_id'],$shelfData['shelf_name'],1)."' title=''>".$Captions_arr['COMMON']['SHOW_ALL']."</a></div>";
										}
										                                    echo $HTML_paging;
					...
					<?php		
							...
							elseif($shelfData['shelf_currentstyle']=='sp1') // case of special layout 1
							{
								switch($shelfData['shelf_displaytype'])
								{
									...
									<?php	
									break;
									case '4row': // case of two in a row 
										$HTML_comptitle = $HTML_maindesc = $HTML_paging = $HTML_showall = '';
										if($cur_title!='')
										{
