		...
		function show_Variable_more_images($product_id,$exclude_prodid,$var_arr=array())
		{
				...
				?>	
					  <table width="100%" border="0" cellpadding="0" cellspacing="3" class="productdethumbtable">
						...
						<ul class="hoverbox">
						<?php
						if ($pass_type=='image_gallerythumbpath') // If the more image type is Thumb then show 3 in a row otherwise show 2 in a row
						{
							$maximg_col 	=1;
							$width				= '100%';
						}	
						...
						foreach ($prodimg_arr as $k=>$v)
						{
							...
							if ($img_det!='')
								$img_det .= '~';
								$imgs_arr = explode('gallerythumb/',$v[$pass_type]);
							$img_det .= $imgs_arr[1];
