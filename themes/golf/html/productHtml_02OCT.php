<?php
	...
	<?php /*?>	<form method="post" name="frm_special_comp" id="frm_special_comp" action="<?php url_productcompare($prodId,$_REQUEST['product_id'],'','')?>#comp_prods" class="frm_cls">
		...
		<table border="0" cellpadding="0" cellspacing="0" class="productdeatilstable">
          ...
          <tr>
			...
			{
				$this->show_Instock_msg($row_prod);
			}	
		 ...
		 ?>
				...
				<tr>
					<td width="2%" align="left" valign="top" class="productdetails_img_td"><img src="<?php url_site_image('pro_link_det_left.gif')?>" title="left" alt="left"/></td>
					<td colspan="2" class="productdetdA" align="right">
						<?php
							...
							{  
							?>
								<a href="../pdf/product.php?product_id=<?PHP echo $_REQUEST['product_id']; ?>&category_id=<?PHP echo $_REQUEST['category_id']; ?>" class="productdetailslink"><?php echo $Captions_arr['PROD_DETAILS']['PRODDET_DOWNLOADPDF'];?></a>
							<?php
							}
							?>
					</td>
					<td width="2%" align="right" valign="top" class="productdetails_img_td"><img src="<?php url_site_image('pro_link_det_right.gif')?>" title="right" alt="right"/></td>
				</tr>
				</table></td>
				</tr>
          <?php				
				...
				if($prod_img_show_type=='FLASH' or $prod_img_show_type=='FLASH_ROTATE')
				{
					...
					if($prod_img_show_type=='FLASH') // case of showing images in a flash container
					{
						...
						<script type="text/javascript">
							...
							document.write('<param name="movie" value="http://<?php echo $ecom_hostname?>/themes/<?php echo $ecom_themename?>/swf/xml4.swf?filepath=http://<?php echo $ecom_hostname?>/getflashimage_images.php?prod_tab_id=<?=$prod_tab_id?>" />');
	...
	<?php	
		...
		?>
			...
			<script type="text/javascript">
				function handle_proddet_compare()
				{
					...
					if (checked_comp=='')
						alert('Please select the product(s) to compare');
					else if(totcnt>maxcnt)
						alert('Sorry!! only '+(maxcnt)+' products can be compared at a time');
					else
					{
				...
				<table border="0" cellpadding="0" cellspacing="0" class="shelfBtable">
				<tr>
					<td colspan="3" class="pro_de_shelfBheader" align="left"><a name="comp_prods"><?php echo $Captions_arr['PROD_DETAILS']['PRODDET_COMPARE_HEAD'];?></td>
				</tr>
				<tr>
		...
		/* Function to show the bulk discount*/
		function show_Instock_msg($row_prod)
		{
			global $db,$ecom_siteid,$Captions_arr,$ecom_hostname;
			$show_notify = false;
			// Section to show the bulk discount details
			if ($row_prod['product_stock_notification_required']=='Y')
			{
				...
				if($show_notify) // Check whether notificataion link is to be shown
				{
					...
					?>
						...
						<div align="right" style=" padding: 0 0 5px 0;" ><a href="javascript: hide_instockmsg_div()"><img src="<?php url_site_image('close.gif')?>" border="0" /></a></div>
							This product is out of stock.
						 <br />
						<span style="font-size:12px;font-weight:normal;color:#000000;">
						If you want to receive email notification when this product is in stock, then give your Email ID 
						</span>
					...
					{
			?>	
						<div  class="alert_inner">This product is out of stock.
						<br />
						<span style="font-size:12px;font-weight:normal;color:#000000;">If you want to receive email notification when this product is in stock, then give your Email ID
						<form action="" method="post" onsubmit="return validate_stocknotify(this)">	 
						...
						<span style="font-weight:bold;"><a href="javascript:handle_instocknotification('<?php echo $row_prod['product_id']?>','<?php echo $ecom_hostname?>')" style="color:#000000;text-decoration:underline">click here</a></span>  
						</div>
			<?php
					}		
				}
		...
		function show_buttons($row_prod)
		{
			...
			{
	?>
			  <div class="quantity_details"><?php echo $Captions_arr['PROD_DETAILS']['PRODDET_QUANTITY']?><input type="text" class="quainput" name="qty"  value="1" maxlength="2" /></div>
	<?php
			}
		...
		function show_more_images($row_prod,$exclude_tabid,$exclude_prodid)
		{
				...
				?>	
					  <table width="100%" border="0" cellpadding="0" cellspacing="3" class="productdethumbtable">
						...
						?>
							<?php /*<td align="center" valign="middle" width="<?php echo $width?>" class="proddetail_moreimgtd" onmouseover="this.className='proddetail_moreimgtd_hover'" onmouseout="this.className='proddetail_moreimgtd'">
							...
							*/
							...
							<?php
								 show_image(url_root_image($v[$pass_type],1),$title,$title);
								 show_image(url_root_image($v['image_thumbpath'],1),$title,$title,'preview');
							?>
							</a>
							</li>
							
							<?php /*<a href="#" onclick="link_submit('<?php echo $_REQUEST['prod_curtab']?>','<?php echo $v['image_id']?>','<?php echo url_product($row_prod['product_id'],$row_prod['product_name'],1)?>',0)" title="<?php echo $title?>">
							<?php
								 show_image(url_root_image($v[$pass_type],1),$title,$title);?></a>
								
								 </td>
							</tr>
						<?php /*?>	<tr>
							  <td align="center" class="img_title"><?php echo ($v['image_title'])?stripslashes($v['image_title']):'&nbsp;'?></td>
							</tr><?php ?>
							</table>						</td> */?>
						<?php
						/*	$curimg_col++;
			...
			<?php
				...
				if ($_REQUEST['prod_curtab'])
				{
					...
					if(count($tabimg_arr))
					{
						...
						?>
						<script type="text/javascript">
							arrMainImage[0] = new Array("<?php echo url_root_image($tabimg_arr[0]['image_iconpath'],1)?>", "<?php echo url_root_image($tabimg_arr[0]['image_bigpath'],1)?>", "<?php echo url_root_image($tabimg_arr[0]['image_extralargepath'],1)?>","<?php echo url_root_image($tabimg_arr[0]['image_thumbpath'],1)?>");
						</script>
						<?php
				...
				if ($show_normalimage)
				{
					...
					if(count($prodimg_arr))
					{
						...
						?>
						<script type="text/javascript">
							arrMainImage[0] = new Array("<?php echo url_root_image($prodimg_arr[0]['image_iconpath'],1)?>", "<?php echo url_root_image($prodimg_arr[0]['image_bigpath'],1)?>", "<?php echo url_root_image($prodimg_arr[0]['image_extralargepath'],1)?>", "<?php echo url_root_image($prodimg_arr[0]['image_thumbpath'],1)?>");
						</script>
						<?php
				...
				else // case of coming with out tab id, so show the normal image list if any
				{
				 ...
				 if (count($prodthumbimg_arr))
				 {
						...
						{
							$icon_arr[$i] = url_root_image($v['image_iconpath'],1);
							$thumb_arr[$i] = url_root_image($v['image_thumbpath'],1);
				?>
								<script type="text/javascript">
									arrThumbImage[<?php echo $i?>] = new Array("<?php echo url_root_image($v['image_iconpath'],1)?>", "<?php echo url_root_image($v['image_bigpath'],1)?>", "<?php echo url_root_image($v['image_extralargepath'],1)?>", "<?php echo url_root_image($v['image_thumbpath'],1)?>");
								</script>
				<?php		
							$i++;
						}
					...
					<div class="content_product_container" style="width:290px;">
						<div class="content_product_images"  style="width:290px;">
							<div>
								...
								<div id="dvMainImageZoom" class="content_product_images_mainimage_zoom" style="width: 290px; height: 370px; display: none;" align="left"> <img id="imgMainImageZoom" src="" alt="" style='border:0'> </div>
								<a id="hypMainImage" href="javascript:ShowZoomImage();" style="display: block;clear:both;" onmouseover="window.status='Click on images to enlarge them'; return true;" onmouseout="window.status=''; return true;"> <img id="imgMainImage" class="content_product_images_mainimage" src="<?php echo $main_img?>" alt="<?php echo $row_prod['product_name']?>" style="border-width: 0px; display: block;" > </a>
								...
								?>
										<div style="display: none; height:370px" id="divFlash" class="content_product_flash">
										 <object codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,28,0">
											<param name="movie" value="http://<?php echo $ecom_hostname?>/themes/<?php echo $ecom_themename?>/swf/flvplayer.swf?video=http://<?php echo $ecom_hostname?>/images/<?php echo $ecom_hostname?>/product_flv/<?php echo $row_prod['product_flv_filename']?>" />
											<param name="quality" value="high" />
											  <embed src="http://<?php echo $ecom_hostname?>/themes/<?php echo $ecom_themename?>/swf/flvplayer.swf?video=http://<?php echo $ecom_hostname?>/images/<?php echo $ecom_hostname?>/product_flv/<?php echo $row_prod['product_flv_filename']?>" quality="high" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash" height="370" width="290" ></embed>
										</object>
										</div>
								...
								<?php
									...
									{
								?>	
									 <li><a href="javascript:ReplaceImageFromThumb(<?php echo ($i+1)?>);" onmouseover="window.status='Click on images to enlarge them'; return true;" onmouseout="window.status=''; return true;"> <img id="imgThumb<?php echo ($i+2)?>" src="<?php echo $icon_arr[$i]?>" alt="Click on images to enlarge them" style="border-width: 1px;"><?php /*?><img id="imgbig<?php echo ($i+2)?>" src="<?php echo $thumb_arr[$i]?>" alt="Click on images to enlarge them" style="border-width: 1px;" class="preview"> <?php */?></a>
									 </li>
								<?php
									}
								?> 
				...
				global $db,$ecomn_siteid,$ecom_hostname,$ecom_themename;	
				  ...
				  if ($_REQUEST['prod_curtab'])
				  {
					...
					if(count($tabimg_arr))
					{
						$exclude_tabid = $tabimg_arr[0]['image_id']; 		// exclude id in case of multi images for products
						?>
						<a href="javascript:showImagePopup('<?php echo $tabimg_arr[0]['image_extralargepath']?>','<?php echo $ecom_hostname?>','<?php echo $ecom_themename?>')" title="Click to Zoom">
						<?php
						show_image(url_root_image($tabimg_arr[0][$pass_type],1),$row_prod['product_name'],$row_prod['product_name']);
						?>
						</a>
