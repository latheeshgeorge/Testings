...
/*############################################################################
	...
	?>
			<form method="post" action="" name='frm_buygiftvoucher' id="frm_buygiftvoucher" class="frm_cls">
  ...
  <td colspan="2">
			<?php
				...
				if ($db->num_rows($ret_paytypes))
				{
					...
					?>
						<input type="hidden" name="voucher_paytype" id="voucher_paytype" value="<?php echo $_REQUEST['voucher_paytype']?>" />
					<?php
					}
					else
					{
						...
						{
							$paymethod[0] = $Captions_arr['GIFT_VOUCHER']['GIFT_VOUCHER_SELECT'];
							...
							while ($row_paytypes = $db->fetch_array($ret_paytypes))
							{
								...
								//$paymethod[$row_paytypes['paytype_id']] = stripslashes($row_paytypes['paytype_name']);	
								if($row_paytypes['paytype_code']=='credit_card')
								{
									if($row_paytypes['paytype_id']==$row_cartdet['paytype_id'])
										$cc_exists = true;
					...
					?>	
							<div class="shoppaymentdiv">
							  <table width="100%" border="0" cellspacing="0" cellpadding="0">
								<tr>
								  <td width="40%" align="center" class="emailfriendtextnormal" valign="middle"><a name="a_pay">&nbsp;</a><?php echo $Captions_arr['GIFT_VOUCHER']['GIFT_VOUCHER_SEL_PAYTYPE']?></td>
									<td width="60%" align="left" valign="middle">
									  <?php
										echo generateselectbox('voucher_paytype',$paymethod,$_REQUEST['voucher_paytype'],'','handle_paytypeselect(this)');
									  ?>
									</td>
								</tr>
							  </table>
							  ...
							  <div class="shoppaymentdiv">
								<table width="100%" border="0" cellspacing="0" cellpadding="0">
								  ...
								  ?>
										...
										<input class="shoppingcart_radio" type="radio" name="voucher_paytype" id="voucher_paytype" value="<?php echo $row_paytypes['paytype_id']?>" onclick="handle_paytypeselect(this)" /><?php echo stripslashes($row_paytypes['paytype_name']).$add_text?>
										</td>
								<?php
										$pay_cnt++;
	...
	if ($db->num_rows($ret_paymethods))
	{
		if ($db->num_rows($ret_paymethods)==1)
		{
			...
			?>
			<input type="hidden" name="voucher_paymethod" id="voucher_paymethod" value="<?php echo $row_paymethods['paymethod_id']?>" />
			<?php
		}
		else
		{
			?>
			<tr id="voucher_paymethod_tr" style="display:none">
				...
				<div class="shoppayment_type_div">
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
					  ...
					  ?>
							<td width="25%" align="left" class="emailfriendtextnormal">
							<input class="shoppingcart_radio" type="radio" name="voucher_paymethod" id="voucher_paymethod" value="<?php echo $row_paymethods['paymethod_key'].'_'.$row_paymethods['paymethod_id']?>" <?php echo ($row_paymethods['paymethod_id']==$row_cartdet['paymethod_id'])?'checked="checked"':''?>  onclick="handle_paytypeselect(this)" /><?php echo stripslashes($caption)?></td>
					<?php
							$pay_cnt++;
							if ($pay_cnt>=$pay_maxcnt)
							{
	...
	?>
	<tr id="voucher_cheque_tr" style="display:none">
	<td colspan="2" align="left" valign="middle">	
	...
	<tr id="voucher_self_tr" style="display:none">
		<td colspan="2" align="left" valign="middle">	
		<table width="100%" cellpadding="1" cellspacing="1" border="0">
		<tr>
		...
		if(frm.checkoutpay_cardtype)
		{
			if(frm.checkoutpay_cardtype.value)
			{
				...
				if(objarr.length==4) /* if the value splitted to exactly 4 elements*/
				{
					...
					{
						alert('Credit card number should have '+cc_count+' digits');
						frm.checkoutpay_cardnumber.focus();
						return false;
					}
					if (frm.checkoutpay_securitycode.value.length!=seccount)
					{
						alert('Security Code should have exactly '+seccount+' digits');
...
<?php		
	...
	?>
		...
		?>
				<tr>
					<td align="left" class="shoppingcartcontent_indent_highlight">
					<?php echo $Captions_arr['CART']['CART_CHECKOUT_FAILED_MSG']?>
					</td>
				</tr>
		<?php	
			}
