		...
		function show_Variable_main_image($product_id,$var_arr=array())
		{
			...
			if ($comb_arr['combid']>0)
			{
				...
				if(count($prodimg_arr))
				{
					...
					?>
					<?php /*?><a href="javascript:showImagePopup('<?php echo $prodimg_arr[0]['image_extralargepath']?>','<?php echo $ecom_hostname?>','<?php echo $ecom_themename?>')"  title="Click to Zoom"><?php */?>
					<?php
