<?php
	...
	class shop_Html
	{
		...
		<div class="sub_shop_con">
        <div class="sub_shop_top"></div>
        <div class="sub_shop_middle" >
		<?php
				if($alert)
				{
			?>
			...
			<?php
				...
				?>	
				 ...
				 <div class="shopimagediv" align="right">
                ...
                setTimeout('hide_processing()', 20000);
		            ...
		            <input type="hidden" name="caturl" value="<? echo $url?>" />
                     ...
                     <input type='hidden' name='shop_id' value="<?=$_REQUEST['shop_id']?>"/>
				  	 <div class="categoreyimagediv" align="right"><a href="javascript:download_pdf()" class="productdetailslink" title="<?php echo $Captions_arr['SHOP_DETAILS']['SHOPDET_DOWNLOADPDF']?>">
						...
						?>
							...
							<div class="moreimg_outer">
									<?php
										...
										{
											?>
											<div class="moreimages_td"><a href="<?php url_shops($row_shop['shopbrand_id'],$row_shop['shopbrand_name'],-1)?>?shopthumb_id=<?php echo $v['image_id']?>" title="<?php echo $row_shop['shopbrand_name']?>">
		...
		?>
				...
				?>
				<tr>
				  <td colspan="3" align="left" valign="middle" class="subshopheader">
				  <?php
				...
				<?php	
					...
					{
				?>
						<td width="33%" align="center" valign="middle" class="subcategoreyimage" onmouseover="this.className='subshop_hover'" onmouseout="this.className='subshopimage'">
						 <div class="subcate_div_image">
						  <?php 
						 if($img_support) // ** Show sub category image only if catimage module is there for the site
							{
						  ?>
						  		<a href="<?php url_shops($row_subshop['shopbrand_id'],$row_subshop['shopbrand_name'],-1)?>" class="subshopnamelink" title="<?php echo stripslashes($row_subshop['shopbrand_name'])?>">
					        <?php
						  ...
						  ?>
						  <a href="<?php url_shops($row_subshop['shopbrand_id'],$row_subshop['shopbrand_name'],-1)?>" class="subshopnamelink" title="<?php echo stripslashes($row_subshop['shopbrand_name'])?>"><?php echo stripslashes($row_subshop['shopbrand_name'])?></a>
						 </div>
						</td>
				<?php
						$cur_col++;
					...
					if ($cur_col<$max_col)
						echo '<td colspan="'.($max_col-$cur_col).'" class="subshopimage">&nbsp;</td>';
				?>	
				</tr>
				<?php
		...
		function Show_Products($ret_prod,$tot_cnt,$start_var,$shop_det)
		{
		    ...
		    ?>
				...
				<input type="hidden" name="compare_products" id="compare_products" value=""><!--<input type="submit" name="submit_Compare_pdts" value="ADD TO COMPARE" class="buttonred_large" onclick="handle_addtoCompare()" />-->	</form>		<? }?>	&nbsp;</div>
				<div class="lst_nav">
     			   <ul>
					<li><?php echo $Captions_arr['SHOP_DETAILS']['SHOPDET_SORTBY']?>
												...
												?>
										         <div class="mid_shelfB">
													...
													<tr>
															...
															if($shop_det['shopbrand_product_showimage']==1)// whether title is to be displayed
															{
															?>
																...
																if(in_array($module_name,$inlineSiteComponents) and $shop_det['shopbrand_product_showrating']==1)
																{
