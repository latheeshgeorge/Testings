<?php
	...
	class cart_Html
	{
		...
		<div class="tree_con">
			<div class="tree_top"></div>
				...
				<div class="pro_det_treemenu"><ul> <li><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> &gt;&gt;</li> <li><?php echo $Captions_arr['CART']['CART_MAINHEADING']?></li>
				</ul></div>
				</div>
			...
			<?php
				  ...
				  <td colspan="6" align="left" valign="middle" class="cartlogin_msg">
					...
					if ($cust_id) // Case logged in 
					{
					  echo $Captions_arr['CART']['CART_LOGGED_IN_AS']; echo "&nbsp;".$cartData['customer']['customer_title']." ".$cartData['customer']['customer_fname']." ".$cartData['customer']['customer_mname']." ".$cartData['customer']['customer_surame'];?>
						<br />
						<? echo $Captions_arr['CART']['CART_IF_YOU_NOT_LOG'];?> <a href="<?php echo $ecom_selfhttp.$ecom_hostname?>/logout.html?rets=1" title="Logout" class="cartlogin_link"><? echo $Captions_arr['CART']['CART_HERE'];?></a><? echo "&nbsp;". $Captions_arr['CART']['CART_TO_LOGOUT'];?> 
					<?php
					}
			  ...
			  <tr>
				...
				<?php
					...
					{
				?>
				  		<input name="clearcart_button" type="button" class="cart_btn" id="clearcart_button" value="<?php echo $Captions_arr['CART']['CART_CLEAR']?>" onclick="if(confirm_message('Are you sure you want to clear all items in the cart?')){show_wait_button(this,'Please Wait...');document.frm_cart.cart_mod.value='clear_cart';document.frm_cart.submit();}" />
			    <?php
				 	}
				 ?>	</td>
			  </tr>
		  ...
		  else
		  {
			  ...
			  ?>
				  <tr>
					...
					?>
					<a href="<?php echo url_product($products_arr['product_id'],$products_arr['product_name'])?>" title="<?php echo stripslashes($products_arr['product_name'])?>" class="shoppingcartprod_link"><?php echo stripslashes($products_arr['product_name'])?></a>					</td>
					...
					?>	
							...
							<input type="hidden" name="cart_qty_<?php echo $products_arr['cart_id']?>" id="cart_qty_<?php echo $products_arr['cart_id']?>" value="<?php echo $products_arr["cart_qty"]?>"   class="regiinput"/>
					...
					<?php	
						...
						{
					?>
							<div class="updatediv" align="center"><input name="cart_qty_<?php echo $products_arr['cart_id']?>" type="text" id="cart_qty_<?php echo $products_arr['cart_id']?>" size="3" maxlength="2" value="<?php echo $products_arr["cart_qty"]?>" style="width:16px;" class="regiinput" />
							</div> 
				...
				?>	
						 ...
						 <td align="right" class="shoppingcartcontent_delivery" colspan="2"><?php echo $Captions_arr['CART']['CART_DELIVERY_LOC']?><a name="a_deliv">&nbsp;</a> 
							...
							<?php
									...
									echo generateselectbox('cart_deliverylocation',$deliverydet_Arr['locations'],$row_cartdet['location_id'],'','handle_form_submit(document.frm_cart,"save_commondetails","#a_deliv")',0,'regiinput');
							?>						</td>
					   </tr>
				   <?php
				   }
				   ...
				   ?>
					   <tr>
						 ...
						 <td align="right" class="shoppingcartcontent">
						  <?php
							echo generateselectbox('cart_deliveryoption',$deliverydet_Arr['del_groups'],$row_cartdet['delopt_det_id'],'','handle_form_submit(document.frm_cart,"save_commondetails","#a_deliv")',0,'regiinput');
						  ?>						 </td>
					   </tr>
					<?php
					}
 ...
 <?php
					...
					<td colspan="6" align="left" valign="middle"><div class="shopprodiv">
					  <table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
						  ...
						  <?php
							...
							?>
									<input name="cart_savepromotional" type="hidden" id="cart_savepromotional" value="0"/>	
								 	<input name="cancel_promotionalcode" type="button" class="cart_btn" id="cancel_promotionalcode" value="<?php echo $Captions_arr['CART']['CART_PROMO_CANCEL_BUTTON']?>" onclick="document.frm_cart.cart_savepromotional.value=1;handle_form_submit(document.frm_cart,'save_commondetails','#a_prom')" />
						  <?php
						  	}
						  	elseif ($cartData["bonus"]['type']=='')
							{
						  ?>
						  		<input name="cart_savepromotional" type="hidden" id="cart_savepromotional" value="0"/>
							  	<input name="cart_promotionalcode" type="text" id="cart_promotionalcode" size="12"  class="regiinput"/>
							  	<input name="submit_promotionalcode" type="button" class="cart_btn" id="submit_promotionalcode" value="<?php echo $Captions_arr['CART']['CART_PROMO_GO']?>" onclick="document.frm_cart.cart_savepromotional.value=1;handle_form_submit(document.frm_cart,'save_commondetails','')" />
						  <?php
						  	}
		  ...
		  <?php
			...
			?>
					 <tr>
					<td colspan="6" align="right"><div class="cart_bonus_more">
						<?php	
				...
				if($cust_id) // case if logged in
				{
					...
					?>
						 <tr>
							...
							<input  class="cart_btn" type="button" value="<?php echo $Captions_arr['CART']['CART_SPEND']?>" onclick="handle_form_submit(document.frm_cart,'save_commondetails','#bonus')">
							</td>
						  </tr>
						  <?php
						  }
						...
						?>
						  <tr>
							<td colspan="5" align="right" valign="middle" class="shoppingcartcontent"><?php echo $Captions_arr['CART']['CART_SPENT_BONUS_POINTS_VALUE']?> </td>
							<td colspan="1" align="right" valign="middle" class="shoppingcartpriceC"><? echo '(-) '.print_price($cartData["bonus"]["value"],true);?></td>
						  </tr>
						<?php	
						}
					}
			...
			<?php
				...
				if ($db->num_rows($ret_paytypes))
				{
					...
					?>	
							  ...
							  <table width="100%" border="0" cellspacing="0" cellpadding="0">
								<tr>
								  <td  class="shoppingcartcontent_delivery" align="right" colspan="2"><a name="a_pay">&nbsp;</a><?php echo $Captions_arr['CART']['CART_SEL_PAYTYPE']?>
								  <?php
            ...
            <td colspan="6" align="right" valign="middle" class="shoppingcartcontent">
			  ...
			  ?>
				...
				?>
				  ...
				  <input name="continue_submit" type="button" class="cart_btn" id="continue_submit" value="<?php echo $Captions_arr['CART']['CART_CONT_SHOP']?>" onclick="show_wait_button(this,'Please Wait...');window.location='<?php echo $ps_url?>'" />
				<?php
					}
					else
						echo '&nbsp;';
			  ...
			  ?>	
             		 ...
             		 <input name="continue_checkout" type="button" class="cart_btn" id="continue_checkout" value="<?php echo $Captions_arr['CART']['CART_GO_CHKOUT']?>" onclick="handle_checkout_submit('<?php echo $ecom_hostname?>',0,'<?php echo $cartData["totals"]["bonus_price"]?>')" />
             		</div> 
			  <?php
			  	}
				else
		...
		<?php
		  ...
		  ?>
         	 	...
         	 	<tr>
					...
					<table width="100%" border="0" cellspacing="1" cellpadding="1">
					<tr>
						<td class="shoppingcartlogintdtext1" valign="top"><strong><?php echo $Captions_arr['CART']['CART_SHOPPED_BEFORE']?>?</strong><br />Please enter your details below to sign in  </td>
					...
					<tr>
						...
						<td><input name="custlogin_uname" type="text" id="custlogin_uname" size="18" class="regiinput"/></td>
						<td>&nbsp;</td>
					</tr>
					<tr>
						<td align="left" valign="middle"><?php echo $Captions_arr['CART']['CART_PASSWORD']?></td>
						<td><input name="custlogin_pass" id="custlogin_pass" type="password" size="18" class="regiinput"/></td>
						<td>&nbsp;</td>
					</tr>
					<tr>
						<td colspan="2" align="left"><input name="custcartlogin_Submit" type="submit" class="cart_btn" value="<?php echo $Captions_arr['CART']['CART_LOGIN']?>" onClick="document.frm_custlogin_cart.pass_url.value = document.frm_cart.pass_url.value" /></td>
						<td align="right">
						<?php
						if($Settings_arr['hide_newuser']==0) // check whether new user page is to be displayed
						{
						?>
						<input name="new_user_cart" type="button" class="cart_btn" id="new_user_cart" value="<?php echo $Captions_arr['CART']['CART_SIGNUP']?>" onClick="window.location='<?php url_link('registration.html?pagetype=cart')?>'" />
						<?php
						}
						?>	</td>
					</tr>
        ...
        <div class="tree_top"></div>
        <div class="tree_middle">
     <div class="pro_det_treemenu"><ul> <li><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> &gt;&gt;</li> <li><?php echo $Captions_arr['CART']['CART_CHECKOUTHEADING']?></li>
	 </ul></div>
        </div>
        <div class="tree_bottom"></div>
      </div>
		...
		<div class="round_middle">		
		  ...
		  <?php
			...
			if (is_Feature_exists('mod_bonuspoints'))
			{
				if ($cartData["customer"]["customer_bonus"]) // does current customer have bonus points
				{
					...
					?>
					  <tr>
						<td colspan="5" align="right" valign="middle" class="shoppingcartcontent"><?php echo $Captions_arr['CART']['CART_SPENT_BONUS_POINTS_VALUE']?> </td>
						<td align="right" valign="middle" class="shoppingcartpriceC"><? echo '(-) '.print_price($cartData["bonus"]["value"],true);?></td>
					  </tr>
			<?php	
					}
				}
			...
			?>
				 <tr>
					...
					<td colspan="3" align="left" valign="middle" class="shoppingcartcontent"><?php echo $cartData["totals"]["bonus"]?></td>
				  </tr>
			<?php	
			}
		  	// show the total final price
		  ...
		  ?>
				...
				<td colspan="6" align="left" >	
			<div class="cart_continue_div" align="left">
	             <input name="backto_cart" type="button" class="cart_btn" id="backto_cart" value="<?php echo $Captions_arr['CART']['CART_BACK_CARTPAGE']?>" onclick="show_wait_button(this,'Please Wait...');gobackto_cart('<?php echo ($protectedUrl)? base64_encode($ecom_hostname):$ecom_hostname?>')"/>
	         </div>  
			   </td>
			  </tr> 
		  <?php
				...
				<td colspan="6" align="right" valign="middle">
					...
					?>
							<tr>
								...
								<?php
								    $class_arr['txtbox_cls']='regiinput';
								    $class_arr['select_cls']='regiinput';
									$class_arr['txtarea_cls']='regiinput';
									echo get_Field($row_checkout['field_key'],$saved_checkoutvals,$cartData['customer'],'',$class_arr);
								?>
								</td>
							</tr>
					<?php
					...
					<td colspan="2" align="left" class="shoppingcartheader"><?php echo $Captions_arr['CART']['CART_FILL_DELIVERY_ADDRESS']?></td>
						...
						?>
								...
								<?php
								    ...
								    $class_arr['select_cls']='regiinput';
									$class_arr['txtarea_cls']='regiinput';
									echo get_Field($row_checkout['field_key'],$saved_checkoutvals,$cartData,'',$class_arr);
								?>
								</td>
								</tr>
						<?php
...
//				$cont_class = 'shoppingcartcontent_noborder'; 
		...
		<div class="tree_middle">
			...
			<li> <?php echo $Captions_arr['CART']['CART_CHECKOUTPREVIEWHEADING']?></li>
			</ul>
			</div>
		</div>
		<div class="tree_bottom"></div>
		  ...
		  <?php
			...
			if ($row_ord['order_bonuspoints_used']>0)
			{
				...
				?>
					  <tr>
						<td colspan="5" align="right" valign="middle" class="shoppingcartcontent"><?php echo $Captions_arr['CART']['CART_SPENT_BONUS_POINTS_VALUE']?> </td>
						<td align="right" valign="middle" class="shoppingcartpriceC"><? echo '(-) '.print_price($row_ord['order_bonuspoint_discount'],true);?></td>
					  </tr>
			<?php	
				}
			}
			...
			*/
			if($ecom_isadword)
			{
				...
				?>
						...
						<img height="1" width="1" style="border-style:none;" alt="" src="https://www.googleadservices.com/pagead/conversion/<?php echo $ecom_adword_conversionid?>/?label=<?php echo $ecom_adword_conversionlabel?>&amp;guid=ON&amp;&script=0">
						</div>
						</noscript>					
			<?php		
		...
		<div class="tree_middle">
			...
			<ul>
			<li><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> >> </li>
			<li><?php echo $Captions_arr['CART']['CART_CHECKOUT_SUCCESS_TITLE']?></li>
			</ul>
			</div>
		</div>
		<div class="tree_bottom"></div>
		...
		<div class="round_middle">
			...
			?>
				 ...
				 <tr>
					...
					<a href="<?php echo $ecom_selfhttp.$ecom_hostname; ?>/mydownloads.html" title="<?php echo $Captions_arr['CART']['CART_DOWNLOAD_LIST_LINK']?>" class="favoriteprodlink"><?php echo $Captions_arr['CART']['CART_DOWNLOADABLE_LINK']?> </a><?php echo $Captions_arr['CART']['CART_DOWNLOADABLE_LINK_CONT']?>.</td>
					</td>
				</tr>
			<?php
				}			
		...
		<div class="tree_middle">
			...
			<ul>
			<li><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> >> </li>
			<li><?php echo $Captions_arr['CART']['CART_CHECKOUT_FAILED_TITLE']?></li>
			</ul>
			</div>
		</div>
		<div class="tree_bottom"></div>
		...
		<div class="tree_middle">
			<div class="pro_det_treemenu">
			<ul>
			<li><a href="<? url_link('');?>" title="<?php echo stripslash_normal($Captions_arr['COMMON']['TREE_MENU_HOME_LINK'])?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> >> </li>
			<li><?php echo $Captions_arr['CART']['CART_NOCHEXCHECKOUT_SUCCESS']?></li>
			</ul>
			</div>
		</div>
		<div class="tree_bottom"></div>
