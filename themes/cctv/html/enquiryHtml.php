	...
	class enquiry_Html
	{
	  function enquiry_Showform()
	  {
		...
		<div class="tree_middle">
			...
			<li> <?php echo $Captions_arr['ENQUIRE']['ENQUIRE_MAINHEADING']?> </li>
			</ul>
			</div>
		</div>
		  ...
		  ?>
			  <tr>
				...
				<?php 
					...
					?>
				<a href="<?php echo url_product($row_prod_det['product_id'],$row_prod_det['product_name'])?>" title="<?php echo $row_prod_det['product_name']?>" class="shoppingcartprod_link"><?php echo stripslashes($row_prod_det['product_name'])?></a></td>
			  ...
			  <tr>
			 <td  align="left" valign="middle"   class="<?=$trmainclass?>" >Add Notes:<br /><textarea  name="enquirenote_<?=$row_select_enq['enquiry_id']?>" rows="5" cols="38" id="note_text_<?=$row_select_enq['enquiry_id']?>"  class="regiinput"><? if($_REQUEST['enquirenote_'.$row_select_enq['enquiry_id']]) echo $_REQUEST['enquirenote_'.$row_select_enq['enquiry_id']];else echo $row_select_enq['enquiry_note'];?></textarea>&nbsp; <input name="buttonenqsaveone_<?=$row_select_enq['enquiry_id']?>" type="submit" class="buttonred_cart" id="buttonenqsaveone_<?=$row_select_enq['enquiry_id']?>" value="Save Note" /></td>
			 </tr></table></td>
			  </tr>
		  <?php
						...
						<tr>
							<td width="48%" align="left" valign="top" class="shoppingcartlogintd">
								...
								<tr>
								  ...
								  <td width="71%"><input name="custlogin_uname" id="custlogin_uname" type="text" class="regiinput" /></td>
								</tr>
								<tr>
								  <td align="left" valign="middle"><?php echo $Captions_arr['CART']['CART_PASSWORD']?></td>
								  <td><input name="custlogin_pass" id="custlogin_pass" type="password" class="regiinput" /></td>
								</tr>
								<tr>
								  <td>&nbsp;</td>
	...
	else
	{
	  ...
	  <form method="post" name="frm_enquire_details" class="frm_cls" action="" onsubmit="return validate_allforms(this);">
		...
		<div class="tree_middle">
			<div class="pro_det_treemenu">
			<ul>
			<li><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> >> <a href="<? url_link('enquiry.html');?>"><?=$Captions_arr['ENQUIRE']['ENQUIRE_MAINHEADING'];?></a> >></li>
			<li> <?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_MAIN']?> </li>
			</ul>
			</div>
		</div>
		<div class="tree_bottom"></div>
       ...
       <tr>
		...
		?>
                  <tr>
                   ...
                   <td width="57%" align="left" valign="top" ><select name="cbo_enqtitle" id="cbo_enqtitle"><option value="" >Select</option><option value="Mr." <? if($row_cust['customer_title']=='Mr.') echo "selected"; else echo '';?>>Mr.</option><option value="Ms." <? if($row_cust['customer_title']=='Ms.') echo "selected"; else echo '';?>>Ms.</option><option value="Mrs." <? if($row_cust['customer_title']=='Mrs.') echo "selected"; else echo '';?>>Mrs.</option><option value="Miss." <? if($row_cust['customer_title']=='Miss.') echo "selected"; else echo '';?>>Miss.</option><option value="M/s." <? if($row_cust['customer_title']=='M/s.') echo "selected"; else echo '';?>>M/s.</option></select></td>
                  </tr>
                  <tr>
...
<?php 				/*$state_arr = array(-1=>'-- Select State --');
   ...
   <? 
	 ...
	 global $ecom_siteid,$db,$ecom_hostname,$Settings_arr,$ecom_themeid,$default_layout,$Captions_arr,$inlineSiteComponents,$ecom_isadword,$ecom_adword_conversionid,$ecom_adword_conversionlanguage,
		...
		function myenquiry_Enquiry($custid){
				...
				<div class="tree_middle">
					...
					<ul>
					<li><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> >> </li>
					<li> <?php echo $Captions_arr['ENQUIRE']['ENQUIRE_MYENQUIRYHEADING']?> </li>
					</ul>
					</div>
				</div>
				<div class="tree_bottom"></div>
		...
		function myenquiry_EnquiryDetails($enqid)
		{
				...
				<div class="tree_middle">
					...
					<ul>
					<li><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> >> </li>
					<li> <a href=""><?php echo $Captions_arr['ENQUIRE']['ENQUIRE_MYENQUIRYHEADING']?></a> >> <?php echo $Captions_arr['ENQUIRE']['ENQUIRE_DET_MAIN']?> </li>
					</ul>
					</div>
				</div>
				<div class="tree_bottom"></div>
				...
				?>
				<tr>
			 		<td align="left" valign="middle" class="<?=$trmainclass?>" > <a href="<?php echo url_product($rowpr['product_id'],$rowpr['product_name'])?>" title="<?php echo $row_prod_det['product_name']?>" class="favoriteprodlink"><? echo stripslashes($rowpr['product_name']); ?></a></td>
				...
				<tr><td>Name</td><td>:<?=stripslashes($row_enq['enquiry_fname']).' '.stripslashes($row_enq['enquiry_middlename']).' '.stripslashes($row_enq['enquiry_lastname'])?></td></tr>
