		...
		{ 
			...
			if (count($advert_arr))
			{
				...
				foreach ($advert_arr as $d=>$k)
				{
					...
					{
					?>
					<div class="banimgtop" align="<?php echo ($k['advert_type']=='IMG')?'left':'center'?>">
					<?php
					}
					switch ($k['advert_type'])
					{
						...
						break;
						case 'SWF':
							$path = url_root_image('not/img/'.$k['advert_source'],1);
							$link = $k['advert_link'];
							$flash_path =  '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="'.$ecom_selfhttp.'download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,28,0" width="518" height="95">
						...
						case 'TXT':
							...
							case 'ROTATE':   // case if ad rotate images are set
								...
								//echo $HTML_Content;
								?>
								 <script src="<?php echo $ecom_selfhttp?>code.jquery.com/jquery-1.7.1.min.js"></script>
    <?php 
