...
/*############################################################################
 ...
 ?>
 <div class="treemenu"><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> >> <? if($product_id!=0)
				{
				?>
				  <a href="<?php url_product($product_id,$prod_name,-1)?>"><?=$prod_name?></a> >> <?=stripslash_normal($Captions_arr['PROD_FREE_DELIVERY']['PROD_FREE_DELIVERY_TREEMENU_TITLE'])?>
				<? 
				 }
				?></div>
		<table border="0" cellpadding="0" cellspacing="0" class="staticpagetable">
			<tr>
			  <td colspan="2" align="left" class="staticpageheader"><?=stripslash_normal($Captions_arr['PROD_FREE_DELIVERY']['PROD_FREE_DELIVERY_TREEMENU_TITLE'])?></td>
			  </tr>
			<tr>
			  <td valign="top" class="staticpagecontent">
			 <?php
			 if($fetch_arr_admin['product_freedelivery_content']!='')
			 { echo stripslashes($fetch_arr_admin['product_freedelivery_content']); } else {
