...
/*############################################################################
	...
	?>
		...
		<?
		if ($Settings_arr['paytype_listingtype']=='icons') // Case if payment types is to be displayed as list items
		{
		?>
		<tr class="gift_mid_table">
		<?php
		...
		elseif($Settings_arr['paytype_listingtype']=='dropdown')
		{
			echo '<script type="text/javascript">
			paytype_arr  = new Array();		
			</script>';
			...
			?>	
			  <td  align="right" class="shoppingcartcontent_delivery" width="50%"><a name="a_pay">&nbsp;</a><?php echo $Captions_arr['CART']['CART_SEL_PAYTYPE']?>
					 <?php
							echo generateselectbox('voucher_paytype',$paymethod,$row_paytypes['paytype_id'],'',$paytype_onclick);
					  ?>
			 </td>
			  </tr>
			<?
...
?>
	...
	if(Validate_Form_Objects(frm,fieldRequired,fieldDescription,fieldEmail,fieldConfirm,fieldConfirmDesc,fieldNumeric))
	{
		...
		if(frm.voucher_paytype.value!=0)
			{ 
...
?>		
	...
	<?php
		...
		?>
				<tr class="gift_mid_table">
					<td colspan="2" align="right" class="gift-send-inner_cnt">
						<input type="button" name="submit_backhome" class='buttongray' value="<?php echo $Captions_arr['GIFT_VOUCHER']['GIFT_VOUCHER_PREVIEW_BACH_HOME']?>" onclick="window.location ='<?php url_link('')?>'" />
					</td>
				</tr>	
		<?php
			}	
		...
		?>
			...
			foreach ($var_arr as $k=>$v)
			{
				...
				if($db->num_rows($ret_var))
				{
					...
					echo '<tr><td align="right" class="productvariabletdB">';
					...
					if($row_var['var_value_exists']==1)
					{
						...
						if ($db->num_rows($ret_val))
						{
							...
							echo '<td align="left" class="productvariabletdB">: '.stripslashes($row_val['var_value']).'
							<input type="hidden" name="var_'.$k.'" value="'.$v.'"/>
							</td>';
						}
					}
					else 
						echo '<td class="productvariabletdB"><input type="hidden" name="var_'.$k.'" value="1"/></td>';
					echo '</tr>';
				}
			}
			?>
