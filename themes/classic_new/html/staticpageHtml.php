	...
	class static_Html
	{
		...
		function Show_StaticPage($row_statpage)
		{
			global 	$Captions_arr,$ecom_siteid,$db,$ecom_hostname,$ecom_themename,
					$ecom_themeid,$default_layout,$inlineSiteComponents,$Settings_arr,$error_static;
		...
		<input type="hidden" name="fproduct_id" value="<?php echo $_REQUEST['product_id']?>" />
			...
			?>
			<tr>
			  <td colspan="2" align="center" class="message"><?php echo stripslashes($error_static)?></td>
			  </tr>
			  <?php
			  }
			  ?>
			<tr>
			 ...
			 <?php
			  if($row_statpage['allow_auto_linker'] == 1) {
				//echo 't';
				echo auto_linker(stripslashes(str_replace($arr_sr,$arr_rp,$row_statpage['content'])));
			} else {
				echo stripslashes(str_replace($arr_sr,$arr_rp,$row_statpage['content']));
			} 
