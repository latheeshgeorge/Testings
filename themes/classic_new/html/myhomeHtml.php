										...
										?>
											...
											<tr>
												<td align="left" valign="middle"  colspan="3">
                                                                                                 <table width="100%" cellpadding="2" cellspacing="3" border="0" class="subcategoreytable">
										<?php
						...
						while ($row_cats = $db->fetch_array($ret_cats)and $totcnt<3)
						{
							...
							$sql_prods_offers = "SELECT a.product_id,a.product_name,a.product_variablestock_allowed,a.product_show_cartlink,
													...
													LIMIT 
														$prod_limit";
							$ret_prods_offers = $db->query($sql_prods_offers);
							$excludeprods_id_arr = array(0);
							$excludeprods_id_str = '';
							if($db->num_rows($ret_prods_offers))
							{
								while ($row_prods_offers = $db->fetch_array($ret_prods_offers))
								{
									$excludeprods_id_arr[] = $row_prods_offers['product_id'];
								}
								$excludeprods_id_str = implode(',',$excludeprods_id_arr);
								$excludeprods_id_str = " AND a.product_id NOT IN ($excludeprods_id_str) ";
							}
