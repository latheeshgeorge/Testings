<?php
	...
	class combo_Html
	{
		...
		function Show_Combo($title,$description,$combo_id)
		{
			...
			<table border="0" cellpadding="0" cellspacing="0" class="shelfBtable_combo">
			<?php
		  	if($proceed_combo)
		  	{
		...
		?>
			...
			<tr>
				  <td align="left" valign="middle" class="shelfBtabletd_combo_desc" colspan="3"><?php echo $description;?></td>
			</tr>
			<?php
			}
			?>
  ...
  function show_combo_product_details($row_prod,$keepselected=false,$disp_type='',$cur_col,$max_col)
		...
		switch($disp_type)
		{
			...
			?>
				<div class="shelfAtabletd">
									...
									<ul class="shelfAul">
									<li><h2 class="shelfAprodname_combo"><a href="<?php url_product($row_prod['product_id'],$row_prod['product_name'],-1)?>" title="<?php echo stripslashes($row_prod['product_name'])?>"><?php echo stripslashes($row_prod['product_name'])?></a></h2></li>
									<li class="compare_li">
									<?php
			...
			?>
					<div class="combo_shelfBtabletd">
												<div class="combo_shelfBtabletdinner">
												<div class="combo_shelfBprodname">
												<a href="<?php url_product($row_prod['product_id'],$row_prod['product_name'],-1)?>" title="<?php echo stripslashes($row_prod['product_name'])?>" class="combo_shelfBprodnamelink"><?php echo stripslashes($row_prod['product_name'])?></a>
												</div>
												<div class="combo_shelfBleft">
												<a href="<?php url_product($row_prod['product_id'],$row_prod['product_name'],-1)?>" title="<?php echo stripslashes($row_prod['product_name'])?>">
												<?php
								...
								?>
												...
												<div class="combo_shelfBmid">	
												<h6 class="combo_shelfBproddes"><?php echo stripslashes($row_prod['product_shortdesc'])?></h6>
												<ul class="shelfBnormalprice"><?php
												    echo print_price($row_prod['combo_discount'],true,false);
													?>
													</ul>
													<?php
												...
												<div class="combo_shelfBright"> 
												<?php 
												if($row_prod['product_freedelivery']==1)
												{	
												?>
													<div class="combo_shelfB_free"></div>
												<?php
												}
												if($row_prod['product_bulkdiscount_allowed']=='Y')
												{
												?>
													<div class="combo_shelfB_bulk"></div>
												<?php
												}
												$frm_name = uniqid('best_');
												?>
												</div>
												<div class="variable_combo_1row">
												<?php
												// Get the combination if exists for current combo product
									if ($comb_id)
									{
									...
									// Case of variables
										...
										?>
												<tr>
													<td align="left" valign="middle" class="combo_mid_shelfBtabletd" width="40%">
														<?php 
															echo stripslash_normal($row_var['var_name']);
														?>
													</td>
													<td align="left" valign="middle" class="combo_mid_shelfBtabletd">
													<?php
													if ($row_var['var_value_exists']==1)
													{
														echo ': '.$row_vardata['var_value'];
	...
	{ 
		...
		while ($row_combos = $db->fetch_array($ret_combos))
		{
			...
			$sql_prod = "SELECT a.product_id,a.product_name,a.product_variablestock_allowed,a.product_show_cartlink,
				...
				?>
						...
						{
						?>	
						<div class="lf_combodeal_top"><?php echo stripslashes($row_combos['combo_name'])?></div>
						<?
						}
						?>
			...
			?>
						<div class="lf_combodeal_price">Bundled Price: <?php echo print_price($bundle_price)?></div>
							...
							<div class="lf_combodeal_bottom">
							<a href="<?php url_combo($row_combos['combo_id'],$row_combos['combo_name'],-1)?>"  title="<?php echo $Captions_arr['COMMON']['SHOW_DEAL']?>" class="lf-combodeal-showall"><?php echo $Captions_arr['COMMON']['SHOW_DEAL']?></a>
							</div>
							</div>	
						</td>
					<?php
		...
		{ 
		       $colcnt=$max_cnt-$cur_cnt;
				echo '<td align=\"center\" valign=\"middle\" width=\"33%\" colspan=\"$colcnt\">&nbsp;</td></tr>';	
		}
		?>
		</table>
		<?php
