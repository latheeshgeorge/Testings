										...
										<?php
											while ($row_cat = $db->fetch_array($ret_cat))
											{
												if($prev_grp != $grpData['catgroup_id'])
												{
													...
													{
											?>
														<li><span class="categoryheader"><?php echo stripslashes($title)?></span></li>
											<?php
													}
							...
							if($shelfData['shelf_currentstyle']=='nor') // case of normal design layout
							{
								switch($shelfData['shelf_displaytype'])
								{
									...
									?>
										...
										<a href="<?php url_product($row_prod['product_id'],$row_prod['product_name'],-1)?>" class="noraml_lft_buy_link">BUY NOW</a></div>
										</div>	  	
										</div>
									<?php
									}
