...
/*############################################################################
	...
	# Description 	: Page which holds the display logic for home page after login
	# Coded by 		: Sny
	...
	class myhome_Html
	{
		...
		?>
			<div class="treemenu"><a href="<? url_link('');?>"><?=stripslash_normal($Captions_arr['COMMON']['TREE_MENU_HOME_LINK']);?></a> >> <?php echo  stripslash_normal($Captions_arr['LOGIN_HOME']['LOGIN_HOME_TREE_MENU'])?></div>
						...
						if($group_id)
						{
							...
							$sql_products_id = "SELECT 
													DISTINCT pc.products_product_id,p.product_id,p.product_name,p.product_variablestock_allowed,p.product_show_cartlink,
														...
														p.product_newicon_show, p.product_newicon_text,p.product_averagerating,p.product_bonuspoints       
												FROM 
								...
								if($allow_discount==1)
								{
									...
									?>
									 <tr>
										...
										<div class="shelfA_top"><?=stripslash_normal($homemsg)?></div>
										<div class="shelfA_mid">
										<?php	
										$max_col = 3;
										$cur_col = 0;
										$prodcur_arr = array();
										$cur_tot_cnt = $db->num_rows($ret_products_id);
										$cur = 1;
		...
		function Show_Recently_Purchased_Products_With_Categories($used_val_arr)
		{
			...
			<?php		
					...
					$sql_prods_offers = "SELECT product_id,product_name,product_variablestock_allowed,product_show_cartlink,
												...
												product_newicon_show,product_newicon_text,product_averagerating,product_bonuspoints 
											FROM 
												products 
											WHERE 
				...
				?>		<div class="shelfA_main_con" >
						<div class="shelfA_mid">
						<div class="shelfA_top"><?php echo stripslash_normal($row_cat['category_name'])?></div>
				<?php
						if($db->num_rows($ret_prods_offers))
						{
						?>
							<div class="myhome_offer_subtext"><?php echo str_replace('[category]',stripslash_normal($row_cat['category_name']),stripslash_normal($Captions_arr['LOGIN_HOME']['LOGIN_HOME_RECENT_RECOMM_PROD']))?>
							</div>
							<?php	
							//*********************************************************
						...
						{
						?>
							<div class="myhome_offer_subtext"><?php echo str_replace('[category]',stripslash_normal($row_cat['category_name']),stripslash_normal($Captions_arr['LOGIN_HOME']['LOGIN_HOME_RECENT_NEW_PROD']))?>
							</div>
							<?php		
							//*******************************************************************
			...
			if ($db->num_rows($ret_favcat))
			{
				...
				list($row_last_login) 	= $db->fetch_array($ret_last_login);
						...
						?>
									<div class="shelfA_main_con" >
									<div class="shelfA_mid">
									<div class="shelfA_top"><?php echo stripslash_normal($row_favcat['category_name'])?></div>
						<?php
								if($db->num_rows($ret_prods_offers))
								{
								?>
									<div class="myhome_offer_subtext"><?php echo str_replace('[category]',stripslash_normal($row_cat['category_name']),stripslash_normal($Captions_arr['LOGIN_HOME']['LOGIN_HOME_FAV_RECOMM_PROD']))?>
									</div>
									<?php	
									//*********************************************************
									// Show the list of products which have offers
								...
								{
								?>
									<div class="myhome_offer_subtext"><?php echo str_replace('[category]', stripslash_normal($row_cat['category_name']),stripslash_normal($Captions_arr['LOGIN_HOME']['LOGIN_HOME_FAV_NEW_PROD']))?>
									</div>
									<?php		
									//*******************************************************************
									// Show the list of new products since last login in current category
								...
								?>
								<div><a href="<?php  url_category_all($row_favcat['category_id'],$row_favcat['category_name'],-1)?>" class="middle_showall_link" title="<?php echo stripslash_normal($Captions_arr['COMMON']['SHOW_ALL'])?>"><? echo stripslash_normal($Captions_arr['COMMON']['SHOW_ALL'])?></a>
								</div>
								</div>
								<div class="shelfA_bottom"></div>
								</div>
		...
		function Show_Products($ret_prods,$prod_arr)
		{
			...
			$cur_col = 0;
			$prodcur_arr = array();
			$cur_tot_cnt = $db->num_rows($ret_prods);
			$cur = 1;
		...
		function Show_Favourite_Products($used_val_arr)
		{
			...
			?>
							...
							<div class="shelf_top"><?php echo stripslash_normal($Captions_arr['LOGIN_HOME']['LOGIN_HOME_FAV_PROD'])?></div>
							<div class="shelf_mid">
						<?php		
							$cur = 1;
					...
					?>
						<div class="shelfA_main_con" > 
						<div class="shelfA_top"><?php echo stripslash_normal($Captions_arr['LOGIN_HOME']['LOGIN_HOME_FAV_PROD'])?></div>
						<div class="shelfA_mid">
						<?php
							$max_col = 3;
							$cur_col = 0;
				...
				list($row_last_login) 	= $db->fetch_array($ret_last_login);
					...
					<?php	
						...
						while ($row_cats = $db->fetch_array($ret_cats) and $totcnt<3)
						{
							...
							?>
								<div class="shelfA_main_con"> 
								<div class="shelfA_top"><?php echo stripslash_normal($row_cats['category_name'])?></div>
								<div class="shelfA_mid">
								<?php
								if($db->num_rows($ret_prods_offers))
								{
								?>
									<div class="myhome_offer_subtext"><?php echo str_replace('[category]',stripslash_normal($row_cat['category_name']),stripslash_normal($Captions_arr['LOGIN_HOME']['LOGIN_HOME_MOST_HITS_CAT_OFFER']))?>
									</div>
									<?php	
									//*********************************************************
									// Show the list of products which have offers
								...
								{
								?>
									<div class="myhome_offer_subtext"><?php echo str_replace('[category]', stripslash_normal($row_cat['category_name']),stripslash_normal($Captions_arr['LOGIN_HOME']['LOGIN_HOME_MOST_HITS_CAT_NEW']))?>
									</div>
									<?php		
									//*******************************************************************
									// Show the list of new products since last login in current category
								...
								?>
									<div><a href="<?php  url_category_all($row_cats['category_id'],$row_cats['category_name'],-1)?>" class="middle_showall_link" title="<?php echo stripslash_normal($Captions_arr['COMMON']['SHOW_ALL'])?>"><? echo stripslash_normal($Captions_arr['COMMON']['SHOW_ALL'])?></a>
									</div>	
									</div>
								<div class="shelfA_bottom"></div>
								</div>
