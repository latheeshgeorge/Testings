<?php
	...
	class enquiry_Html
	{
	  function enquiry_Showform()
	  {
		...
		//print_r($prod_array );
			?>
			<div class="treemenu"><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> >> <?php echo $Captions_arr['ENQUIRE']['ENQUIRE_MAINHEADING']?></div>
		...
		while($row_select_enq = $db->fetch_array($ret_select_enq))
		{
		 ...
		 $prod_sql_det = "SELECT product_id,product_name,product_default_category_id,product_webprice,
		  ...
		  ?>
			  ...
			  <tr>
			 <td  align="left" valign="middle"   class="<?=$trmainclass?>" >Add Notes:<br /><textarea  name="enquirenote_<?=$row_select_enq['enquiry_id']?>" rows="5" cols="38" id="note_text_<?=$row_select_enq['enquiry_id']?>" ><? if($_REQUEST['enquirenote_'.$row_select_enq['enquiry_id']]) echo $_REQUEST['enquirenote_'.$row_select_enq['enquiry_id']];else echo $row_select_enq['enquiry_note'];?></textarea>&nbsp; <input name="buttonenqsaveone_<?=$row_select_enq['enquiry_id']?>" type="submit" class="cart_btn" id="buttonenqsaveone_<?=$row_select_enq['enquiry_id']?>" value="Save Note" /></td>
			 </tr></table></td>
			  </tr>
		  <?php
		  ...
		  ?>
           ...
           <!--<tr>
		   	 <td  align="center" id="td_save_notes" style="display:none"  ><input name="buttonenq_all" type="submit" class="cart_btn" id="buttonenq_all" value="Save All Notes" /></td>
		     <td  align="center" colspan="2" ><input name="buttonenq1" type="button" class="cart_btn" id="buttonenq1" value="Submit Enquiry" onclick="location.href='<?php //url_link('fillenquiry.html')?>'"/></td>
		  </tr>-->
		            <tr>
            		...
            		<td colspan="6" align="left" valign="middle" class="shoppingcartcontent">
						...
						<tr>
							<td width="48%" align="left" valign="top" class="shoppingcartlogintd">
								...
								<tr>
								  <td>&nbsp;</td>
								  <td><input name="custenquirelogin_Submit" type="submit" class="cart_btn" value="Login" onclick="document.frm_custlogin_enquire.pass_url.value = document.frm_enquire.pass_url.value" /></td>
								</tr>
								</table>
							</td>	
							<td width="52%" align="left" valign="top" class="shoppingcartlogintd">
								...
								?>
									...
									<tr>
										<td align="left"><input name="new_user_enquire" type="button" class="cart_btn" id="new_user_enquire" value="Sign Up" onclick="window.location='<?php url_link('registration.html?pagetype=enquire')?>'" /></td>
									</tr>
									</table>
								<?php
								}
...
<?php 				/*$state_arr = array(-1=>'-- Select State --');
		...
		<?php 
			...
			?>
			<tr>
			 <td  align="center"  colspan="2" > <input name="enquiry_submit" type="submit" class="cart_btn" id="enquiry_submit" value="Save Details" /></td>
		</tr>
      ...
      <tr>
		...
		function myenquiry_Enquiry($custid){
				...
				$prod_array = array();
					...
					<div class="treemenu"><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> >> <?php echo $Captions_arr['ENQUIRE']['ENQUIRE_MYENQUIRYHEADING']?></div>
					<div class="shelf_main_con" >
					<div class="shelf_top"><?php echo $Captions_arr['ENQUIRE']['ENQUIRE_MYENQUIRYHEADING']?></div>
					<div class="shelf_mid">
				  <table width="100%" border="0" cellpadding="0" cellspacing="2" class="shoppingcarttable">
                    ...
                    <td width="7%" nowrap="nowrap"><?php echo $sort_by_txt?></td>
							   <td class="usermenucontent">	<input name="clearenq_button" type="button" class="cart_btn" id="clearenq_button" value="<?php echo $Captions_arr['ENQUIRE']['ENQUIRE_SEARCH']?>" onclick="document.my_enquire.submit()" />
							   </td>
					  </tr>
					  </table>
					  </td> 
		...
		function myenquiry_EnquiryDetails($enqid)
		{
				...
				//print_r($prod_array );
					?>
					<div class="treemenu"><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> >> <a href=""><?php echo $Captions_arr['ENQUIRE']['ENQUIRE_MYENQUIRYHEADING']?></a> >> <?php echo $Captions_arr['ENQUIRE']['ENQUIRE_DET_MAIN']?></div>
					<div class="shelf_main_con" >
					<div class="shelf_top"> <?php echo $Captions_arr['ENQUIRE']['ENQUIRE_DET_MAIN']?></div>
					<div class="shelf_mid">
				  ...
				  <table width="100%" border="0" cellpadding="0" cellspacing="2" class="shoppingcarttable">
					  <tr>
						...
						<?php
							...
							{
						?>
								<input name="clearenq_button" type="button" class="cart_btn" id="clearenq_button" value="<?php echo $Captions_arr['ENQUIRE']['ENQUIRE_CANCEL']?>" onclick="if(confirm_message('Are you sure you want to cancel the Enquiry?')){document.my_enquire_det.enq_mod.value='cancel_enquiry';document.my_enquire_det.enquiry_id.value='<?=$row_enq['enquiry_id']?>';document.my_enquire_det.submit();}" />
						<?php
							}
						 ?>	</td>
					  </tr>
