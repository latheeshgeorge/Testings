...
/*############################################################################
	...
	?>
		...
		<div class="shelf_top"><?=$Captions_arr['GIFT_VOUCHER']['GIFT_VOUCHER_BUY_TREEMENU_TITLE']?></div>
		<div class="shelf_mid">
...
?>
  ...
  <tr>
    ...
    <td align="left" valign="middle" class="emailfriendtext">
      <?php 
		...
		$vImage->showCodBox(1,'buygiftvoucher_Vimg','class="Img_ver_txt"'); 
	?>
	</td>
  </tr>
  <? }?>
  ...
  <tr>
    <td colspan="2" align="center" class="emailfriendtextnormal"><input name="buygiftvoucher_Submit" type="button" class="cart_btn" id="buygiftvoucher_Submit" value="<?=$Captions_arr['GIFT_VOUCHER']['GIFT_VOUCHER_BUY_VOUCHER_BUY_BUTTON']?>" onclick="validate_buygiftvoucher(document.frm_buygiftvoucher)" /></td>
  </tr>
<?php
	}
	else
...
?>		
		...
		<div class="shelf_main_con" >
		<div class="shelf_top"><?=$Captions_arr['GIFT_VOUCHER']['GIFT_VOUCHER_BUY_TREEMENU_PREVIEWTITLE']?></div>
		<div class="shelf_mid"><table width="100%" border="0" cellpadding="0" cellspacing="3"  class="emailfriendtable">
		<?php 
			if($alert)
			{ 
		?>
	...
	<?php
		...
		?>
				<tr>
					<td colspan="2" align="right" class="emailfriendtextnormal">
						<input type="button" name="submit_backhome" class="cart_btn" value="<?php echo $Captions_arr['GIFT_VOUCHER']['GIFT_VOUCHER_PREVIEW_BACH_HOME']?>" onclick="window.location ='<?php url_link('')?>'" />
					</td>
				</tr>	
		<?php
			}	
...
<?php		
	...
	function Show_VoucherFailed()
	{
		...
		$Captions_arr['GIFT_VOUCHER'] 	= getCaptions('GIFT_VOUCHER'); // Getting the captions to be used in this page
	?>
		<div class="treemenu"><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> >> <?php echo $Captions_arr['GIFT_VOUCHER']['GIFT_VOUCHER_FAILED_TITLE']?></div>
		<div class="shelf_main_con" >
		<div class="shelf_top"><?php echo $Captions_arr['GIFT_VOUCHER']['GIFT_VOUCHER_FAILED_TITLE']?></div>
		<div class="shelf_mid">
	...
	?>			
				...
				<?php 
					...
					{
					?>
					<input name="compvoucher_Submit" type="submit" class="gift_user_btn" id="compvoucher_Submit" value="<?php echo $Captions_arr['GIFT_VOUCHER']['VOUCHER_GO']?>" />
					<?php
					}
					?>
				  </td>
				...
				?>
					<tr>
						...
						<?php
							...
							?>
										<div class="shelf_main_con" >
										<div class="shelf_top"><?php echo $Captions_arr['GIFT_VOUCHER']['GIFT_VOUCHER_PROM_PRODUCTS']?>	</div>
										<div class="shelf_mid">										
										<?php
										$cur = 1;
	...
	?>
		...
		<li class="shlf_pdt_des"><h6>	<?php echo stripslashes($row_prod['product_shortdesc'])?></h6>
		<?php 
		if(count($var_arr))
		{
		?>
			...
			foreach ($var_arr as $k=>$v)
			{
				...
				{
					$cls = ($var_cnts%2==0)?'productvariabletdB':'productvariabletdA';
					$var_cnts++;
					$row_var = $db->fetch_array($ret_var);
					echo '<tr><td align="right" class="'.$cls.'">';
					...
					if($row_var['var_value_exists']==1)
					{
						...
						if ($db->num_rows($ret_val))
						{
							...
							echo '<td align="left" class="'.$cls.'">: '.stripslashes($row_val['var_value']).'
							<input type="hidden" name="var_'.$k.'" value="'.$v.'"/>
							</td>';
						}
					}
					else 
						echo '<td class="'.$cls.'"><input type="hidden" name="var_'.$k.'" value="1"/></td>';
					echo '</tr>';
				}
			}
		}	
		...
		<?php	
			...
			{
			?>
				<div><input type="button" class="buttongray" onclick="show_wait_button(this,'Please Wait...');document.<?php echo $frm_name?>.fpurpose.value='Prod_Addcart';document.<?php echo $frm_name?>.submit();" value="<?php echo stripslash_normal($Captions_arr['PROD_DETAILS'][$caption_key]);?>"/></div>
			<?php
			}
			?>
		</div>
