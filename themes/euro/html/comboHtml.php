			...
			?>
			   ...
			   <?php 
					...
					while($row_prod = $db->fetch_array($ret_prod))
					{
						...
						{
						?>
							<div class="img_center" align="center"><img src="<?php url_site_image('combosep.gif')?>" border="0" alt="combo deals" /></div>
						<? 
					  	}
						$combo_pdt_cnt ++;
						...
						if($tot_combinations) // Case if combination exists
						{
							...
							$cur_tot_cnt = $db->num_rows($ret_combination); 
							$cur =1;
							while ($row_combination = $db->fetch_array($ret_combination))
							{
								...
								if($cur_combination==1)
									...
									if($disp_type=='3row')
									{
										...
										if($cur_cnt==0)
										{
											$main_inner_shelf = 'shlfA_inner_main_lst_combo';
											?>
											<div class="<?php echo $main_shelf?>">
											<?php
										}
										else
											$main_inner_shelf = 'shlfA_inner_main_combo';
										?>
  ...
  function show_combo_product_details($row_prod,$keepselected=false,$disp_type='',$cur_col,$max_col)
		...
		<?php
			...
			if ($comb_id)
			{
				...
				?>
							...
							{
							$cls= 'combovar_name_new';
							}
							?>
							<div class="<?=$cls?>"><?php echo stripslashes($row_var['var_name'])?></div>
							<div class="combovar_val">
							<?php
							if ($row_var['var_value_exists']==1)
							{
	...
	function Show_Combo_Multiple($ret_combos,$combprod_id,$deal_mod)
	{
		...
		?>
			...
			<div class="shelf_main_con" >
			<div class="shelf_top"><? echo $Captions_arr['COMBO']['COMBO_BUNDLED_OFFER'] ?></div>
			<div class="shelf_mid">
