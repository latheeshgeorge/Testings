<?php
	...
	class payonaccountHtml
	{
		   function show_payonaccount_request($alert='',$hide_status=0)
			...
			?>
			<div class="treemenu"><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> >> <?php echo stripslash_normal($Captions_arr['PAYONACC']['PAYONACCOUNT_MAINHEADING'])?></div>
			<div class="shelf_main_con" >
		<div class="shelf_top"><?=$Captions_arr['PAYONACC']['PAYONACCOUNT_MAINHEADING']?></div>
		<div class="shelf_mid">
			...
			?>
			<tr>
			<td align="left" class="payonaccount_text" colspan="2">
			<?php
			...
			if($db->num_rows($ret_cust))
			{
				...
				?>
						...
						<td align="center" colspan="2">
							...
							<input type='submit' name='payonaccount_request_place' id ='payonaccount_request_place' value='<?php echo $Captions_arr['PAYONACC']['PAYONACC_PLACE_REQUEST']?>' class="buttongray"/>
							</form>
						</td>
						</tr>
				<?php
			...
			<form name="frm_custlogin_payonacc" id="frm_custlogin_payonacc" method="post" action="" onsubmit="return validate_login(this)" class="frm_cls">
         	 	...
         	 	<tr>
					...
					<tr>
						...
						{
						?>
						<input name="new_user_cart" type="button" class="cart_btn" id="new_user_cart" value="<?php echo $Captions_arr['CART']['CART_SIGNUP']?>" onClick="window.location='<?php url_link('registration.html?pagetype=payonacc')?>'" />
						<?php
						}
						?>	</td>
					</tr>
	...
	<div class="shelf_bottom"></div>	
		  ...
		  function show_Summary($alert='')
		  {
			...
			<div class="treemenu"><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> >> <?php echo $Captions_arr['PAYONACC']['PAYONACCOUNT_MAINHEADING']?></div>
			<div class="shelf_main_con" >
			<div class="shelf_top"><?php echo $Captions_arr['PAYONACC']['PAYONACCOUNT_MAINHEADING']?></div>
			<div class="shelf_mid">
				 ...
				 $where_conditions = " WHERE 
				   ...
				   <td colspan="2"></td><!--onclick="window.location='<?php //url_link('payonaccountpayment.html')?>'"-->
					<td align="right"><div id="make_pay_top_div"><? if($row_cust['customer_payonaccount_usedlimit']>0){?><input  type="button" name="make_payment" value="<?php echo $Captions_arr['PAYONACC']['MAKEPAY_BUTTON']?>" class="cart_btn" onclick="handle_make_payment_top()" />
					<? }?></div></td>
					 <td colspan="2"><? if($numbillcount>0){?><input type="button" name="view_statements" title="View Statements" value="<?php echo $Captions_arr['PAYONACC']['VIEWSTATEMENT_BUTTON']?>" class="cart_btn" onclick="window.location='<?php url_link('accountviewbills.html')?>'" /><? }?>
					</td>
				   </tr>
				   <? }?>
						...
						<tr>
							...
							<td align="center" width="75%">
									<table cellspacing="4" cellpadding="4" border="0" width="100%" bgcolor="#D9D9D9">
										...
										<tr>
										  <td  valign="middle" align="center"  > <input name="Cancel" type="button" class="cart_btn" id="cancel" value="<?php echo $Captions_arr['PAYONACC']['CANCEL_BUTTON']?>" onclick="handle_make_payment_cancel()"  /></td>
										  <td  align="left" valign="middle"  ><input name="make_payment" type="button" class="cart_btn" id="make_payment" value="<?php echo $Captions_arr['PAYONACC']['MAKEPAY_BUTTON']?>" onclick="validate_payment(this.form)" /> </td>
										</tr>
								</table>
							  </td>
								<td width="14%" >&nbsp;</td>
		...
		<div class="shelf_bottom"></div>	
		  ...
		  ?>
		   ...
		   <div class="treemenu"><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> >> <a href="<? url_link('mypayonaccountpayment.html');?>"><?php echo $Captions_arr['PAYONACC']['PAYONACCOUNT_MAINHEADING']?></a> >> <?php echo $Captions_arr['PAYONACC']['VIEWBILLS_MAINHEADING']?></div>
		   <div class="shelf_main_con" >
	<div class="shelf_top"><?php echo $Captions_arr['PAYONACC']['VIEWBILLS_MAINHEADING']?></div>
	<div class="shelf_mid">
				  <table width="100%" border="0" cellpadding="3" cellspacing="0" >
						  <tr>
...
function handle_paytypeselect_payonaccount(obj)
{
	...
	else 
	{
		...
		else
		{
			...
			<div class="shelf_main_con" >
	<div class="shelf_top"><?="Pay on Account Payment"?></div>
	<div class="shelf_mid">
  ...
  <tr>
    <td colspan="6" align="center" class="emailfriendtextnormal"><?=$Captions_arr['PAYONACC']['PAYON_VERIFICATION_CODE']?>&nbsp;<span class="redtext">*</span><span class="emailfriendtext">
	  <?php 
		...
		$vImage->showCodBox(1,'payonaccountpayment_Vimg','class="Img_ver_txt"'); 
	?>
	</span> </td>
    </tr>
  <? }?>
...
?>
  ...
  <td colspan="2">
	...
	?>
	 <tr>
    <td colspan="4" align="right" class="emailfriendtext"><input name="payonaccount_payment_Submit" type="submit" class="buttongray" id="payonaccount_payment_Submit" value="<?php echo $Captions_arr['PAYONACC']['MAKEPAY_BUTTON']?>"/></td>
    </tr>
	<?php
	}
        }
	...
	?>	
		...
		function Show_payonaccountFailed()
		{
			...
			$Captions_arr['PAYONACC'] 	= getCaptions('PAYONACC'); // Getting the captions to be used in this page
		?>
		<div class="treemenu"><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> >> <?php echo $Captions_arr['PAYONACC']['PAYON_FAILED_TITLE']?></div>
			<div class="shelf_main_con" >
	<div class="shelf_top"><?php echo $Captions_arr['PAYONACC']['PAYON_FAILED_TITLE']?></div>
	<div class="shelf_mid">
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="shoppingcarttable">
			<?php
				if($msg)
