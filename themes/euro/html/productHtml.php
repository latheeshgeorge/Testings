		...
		{ 
			...
			{ 
				case 'price_qty':
					...
					handle_show_prod_det_bulk_disc('price',docroot,prodid,loading_gif);	
					return;
				break;
				case 'price': 
					var qtys
			 ...
			 while(list($heading_title, $heading_id) = $db->fetch_array($res))
			 {
			   ...
			   $sql_prods = "SELECT product_sizechart_mainheading 
				...
				if ($db->num_rows($ret_prods))
				{
					...
					$sizechartmain_title = stripslashes($row_prods['product_sizechart_mainheading']); 
				}
				if($main_title == '')
				{
		 ...
		 ?>
          ...
          <?php				
				...
				{ 
					...
					{
					?>
					<div class="pro_det_image" id='mainimage_holder'>111
					<?php
					...
					{
						?>
						<div class="bookmark_cls"><br /><font color="#000000"><b><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['BOOKMARK_HEADING'])?></b></font><br />
					...
					{
					?>
					<div class="bookmark_cls">&nbsp;</div>
					<?php
					}
					if($row_prod['product_flv_filename']!='')
					{
						echo $HTML_video = '<div class="deat_pdt_button"><a href="javascript:show_normal_video()"><img src="'.url_site_image('play-video.gif',1).'" border="0" /></a>
										</div><div id="div_defaultFlash_outer" class="flashvideo_outer" style="display:none"></div>
					...
					{
					?>
						<div class="stockdetailstd"><?php echo $stk_det;?></div>
					<?php
					}	
					if ($Settings_arr['bonus_points_instock'] and $row_prod['product_bonuspoints']>0)
					{
...
<?php				
       ...
       <div class="<?php echo $pro_det_varble_mid?>">
						...
						?>
								  <tr>
									<td colspan="2" align="left" valign="middle" class="<?php echo $clss?>"><?php echo stripslashes($row_var['var_name'])?> :</td></tr>
									<tr>
									<td colspan="2" align="left" valign="middle" class="<?php echo $clss?>">
										<?php
										if ($row_var['var_value_exists']==1)
										{
						...
						if($db->num_rows($ret_grp))
						{
							$ret_val = '<div class="pro_det_ke_fea" >
									   ...
									   <div class="pro_det_ke_fea_mid">
										...
										<tr>
										<td class="variable_bottom_border" colspan="2">'.$Captions_arr['PROD_DETAILS']['PRODDET_KEY_FEATURES'].'</li>
										</td>
										</tr>
										';
							$i=1;
									...
									if(is_array($v))
									{
										if(count($v))
										{
											for($i=0;$i<count($v);$i++)
											{
												...
												$clss = ($i%2==0)?'productlabeltdA':'productlabeltdB';
												$ret_val .= '
															<tr>
																<td align="left" valign="middle" class="'.$clss.'">'.$v[$i]['name'].'</td>
																<td align="left" valign="middle" class="'.$clss.'_right">:&nbsp;'.$v[$i]['val'].'</td>
															</tr>';	
											}
										}
									}
		...
		function show_buttons($row_prod)
		{
			...
			if($showqty==1)// this decision is made in the main shop settings
			{
				...
				{
	?>
			  		<div class="quantity_details"><?php echo $cur_qty_caption?><input type="text" class="quainput_det" name="qty"  value="<?php echo ($_REQUEST['qty'])?$_REQUEST['qty']:1?>"  <?php echo $onchange_function?> /></div>
	<?php
				}
	 ...
	 ?>
		...
		<?php
			...
			{
	 ?>			
				<input name="Submit_enq" type="submit" class="ajx_notbuy_btn" id="Submit_enq" value="<?php echo $Captions_arr['PROD_DETAILS']['PRODDET_ENQUIRY'];?>" onclick="show_wait_button(this,'Please Wait...');document.frm_proddetails.fpurpose.value='Prod_Enquire'" />
	<?php
			}
		...
		{	
			...
			{
  ?>
				<br /><input name="submit_wishlist" type="submit" class="ajx_notbuy_btn" id="submit_wishlist" value="<?php echo $Captions_arr['PROD_DETAILS']['PRODDET_ADDWISHLIST'];?>"  onclick="show_wait_button(this,'Please Wait...');document.frm_proddetails.fpurpose.value='Prod_Addwishlist'"  />
  <?php
			}	
			else
			{
				...
				?>
				<input name="submit_wishlist" type="button" class="ajx_notbuy_btn" id="submit_wishlist" value="<?php echo $Captions_arr['PROD_DETAILS']['PRODDET_ADDWISHLIST'];?>" onclick="<?php echo $wishlist_onclick?>" />
				<?php
			}	
		}		
		?>
		...
		function show_more_images($row_prod,$exclude_tabid,$exclude_prodid)
		{
				...
				if ($_REQUEST['prod_curtab'] and $row_prod['product_variablecombocommon_image_allowed']=='N')// case if came by clicking the tab
				{
					if ($exclude_tabid)
