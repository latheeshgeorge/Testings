<?php
	...
	class search_Html
	{
				        ...
				        if ($_REQUEST['req']!='')// LIMIT for products is applied only if not displayed in home page
						{
							...
							if ($db->num_rows($ret_search))
							{
								...
								switch($Settings_arr['search_prodlisting'])
								{
									  case '1row':
								  ?>
								  		<div class="treemenu"><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a><? if($_REQUEST['quick_search']!=""){?> >> <? echo $_REQUEST['quick_search']; } ?></div>
										<div class="shelf_main_con" >
										<div class="shelf_top"><? echo $_REQUEST['quick_search'];?></div>
										<?php 
										if(trim($search_desc)) 
										{
										...
										{
										?>
										<div class="shlfA_pdt_des" style="padding-left:2px">
										<?php echo stripslashes($top_content)?></div>
										<?php		
										}
										?>
										<div class="shelf_sort_section_top">
										...
										<select name="searchprod_sortbytop" id="searchprod_sortbytop">
											...
											<option value="price" <?php echo ($prodsort_by=='product_webprice')?'selected="selected"':''?>><?php echo $Captions_arr['SEARCH']['SEARCH_PROD_PRICE']?></option>
										...
										<?php
											$tot_cnt = $db->num_rows($ret_search);
											if ($tot_cnt>0)
											{
											?>
												<div class="pagingcontainertd_left">
												<?php echo paging_show_totalcount($tot_cnt,'Products',$start_var['pg'],$start_var['pages']);?>
												</div>
												<div class="pagingcontainertd_right">
													<?php 
										...
										?>
												...
												<div class="shlf_pdt_img">
													...
													if(in_array($module_name,$inlineSiteComponents))
													{
														...
														?>
															...
															<?php
																display_rating($row_search['product_averagerating']);
															?>
															</div>
														<?php
														}
												...
												<?php
													if($row_search['product_freedelivery']==1)
													{	
												?>
														<div class="free_delivery"></div>
												<?php
													}
													if($row_search['product_bulkdiscount_allowed']=='Y')
													{
												?>
												...
												<li class="shlf_pdt_name" ><h3><a href="<?php url_product($row_search['product_id'],$row_search['product_name'],-1)?>" title="<?php echo stripslashes($row_search['product_name'])?>"><?php echo stripslashes($row_search['product_name'])?></a></h3></li>
												<li class="shlf_pdt_des"><h6>	<?php echo stripslashes($row_search['product_shortdesc'])?></h6></li>
												</ul>
												<?php
													...
													echo show_Price($row_search,$price_class_arr,'shelfcenter_1');
										...
										?>
										<div class="shelf_sort_section_bottom">&nbsp;
										<table width="100%" border="0" cellpadding="0" cellspacing="0" class="productpagenavtable">
										<tr>
												<td width="63%" height="30" align="right" valign="middle" class="productpagenavtext" colspan="2"><?php echo $Captions_arr['SEARCH']['SEARCH_SORT']?>
								...
								<?
									...
									?>
										<div class="treemenu"><a href="http://<?=$ecom_hostname?>" title="<?=$ecom_hostname?>">Home</a><? if($_REQUEST['quick_search']!=""){?> >> <? echo $_REQUEST['quick_search']; } ?> </div>
										<div class="shelfA_main_con" >
										<div class="shelfA_top"><? echo $_REQUEST['quick_search']; ?></div>
										<div class="shelfA_mid">
										<?php
										// Getting the top content for searches
											if(trim($search_desc)) 
											{
												$top_content = stripslashes($search_desc);
											}
											 else if($row_desc['search_content'] && $_REQUEST['quick_search'])
											{
												$srch_arr 			= array('[title]','[keywords]','[first_keyword]');
										...
										{
									?>
											<div class="shlfA_pdt_des" style="padding-left:2px"><?php echo stripslashes($top_content)?></div>
									<?php		
										}
									?>
									<div class="shelf_sort_section_top">
									<table width="100%" border="0" cellpadding="0" cellspacing="0" class="productpagenavtable">
										<tr>
												...
												<input type="button" name="top_submit_Page" value="<?php echo $Captions_arr['SEARCH']['SEARCH_GO']?>" class="buttonred"  onclick="handle_searchdropdownval_sel('<?php echo $ecom_hostname?>','searchprod_sortbytop','searchprod_sortordertop','searchprod_prodperpagetop')" />
												</td>
										</tr>
										</table>
										<?php 
									...
									<?php	
										$cur_tot_cnt = $db->num_rows($ret_search);
										if ($tot_cnt>0)
										{
										?>
											<div class="pagingcontainertd_left">
										...
										?>
												<div class="<?php echo $main_inner_shelf?>">
												 ...
												 <li class="shlfA_pdt_name" ><h3><a href="<?php url_product($row_search['product_id'],$row_search['product_name'],-1)?>" title="<?php url_product($row_search['product_id'],$row_search['product_name'],-1)?>"><?php echo stripslashes($row_search['product_name'])?></a></h3></li>
												 </ul>
												 </div>
												...
												?>
												  ...
												  <div class="shlfA_pdt_txt">
															...
															?>
															<li class="shlfA_pdt_des" ><h6><?php echo stripslashes($row_search['product_shortdesc'])?></h6></li>
														<li class='shlfA_pdt_bonus'><h6><?php show_bonus_points_msg($row_search,'bonus_point'); // Show bonus points?></h6></li>
														<?php
															if($row_search['product_saleicon_show']==1)
															{
																$desc = stripslash_normal(trim($row_search['product_saleicon_text']));
														...
														<li>
															<?php
																...
																echo show_Price($row_search,$price_class_arr,'shelfcenter_3');
																show_excluding_vat_msg($row_search,'vat_div');// show excluding VAT msg
															?>	
														</li>
														...
														<?php
															...
															<input type="hidden" name="fproduct_url" value="<?php url_product($row_search['product_id'],$row_search['product_name'])?>" />
															<div class="infodivB">
																<div class="infodivleftB"><?php show_moreinfo($row_search,'infolinkB')?></div>
										...
										<tr>
												...
												<input type="button" name="bottom_submit_Page" value="<?php echo $Captions_arr['SEARCH']['SEARCH_GO']?>" class="buttonred" onclick="handle_searchdropdownval_sel('<?php echo $ecom_hostname?>','searchprod_sortbybottom','searchprod_sortorderbottom','searchprod_prodperpagebottom')"  />
												</td>
										</tr>
										</table>
					...
					if(count($search_rel))
						{
							...
							?>
							<div class="shelf_main_con" >
							<div class="shelf_top">Related searches</div>
							<div class="shelf_mid">
		...
		function Show_Search_Category($search_sql,$tot_cnt)
		{
			...
			$first_querystring=$query_string; //Assigning the initial string to the variable.
				        ...
				        if ($_REQUEST['req']!='')// LIMIT for products is applied only if not displayed in home page
						{
						  ...
						  ?>
									<div class="shelf_main_con" >
										<div class="shelf_top"><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a><? if($_REQUEST['quick_search']!=""){?> >> <? echo $_REQUEST['quick_search']; } ?> </div>
										<div class="shelf_sort_section_top">&nbsp;
										<table width="100%" border="0" cellpadding="0" cellspacing="0" class="productpagenavtable">
										<tr>
												<td width="63%" height="30" align="right" valign="middle" class="productpagenavtext" colspan="2"><?php echo $Captions_arr['SEARCH']['SEARCH_SORT']?>
										...
										?>
												...
												<div class="shlf_pdt_txt">
												<ul class="shlf_pdt_ul">
												<li class="shlf_pdt_name" ><h3><a href="<?php url_category($row_search['category_id'],$row_search['category_name'],-1)?>" title="<?php echo stripslashes($row_search['category_name'])?>"><?php echo stripslashes($row_search['category_name'])?></a></h3></li>
												<li class="shlf_pdt_des"><h6><?php echo stripslashes($row_search['category_shortdescription'])?></h6></li>
												</ul>
												</div>
												</div>
										<?php
		...
		{
			global $ecom_siteid,$db,$ecom_hostname,$Settings_arr,$Captions_arr,$search_only_category;
			$Captions_arr['SEARCH'] 	= getCaptions('SEARCH');
			...
			?>
			<div class="treemenu"><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> >> <?php echo $Captions_arr['SEARCH']['SEARCH_ADVANCED_SEARCH'];?></div>
			<div class="shelf_main_con" >
			<div class="shelf_top"><?php echo $Captions_arr['SEARCH']['SEARCH_ADVANCED_SEARCH'];?></div>
			<div class="shelf_mid">
			...
			<tr>
			  <td colspan="3" align="left" valign="top" ><div class="treemenu"></div></td>
			</tr>
			<?php
		...
		?>
				...
				<table align = "center" width="98%" border="0" cellspacing="0" cellpadding="3" class="">
					<tr>
						...
						{
							$caption =  stripslashes($Captions_arr['SEARCH']['SEARCH_NO_PRODUCTS_WITH_KEYWORD']);
