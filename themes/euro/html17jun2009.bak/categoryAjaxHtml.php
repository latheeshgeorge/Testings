			...
			<script type="text/javascript">
				...
				function handle_show_prod_det_bulk_disc(opt)
				{
					...
					switch (opt)
					{
						...
						Handlewith_Ajax('themes/<?php echo $ecom_themename?>/html/categoryAjax_onchange.php','fpurpose='+fpurpose+'&prodid='+prodid+'&pass_var='+varstr+'&pass_varid='+varidstr);
				}
				function ajax_return_categorycontents() 
				{
					...
					if(req.readyState==4)
					{
						if(req.status==200)
						{
							...
							if (document.getElementById('desc_change').value==0)
							{
								retobj 				= eval("document.getElementById('"+document.getElementById('counter').value+"')");				
								retobj.innerHTML	= ret_val;
							}
							else if (document.getElementById('desc_change').value==1)
							{
								retobj			= document.getElementById('proddet_maincontent');		
								retobj.innerHTML	= ret_val;
							}	
							else if (document.getElementById('desc_change').value==3) // case of price
							{
