<?php
	...
	class components
	{
			...
			switch($position)
			{
				...
				if ($cache_exists==false)
				{
					...
					if ($position == 'top') // Case if value of position is top;
					{
						...
						?>					
							<td colspan="3" align="right" valign="top" class="static_table_td">
								<ul class="staticlink">  
					<?php
						if(count($grp_array))
						{
					...
					if ($position == 'bottom') // Case if value of position is bottom;
					{
						...
						<?php			
								...
								while ($row_pg = $db->fetch_array($ret_pg))
								{
									$target = ($row_pg['page_link_newwindow']==1 and $row_pg['page_type']!='Page')?'target="_blank"':'';
								?>
										<li><a href="<?php if ($row_pg['page_type']=='Page') url_static_page($row_pg['page_id'],$row_pg['title']); else echo $row_pg['page_link'];?>" class="bottomlink" title="<?php echo stripslashes(htmlspecialchars($row_pg['title']))?>" <?php echo $target?>><?php echo stripslashes(htmlspecialchars($row_pg['title']));?></a></li>
								<?php
								}
							}									
								//if($show_bottom==0)
		...
		function mod_shoppingcart($title)
		{
			...
			if($_REQUEST['req']=='cart' and $_REQUEST['cart_mod']=='show_checkoutsuccess')
			{
				$cart_tot = print_price(0,true);
				$pass_tot = print_price(0,true,true);
				$tot_items = 0;
			}
			else
			{	
				...
				$tot_items = (get_session_var('cart_total_items'))?get_session_var('cart_total_items'):0;
			}
				
			if ($position=='top') // Best sellers is allowed in left or right panels
			{	
			?><td align="right" valign="top" class="top_cart_x">				
				<div id="shoppingcart_ajax_container"><table width="100%" border="0" cellspacing="0" cellpadding="0" class="top_cart_x_table">
			  <tr>
			  	<td><a href="#" onclick="gobackto_cart('<?php echo ($protectedUrl)?base64_encode($ecom_hostname):$ecom_hostname?>')" class="cart_view_link" title="<?php echo stripslash_normal($Captions_arr['COMMON']['VIEW_CART'])?>"><div class="top_basket_hdr">Your Basket</div></a><div class="top_basket_txt"> <?php echo  $tot_items;?>&nbsp;&nbsp;<?php echo stripslash_normal($Captions_arr['COMMON']['ITEMS_IN_CART'])?> - <?php echo $cart_tot?></div></td>
				<td><a href="#" class="cart_view_link" onclick="handle_checkout_submit('<?php echo $ecom_hostname?>',0,'<?php echo $pass_tot?>')"  title="<?php echo stripslash_normal($Captions_arr['COMMON']['CHECK_OUT'])?>"><img src="<?php url_site_image('chk-out.gif') ?>"  alt="checkout" /></a></td>
			  </tr>
			  </table></div>
			  </td>				
			<?php
									...
									{ 
										...
										if ($grpData['catgroup_show_subcat_indropdown']== 1 and $subcatdropdownsupport)// case if categories are to be shown in list menu
										{
											...
											<?php
												...
												{
											?>
													<li class="item1"><a href="<?php url_category($row_cat['category_id'],$row_cat['category_name'],-1)?>" class="category_menu" title="<?php echo stripslash_normal(htmlspecialchars($row_cat['category_name']))?>"><span><?php echo ucwords(stripslash_normal(htmlspecialchars($row_cat['category_name'])));?></span></a></a>
													<?php
                                                    ...
                                                    <?php 
														...
														<ul  id="main_navigation"> 
															<?php
																...
																$style = '';
																if($ecom_siteid==95 || $ecom_siteid==120 || $ecom_siteid==61)
																{
																$style = 'style="width:30px;height:39px"';
																}
																?>
																<li class='top_cat_home_cls'><a href="<?php url_link('')?>" class="category_home" title="<?php echo $Captions_arr['STATIC_PAGES']['STAT_HOME']?>"><img <?php echo $style;?> src="<?php url_site_image('home-icn.png')?>" border="0" alt="home"/></a></li>
																<?php
																while ($row_cat = $db->fetch_array($ret_cat))
																{
																		$img_url = '';
																...
																?>
																	...
																	<li class="top_cat_li_main">
																	<a href="<?php url_category($row_cat['category_id'],$row_cat['category_name'],-1)?>" class="category_menu" title="<?php echo stripslash_normal(htmlspecialchars($row_cat['category_name']))?>"><span><?php echo ucwords(stripslash_normal(htmlspecialchars($row_cat['category_name'])));?></span></a>
																<?php
																	// Check whether there exists first level subcategories for current category
																	$sql_subcat = "SELECT category_id,category_name 
																					FROM 
																	...
																	if($num_rows)
																	{
																		...
																		elseif($cursubcat_cnt>=$center_point) //elseif($cursubcat_cnt>=$center_point)
																			$align_cls	= 'align-center4';//'align-right';
																		$cursubcat_cnt++;
																?>
																		<div class="dropdown <?php echo $align_cls.' '.$nav_cls?>">
																		<div class="dropdown_inner_cls">
																		<div class='mainimage_cls' style=" z-index:999; float:left;<?php echo $img_str?> width:100%; height:100%;padding: 10px 0px 35px;">
																<?php
																		$subcat_cnt 	= 0;
																		$subcat_maxcnt 	= 3;				
																		while($row_subcat = $db->fetch_array($ret_subcat))
																		...
																		?>
																			<ul>
																			<li class="cat_subcat"><a href="<?php url_category($row_subcat['category_id'],$row_subcat['category_name'],-1)?>"><?php echo stripslashes(htmlspecialchars($row_subcat['category_name']))?></a></li>
																			<?php
																				...
																				if($num_subrows)
																				{
																					...
																					{
																			?>			
																						<li><a href="<?php url_category($row_subsubcat['category_id'],$row_subsubcat['category_name'],-1)?>"><?php echo stripslashes(htmlspecialchars($row_subsubcat['category_name']))?></a></li>
																			<?php
																					}
																				}
																			?>
																...
																?>
																		...
																		<div class="menuadvert_div"><div>&nbsp;<img src="<?php url_site_image('advert.png')?>" alt="advert" /></div></div>
																		</div>
																		</div>	
																<?php		
																	}		
								...
								foreach ($grp_array as $k=>$grpData)
								{
									...
									if ($db->num_rows($ret_cat))
									{
										...
										<?php
											...
											{
										?>
														<li><a href="<?php url_category($row_cat['category_id'],$row_cat['category_name'],-1)?>" class="categorybottom" title="<?php echo stripslashes(htmlspecialchars($row_cat['category_name']))?>"><?php echo stripslashes(htmlspecialchars($row_cat['category_name']));?></a></li>
										<?php
											}
										?>
													</ul>
								...
								$sql_cat = "SELECT a.category_id,a.category_name,parent_id,b.category_displaytype,b.category_islink,a.category_subcatlisttype   
									...
									if ($db->num_rows($ret_cat))
									{
										...
										<?php
											...
											{
											?>
												 <li><h2><span class="subcategoryheader"><a href="<?php url_category($row_cat['category_id'],$row_cat['category_name'],-1)?>" class="subcategoryheaderlink" title="<?php echo stripslashes($row_cat['category_name'])?>"><?php echo stripslashes($row_cat['category_name']);?></a></span></h2></li>
											<?
											}
											elseif($row_cat['category_displaytype']=='Heading' && !$row_cat['category_islink'])
											{
											?>
												 <li><h2><span class="subcategoryheader"><?php echo stripslashes($row_cat['category_name']);?></span></h2></li>
											<?
											}*/
											...
											<div class="<?php echo $maincat_class?>"><a href="<?php url_category($row_cat['category_id'],$row_cat['category_name'],-1)?>" title="<?php echo stripslashes(htmlspecialchars($row_cat['category_name']))?>"><?php echo stripslashes(htmlspecialchars($row_cat['category_name']));?></a></div>
											<?php
											//echo $row_cat['category_subcatlisttype'];
												...
												if ($row_cat['category_subcatlisttype']=='List' or $row_cat['category_subcatlisttype']=='Both')
												{
													...
													{	
														...
														<?php	
															...
															{
														?>
																	<li><a href="<?php url_category($row_child['category_id'],$row_child['category_name'],-1)?>" title="<?php echo stripslashes(htmlspecialchars($row_child['category_name']))?>"><?php echo stripslashes(htmlspecialchars($row_child['category_name']));?></a></li>
														<?php		
															}
														?>
															</ul>
		...
		{
			global $Captions_arr,$position,$ecom_hostname,$protectedUrl,$enable_auto_search,$Settings_arr,$ecom_siteid;
			$checkout_link = get_Checkoutlink(1);
			if($_REQUEST['req']=='cart') // if current page is related to cart then call cartcalc function to refresh the cart totals and number of items in cart
			{
				$cartData = cartCalc(); // calling cart calc 
			}	
			if ($position=='right') // show only if position value is top
			{
				$cart_tot = print_price(get_session_var('cart_total'),true);
				$pass_tot = print_price(get_session_var('cart_total'),true,true);
		...
		?>
						...
						<form method="get" name="frm_quicksearch" class="frm_cls" action="<?php url_link('search.html')?>" onsubmit="return handle_srch_click_new()">

						<span class="searchtxt"><?php echo $title ;?></span>
						<table border="0" cellspacing="0" cellpadding="0" style="width:97%;">
						<tr>
						<td><input name="quick_search" type="text" class="inputAB ac_input" id="quick_search" value="Search here" onblur="if (this.value==''){ this.value='Search here'}" onfocus="if (this.value=='Search here'){ this.value=''}" /></td>
						<td><input name="button_submit_search" type="submit" class="buttongrayBA" id="button3" value="<?php echo $Captions_arr['COMMON']['SEARCH_GO']?>" /></td>
						</tr>
						</table>
						</form>
						</div>
						<script type="text/javascript">
						function handle_srch_click_new()
						{
							var srvl = document.getElementById('quick_search').value.trim();
							if(srvl != '' && srvl !='Search here')
							{
								//show_wait_button(document.getElementById('button3'),'Please wait...');
								return true;
							}
							else
							{
						...
						<?php /*
					<div class="topsearch" align="left">
					<form method="post" name="frm_quicksearch" class="frm_cls" action="<?php url_link('search.html')?>">
						<table border="0" cellspacing="0" cellpadding="0">
						<tbody>
						<tr>
							<td><input name="quick_search" type="text" class="inputAB" id="quick_search"  value=""/></td>
							<td><input name="button_submit_search" type="submit" class="buttongrayB" id="button3" value="<?php echo $Captions_arr['COMMON']['SEARCH_GO']?>" onclick="show_wait_button(this,'Please wait...')" /></td>
		...
		<?php	
			...
			if ($position=='left' && ($ecom_siteid == 95 || $ecom_siteid == 120 || $ecom_siteid == 61)) // show only if position value is top
			{
				$cart_tot = print_price(get_session_var('cart_total'),true);
				$pass_tot = print_price(get_session_var('cart_total'),true,true);
		...
		function mod_customerlogin($title)
		{
			...
			//echo "position is - ".$position."<br>"."<br>";echo $ecom_siteid;
			if ($position=='topband' && ($ecom_siteid == 95 || $ecom_siteid == 120 || $ecom_siteid == 61))
			{
				if (!$cust_id)
				{
		?>		<a href="<?php url_link('custlogin.html')?>" class="loginlink" title="<?php echo $Captions_arr['CUST_LOGIN']['CUSTOMER_LOGIN']?>"><?php echo $Captions_arr['CUST_LOGIN']['CUSTOMER_LOGIN']?></a> <span class="loginseparator"></span> <a href="<?php url_link('registration.html')?>" class="loginlink" title="<?php echo $Captions_arr['CUST_LOGIN']['CUSTOMER_REGISTER']?>"><?php echo $Captions_arr['CUST_LOGIN']['CUSTOMER_REGISTER']?></a>
        <?php
				}
				else
				{
		?>		<a href="<?php url_link('logout.html')?>" class="userloginmenuytoplink"><?=$Captions_arr['LOGIN_MENU']['LOGOUT']?></a>
        <?php
				}
			}
			else
		...
		function mod_bestsellers($ret_main,$title,$display_id)
		{
			...
			<?php						$price_arr =  show_Price($row_main,array(),'compshelf',false,4);
					...
					if ($proceed)
					{
						...
						if ($db->num_rows($ret_prod))
						{
							...
							if($shelfData['shelf_currentstyle']=='nor' || $shelfData['shelf_currentstyle']=='new') // case of christmas layout
							{
								...
								if($shelfData['shelf_showtitle']==1)// whether title is to be displayed
								{
								?>				 <div class="shlf_slr_pdt_name"><a href="<?php url_product($row_prod['product_id'],$row_prod['product_name'],-1)?>" title="<?php echo stripslashes($row_prod['product_name'])?>"><?php echo stripslashes($row_prod['product_name'])?></a>
								</div>
								<?php		
								}
							...
							else if($shelfData['shelf_currentstyle']=='sidenew')
							{
								//echo "<pre>";
								//print_r($shelfData);die();
								switch($shelfData['shelf_displaytype'])
								{
									...
									case '3row':
									//echo "<pre>";print_r($shelfData);
								?>		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="shlf_slr_table_oname">
								<?php	if ($title)
								{
								?>		<tr><td class="shlf_slr_table_hdr_oname"><?php echo $title?></td></tr>
								<?php	}
								while($row_prod = $db->fetch_array($ret_prod))
								{
								?>		<tr>
								<td class="shlf_slr_table_pdt_oname">
								<div class="shlf_slr_pdt_otr_oname">
								<?php
								if($shelfData['shelf_showtitle']==1)// whether title is to be displayed
								{
								?>				 <div class="shlf_sidenew_name"><a href="<?php url_product($row_prod['product_id'],$row_prod['product_name'],-1)?>" title="<?php echo stripslashes($row_prod['product_name'])?>"><?php echo stripslashes($row_prod['product_name'])?></a>
								</div>
								<?php		
								}
								?>
      ...
      <div class="gift_use_banner"> 
		...
		{
			global $position,$ecom_siteid,$db,$ecom_hostname,$Settings_arr,$Captions_arr;
			//echo "position - ".$position." <br> site id - ".$ecom_siteid;
						...
						if ($proceed)
						{
							...
							if($cache_exists!=true)
							{
								...
								{ 
									...
									elseif ($position=='right') // ############## Right ##############
									{
										...
										if ($groupData['shopbrandgroup_listtype']== 'Menu')// case if shops are to be shown in list menu
										{
											...
											while ($row_shop = $db->fetch_array($ret_shop))
											{
												...
												?>
												 <li><h2><a href="<?php url_shops($row_shop['shopbrand_id'],$row_shop['shopbrand_name'],-1,$groupData['shopbrandgroup_id'])?>" class="shoplinkright" title="<?php echo stripslashes($row_shop['shopbrand_name'])?>"><?php echo stripslashes($row_shop['shopbrand_name']);?></a></h2></li>
												<?php
												if ($row_shop['shopbrand_subshoplisttype']=='List')
												{
													if (($row_shop['shopbrand_id']==$_REQUEST['shop_id']) or ($row_shop['shopbrand_id']==$parent))
													{
														...
														<?php	
															...
															{
														?>
																	<li><h2><a href="<?php url_shops($row_child['shopbrand_id'],$row_child['shopbrand_name'],-1,$groupData['shopbrandgroup_id'],$row_shop['shopbrand_id'])?>" title="<?php echo stripslashes($row_child['shopbrand_name'])?>" class="shoplinkright"><?php echo stripslashes($row_child['shopbrand_name']);?></a></h2></li>
														<?php		
															}
														?>
															</ul>
									...
									elseif($position == 'mainbodytop' && ($ecom_siteid == 95 || $ecom_siteid == 120 || $ecom_siteid == 61))
									{
										// Check the listing type for shop in product shop group
										if ($groupData['shopbrandgroup_listtype']== 'Menu')// case if shops are to be shown in list menu
										{
                                    ...
                                    <div id="slider1_container" style="position: relative; top: 10px; left: 13px; width: 710px; height: 60px; overflow: hidden; ">
		...
		?>
			...
			<tr>
				...
				<li><h2 align="right"><a href="#" class="showall" title="<?php echo $Captions_arr['COMMON']['SHOW_ALL']?>" onclick="if(confirm('Are You sure You want to clear the recent product list')){ document.<?=$frm_name?>.submit()};"><?php echo $Captions_arr['COMMON']['COMON_RECENT']?></a></h2></li>
				</ul>
				</td>
				</tr>
			</table>
		...
		function mod_adverts($advert_arr,$title)
		{
			...
			//echo "position is - ".$position."<br><br>site id - ".$ecom_siteid;
			if ($position=='mainbodytop' && ($ecom_siteid == 95 || $ecom_siteid == 120 || $ecom_siteid == 61))
			{
				if (count($advert_arr))
				{
				...
				if (count($advert_arr))
				{
					foreach ($advert_arr as $d=>$k)
					{
						...
						if ($cache_exists==false)
						{
							...
							if($k['advert_id'] == 821 || $k['advert_id'] == 317 || $k['advert_id'] == 827)
							{
								$idadv = 'id="advert_comp_table_id"';
							   
							}
					?>
							<table width="100%" border="0" cellpadding="0" cellspacing="0" class="advert_comp_table" <?php echo $idadv; ?>>
					<?php	
							if($title)
							{/*
					?>
				  ...
				  <?php
						...
						?>
						<tr>
						<td align="center" valign="middle" style="background-color:#fff;height:2px;font:size:10px;">&nbsp;</td>
						</tr>
						<?php
						}*/
						?>
						...
						{
						?>
						<div style="background-color:#fff;height:6px;font:size:10px;"></div>
						<?php
						}
						?>
						<?php
		...
		?>
			<tr>
                ...
                <?php /* FB login script */
					...
					{
				?>
				<li><h2><a href="<?php echo $logoutUrl; ?>" class="userloginmenuytoplink"><?=$Captions_arr['LOGIN_MENU']['LOGOUT']?></a></h2></li>
				<?php
					}
					else
					{
				...
				?>
				<li><h2><a href="<?php url_link('mypricepromise.html')?>" class="userloginmenuytoplink">
		...
		function mod_preorder($ret_main,$title,$display_id)
		{
			...
			{	
				...
				<?php
							...
							{
				?>	
								<li><h2><a href="<?php url_product($row_main['product_id'],$row_main['product_name'],-1)?>" class="preorderlink" title="<?php echo stripslashes($row_main['product_name'])?>"><?php echo stripslashes($row_main['product_name'])?></a></h2></li>
		
				<?php
						
							}		
				?>
								<li><h2 align="right"> <a href="<? url_link('preorder'.$display_id.'.html')?>" class="showall" title="<?php echo $Captions_arr['COMMON']['SHOW_ALL']?>" <?php /*onclick="document.bestseller_component_left<?=$display_id?>.submit();"*/?>><?php echo $Captions_arr['COMMON']['SHOW_ALL']?></a> </h2> </li>
							</ul>
				...
				?>
								<li><h2 align="right"><a href="<? url_link('preorder'.$display_id.'.html')?>" class="showall" title="<?php echo $Captions_arr['COMMON']['SHOW_ALL']?>" <?php /*onclick="document.bestseller_component_right<?=$display_id?>.submit();"*/?>><?php echo $Captions_arr['COMMON']['SHOW_ALL']?></a> </h2> </li>
							</ul>
