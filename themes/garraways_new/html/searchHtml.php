				        ...
				        if ($_REQUEST['req']!='')// LIMIT for products is applied only if not displayed in home page
						{
								...
								switch($Settings_arr['search_prodlisting'])
									{
									  case '2row':
								  ?>
										<table id="shoppingcarttable_id" border="0" cellpadding="0" cellspacing="0" class="shelfBtable">
										<tr>
							...
							if(trim($search_desc)) {
									$top_content = $search_desc;
										...
										<tr>
				<td colspan="3" class="shelfAheader_A" ><?php echo paging_show_totalcount($tot_cnt,'Product(s)',$start_var['pg'],$start_var['pages'])?></td></tr>
			<tr>
										<tr>
								...
								while ($row_search = $db->fetch_array($ret_search))
								{
									$prodcur_arr[] = $row_search;
								     ...
								     <td align="left" valign="top" class="<?php echo $cls?>">
										...
										<tr>
										<td colspan="2" class="prod_list_name_td"><div class="prod_list_name">
										<? //if($cat_det['product_showtitle']==1)// whether title is to be displayed
											 {?>
											<div class="prod_list_name_link"><span class="shelfBprodname"><a href="<?php url_product($row_search['product_id'],$row_search['product_name'],-1)?>" title="<?php echo stripslashes($row_search['product_name'])?>"><?php echo stripslashes($row_search['product_name'])?></a></span>
											</div>
											<? 
											}
										...
										?>
										<div class="prod_list_img_div"><a href="<?php url_product($row_search['product_id'],$row_search['product_name'],-1)?>" title="<?php echo stripslashes($row_search['product_name'])?>">
										<?php											
										...
										/*echo '<div class="prod_list_bonus">';
										...
										<? if ($cat_det['product_showshortdescription']==1)// Check whether description is to be displayed
											 {?>
										<div class="prod_list_des"><?php echo stripslashes($row_search['product_shortdesc'])?><?php show_moreinfo($row_search,'list_more')?></div>
										<?php
										}
