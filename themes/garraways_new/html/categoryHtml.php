<?php
	...
	class category_Html
	{
	  ...
	  ?>
        ...
        <table border="0" cellpadding="0" cellspacing="0" class="shelfAtable" id="shoppingcarttable_id">
		<tr>
        <td colspan="3" > 
			  ...
			  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="categoreyimage">
				...
				<?php 
				   ...
				   <? if($custom_id)
						...
						{
				?>
							<a  href="javascript:download_pdf_common('<?php echo $_SERVER['HTTP_HOST']?>','<?php echo $_SERVER['REQUEST_URI']?>')" title="<?php echo $Captions_arr['CAT_DETAILS']['CATDET_DOWNLOADPDF']?>"><img src="<?php url_site_image('pdf_download.gif')?>" alt="<?php echo $Captions_arr['CAT_DETAILS']['CATDET_DOWNLOADPDF']?>" border="0" style="width:14px;height:14px;" /></a>
				<?php	 
						}
				...
				if($num_prods>0) {	
				   	?>
						<a href="<?php url_category_rss($row_cat['category_id'],$row_cat['category_name'])?>" title="<?php echo $Captions_arr['CAT_DETAILS']['CAT_RSS']?>"><img src="<?php url_site_image('rss_icon.gif')?>" alt="<?php echo $Captions_arr['CAT_DETAILS']['CAT_RSS']?>" border="0" style="width:14px;height:14px;"/></a>
				 <? } ?>  		</div>
				   	</form>
				   </td>
				</tr>
		...
		?>
			  	<table id="shoppingcarttable_id" width="100%" border="0" cellpadding="0" cellspacing="2" class="subcategoreytable">
				...
				<?php
								...
								if($img_support && $row_cat['category_showimage']==1  && $subcategory_showimagetype!='None' ) // ** Show sub category image only if catimage module is there for the site subcategoreynamelink
								 ...
								 <?php
										...
										switch($subcategory_showimagetype)
									     ...
									     $row_subcat['category_name'] = htmlspecialchars($row_subcat['category_name']);
				  ...
				  <?php 
									...
									{
								  ?>
										<a href="<?php url_category($row_subcat['category_id'],$row_subcat['category_name'],-1,$_REQUEST['catgroup_id'],0)?>" class="subcategoreyimage_link" title="<?php echo stripslashes(htmlspecialchars($row_subcat['category_name']))?>">
								 <?php
										...
										if ($row_subcat['category_showimageofproduct']==0) // Case to check for images directly assigned to category
										{
											...
											if(count($img_arr))
											{
												show_image(htmlspecialchars(url_root_image($img_arr[0][$pass_type],1)),$row_subcat['category_name'],$row_subcat['category_name']);
												$show_noimage = false;
											}
											else
												$show_noimage = true;
										...
										else // Case of check for the first available image of any of the products under this category
										{
											...
											if ($cur_prodid)// case if any product with image assigned to it under current category exists
											{
												...
												if(count($img_arr))
												{
													show_image(htmlspecialchars(url_root_image($img_arr[0][$pass_type],1)),$row_subcat['category_name'],$row_cat['category_name']);
													$show_noimage = false;
												}
												else 
													$show_noimage = true;
							...
							<table width="100%" border="0" cellpadding="0" cellspacing="0" >
								...
								<td colspan="2" class="prod_list_name_td">
									<div class="prod_list_name">
										...
										{
										?>
											<div class="prod_list_name_link"><span class="shelfBprodname"><a href="<?php url_product($row_prod['product_id'],$row_prod['product_name'],-1)?>" title="<?php echo stripslashes(htmlspecialchars($row_prod['product_name']))?>"><?php echo stripslashes(htmlspecialchars($row_prod['product_name']))?></a></span>
											</div>
										<? 
										}
										...
										{
										?>
											<div class="prod_list_bulk"><img src="<?php url_site_image('bulk.png')?>" alt="bulkdisc" /></div>
										<?php 
										} 
										?>
									</div>
								...
								<tr>
									<td class="prod_list_img_td" valign="top">
										...
										$row_prod['product_name'] = htmlspecialchars($row_prod['product_name']);
										?>
											<div class="prod_list_img_div">
												<a href="<?php url_product($row_prod['product_id'],$row_prod['product_name'],-1)?>" title="<?php echo stripslashes(htmlspecialchars($row_prod['product_name']))?>">
												<?php											
												// Calling the function to get the image to be shown
								...
								if ($cat_det['product_showshortdescription']==1)// Check whether description is to be displayed
								 {
							    ?>
								<div class="prod_list_des"><?php echo stripslashes(htmlspecialchars($row_prod['product_shortdesc']))?><?php show_moreinfo($row_prod,'list_more')?></div>
								<?php
								}
								if($row_prod['product_saleicon_show']==1)
								{
		...
		function Show_NoProducts()
		{
			global $Captions_arr;
		?>
			<table border="0" cellpadding="0" cellspacing="0" class="shelfBtable" id="shoppingcarttable_id">
        	<tr>
		...
		?>
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="subcategoreytable" id="shoppingcarttable_id">
				<?php
				if($alert)
				{
				?>
