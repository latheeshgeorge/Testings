...
function Show_ProductDetails($row_prod)
{
		...
		<img src="'.url_site_image('proddet_loading.gif',1).'" style="width:43px;height:11px;" alt="loading..." width="43px" height="11px;align:left">
		</div></div>';	
		?>
			...
			if($ecom_siteid==61 || $ecom_siteid==95)
			{
				...
				{ ?>
					...
					<td align="right" colspan="2">
						<?php
						/*
					<img src="<? url_site_image("Request_Lease_Quote.gif")?>" alt="" style="width:28px;height:27px" align="absmiddle" border="0">
					<a class="fontleesmall" href="http://<?php echo $ecom_hostname?>/pg695/request-lease-quote.html">Request Lease Quote</a><br />
					*/ ?> 
					<img src="<? url_site_image('Send_Brochure_Pack.gif');?>" align="absmiddle" style="width:28px;height:27px">
		...
		*/ 
		...
		if($db->num_rows($ret_tab))
		{
				...
				while ($row_tab = $db->fetch_array($ret_tab))
				{
						...
						{
							echo $sql_comm_tab = "SELECT tab_content FROM product_common_tabs WHERE 
							common_tab_id = ".$row_tab['product_common_tabs_common_tab_id']."
							AND sites_site_id = ".$ecom_siteid."  
							AND tab_hide=0 LIMIT 1";
								$ret_comm_tab = $db->query($sql_comm_tab);
...
function Show_Linked_Product($ret_prod)
{
		...
		$prod_compare_enabled = isProductCompareEnabled();
					...
					{
					?>											<div class="prod_list_bulk"><img src="<?php url_site_image('bulk.png')?>"   style="width:auto;"/></div>
					<?php				}
					?>										</div>
					</td>
					</tr>
...
function show_offer_buttons($row_prod)
{
					...
					$freedel_override = true;
					if($in_combo==1 or $freedel_override or $row_prod['product_show_pricepromise']==1)
					{
						?>
						...
						{
							?>	
							<div class="prod_price_promice"><a href="javascript:handle_price_promise()" title="Price Promise"><img src="<?php url_site_image('promise.jpg')?>" border="0" style="width:auto;"/></a></div>
							<?php
						}	
						if($in_combo==1)
						{
							?>
							<div class="prod_combo"><a href="<?php url_link('showallbundle'.$row_prod['product_id'].'.html')?>" title=""><img src="<?php url_site_image('combo-offer.gif')?>" border="0"/></a></div>
							<?php
						}
						...
						{
							?>	
							<div class="prod_free"><a href="<?php url_link('freedelivery'.$row_prod['product_id'].'.html')?>" title="Free Delivery"><img src="<?php url_site_image('free-del.gif')?>" border="0" style="width:auto;" /></a></div>
							<?php
						}
						?>
						</div>
...
{
global $Captions_arr,$showqty,$Settings_arr,$ecom_hostname,$ecom_siteid;
					$addto_cart_withajax =  $Settings_arr['enable_ajax_in_site'];
					$cust_id 	= get_session_var("ecom_login_customer");
					$showqty	= $Settings_arr['show_qty_box'];// show the qty box
...
function show_buttons_ajax($row_prod)
					...
					if ($row_prod['product_show_enquirelink']==1)
					{
						...
						{
							?>
						<input name="Submit_enq" type="submit" class="buttonblackbuy" id="Submit_enq" value="<?php echo $Captions_arr['PROD_DETAILS']['PRODDET_ENQUIRY'];?>" onclick="show_wait_button(this,'Please Wait...');document.frm_proddetails_ajax.fpurpose.value='Prod_Enquire'" />
						<?php
...
function add_enq_vars(var_id,value_id)
{
	document.frm_proddetails.fpurpose.value='Prod_Enquire';
	//alert(document.frm_proddetails.fpurpose.value);	
	document.getElementById('var_'+var_id).value=value_id;
	//alert(document.getElementById('var_'+var_id).value);
	document.frm_proddetails.submit();
}
</script>
<?php
								...
								while ($row_var = $db->fetch_array($ret_var))
								{
										...
										if ($var_Proceed)// Show the variable if it is valid to show
										{
											...
											$value_id = '';
												...
												<?php /*?><input type="hidden" id="var_<?php echo $var_id?>" name="var_<?php echo $var_id?>" value="<?php echo $value_id?>"/><?php */?>
												...
												<div class="p_variable_buy_C_Otr">
													...
													if ($row_prod['product_show_enquirelink']==1)
													{
														  if($ecom_siteid==95)
														  {
															  ...
															  {
														   ?>
                                                           <input type="hidden" name="var_<?php echo $var_id ?>" id="var_<?php echo $var_id ?>" value="" />
                                                           <?php
															  }
														   ?>
														   <div class="p_variable_buy_C_Q"><a href="javascript:void(0)" onclick="add_enq_vars('<?php echo $var_id ?>','<?php echo $value_id ?>');"><?php echo $Captions_arr['PROD_DETAILS']['PRODDET_ENQUIRY'];?></a></div><?php /*?>document.<?php echo $frm_name;?>.submit();<?php */?>
														   <?php
														   		$var_id_added	=	1;
														  }
														  else														   
													...
													?>	
														<?php /*?><div class="p_variable_buy_C_Q"><a href="javascript:void(0)" onclick="ajax_addto_cart_special_display('add_prod_tocart_ajax','Prod_Enquire','<?php echo $frm_name ?>','<?php echo SITE_URL?>','<?php echo $var_id ?>','<?php echo $value_id ?>')"><?php echo $Captions_arr['PROD_DETAILS']['PRODDET_ENQUIRY'];?></a></div>	<?php */?>								
													<?php 
													}
													if ($Settings_arr['proddet_showwishlist'])
													{
