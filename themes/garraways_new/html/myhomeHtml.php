...
class myhome_Html
{
	function Display_WelcomeMessage($mesgHeader,$Message)
	{
		...
		list($c_title,$username,$c_lastname,$customer_discount,$allow_discount) = $db->fetch_array($ret_user);
...
?>		<div class="treemenu"><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> </div>
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="loginwelcomemsg_table" id="shoppingcarttable_id">
		<tr>
			<td align="left" valign="middle" class="loginwelcomemsg_header" > 
<?php			
				$rp_txt = $c_title.' '.$username.' '.$c_lastname;
				$str = str_replace("[username]",$rp_txt,$Captions_arr['LOGIN_HOME']['LOGIN_HOME_HEADER_MESG']);
