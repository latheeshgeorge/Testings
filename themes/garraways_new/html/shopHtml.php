<?php
	...
	class shop_Html
	{
				 ...
				 <tr>
		            ...
		            <input type="hidden" name="caturl" value="<? echo $url?>" />
                     ...
                     <input type='hidden' name='shop_id' value="<?=$_REQUEST['shop_id']?>"/>
				  	 <div class="categoreyimagediv" align="right"><?php /*<a href="<?=url_Shop_PDF($_REQUEST['shop_id'],$row_shop['shopbrand_name'])?>" class="productdetailslink" title="<?php echo $Captions_arr['SHOP_DETAILS']['SHOPDET_DOWNLOADPDF']?>">
				  	 <img src="<?php url_site_image('pdf_download.gif')?>" alt="<?php echo $Captions_arr['SHOP_DETAILS']['SHOPDET_DOWNLOADPDF']?>" border="0" style="width:14px;height:14px;" /></a>*/?></div>
				   </form>
				 	</td>
				 </tr>
				 <?
		...
		function Show_Products($ret_prod,$tot_cnt,$start_var,$shop_det)
		{
			...
			{
			?>							
			<div class="prod_list_bulk">
				<img src="<?php url_site_image('bulk.png')?>" />
			</div>
			<?php			
			}
			?>						
			...
			*/
			...
			<td colspan="2">
				...
				<?php echo stripslashes($row_prod['product_shortdesc'])?><?php show_moreinfo($row_prod,'list_more')?></div>
