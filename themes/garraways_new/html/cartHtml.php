<?php
	...
	class cart_Html
	{
		...
		?>	
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="shoppingcarttable" id="shoppingcarttable_id">
			<?php
			...
			<?php
				...
				<tr>
				  <td colspan="5" align="left" valign="middle" class="cartlogin_msg">
					...
					{
					  echo $Captions_arr['CART']['CART_LOGGED_IN_AS']; echo "&nbsp;".$cartData['customer']['customer_title']." ".$cartData['customer']['customer_fname']." ".$cartData['customer']['customer_mname']." ".$cartData['customer']['customer_surname'];?>
						<br />
		  ...
		  else
		  {
			  ...
			  foreach ($cartData['products'] as $products_arr)
			  {
				...
				if($ecom_siteid==61) // only for garraways
				{
					if($products_arr['combination_id']) // case if combination exists
					{
						$sql_unqprod = "SELECT comb_special_product_code FROM product_variable_combination_stock WHERE comb_id = ".$products_arr['combination_id']." LIMIT 1";
						$ret_unqprod = $db->query($sql_unqprod);
						if($db->num_rows($ret_unqprod))
						{
							$row_unqprod 	= $db->fetch_array($ret_unqprod);
							$spunique_id	= $row_unqprod['comb_special_product_code'];
						}	
					} 
					else
					{
						// call a new function to check whether combination id can be retrieved based on selected variables
						$combinationid = 0;
						$pvar_arr = array();
						if($products_arr['prod_vars'])
						{
							foreach($products_arr["prod_vars"] as $productVars)
							{
								if (trim($productVars['var_value'])!='')
									$pvar_arr[$productVars['var_id']] = $productVars['var_value_id']; 
							}	
							$com_sp_arr 	= find_combination_id_special($products_arr['product_id'],$pvar_arr);
							$combinationid 	= $com_sp_arr['combid'];
						}
						if($combinationid)
						{	
							//echo "here ".$combinationid." --- ".$products_arr['product_id'];
							$sql_unqprod = "SELECT comb_special_product_code FROM product_variable_combination_stock WHERE comb_id = ".$combinationid." LIMIT 1";
							$ret_unqprod = $db->query($sql_unqprod);
							if($db->num_rows($ret_unqprod))
							{
								$row_unqprod 	= $db->fetch_array($ret_unqprod);
						...
						{
							$sql_unqprod = "SELECT product_special_product_code FROM products WHERE product_id = ".$products_arr['product_id']." LIMIT 1";
							$ret_unqprod = $db->query($sql_unqprod);
							if($db->num_rows($ret_unqprod))
							{
								$row_unqprod = $db->fetch_array($ret_unqprod);
								$spunique_id	 = $row_unqprod['product_special_product_code'];
							}
						}	
					}
					if($spunique_id)
					{
						$sp_caption = $Captions_arr['CART']['CART_SPCODE_CAPTION'].': '.$spunique_id;
					}
				}
			  ...
			  ?>
				  <tr>
					...
					<td align="left" valign="middle" class="<?php echo $tdpriceBclass?>"><?php echo print_price($products_arr['product_webprice'])?></td>
					...
					<div class="updatediv" align="center"><a href="#" onclick="document.frm_cart.remcart_id.value='<?php echo $products_arr['cart_id']?>';handle_form_submit(document.frm_cart,'Update_qty','#upd')" title="<?php echo $Captions_arr['CART']['CART_UPDATE']?>"><img src='<?php url_site_image('cart_update.gif')?>' border="0" alt="<?php echo $Captions_arr['CART']['CART_UPDATE']?>" style="width:10px;height:11px;"></a>  <a href="#" class="update_link" onclick="if (confirm_message('<?php echo $Captions_arr['CART']['CART_ITEM_REM_MSG']?>')) {document.frm_cart.remcart_id.value='<?php echo $products_arr['cart_id']?>';handle_form_submit(document.frm_cart,'Remove_qty','#rem')}" title="<?php echo $Captions_arr['CART']['CART_REMOVE']?>"><img src='<?php url_site_image('cart_delete.gif')?>' border="0" alt="<?php echo $Captions_arr['CART']['CART_REMOVE']?>" style="width:10px;height:11px;"></a></div>
					<?php
						}
					?>
					</td>
			  ...
			  $class_changeA    = 'shoppingcartcontent';
			  $class_changeB    = 'shoppingcartpriceC';
			  $class_changeAB   = 'shoppingcartcontent';
			  $class_changeBB    = 'shoppingcartpriceC';
			  if($proceed_new   == true)
			  {
			  $class_changeA    = 'shoppingcartcontent_new';
			  $class_changeB    = 'shoppingcartpriceC_new';
			  $class_changeAB   = 'shoppingcartcontent_newB';
			  $class_changeBB    = 'shoppingcartpriceC_newB';
			  }
		  ?>
			 	<tr>
				<td align="right" valign="middle" class="<?php echo $class_changeA ?>" colspan="4"><?php echo $Captions_arr['CART']['CART_TOTPRICE']?></td>
				<td align="right" valign="middle" class="<?php echo $class_changeB ?>"><?php echo print_price($cartData["totals"]["subtotal"],true)?></td>
			  </tr>
		  <?php
			...
			?>	
						<tr>
						<td colspan="3" align="left" valign="middle" class="shoppingcartcontent"><a name="a_gwrap">&nbsp;</a><?php echo $Captions_arr['CART']['CART_GIFTWRAP_REQ']?> (<?php echo print_price($row_gift['giftwrap_minprice'],true)?>)</td>
		  ...
		  ?>
				<tr>
				<td colspan="5" align="right" valign="middle" class="shoppingtd_Rev">
				...
				?>	
						 <tr>
						 <td align="right" class="shoppingcartcontent_deliveryA" colspan=""><?php echo $Captions_arr['CART']['CART_DELIVERY_LOC']?><a name="a_deliv">&nbsp;</a> 
					     </td>
					   </tr>
								
					   <tr>
						   <td align="right" class="shoppingcartcontent_deliveryB" colspan="">
					   <?php
								
									echo $this->generateradio('cart_deliverylocation',$deliverydet_Arr['locations'],'',$row_cartdet['location_id'],'handle_form_submit(document.frm_cart,"save_commondetails","#a_deliv")','','shoppingcartcontent_deliveryT','shoppingcartcontent_deliveryC');
				   ...
				   <?php
						...
						?>
						 <tr>
						 <td align="right" class="shoppingcartcontent_delivery" colspan="2"><?php echo $Captions_arr['CART']['CART_DELIVERY_LOC']?><a name="a_deliv">&nbsp;</a> 
							<?php
				   ...
				   ?>
				    <tr>
						 <td align="right" class="shoppingcartcontent_deliveryA" colspan=""><?php echo $Captions_arr['CART']['CART_DELIVERY_OPT']?><a name="a_deliv">&nbsp;</a> 
					   </tr>
								</td>
					   <tr>
						   <td align="right" class="shoppingcartcontent_deliveryB" colspan="">
					   <?php
						if($row_cartdet['delopt_det_id']==0)
						{
						  $row_cartdet['delopt_det_id'] = key($deliverydet_Arr['del_groups']);
						}	
						echo $this->generateradio('cart_deliveryoption',$deliverydet_Arr['del_groups'],'',$row_cartdet['delopt_det_id'],'handle_form_submit(document.frm_cart,"save_commondetails","#a_deliv")','','shoppingcartcontent_deliveryT','shoppingcartcontent_deliveryC');
					   ...
					   ?>
					   <tr>
						 <td align="right" class="shoppingcartcontent"><?php echo $Captions_arr['CART']['CART_DELIVERY_OPT']?> </td>
			...
			?>
			<tr>
				<td colspan="4" align="right" valign="middle" class="<?php echo $class_changeA ?>"><?php echo $Captions_arr['CART']['CART_TOTAL_DELIVERY_CHARGES']?></td>
				<td align="right" valign="middle" class="<?php echo $class_changeB ?>"><?php echo print_price($cartData["totals"]["delivery"],true)?></td>
			  </tr>
			<?php
			}
			 ...
			 ?>
				 <tr>
					<td colspan="4" align="right" valign="middle" class="<?php echo $class_changeA ?>"><?php echo $Captions_arr['CART']['CART_EXTRA_SHIPPING']?></td>
					<td align="right" valign="middle" class="<?php echo $class_changeB ?>"><?php echo print_price($cartData["totals"]["extraShipping"],true)?></td>
				  </tr>
			 <?php	
			 }
		   // Section to show the tax details
			 ...
			 ?>
					<tr>
						<td colspan="4" align="right" valign="top" class="<?php echo $class_changeA ?>">
						...
						?>						</td>
						<td align="right" valign="top" class="<?php echo $class_changeB ?>">
						<?	
							foreach($cartData["tax"] as $tax)
							{
				...
				if (($row_cartdet['promotionalcode_id']!=0 or $row_cartdet['voucher_id']!=0))
				{
				 ?>
				  <tr>
					<td colspan="4" align="right" valign="middle" class="<?php echo $class_changeA ?>">
					<?php 
					...
					?>					</td>
					<td align="right" valign="middle" class="<?php echo $class_changeB ?>">
					<?php 
					if ($row_cartdet['promotionalcode_id']!=0 and $cartData["bonus"]['type']=='promotional' and $cartData['totals']['pro_type']!='product')
		  ...
		  ?>
				  <tr>
					<td colspan="5" align="left" valign="middle"><div class="shopprodiv">
					  <table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
						  <td width="71%" align="left" class="<?php echo $class_changeA ?>"><a name="a_prom">&nbsp;</a>
						  <?php	
						  ...
						  ?>						  </td>
						  <td width="29%" align="right" class="<?php echo $class_changeA ?>">
						  <?php
		  ...
		  <?php
			...
			if (is_Feature_exists('mod_bonuspoints'))
			{
				...
				?>
					<tr>
					<td colspan="5" align="right"><div class="cart_bonus_more">
				<?php
			...
			{	
				...
				?>
					 <tr>
						<td colspan="1" align="left" valign="middle" class="<?php echo $class_changeA ?>"><?php echo $Captions_arr['CART']['CART_TOTAL_POINTS_EARN']?>&nbsp;</td>
						<td colspan="4" align="left" valign="middle" class="<?php echo $class_changeB ?>"><?php echo $cartData["totals"]["bonus"]?></td>
					  </tr>
				<?php	
				}
				if($cust_id) // case if logged in
				{
					if ($cartData["customer"]["customer_bonus"] and $Settings_arr['cust_allowspendbonuspoints']==1) // does current customer have bonus points
					{
						...
						?>
						 ...
						 <tr>
							<td colspan="1" align="left" valign="middle" class="<?php echo $class_changeA ?>"><a name="bonus">&nbsp;</a><?php echo $Captions_arr['CART']['CART_SPENT_BONUS_POINTS']?></td>
							<td colspan="4" align="left" valign="middle" class="<?php echo $class_changeA ?>">
						  ...
						  /*
						  <tr>
							<td colspan="1" align="left" valign="middle" class="<?php echo $class_changeA ?>"><a name="bonus">&nbsp;</a><a href='http://www.garraways.co.uk/pg49716/amazing-children-uganda.html' target='_blank' style='color:#000000'><?php echo $Captions_arr['CART']['CART_DONATE_BONUS_POINTS']?></a></td>
							<td colspan="4" align="left" valign="middle" class="<?php echo $class_changeA ?>">
							<input type="hidden" name="maxBonusPoints"  class="input" value="<? print $cartData["customer"]["customer_bonus"]; ?>">
							<input type="text" name="donateBonusPoints" size="4" value="<? print $cartData["bonus"]["donating"]; ?>">
							<input type="hidden" name="leftBonusPoints" value="<? print $cartData["bonus"]["left"] ?>">
							<input  class="buttonred_cart" type="button" value="<?php echo $Captions_arr['CART']['CART_DONATE']?>" onclick="handle_form_submit(document.frm_cart,'save_commondetails','#bonus')">
							&nbsp;&nbsp;<strong>(<? print (int)(($cartData["bonus"]["left"])); ?> &nbsp;<?php echo $Captions_arr['CART']['CART_REMAINING']?>)</strong>
							</td>
						  </tr>
						  */
						...
						?>
						  <tr>
							<td colspan="4" align="right" valign="top" class="<?php echo $class_changeA ?>"><?php echo $Captions_arr['CART']['CART_SPENT_BONUS_POINTS_VALUE']?> </td>
							<td colspan="1" align="right" valign="top" class="<?php echo $class_changeB ?>"><? echo '(-) '.print_price($cartData["bonus"]["value"],true);?></td>
						  </tr>
				<?php	
						}
					}
				...
				else
				{
					...
					?>
					<tr>
						<td colspan="1" align="left" valign="middle" class="<?php echo $class_changeAB ?>"><?php echo $Captions_arr['CART']['CART_TOTAL_POINTS_EARN']?>&nbsp;</td>
						<td colspan="4" align="left" valign="middle" class="<?php echo $class_changeBB ?>"><?php echo $cartData["totals"]["bonus"]?></td>
					  </tr>
				<?php	
					}
				}
			...
			?>
				<tr>
					<td class="shoppingcartcontent" colspan="5" align="left">
			...
			?>
			  ...
			  <tr>
				<td colspan="4" align="right" valign="middle" class="<?php echo $class_changeA ?>"><?php echo $Captions_arr['CART']['CART_TOTAL_FINAL_COST']?>&nbsp; </td>
				<td align="right" valign="middle" class="<?php echo $class_changeB ?>"><?php echo print_price($cartData["totals"]["bonus_price"],true)?></td>
			  </tr>
		  <?php
		  	}
			...
			?>
				<tr>
					<td colspan="4" align="right" valign="middle" class="<?php echo $class_changeA ?>"><?php echo $Captions_arr['CART']['CART_LESS_AMT_LATER']?>&nbsp; </td>
					<td align="right" valign="middle" class="<?php echo $class_changeB ?>"><?php echo print_price($str_reduceval,true)?></td>
			  	</tr>
				<tr>
					<td colspan="4" align="right" valign="middle" class="<?php echo $class_changeA ?>"><?php echo $Captions_arr['CART']['CART_TOTAL_AMT_NOW']?>&nbsp; </td>
					<td align="right" valign="middle" class="<?php echo $class_changeB ?>"><?php echo print_price($rem_val,true)?>
...
// Show the payment type and payment method sections only if total amount > 0
			...
			<?php
				...
				if ($db->num_rows($ret_paytypes))
				{
					...
					else
					{
						 if($proceed_new==true)
						 {
							$Settings_arr['paytype_listingtype'] = 'radio';
					     }  
					...
					<?php
						...
						// Check how the payment types to be displayed to the user
						if ($Settings_arr['paytype_listingtype']=='radio') // Case of payment listing type is to shown as drop downbox
						{
							$paymethod[0] = $Captions_arr['CART']['CART_SELECT'];
							while ($row_paytypes = $db->fetch_array($ret_paytypes))
							{
								if($row_paytypes['paytype_code']=='pay_on_account')
								{
									...
									$add_text = '';
								$paymethod[$row_paytypes['paytype_id']] = stripslashes($row_paytypes['paytype_caption']).$add_text;	
								$paymethod2[$row_paytypes['paytype_id']] = $row_paytypes['paytype_forsites_id'];
								if($row_paytypes['paytype_code']=='credit_card')
								{
									if($row_paytypes['paytype_id']==$row_cartdet['paytype_id'])
										$cc_exists = true;
					...
					?>	
							  ...
							  <table width="100%" border="0" cellspacing="0" cellpadding="0">
								<tr>
						 <td align="right" class="shoppingcartcontent_deliveryA" colspan=""><?php echo $Captions_arr['CART']['CART_SEL_PAYTYPE']?>
					     </td>
					   </tr>
					   ...
					   <tr>
						   <td align="right" class="shoppingcartcontent_deliveryB" colspan="">
								<a name="a_pay">&nbsp;</a>								
								 <?php
										if($totpaycnt>1)// decide whether to add the onchange event
											echo $this->generateradio('cart_paytype',$paymethod,$paymethod2,$row_cartdet['paytype_id'],'handle_form_submit(document.frm_cart,"save_commondetails","#a_pay")','','shoppingcartcontent_deliveryT','shoppingcartcontent_deliveryP');
										else 
									  		echo $this->generateradio('cart_paytype',$paymethod,$paymethod2,$row_cartdet['paytype_id'],'','','shoppingcartcontent_deliveryT','shoppingcartcontent_deliveryP');
				...
				if ($db->num_rows($ret_paymethods))
				{
					...
					else
					{
					 if($proceed_new==true)
					 {
						 while ($row_paymethods = $db->fetch_array($ret_paymethods))
									{
										...
										$paymethodA[$row_paymethods['paymethod_id']] = $caption;
									}	
						?>
						<tr>
						<td colspan="5" align="left" valign="middle">
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
								<tr>
						 <td align="right" class="shoppingcartcontent_deliveryA" colspan=""><?php echo $Captions_arr['CART']['CART_SEL_PAYGATEWAY']?>
					     </td>
					   </tr>
					   ...
					   <tr>
						   <td align="right" class="shoppingcartcontent_deliveryB" colspan="">
								 <?php
											echo $this->generateradio('cart_paymethod',$paymethodA,'',$row_cartdet['paymethod_id'],'','','shoppingcartcontent_deliveryT','shoppingcartcontent_deliveryC');
		...
		<?php
			...
			?>
				...
				?>
					<tr>
						<td align="center" valign="top" class="google_tdA" colspan="2">
						<center>	
						<div style=" font-weight:bold; font-size:12px; color:#604713; padding-left:20px;">
						Pay securely by going directly to PayPal
						</div>
							<?php
							if($ecom_siteid == 61 || $proceed_new==true) // garraways 61 live id 
							{
								//if(($totpaycnt>0) or ($google_exists and $google_recommended==0))
								{
									/*?>
						...
						<div style=" font-weight:bold; font-size:12px; color:#604713; padding-left:20px;">
						Paypal Accepts Credit Card Payments
						</div>
						</center>
						</td>
					</tr>	
		...
		?>
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="shoppingcarttable" id="shoppingcarttable_id" style="padding-top:10px;padding-bottom:10px;">
		<tr>
			<td style="align:left;font-weight:bold;font-style:italic;font-size:11px;color:#FF0000;">
		SECURITY NOTICE</td>
		</tr>
		<tr>
			<td style="align:left;font-weight:normal;font-style:italic;font-size:11px;color:#FF0000;">
			Due to recent updates to maintain your security at the checkout, SagePay and PayPal have disabled support for Internet Explorer 6 (IE6). If you are using this browser, you will be unable to proceed to the payments page and it is recommended that you either upgrade to a newer version or access Garraways using alternative browsers such as Firefox, Google Chrome or Safari.
			</td>
		</tr>
		</table>
		<?php
		  ...
		  ?>
 		  ...
 		  <tr>
		       <td valign="top" class="cust_regmiddle_td_left">
				  <table width="100%" border="0" cellspacing="4" cellpadding="0" class="cust_regmiddle">
			<tr>
			...
			<?php
			  ...
			  <td valign="top" class="cust_regmiddle_td_right">
			  <table width="100%" border="0" cellspacing="4" cellpadding="0"  class="cust_regmiddle">
          <tr>
          ...
          <tr>
            ...
            <td width="54%"><input name="custlogin_uname" id="custlogin_uname" type="text" class="inputA_new" size="40" /></td>
          </tr>
          <tr>
            <td align="right" valign="middle" class="regiconent"><?php echo $Captions_arr['CART']['CART_PASSWORD']?></td>
            <td><input name="custlogin_pass" id="custlogin_pass" type="password" class="inputA_new" size="40" /></td>
          </tr>
          <tr>
            <td>&nbsp;</td>
            <td> 
		  ...
		  <input type="hidden" name="pass_url" id="pass_url" value="<?php echo $ps_url?>" />
		  	<input type="hidden" name="cart_mod" value="show_cart" /> 
			<input name="custcartlogin_Submit" type="submit" class="buttonred_cartA" value="Login" <?php /*?>onclick="document.frm_custlogin_cart.submit();"<?php */?> />
			</td>
	      </tr>
		  <?php
			if($Settings_arr['hide_forgotpass']==0) // check whether the forgot password link is disabled from main shop settings
			{
			?>
			  <tr>
				<td>&nbsp;</td>
		...
		function generateradio($name,$option_values,$option_values2=array(),$selected,$onclick='',$class='',$classA='',$classB='',$noid=false,$id='') 
		{
			$return_value ="
			<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\" class=\"$classA\" width=\"100%\">";
			...
			foreach($option_values as $k => $v) {
				...
				<tr>"
				;
				if($option_values2[$k]>0)
				{
					$return_value .="
							<td valign=\"middle\" align=\"right\" class=\"$classB\" colspan=\"\">";
						// image to shown for payment types
						$pass_type = 'image_iconpath';
						// Calling the function to get the image to be shown
						$img_arr = get_imagelist('paytype',$option_values2[$k],$pass_type,0,0,1);
						if(count($img_arr))
						{
							$return_value .=show_image(url_root_image($img_arr[0][$pass_type],1),$v,$v,'','',1);
						}
						else
						{
						
							$return_value .="<img src=\"".url_site_image('cash.gif',1)."\" alt=\"Payment Type\"/>";
						}
						$return_value .="</td>";
				}
				$return_value .="
				...
				$return_value .= "<input type='radio' name='$name' value='$k' checked='checked'";
				}else
				{
				$return_value .= "<input type='radio' name='$name' value='$k'";
				}
			 }
		else {
				if($selected == $k) {
            ...
            <td align=\"left\" class=\"$classB\" colspan=\"\">
            ";
		...
		?>	
		  ...
		  <?php
			...
			if ($deliverydet_Arr['delivery_type'] != 'None') // Check whether any delivery method is selected for the site
			{
				...
				?>	
							<tr>
								<td colspan="2" align="left" valign="middle" class="shoppingcartcontent"><?php echo $Captions_arr['CART']['CART_DELIVERY_LOC']?></td>
				...
				?>	
							<tr>
								<td colspan="2" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo $Captions_arr['CART']['CART_DELIVERY_OPT']?></td>
		  ...
		  <?php
			...
			?>
				 <tr>
					...
					<td colspan="3" align="center" valign="middle" class="shoppingcartcontent"><?php echo $cartData["bonus"]["donating"]?></td>
				  </tr>
			<?php	
			}
			/* Donate bonus End */
		  ...
		  ?>
			...
			<div class="cart_continue_div" align="left">
	         ...
	         <div class="cart_continue_div_new" style="float:right;width: 50%;float:left;padding-top:5px;">
	             <?php
	             if (!$cust_id)
	             {
	             ?>
					<strong><?php echo $Captions_arr['CART']['CART_NOT_LOGGED_IN']; ?> <a class="cartlogin_link" href="http://<?php echo $ecom_hostname?>/custlogin.html?redirect_back=1&pagetype=checkout"><? echo $Captions_arr['CART']['CART_HERE'];?></a><? echo "&nbsp;". $Captions_arr['CART']['CART_TO_LOGIN'];?></strong>
				<?php
				}
				?>	
	         </div>  
...
//				$cont_class = 'shoppingcartcontent_noborder'; 
			...
			?>
			  <tr>
				<td colspan="5" align="left" valign="middle" class="cartterms">			
					 <?php
		...
		function Show_orderPreview_new($return_order_arr)
		{
			return;
				global $ecom_siteid,$db,$ecom_hostname,$Settings_arr,$ecom_themeid,$default_layout,
			...
			if($row_ord['order_paymentmethod']!='') // If payment method exits
			{
				...
				$auto_submit	= false;
				?>
				<table width="100%" border="0" cellpadding="0" cellspacing="0" class="shoppingcarttable_Rev">
			<?php
			if($auto_submit==false)
			{
			?>
			<tr>
			<td  align="left" valign="middle"><div class="treemenu"><a href="http://<?php echo $ecom_hostname?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> >> <?php echo $Captions_arr['CART']['CART_CHECKOUTPREVIEWHEADING']?></div></td>
			</tr>

			<tr>
			<td  align="left" valign="middle">

			<div class="orderReviewNewWrap">
			<div class="orderReviewNewLeft">
			<?php
			// Including the file to show the dynamic fields for orders to the give position of static fields
			$cur_pos 		= 'Top';
			$show_header	= 1;
			include 'show_dynamic_fields_orders_new.php';
			?>

			<div class="orderReviewNewbillwrap">
			<span class="billTitles"><?php echo $Captions_arr['CART']['CART_BILL_DETAILS']?></span>
			...
			while($row_checkout = $db->fetch_array($ret_checkout))
			{
				...
				{
				$exc = 0;
				if($row_checkout['field_orgname']=='order_custtitle')
				{ 
				   $exc = 1;
				   $en1 = 1;
				   $name1 = stripslashes($row_ord[$row_checkout['field_orgname']]);
				}
				if($row_checkout['field_orgname']=='order_custfname')
				{
				   $exc = 1;
				   $en2 = 1;
				   $name2 = " ".stripslashes($row_ord[$row_checkout['field_orgname']]);
				}	
				if($row_checkout['field_orgname']=='order_custmname')
				{
				   $exc = 1;
				   $en3 = 1;
				   $name3 = " ".stripslashes($row_ord[$row_checkout['field_orgname']]);
				}	
				if($row_checkout['field_orgname']=='order_custsurname' && $en4==0)
				{
				   $exc = 1;
				   $en4 = 1;
				   $name4 = " ".stripslashes($row_ord[$row_checkout['field_orgname']]);
			...
			?>							
				<?php
				 ...
				 elseif($exc==0 && $row_checkout['field_orgname']!='order_notes')
				 {   
			...
			{	
			?>
			<span class="billTitles"><?php echo $Captions_arr['CART']['DELIVERY_ADDRESS']?></span>
			<div class="billContent">
			<?php		
			// Get the list of billing address static fields to be shown in the checkout out page in required order
			$sql_checkout = "SELECT field_name,field_orgname 
			...
			{	
				...
				{
				$exc = 0;
				if($row_checkout['field_orgname']=='delivery_title')
				{ 
				   $exc = 1;
				   $en1 = 1;
				   $name1 = stripslashes($row_del[$row_checkout['field_orgname']]);
				}
				if($row_checkout['field_orgname']=='delivery_fname')
				{
				   $exc = 1;
				   $en2 = 1;
				   $name2 = " ".stripslashes($row_del[$row_checkout['field_orgname']]);
				}	
				if($row_checkout['field_orgname']=='delivery_mname')
				{
				   $exc = 1;
				   $en3 = 1;
				   $name3 = " ".stripslashes($row_del[$row_checkout['field_orgname']]);
				}	
				if($row_checkout['field_orgname']=='delivery_lname' && $en4==0)
				{
				   $exc = 1;
				   $en4 = 1;
				   $name4 = " ".stripslashes($row_del[$row_checkout['field_orgname']]);
				   $name  = $name1.$name2.$name3.$name4;
				}
			...
			?>
			<div class="orderReviewNewbillwrap">
			<span class="billTitles"><?php echo $Captions_arr['CART']['CART_DELIVERY_LOC_NEW']?></span>
			<?php
			// Case if location is to be displayed
			...
			?>	
			<div class="billContent"><?php //echo $Captions_arr['CART']['CART_DELIVERY_LOC']?>
				<?php echo stripslashes($row_ord['order_deliverylocation'])?>
				</div>
			<?php
			}
			// Check whether any delivery group exists for the site
			...
			{
			?>
			<span class="billTitles"><?php echo $Captions_arr['CART']['CART_DELIVERY_OPT_NEW']?></span>
	
			<div class="billContent"><?php //echo $Captions_arr['CART']['CART_DELIVERY_OPT']?>
				<?php echo stripslashes($row_ord['order_delivery_option'])?></div>
			<?php
			}	
			if ($row_ord['order_splitdeliveryreq']==1)
			{
			?>
			<div class="billContent"><?php echo $Captions_arr['CART']['CART_DELIVERY_SPLIT_ACTIVATED']?>:<?php echo $Captions_arr['CART']['CART_YES']?></div>
			<?php	
			}
			?>				
			...
			?>
			<div class="orderReviewNewbillwrap">
			<span class="billTitles"><?php echo $Captions_arr['CART']['CART_PAYMENT_TYPE']?></span>
			<div class="billContent"><?php echo getpaymenttype_Name($row_ord['order_paymenttype']); ?></div>

			<?php
			if($row_ord['order_paymenttype'] =='credit_card')
			{
			?>
			<span class="billTitles"><?php echo $Captions_arr['CART']['CART_GATEWAY_USED']?></span>
			<div class="billContent"><?php echo getpaymentmethod_Name($row_ord['order_paymentmethod']); ?></div>
			<?php
			}
			?>
			</div>
			...
			<div class="orderReviewNewRight">
			<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
			<tr>
			<td colspan="3" align="left" valign="middle" class="reviewTitle"><?php echo $Captions_arr['CART']['CART_REV_HEAD']?></td>
			</tr>
			<tr >
			<td width="55%" class="reviewTdLineHead"><?php echo $Captions_arr['CART']['CART_ITEM_REV']?></td>
			<td width="31%" align="right" class="reviewTdLineHead" ><?php echo $Captions_arr['CART']['CART_QTY']?></td>
			<td width="14%" align="right" class="reviewTdLineHead"><?php echo $Captions_arr['CART']['CART_TOTAL_REV']?></td>
			</tr>
			<?PHP
			// Following section iterate through the products in cart and show its list
			...
			?>
			<tr >
			<td align="left" valign="middle" class="reviewTdLine"><span class="reviewItemTitle"><a href="<?php echo url_product($row_orddet['products_product_id'],$row_orddet['product_name'])?>" title="<?php echo $row_orddet['product_name']?>" class="reviewItemTitle_link"><?php echo stripslashes($row_orddet['product_name'])?></a></span> 
			<?php
			// If variables exists for current product, show it in the following section
			if ($vars_exists) 
			{
			?>
			<div class="reviewItemVariable">
			<?
			...
			while($row_vars = $db->fetch_array($ret_vars))
			{
				if (trim($row_vars['var_value'])!='')
					print "<span class='reviewItemVariable_cartvariable'>".$row_vars['var_name'].": ". $row_vars['var_value']."</span><br />"; 
				else
					print "<span class='reviewItemVariable_cartvariable'>".$row_vars['var_name']."</span><br />"; 
			...
			{
				print "<span class='reviewItemVariable_cartvariable'>".stripslashes($row_msgs['message_caption']).": ".stripslashes($row_msgs['message_value'])."</span><br />"; 
			}	
			}	
			?>
			</div>	
			...
			<td align="right" valign="top" class="reviewTdLineQty"><?php 
			...
			echo $row_orddet["order_qty"]
			?></td>
			<td align="right" valign="top" class="reviewTdLineTot"><?php echo print_price($row_orddet['order_rowtotal'],true)?> </td>
			</tr>
			<?php
			}
			?>
			<tr class="reviewTdLine">
			<td  class="reviewTdLine">&nbsp;</td>
			<td  class="reviewTdLine">&nbsp;</td>
			<td  class="reviewTdLine">&nbsp;</td>
			</tr>
			<tr class="reviewTdLine">
			<td  class="reviewTdLine">&nbsp;</td>
			<td align="right"  class="reviewTdLine"><?php echo $Captions_arr['CART']['CART_TOTAL_REV']?></td>
			<td align="right"  class="reviewTdLine"><?php echo print_price($row_ord["order_subtotal"],true)?></td>
			</tr>
			<?php
			...
			<td align="right"  class="reviewTdLine"><?php echo $Captions_arr['CART']['CART_GIFTWRAP_REQ']?> (<?php echo print_price($row_ord['order_giftwrap_minprice'],true)?>)</td>
			<td align="right"  class="reviewTdLine"><?php if ($row_ord['order_giftwrap']=='Y') echo 'Yes'?></td>
			</tr>
			<?php
			...
			<td align="right"  class="reviewTdLine"><?php echo $Captions_arr['CART']['CART_GIFTWRAP_MSG']?>(<?php echo print_price($row_ord['order_giftwrap_message_charge'],true)?>)</td>
			<td align="right"  class="reviewTdLine">Yes</td>
			</tr>
			<tr>
			<td  class="reviewTdLine">&nbsp;</td>

			<td align="right"  class="reviewTdLine"><?php echo $Captions_arr['CART']['CART_MSG']?></td>
			<td align="right"  class="reviewTdLine"><?php echo nl2br(stripslashes($row_ord['order_giftwrapmessage_text']))?></td>
			</tr>
			<?php
			}
			...
			<td align="right"  class="reviewTdLine">
			<?php 
			...
			<td align="right"  class="reviewTdLine"><?php echo stripslashes($row_giftwrap_det['giftwrap_name'])?></td>
			</tr>
			<?php		
			}
			?>
			...
			<td align="right"  class="reviewTdLine"><?php echo $Captions_arr['CART']['CART_GIFTWRAP_TOTAL']?></td>
			<td align="right"  class="reviewTdLine"><?php echo print_price($row_ord["order_giftwraptotal"],true)?></td>
			</tr>
			<?php		
			}
			}
			...
			<tr class="reviewTdLine">
			<td  class="reviewTdLine">&nbsp;</td>
			<td align="right"  class="reviewTdLine"><?php echo $Captions_arr['CART']['CART_TOTAL_DELIVERY_CHARGES']?></td>
			<td align="right"  class="reviewTdLine"><?php echo print_price($row_ord["order_deliverytotal"],true)?></td>
			</tr> 
			<?php
			}
			}
			...
			<td  class="reviewTdLine">&nbsp;</td>
			<td align="right"  class="reviewTdLine"><?php echo $Captions_arr['CART']['CART_EXTRA_SHIPPING']?></td>
			<td align="right"  class="reviewTdLine"><?php echo print_price($row_ord["order_extrashipping"],true)?></td>
			</tr>
			<?php
			}
			// Section to show the tax details
			...
			<tr class="reviewTdLine">
			<td  class="reviewTdLine">&nbsp;</td>
			<td align="right"  class="reviewTdLine"><?php echo $Captions_arr['CART']['CART_TAX_CHARGE_APPLIED']?>
			<?	
			$charge_arr = array();
			...
			<td  class="reviewTdLine">&nbsp;</td>
			<td align="right"  class="reviewTdLine"><?php
			...
			<tr class="reviewTdLine">
			<td  class="reviewTdLine">&nbsp;</td>
			<td align="right"  class="reviewTdLine"><?php echo $Captions_arr['CART']['CART_SPENT_BONUS_POINTS_VALUE']?> </td>
			<td align="right"  class="reviewTdLine"><? echo '(-) '.print_price($row_ord['order_bonuspoint_discount'],true);?> </td>
			</tr>
			...
			<tr class="reviewTdLine">
			<td  class="reviewTdLine">&nbsp;</td>
			<td align="right"  class="reviewTdLine"><?php echo $Captions_arr['CART']['CART_BONUS_DONATING']?></td>
			<td align="right"  class="reviewTdLine"><?php echo $row_ord["order_bonuspoints_donated"]?></td>
			</tr>
			...
			<tr class="reviewTdLine">
			<td  class="reviewTdLine">&nbsp;</td>
			<td align="right"  class="reviewTdLine"><?php echo $Captions_arr['CART']['CART_TOTAL_POINTS_EARN']?></td>
			<td align="right"  class="reviewTdLine"><?php echo $row_ord["order_bonuspoint_inorder"]?></td>
			</tr>
			...
			<tr class="reviewTdLine">
			<td  class="reviewTdLine">&nbsp;</td>
			<td align="right"   class="reviewTdLine grandTotal"><strong><?php echo $Captions_arr['CART']['CART_TOTAL_FINAL_COST_REV']?></strong></td>
			<td align="right"  class="reviewTdLine grandTotal"><strong><?php echo print_price($row_ord['order_totalprice'],true)?></strong></td>
			</tr>
			...
			<tr class="reviewTdLine">
			<td  class="reviewTdLine">&nbsp;</td>
			<td align="right"  class="reviewTdLine"><?php echo $Captions_arr['CART']['CART_LESS_AMT_LATER']?></td>
			<td align="right"  class="reviewTdLine"><?php echo print_price($row_ord['order_deposit_amt'],true)?> </td>
			</tr>
			<tr class="reviewTdLine">
			<td  class="reviewTdLine">&nbsp;</td>
			<td align="right"  class="reviewTdLine grandTotal"><strong><?php echo $Captions_arr['CART']['CART_TOTAL_AMT_NOW']?></strong></td>
			<td align="right"  class="reviewTdLine grandTotal"><strong><?php echo print_price($rem_val,true)?> </strong></td>
			</tr>
			...
			?>
			 <tr class="reviewTdLine">
				...
				<td colspan="3" align="left"  class="reviewTdnoLine grandTotal">
				<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0" class="reviewTdbt">
				<tr>
				<td width="49%" align="left">&nbsp;<input value="CHANGE YOUR ORDER" name="backto_cart" type="button" class="order_Edit_bt" id="backto_cart" onclick="show_wait_button(this,'Please Wait...');gobackto_cart('<?php echo ($protectedUrl)? base64_encode($ecom_hostname):$ecom_hostname?>')"/></td>
				<td width="51%" align="right"><?php
				$display_option = 'ALL';
		...
		function Show_OrderPreview($return_order_arr)
		{
			...
			if($proceed_new==true)
			{
			    $this->Show_orderPreview_new($return_order_arr);
			}
			else
			{
		...
		?>	
			<tr>
				<td colspan="5" align="left" class="shoppingcartheader"><?php echo $Captions_arr['CART']['CART_BILL_DETAILS']?></td>
			</tr>
			<?php
					// Including the file to show the dynamic fields for orders to the give position of static fields
					$cur_pos 		= 'TopInStatic';
				...
				?>
							<tr>
								<td align="left" colspan="2" class="shoppingcartcontent_noborder">
			...
			?>
			  ...
			  ?>
			<tr>
            <td colspan="5" align="right" valign="middle" class="shoppingcartcontent inputnone">	
           	<?php
		...
		function Show_CheckoutSuccess($order_id=0)
		{
			...
			$fp = fopen ('finance_test/paypalsuccess_response.txt','a+');
			...
			/*
				...
				##################################################################################
			*/
			if($ecom_isadword) // this is disabled as the purchase script is placed in theme function Google_conversion_onpage()
			{
				if ($order_id>0)
				{
				?>
		...
		?>
			...
			?>
					<tr>
				<td align="left" class="shoppingcartcontent_indent_highlight1">
                <table cellpadding="3" cellspacing="3" border="0" width="100%">
                <tr>
                <td style="font-family:Tahoma, Geneva, sans-serif; font-size:14px;font-weight:bold;color:#AA0000;" align="left">Thank you for considering buying from Garraways.<?php //echo $Captions_arr['CART']['CART_CHECKOUT_FAILED_MSG']?></td>
                </tr>
                <tr>
                <td style="font-family:Tahoma, Geneva, sans-serif; font-size:12px;font-weight:normal;color:#AA0000;"  align="left">
You have been returned to this page as the payment process has failed.  Your items are still in the cart (top right of the screen) if you want to add anything else to the cart and still want to proceed with your order.
                <br />
                <br />
                If you are having difficulty with the payment system, please telephone one of our team on 0800 865 4444 and we will be delighted to help.
                <br />
                <br />
                </td>
                 <td style="font-family:Tahoma, Geneva, sans-serif; font-size:12px;font-weight:normal;color:#AA0000;"  align="left">
			...
			<script type="text/javascript">
			 $ajax_j(document).ready(function(){
				  setTimeout(function() {
				   window.location.href = "http://<?php echo $ecom_hostname?>/cart.html"
				  }, 15000);
				});
			</script>
		...
		?>
			...
			<div align="right" class="carterrormsg_span" ><a href="javascript: hide_instockmsg_div()"><img src="<?php url_site_image('close.gif')?>" border="0" style="width:25px;height:25px;"/></a></div>
		...
		?>
			...
			<div align="right" class="carterrormsg_span" ><a href="#" onclick="hide_instockmsg_div();document.frm_cart.cart_savepromotional.value=1;handle_form_submit(document.frm_cart,'save_commondetails','#a_prom')"><img src="<?php url_site_image('close.gif')?>" border="0" style="width:25px;height:25px;"/></a></div>
			<span class="carterrormsg_msg">
			<?php echo $msg?></span>
			</div>
		<?php	
		...
		function site_check()
		{
			global $ecom_siteid;
			$proceed_new = false;
			//if($ecom_siteid==61 or $ecom_siteid==95)
			{
			$proceed_new = true;
			}
			return $proceed_new;		  
