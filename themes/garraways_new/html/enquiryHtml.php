	  ...
	  function enquiry_Showform()
	  {
		...
		//print_r($prod_array );
          ...
          <tr>
            ...
            <?php
            /*
			if($ecom_siteid !=95 || $ecom_siteid !=61)
			{
			?>
			...
			*/ 
			?>
		    <td align="center" valign="middle" class="shoppingcartheaderA"><?php echo $Captions_arr['ENQUIRE']['ENQUIRE_ACTION']?></td>
		  </tr>
		  <?php
		if($db->num_rows($ret_select_enq))
		{  
		while($row_select_enq = $db->fetch_array($ret_select_enq))
		{
		  ...
		  /*$prod_sql_det = "SELECT product_id,product_name,product_webprice,product_variables_exists  FROM products 
				...
				//echo "enquiery id - ".$row_select_enq['products_product_id']."<br>";
				$prod_cart_array[]=$row_select_enq['products_product_id'];
		  ...
		  ?>
			  <tr>
				...
				<?php 
                    ...
                    Please submit your enquiry for the best deal
                    </td>
				<?php
				/*
				if($ecom_siteid != 95 || $ecom_siteid != 61)
				...
				*/ 
				?>
			    <td align="center" valign="middle" class="<?=$tdpriceAclass?>"><img src="<?php url_site_image('delete.gif')?>" onclick="if(confirm_message('Are you sure you want to Delete this Enquiry?')){document.frm_enquire.enq_mod.value='delete_enquiry';document.frm_enquire.enquiry_id.value='<?=$row_select_enq['enquiry_id']?>';document.frm_enquire.submit();}" alt="Delete" title="Delete" /></td>
			  </tr>
			  ...
			  <tr>
			 <td  align="left" valign="middle"   class="<?=$trmainclass?>" >Add Notes:<br /><textarea  name="enquirenote_<?=$row_select_enq['enquiry_id']?>" rows="5" cols="35" id="note_text_<?=$row_select_enq['enquiry_id']?>" ><? if($_REQUEST['enquirenote_'.$row_select_enq['enquiry_id']]) echo $_REQUEST['enquirenote_'.$row_select_enq['enquiry_id']];else echo $row_select_enq['enquiry_note'];?></textarea>&nbsp; <input name="buttonenqsaveone_<?=$row_select_enq['enquiry_id']?>" type="submit" class="buttonred_cart" id="buttonenqsaveone_<?=$row_select_enq['enquiry_id']?>" value="Save Note" /></td>
			 </tr></table></td>
			  </tr>
		  <?php
		...
		<?php
			if($ecom_siteid == 95 || $ecom_siteid == 61)
			{
        		$this->enquiry_Fillform($alert,$cust_id,$prod_cart_array,false);
			}
        ?>
        <?php
				//
		  ...
		  {  
		  ?>
		  <?php /*?><tr><td colspan="6" align="right" valign="middle"><?php */?>
 		  ...
 		  <tr>
			...
			<tr>
				...
				<tr>
				 <td class="regiconent" align="left"> 
			...
			<?php
				...
				<a href="<?php url_link('registration.html?pagetype=enquire')?>" class="cust_login_button">Create New Account&nbsp;</a>
				</div>
				 </td>
				</tr>
				  </table>
		  ...
		  <input type="hidden" name="pass_url" id="pass_url" value="<?php echo $ps_url?>" />
			...
			<input name="custenquirelogin_Submit" type="submit" class="buttonred_cartA" value="Login" onclick="document.frm_custlogin_enquire.submit();" />
			</td>
...
<?php */?>		  
	  ...
	  function enquiry_Fillform($alert,$cust_id,$prod_cart_arr,$showBreadcrumb=true)
	  {
		 /* echo "alert - ".$alert."<br>";
		  echo "cust id - ".$cust_id."<br>";
		  echo "<pre> - ".print_r($prod_cart_arr)."<br>";*/
	   ...
	   $Captions_arr['ENQUIRE'] 	= getCaptions('ENQUIRE');
			...
			$country_arr = array(0=>'-- Select Country --');
						...
						//echo "hai - <pre>".print_r($Bottomparameters);
...
/* Function to validate the Enquie Details*/
			function validate_defaultregistration(frm)
			{
				//alert(frm);alert('hai');
				//return false;
				fieldRequired 		= Array('cbo_enqtitle','txt_enqfname','txt_enqlname','txt_enqpostcode','txt_enqphone','txt_enqemail');
				fieldDescription 	= Array('<?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_TITLE']?>','<?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_FNAME']?>','<?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_LNAME']?>','Post Code','<?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_PHONE']?>','<?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_EMAIL']?>');
				fieldEmail 			= Array('txt_enqemail');
				fieldConfirm 		= Array();
				fieldConfirmDesc  	= Array();
				fieldNumeric 		= Array();
				fieldSpecChars 		= Array('txt_enqfname','txt_enqlname','txt_enqphone');
				fieldCharDesc       = Array('First Name','Last Name','Phone');
				//alert(fieldRequired);
				//alert(fieldDescription);
				//alert(fieldSpecChars);
				//alert(fieldCharDesc);
...
function validate_Bottomregistration(frm)
{
	...
	else
	{
	  ...
	  <form method="post" name="frm_enquire_details" class="frm_cls" action="" onsubmit="return validate_allforms(this);">
			<input type="hidden" name="fpurpose" value="" />
        	<input type="hidden" name="enq_mod" value="show_enquiryform" />
		<?php
	  ...
	  <table width="100%" border="0" cellpadding="0" cellspacing="6" class="regitable">
		  <?php
				$sql_stat = "SELECT * from static_pages WHERE sites_site_id = $ecom_siteid AND pname='enquiry-form-content' LIMIT 1";
				$ret_stat = $db->query($sql_stat);
				if($db->num_rows($ret_stat))
				{
					$row_stat = $db->fetch_array($ret_stat);
					$content = trim(stripslashes($row_stat['content']));
				}
				if($content)
				{
			...
			?>
		  <tr>
		  <td align="left" class="regifontnormal">
			  <?php
			echo $content;
			?>
...
$row_cust = $db->fetch_array($ret_cust);
		...
		$texttd_class= 'regiconent_normal'; 
		$cellpadding = 0;		
		$cellspacing =0;
       ...
       <tr>
		...
		?>
                  <tr>
                   ...
                   <td width="57%" align="left" valign="top" class='regiconent_normal' ><select name="cbo_enqtitle" id="cbo_enqtitle"><option value="" >Select</option><option value="Mr." <? if($row_cust['customer_title']=='Mr.') echo "selected"; else echo '';?>>Mr.</option><option value="Ms." <? if($row_cust['customer_title']=='Ms.') echo "selected"; else echo '';?>>Ms.</option><option value="Mrs." <? if($row_cust['customer_title']=='Mrs.') echo "selected"; else echo '';?>>Mrs.</option>
				   <option value="Miss." <? if($row_cust['customer_title']=='Miss.') echo "selected"; else echo '';?>>Miss.</option>
				   <option value="M/s." <? if($row_cust['customer_title']=='M/s.') echo "selected"; else echo '';?>>M/s.</option></select></td>
                  </tr>
                  <tr>
                    <td class="regiconent" align="left" valign="top" width="33%"><?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_FNAME']?><span class="redtext">*</span></td>

                    <td align="left" valign="top" class='regiconent_normal'><input name="txt_enqfname" value="<?=$row_cust['customer_fname']?>" type="text" maxlength="<?=$short?>"></td>
                  </tr>
                 <?php /*?> <tr>
                    <td class="regiconent" align="left" valign="top"><?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_MNAME']?> </td>
                    <td align="left" valign="top" class='regiconent_normal'><input name="txt_enqmiddlename" value="<?=$row_cust['customer_mname']?>" type="text" maxlength="<?=$short?>"></td>

                  </tr><?php */?>
				   <tr>
                    <td class="regiconent" align="left" valign="top"><?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_LNAME']?><span class="redtext">*</span></td>
                    <td align="left" valign="top" class='regiconent_normal'><input name="txt_enqlname" value="<?=$row_cust['customer_surname']?>" type="text" maxlength="<?=$short?>"></td>

                  </tr>
                  <tr>
                    <td class="regiconent" align="left" valign="top"><?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_ADDRESS']?>  </td>
                    <td align="left" valign="top" class='regiconent_normal'><textarea name="txt_enqaddress" cols="25" rows="4"><?=$_REQUEST['txt_enqaddress']?></textarea></td>
                  </tr>
                 <tr>
                    <td class="regiconent" align="left" valign="top"><?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_STATE']?> </td>
                    <td align="left" valign="top" class='regiconent_normal'>
<?php 				/*$state_arr = array(-1=>'-- Select State --');
                  ...
                  <tr>
                    <td class="regiconent" align="left" valign="top"><?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_PCODE']?> <span class="redtext">*</span> </td>
                    <td align="left" valign="top" class='regiconent_normal'><input name="txt_enqpostcode" value="<?=$row_cust['customer_postcode']?>" type="text" maxlength="<?=$short?>"></td>
                  </tr>
                  <tr>

                    <td class="regiconent" align="left" valign="top"><?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_PHONE']?> <span class="redtext">*</span></td>
                    <td align="left" valign="top" class='regiconent_normal'><input name="txt_enqphone" value="<?=$row_cust['customer_phone']?>" type="text" maxlength="<?=$short?>"></td>
                  </tr>
				  <tr>

                    <td class="regiconent" align="left" valign="top"><?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_MOBILE']?></td>
                    <td align="left" valign="top" class='regiconent_normal'><input name="txt_enqmobile" value="<?=$row_cust['customer_mobile']?>" type="text" maxlength="<?=$short?>"></td>
                  </tr>
               <?php /*?>   <tr>
                    <td class="regiconent" align="left" valign="top"><?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_FAX']?></td>
                    <td align="left" valign="top" class='regiconent_normal'><input name="txt_enqfax" value="<?=$row_cust['customer_fax']?>" type="text" maxlength="<?=$short?>"></td>
                  </tr><?php */?>
                  <tr>
                    <td class="regiconent" align="left" valign="top"><?=$Captions_arr['ENQUIRE']['ENQUIRE_DET_EMAIL']?><span class="redtext">*</span></td>
                    <td align="left" valign="top" class='regiconent_normal'><input name="txt_enqemail" value="<?=$row_cust['customer_email_7503']?>" type="text" maxlength="<?=$medium?>"></td>
                  </tr>
				   <?php /*?><tr>
...
<?php
				   ...
				   <tr>
                    ...
                    <td align="left" valign="top" class='regiconent_normal'><textarea name="cbo_enqtext" rows="2" cols="30"><?=$_REQUEST['cbo_enqtext']?></textarea></td>
                  </tr>
                  <tr>
		...
		<?php 
			...
			?>
			<tr>
			 <td  align="center"  colspan="2" > <input name="enquiry_submit" type="submit" class="buttonred_cart" id="enquiry_submit" value="Submit Enquiry" /></td>
		</tr>
   ...
   <? 
	...
	function Display_Message()
	{
		...
		$tblClass				=	' class="message"';
		if($ecom_siteid ==95 || $ecom_siteid == 61)
		{
			$sql_enq_result_page	=	"SELECT * FROM static_pages WHERE sites_site_id = ".$ecom_siteid." AND pname= 'enquiry-success-content' ORDER BY page_id DESC";
			$ret_enq_result_page	=	$db->query($sql_enq_result_page);
			$enq_result_page_cont	=	0;
			if($db->num_rows($ret_enq_result_page) > 0)
			{
				$row_enq_result_page	=	$db->fetch_array($ret_enq_result_page);
				if(trim($row_enq_result_page['content']) != "")
				{
					$message	=	$row_enq_result_page['content'];
					$tblClass	=	'';
				}
			}
		}
		?>
		<table width="100%" border="0" cellspacing="4" cellpadding="0" class="regifontnormal" id="shoppingcarttable_id">
		<tr>
			<td width="7%" align="left" valign="middle" class="message_header" ><?php echo $header;?></td>		
		</tr>
		<tr>
			<td align="left" valign="middle" <?php echo $tblClass;?>>
			<?php echo $message; ?></td>		
		</tr>
		</table>
	<?php	
		...
		function myenquiry_Enquiry($custid){
				...
				$prod_array = array();
				  ...
				  <table width="100%" border="0" cellpadding="0" cellspacing="2" class="shoppingcarttable" id="shoppingcarttable_id">
					  <tr>
		...
		function myenquiry_EnquiryDetails($enqid)
		{
				...
				//print_r($prod_array );
				  ...
				  <table width="100%" border="0" cellpadding="0" cellspacing="2" class="shoppingcarttable">
					  <tr>
						<td colspan="3" align="left" valign="middle"><div class="treemenu"><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> >> <a href=""><?php echo $Captions_arr['ENQUIRE']['ENQUIRE_MYENQUIRYHEADING']?></a> >> <?php echo $Captions_arr['ENQUIRE']['ENQUIRE_DET_MAIN']?></div></td>
					    <td  align="right" valign="middle" class="shoppingcartheader" >&nbsp;
						<?php
							// Check whether the clear cart button is to be displayed
				...
				?></td></tr>
				<tr><td>State</td><td>: <?=$row_enq['site_state_state_id']?></td></tr>
				 </table></td></tr>
				 <? 
