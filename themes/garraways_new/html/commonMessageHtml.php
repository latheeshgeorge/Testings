...
/*############################################################################
	...
	class commonmessage_Html
	{
		...
		function Newsletter_Message()
		{
			...
			$sql_success_msg	=	"SELECT * FROM static_pages WHERE pname = 'newsletter-success' AND sites_site_id = ".$ecom_siteid;
...
<?php
		...
		function Createaccount_Message()
		{
			...
			$sql_success_msg	=	"SELECT * FROM static_pages WHERE pname = 'create-account-success' AND sites_site_id = ".$ecom_siteid;
			//echo $sql_success_msg;
			$ret_success_msg	=	$db->query($sql_success_msg);
			if ($db->num_rows($ret_success_msg))
			{
