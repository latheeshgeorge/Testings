...
function Show_ProductDetails($row_prod)
{
		...
		if($prod_img_show_type=='FLASH' or $prod_img_show_type=='FLASH_ROTATE')
		{
			...
			if ($Settings_arr['bonus_points_instock'] and $row_prod['product_bonuspoints']>0)
			{
				...
				<span class="bonus_point_caption_a">'.$row_prod['product_bonuspoints'].'</span>
				<span class="bonus_point_number_a"><span>'.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_BONUSPOINTS']).'</span></span></div>';
			}
			echo $HTML_bonus;
		...
		<?php	echo stripslashes($row_prod['product_name']);
...
function show_offer_buttons($row_prod)
{
					...
					if($in_combo==1 or $row_prod['product_freedelivery']==1 or $row_prod['product_show_pricepromise']==1)
					{
						...
						{
							?>	
							<div class="prod_price_promice"><a href="javascript:handle_price_promise()" title="Price Promise"><img src="<?php url_site_image('promise.jpg')?>" border="0"/></a></div>
							<?php
						}	
						if($in_combo==1)
						{
						...
						{
							?>	
							<div class="prod_free"><a href="<?php url_link('freedelivery'.$row_prod['product_id'].'.html')?>" title="Free Delivery"><img src="<?php url_site_image('free-del.gif')?>" border="0" /></a></div>
							<?php
						}
						?>
						</div>
					...
					if ($caption_key!='') // Check whether the buynow or preorder button is to be displayed
					{
							...
							if($showqty==1)// this decision is made in the main shop settings
							{
								if($row_prod['product_det_qty_type']=='NOR')
								{
									...
									<td align="left" valign="middle" class="details_buy_b"><input type="text" class="quainput_big" name="qty" value="<?php echo ($_REQUEST['qty'])?$_REQUEST['qty']:1?>" maxlength="2" />
									</td>
									<?php
								}
										...
										if ($var_Proceed)// Show the variable if it is valid to show
										{
											...
											$value_id = '';
												...
												{	
													...
													<div class="<?php echo ($mod=='normal')?'p_variable_buy_B_A':'p_variable_buy_B'; ?>"><input name="var_qty_<?php echo $value_id?>" value="1" type="text" /></div>
													<?php
													}
													elseif($row_prod['product_det_qty_type']=='DROP')
													{
														...
														?>
															<div class="quantity_details"><?php echo $cur_qty_caption ?>
															<select name="var_qty_<?php echo $value_id?>">
															<?php 
																$qty_prefix = stripslashes($row_prod['product_det_qty_drop_prefix']);
																$qty_suffix = stripslashes($row_prod['product_det_qty_drop_suffix']);
																foreach ($dropdown_values as $k=>$v)
												...
												?>
												<div class="<?php echo ($mod=='normal')?'p_variable_buy_C_A':'p_variable_buy_C'; ?>"><a href="javascript:void(0)" onclick="ajax_addto_cart_special_display('add_prod_tocart_ajax','Prod_Addcart','<?php echo $frm_name ?>','<?php echo SITE_URL?>','<?php echo $var_id ?>','<?php echo $value_id ?>')"><?php echo $Captions_arr['PROD_DETAILS'][$caption_key];?></a></div>
												<?php
											    }											   
												?>
												</div>
