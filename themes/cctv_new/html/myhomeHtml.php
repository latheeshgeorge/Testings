...
/*############################################################################
	...
	# Description 	: Page which holds the display logic for login home page
	# Coded by 		: ANU
					...
					{ 
						...
						if($db->num_rows($ret_products_id )>0 || !$group_id || $row_discount['cust_disc_grp_discount']==0)
						{
							 ...
							 {		
								 ...
								 ?> 
									  <tr>
										  <td align="left" valign="middle" class="loginwelcomemsg_text" colspan='2'><?=stripslash_normal($Captions_arr['LOGIN_HOME']['LOGIN_HOME_NORMAL_DISC'])?></td><td align="left" valign="middle" class="loginwelcomemsg_text">:&nbsp;<?=$customer_discount.'%'?></td>
									  </tr>
								  <?
								  }
							  }  
						 ...
						 {	
							...
							?>
								<tr>
									<td align="left" valign="middle" class="loginwelcomemsg_text" colspan='2'><?=stripslash_normal($Captions_arr['LOGIN_HOME']['LOGIN_HOME_GORUP_DISC'])?></td><td align="left" valign="middle" class="loginwelcomemsg_text">:&nbsp;<?=$row_discount['cust_disc_grp_discount'].'%'?></td>
								</tr>
							<?
							 }
						 }	 
						 ...
						 <a href="<?php url_category($row_subcat['category_id'],$row_subcat['category_name'],-1)?>" class="subcategoreynamelink" title="<?php echo stripslashes($row_subcat['category_name'])?>"><?php echo stripslashes($row_subcat['category_name'])?></a>
									...
									while($row_prod = $db->fetch_array($ret_products_id))
									{
										...
										<table width="100%" border="0" cellpadding="0" cellspacing="0"  class="mid_shelfA_table">
											...
											<?php $frm_name = uniqid('myhomediscprod_'); ?>
			...
			<?php	
				for($i=0;$i<count($cur_cat_arr);$i++)
				{
					...
					$sql_prods_new = "SELECT a.product_id,a.product_name,a.product_variablestock_allowed,a.product_show_cartlink,
												...
												a.product_newicon_show,a.product_newicon_text,a.product_averagerating,product_bonuspoints     
											FROM 
												products a 
											WHERE 
					...
					<?php	
						...
						while ($row_favcat = $db->fetch_array($ret_favcat))
						{
							...
							$sql_prods_offers = "SELECT a.product_id,a.product_name,a.product_variablestock_allowed,a.product_show_cartlink,
														...
														a.product_newicon_text,a.product_averagerating,a.product_bonuspoints 
													FROM  
							...
							?>
							 <div class="mid_shelfB">
													...
													<td class="mid_shelfB_btm_mid">
														<?php
														$frm_name = uniqid('myhomefavprod_');
														?>	
															<form method="post" action="<?php url_link('manage_products.html')?>" name='<?php echo $frm_name?>' id="<?php echo $frm_name?>" class="frm_cls" onsubmit="return product_enterkey(this,<?php echo $row_prod['product_id']?>)">
															<input type="hidden" name="fpurpose" value="" />
															<input type="hidden" name="fproduct_id" value="" />
					...
					?>
						...
						while($row_prod = $db->fetch_array($ret_fav_products))
						{
										...
										<table width="100%" border="0" cellpadding="0" cellspacing="0"  class="mid_shelfA_table">
											...
											<tr>
											<td class="mid_shelfA_btm_lf">&nbsp;</td>
											<?php $frm_name = uniqid('myhomeprod_'); ?>
											<td class="mid_shelfA_btm_mid">
