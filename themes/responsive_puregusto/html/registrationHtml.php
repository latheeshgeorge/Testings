...
<label class="sr-onlyA" for="form-customer_pwd_cnf"><?=stripslash_normal($Captions_arr['CUST_REG']['CUSTOMER_CONF_PASSWORD'])?><span class="redtext">*</span></label>
  ...
  <input name="customer_pwd_cnf" class="form-control" id="customer_pwd_cnf"  type="password" placeholder="<?=stripslash_normal($Captions_arr['CUST_REG']['CUSTOMER_CONF_PASSWORD'])?>" >
  			...
  			<button type="submit" id="chkbutton-bill" class="btn-primary-bt topcart-bt" ><?=stripslash_normal($Captions_arr['CUST_REG']['CUSTOMER_REGISTRATION_SAVE_BUTTON'])?></button>
</div>
  </fieldset>
        ...
        <button class="btncart btn btn-add-to-cart btn-lg sharp" type="submit" onclick="javascript:document.frm_custlogin_middle.submit()" name="button3" id="button3" >Sign in</button>
      <div class="checkbox1">
          <a class="signup_a" href="<?php url_link('registration.html')?>">Signup Now</a>
			<?php /*<a href="<?php url_link('forgotpassword.html')?>" class="lgn_txt_link"><?php echo stripslash_normal($Captions_arr['CUST_LOGIN']['FORGOT_PASS'])?></a> */ ?>
        </div>
      </form>
      </div>
  ...
  echo $HTML_treemenu;	
		?>
			<div class="container forgotcont">
				...
				<input name="forgotpassword_Submit" type="submit" id="forgotpassword_Submit" class="btn-default topcart-bt" value="<?=stripslash_normal($Captions_arr['FORGOT_PASSWORD']['FORGOT_PWD_SUBMIT'])?>">
