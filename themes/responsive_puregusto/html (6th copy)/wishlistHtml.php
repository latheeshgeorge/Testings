              ...
              <? 
					...
					?>
						<div class="cart_shop_cont"><div>
										<input name="clearenq_button" type="button" class="blackbutton" id="clearenq_button" value="<?php echo stripslash_normal($Captions_arr['WISHLIST']['WISHLIST_CLEAR'])?>" onclick="if(confirm_message('<? echo stripslash_javascript($Captions_arr['WISHLIST']['WISHLIST_CLEARALLMSG'])?>')){document.frm_wishlist.wish_mod.value='clear_wishlist';document.frm_wishlist.submit();}" />
						</div></div>
		  ...
		  <?php 
				...
				?>
				  ...
				  <input type="checkbox" name="wishlist_check[]" value="<?=$row_prod_det['product_id']?>" />
					...
					<td align="left" valign="middle" class="cart_td_img" width="20%" >
					<?php 
			  ...
			  ?>
					<div class="cart_shop_cont"><div>
							<input name="submit_wishlist" type="button" class="btn-primary-bt topcart-bt" id="submit_wishlist" value="<?php echo stripslash_normal($Captions_arr['WISHLIST']['WISHLIST_BUYNOW']);?>"  onclick="return select_products();"  />
					</div></div>
			  <?php
					}
					?>
