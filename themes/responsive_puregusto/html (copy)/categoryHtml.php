...
global $Captions_arr,$ecom_siteid,$db,$ecom_hostname,$ecom_themename,$Settings_arr,$ecom_themeid,$default_layout,$inlineSiteComponents,$Settings_arr;
    ...
    <div class="container">
      <div class="row">
				...
				if ($img_support) // ** Support Category Image
				{
					if ($row_cat['category_showimageofproduct']==0) // Case to check for images directly assigned to category
					{
						...
						if(count($catimg_arr))
						{
							...
							$HTML_image = '<div class="product-pic">'.show_image(url_root_image($catimg_arr[0][$pass_type],1),$row_cat['category_name'],$row_cat['category_name'],'imgwraptext','',1).'<ul class="price-avl">
					...
					else // Case of check for the first available image of any of the products under this category
					{
						...
						if ($cur_prodid)// case if any product with image assigned to it under current category exists
						{
							...
							if(count($img_arr))
							{
								$HTML_image = '<div class="product-pic">'.show_image(url_root_image($img_arr[0][$pass_type],1),$row_cat['category_name'],$row_cat['category_name'],'imgwraptext','',1).'<ul class="price-avl">
		...
		<div class="col-md-3"><div class="product-grid">
		...
		function Show_Subcategories($ret_subcat,$url,$row_cat)
		{
			...
			if($alert)
			{
				echo $HTML_alert = '<div class="red_msg">- '.$alert.' -</div>';
					...
					?>
					<div class="container"><div class="col-md-12 listitems">    <h2>recently viewed</h2>
					<?php
						$cur_col = 0;
                    	...
                    	$selval_arr =array();
								$selval_arr = array (
														...
														'product_id'	=> stripslash_normal($Captions_arr['CAT_DETAILS']['SHOPDET_DATEADDED']));
																$cnt=0;	
           echo $select_sortby = ($_REQUEST['catdet_sortby'])?$_REQUEST['catdet_sortby']:'custom';
