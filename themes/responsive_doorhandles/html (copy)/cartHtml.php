<?php
	...
	class cart_Html
	{
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="ctable">
            ...
            $HTML_topdesc1 .= '<div class="cart_top_info">';
            if ($cust_id) // Case logged in 
            {
            $HTML_topdesc1 .=' <div class="cart_top_info_name"><strong>'.stripslash_normal($Captions_arr['CART']['CART_LOGGED_IN_AS']).'&nbsp;'.get_session_var('ecom_login_customer_shortname').'</strong></div>
            ...
            {
            $HTML_topdesc1 .='<div class="cart_top_info_link">'.stripslash_normal($Captions_arr['CART']['CART_NOT_LOGGED_IN']).'&nbsp;<a href="'.url_link('custlogin.html',1).'?redirect_back=1&pagetype=cart" title="Login" class="cartlogin_link">'.stripslash_normal($Captions_arr['CART']['CART_HERE']).'</a>&nbsp;'.stripslash_normal($Captions_arr['CART']['CART_TO_LOGIN']).'</div>';
            }
            $HTML_topdesc1 .= '</div>';
            }
            ...
            {
            //$HTML_topdesc .= '<div class="cart_shop_cont"><div><a href="#" class="btn btn-default add-to-cart" onClick="show_wait_button(this,\'Please Wait...\');window.location=\''.$ps_url.'\'">'.stripslash_normal($Captions_arr['CART']['CART_CONT_SHOP']).'</a></div></div>';
            }
            else
            $HTML_topdesc .= '&nbsp;';
            $HTML_topdesc .= '</div>';
            ...
            <tr>
                <td class="tda"> 
                    ...
                    <tr>
                        <td colspan="2" class="tdbgrys"><a href="<?php echo url_product($products_arr['product_id'],$products_arr['product_name'])?>" title="<?php echo $products_arr['product_name']?>" class="pd_namecart" ><?php echo $products_arr['product_name']?></a></td>
                    </tr>
                    <tr>
                           <td class="image_td" >
							   <?php
						...
						{
							echo "<label class='label_qty'>&nbsp;Tenants:&nbsp";
						...
						{ 
							?>
							<label class='label_qty_text'><span class="qty_tenant">Tenants:&nbsp;</span>
							<input name="cart_qty_<?php echo $products_arr['cart_id']?>" type="text" id="cart_qty_<?php echo $products_arr['cart_id']?>" size="5" maxlength="4" value="<?php echo $products_arr["cart_qty"]?>"  class="det_qty_txt form-control"  />
							                           </label>							
                    ...
                    <tr>
						...
						*/ 
                        ?><?php echo stripslash_normal($Captions_arr['CART']['CART_TOTPRICE'])?>
                        </td>
					...
					<td colspan="6" align="left" valign="middle"><div class="shopprodiv">
					  <table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
						  ...
						  ?>
						  		<input name="cart_savepromotional" type="hidden" id="cart_savepromotional" value="0"/>
							  	<input name="cart_promotionalcode" type="text" id="cart_promotionalcode" size="12" class="form-control" />
							  	<input name="submit_promotionalcode" type="button" class="add-to-cart-in cart_margin" id="submit_promotionalcode" value="<?php echo $Captions_arr['CART']['CART_PROMO_GO']?>" onclick="document.frm_cart.cart_savepromotional.value=1;handle_form_submit(document.frm_cart,'save_commondetails','')" />
						  <?php
						  	}
							/*<a name="a_prom">&nbsp;</a>*/
							/*
		  		...
		  		*/ 
                    ...
                    if ($db->num_rows($ret_paytypes))
                    {
                    	if($db->num_rows($ret_paytypes)==1)// Check whether there are more than 1 payment type. If no then dont show the payment option to user, just use hidden field
						{
							...
							?>
							<input type="hidden" name="cart_paytype" id="cart_paytype" value="<?php echo $row_paytypes['paytype_id']?>" class="" />
							<?php
                    	...
                    	else
						{
							...
							<table width="100%" border="0" cellspacing="0" cellpadding="0" class="gtable">
                                ...
                                <tr>
                                    <td class="td_paymentmaintd"><?php /*<a name="a_pay">&nbsp;</a> */?>
                                    <?php
                    ...
                    if($google_exists and $google_recommended==0 && $show_checkoutbutton==true)
					{
					 ...
					 $HTML_bottomdesc ="<table width='100%' border='0' cellspacing='0' cellpadding='0' class='cont-checkout'>
										<tr>";
                    ...
                    if (count($cartData['products'])>0)
                    {
						...
						if($hide_direct_checkout_button==false)// today start
						{
							if($show_checkoutbutton==true)	
							{
								$HTML_bottomdesc .= '<a class="btn btn-default topcart-bt" href="#" onClick="handle_checkout_submit(\''.$ecom_hostname.'\',0,\''.$cartData["totals"]["bonus_price"].'\')">'.stripslash_normal($Captions_arr['CART']['CART_GO_CHKOUT']).'</a>';
							}
							else
							{
								$HTML_bottomdesc .= "<a class=\"btn btn-default topcart-bt\" href=\"".url_link('custlogin.html',1)."?redirect_back=1&pagetype=cart\" title=\"Login\" class=\"cartlogin_link\">Please Click here to login to continue to Checkout.</a>";
							}
						}// today start
						$HTML_bottomdesc .='</td>';
                    }
                    if($ps_url) // show the continue shopping button only if ps_url have value
                    {
						...
						$HTML_bottomdesc .= '<a class="btn btn-default add-to-cart" href="#" onClick="show_wait_button(this,\'Please Wait...\');window.location=\''.$ps_url.'\'">'.stripslash_normal($Captions_arr['CART']['CART_CONT_SHOP']).'</a>';
						$HTML_bottomdesc .='</td>
						';
                    }
                    else
		...
		?>	
				<div class="container">
						<table class="orderSummaryTable" width="100%">
						<tbody><tr>
						<td colspan="2" class="colHead"><?php echo $Captions_arr['CART']['CART_ITEM']?></td>
						<td class="colHead" align="center">Tenants</td>
...
?>						<tr>						
						     ...
						     <td class="desc" valign="top" colspan="2" align="left">
					...
					if($products_arr['product_det_qty_type']=='DROP')
					{
						...
						<td class="price" valign="top" align="right"><?php echo print_price($products_arr['final_price'],true)?></td>
						</tr>                       
						<tr><td colspan="4">
					<?php
						// If variables exists for current product, show it in the following section
			...
			?>
		  <tr>
				<td  align="right" valign="middle" class="grandTotalHead" colspan="2"><?php echo $Captions_arr['CART']['CART_TOTAL_FINAL_COST']?>&nbsp; </td>
				<td align="right" valign="middle" class="grandTotal" colspan="2"><?php echo print_price($cartData["totals"]["bonus_price"],true)?></td>
			  </tr>
		  <?php
		  	}
			$rem_val = $cartData["totals"]["bonus_price"] - $str_reduceval;
			...
			?>
				<tr>
					<td  align="right" valign="middle" class="totalsHead" colspan="2"><?php echo $Captions_arr['CART']['CART_LESS_AMT_LATER']?>&nbsp; </td>
					<td align="right" valign="middle" class="total" colspan="2"><?php echo print_price($str_reduceval,true)?></td>
			  	</tr>
				<tr>
					<td  align="right" valign="middle" class="grandTotalHead" colspan="2"><?php echo $Captions_arr['CART']['CART_TOTAL_AMT_NOW']?>&nbsp; </td>
					<td align="right" valign="middle" class="grandTotal" colspan="2"><?php echo print_price($rem_val,true)?>
					<input type="hidden" name="store_reduceval" value="<?php echo $str_reduceval?>" />
					</td>
			  	</tr>
			<?php
			...
			<div class="cart_outer">
					...
					<div class="cart_cont_outr">
						<div class="cart_cont_inner_cont_a">
							...
							<div class="chk_back_btn">
								<div><a href="#" class="btn btn-default to-log-in" onclick="show_wait_button(this,'Please Wait...');gobackto_cart('<?php echo ($protectedUrl)? base64_encode($ecom_hostname):$ecom_hostname?>')"><?php echo stripslash_normal($Captions_arr['CART']['CART_BACK_CARTPAGE'])?></a></div>
							</div> 
						</div>
						<form role="form" method="post" name="frm_checkout"  id="frm_checkout" class="registration-form" action="<?php echo $http?>" <?php/*onsubmit="return validate_checkout_fields(document.frm_checkout)" */?>>
						...
						<?php
										$chkout_Req_step		= array();
										$chkout_Req_Desc_step	= array();
										...
										$sql_checkout = "SELECT field_det_id,field_key,field_name,field_req,field_error_msg   
													...
													{	
														...
														<h3>Step 1 / <?php echo $cartData['totals']['qty']; ?></h3>
														<p>Fill the main tenant details</p>
														</div>
														</div>
				...
				$chkout_Req			= $chkout_Req_Desc = $chkout_Email = $chkout_Confirm = $chkout_Confirmdesc = $chkout_Req_step = $chkout_Req_Desc_step = $chkout_Email_step = array();
														...
														{		
															...
															if($row_checkout['field_req']==1)
															{
																if($row_checkout['field_key']=='checkout_email')
																{
																	...
																	$chkout_Email_step[] = "'".$row_checkout['field_key']."'";
																}
																$chkout_Req[]		= "'".$row_checkout['field_key']."'";
																$chkout_Req_Desc[]	= "'".$row_checkout['field_error_msg']."'";
																
																$chkout_Req_step[]		= "'".$row_checkout['field_key']."'";
																$chkout_Req_Desc_step[]	= "'".$row_checkout['field_error_msg']."'";			
															}
																
															?>
																<div class="form-group">
																 <label class="sr-onlyA" for="form-<?php echo $row_checkout['field_key'];?>"><?php echo stripslash_normal($row_checkout['field_name']); if($row_checkout['field_req']==1) { echo '<span class="redtext">*</span>';}?></label>


																	<?php
																	$class_array['txtarea_cls'] = 'regiinput';
																		echo get_Field_responsive($row_checkout['field_key'],$row_checkout['field_name'],$saved_checkoutvals,$cartData['customer'],'',$class_array);
																	?>
																  </div>
														<?php
														}
...
//				$cont_class = 'shoppingcartcontent_noborder';
				...
				include 'show_dynamic_fields.php';	
														...
														function validateStep1(mod)
														{ window.scrollTo(0, 0);
															var isValid = true; 
															<?php
																// Blank checking
																if (count($chkout_Req_step))
																{
																	$chkout_Req_Str 			= implode(",",$chkout_Req_step);
																	$chkout_Req_Desc_Str 		= implode(",",$chkout_Req_Desc_step);
																...
																// Email checking
																if (count($chkout_Email_step))
																{
																	$chkout_Email_Str = implode(",",$chkout_Email_step);
																	echo "fieldEmail 		= Array(".$chkout_Email_Str.");";
																}
																else
																	echo "fieldEmail 		= Array();";
																// Password checking
																if (count($chkout_Confirm_step))
																{
																	$chkout_Confirm_Str 	= implode(",",$chkout_Confirm_step);
																	$chkout_Confirmdesc_Str	= implode(",",$chkout_Confirmdesc_step);
																...
																// Numeric checking
																if (count($chkout_Numeric_step))
																{
																	$chkout_Numeric_Str 	= implode(",",$chkout_Numeric_step);
																	echo "fieldNumeric 		= Array(".$chkout_Numeric_Str.");";
																}
																else
																	echo "fieldNumeric 		= Array();";
																?>
															//input_error(fieldRequired,fieldDescription,fieldEmail);	
															isValid = check_validate_new(fieldRequired,fieldDescription,fieldEmail,mod);	
															return isValid;
														}
														</script>		
														<?php
														...
														{
														?>													
														<button type="button" id="chkbutton-bill" class="btn-next success" onclick="validateStep1('');">Next</button>
														<?php
														}
														else
														{
														?>														
														<button type="button" id="chkbutton-bill" class="btn-default topcart-bt" onclick="validateStep1('submit');">Checkout</button>
													...
													$saved_checkoutvals_extrabilling = get_CheckoutValues_extrabilling($address);
													$cnt = 0;
													if(($cartData['totals']['qty'])>1)
													{
														for($i=1;$i<$cartData['totals']['qty'];$i++)
														{
														$cnt = $i+1;
														...
														<div class="form-top">
														<div class="form-top-left">
														<h3>Step <?php echo $cnt;?> / <?php echo $cartData['totals']['qty'];?></h3>
														<p>Fill <?php echo $tenantname;?> details</p>
														</div>														
														</div>
														<div class="form-bottom">
														<?php
														$chkout_Req_step[$cnt] = $chkout_Req_Desc_step[$cnt] = $chkout_Email_step[$cnt] = array();
														// Get the list of billing address static fields to be shown in the checkout out page in required order
														$sql_checkout = "SELECT field_det_id,field_key,field_name,field_req,field_error_msg   
															FROM 
																general_settings_site_checkoutfields 
														...
														{		
															...
															{ 	
																if($row_checkout['field_key']=='checkout_email')
																{ 
																	$chkout_Email[] = "'".$ext_name.$row_checkout['field_key']."'";
																	$chkout_Email_step[] = "'".$ext_name.$row_checkout['field_key']."'";
																}
																$chkout_Req[]		= "'".$ext_name.$row_checkout['field_key']."'";
																$chkout_Req_Desc[]	= "'".$row_checkout['field_error_msg']." For $tenantname'"; 			
																$chkout_Req_step[$cnt][]		= "'".$ext_name.$row_checkout['field_key']."'";
																$chkout_Req_Desc_step[$cnt][]	= "'".$row_checkout['field_error_msg']." For $tenantname'"; 			
															}
															?>
															<div class="form-group">
																<label class="sr-onlyA" for="form-<?php echo $row_checkout['field_key'];?>"><?php echo stripslash_normal($row_checkout['field_name']); if($row_checkout['field_req']==1) { echo '<span class="redtext">*</span>';}?></label>
																<?php
																$class_array['txtarea_cls'] = 'regiinput';
																	echo get_Field_tenant_responsive($ext_name,$ext_name.$row_checkout['field_key'],stripslash_normal($row_checkout['field_name']),$saved_checkoutvals_extrabilling,$cartData['customer'],'',$class_array);
																?>
															  </div>
														<?php
														...
														?>
															<script type="text/javascript">
															function validateStep<?php echo $cnt?>(mod)
															{ window.scrollTo(0, 0);
																var isValid = true; 
																<?php
																	// Blank checking
																	if (count($chkout_Req_step[$cnt]))
																	{
																		$chkout_Req_Str 			= implode(",",$chkout_Req_step[$cnt]);
																		$chkout_Req_Desc_Str 		= implode(",",$chkout_Req_Desc_step[$cnt]);
																		echo "fieldRequired 		= Array(".$chkout_Req_Str.");";
																		echo "fieldDescription 		= Array(".$chkout_Req_Desc_Str.");";
																	}
																	else
																	...
																	// Email checking
																	if (count($chkout_Email_step[$cnt]))
																	{
																		$chkout_Email_Str = implode(",",$chkout_Email_step[$cnt]);
																		echo "fieldEmail 		= Array(".$chkout_Email_Str.");";
																	}
																	else
																		echo "fieldEmail 		= Array();";
																	// Password checking
																	if (count($chkout_Confirm_step[$cnt]))
																	{
																		$chkout_Confirm_Str 	= implode(",",$chkout_Confirm_step[$cnt]);
																		$chkout_Confirmdesc_Str	= implode(",",$chkout_Confirmdesc_step[$cnt]);
																		echo "fieldConfirm 		= Array(".$chkout_Confirm_Str.");";
																		echo "fieldConfirmDesc 	= Array(".$chkout_Req_Desc_Str.");";
																	}
																	else
																	...
																	// Numeric checking
																	if (count($chkout_Numeric_step[$cnt]))
																	{
																		$chkout_Numeric_Str 	= implode(",",$chkout_Numeric_step[$cnt]);
																		echo "fieldNumeric 		= Array(".$chkout_Numeric_Str.");";
																	}
																	else
																		echo "fieldNumeric 		= Array();";
														...
														?>
														<button type="button" class="btn-default btn-previous to-log-in" onclick="validateStep1('');">Previous</button>					
														<?php
														if($cnt==$cartData['totals']['qty'])
														{
														?>
														<div class="sep_panel_small"></div>
														<button type="button" id="chkbutton-billtenant" class="btn-default topcart-bt" onclick="validateStep<?php echo $cnt?>('submit');">Checkout</button>
														<?php
														}
														else
														{	
														?>
														<div class="sep_panel_small"></div>
														<button type="button" id="chkbutton-billtenant" class="btn-next success" onclick="validateStep<?php echo $cnt?>('');">Next</button>
														<?php
														}
														?>
														</div>
									...
									?>
									<input type="hidden" name="next_step" value="true" id="next_step">	                    
						</div>
						</div>
						</div>
						</div>            
						...
						function check_validate_new(fieldRequired,fieldDescription,fieldEmail,mod)
						{  
							var next_step = true;
							...
							if(fieldRequired.length)
							{
								...
								for(xx=0;xx<fieldRequired.length;xx++)
								{
									var un = $('#'+fieldRequired[xx]).val();
									 if(!un && un.length <= 0){
										 alert_cnt++;
										 $('#'+fieldRequired[xx]).focus();
										 alert("Enter "+fieldDescription[xx]);
										 var next_step = false;
										 $("#next_step").val("false");
										return false;
									}
							...
							{
								for(xx=0;xx<fieldEmail.length;xx++)
								{
									var un = $('#'+fieldEmail[xx]).val();
							...
							else
							{
							  if(mod=='submit')
							  {
							  validate_checkout_fields(document.frm_checkout);
							  }
							  return true;
							}
						...
						function isValidEmailAddress(emailAddress) {
						  ...
						  return pattern.test(emailAddress);
								...
								<tr>
									...
									<td>
									<table width="100%" border="0" cellspacing="0" cellpadding="0" style="dispaly:none">
										<tr>
									<td align="right" valign="top"class="chk_td_chkout">
										<?
											...
											<tr><td valign="bottom" class="checkout_right" height="20px">
...
/*
		...
		function old_validate_checkout_fields(frm)
		{
			<?php
				// Blank checking
				if (count($chkout_Req))
		...
		order_stock_combination_id,order_detail_discount_type,order_orgqty  
		FROM 
		order_details 
		WHERE 
		orders_order_id = ".$return_order_arr['order_id']." 
		...
		$auto_submit	= false;
			...
			echo $HTML_treemenu;	
		?>
		<link href="<?php url_head_link('images/'.$ecom_hostname.'/css/accordion.css')?>" rel="stylesheet" type="text/css" media="screen">
		<script type="text/javascript" src="<?php url_head_link('images/'.$ecom_hostname.'/scripts/accordian/accordion.js')?>"></script>	
		<div class="container">
		...
		<div align="right" style="float:right;padding-bottom:10px;"><input class="btn btn-default to-log-in" type="button" onclick="show_wait_button(this,'Please Wait...');gobackto_cart('<?php echo ($protectedUrl)? base64_encode($ecom_hostname):$ecom_hostname?>')" value="<?php echo stripslash_normal($Captions_arr['CART']['CART_BACK_CARTPAGE'])?>" /></div>
		...
		$row_checkprods = $db->fetch_array($ret_checkprods);
					...
					if($row_checkprods['product_det_qty_type']=='DROP')
					{
						...
						echo $row_orddet["order_qty"]?></td>
						<td class="price" valign="top" align="right"><?php echo print_price($row_orddet['order_rowtotal'],true)?></td>
						</tr>                       
						<tr><td colspan="4">
					<?php
						// If variables exists for current product, show it in the following section
...
if ($db->num_rows($ret_vars))
		...
		{
								print "<span class='cartvariable'>".$row_msgs['message_title'].": ". $row_msgs['message_value']."</span><br />"; 
							}	
							echo "</div>";
						}
				}
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
		<tr>
		<td align="l" valign="middle" class="cart_total" width="85%"><?php echo stripslash_normal($Captions_arr['CART']['CART_TOTPRICE'])?>&nbsp;</td>
		...
		?>
			...
			<div class="panel panel-default">
					<a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion" href="#collapse1"><div class="panel-heading">
					<h4 class="panel-title">

					<?php echo " Details of Tenant # 1"?>
					</h4>
					</div></a>
		...
		//Extra billing Details Start Here
		$sql_ord_qty = "SELECT order_orgqty FROM order_details WHERE orders_order_id=".$return_order_arr['order_id']." LIMIT 1";
		$ret_ord_qty = $db->query($sql_ord_qty);
		if($db->num_rows($ret_ord_qty)>0)
		{
		  $row_ord_qty = $db->fetch_array($ret_ord_qty);
		  $ord_qty     = $row_ord_qty['order_orgqty'];
		  $cnt = 1;
		  ?>
		  ...
		  {
			  $cnt = $cnt+1;
			  $tenant_name = "tenant".$cnt;
			  $tenantname = "Tenant ".$cnt;
			  $sql_extra  = "SELECT * FROM order_extra_billing_details WHERE order_id=".$return_order_arr['order_id']." AND order_tenant_name='".$tenant_name."' AND sites_site_id=$ecom_siteid LIMIT 1";
			  $ret_extra  = $db->query($sql_extra);
			  if($db->num_rows($ret_extra)>0)
			  {
				 $row_extra = $db->fetch_array($ret_extra);
				  // Get the list of billing address static fields to be shown in the checkout out page in required order
				$sql_checkout = "SELECT field_name,field_orgname 
				FROM 
				general_settings_site_checkoutfields 
				...
				{	
					...
					<table width="100%" cellpadding="1" cellspacing="1" border="0">
						...
						<tr>
							...
							<td align="left" width="50%" class="regi_txtfeild">
							<?php
								echo stripslash_normal($row_extra[$row_checkout['field_orgname']]);		?>
							</td>
						</tr>
						<?php
						}
		...
		function Show_CheckoutSuccess($order_id=0)
		{
			...
			*/
			...
			$HTML_treemenu11 = '<div class="tree_menu_conB">
						...
						$HTML_treemenu11 = '<div class="treemenu"><a href="'.url_link('',1).'" title="'.stripslash_normal($Captions_arr['COMMON']['TREE_MENU_HOME_LINK']).'">'.stripslash_normal($Captions_arr['COMMON']['TREE_MENU_HOME_LINK']).'</a><span>&raquo;</span>'.stripslash_normal($Captions_arr['CART']['CART_CHECKOUT_SUCCESS_TITLE']).'</div>';
		...
		function Show_CheckoutFailed()
		{
			...
			$HTML_treemenu1 = '<div class="tree_menu_conB">
							  <div class="tree_menu_topB"></div>
							  <div class="tree_menu_midB">
								<div class="tree_menu_content">
								  <ul class="tree_menu">
