...
/* Function to validate the fields in contest*/

function contest_validation(frm)
{
	var atleastone		= false;	
	fieldRequired		= new Array();
	fieldDescription	= new Array();
	var i=0;
	if(document.getElementById('contest_title'))
	{
		fieldRequired[i] 	= 'contest_title';
		fieldDescription[i] = 'Your Title';
		i++;
	}
	if(document.getElementById('contest_name'))
	{
		fieldRequired[i] 	= 'contest_name';
		fieldDescription[i] = 'Your Name';
		i++;
	}
	if(document.getElementById('contest_sir_name'))
	{
		fieldRequired[i] 	= 'contest_sir_name';
		fieldDescription[i] = 'Your Sur Name';
		i++;
	}
	if(document.getElementById('contest_email'))
	{
		fieldRequired[i] 	= 'contest_email';
		fieldDescription[i] = 'Your Email Id';
		i++;
	}
	
	fieldEmail 			= Array('contest_email');
	fieldConfirm 		= Array();
	fieldConfirmDesc  	= Array();
	fieldNumeric 		= Array();
	if(Validate_Form_Objects(frm,fieldRequired,fieldDescription,fieldEmail,fieldConfirm,fieldConfirmDesc,fieldNumeric))
