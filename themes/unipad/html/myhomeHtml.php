	...
	class myhome_Html
	{
		function Display_WelcomeMessage($mesgHeader,$Message)
		{
			...
			if($db->num_rows($ret_user))
			{
				...
				$username = stripslashes($row_user['customer_fname']).' '.stripslashes($row_user['customer_surname']);
			...
			$HTML_treemenu=$HTML_headermsg=$HTML_lgnhmrmsg=$HTML_Dischead=$HTML_tophomemsg=$HTML_topinnerdiv=$HTML_tophomeinnermsg='';
				...
				'<div class="treemenu">
					...
					<span>»</span>'.stripslash_normal($Captions_arr['LOGIN_HOME']['LOGIN_HOME_TREE_MENU']).'
					</div>';
			echo $HTML_treemenu ;	
			...
			?>
			<?php
				$sql_stat = "SELECT * from static_pages WHERE sites_site_id = $ecom_siteid AND pname='loginhome-content' LIMIT 1";
