<?php
	...
	class category_Html
	{
		...
		function Show_CategoryDetails($row_cat)
		{
			...
			{
				$HTML_treemenu = '<div class="treemenu">'.generate_tree_menu($_REQUEST['category_id'],-1,'<span>&raquo;</span>','','').'</div>';
	  		}
			if($alert)
			{
			...
			if ($cat_desc!='')
			{
				...
				$HTML_catdesc = '<div class="cat_main_desc"><p>'.$cat_desc.'</p></div>';
			}
			...
			//echo "cat prdt display setting categoryHtml.php - ".$row_cat['product_displaywhere'];
	  ...
	  function download_pdf() 
	  {
        ...
        <div class="listwrap">
        	<div class="property_shelf">
                <h3><?php echo stripslashes($row_cat['category_name'])?></h3>
                <?php /*?><div class="cat_main_data"><?=$HTML_image.$HTML_catdesc;?></div><?php */?>
                <div class="cat_main_data"><?=$HTML_catdesc?></div>
                <?php /*?><div class="sub_cat_hdr_icon"><?=$HTML_icons?></div><?php */?>
        
	  	<?php
			//echo "subcat show in categoryHtml.php - ".$subcatshow;
			if($subcatshow)
			{
				...
				//echo "cache exists in categoryHtml.php - ".$cache_exists;
				if ($cache_exists==false)
				{
					...
					$sql_subcat	=	"SELECT
													category_id,category_name,category_showimageofproduct,
													default_catgroup_id,subcategory_showimagetype,category_shortdescription
											FROM	product_categories 
											WHERE	parent_id = ".$_REQUEST['category_id']." 
											AND		sites_site_id = $ecom_siteid 
											AND		category_hide=0
											ORDER BY category_order";
					//echo "sql_subcat categoryHtml.php - ".$sql_subcat;
			...
			<?php
			//echo "prodshow in categoryHtml.php - ".$prodshow;
				...
				switch ($row_cat['category_subcatlistmethod'])
				{
					case 'normal': // normal 5 in a row
								...
								while ($row_subcat = $db->fetch_array($ret_subcat))
								{
									if($cnt==3)
									{
										$cls_wrap = "shelfwrapright";
										}
										else
										{
									$cls_wrap = "shelfwrap";
								}
									echo '<div class="'.$cls_wrap.'">';
									$HTML_subcatname = $HTML_image = $HTML_short_desc = '';
									
									if($row_cat['category_showimage']==1)
									{
										$HTML_image = '<div class="shelfimgwrap"><a href="'.url_category($row_subcat['category_id'],$row_subcat['category_name'],1).'">';
									...
									if($row_cat['category_showname']==1)
									{
										...
										$HTML_subcatname= stripslash_normal($row_subcat['category_name']);
								?>
                                	<div class="titlewrap">
                                    	<span class="title">
                                        	<a href="<?php echo url_category($row_subcat['category_id'],$row_subcat['category_name'],1)?>" title="<?php stripslash_normal($row_subcat['category_name'])?>" class="<?php echo $class?>"><?=$HTML_subcatname?></a>
										</span>
									</div>
								<?php
									}
		...
		function Show_Products($ret_prod,$tot_cnt,$start_var,$cat_det,$def_orderfield,$def_orderby)
		{
			...
			$HTML_bottomcontent	.=	'<table class="productpagenavtable" border="0" cellpadding="0" cellspacing="0" width="100%">
								<tbody>
									<tr>
										<td class="productpagenavtext" align="left"  valign="middle" width="63%">
										'.stripslash_normal($Captions_arr['CAT_DETAILS']['CATDET_SORTBY']).'
										'.generateselectbox('catdet_sortbybottom',$selval_arr,$prodsort_by,'','',0,'',false,'catdet_sortbybottom').'
										'.generateselectbox('catdet_sortorderbottom',$selord_arr,$prodsort_order,'','',0,'',false,'catdet_sortorderbottom').'
										</td>
										<td class="productpagenavtext" align="right" valign="middle" width="37%">
										'.stripslash_normal($Captions_arr['CAT_DETAILS']['CATDET_ITEMSPERPAGE']).'
										'.generateselectbox('catdet_prodperpagebottom',$perpage_arr,$catdet_prodperpage,'','',0,'',false,'catdet_prodperpagebottom').'
										<input type="button" name="submit_Page" value="'.stripslash_normal($Captions_arr['CAT_DETAILS']['CATDET_GO']).'" class="buttonred" onclick="handle_categorydetailsdropdownval_sel(\''.url_category($_REQUEST['category_id'],stripslash_javascript($cat_det['category_name']),4,$_REQUEST['catgroup_id'],1).'\',\'catdet_sortbybottom\',\'catdet_sortorderbottom\',\'catdet_prodperpagebottom\')" />
										</td>
									</tr>
								</tbody>
								</table>';
			...
			//{
				...
				while($row_prod = $db->fetch_array($ret_prod))
				{
					...
					//echo "<pre>";print_r($tabimg_arr);
					//echo "<br>".$tabimg_arr[0]['image_extralargepath'];
