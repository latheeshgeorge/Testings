...
function Show_ProductDetails($row_prod)
{
		...
		?>
		<table border="0" cellpadding="0" cellspacing="0" class="prod_details_table">
		<?php
		if($alert)
		{
			?>
		...
		<div class="prod_det_name_link"><h1><?php	echo stripslashes($row_prod['product_name']);?></h1>
		...
		?>			</div>
			...
			<img src="<?php url_site_image('pro_link_det_left.png')?>" title="left" alt="left" />
			</td>
			...
			if( $compare_show==1)
			{
				...
				</script>--> 	<?php /*<a href="<?php url_productcompare($_REQUEST['product_id'],$row_prod['product_name'])?>" class="productdetailslink"><? echo $Captions_arr['PROD_DETAILS']['PRODDET_COMPARE_LINK'] ?></a> */?>
				<?php
			}
			...
			<img src="<?php url_site_image('pro_link_det_right.png')?>" title="right" alt="right" />
			</td>
			</tr>
			</table>
			</td>
		...
		else
		{
			...
			if($row_prod['product_flv_filename']!='')
			{
				...
				<a href="javascript:show_normal_video()"><img src="'.url_site_image('video-but.png',1).'" border="0" /></a>
				</div>
		...
		*/ 
		...
		{
			echo '<div class="detweight_cls"><span class="detweight_caption">'.$Captions_arr['PROD_DETAILS']['PRODDET_COMPWEIGHT'].': </span>'.$row_prod['product_weight'].' '.$Settings_arr['unit_of_weight'].'</div>';
		}
		echo '</div>';
		echo $HTML_bonus;
		show_excluding_vat_msg($row_prod,'vat_div');// show excluding VAT msg
		...
		* */
		...
		if (count($tabs_arr))
		{
				?>	
				<script type="text/javascript">
				function handle_newtabs(passtabid)
				{
					document.getElementById('main_tab_content_td').innerHTML = 'loading...';
					objs_head = eval('document.getElementById("tab_head_'+passtabid+'")');
					objs_cont = eval('document.getElementById("tab_content_'+passtabid+'")');
					document.getElementById('main_tab_content_td').innerHTML = objs_cont.innerHTML;
					
					var divsa = document.getElementsByTagName("div");
					for(var i = 0; i < divsa.length; i++)
					{
						if(divsa[i].id.substr(0,9)=='tab_head_')
						{
							document.getElementById(divsa[i].id).className='selectedtab_unsel';
						}	
					}
					objs_head.className = 'selectedtab_sel';
				}
				</script>
				
				<table class="overview_table" border="0" cellpadding="0" cellspacing="0">
				<tr>
				<td  class="overview_table_hdr_tab">
				<?php		$curtab 		= ($_REQUEST['prod_curtab'])?$_REQUEST['prod_curtab']:0;
				$prodimgdetid 	= (!$_REQUEST['prodimgdet'])?0:$_REQUEST['prodimgdet'];
				...
				{
					?>
					<div style="display:none;" id="tab_content_<?php echo $k_tabid?>"><?php echo $tabs_content_arr[$k_tabid]?></div>
					<?php
						$cnt++;
						...
						?>
						<div class="<?php echo $class?>" id="tab_head_<?php echo $k_tabid?>" onclick="handle_newtabs(<?php echo $k_tabid?>)">
							<?php
							echo $v_tabtitle;
							?>
							</div>
				...
				<tr>
				<td  class="overview_table_cont" id="main_tab_content_td"><?php echo $tabs_content_arr[$curtab];?></td>
				</tr>
				</table>
				<?php	
		}	 
...
function show_BulkDiscounts($row_prod,$var_arr=array())
{
 ...
 $cust_id 	= get_session_var("ecom_login_customer");
					...
					{
							?>	
						<div class="bulkdiscount_wrap">
						<div class="bulk_top"></div>
						<div class="bulk_bg">	
					...
					{ 
						...
						{ 
							...
							{ 
								...
								{
									$ret_val = '<table width="100%" border="0" cellpadding="0" cellspacing="0" class="variabletableB"';
									//if ($pos=='row') 
...
function show_offer_buttons($row_prod)
{
					...
					{
						?>
						<div class="det_btn_con">	
						<?php
						if($row_prod['product_show_pricepromise']==1)
						{
							?>	
							<div class="prod_price_promice"><a href="javascript:handle_price_promise()" title="Price Promise"><img src="<?php url_site_image('promise.png')?>" border="0"/></a></div>
							<?php
						}	
						if($in_combo==1)
						{
							?>
							<div class="prod_combo"><a href="<?php url_link('showallbundle'.$row_prod['product_id'].'.html')?>" title=""><img src="<?php url_site_image('combo-offer.png')?>" border="0"/></a></div>
							<?php
						}
						if($row_prod['product_freedelivery']==1)
						{
							?>	
							<div class="prod_free"><a href="<?php url_link('freedelivery'.$row_prod['product_id'].'.html')?>" title="Free Delivery"><img src="<?php url_site_image('free-del.png')?>" border="0" /></a></div>
							<?php
						}
						?>
						</div>
...
global $Captions_arr,$showqty,$Settings_arr,$ecom_hostname;
					...
					if ($caption_key!='') // Check whether the buynow or preorder button is to be displayed
					{
							...
							{
								?>
								<td align="left" valign="middle" class="details_buy_c"><input name="Submit_buy" type="button" class="" id="Submit_buy" value="<?php echo $Captions_arr['PROD_DETAILS'][$caption_key];?>"	onclick="ajax_addto_cart_fromlist('add_prod_tocart_ajax','Prod_Addcart','frm_proddetails','<?php echo SITE_URL?>')" /></td>
							...
							{
								?>
								<td align="left" valign="middle" class="details_buy_c"><input name="Submit_buy" type="submit" class="" id="Submit_buy" value="<?php echo $Captions_arr['PROD_DETAILS'][$caption_key];?>"	onclick="show_wait_button(this,'Please Wait...');document.frm_proddetails.fpurpose.value='Prod_Addcart'" /></td>
								<?php
							}
							?>
							</tr>
					...
					if ($row_prod['product_show_enquirelink']==1 || $Settings_arr['proddet_showwishlist'])
					{
								...
								if ($Settings_arr['proddet_showwishlist'])
								{
									...
									{
										?>
										<input name="submit_wishlist" type="submit" class="buttonwishlists" id="submit_wishlist" value="<?php echo $Captions_arr['PROD_DETAILS']['PRODDET_ADDWISHLIST'];?>" onclick="show_wait_button(this,'Please Wait...');document.frm_proddetails.fpurpose.value='Prod_Addwishlist'" />
										<?php
									}
									else
									{
									...
									?>
										<input name="submit_wishlist" type="button" class="buttonwishlists" id="submit_wishlist" value="<?php echo $Captions_arr['PROD_DETAILS']['PRODDET_ADDWISHLIST'];?>" onclick="<?php echo $wishlist_onclick?>" />
...
global $ecom_hostname,$ecom_themename,$db;
					...
					<table class="leftsideTableA" border="0" cellpadding="0" cellspacing="0">
					<tr>
					...
					src="<?php url_site_image('product_ZoomPlus.png')?>" alt="Zoom In"
