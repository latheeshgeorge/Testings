<?php
	...
	class cart_Html
	{
				...
				if(!$cust_id)
				{
					echo "
							<script type='text/javascript'>
								...
								window.location = 'http://".$ecom_hostname."/custlogin.html';
							</script>
		...
		?>	
			  ...
			  <tr>
			    ...
			    <?php
				 ...
				 ?>	</td>
			     	...
			     	<tr>
						<td colspan="5" align="center" valign="middle" class="red_msg">
						KQF Foods are no longer selling online. <br/> 
						Please Telephone KQF Foods on 01254 56699 with your Enquiry.
						</td>
					</tr>	
					<?php
					exit;
		  ...
		  ?>
			 	<tr>
				<td align="right" valign="middle" class="shoppingcartcontent" colspan="4"><?php echo $Captions_arr['CART']['CART_TOTPRICE']?></td>
		  ...
		  ?>
				...
				?>	
				   ...
				   <?php
				           ...
				           //datetime script start here

				  if($row_cartdet['location_id']>0)
				  {
					    $location_id  = $row_cartdet['location_id'];
...
?>
 <tr>
						 <td align="right" class="shoppingcartcontent_date" colspan="2">
<?php
							$this->show_location_datetime('date',$location_id,$session_id);
							?>
							</td>
							</tr>
							<tr>
						 <td align="right" class="shoppingcartcontent_time" colspan="2">
							<?php
							$this->show_location_datetime('time',$location_id,$session_id);
							?>
							</td>
							</tr>
							<?php
		  ...
		  <?php
			...
			{	
				...
				?>
					 <tr>
						<td colspan="1" align="left" valign="middle" class="shoppingcartcontent"><?php echo $Captions_arr['CART']['CART_TOTAL_POINTS_EARN']?>&nbsp;</td>
						<td colspan="4" align="left" valign="middle" class="shoppingcartpriceC"><?php echo $cartData["totals"]["bonus"]?></td>
					  </tr>
				<?php	
				}
				if($cust_id) // case if logged in
				{
					if ($cartData["customer"]["customer_bonus"] and $Settings_arr['cust_allowspendbonuspoints']==1) // does current customer have bonus points
					{
						...
						?>
						 ...
						 <tr>
							<td colspan="1" align="left" valign="middle" class="shoppingcartcontent"><a name="bonus">&nbsp;</a><?php echo $Captions_arr['CART']['CART_SPENT_BONUS_POINTS']?></td>
						  ...
						  <!--
						  <tr>
							<td colspan="1" align="left" valign="middle" class="shoppingcartcontent"><a name="bonus">&nbsp;</a><a href='http://www.garraways.co.uk/pg49716/amazing-children-uganda.html' target='_blank' style='color:#000000'><?php echo $Captions_arr['CART']['CART_DONATE_BONUS_POINTS']?></a></td>
							<td colspan="4" align="left" valign="middle" class="shoppingcartcontent">
							<input type="hidden" name="maxBonusPoints"  class="input" value="<? print $cartData["customer"]["customer_bonus"]; ?>">
...
// Show the payment type and payment method sections only if total amount > 0
          ...
          <tr>
			...
			<?php
				...
				$paytype_add_condition = $paytype_add_condition_new = '';
				// Running the qry to pick the payment types to be displayed
				if($cust_id) // case if customer is logged in
				{
					global $discthm_group_custgroup_val;
					$trade_group_arr = array(71,74,75,70,72,76);
					//echo "i am here".$discthm_group_custgroup_val;
					if($discthm_group_custgroup_val!='')
					{
						if (in_array($discthm_group_custgroup_val,$trade_group_arr))
						{
							$paytype_add_condition_new = " AND a.paytype_code <> 'invoice' ";
							//echo "i am here".$discthm_group_custgroup_val;
						}
					}					
          ...
          <tr>
            <td colspan="5" align="right" valign="middle" class="shoppingcartcontent">
			  ...
			  <input type="hidden" name="del_msg_disp" id="del_msg_disp" value="<?php echo $Captions_arr['CART']['CART_DELIVERY_LOC_MSG']?>" />
	       	 <input type="hidden" name="date_msg_disp" id="date_msg_disp" value="<?php echo $Captions_arr['CART']['CART_SEL_DATE_MSG']?>" />
			  <input type="hidden" name="time_msg_disp" id="time_msg_disp" value="<?php echo $Captions_arr['CART']['CART_SEL_TIME_MSG']?>" />
		...
		<?php
			...
			?>
				...
				?>
					<tr>
						<td align="right" valign="top" class="google_tdA" width="60%">
							<?php
							/*if($ecom_siteid == 61) // garraways 61 live id 
							{
								if(($totpaycnt>0) or ($google_exists and $google_recommended==0))
								{
									?>
						...
						<td align="left" valign="middle" class="google_tdA">
		 ...
		 <table width="100%" border="0" cellspacing="4" cellpadding="0" class="regifontnormal">
 		  <tr>
		       <td valign="top" style="text-align:center;padding: 10px 5px;"><a href="http://www.blog.sagepay.com/an-update-on-poodle-from-sage-pays-cto" target="_blank"><img src="<?php url_site_image('browser_support.gif')?>" alt="browser support warning" width="525px" height="87px" border="0"></a></td>
		   </tr>
		  </table>     
		 <?php	
		  ...
		  <input type="hidden" name="pass_url" id="pass_url" value="<?php echo $ps_url?>" />
		  	<input type="hidden" name="cart_mod" value="show_cart" /> 
			<input name="custcartlogin_Submit" type="submit" class="buttonred_cartA" value="Login" onclick="document.frm_custlogin_cart.submit();" />
			</td>
	      </tr>
		  <?php
		...
		function show_location_datetime($mod,$loc_id,$session_id)
		{ 
		global $ecom_siteid,$db,$ecom_hostname,$Settings_arr,$ecom_themeid,$default_layout,$Captions_arr,$inlineSiteComponents,$Settings_arr,$protectedUrl,$ecom_common_settings;
		 ...
		 $sql_cartdt = "SELECT date,time_id  
		FROM 
		cart_supportdetails 
		WHERE 
		location_id = ".$loc_id."   
		...
		LIMIT 
		1";
		$ret_cartdt = $db->query($sql_cartdt);
		$row_cartdt = $db->fetch_array($ret_cartdt);

		/*
		$sql_location = "SELECT location_datetime_applicable FROM delivery_site_location WHERE location_id = $loc_id AND sites_site_id = $ecom_siteid LIMIT 1";
		$ret_location 	  = $db->query($sql_location); 
		$row_location     = $db->fetch_array($ret_location);
		*/
		if($Settings_arr['date_compulsary']==1)
		{
		?>
		<input type="hidden" name="date_compulsary" id="date_compulsary" value="1" />
		...
		{
			?>
		<input type="hidden" name="date_compulsary" id="date_compulsary" value="0" />

		<?php
		}
		if($Settings_arr['time_compulsary']==1)
		{
		?>
		<input type="hidden" name="time_compulsary" id="time_compulsary" value="1" />
		...
		{
		?>
		<input type="hidden" name="time_compulsary" id="time_compulsary" value="0" />

		<?php
		}

		//if($row_location['location_datetime_applicable']==1)
		{   
			if($mod=='date')
			{
				?>
				<script type="text/javascript" src="http://<?php echo $ecom_hostname?>/images/<?php echo $ecom_hostname?>/scripts/datepicker/jquery-1.7.2.js"></script>
				<script type="text/javascript" src="http://<?php echo $ecom_hostname?>/images/<?php echo $ecom_hostname?>/scripts/datepicker/jquery.ui.core.js"></script>
				<script type="text/javascript" src="http://<?php echo $ecom_hostname?>/images/<?php echo $ecom_hostname?>/scripts/datepicker/jquery.ui.datepicker.js"></script>
				<!-- script type="text/javascript" src="js/jquery.ui.datepicker-es.js"></script -->

				<!-- loads mdp -->
				<script type="text/javascript" src="http://<?php echo $ecom_hostname?>/images/<?php echo $ecom_hostname?>/scripts/datepicker/jquery-ui.multidatespicker.js"></script>
		...
		*/
		...
		<link rel="stylesheet" type="text/css" href="http://<?php echo $ecom_hostname?>/images/<?php echo $ecom_hostname?>/css/datepicker/mdp.css">
		<link rel="stylesheet" type="text/css" href="http://<?php echo $ecom_hostname?>/images/<?php echo $ecom_hostname?>/css/datepicker/prettify.css">
		<script type="text/javascript" src="http://<?php echo $ecom_hostname?>/images/<?php echo $ecom_hostname?>/scripts/datepicker/prettify.js"></script>
		<script type="text/javascript" src="http://<?php echo $ecom_hostname?>/images/<?php echo $ecom_hostname?>/scripts/datepicker/lang-css.js"></script>
		<script type="text/javascript">
		$(function() {
		prettyPrint();
		});
		...
		if($db->num_rows($ret_cartdt)>0)
		{
				if($row_cartdt['date']!='0000-00-00')
				{ 
					$dateArrs	=	explode("-",$row_cartdt['date']);
					$dateJAS		.=	"addDates: [";
					//echo "<pre>";print_r($dateArr);
					$dateJAS		.=	"'".$dateArrs[1]."/".$dateArrs[2]."/'+y";
					$dateJAS		.=	"],";
				}
		}			
				//echo $dateJAS;
				$date_array = array();
				$sql_date_map     = "SELECT * FROM delivery_location_date_map WHERE location_location_id = $loc_id AND sites_site_id = $ecom_siteid ORDER BY  date ASC";
				$ret_date_map 	  = $db->query($sql_date_map); 
				$dateJS		=	"";
				if($db->num_rows($ret_date_map) > 0)
				{																
					
					
					$dateJSs    = '';
					while($row_date = $db->fetch_array($ret_date_map))
					{
						if($row_date['date']!='0000-00-00')
						{
					$dateArr	=	array();
					$dateArr	=	explode("-",$row_date['date']);
					//echo "<pre>";print_r($dateArr);
					$dateJSs		.=	"'".$dateArr[1]."/".$dateArr[2]."/'+y, ";
						}
					
					}
					if($dateJSs!='')
					{
					$dateJS		.=	"addDisabledDates: [";
					$dateJSs		=	substr($dateJSs, 0, -2);
					$dateJS		.= $dateJSs;
					$dateJS		.=	"],";
					}
				}
		...
		<tr>
		<td align="left" valign="top" width="">Select Delivery Date </td><td align="left" class="date_td"><div id="multipleDate"></div><br><input type="hidden" id="datesField" name="datesField" readonly="readonly"></td>
		</tr>
		</table>
				<?php
				//if($datetimeapp == 1)
		...
		<?php	}
			...
			{	//print_r($row_cartdet);
				//echo "trest".$row_cartdet['time_id'];
				$sql_time_map     = "SELECT * FROM  delivery_location_time_map	 WHERE location_location_id = $loc_id AND sites_site_id = $ecom_siteid ORDER BY  sort_order ASC";
				$ret_time_map 	  = $db->query($sql_time_map); 
				if($db->num_rows($ret_time_map)>0)
				{
					$time_array[0] = "--Select--";
				while($row_time_map     = $db->fetch_array($ret_time_map))
				{
				   $time_array[$row_time_map['id']] = $row_time_map['time'];
				}
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr>
					<td align="left" valign="top" width="33%">Select Delivery Time </td>
					<td align="left" class="time_td">
					<?php
					echo generateselectbox('cart_deliverytime',$time_array,$row_cartdt['time_id'],'','');
					?>
