				        ...
				        if ($_REQUEST['req']!='')// LIMIT for products is applied only if not displayed in home page
						{
							...
							if(trim($search_desc)) {
									$top_content = $search_desc;
											...
											<h1 class="search_res_head_h1"><?=$Captions_arr['COMMON']['SEARCHEDFOR'];?> '<? echo strtoupper($_REQUEST['quick_search']); ?>'</h1>
											</td>
											</tr>
											<?php
											}
										...
										<tr>
				<td colspan="3" class="shelfAheader_A" ><div id="loader_img" class="loader_img" style="display:none;" alt="Loading, please wait.." ><img src="<?php url_site_image('loading.gif')?>"/></div><br /><?php echo paging_show_totalcount($tot_cnt,'Product(s)',$start_var['pg'],$start_var['pages'])?></td></tr>
			<tr>
										<tr>
                                ...
                                <?php
											...
											//{
								?>		<div class="product_name"><a href="<?php url_product($row_search['product_id'],$row_search['product_name'],-1)?>" title="<?php echo stripslashes($row_search['product_name'])?>"><?php echo stripslashes($row_search['product_name'])?></a></div>
								<?php
											//}
								...
								<?php 
											...
											{
												if($row_search['product_bonuspoints'] > 0)
												{
													echo '<div class="prod_list_bonusB">
														...
														<span class="bonus_point_caption_b">'.$row_search['product_bonuspoints'].'</span>
                                ...
                                ?>	<!--<div class="prod_list_des">
                                        <?php echo stripslashes($row_search['product_shortdesc'])?><?php show_moreinfo($row_search,'list_more')?>
                                    </div>-->
                                <?php
											}
											if($row_search['product_saleicon_show']==1)
                                ...
                                ?>
                                    <div class="moreinfo">
                                    <a href="<?php url_product($row_search['product_id'],$row_search['product_name'],-1)?>" title="<?php echo stripslashes($row_search['product_name'])?>">More Info</a>
                                    </div>
					...
					if(count($search_rel))
						{
							 ...
							 $max_cn =2;
							?>
							<div class="saved_searchA">
								<div class="pro_de_shelfBheader">Related searches</div>
							<table width="100%" border="0" cellspacing="0" cellpadding="0" class="saved_searchA">
									...
									<tr>
										...
										foreach ($search_rel as $k=>$search_values)
										{
											 ...
											 /*
											 ?>
												<td align="center" class="saved_searchB"  ><a href="<? url_link('s'.$search_values['search_id'].'/'.strip_url($search_values['search_keyword']).'.html')?>" class="link">
											 <?
											 */
											 ?>
											 <td align="center" class="saved_searchB"  ><a href="<?php url_savedsearch($search_values['search_id'],$search_values['search_keyword'])?>" class="link">
		...
		{ 
			...
			$sql_cat = "SELECT category_id,category_name 
								...
								WHERE 
									...
									AND category_hide =0".$more_conditions." 
								ORDER BY 
									category_name ASC";
