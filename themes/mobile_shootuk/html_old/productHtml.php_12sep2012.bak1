<?php
	...
	class product_Html
	{
		...
		function Show_ProductDetails($row_prod)
		{
			...
			<?=$HTML_treemenu?>
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="ctable_a">
              <tr>
            <td class="curvea_top" colspan="2"><div class="white"><?php echo stripslash_normal($row_prod['product_name']); 
              ...
              <tr>
                <td class="td">
					<div id="mainimage_holder"><?php $this->Show_Image_Normal($row_prod,'main');?></div></td>
                <td class="td"><div id="price_holder"><?=$HTML_price?></div>
                </td>
              </tr>
		...
		$selC_a = ($curtab == 103)?' class="linkbold_a"':'class="linkbold_click"';
           ...
           $return_arr_more = $this->Show_Image_Normal($row_prod,'main_tab',true); 
		   $show_pic_tab = $this->show_more_images($row_prod,$return_arr_more['exclude_tabid'],$return_arr_more['exclude_prodid'],true);
       ?>
  ...
  <tr>
    <td <?php echo $selA;?> id="show_sum"><a href="#protabs" id="show_sum_a"  <?php echo $selA_a?> onclick="show_summary_ajax('<?php echo $row_prod['product_id']?>','summary')">Summary</a></td>
    <?php
    if($show_pic_tab==true)
    {
    ?>
    <td <?php echo $selB;?> id="show_im"><a href="#protabs"  id="show_im_a"<?php echo $selB_a?> onclick="show_summary_ajax('<?php echo $row_prod['product_id']?>','show_img')">Picture</a></td>
    <?php
    }
    ?>
    <td <?php echo $selC;?> id="show_desc"><a href="#protabs" id="show_desc_a"<?php echo $selC_a?> onclick="show_summary_ajax('<?php echo $row_prod['product_id']?>','show_desc')">Description</a></td>
  </tr>
</table>
    </td>
  </tr> 
  <tr id="show_img_tr" style="display:none"><td>
	   <?php
   $this->show_summary("show_img",$_REQUEST['product_id'],$row_prod);
  ?>
	  </td>
	  </tr>
  
  
  <tr id="show_ajax_tr"><td>
  <div id="summary_ajax_container">
   <?php
   $this->show_summary("summary",$_REQUEST['product_id'],$row_prod);
  ?></div> 
    </td></tr>
	  ...
	  if ($_REQUEST['prod_curtab'] and $row_prod['product_variablecombocommon_image_allowed']=='N')
	  {
		...
		*/
		...
		if(count($tabimg_arr))
		{
			...
			?><ul id="Gallery">
				<li>
			<a href="<?php url_root_image($tabimg_arr[0]['image_thumbpath'])?>" title="<?php echo $row_prod['product_name']?>"  >
			<?php
		...
		if ($show_normalimage)
		{
			...
			*/
			...
			if(count($prodimg_arr))
			{
				...
				<ul id="Gallery">
				<li>
				<a href="<?php url_root_image($prodimg_arr[0]['image_thumbpath'])?>"  title="<?=$row_prod['product_name']?>">
				<?php
		...
		else
		{
			...
			?>
	          ...
	          <li><div class="det_thumbimg_pdt">
					...
					<a href="<?php url_root_image($v['image_thumbpath'])?>"  title="<?=$title?>">
					<?php
