	...
	class product_Html
	{
		...
		function Show_ProductDetails($row_prod)
		{
			...
			if(in_array('mod_product_reviews',$inlineSiteComponents)) // Check whether the product review module is there for current site
			{
				...
				{
					$HTML_readrev = '<div><a href="'.url_link('readproductreview'.$_REQUEST['product_id'].'.html',1).'" title="'.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_READREVIEW']).'"><img src="'.url_site_image('det-icon_05.gif',1).'" border="0" /> '.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_READREVIEW']).'</a></div>';
				}
			}
			if($email_show==1)
			{
				$HTML_email = '<div><a href="'.url_link('emailafriend'.$_REQUEST['product_id'].'.html',1).'" title="'.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_EMAILFRIEND']).'"><img src="'.url_site_image('det-icon_07.gif',1).'" border="0" /> '.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_EMAILFRIEND']).'</a></div>';
			}
			if($pdf_show==1) // Check whether the download pdf module is there for current site
			{ 
				//$HTML_pdf = '<div class="down_pdf"><a href="javascript:download_pdf_stream(\''.$_SERVER['HTTP_HOST'].'\',\''.$_SERVER['REQUEST_URI'].'\')" title="'.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_DOWNLOADPDF']).'"><img src="'.url_site_image('det-icon_09.gif',1).'" border="0" /> '.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_DOWNLOADPDF']).'</a></div>';
			}
			...
			{
			      $HTML_bonus = '<div class="deat_bonus_sp">
								 </div>';
			}
			...
			elseif ($row_prod['product_shortdesc'])
				$tabs_content_arr[0]	= stripslashes($row_prod['product_shortdesc']);
				       //$rp_cont = '<p xmlns="http://www.w3.org/1999/xhtml"><a href="http://cablestripping.co.uk">Ferrules</a> - <a href="http://cablestripping.co.uk/index.php?cPath=27">Cable Cutting</a> - <a href="http://cablestripping.co.uk/index.php?cPath=28">Wire Stripping</a> - <a href="http://cablestripping.co.uk/index.php?cPath=28">Cable Stripping</a> - <a href="http://cablestripping.co.uk/index.php?cPath=27">Wire Cutting</a> - <a href="http://cablestripping.co.uk/index.php?cPath=23">Cable Marking</a> - <a href="http://cablestripping.co.uk/index.php?cPath=26">Crimping Machine</a> - <a href="http://www.cstautomation.co.uk">Schleuniger</a> - <a href="http://www.loepfe-ag.ch/main.php?seite=5&amp;sprache=3 (http://www.loepfe-ag.ch/main.php?seite=5&sprache=3)">Loepfe-ag</a> - <a href="http://www.cablestripping.co.uk">CST Automation</a> - <a href="http://ulmer-gmbh.com/neu/index.php?lang=en">Ulmer Gmbh</a> - <a href="http://cablestripping.co.uk/prefeeding-dereeling-c-33.html">Prefeeding and Dereeling</a></p>';
           $rp_cont ="CST Automation";
            $tabs_content_arr[0] = str_replace($rp_cont,"",$tabs_content_arr[0]);
			...
			function ajax_return_productdetailscontents() 
			{
				...
				var cursiteid = '<?php echo $ecom_siteid?>';
				if(req.readyState==4)
				{
					if(req.status==200)
					{
			...
			if($row_prod['product_freedelivery']==1)
				{
					echo  '<div class="deat_pdt_free"><a href="'.url_link('freedelivery'.$row_prod['product_id'].'.html',1).'" title="Free Delivery"><img src="'. url_site_image('free-details.gif',1).'" border="0" /></a></div>';
				}
				*/
			?>	
				<div class="deat_img_top"></div>
				<div class="deat_img_cont">
				<div class="deat_pdt_name"><h1>
                 ...
                 <?php if($ecom_siteid==62)
				 {
					 /*
				 ?>
				<div id="bulkdisc_holder">
				<? 
				$this->show_BulkDiscounts($row_prod,array(),$class_array);?>
				</div>
                <?php 
                */
				 }
			   ...
			   <div class="deat_pdt_icons">
				 ...
				 <div class="deat_pdt_price">
				  <div class="deat_price"><img src="<?php url_site_image('call-us-img.gif') ?>" alt="Call us" />
						 </div>
				 <div id='price_holder'>
				 ...
				 <div class="deat_pdt_buy_outr">
					 ...
					 {
					 ?>
					 <div class="enquire_out" title="<?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_ENQUIRY'])?>" onclick="show_wait_button(this,'Please Wait...');document.frm_proddetails.fpurpose.value='Prod_Enquire';document.frm_proddetails.submit();"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_ENQUIRY']); ?></div>
					<?php
					}
					else
					{
				 ...
				 <div class="deat_offer_btn_outer">
				<?//=$HTML_bonus?>
				 <div class="deat_offer_btn_right"> 
			<?php
			   /*
		...
		if ($db->num_rows($ret_var) or $db->num_rows($ret_msg))
		{
			while ($row_var = $db->fetch_array($ret_var))
			{
				...
				if ($var_Proceed)// Show the variable if it is valid to show
				{
				  ...
				  ?>
				   <tr>
						<td align="left" valign="middle" class="varable_table_left" colspan="2"><?php echo stripslash_normal($row_var['var_name'])?> : </td>
						</tr>
						<tr>
...
<?php
	...
	?>
	<div class="buyBinner_link"><input name="Submit_buy" type="button" class="det_buy_link_ajax" id="Submit_buy" value="<?php echo $Captions_arr['PROD_DETAILS'][$caption_key];?>"	onclick="ajax_addto_cart_fromlist('add_prod_tocart_ajax','Prod_Addcart','frm_proddetails_ajax','<?php echo SITE_URL?>')" /></div>
	<?php
	}
...
{
	global $Captions_arr,$showqty,$Settings_arr,$ecom_siteid;
	$cust_id 	= get_session_var("ecom_login_customer");
	$showqty	= $Settings_arr['show_qty_box'];// show the qty box
	...
	if ($caption_key!='') // Check whether the buynow or preorder button is to be displayed
	{
			if($ecom_siteid==62)//'62')
			{
			?>	
			<div class="deat_pdt_buyAB">
			<div class="deat_pdt_buyAZ">
			<?php	
					...
					if($showqty==1)// this decision is made in the main shop settings
					{
						...
						{
							$onchange_function = "onchange ='handle_show_prod_det_bulk_disc(\"price_qty\",\"".SITE_URL."\",\"".$row_prod['product_id']."\",\"".url_site_image('loading.gif',1)."\")' ";
						}
						else
						{
							$onchange_function = '';
					...
					?>
						...
						<tr>
							<td class="buyBinner_qty"><?=$cur_qty_caption?></td>
							<td class="buyBinner_txt"><input type="text" class="det_qty_txt" name="qty"  value="<?php echo ($_REQUEST['qty'])?$_REQUEST['qty']:1?>" maxlength="4"  <?php echo $onchange_function?> /></td>
						</tr>
						</table>
					<?php
					 }
					...
					while ($row_grp = $db->fetch_array($ret_grp))
					{
						...
						$sql_labels = "SELECT a.label_id,a.label_name,a.in_search,a.is_textbox,c.product_site_labels_values_label_value_id,c.label_value 
											...
											WHERE 
												...
												AND a.label_name <> 'Specification' 
											ORDER BY 
												b.map_order";
						$ret_labels = $db->query($sql_labels);
...
function Show_Compare_Product($ret_prod)
{
	...
	<?=$HTML_treemenu?>
        ...
        while($row_prod = $db->fetch_array($ret_prod))
        {
            ...
            if($row_prod['product_freedelivery']==1)
            {	
                $HTML_free ='<div class="normal_shlfA_free"></div>';
            } 
            if($row_prod['product_saleicon_show']==1)
            {
                $desc = stripslash_normal(trim($row_prod['product_saleicon_text']));
        ...
        ?>
            ...
            <div class="normal_shlfB_pdt_bottom"></div>
            </div> 
        <?
            $cur_col++;
            if($cur_col>=$max_col)
...
{
global $db,$ecom_siteid,$Captions_arr;
	 				$class_array['outr'] 	= 'deat_bulk_outr';
					$class_array['top'] 	= 'deat_bulk_top';
					$class_array['bottom'] 	= 'deat_bulk_bottom';
					$class_array['conts'] 	= 'deat_bulk_conts';
	...
	{
	?>	
		<div class="<?php echo $class_array['outr']?>">
		<div class="<?php echo $class_array['top']?>"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['BULK_DISC_HEAD'])?></div>
		<div class="<?php echo $class_array['bottom']?>">
		<div class="<?php echo $class_array['conts']?>">
		<?php
...
?>
		...
		?>
			...
			<div class="donloadsleft"><span><a href="http://<?php echo $ecom_hostname?>/product_download.php?attach_id=<?php echo $row_attach['attachment_id']?>" title="<?php echo $Captions_arr['PROD_DETAILS']['PRODDET_CLICKDOWNLOAD'];?>"><?php echo stripslashes($row_attach['attachment_title'])?></a></span></div>
			</li>
		<?php
			$cnts++;
		}
