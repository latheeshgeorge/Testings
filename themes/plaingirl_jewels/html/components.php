		...
		function mod_staticgroup($grp_array,$title)
		{
			...
			switch($position)
			{
				...
				if ($cache_exists==false)
				{
					...
					// ############## Top ##############
					if ($position == 'topout') // Case if value of position is top;
					{
								...
								foreach ($grp_array as $k=>$grpData)
								{
									...
									if ($db->num_rows($ret_cat))
									{
										...
										if ($grpData['catgroup_show_subcat_indropdown']== 1 and $subcatdropdownsupport)// case if categories are to be shown in list menu
										{
											...
											?>
											<?php /*<li class="item1"><a href="#" class="category_menu" title="blog" target="_blank"><span>blog</span></a></a> */ ?>
...
<?php
		...
		function mod_quicksearch($title)
		{
            ...
            <input name="quick_search" type="text" value="SEARCH" onfocus="if(this.value=='SEARCH') this.value=''" onblur="if(this.value=='') this.value='SEARCH'" onload="if(this.value=='') this.value='SEARCH'"  class="searchinput" id="quick_search" />
            </div>
...
<?php
		...
		{
			global $Captions_arr,$ecom_hostname,$inlineSiteComponents,$Settings_arr,$db,$ecom_siteid,$position,$ecom_themename,$components;
            	...
            	<div class="top_news_con">
                    ...
                    if (!$cust_id) // case customer is not logged in
                    {
                        ...
                        ?>
                          <div class="top_news_con_left"><a href="<?php url_link('custlogin.html')?>" title="<?php echo stripslash_normal($Captions_arr['CUST_LOGIN']['LOGIN'])?>">
                                        <?=stripslash_normal($Captions_arr['CUST_LOGIN']['SIGNUP'])?>/<?=stripslash_normal($Captions_arr['CUST_LOGIN']['LOGIN'])?></a></div>
                        <?
                        }
                  }
                  else
                  {
                    if($position=='topband')
                    {
                       ...
                       $rp_arr = array('','','','','','','','');
                        ?>
                          <div class="top_news_con_leftC">
                                 <div class='welcome_classC'><?=stripslash_normal($Captions_arr['LOGIN_MENU']['WELCOME'])?> <span><?php echo str_replace($sr_arr,$rp_arr,get_session_var('ecom_login_customer_shortname'))?></span></div> 
                 ...
                 <div class="top_news_con_mid">
					<div class="top_phone"></div>	<div class="top_social"><a href="https://m.facebook.com/House-of-Jewels-UK-859846604112756/" title="Facebook" target="_blank"><img src="<?php url_site_image('soci-top_01.gif')?>"></a> <?php /*?><a href="#" title="Twitter" target="_blank"><img src="<?php url_site_image('soci-top_02.gif')?>"></a><a href="#"><img src="<?php url_site_image('soci-top_03.gif')?>"></a><img src="<?php url_site_image('soci-top_03.gif')?>"><img src="<?php url_site_image('soci-top_04.gif')?>"><?php */?>
					<a href="https://instagram.com/houseofjewelsuk/" target="_blank" title="Instagram"><img src="<?php url_site_image('insta.png')?>" alt="Instagram" /></a>
					</div>
					<?php
					    $position = 'topband'; 
          				include "themes/$ecom_themename/modules/mod_currencyselector.php"; 
					?> 
		...
		function mod_shoppingcart($title)
		{
			...
			?>
			 <div class="cart_btn">
					...
					<li style="background:none"> <span>&nbsp;</span></li>	
					 <li> <span><a href="<?php url_link('enquiry.html')?>" class="cart_view_linkA" title="<?php echo stripslash_normal($Captions_arr['COMMON']['VIEW_ENQUIRY'])?>"><?php echo stripslash_normal($Captions_arr['COMMON']['VIEW_ENQUIRY'])?></a></span> </li>
					 <?php /*<li> <span><a href="#" class="cart_view_linkB" onclick="handle_checkout_submit('<?php echo $ecom_hostname?>',0,'<?php echo $pass_tot?>')"  title="<?php echo stripslash_normal($Captions_arr['COMMON']['CHECK_OUT'])?>"><?php echo stripslash_normal($Captions_arr['COMMON']['CHECK_OUT'])?></a> </span></li> */ ?>
					 
					  <li> <span><a href="#" onclick="gobackto_cart('<?php echo ($protectedUrl)?base64_encode($ecom_hostname):$ecom_hostname?>')" class="cart_view_linkC" title="<?php echo stripslash_normal($Captions_arr['COMMON']['VIEW_CART'])?>"><?php echo stripslash_normal($Captions_arr['COMMON']['VIEW_CART'])?></a></span> </li>
					</ul>
				  </div>
            <div class="cart_con">
...
<?php
		...
		function mod_currencyselector($title)
		{
			...
			$curr_arr = get_currency_list();
			 $comp_uniqid = uniqid('');
			 $title = "Currency";
			 if($position == 'topband' and $_REQUEST['req']!='cart')
			 {
	  	?>
				  <div class="rt_curr">
