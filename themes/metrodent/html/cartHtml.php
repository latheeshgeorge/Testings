<?php
	...
	class cart_Html
	{
		...
		function Show_Cart()
		{
			...
			if($ecom_siteid==70) // nationwidefireextinguishers
			{
				// check whether any payment type is selected
				$sql_cartsup = "SELECT paytype_id,paymethod_id 
									FROM 
										cart_supportdetails 
									WHERE 
									...
									LIMIT 
										1";
				$ret_cartsup = $db->query($sql_cartsup);
				$insert_paytype = '';
				if($db->num_rows($ret_cartsup))
				{
					$row_cartsup = $db->fetch_array($ret_cartsup);
					if($row_cartsup['paytype_id']==0)
					{
						if(count($cartData['products'])>0)
							$insert_paytype = 'update';
					}
				}
				else
				{
					if(count($cartData['products'])>0)
						$insert_paytype = 'insert';
				}
				if($insert_paytype!='')
				{
					//Check whether credit card type is active for this website
					$sql_pcheck = "SELECT paytype_id  
										FROM 
											payment_types 
										WHERE 
					...
					$cc_check_id = $row_pcheck['paytype_id'];
					$sql_sitepcheck = "SELECT paytype_forsites_id 
										FROM 
											payment_types_forsites 
										WHERE 
											sites_site_id = $ecom_siteid 
											AND paytype_id = $cc_check_id 
											AND paytype_forsites_active = 1 
										LIMIT 
											1";
					$ret_sitepcheck = $db->query($sql_sitepcheck);
					if($db->num_rows($ret_sitepcheck))
					{
						$row_sitepcheck = $db->fetch_array($ret_sitepcheck);
						if($insert_paytype=='update')
						{
							// update the cart_supportdetails with this detail
							$update_cartsupp = "UPDATE cart_supportdetails 
												SET 
													paytype_id='".$cc_check_id."' 
												WHERE 
													sites_site_id = $ecom_siteid 
													AND session_id = '".$session_id."' 
												LIMIT 
													1";
							$db->query($update_cartsupp);
						}
						else
						{
							// insert to the cart_supportdetails with this detail
							$insert_cartsupp = "INSERT INTO cart_supportdetails 
												SET 
													session_id='".$session_id."',
													sites_site_id = $ecom_siteid, 
													paytype_id='".$cc_check_id;
							$db->query($insert_cartsupp);
						}							
						// calling the cart calc once again to refresh the values 
						$cartData 					= cartCalc(); // Calling the function to calculate the details related to cart							
					}
		...
		?>	
			<div class="treemenu">
			  <ul>
				...
				<li><?php echo stripslash_normal($Captions_arr['CART']['CART_MAINHEADING'])?></li>
			  </ul>
			</div>
			<div class="inner_header"><?php echo stripslash_normal($Captions_arr['CART']['CART_MAINHEADING'])?></div>
			<div class="inner_con" >
			...
			<?php
				...
				?>
			  <tr>
				<td colspan="5" align="left" valign="middle" class="cartlogin_msg">
				<?
				if (count($cartData['products'])>0)
				{
				?>
				<?php
					...
					{
					  echo stripslash_normal($Captions_arr['CART']['CART_LOGGED_IN_AS']); echo "&nbsp;".$cartData['customer']['customer_title']." ".$cartData['customer']['customer_surname'].'!';?>
						<br />
						<? echo stripslash_normal($Captions_arr['CART']['CART_IF_YOU_NOT_LOG']);?> <a href="http://<?php echo $ecom_hostname?>/logout.html?rets=1" title="Logout" class="cartlogin_link"><? echo stripslash_normal($Captions_arr['CART']['CART_HERE']);?></a><? echo "&nbsp;". stripslash_normal($Captions_arr['CART']['CART_TO_LOGOUT']);?> 
					<?php
					}
					else // Case not logged in
					{
						...
						{
					?>
						<?php echo stripslash_normal($Captions_arr['CART']['CART_NOT_LOGGED_IN']); ?> <a href="<?php url_link('custlogin.html')?>?redirect_back=1&pagetype=cart" title="Login" class="cartlogin_link"><? echo stripslash_normal($Captions_arr['CART']['CART_HERE']);?></a><? echo "&nbsp;". stripslash_normal($Captions_arr['CART']['CART_TO_LOGIN']);?> 
					<?php
						}
					}
					?>	
				...
				<?php
					...
					{
				?>
				  		<input name="clearcart_button" type="button" class="cart_btn" id="clearcart_button" value="<?php echo stripslash_normal($Captions_arr['CART']['CART_CLEAR'])?>" onclick="if(confirm_message('Are you sure you want to clear all items in the cart?')){show_wait_button(this,'Please Wait...');document.frm_cart.cart_mod.value='clear_cart';document.frm_cart.submit();}" />
			    <?php
				 	}
				 ?>	</td>
			  </tr>
			  <tr>
				<td align="left" valign="middle" class="shoppingcartheaderleft"><?php echo stripslash_normal($Captions_arr['CART']['CART_ITEM'])?></td>
				<td align="left" valign="middle" class="shoppingcartheaderA"><?php echo stripslash_normal($Captions_arr['CART']['CART_PRICE'])?></td>
				<?php
				if($Settings_arr["product_show_instock"]==1)
				{
				?>
				<td align="center" valign="middle" class="shoppingcartheaderA"><?php echo stripslash_normal($Captions_arr['CART']['CART_AVAIL'])?></td>
				<?php
				}
				?>
				<td align="center" valign="middle" class="shoppingcartheaderA" <?php echo ($Settings_arr["product_show_instock"]==0)?'colspan="2"':''?>><?php echo stripslash_normal($Captions_arr['CART']['CART_DISCOUNT'])?></td>
				<td align="center" valign="middle" class="shoppingcartheaderA"><?php echo stripslash_normal($Captions_arr['CART']['CART_QTY'])?></td>
				<td align="right" valign="middle" class="shoppingcartheaderright"><?php echo stripslash_normal($Captions_arr['CART']['CART_TOTAL'])?></td>
			  </tr>
		  <?php
		  ...
		  ?>
				<tr>
					<td align="center" valign="middle" class="shoppingcart_pdt1" colspan="6">
		...
		<?php
		  ...
		  else
		  {
			  ...
			  ?>
				  <tr>
					...
					?>
					<a href="<?php echo url_product($products_arr['product_id'],$products_arr['product_name'])?>" title="<?php echo $products_arr['product_name']?>" class="cart_pdt_link"><?php echo $products_arr['product_name']?></a>					</td>
					<td align="left" valign="middle" class="<?php echo $cur_class?>"><?php echo print_price($products_arr['product_webprice'],true)?></td>
					<?php
					if($Settings_arr["product_show_instock"]==1)
					{
					?>
					<td align="center" valign="middle" class="<?php echo $cur_class?>">
					<?php
					...
					?>
					<td align="center" valign="middle" class="<?php echo $cur_class?>" <?php echo ($Settings_arr["product_show_instock"]==0)?'colspan="2"':''?>>
					-<?php 
							...
							/*if ($products_arr["prom_prodcode_disc"] and $products_arr["savings"]["product"]) // Case if promotional code disc is there for current product
								echo stripslash_normal($Captions_arr['CART']['CART_PROM_DISC']).' '.print_price($products_arr["savings"]["product"],true);
							...
							{	
								...
								{
									case 'custgroup':
										echo stripslash_normal($Captions_arr['CART']['CART_GROUP_DISC']);
									break;
									case 'customer':
										echo stripslash_normal($Captions_arr['CART']['CART_CUST_DISC']);
									break;
								};
							...
							{
								echo stripslash_normal($Captions_arr['CART']['CART_COMBO_DISC']).'<br/> '.print_price($products_arr["savings"]["product_combo"],true);
							}
							elseif($products_arr["savings"]["bulk"]) // Check whether bulk discount is there
							{
								echo stripslash_normal($Captions_arr['CART']['CART_BULK_DISC']).' <br/>'.print_price($products_arr["savings"]["bulk"],true);
							}
							else
					...
					?>
							<div class="updatediv" align="center">&nbsp;<input type="hidden" name="cart_qty_<?php echo $products_arr['cart_id']?>" id="cart_qty_<?php echo $products_arr['cart_id']?>" value="<?php echo $products_arr["cart_qty"]?>" /></div>	
							<div class="updatedivAnew" align="center"><a href="#" class="update_link" onclick="if (confirm_message('<?php echo stripslash_normal($Captions_arr['CART']['CART_ITEM_REM_MSG'])?>')) {document.frm_cart.remcart_id.value='<?php echo $products_arr['cart_id']?>';handle_form_submit(document.frm_cart,'Remove_qty','#rem')}" title="<?php echo stripslash_normal($Captions_arr['CART']['CART_REMOVE'])?>"><img src='<?php url_site_image('cart_delete.gif')?>' border="0" alt="<?php echo stripslash_normal($Captions_arr['CART']['CART_REMOVE'])?>"></a></div>
					<?php	
						}
						else
						{
					?>
							<div class="updatediv" align="center"><input name="cart_qty_<?php echo $products_arr['cart_id']?>" type="text" id="cart_qty_<?php echo $products_arr['cart_id']?>" size="5" maxlength="4" value="<?php echo $products_arr["cart_qty"]?>" style="width:16px;" class="cart_inputA" />
							</div> 
							<div class="updatedivA" align="center"><a href="#" onclick="document.frm_cart.remcart_id.value='<?php echo $products_arr['cart_id']?>';handle_form_submit(document.frm_cart,'Update_qty','#upd')" title="<?php echo stripslash_normal($Captions_arr['CART']['CART_UPDATE'])?>"><img src='<?php url_site_image('cart_update.gif')?>' border="0" alt="<?php echo stripslash_normal($Captions_arr['CART']['CART_UPDATE'])?>"></a>  <a href="#" class="update_link" onclick="if (confirm_message('<?php echo stripslash_normal($Captions_arr['CART']['CART_ITEM_REM_MSG'])?>')) {document.frm_cart.remcart_id.value='<?php echo $products_arr['cart_id']?>';handle_form_submit(document.frm_cart,'Remove_qty','#rem')}" title="<?php echo stripslash_normal($Captions_arr['CART']['CART_REMOVE'])?>"><img src='<?php url_site_image('cart_delete.gif')?>' border="0" alt="<?php echo stripslash_normal($Captions_arr['CART']['CART_REMOVE'])?>"></a></div>
					<?php
						}
					?>
					</td>
					<td align="right" valign="middle" class="<?php echo $cur_class?>"><?php echo print_price($products_arr['final_price'],true)?></td>
				  </tr>
				  <?php
		  ...
		  ?>
			 	<tr>
				<td align="right" valign="middle" class="shoppingcartpricebdr" colspan="5"><?php echo stripslash_normal($Captions_arr['CART']['CART_TOTPRICE'])?></td>
		  ...
		  <?php/*
			...
			?>	
						...
						<tr id="giftwrap_details_tr" <?php echo ($row_cartdet['giftwrap_req'])?'':'style="display:none"'?>>
							<td colspan="6" align="left" valign="middle" class="<?php echo $cur_class?>">
								<table width="100%" border="0" cellspacing="1" cellpadding="1">
								<tr>
								<td width="2%" align="left" valign="top">&nbsp;</td>
								<td width="53%" align="left" valign="top"><?php echo stripslash_normal($Captions_arr['CART']['CART_GIFTWRAP_MSG'])?>(<?php echo print_price($row_gift['giftwrap_messageprice'],true)?>)</td>
								...
								<tr id="giftwrap_message_req_tr" <?php echo ($row_cartdet['giftwrap_msg_req'])?'':'style="display:none"'?>>
								  ...
								  <td colspan="2" align="left" valign="top"><?php echo stripslash_normal($Captions_arr['CART']['CART_TYP_MSG'])?><br />								    
							      <textarea name="giftwrap_message" cols="50" rows="4"><?php echo stripslash_normal($row_cartdet['giftwrap_msg'])?></textarea></td>
								  </tr>
								</table>
								<?php
								...
								?>
										...
										<tr>
										<td align="left" class="shoppingcartgiftwrap_detheading"><a name="a_gwrapopt">&nbsp;</a><?php echo stripslash_normal($Captions_arr['CART']['CART_WRAP_OPT'])?></td>
										</tr>
										</table>
										<table width="100%" border="0" cellpadding="1" cellspacing="1" class="shoppingcartgiftwrap_det">
										<tr>
										...
										?>
											<td width="25%" align="left" valign="top" class="shoppingcartgiftwrap_dettd">
												<span class="shoppingcartgiftwrap_detsubheading"><?php echo stripslash_normal($Captions_arr['CART']['CART_RIBBON'])?></span><br />
												&nbsp;&nbsp;&nbsp;&nbsp;<input name="ribbon_radio" type="radio" class="shoppingcart_radio" id="ribbon_radio" value="0" checked="checked" onclick="handle_form_submit(document.frm_cart,'save_commondetails','#a_gwrapopt')" />
												 <?php echo stripslash_normal($Captions_arr['CART']['CART_NONE'])?><!--&nbsp; <img src="<?php //url_site_image('giftminus.gif')?>" onclick="hideall_giftwrapimagediv(document.frm_cart,'ribbonimg_')" alt="Collapse All" title="Collapse All"/>--><br />
												<?php
												...
												?>		
														...
														<input type="radio" name="ribbon_radio" id="ribbon_radio" value="<?php echo $row_ribbon['ribbon_id']?>" class="shoppingcart_radio" <?php echo $checked?>  onclick="handle_form_submit(document.frm_cart,'save_commondetails','#a_gwrapopt')"/><?php echo stripslash_normal($row_ribbon['ribbon_name'])?> (<?php echo print_price($row_ribbon['ribbon_extraprice'],true)?>)<br />
										...
										?>
											<td width="25%" align="left" valign="top" class="shoppingcartgiftwrap_dettd">
												<span class="shoppingcartgiftwrap_detsubheading"><?php echo stripslash_normal($Captions_arr['CART']['CART_PAPER'])?></span><br />
												...
												<?php echo stripslash_normal($Captions_arr['CART']['CART_NONE'])?> <!--<img src="<?php //url_site_image('giftminus.gif')?>" onclick="hideall_giftwrapimagediv(document.frm_cart,'paperimg_')" alt="Collapse All" title="Collapse All"/>--><br />
												<?php
												...
												?>		
														...
														<input type="radio" name="paper_radio" id="paper_radio" value="<?php echo $row_paper['paper_id']?>" class="shoppingcart_radio" <?php echo $checked?> onclick="handle_form_submit(document.frm_cart,'save_commondetails','#a_gwrapopt')"/><?php echo stripslash_normal($row_paper['paper_name'])?> (<?php echo print_price($row_paper['paper_extraprice'],true)?>)<br />
										...
										?>
											<td width="25%" align="left" valign="top" class="shoppingcartgiftwrap_dettd">
												<span class="shoppingcartgiftwrap_detsubheading"><?php echo stripslash_normal($Captions_arr['CART']['CART_CARDS'])?></span><br />
												...
												<?php echo stripslash_normal($Captions_arr['CART']['CART_NONE'])?><!-- <img src="<?php //url_site_image('giftminus.gif')?>" onclick="hideall_giftwrapimagediv(document.frm_cart,'cardimg_')" alt="Collapse All" title="Collapse All"/>--><br />
												<?php
												...
												?>		
														...
														<input type="radio" name="card_radio" id="card_radio" value="<?php echo $row_card['card_id']?>" class="shoppingcart_radio" <?php echo $checked ?> onclick="handle_form_submit(document.frm_cart,'save_commondetails','#a_gwrapopt')"/><?php echo stripslash_normal($row_card['card_name'])?> (<?php echo print_price($row_card['card_extraprice'],true)?>)<br />
										...
										?>
											<td width="25%" align="left" valign="top" class="shoppingcartgiftwrap_dettd">
												<span class="shoppingcartgiftwrap_detsubheading"><?php echo stripslash_normal($Captions_arr['CART']['CART_BOWS'])?></span><br />
												...
												<?php echo stripslash_normal($Captions_arr['CART']['CART_NONE'])?> <!--<img src="<?php //url_site_image('giftminus.gif')?>" onclick="hideall_giftwrapimagediv(document.frm_cart,'bowimg_')" alt="Collapse All" title="Collapse All"/>--><br />
												<?php
												...
												?>		
														...
														<input type="radio" name="bow_radio" id="bow_radio" value="<?php echo $row_bow['bow_id']?>" class="shoppingcart_radio" <?php echo $checked ?> onclick="handle_form_submit(document.frm_cart,'save_commondetails','#a_gwrapopt')"/><?php echo stripslash_normal($row_bow['bow_name'])?> (<?php echo print_price($row_bow['bow_extraprice'],true)?>)<br />
		  ...
		  <?php
			...
			?>
			  <tr>
				<td colspan="5" align="right" valign="middle" class="shoppingcartpricebdr"><?php echo stripslash_normal($Captions_arr['CART']['CART_GIFTWRAP_TOTAL'])?></td>
		  ...
		  ?>
				<tr>
				<td colspan="6" align="right" valign="middle" class="cart_dlvry">
				...
				?>	
						 <tr>
						 <td align="right" class="shoppingcartcontent_delivery" colspan="2"><?php echo stripslash_normal($Captions_arr['CART']['CART_DELIVERY_LOC'])?><a name="a_deliv">&nbsp;</a> 
							<?php
				   ...
				   ?>
					   <tr>
						 <td align="right" class="shoppingcartcontent" colspan="2"><strong><?php echo stripslash_normal($Captions_arr['CART']['CART_DELIVERY_OPT'])?></strong>
						  <?php
				...
				?>
					<tr>
						<td colspan="6" align="center">
							<table width="100%" border="0" cellspacing="1" cellpadding="1">
							<tr>
							<td width="53%" align="left" valign="middle" class="shoppingcartcontent"><?php echo stripslash_normal($Captions_arr['CART']['CART_WANT_DELIVERY_SPLIT'])?></td>
							<td width="47%" align="left" valign="middle" class="shoppingcartcontent"><input type="checkbox" name="cart_splitdelivery" id="cart_splitdelivery" <?php echo ($row_cartdet['split_delivery']==1)?'checked="checked"':''?> onclick="handle_form_submit(document.frm_cart,'save_commondetails','#a_deliv')" /></td>
							</tr>
							</table></td>
					</tr>
				<?php	
				...
				{ 
					...
					?>
						<tr>
							...
							<td class="shoppingcartcontent" colspan="2" align="left">
			...
			?>
			<tr>
				<td colspan="5" align="right" valign="middle" class="shoppingcartpricebdr"><?php echo stripslash_normal($Captions_arr['CART']['CART_TOTAL_DELIVERY_CHARGES'])?></td>
			...
			<?php
			 ...
			 ?>
				 <tr>
					<td colspan="5" align="right" valign="middle" class="shoppingcartpricebdr"><?php echo stripslash_normal($Captions_arr['CART']['CART_EXTRA_SHIPPING'])?></td>
		   ...
		   // Section to show the tax details
			 ...
			 ?>
					<tr>
						<td colspan="5" align="right" valign="top" class="shoppingcartpricebdr">
				...
				if (($row_cartdet['promotionalcode_id']!=0 or $row_cartdet['voucher_id']!=0))
				{
				 ?>
				  <tr>
					<td colspan="5" align="right" valign="middle" class="shoppingcartpricebdr">
					<?php 
					if ($row_cartdet['promotionalcode_id']!=0 and $cartData["bonus"]['type']=='promotional' and $cartData['totals']['promotional_type']!='product')
					{
					?>
				  ...
				  <tr>
					<td colspan="6" align="left" valign="middle"><div class="shopprodiv">
					  <table width="100%" border="0" cellspacing="0" cellpadding="5">
						<tr>
						  <td width="71%" align="center"><a name="a_prom">&nbsp;</a>
						  <?php	
						  ...
						  <?php
							...
							?>
									<input name="cart_savepromotional" type="hidden" id="cart_savepromotional" value="0"/>	
								 	<input name="cancel_promotionalcode" type="button" class="cart_btn" id="cancel_promotionalcode" value="<?php echo stripslash_normal($Captions_arr['CART']['CART_PROMO_CANCEL_BUTTON'])?>" onclick="document.frm_cart.cart_savepromotional.value=1;handle_form_submit(document.frm_cart,'save_commondetails','#a_prom')" />
						  <?php
						  	}
						  	elseif ($cartData["bonus"]['type']=='')
							{
						  ?>
							  	...
							  	<input name="submit_promotionalcode" type="button" class="cart_btn" id="submit_promotionalcode" value="<?php echo stripslash_normal($Captions_arr['CART']['CART_PROMO_GO'])?>" onclick="document.frm_cart.cart_savepromotional.value=1;handle_form_submit(document.frm_cart,'save_commondetails','')" />
						  <?php
						  	}
		  ...
		  <?php
			...
			<?php		
		      ...
		      $fetch_arr_admin 	= $db->fetch_array($res_admin);
			   ...
			   {
			?>
				   <div class="cart_bonus_more"><a href="<?php url_link('bonuspoint_content.html')?>" title=""><img src="<?php url_site_image('bonusmoreinfo.gif')?>" border="0" /></a></div>
			<?php
				}
			   ?>
			...
			{	
				...
				?>
					 <tr>
						<td colspan="1" align="left" valign="middle" class="shoppingcartpricebdr"><?php echo stripslash_normal($Captions_arr['CART']['CART_TOTAL_POINTS_EARN'])?>: <?php /*?><span class='shoppingcartpriceC'><?php echo $cartData["totals"]["bonus"]?></span><?php */?></td>
						<td colspan="5" align="left" valign="middle" class="shoppingcartpriceC"><?php echo $cartData["totals"]["bonus"]?></td>
					  </tr>
				<?php	
				}
				...
				?>
					 <tr>
						<td colspan="1" align="left" valign="middle" class="shoppingcartpricebdr"><a name="bonus">&nbsp;</a><?php echo stripslash_normal($Captions_arr['CART']['CART_SPENT_BONUS_POINTS'])?>(<? print (int)(($cartData["customer"]['customer_bonus']-$cartData["bonus"]["spending"])); ?> &nbsp;<?php echo stripslash_normal($Captions_arr['CART']['CART_REMAINING'])?>)&nbsp;</td>
						<td colspan="5" align="left" valign="middle" class="shoppingcartpriceC">
						...
						<input  class="cart_btn" type="button" value="<?php echo stripslash_normal($Captions_arr['CART']['CART_SPEND'])?>" onclick="handle_form_submit(document.frm_cart,'save_commondetails','#bonus')">
						</td>
					  </tr>
					 <?php
						}
						...
						?>
						<tr>
							<td colspan="6" class="redtext" align="left"><?php echo str_replace('[min_points]',$Settings_arr['minimum_bonuspoints'],$Captions_arr['CART']['CART_MIN_BONUS_REQ'])?></td>
						</tr>
						<?php
						} 
						...
						?>
						  <tr>
							<td colspan="5" align="right" valign="top" class="shoppingcartpricebdr"><?php echo stripslash_normal($Captions_arr['CART']['CART_SPENT_BONUS_POINTS_VALUE'])?> </td>
			...
			if(!$cust_id)
			{
				...
				?>
					<tr>
						<td class="shoppingcartcontent" colspan="6" align="left">
						<input type="button" name="cust_login_bonus" id="cust_login_bonus" value="<?php echo $Captions_arr['CART']['CART_BONUS_LOGIN_MSG']?>"  class="buttonred_cart" onclick="window.location ='<?php url_link('custlogin.html?redirect_back=1&pagetype=cart')?>'"/>
						</td>
					</tr>
				<?
				}
			...
			?>
			  ...
			  <tr>
				<td colspan="5" align="right" valign="middle" class="shoppingcartpricebdr"><?php echo stripslash_normal($Captions_arr['CART']['CART_TOTAL_FINAL_COST'])?>&nbsp; </td>
		  ...
		  <?php
			...
			?>
				<tr>
					<td colspan="5" align="right" valign="middle" class="shoppingcartpricebdr"><?php echo stripslash_normal($Captions_arr['CART']['CART_LESS_AMT_LATER'])?>&nbsp; </td>
					<td align="right" valign="middle" class="shoppingcartpriceC"><?php echo print_price($str_reduceval,true)?></td>
			  	</tr>
				<tr>
					<td colspan="5" align="right" valign="middle" class="shoppingcartpricebdr"><?php echo stripslash_normal($Captions_arr['CART']['CART_TOTAL_AMT_NOW'])?>&nbsp; </td>
			...
			<?php
				...
				if ($db->num_rows($ret_paytypes))
				{
					...
					?>
								...
								<table width="100%" border="0" cellspacing="0" cellpadding="0" class="cart_payment_table">
								  <tr>
									<td colspan="<?php echo $pay_maxcnt?>" class="cart_payment_header"><?php echo stripslash_normal($Captions_arr['CART']['CART_SEL_PAYTYPE'])?></td>
								  </tr>
								  <tr>
								  <?php
									while ($row_paytypes = $db->fetch_array($ret_paytypes))
								  ...
								  ?>
										...
										<?php
											...
											if($totpaycnt>1)// case if number of payment methods are more than one.
											{
												...
												$on_change = '';
											?>
											<input class="shoppingcart_radio" type="radio" name="cart_paytype" id="cart_paytype" value="<?php echo $row_paytypes['paytype_id']?>" <?php echo ($row_paytypes['paytype_id']==$row_cartdet['paytype_id'])?'checked="checked"':''?> <?php echo $on_change?>/><?php echo stripslash_normal($row_paytypes['paytype_caption']).$add_text?>
										</td>
								<?php
										$pay_cnt++;
            ...
            <td colspan="6" align="right" valign="middle" class="shoppingcartcontent">
			  ...
			  ?>
				...
				?>
				 	 ...
				 	 <img src="<?php url_site_image('cntne-shping.gif')?>" alt="<?php echo stripslash_normal($Captions_arr['CART']['CART_CONT_SHOP'])?>" onclick="show_wait_button(this,'Please Wait...');window.location='<?php echo $ps_url?>'" />
				<?php
					}
					else
					{
				?>
				 	 ...
				 	 <img src="<?php url_site_image('cntne-shping.gif')?>" alt="<?php echo stripslash_normal($Captions_arr['CART']['CART_CONT_SHOP'])?>" onclick="show_wait_button(this,'Please Wait...');window.location='<?php url_link('')?>'" />
				<?php
					}
				?>  
				  </div>
			  ...
			  ?>	
             		 ...
             		 <img src="<?php url_site_image('chect-out.gif')?>" alt="<?php echo stripslash_normal($Captions_arr['CART']['CART_GO_CHKOUT'])?>" onclick="handle_checkout_submit('<?php echo $ecom_hostname?>',0,'<?php echo $cartData["totals"]["bonus_price"]?>')" />
             		</div> 
			  <?php
			  	}
				else
	...
	?>
		...
		<div class="inner_bottom"></div>	
			  ...
			  <?php 
		    if($ecom_siteid!=70)// Live site id for hiding the registration link for nationwidefireextinguishers
	   		{
          ...
          <table class="lgn_table" width="100%" border="0" cellpadding="0" cellspacing="4">
            <tbody>
              ...
              <tr>
				...
				<div class="lgn_btn">			
				<div class="lgn_btn_innerA"><a href="<?php url_link('registration.html?pagetype=cart')?>"><img src="<?php url_site_image('new-account.gif')?>" border="0" /></a></div>
				</div>
      ...
      <div class="lgn_contnt" >
          ...
          <table class="regifontnormal" width="100%" border="0" cellpadding="0" cellspacing="4">
            <tbody>
              ...
              <tr>
				...
				<div class="lgn_btn">
				<div class="lgn_btn_innerA"><a href="javascript:document.frm_custlogin_cart.submit()"><img src="<?php echo url_site_image('login-img.gif')?>" border="0" /></a></div>
				</div></td>
              </tr>
            </tbody>
          </table>
		...
		?>	
			<div class="treemenu">
			  <ul>
				<li><a href="<? url_link('');?>"><?=stripslash_normal($Captions_arr['COMMON']['TREE_MENU_HOME_LINK']);?></a> &gt;&gt;</li>
				<li><?php echo stripslash_normal($Captions_arr['CART']['CART_CHECKOUTHEADING'])?></li>
			  </ul>
			</div>
			<div class="inner_header"><?php echo stripslash_normal($Captions_arr['CART']['CART_CHECKOUTHEADING'])?></div>
			<div class="inner_con" >
	        <div class="inner_top"></div>
    	    <div class="inner_middle_cart">		
			  ...
			  ?>
				  <tr>
					...
					-<?php 
							// if combo disc exists then that should be displayed otherwise the normal or promotional disc is to be shown
							/*if ($products_arr["prom_prodcode_disc"] and $products_arr["savings"]["product"]) // Case if promotional code disc is there for current product
								echo stripslash_normal($Captions_arr['CART']['CART_PROM_DISC']).'<br/> '.print_price($products_arr["savings"]["product"],true);
							elseif ($products_arr["cust_disc_type"] !='' and $products_arr["savings"]["product"]) // Case if promotional code disc is there for current product
							{	
								switch($products_arr['cust_disc_type'])
								{
		  ...
		  ?>
			 	<tr>
				<td align="right" valign="middle" class="shoppingcartpriceC">&nbsp;</td>
				<td align="right" valign="middle" class="shoppingcartpriceC">&nbsp;</td>
				<td align="right" valign="middle" class="shoppingcartpriceC">&nbsp;</td>
				<td align="right" valign="middle" class="shoppingcartpricebdr" colspan="2"><?php echo stripslash_normal($Captions_arr['CART']['CART_TOTPRICE'])?>&nbsp;</td>
				<td align="right" valign="middle" class="shoppingcartpriceC"><?php echo print_price($cartData["totals"]["subtotal"],true)?></td>
			  </tr>
		  <?php
		  		$module_name = 'mod_giftwrap';
			...
			?>	
						<tr>
						<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($Captions_arr['CART']['CART_GIFTWRAP_REQ'])?> (<?php echo print_price($row_gift['giftwrap_minprice'],true)?>)</td>
						<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php if ($row_cartdet['giftwrap_req']==1) echo 'Yes'?></td>
						</tr>
					<?php
					...
					?>
						<tr>
							<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($Captions_arr['CART']['CART_GIFTWRAP_MSG'])?>(<?php echo print_price($row_gift['giftwrap_messageprice'],true)?>)</td>
							<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($Captions_arr['CART']['CART_YES'])?></td>
						</tr>
						<tr>
							<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($Captions_arr['CART']['CART_MSG'])?></td>
							<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo nl2br(stripslash_normal($row_cartdet['giftwrap_msg']))?></td>
						</tr>
					<?php
						}
					...
					?>
						<tr>
							<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($Captions_arr['CART']['CART_RIBBON'])?>(<?php echo print_price($row_ribbon['ribbon_extraprice'],true)?>)</td>
							<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($row_ribbon['ribbon_name'])?></td>
						</tr>
					<?php	
						}
					...
					?>
						<tr>
							<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($Captions_arr['CART']['CART_PAPER'])?>(<?php echo print_price($row_paper['paper_extraprice'],true)?>)</td>
							<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($row_paper['paper_name'])?></td>
						</tr>
					<?php	
						}
					...
					?>
						<tr>
							<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($Captions_arr['CART']['CART_CARDS'])?>(<?php echo print_price($row_card['card_extraprice'],true)?>)</td>
							<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($row_card['card_name'])?></td>
						</tr>
					<?php	
						}
					...
					?>
						<tr>
							<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($Captions_arr['CART']['CART_BOWS'])?>(<?php echo print_price($row_bow['bow_extraprice'],true)?>)</td>
							<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($row_bow['bow_name'])?></td>
						</tr>
					<?php	
						}
			...
			if ($deliverydet_Arr['delivery_type'] != 'None') // Check whether any delivery method is selected for the site
			{
				...
				?>	
							<tr>
								<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($Captions_arr['CART']['CART_DELIVERY_LOC'])?></td>
								<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($row_loc['location_name'])?></td>
							</tr>
				<?php
						}
					}
				...
				?>	
							<tr>
								<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($Captions_arr['CART']['CART_DELIVERY_OPT'])?></td>
								<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($row_grp['delivery_group_name'])?></td>
							</tr>
				<?php
						}
					}	
				...
				?>
					<tr>
						<td colspan="3" align="left" valign="middle" class="shoppingcartcontent"><?php echo stripslash_normal($Captions_arr['CART']['CART_DELIVERY_SPLIT_ACTIVATED'])?></td>
						<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($Captions_arr['CART']['CART_YES'])?></td>
					</tr>
				<?php	
				}
		  ...
		  <?php
			...
			?>
				 <tr>
					<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($Captions_arr['CART']['CART_TOTAL_POINTS_EARN'])?>&nbsp;</td>
					<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo $cartData["totals"]["bonus"]?></td>
				  </tr>
			<?php	
			}
		  	// show the total final price
		  ...
		  ?>
				...
				<td colspan="6" align="left" >	
			<div class="cart_continue_div" align="left">
	             <input name="backto_cart" type="button" class="cart_btn" id="backto_cart" value="<?php echo stripslash_normal($Captions_arr['CART']['CART_BACK_CARTPAGE'])?>" onclick="show_wait_button(this,'Please Wait...');gobackto_cart('<?php echo ($protectedUrl)? base64_encode($ecom_hostname):$ecom_hostname?>')"/>
	         </div>  
			   </td>
			  </tr> 
			  </table>
		...
		<table class="regifontnormal" width="100%" border="0" cellpadding="0" cellspacing="0">	
			<tr>
				<td colspan="6" align="left" class="regiheader"><span class="reg_header"><span><?php echo stripslash_normal($Captions_arr['CART']['CART_FILL_BILLING_DETAILS'])?></span></span></td>
			</tr>
			<?php
		   ...
		   <tr>
			...
			<?php
			 ...
			 // check whether billing and shipping address can be different in the site.
			  ...
			  ?>
			  	<tr>
					<td colspan="6" align="left" valign="middle" class="regiconentA">
				...
				<tr id="checkout_delivery_tr">
					...
					<table width="100%" cellpadding="1" cellspacing="1" border="0">
					<tr>
					<td colspan="6" align="left" class="regiheader"><span class="reg_header"><span><?php echo stripslash_normal($Captions_arr['CART']['CART_FILL_DELIVERY_ADDRESS'])?></span></span></td>
					</tr>	
						<?php
		...
		<div class="inner_bottom"></div>
				...
				$texttd_class			= 'regi_txtfeild';
				$specialhead_tag_start 	= '<span class="reg_header"><span>';
				$specialhead_tag_end 	= '</span></span>';				$cellpadding 			= 6;		
				include 'show_dynamic_fields.php';
				?>
				 <div class="inner_con" >
		<div class="inner_top"></div>
		...
		<table class="regifontnormal" width="100%" border="0" cellpadding="0" cellspacing="0">
			...
			?>
					 <tr>
						...
						<table width="100%" cellpadding="1" cellspacing="1" border="0">
						<tr>
						 <td colspan="2" class="regiheader" align="left"><span class="reg_header"><span><?php echo stripslash_normal($Captions_arr['CART']['CART_CREDIT_CARD_DETAILS'])?></span></span></td>
						</tr>
						<?php
			...
			?>
					<tr>
						...
						<tr>
							...
							<?php echo stripslash_normal($Captions_arr['CART']['CART_ABLE_TO_BUY'])?>
						</td>
						<?php	
						...
						?>
								...
								<td width="90%" align="left" class="regiconent">
			...
			<?php	
				...
				else// case if credit card details not to be picked
				{
					...
					{	
						...
						<table width="100%" cellpadding="1" cellspacing="1" border="0">
						<tr>
						 <td colspan="2" class="regiheader" align="left"><span class="reg_header"><span><?php echo stripslash_normal($Captions_arr['CART']['CART_CHEQUE_DETAILS'])?></span></span></td>
						</tr>
						<?php
							// Get the list of credit card static fields to be shown in the checkout out page in required order
							$sql_checkout = "SELECT field_det_id,field_key,field_name,field_req,field_error_msg   
						...
						break;
						case 'invoice':
							$checkout_caption = 'Place Order on Account';
						break;
			...
			?>
			  <tr>
				<td colspan="6" align="right" valign="middle" class="cartterms">			
					 <?php
						if($Settings_arr['terms_and_condition_at_checkout'])
						{
					?>
							<br /><input class="shoppingcart_radio" type="checkbox" name="cart_terms" id="cart_terms" value="1" /><a href="<?php url_link('terms-conditions-pg49796.html')?>" title="<?php echo stripslash_normal($Captions_arr['CART']['CART_ACCEPT_TERMS_CONDITIONS'])?>" target="_blank"><strong><?php echo stripslash_normal($Captions_arr['CART']['CART_ACCEPT_TERMS_CONDITIONS'])?></strong></a>
					<?php	
						}
					?>	
				</td>
            ...
            <div class="cart_continue_div" align="left">
			 ...
			 // Handling the case of login required before checkout.
			  ...
			  ?>	
			  		<div class="cart_checkout_div" align="right">
						<?php /*?><input name="continue_checkout" type="submit" class="cart_btn" id="continue_checkout" value="<?php echo $checkout_caption?>"/><?php */?>
					<?php
					$prev_req = 0;
		...
		<div class="treemenu">
			  <ul>
				<li><a href="<? url_link('');?>"><?=stripslash_normal($Captions_arr['COMMON']['TREE_MENU_HOME_LINK']);?></a> &gt;&gt;</li>
				<li><?php echo stripslash_normal($Captions_arr['CART']['CART_CHECKOUTPREVIEWHEADING'])?></li>
			  </ul>
			</div>
			<?php
			}
			?>
			<div class="inner_header"><?php 
			if($auto_submit==false)
			{
				echo stripslash_normal($Captions_arr['CART']['CART_CHECKOUTPREVIEWHEADING']);
			}	
			?></div>
			<div class="inner_con" >
	        <div class="inner_top"></div>
		 ...
		 <?php
			  ...
			  ?>
				  <tr>
					...
					?>
					<a href="<?php echo url_product($row_orddet['products_product_id'],$row_orddet['product_name'])?>" title="<?php echo $row_orddet['product_name']?>" class="shoppingcartprod_link"><?php echo $row_orddet['product_name']?></a>
					</td>
					<td align="left" valign="middle" class="<?php echo $cur_class?>"><?php echo print_price($row_orddet['order_retailprice'],true)?></td>
				<?php
					if($Settings_arr["product_show_instock"]==1)
					{
					?>
					...
					-<?php 
							/*if ($row_orddet["order_discount"]>0)
							{
								...
								if ($row_orddet["order_discount_type"]=='promotional') // Case if promotional code disc is there for current product
									echo stripslash_normal($Captions_arr['CART']['CART_PROM_DISC']).' '.print_price($row_orddet["order_discount"],true);
								elseif($row_orddet["order_discount_type"]=='combo') // Check whether combo discount is there
								{
									echo stripslash_normal($Captions_arr['CART']['CART_COMBO_DISC']).' '.print_price($row_orddet["order_discount"],true);
								}
								elseif($row_orddet["order_discount_type"]=='bulk') // Check whether bulk discount is there
								{
									echo stripslash_normal($Captions_arr['CART']['CART_BULK_DISC']).' '.print_price($row_orddet["order_discount"],true);
								}
								else
					...
					<td align="right" valign="middle" class="<?php echo $cur_class?>"><?php echo print_price($row_orddet['order_rowtotal'],true)?></td>
				  </tr>
				  <?php
				  	// If variables exists for current product, show it in the following section
					if ($vars_exists) 
		  ...
		  <?php
			...
			?>	
						<tr>
						<td colspan="3" align="left" valign="middle" class="shoppingcartcontent"><?php echo stripslash_normal($Captions_arr['CART']['CART_GIFTWRAP_REQ'])?> (<?php echo print_price($row_ord['order_giftwrap_minprice'],true)?>)</td>
						<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php if ($row_ord['order_giftwrap']=='Y') echo 'Yes'?></td>
						</tr>
					<?php
					...
					?>
						<tr>
							<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($Captions_arr['CART']['CART_GIFTWRAP_MSG'])?>(<?php echo print_price($row_ord['order_giftwrap_message_charge'],true)?>)</td>
							<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent">Yes</td>
						</tr>
						<tr>
							<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($Captions_arr['CART']['CART_MSG'])?></td>
							<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo nl2br(stripslash_normal($row_ord['order_giftwrapmessage_text']))?></td>
						</tr>
					<?php
						}
					...
					?>
								<tr>
									...
									<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($row_giftwrap_det['giftwrap_name'])?></td>
								</tr>
					<?php		
							}
					?>
			...
			if ($row_ord['order_deliverytype'] != 'None') // Check whether any delivery method is selected for the site
			{
				...
				?>	
							<tr>
								<td colspan="3" align="left" valign="middle" class="shoppingcartcontent"><?php echo stripslash_normal($Captions_arr['CART']['CART_DELIVERY_LOC'])?></td>
								<td colspan="3" align="left" valign="middle" class="shoppingcartcontent"><?php echo stripslash_normal($row_ord['order_deliverylocation'])?></td>
							</tr>
				<?php
					}
					// Check whether any delivery group exists for the site
				...
				?>	
							<tr>
								<td colspan="3" align="left" valign="middle" class="shoppingcartcontent_indent"><?php echo stripslash_normal($Captions_arr['CART']['CART_DELIVERY_OPT'])?></td>
								<td colspan="3" align="left" valign="middle" class="shoppingcartcontent"><?php echo stripslash_normal($row_ord['order_delivery_option'])?></td>
							</tr>
				<?php
					}	
				...
				?>
					<tr>
						<td colspan="3" align="left" valign="middle" class="shoppingcartcontent"><?php echo stripslash_normal($Captions_arr['CART']['CART_DELIVERY_SPLIT_ACTIVATED'])?></td>
						<td colspan="3" align="left" valign="middle" class="shoppingcartcontent"><?php echo stripslash_normal($Captions_arr['CART']['CART_YES'])?></td>
					</tr>
				<?php	
				}
				// Check whether delivery is charged, then show the total after applying delivery charge
		  ...
		  <?php
			...
			?>
				  <tr>
					<td colspan="3" align="left" valign="middle" class="shoppingcartpricebdr"><?php echo stripslash_normal($Captions_arr['CART']['CART_TOTAL_POINTS_EARN'])?>&nbsp;</td>
		...
		?>	
			<tr>
				 <td colspan="6" class="regiheader" align="left"><span class="reg_header"><span><?php echo stripslash_normal($Captions_arr['CART']['CART_BILL_DETAILS'])?></span></span></td>
			</tr>
			<?php
					// Including the file to show the dynamic fields for orders to the give position of static fields
					$cur_pos 		= 'TopInStatic';
				...
				?>
							<tr>
								...
								<?php echo stripslash_normal($row_checkout['field_name'])?></div>
								</td>
								<td align="left" colspan="4" class="shoppingcartcontent">:
								<?php
			...
			<?php
				...
				?>
				<tr>
					<td colspan="6" align="left" valign="middle" class="regiheader"><span class="reg_header"><span><?php echo stripslash_normal($Captions_arr['CART']['DELIVERY_ADDRESS'])?></span></span></td>
				</tr>	
				<?php
					// Get the list of billing address static fields to be shown in the checkout out page in required order
					$sql_checkout = "SELECT field_name,field_orgname 
			...
			?>
			  <tr>
				<td colspan="2" align="left" valign="middle" class="shoppingcartcontent"><div style="padding-left:60px;">	
					<?php echo stripslash_normal($Captions_arr['CART']['CART_PAYMENT_TYPE'])?></div>
				</td>
				<td colspan="4" align="left" valign="middle" class="shoppingcartcontent">			
				<?php 
			  ...
			  ?>
				  <tr>
					<td colspan="2" align="left" valign="middle" class="shoppingcartcontent"><div style="padding-left:60px;">		
						<?php echo stripslash_normal($Captions_arr['CART']['CART_GATEWAY_USED'])?></div>
					</td>
					<td colspan="4" align="left" valign="middle" class="shoppingcartcontent">			
					<?php 
		...
		?>
			<div class="treemenu">
			  <ul>
				<li><a href="<? url_link('');?>"><?=stripslash_normal($Captions_arr['COMMON']['TREE_MENU_HOME_LINK']);?></a> &gt;&gt;</li>
				<li><?php echo stripslash_normal($Captions_arr['CART']['CART_CHECKOUT_SUCCESS_TITLE'])?></li>
			  </ul>
			</div>
			<div class="inner_header"><?php echo stripslash_normal($Captions_arr['CART']['CART_CHECKOUT_SUCCESS_TITLE'])?></div>
			<div class="inner_con" >
	        <div class="inner_top"></div>
    	    <div class="inner_middle_cart">		
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="shoppingcarttable">
			  ...
			  <tr>
				...
				?>
				<br /> <br />
				<a href="<?php url_link('')?>">Please click here to return to Our Home Page</a>
		...
		?>
			<div class="treemenu">
			  <ul>
				<li><a href="<? url_link('');?>"><?=stripslash_normal($Captions_arr['COMMON']['TREE_MENU_HOME_LINK']);?></a> &gt;&gt;</li>
				<li><?php echo stripslash_normal($Captions_arr['CART']['CART_CHECKOUT_FAILED_TITLE'])?></li>
			  </ul>
			</div>
			<div class="inner_header"><?php echo stripslash_normal($Captions_arr['CART']['CART_CHECKOUT_FAILED_TITLE'])?></div>
			<div class="inner_con" >
	        <div class="inner_top"></div>
    	    <div class="inner_middle_cart">	
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="shoppingcarttable">
		...
		?>
			<div class="treemenu">
			  <ul>
				<li><a href="<? url_link('');?>"><?=stripslash_normal($Captions_arr['COMMON']['TREE_MENU_HOME_LINK']);?></a> &gt;&gt;</li>
				<li><?php echo stripslash_normal($Captions_arr['CART']['CART_NOCHEXCHECKOUT_SUCCESS'])?></li>
			  </ul>
			</div>
			<?php /*?><div class="inner_header"><?php echo stripslash_normal($Captions_arr['CART']['CART_NOCHEXCHECKOUT_SUCCESS'])?></div><?php */?>
			<div class="inner_con" >
	        <div class="inner_top"></div>
    	    <div class="inner_middle_cart">	
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="shoppingcarttable">
