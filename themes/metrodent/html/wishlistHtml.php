		...
		<form method="post" name="frm_wishlist" class="frm_cls" action="<?php url_link('wishlist.html')?>">
			...
			<div class="treemenu">
			  <ul>
				...
				<li><?php echo stripslash_normal($Captions_arr['WISHLIST']['WISHLIST_MAINHEADING'])?></li>
			  </ul>
			</div>
			<div class="inner_header"><?php echo $Captions_arr['WISHLIST']['WISHLIST_MAINHEADING']?></div>
			<div class="inner_con" >
			...
			<?php
				...
				{  
					...
					if ($custom_id) // Case logged in 
					{
					  echo stripslash_normal($Captions_arr['CART']['CART_LOGGED_IN_AS']); echo "&nbsp;".$cartData['customer']['customer_title']." ".$cartData['customer']['customer_fname']." ".$cartData['customer']['customer_mname']." ".$cartData['customer']['customer_surame'];?>
						<br />
						<? echo stripslash_normal($Captions_arr['CART']['CART_IF_YOU_NOT_LOG']);?> <a href="http://<?php echo $ecom_hostname?>/logout.html" title="Logout" class="cartlogin_link"><? echo stripslash_normal($Captions_arr['CART']['CART_HERE']);?></a><? echo "&nbsp;". stripslash_normal($Captions_arr['CART']['CART_TO_LOGOUT']);?> 
					<?php
					}
					?>
					<?php
              ...
              <? 
					if($Settings_arr['empty_wishlist']==1)
					{
						...
						{
					?>
							<input name="clearenq_button" type="button" class="cart_btn" id="clearenq_button" value="<?php echo stripslash_normal($Captions_arr['WISHLIST']['WISHLIST_CLEAR'])?>" onclick="if(confirm_message('<? echo stripslash_javascript($Captions_arr['WISHLIST']['WISHLIST_CLEARALLMSG'])?>')){document.frm_wishlist.wish_mod.value='clear_wishlist';document.frm_wishlist.submit();}" />
					<?php
						}
				    }
				 ?>	</td>
			   ...
			   <tr>
					<td align="left" valign="middle" class="shoppingcartheaderleft" >&nbsp;</td>
					<td align="left" valign="middle" class="shoppingcartheaderA" ><?php echo stripslash_normal($Captions_arr['WISHLIST']['WISHLIST_ITEM'])?></td>
					<td align="left" valign="middle" class="shoppingcartheaderA" ><?php echo stripslash_normal($Captions_arr['WISHLIST']['WISHLIST_VAR'])?></td>
					<td align="center" valign="middle" class="shoppingcartheaderA"><?php echo stripslash_normal($Captions_arr['WISHLIST']['WISHLIST_PRICE'])?></td>
					<td align="center" valign="middle" class="shoppingcartheaderright"><?php echo stripslash_normal($Captions_arr['WISHLIST']['WISHLIST_ACTION'])?></td>
		 	  </tr>
		  <?php
				...
				?>
				  <tr>
				  <td align="left" valign="middle" class="<?=$cur_class?>" width="8%" >
				  <input type="checkbox" name="wishlist_check[]" value="<?=$row_prod_det['product_id']?>" />
					...
					<td align="left" valign="middle" class="<?=$cur_class?>"  width="30%">
					<?php
					...
					<td align="center" valign="middle" class="<?=$cur_class?>"  width="20%">
					<?php
			  ...
			  if($_REQUEST['fproduct_id']){?>
			  	<input name="back_Submit" class="cart_btn" value="Continue Shopping" type="button"  onclick="window.location='<?php echo $ps_url?>'"> 
			 	<?php
				}
				?>
				</td>
				...
				if($custom_id) // ** Show the wishlist button only if logged in 
					{
			  ?>
						<input name="submit_wishlist" type="button" class="cart_btn" id="submit_wishlist" value="<?php echo stripslash_normal($Captions_arr['WISHLIST']['WISHLIST_BUYNOW']);?>"  onclick="return select_products();"  />
			  <?php
					}
					?>
					</td>
