					...
					if ($proceed)
					{
						...
						else
						{
							if($shelfData['shelf_currentstyle']=='nor' and $shelfData['shelf_displaytype']=='3row')
								$Limit = ' LIMIT 3';
							elseif($shelfData['shelf_currentstyle']=='nor' and $shelfData['shelf_displaytype']=='3rowall')
								$Limit = ' LIMIT 6';
							elseif($shelfData['shelf_currentstyle']=='sp1' and $shelfData['shelf_displaytype']=='3row')
								$Limit = ' LIMIT 2';
							elseif($shelfData['shelf_currentstyle']=='sp1' and $shelfData['shelf_displaytype']=='3rowall')
								$Limit = ' LIMIT 2';	
                        }                       
							...
							if($shelfData['shelf_currentstyle']=='nor') // case of normal design layout
							{
								...
								<?php		
									...
									case '3rowall': // case of three in a row for normal
										...
										{
											 $desc = stripslashes($desc);
											 $HTML_maindesc = '<div class="normal_shlfA_desc_outrB">'.$desc.'</div>';
										}
										if ($tot_cnt>0 and ($_REQUEST['req']!='') and $tot_cnt>$prodperpage)
										{
											$paging 		= paging_footer_advanced($path,$query_string,$tot_cnt,$start_var['pg'],$start_var['pages'],'',$pg_variable,'Products',$pageclass_arr);
					...
					<?php		
							...
							elseif($shelfData['shelf_currentstyle']=='sp1') // case of special layout 1
							{
								switch($shelfData['shelf_displaytype'])
								{
									...
									<?php	
									break;
									case '3rowall': // case of two in a row 
										$HTML_comptitle = $HTML_maindesc = $HTML_paging = $HTML_showall = '';
										if($cur_title!='')
										{
