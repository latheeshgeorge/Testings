...
if($_REQUEST['contactus_send_button'])
{
	// Sending contact us mail
	$name = $HTTP_POST_VARS["name"];
	$message = " Name : " . $HTTP_POST_VARS["name"] . "\nEmail : " . $HTTP_POST_VARS["email"] . "\nContact No : " . $HTTP_POST_VARS["contactNumber"] . "\nEnquiry : " . $HTTP_POST_VARS["enquiry"] ;
	$email = $HTTP_POST_VARS["email"];
	$headers = "From: \"$name\" <$email>";
	$to = "enquiries@stoneybrook.co.uk";
	mail($to, "Enquiry Form", $message, $headers);
	$error_static = "Details Submitted successfully";
}
