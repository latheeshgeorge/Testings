				...
				if ($cache_exists==false)
				{
	                ...
	                <?php 
	                        if($prev_grp != $grpData['group_id'])
							{
								if ($grpData['group_hidename']==0) // Decide whether or not to show the groupname
								{
									echo "<span class='footer-title'>".stripslashes($title)."</span>";
								}
								$prev_grp = $grpData['group_id'];
							}	
		  			?>			
	...
	if(document.getElementById('cate_menu_img_'+cateID))
	{
		...
		?>				
					...
					<form method="post" name="frm_quicksearch" class="frm_cls" action="<?php url_link('search.html')?>">
						...
						<tbody>
						<tr>
							<td class="inputAB"><input name="quick_search" type="text" class="inputAB" id="quick_search"  value="Search Puregusto" onFocus="if (this.value=='Search Puregusto') this.value='';" onBlur="if (this.value=='') this.value='Search Puregusto';"/>
