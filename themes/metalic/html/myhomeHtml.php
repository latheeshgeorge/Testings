...
# Description 	: Page which holds the display logic for listing my details in myhome page
# Coded by 		: Sny
...
class myhome_Html
{
	function Display_WelcomeMessage($mesgHeader,$Message)
	{
		...
		?>
			...
			<div class="pro_det_treemenu" align="left">
			<ul>
			<li><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a>>></li> <li><?php echo  stripslash_normal($Captions_arr['LOGIN_HOME']['LOGIN_HOME_TREE_MENU'])?></li>
			</ul>
			</div>
	...
	function Show_Recently_Purchased_Products_With_Categories($used_val_arr)
	{
		...
		if(count($cur_cat_arr))
		{
			...
			for($i=0;$i<count($cur_cat_arr);$i++)
			{
				...
				{
				?>
					<div class="shelf_1row_header" align="left"><?php echo stripslash_normal($row_cat['category_name'])?></div>
				<?php
		...
		if ($db->num_rows($ret_favcat))
		{
			...
			list($row_last_login) 	= $db->fetch_array($ret_last_login);
					...
					while ($row_favcat = $db->fetch_array($ret_favcat))
					{
						...
						{
						?>
								<div class="shelf_1row_header" align="left"><?php echo stripslash_normal($row_favcat['category_name'])?></div>
						<?php
						if($db->num_rows($ret_prods_offers))
						{
						?>
						...
						?>
						<div class="showall_div"><a href="<?php  url_category_all($row_favcat['category_id'],$row_favcat['category_name'],-1)?>" class="middle_showall_link" title="<?php echo stripslash_normal($Captions_arr['COMMON']['SHOW_ALL'])?>"><? echo stripslash_normal($Captions_arr['COMMON']['SHOW_ALL'])?></a>
						</div>
						<?php					
						}
					}
	...
	function Show_Favourite_Products($used_val_arr)
	{
		...
		if($db->num_rows($ret_fav_products)>0)
		{
			?>
			<div class="shelf_1row">
			<div class="shelf_1row_header" align="left"><?php echo stripslash_normal($Captions_arr['LOGIN_HOME']['LOGIN_HOME_FAV_PROD'])?></div>
			<?php
			switch($displaytype)
			{
