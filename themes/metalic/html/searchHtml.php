<?php
	...
	class search_Html
	{
			...
			$first_querystring=$query_string; //Assigning the initial string to the variable.
				        ...
				        if ($_REQUEST['req']!='')// LIMIT for products is applied only if not displayed in home page
						{
							...
							<div class="shelf_1row">
								...
								<li><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a><? if($_REQUEST['quick_search']!=""){?> >> <? echo $_REQUEST['quick_search']; } ?></li></ul></div>
								<?php
								if(trim($search_desc)) {
								...
								<div class="shelf_1row_content"><?php echo stripslashes($top_content)?> </div>
								<div class="cat_navigation">
									<ul>
										<li><?php echo $Captions_arr['SEARCH']['SEARCH_SORT']?></li>
										<li> <select name="searchprod_sortbytop" id="searchprod_sortbytop">
										...
										<li><?php echo $Captions_arr['SEARCH']['SEARCH_ITEMS'] ?></li>
										<li><select name="searchprod_prodperpagetop" id="searchprod_prodperpagetop">
										<?php
										...
										<li><input type="submit" name="top_submit_Page" value="<?php echo $Captions_arr['SEARCH']['SEARCH_GO'];?>" class="buttonred" onclick="handle_searchdropdownval_sel('<?php echo $ecom_hostname?>','searchprod_sortbytop','searchprod_sortordertop','searchprod_prodperpagetop')" /></li>
									</ul>
								</div>
								...
								{
									?>
									<div class="shelf_1row_header"><?php echo $cur_title?></div>
									<?
								}
								if ($tot_cnt>0 )
								{
								...
								<div class="shelf_1row_img"><a href="<?php url_product($row_search['product_id'],$row_search['product_name'],-1)?>" title="<?php echo stripslashes($row_search['product_name'])?>">
											<?php
								...
								<div class="shelf_1row_name"><a href="<?php url_product($row_search['product_id'],$row_search['product_name'],-1)?>" title="<?php echo stripslashes($row_search['product_name'])?>"><?php echo stripslashes($row_search['product_name'])?></a></div>
								<div class="shelf_1row_content"><?php echo stripslashes($row_search['product_shortdesc'])?> </div>
								...
								<ul class="shelf_button">
									<li class="shelf_button_li">
										<div class="more_div">
											<?php show_moreinfo($row_search,'button_yellow')?>
										</div>
										<?php
										$class_arr 					= array();
										...
										show_addtocart($row_search,$class_arr,$frm_name,'','','',$class_div)
										?>
									</li>
								</ul>
								</form>
							...
							//show_paging($path,$_REQUEST[$pg_variable],$start_arr['counterstart'],$tot_cnt,$prodperpage,$pg_variable,'')
								 $query_string =$first_querystring;
								$query_string .= "search_sortby=".$prodsort_by."&search_sortorder=".$prodsort_order."&search_prodperpage=".$prodperpage."&pos=bottom";
								...
								<div class="cat_navigation">
									<ul>
										...
										<li><input type="submit" name="bottom_submit_Page" value="<?php echo $Captions_arr['SEARCH']['SEARCH_GO']?>" class="buttonred" onclick="handle_searchdropdownval_sel('<?php echo $ecom_hostname?>','searchprod_sortbybottom','searchprod_sortorderbottom','searchprod_prodperpagebottom')"  />
										</li>
									</ul>
								</div>
		...
		function Show_Search_Category($search_sql,$tot_cnt)
		{
			...
			$first_querystring=$query_string; //Assigning the initial string to the variable.
				        ...
				        if ($_REQUEST['req']!='')// LIMIT for products is applied only if not displayed in home page
						{
						  ...
						  ?>
								...
								<div class="cat_navigation">
									<ul>
										<li><?php echo $Captions_arr['SEARCH']['SEARCH_SORT']?></li>
										<li> <select name="searchcat_sortbytop" id="searchcat_sortbytop">
										...
										<li><?php echo $Captions_arr['SEARCH']['SEARCH_ITEMS'] ?></li>
										<li><select name="searchcat_prodperpagetop" id="searchcat_prodperpagetop">
												<?php
													for ($ii=$Settings_arr["productlist_interval"];$ii<=$Settings_arr["productlist_maxval"];$ii+=$Settings_arr["productlist_interval"])
													{
												?>
										...
										<li><input type="submit" name="top_submit_Page" value="<?php echo $Captions_arr['SEARCH']['SEARCH_GO'];?>" class="buttonred" onclick="handle_searchcatdropdownval_sel('<?php echo $ecom_hostname?>','searchcat_sortbytop','searchcat_sortordertop','searchcat_prodperpagetop')" /></li>
									</ul>
								</div>
						...
						$pass_type = 'image_iconpath';
								...
								<div class="shelf_1row_name"><a href="<?php url_category($row_search['category_id'],$row_search['category_name'],-1)?>" title="<?php echo stripslashes($row_search['category_name'])?>"><?php echo stripslashes($row_search['category_name'])?></a></div>
								<div class="shelf_1row_content"><?php echo stripslashes($row_search['category_shortdescription'])?> </div>
								</div>
								<?
								}
								?>
								...
								<div class="cat_navigation">
									<ul>
										...
										<li><input type="button" name="bottom_submit_Page" value="<?php echo $Captions_arr['SEARCH']['SEARCH_GO']?>" class="buttonred" onclick="handle_searchcatdropdownval_sel('<?php echo $ecom_hostname?>','searchcat_sortbybottom','searchcat_sortorderbottom','searchcat_prodperpagebottom')"  />
										</li>
									</ul>
								</div>
		...
		function advancedSearch()
		{
			...
			<table width="100%" border="0" cellspacing="1" cellpadding="1" class="advance-search-table">
			<tr>
			  <td colspan="3" align="left" valign="top" >
			   <div class="pro_det_treemenu" align="left"> <ul>
							<li><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a>>></li> <li><?php echo $Captions_arr['SEARCH']['SEARCH_ADVANCED_SEARCH']?></li></ul></div>
			 </td>
			</tr>
			<?php
			...
			<td colspan="3">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
					<td align="left" valign="middle" class="searchfont_header" width="28%">Search keyword </td>
					<td  colspan="2" align="left" valign="middle" class="searchfont"><input name="quick_search" type="text"  id="adv_quick_search"  value="<?=$_REQUEST['quick_search']?>" size="15"/>
		...
		?>
				<table align = "center" width="98%" border="0" cellspacing="0" cellpadding="3" class="">
					<tr>
						...
						{
							echo $Captions_arr['SEARCH']['SEARCH_NO_PRODUCTS_WITH_NO_KEYWORD']	;
						}	
