...
?>
	<div class="pro_det_treemenu" align="left"> <ul>
							<li><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a>>></li> <li><?=$Captions_arr['SITEMAP']['SITEMAP_TREEMENU_TITLE']?></li></ul></div>
		<?
 //////////////***************LISTING OF STATIC PAGES ,CATEGORIES AND PRODUCTS FOR SITE MAP*************///////////////////
		...
		$sql_last_categid = "SELECT product_default_category_id 
								FROM products 
								WHERE product_id = ".$pdt_id." AND sites_site_id = $ecom_siteid LIMIT 1";
		...
		list($last_categid) = $db->fetch_array($ret_last_categid);
			...
			$sql_last_catname = "SELECT category_id,category_name 
									...
									WHERE category_id = ".$last_categid." AND sites_site_id = ".$ecom_siteid;	
