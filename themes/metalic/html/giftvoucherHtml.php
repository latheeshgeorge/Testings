	...
	?>
			<form method="post" action="<?php echo $http?>" name='frm_buygiftvoucher' id="frm_buygiftvoucher" class="frm_cls">
				...
				<div class="pro_det_treemenu" align="left"> <ul>
							<li><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a>>></li> <li><?php echo $Captions_arr['GIFT_VOUCHER']['GIFT_VOUCHER_BUY_TREEMENU_TITLE']?></li></ul></div>
...
<?php
          ...
          ?>
            <tr>
    		<td colspan="2" align="center" class="emailfriendtextnormal">
             <table width='100%' cellpadding='0' cellspacing='0' border='0'>
             <tr>
...
<?php		
	...
	?>
		...
		<div class="shelf_1row_content"><?php echo stripslashes($row_prod['product_shortdesc']); 
		if(count($var_arr))
		{
		?>
			...
			foreach ($var_arr as $k=>$v)
			{
				...
				if($db->num_rows($ret_var))
				{
					...
					if($row_var['var_value_exists']==1)
					{
						...
						echo '<input type="hidden" name="var_'.$k.'" value="1"/>';
					echo '</li>';
				}
			}
			?>
