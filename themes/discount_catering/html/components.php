	...
	class components
	{
		...
		function mod_shoppingcart($title)
		{
			...
			{	
				...
				{
				 $items_capt = $Captions_arr['COMMON']['ITEMS_IN_CART'];
				}
				else
				{
				   $items_capt = $Captions_arr['COMMON']['ITEM_IN_CART'];
				}
			?>
            <tr>
            <td align="right" valign="top" class="top_cart_x">
			  ...
			  <tr>
			  	<td><div class="cart_icon_top" onclick="gobackto_cart('<?php echo ($protectedUrl)?base64_encode($ecom_hostname):$ecom_hostname?>')" style="cursor:pointer"></div><a href="#" onclick="gobackto_cart('<?php echo ($protectedUrl)?base64_encode($ecom_hostname):$ecom_hostname?>')" class="cart_view_link" title="<?php echo stripslash_normal($Captions_arr['COMMON']['VIEW_CART'])?>"><div class="top_basket_hdr">Your Basket</div></a><div class="top_basket_txt"> <?php echo  (get_session_var('cart_total_items'))?get_session_var('cart_total_items'):0?>&nbsp;&nbsp;<?php echo $items_capt;?> - <?php echo $cart_tot?></div><div class="checkout_view_link" ><a href="#" class="cart_view_link" onclick="handle_checkout_submit('<?php echo $ecom_hostname?>',0,'<?php echo $pass_tot?>')"  title="<?php echo stripslash_normal($Captions_arr['COMMON']['CHECK_OUT'])?>"><img src="<?php url_site_image('chk-out.png') ?>"  /></a></div></td>
			  </tr>
														...
														<ul  id="main_navigation"> 
															<?php
																...
																while ($row_cat = $db->fetch_array($ret_cat))
																{
																		...
																		if($img_url!='')
																			$img_str = "background: url(".$img_url.")  right top no-repeat;";
																		else
																			$img_str = '';
																...
																?>
																		...
																		<div class="menuadvert_div"><div>&nbsp;<img src="<?php url_site_image('advert.png')?>" align="Advert" /></div></div>
																		</div>
																		</div>
																		</div>	
																<?php		
		...
		?>				
					...
					<form method="post" name="frm_quicksearch" class="frm_cls" action="<?php url_link('search.html')?>">
						...
						<tbody>
						<tr>
							<td class="inputAB"><input name="quick_search" type="text" class="inputAB" id="quick_search"  value="Search Discount Catering Direct" onFocus="if (this.value=='Search Discount Catering Direct') this.value='';" onBlur="if (this.value=='') this.value='Search Discount Catering Direct';"/>
		...
		?>		
				...
				<tr>
					<td align="right" valign="middle" class="logintablecontentright"><?php
			    ...
			    WHERE 
					...
					while($row_keyword=$db->fetch_array($ret_keyword))
					{
						...
						<h2 class="unitline"><?php echo $row_keyword['refine_caption']?></h2>
							...
							<div class="clear_checkbox" id="clear_<?php echo $rf_id ?>" style="display:none;"><input class="clearbut" type="button" id="clearcheck_<?php echo $rf_id ?>" name="clearcheck_<?php echo $rf_id ?>" value="Clear <?php echo $row_keyword['refine_caption'];?>"></div>
						...
						<h2 class="unitline"><?php echo $row_keyword['refine_caption']?></h2>
							...
							<div class="slider"> 
							<div class="clear_slider" id="clear_<?php echo $rf_id ?>" style="display:none;"><input type="button" id="clearslider_<?php echo $rf_id ?>" name="clearslider_<?php echo $rf_id ?>" value="Clear <?php echo $row_keyword['refine_caption'];?>"></div>
							<p>
						...
						<h2 class="unitline"><?php echo $row_keyword['refine_caption']?></h2>
						    ...
						    <div class="clear_color" id="clear_<?php echo $rf_id ?>" style="display:none;"><input class="clearbut" type="button" id="clearcheck_<?php echo $rf_id ?>" name="clearcheck_<?php echo $rf_id ?>" value="Clear <?php echo $row_keyword['refine_caption'];?>"></div>
