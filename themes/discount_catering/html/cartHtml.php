...
<?php echo stripslashes($caption)?>
          ...
          <tr>
						<td colspan="6" align="center" valign="middle" class="red_msg_uk">
							Online Orders Can Only Be Placed If Your Delivery Address Is in the United Kingdom. If Your Delivery Address is outside the United Kingdom <br>please <strong>telephone +44 1922 629009 </strong>for assistance or alternatively email <a href="mailto:info@discountcateringdirect.co.uk">info@discountcateringdirect.co.uk</a> for assistance
...
<div id="uk_alert_msg" class="uk_alert_msg" >
		<div class="closebts_fade" id="closebts_fade" ><img id="close_pop" src="<?php url_site_image('close_pop.png')?>" width="22" height="21" onclick="handle_form_uk_popup('close')"/></div>

As your delivery country is not in the United Kingdom please telephone +44 1922 629009 for assistance or <br>alternatively email <a href="mailto:info@discountcateringdirect.co.uk">info@discountcateringdirect.co.uk</a> for assistance
		</div>		
<div class="fade_uk" id="fade_uk" ></div>
</div>	
...
<script type="text/javascript">
		function validate_checkout_fields(frm)
		{
			...
			?>
				...
				//var contry_id = 23733;
				var contry_id = 20977;//live
				var msg1 = "As your delivery country is not in the United Kingdom please telephone +44 1922 629009 for assistance or alternatively email info@discountcateringdirect.co.uk for assistance";
				var proceed = false;
				if(frm.checkout_country.value == contry_id && frm.checkout_billing_same.value == 'Y')
				{
			       proceed = true;
			    }
			    else if((frm.checkout_country.value==contry_id && frm.checkout_billing_same.value == 'N'))
			    {
				    if(frm.checkoutdelivery_country.value==contry_id)
				    proceed = true;
				    else
				    proceed = false;
				}
				else if((frm.checkout_country.value!=contry_id && frm.checkout_billing_same.value == 'Y'))
			    {
				    proceed = false;
				}
				 else if((frm.checkout_country.value!=contry_id && frm.checkout_billing_same.value == 'N'))
				...
				{
					 /*jquery_popup();
				   document.getElementById('uk_alert_msg').style.display='';
				    document.getElementById('fade_uk').style.display='';
			...
			function handle_form_uk_popup()
			{
			   document.getElementById('uk_alert_msg').style.display='none';
			    document.getElementById('fade_uk').style.display='none';
	...
	$j('#fade_uk').css({'width':maskWidth,'height':maskHeight});
	 
	//transition effect
	$j('#fade_uk').fadeIn(500);
	$j('#fade_uk').fadeTo("slow",0.9); 
...
<?php
			  ...
			  <?php
				...
				?>
				<tr>
						<td colspan="4">			<div class="divider">&nbsp;</div>
						</td></tr>
				<?php
			  }
			...
			*/
			if($ecom_isadword)
			{
				...
				?>
						...
						src="//www.googleadservices.com/pagead/conversion/<?php echo $ecom_adword_conversionid?>/?value=<?=$total_price?>&amp;currency_code=GBP&amp;label=<?php echo $ecom_adword_conversionlabel?>&amp;guid=ON&amp;script=0"/>
						</div>
						</noscript>
			<?php		
