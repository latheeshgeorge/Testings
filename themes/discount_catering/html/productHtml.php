...
function Show_ProductDetails($row_prod)
{
		...
		{
			?>
			<div class="red_msg_new" onclick="this.style.display='none'"> - <?php echo $alert?> - </div>
			<?php
		}
		elseif($_REQUEST['stockalert'])
		{ 
		?>
			<div class="red_msg_new" onclick="this.style.display='none'"> - <?php echo $Captions_arr['PROD_DETAILS'][$_REQUEST['stockalert']]	?> - </div>
		<?php	
		}
		?>
...
<table width="100%" cellpadding="0" cellspacing="0" class="prod_details_tbl_l">
		...
		else
		{
			...
			$j("#zoom_03").elevateZoom({zoomWindowPosition: 1, zoomWindowOffetx: 20,gallery:'gallery_01',easing : true,scrollZoom : false, zoomWindowWidth:350, zoomWindowHeight:350,tint:true, tintColour:'#DCDDE0', tintOpacity:0.5});
...
?>
  ...
  <div class="pdleftdetails" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
...
<?php
										...
										<div class="list_compare_div">
												...
												if($comp_active)  { // to display Compare icon
								...
								while ($row_var = $db->fetch_array($ret_var))
								{
										...
										if ($var_Proceed)// Show the variable if it is valid to show
										{
											...
											<?php
													...
													{				
														?>
														<select name="var_<?php echo $row_var['var_id']?>" <?php echo $onchange_function?> style='float:left'>
														<?php 
														$val_img_set =false;
...
function show_offer_buttons($row_prod)
{
					...
					if($ecom_siteid==106 or $ecom_siteid==104 or  $ecom_siteid==113)
					{
						...
						*/ 
						...
						<div class="pdIconWrap"><div class="insure_logo"></div></div>
						<?php
						}
						?>

						
						<?php /*<div class="pdIconWrap"><div class="eng_del_logo"></div></div>
						<div class="pdIconWrap"><div class="vat_del_logo"></div></div>*/?>
						<?php
					}
					else
					{
					?>
					<div class="logo_wrap">
					<?php /*<div class="pdIconWrap"><div class="eng_del_logo"></div>
...
global $Captions_arr,$showqty,$Settings_arr,$ecom_hostname;
					...
					if ($row_prod['product_show_enquirelink']==1 || $Settings_arr['proddet_showwishlist'])
					{
								...
								if ($Settings_arr['proddet_showwishlist'])
								{
									...
									{
										?>
										<input name="submit_wishlist" type="submit" class="buttonblackbuyw" id="submit_wishlist" value="<?php echo $Captions_arr['PROD_DETAILS']['PRODDET_ADDWISHLIST'];?>" onclick="show_wait_button(this,'Please Wait...');document.frm_proddetails.fpurpose.value='Prod_Addwishlist'" />
										<?php
									}
									else
									{
									...
									?>
										<input name="submit_wishlist" type="button" class="buttonblackbuyw" id="submit_wishlist" value="<?php echo $Captions_arr['PROD_DETAILS']['PRODDET_ADDWISHLIST'];?>" onclick="<?php echo $wishlist_onclick?>" />
