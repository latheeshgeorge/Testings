	     ...
	     if($section_typ=='register' || $section_typ=='enquire')
		 {
		   $show_tprop = false;
		       ...
		       if($show_tprop == true)
	        	...
	        	?>
			  <div >
				<table width="100%"  style="margin-left:12px;" border="0" cellpadding="<?php echo $cellpadding ?>" cellspacing="<?php echo $cellspacing ?>" class="<?php echo $table_class?>">
				<?php	
				}
				if($texttd_class!='')
				...
				?>
						...
						<div class="reg_shlf_hdr_in"><span style="padding-left:6px;">
						<?php echo stripslash_normal($row_dyn['section_name'])?></span>
						</div>
						</td>
						</tr>
				<?php	
				...
				?>
						...
						if($row_elem['mandatory']=='Y' and $row_elem['element_type']!='checkbox' and $row_elem['element_type']!='radio') // If the current field is mandatory move respective name and message to respective array
						{
							...
							$chkout_Req_step[]		= "'".$row_elem['element_name']."'";
							$chkout_Req_Desc_step[]	= "'".$row_elem['error_msg']."'"; 
						}	
						if($row_elem['element_type']=='radio' || $row_elem['element_type']=='checkbox')
						{
						$textcls = " Class = regi_txtfeild_radio";
						}
						else
						{
						$textcls = " Class ='".$texttd_class."'";
					...
					<td colspan="2" >
						<div class="form-group">
							<label><?php echo stripslash_normal($row_elem['element_label'])?></label><br>
						<label class="sr-only" for="form-<?php echo $row_checkout['field_key'];?>"><?php stripslash_normal($row_elem['element_label']); if($row_elem['mandatory']=='Y') { echo '<span class="redtext">*</span>';}?></label>
						<?php
						...
						switch($row_elem['element_type'])
						{
							case 'text':
						?>
								<input class="form-control" type="text" name="<?php echo $row_elem['element_name']?>" id="<?php echo $row_elem['element_name']?>" value="<?=$_REQUEST[$row_elem['element_name']]?>" size="<?php echo $row_elem['element_size']?>" <?php echo ($row_elem['maxlength']>0)?'maxlength="'.$row_elem['maxlength'].'"':''?>/>
						<?php
							break;
							case 'textarea':
						?>
								...
								if ($db->num_rows($ret_check)!=0)// case of no values
								{
									while ($row_check = $db->fetch_array($ret_check))
									{
										if($no>0)
											echo "<br/>";
						?>
										<input class="regiinput" name="<?php echo $row_elem['element_name']?>[]" id="<?php echo $row_elem['element_name']?>[]" type="checkbox" value="<?php echo stripslash_normal($row_check['element_values'])?>" <?php if($_REQUEST[$row_elem['element_name']]==$row_check['element_values']) {echo "checked";} else{ if($row_check['selected']==1){echo "checked";} }?>/>										
						...
						<?php			
							...
							?>
											<input class="regiinput" name="<?php echo $row_elem['element_name']?>[]" id="<?php echo $row_elem['element_name']?>[]" type="radio" value="<?php echo stripslash_normal($row_check['element_values'])?>" <?php if($_REQUEST[$row_elem['element_name']]==$row_check['element_values']) {echo "checked";} else{ if($row_check['selected']==1){echo "checked";} }?>/>											
							<?php		
											echo stripslash_normal($row_check['element_values']);	
											$no++;	
										}
