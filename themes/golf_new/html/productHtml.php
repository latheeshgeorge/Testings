<?php
	...
	class product_Html
	{
		...
		<div class="treemenu_special"><?php echo generate_tree(-1,$_REQUEST['product_id'])?></div>
		...
		?>
		<tr>
		<td colspan="2" class="productdeheader" align="left"><h1><?php echo stripslashes($row_prod['product_name'])?></h1>
		<?php
		...
		?>
			<tr>
				...
				<img src="<?php url_site_image('pro_link_det_left.gif')?>" title="left" alt="left" /></td>
				<td class="productdetdA" align="right">
				<?php
				...
				{  
				?>
					<a href="<?=url_Prod_PDF($_REQUEST['product_id'],$_REQUEST['category_id'],$row_prod['product_name'])?>" class="productdetailslinkD"><?php echo $Captions_arr['PROD_DETAILS']['PRODDET_DOWNLOADPDF'];?></a>
				<?php
				}
				?>
				</td>
		...
		?>
		<tr>
		<td class="productdetmain" valign="top" align="left">
		<div class="pro_det_img_top" >
		<?php
		...
		<td class="productdetd" valign="top">
		...
		<?php
		if ($row_prod['product_variable_in_newrow']==0)
		{
			$var_listed = $this->show_ProductVariables($row_prod,'column');
			
			if($Settings_arr['showsizechart_in_popup']==1 and $sizechart_heading['size_Availvalues']==true) // check whether size chart is set to show in a pop up window
			{
			?>
				...
				<img src="<?php url_site_image('size-chart.gif')?>" border="0"/></a></div>
			<?php
			}
	...
	<?php
		...
		if ($row_prod['product_show_enquirelink']==1)
		{
			...
			{
	?>		<img alt="<?php echo $Captions_arr['PROD_DETAILS']['PRODDET_ENQUIRY'];?>" title="<?php echo $Captions_arr['PROD_DETAILS']['PRODDET_ENQUIRY'];?>" src="<?php url_site_image('add-to-enq.gif')?>" onclick="ajax_addto_cart_fromlist('add_prod_tocart_ajax','Prod_Enquire','frm_proddetails','<?php echo SITE_URL;?>')" />
    <?php
			}
			else
			{
	...
	?>
		...
		?>
			...
			{
			?>
				<a href="javascript:handle_price_promise()"><img src="<?php url_site_image('price-promise.gif')?>" border="0" /></a>
			<?php
			}
		...
		$sr1_arr = array('08000 430 357','08000 430357');
		$rp1_arr = array('','');
		if($row_prod['product_longdesc'])
			$tabs_content_arr[0]	= str_replace($sr1_arr,$rp1_arr,stripslashes($row_prod['product_longdesc']));
		...
		?>
			...
			<tr>
				...
				<?php	echo $tabs_content_arr[$curtab];?><br /><br />
				</td>
			</tr>
		...
		?>
			...
			<tr>
			  <td class="productdownloadheader">
			 <ul class="downloadtab">
			  <li id="var_li" class="downloadtabselected">
			 	 <?php echo $Captions_arr['PROD_DETAILS']['PRODDET_DOWNLOADS'];?>
			  </li>
			  </ul>
			  </td>
			</tr>
	...
	<?php	
		...
		?>
			...
			<?php
				...
				?>
						...
						<h2 class="newshelfBprodname"><input type="checkbox" name="chkproddet_comp_<?php echo $row_prod['product_id']?>" id="chkproddet_comp_<?php echo $row_prod['product_id']?>" value="<?php echo $row_prod['product_id']?>" <?php echo $compare_checked?>/><a href="<?php url_product($row_prod['product_id'],$row_prod['product_name'],-1)?>" title="<?php echo stripslashes($row_prod['product_name'])?>"><?php echo stripslashes($row_prod['product_name'])?></a></h2>
		...
		function show_ProductVariables($row_prod,$pos='column')
		{
		  ...
		  ?>
				...
				<li id="var_li" class="variableselected"><?php echo $Captions_arr['PROD_DETAILS']['PRODDET_PRODVARS']?></li>	
				</ul>
				</div>
				</td>
				</tr>
				...
				<td>
					<table width="100%" border="0" cellpadding="0" cellspacing="0" class="variabletable" id="proddet_var_table">
								  ...
								  <tr>
									...
									<td align="left" valign="middle" class="<?php echo $clss?>">
										...
										if ($row_var['var_value_exists']==1)
										{
											...
											{
												$retfn_arr = get_function_for_variables($_REQUEST['product_id'],$row_var['var_id'],$row_var['var_value_exists']);
												
												$onchange_function      = $retfn_arr['fn'];
												$onclick_var       		= $retfn_arr['var'];
											}
										...
										{
										?>
											<input type="checkbox" name="var_<?php echo $row_var['var_id']?>" id="var_<?php echo $row_var['var_id']?>" value="1" <?php echo ($_REQUEST['var_'.$row_var['var_id']])?'checked="checked"':''?> <?php echo $onchange_function?>/><?php echo Show_Variable_Additional_Price($row_prod,$row_var['var_price'],$vardisp_type)?>
										<?php
										}
										?>
									</td>
						...
						?>
							<script type="text/javascript">
								handle_show_prod_det_addprice('price','<?php echo  SITE_URL?>','<?php echo $_REQUEST['product_id']?>','<?php url_site_image('loading.gif',1)?>');
							</script>
						<?php
						}
						// ##############################################################################
				...
				if($db->num_rows($ret_grps))
				{
					...
					if($grp_nos)
					{
						...
						if($db->num_rows($ret_grp))
						{
							$ret_val = '<tr>
										<td colspan="2">
										<table width="100%" border="0" cellpadding="0" cellspacing="0" class="keyfeaturetable">
										<tr>
										<td colspan="2" class="keyfeature_header">
										<ul class="keyfeatures">
											<li id="var_li" class="keyfeaturesselected">'.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_KEY_FEATURES']).'
											</li>
										</ul>
										</td>
										</tr>';
	...
	<?php
			...
			if($addto_cart_withajax == 1)
			{
	?>		<input name="Submit_buy" type="submit" class="button_buy_det" id="Submit_buy" value="<?php echo $Captions_arr['PROD_DETAILS'][$caption_key];?>" onclick="show_wait_button(this,'Please Wait...');document.frm_proddetails.fpurpose.value='Prod_Addcart';document.frm_proddetails.submit();" />
    <?php
			}
			else
			{
	 ...
	 ?>	
		...
		function show_more_images($row_prod,$exclude_tabid,$exclude_prodid)
		{
				...
				{
					if ($exclude_tabid)
						$prodimg_arr = get_imagelist('prodtab',$_REQUEST['prod_curtab'],$pass_type.',image_bigcategorypath',$exclude_tabid,0);	
				...
				else // case of coming with out tab id, so show the normal image list if any
				{
				 ...
				 if ($show_normalimage==true) // the following is to be done only coming for normal image display
				 {
					...
					{
						if ($exclude_prodid)
							$prodimg_arr = get_imagelist_combination($row_prod['default_comb_id'],'image_bigcategorypath',$exclude_prodid,0);
					}		
					else
					{
						if ($exclude_prodid)
				...
				?>	
						...
						?>
							<a href="<?php url_root_image($v['image_extralargepath'])?>" rel='lightbox[gallery]' title="<?=$title?>"><?php show_image(url_root_image($v['image_bigcategorypath'],1),$title,$title,'preview');?></a>
						<?php
						}
						?>	
						</div>
