<?php
    ...
    function show_combo_product_details($row_prod,$keepselected=false,$disp_type='',$cur_col,$max_col)
	{
		...
		switch($disp_type)
		{
			...
			?>
					<td align="left" valign="middle"class="newshelfBtabletd" onmouseover="this.className='newnormalshelf_hover'" onmouseout="this.className='newshelfBtabletd'" colspan="3">
	...
	function Show_Combo_Multiple($ret_combos,$combprod_id,$deal_mod)
	{
		...
		$cur_cnt = 0;
		?>
		<table width="33%" border="0" cellpadding="0" cellspacing="0"  class="combotable1_middle">
		<?php
		...
		while ($row_combos = $db->fetch_array($ret_combos))
		{
			...
			if($active==1)
			{
				$proceed_date	= validate_component_dates($row_combos['combo_displaystartdate'],$row_combos['combo_displayenddate']);
			}
			else
				$proceed_date	= true;	
			...
			$sql_prod = "SELECT a.product_id,a.product_name,a.product_variablestock_allowed,a.product_show_cartlink,
				...
				if($proceed_combo==true and $proceed_date==true)
				{
				if($cur_cnt == 0)
				echo "<tr>";
				$cur_cnt++;
				?>
					<td class="combocontent_middle" align="center" valign="top">
					<table border="0" cellpadding="0" cellspacing="0" class="shelfBtable1">
					<tr>
						<td align="center" valign="middle">
									<div class="lf_combodeal">
									<?php
									if ($row_combos['combo_name'])
									{
									?>	
									<div class="lf_combodeal_top"><? echo stripslashes($row_combos['combo_name'])?></div>
									<?php
									}
									...
									<a href="<?php url_combo($row_combos['combo_id'],$row_combos['combo_name'],-1)?>" title="<?php echo $Captions_arr['COMMON']['SHOW_DEAL']?>">
									<?php
										// Calling the function to get the type of image to shown for current 
									...
									<?php if($cnt>0 && $cnt%3!=0 && $cnt!=$tot_cnts)
									   { 
									  ?>
									...
									<div class="lf_combodeal_bottom"><a href="<?php url_combo($row_combos['combo_id'],$row_combos['combo_name'],-1)?>" class="lf-combodeal-showall" title="<?php echo $Captions_arr['COMMON']['SHOW_DEAL']?>"><?php echo $Captions_arr['COMMON']['SHOW_DEAL']?></a></div>
									</div>	
						</td>
					</tr>
					</table>
