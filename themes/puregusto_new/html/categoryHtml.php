<?php
	...
	class category_Html
	{
		...
		{ 
			...
			//$best_sell_prodarray = $this->get_bestsellers_prod();
...
?>
				...
				{ 
					...
					<script type="text/javascript">
						...
						$j(document).ready(function(){
							...
							{ 
							   var dataString = '';
							   if($j('#catdet_sortbytop').val())
							   {
								   var sortby = '';
								   switch ($j('#catdet_sortbytop').val())
									{
										case 'custom': // case of order by customer fiekd
											sortby		= 'b.product_order';
										break;
										case 'product_name': // case of order by product name
							   ...
							   var qrystr = "&req=categories&catdet_sortorder="+sortorder+"&catdet_sortby="+sortby+"&catdet_prodperpage="+prodperpage+'&showheader='+showheader+'&'+dataString+'&liststyle='+liststyle;
							   $j('div#last_msg_loader').html('<img src="<?php url_site_image('bigLoader.gif')?>">');
						       $j.post("../../../includes/base_files/categories.php?category_id="+<?php echo $_REQUEST['category_id']?>+"&fromajax_prodautoload=true&catdet_pg="+sp2+qrystr,
								function(data)
								{
									if (data != "") 
									{
		...
		function Show_Subcategories($ret_subcat,$url,$row_cat)
		{
			...
			$subcategory_showimagetype = 'Thumb';//$row_cat['subcategory_showimagetype'];
				...
				<?php
					...
					<?php
							...
							while ($row_subcat = $db->fetch_array($ret_subcat))
							{
								...
								{
									$curcnt = 0;
									$subcat_outer_div_main = 'subcat_outer_div_main_rt';
								}
								else
								{
									$subcat_outer_div_main = 'subcat_outer_div_main';
								}
						?>
								<div class="<?php echo $subcat_outer_div_main?>">
								<div class="subcat_outer_div">
								<div class="subcat_img_div">
				  <?php 
		...
		function get_bestsellers_prod()
		{
			global $db,$Settings_arr,$Captions_arr,$inlineSiteComponents,$compare_button_displayed,$inlineSiteComponents,$ecom_siteid;
			$prod_arr = array();
			...
			if($cat_id)
			{
				$sql_prod = "
				SELECT 
					b.orders_order_id,b.products_product_id,SUM(b.order_qty) as quantity 
				FROM 
				...
				AND 
					a.sites_site_id=$ecom_siteid 
				AND
					a.product_default_category_id = $cat_id 
				GROUP BY b.products_product_id 
				ORDER BY SUM(b.order_qty) DESC LIMIT 10 ";
				$ret_prod  = $db->query($sql_prod);
				while($row_prod = $db->fetch_array($ret_prod))
				{
				 $prod_arr[] = $row_prod['products_product_id'];			  		
				}
				return $prod_arr;
			}
		...
		function Show_Products($ret_prod,$tot_cnt,$start_var,$cat_det,$def_orderfield,$def_orderby,$from_filter=false)
		{
			...
			<tr>
				<td colspan="4" align="left" valign="top" class="shelfAheader_A">
					...
					<tr>
						<td width="20%" height="30" align="right" valign="middle" class="productpagenavtext"><?php echo $Captions_arr['CAT_DETAILS']['CATDET_SORTBY']?>
						...
						<option value="product_name" <?php echo ($prodsort_by=='a.product_name')?'selected="selected"':''?>><?php echo $Captions_arr['CAT_DETAILS']['CATDET_PRODNAME']?></option>
						<option value="price" <?php echo ($prodsort_by=='calc_disc_price')?'selected="selected"':''?>><?php echo $Captions_arr['CAT_DETAILS']['CATDET_PRICE']?></option>
						...
						<input type="button" name="submit_Page" value="<?php echo $Captions_arr['CAT_DETAILS']['CATDET_GO']?>" class="buttonred" onclick="last_msg_funtion(2);<?php /*handle_categorydetailsdropdownval_sel('<?php echo url_category($_REQUEST['category_id'],$cat_det['category_name'],4,$_REQUEST['catgroup_id'],0)?>','catdet_sortbytop','catdet_sortordertop','catdet_prodperpagetop')*/?>" />
						</td>
						...
						<input type="hidden" name="catdet_prodperpagetop" id="catdet_prodperpagetop" value="<?php echo $Settings_arr['product_maxcntperpage'] ?>"/>
			...
			<tr>
				...
				{
					$cat_det['product_displaytype'] = $_REQUEST['liststyle'];
				}
				...
				<table border="0" cellpadding="0" cellspacing="0" class="shelfBtable">
					<tr>
							<td colspan="3"  align="center" class="shelfBtable_td" >
					...
					{
						$disp_best = false;
						if(in_array($row_prod['product_id'],$best_sell_prodarray))
						{
						  $disp_best = true;
						}
					...
					?>
						...
						<?php
							...
							{
							?>
								<div class="icon-list"><img src="<?php url_site_image('best_seller.png')?>" alt="best sell"></div>
							<?php				
							}
						...
						<table border="0" cellpadding="0" cellspacing="0" class="shelfBtable">
                            <tr>
								<td colspan="3" class="" align="center" class="shelfBtable_td">
		...
		function show_RefineProducts_new($ret_refine,$tot_cnt,$start_var,$row_cat,$base_sort_by,$prodsort_order,$from_filter=false)
		{
		?>
		<table border="0" cellpadding="0" cellspacing="0" class="shelfBtable">
		<tr>
			<td align="left" valign="middle" class="shelfBtabletd">
				...
				<div id="last_msg_loader"></div>
				<?php 
			$this->Show_Products($ret_refine,$tot_cnt,$start_var,$row_cat,$base_sort_by,$prodsort_order,$from_filter=false);
				?>
			</td>
		</tr>
		</table>	
		...
		function Show_NomoreProducts()
		{
			global $Captions_arr;
		?>
			<div class="nomoreproducts"><?php echo $Captions_arr['CAT_DETAILS']['NOMORE_PROD_MSG']?></div>
		...
		?>
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="subcategoreytable_home">
				<?php
				if($alert)
				{
				?>
			...
			<?php
				...
				?>
						<td width="33%" align="center" valign="middle" class="subcategoreyimage_td">
	  ...
	  <?php 
		...
		{
	  ?>
		<a href="<?php url_category($row_category['category_id'],$row_category['category_name'],-1)?>" class="subcategoreyimage_new" title="<?php echo stripslashes($row_category['category_name'])?>">
						 <?php
				...
				<?php
					...
					if ($cur_col<$max_col)
						echo '<td colspan="'.($max_col-$cur_col).'" class="subcategoreyimage_new">&nbsp;</td>';
				?>	
				</tr>
			  </table>
				</form>	
