...
function Show_ProductDetails($row_prod)
{
		...
		*/
		...
		function increment_quantity(frm)
		{
			obj = eval('document.'+frm);
			var Qty = parseInt(obj.qty.value);
			if(isNaN(Qty)==false)
			{
				Qty += 1;
				obj.qty.value = Qty;
		...
		function decrement_quantity(frm)
		{
			obj = eval('document.'+frm);
			var Qty = parseInt(obj.qty.value);
			if(isNaN(Qty)==false )
...
";
		echo "
					<script  type=\"text/javascript\" src=\"".url_head_link("images/".$ecom_hostname."/scripts/zoom/jquery-1.8.3.min.js",1)."\" ></script>";

		echo "
					<script  type=\"text/javascript\" src=\"".url_head_link("images/".$ecom_hostname."/scripts/zoom/jquery.elevatezoom.js",1)."\" ></script>";
		echo "
					<script  type=\"text/javascript\" src=\"".url_head_link("images/".$ecom_hostname."/scripts/zoom/jquery.fancybox.pack.js",1)."\" ></script>";
		...
		<input type="hidden" name="pagetype" id="pagetype" value="" />
			...
			?>
			<link rel="stylesheet" href="http://<? echo $ecom_hostname?>/images/<? echo $ecom_hostname?>/scripts/thumbscroll/jquery.mThumbnailScroller.css">
		   ...
		   <div id="content-2" class="content light">
			...
			<script src="http://<? echo $ecom_hostname?>/images/<? echo $ecom_hostname?>/scripts/thumbscroll/jquery.mThumbnailScroller.js"></script>
			<script>
				 jQuery.noConflict();
					var $j = jQuery; 
					(function($j){
            ...
            /*$j("#zoom_03").elevateZoom({gallery:'gallery_01',zoomType:'lens', cursor: 'pointer', galleryActiveClass: 'active',tint:true, tintColour:'#F90', tintOpacity:0.5, imageCrossfade: true, loadingIcon: 'http://www.elevateweb.co.uk/spinner.gif'}); */
           	var zoomConfig = {zoomWindowWidth:460, zoomWindowHeight:510 ,easing : true,constrainType:"height", <?php echo $zoomtype?> containLensZoom: true,constrainSize:460, gallery:'gallery_02', cursor: 'pointer', galleryActiveClass: "active"}; 
			$j("body").on("mouseover", "#zoom_02 ,#gallery_02", function () {
					var zoomImage = $j('img#zoom_02');
				//var url_img = $j(this).attr('data-zoom-image');
				//alert(url_img);
             	//$j('.zoomWindowContainer div').stop().css("background-image","url("+ url_img +")");

				$j("#zoom_02").elevateZoom(zoomConfig); 
				$j("#gallery_02 a").click(function() {
				$j("#zoom_02").css("display", 'none');
				// Remove old instance od EZ
				$j('.zoomContainer').remove();
				$j('.zoomWindowContainer div').remove();
				zoomImage.removeData('elevateZoom');

				// Update source for images
				zoomImage.attr('src', $j(this).data('image'));
				zoomImage.data('zoom-image', $j(this).data('zoom-image'));
				// Reinitialize EZ
				zoomImage.elevateZoom(zoomConfig);
			    

				//$j("#zoom_02").elevateZoom({constrainType:"height", <?php echo $zoomtype?> containLensZoom: true,constrainSize:460, gallery:'gallery_02', cursor: 'pointer', galleryActiveClass: "active"}); 
                
				var myVar = setInterval(function(){ // wait for fading
                
				//$j('.zoomContainer').stop().css("background-color","url("+ url_img +")");

				var width = $j(".detail_big_img_wrap").css("width");
				$j("#zoom_02").css("width", width);
				var height = $j(".detail_big_img_wrap").css("height");
				$j("#zoom_02").css("height", height);
				clearInterval(myVar);
				$j("#zoom_02").css("display", '');
				}, 20);
				}); 
                 
				});

//$j("#zoom_03").elevateZoom({constrainType:"height", constrainSize:274, zoomType: "lens", containLensZoom: true, gallery:'gallery_01', cursor: 'pointer', galleryActiveClass: "active"});
				$j("#zoom_03").elevateZoom({easing : true,constrainType:"height", <?php echo $zoomtype?> containLensZoom: true,constrainSize:460, gallery:'gallery_01', cursor: 'pointer', galleryActiveClass: "active"}); 
				...
				*/
				$j("#gallery_01 a").click(function() {
					$j("#zoom_03").css("display", 'none');

				var myVar = setInterval(function(){ // wait for fading
				var width = $j(".detail_big_img_wrap").css("width");
				$j("#zoom_03").css("width", width);
				var height = $j(".detail_big_img_wrap").css("height");
				$j("#zoom_03").css("height", height);
				clearInterval(myVar);
				$j("#zoom_03").css("display", '');
       ...
       ?>
			<ul class="save-icons">
				...
				*/ 
				...
				{
				?>
				<li><img src="<?php echo url_site_image('price_promise.jpg')?>" width="61" height="73" /></li>
...
function Show_Linked_Product($ret_prod)
{
		...
		$prod_compare_enabled = isProductCompareEnabled();
                    ...
                    <div class="shlf_b_nav"><a onmouseout="stopMe()" onmouseover="scrollDivRight('containerA')" href="#null"><img src="     <?php echo url_site_image('shlfbarw-l.png');?>">
                    </a></div>
                    ...
                    while($row_prod = $db->fetch_array($ret_prod))
                    {
							...
							<div class="<?php echo $wrapmainclass?>" id="catprod_<?php echo $row_prod['product_id']; ?>"  >
							...
							<?php
								...
								?>
								<div id="containers">
								<div class="quickviewclass"><a href=""><img src="<?php url_site_image('quick_view.png')?>" alt="Quick View"></a></div>
                    ...
                    <div class="shlf_b_navA"><a href="#" onmouseover="scrollDivLeft('containerA','<?php echo $div_width?>')" onmouseout="stopMe()"><img src="<?php url_site_image('shlfbarw-r.png')?>"  /></a></div>
                    </div>
					<?php						
...
function show_more_images($row_prod,$exclude_tabid,$exclude_prodid)
{
			...
			<div id="gallery_01">
							...
							*/?>
							<a href="#" data-image="<?php echo url_root_image($v['image_bigpath']);?>" data-zoom-image="<?php echo $extralarge_img?>">
...
<?php
					...
					if($sml_width and $sml_height and $big_width and $big_height)
					{
						...
						$extralarge_img = url_root_image($prodimg_arr[0]['image_extralargepath'],1);
					?>
					<img id="zoom_03" src="<?php echo url_root_image($prodimg_arr[0]['image_bigpath'],1)?>"  data-zoom-image="<?php echo $extralarge_img?>"/>
					...
					if ($caption_key!='') // Check whether the buynow or preorder button is to be displayed
					{
							...
							if($showqty==1)// this decision is made in the main shop settings
							{
								if($row_prod['product_det_qty_type']=='NOR')
								{
									?>
									<li><?php echo $cur_qty_caption?></li>
									<li><img src="<?php echo url_site_image('plus-qty.png')?>" width="20" height="20" /></li>
									<li><input type="text" class="text-field" name="qty" value="<?php echo ($_REQUEST['qty'])?$_REQUEST['qty']:1?>" maxlength="4" /></li>
									<li><img src="<?php echo url_site_image('minus-qty.png')?>" width="20" height="20" /></li>
									<?php
								}
								elseif($row_prod['product_det_qty_type']=='DROP')
								{
