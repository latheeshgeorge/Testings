<?php
	...
	class search_Html
	{
		...
		function Show_Search($search_sql,$tot_cnt,$sql_relate,$from_filter=false)
		{
			...
			<tr>
			<td class="treemenu" align="left"><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a><? if($_REQUEST['quick_search']!=""){?> >> <? echo $_REQUEST['quick_search']; } ?> </td>
			</tr>
			<?php
			if(trim($search_desc)) {
			...
			?>
				<tr>
				<td class="search_topcontent" align="left"><?php echo stripslashes($top_content)?></td>
				</tr>
			<?php
			}
			...
			<tr>
			<td align="left" valign="top" class="shelfAheader_A">
			...
			?>
				<tr>
				<td class="shelfAheader" align="left"><?php echo $cur_title?></td>
				</tr>
			<?php
			}
			?>
			...
			<div id="quickpopup_box"><!-- OUR PopupBox DIV--></div>
			<script type="text/javascript">
				...
				$j(document).ready(function(){
					...
					var quick_search = '<?php echo $_REQUEST['quick_search']?>';
					var dataString = '';
					   if($j('#searchprod_sortbytop').val())
					   {
						   var sortby = '';
						   switch ($j('#searchprod_sortbytop').val())
							{
								case 'product_name': // case of order by product name
									sortby		= 'product_name';
								break;
					   ...
					   else
					   {
						...
						if($j('#searchprod_sortordertop').val())
						{
							var sortorder = $j('#searchprod_sortordertop').val();
						}
						else
						{
					   ...
					   if (document.getElementById('ajax_paging_prevcntr'))
					   {
						...
						var qrystr = "&req=search&search_meth=ord_search&rdo_mainoption=prod&search_sortorder="+sortorder+"&search_sortby="+sortby+"&search_prodperpage="+prodperpage+'&showheader='+showheader+'&liststyle='+liststyle;
						$j('div#last_msg_loader').html('<img src="<?php url_site_image('bigLoader.gif')?>">');
						$j.post("../../../includes/base_files/search.php?quick_search="+quick_search+"&fromajax_prodautoload=true&search_pg="+sp2+qrystr,
						function(data){
							if (data != "") {
				...
				{
					objs = eval('document.getElementById("prodmask_'+id+'")');
...
<?
		...
		function Show_Products($ret_prod,$tot_cnt,$start_var,$cat_det,$def_orderfield,$def_orderby,$from_filter=false)
		{
			...
			$Captions_arr['SEARCH'] 	= getCaptions('SEARCH');
			$showqty	= $Settings_arr['show_qty_box'];// show the qty box
		    $prodsort_by			= ($_REQUEST['search_sortby'])?$_REQUEST['search_sortby']:$def_orderfield;
			$prodperpage			= ($_REQUEST['search_prodperpage'])?$_REQUEST['search_prodperpage']:$Settings_arr['product_maxcntperpage'];// product per page
			$prodsort_order	= ($_REQUEST['search_sortorder'])?$_REQUEST['search_sortorder']:$def_orderby;
			...
			<tr>
				<td colspan="4" align="left" valign="top" class="shelfAheader_A">
					...
					<tr>
						<td width="20%" height="30" align="right" valign="middle" class="productpagenavtext"><?php echo $Captions_arr['SEARCH']['SEARCH_SORT']?>
                          ...
                          <select name="searchprod_sortordertop" id="searchprod_sortordertop">
                            ...
                            <option value="DESC" <?php echo ($prodsort_order=='DESC')?'selected="selected"':''?>><?php echo $Captions_arr['SEARCH']['SEARCH_HIGH2LOW']?></option>
                                                    ...
                                                    <input type="button" name="submit_Page" value="<?php echo $Captions_arr['SEARCH']['SEARCH_GO']?>" class="buttonred" onclick="last_msg_funtion(2)" /></td>
						<td width="32%" align="center" valign="middle" class="productpagenavtext">
							<?php //echo $Captions_arr['SEARCH']['CATDET_ITEMSPERPAGE']?>
						<?php
						if(!$_REQUEST['search_prodperpage'])
						{
							$search_prodperpage = $Settings_arr['product_maxcntperpage'];
						}
						else
						{
						...
						<input type="hidden" name="searchprod_prodperpagetop" id="searchprod_prodperpagetop" value="<?php echo $Settings_arr['product_maxcntperpage'] ?>"/>
			...
			<tr>
				...
				<?php
						...
						?>
						<table border="0" cellpadding="0" cellspacing="0" class="shelfBtable_search">
                            <tr>
							...
							<div id="prodmask_<?php echo $row_prod['product_id']; ?>" class="overlay-box">
		...
		function Show_NomoreProducts()
		{
			...
			$Captions_arr['SEARCH'] 	= getCaptions('SEARCH');
		?>
			<table border="0" cellpadding="0" cellspacing="0" class="nomoreproducts_table">
           	<tr>
        	 	<td align="center" valign="middle" class="nomoreproducts"><?php echo $Captions_arr['SEARCH']['NOMORE_PROD_MSG']?>
			 	</td>
			</tr>	 
			</table>
		<?php	
