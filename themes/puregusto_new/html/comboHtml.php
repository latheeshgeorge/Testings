		...
		?>
			...
			<tr>
			   ...
			   <?php 
					...
					$prod_compare_enabled = 0;//isProductCompareEnabled();
			...
			?>
					...
					if($row_prod['cur_combination'] == $row_prod['tot_combination'])
					{
						...
						if($db->num_rows($ret_comb))
						{
							...
							$sql_allvars = "SELECT distinct a.var_id,a.var_value_id 
												FROM 
													combo_products_variable_combination_map a, product_variables b
												WHERE 
													a.var_id = b.var_id
			...
			$sql_prod = "SELECT a.product_id,a.product_name,a.product_variablestock_allowed,a.product_show_cartlink,
				...
				?>
						...
						<div class="lf_combodeal">
							...
							<div class="lf_combodeal_bottom"> <a href="<?php url_combo($row_combos['combo_id'],$row_combos['combo_name'],-1)?>" class="in-combodeal-showall" title="<?php echo $Captions_arr['COMMON']['SHOW_DET']?>"><?php echo $Captions_arr['COMMON']['SHOW_DET']?></a> </div>
						   </div>
						</td>
					<?
				}	
