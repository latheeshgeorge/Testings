	  ...
	  function wishlist_Showform()
	  {
		...
		<form method="post" name="frm_wishlist" class="frm_cls" action="<?php url_link('wishlist.html')?>">
			...
			<input type="hidden" name="fproduct_id" value="<?=$_REQUEST['fproduct_id']?>" />
			  ...
			  <table width="100%" border="0" cellpadding="2" cellspacing="2" class="shoppingcarttable" >
				<?php
				if($alert)
				{
			?>
			...
			if($db->num_rows($ret_select_wish))
			  ...
			  <td colspan="3">&nbsp;
			   ...
			   <tr>
					<td align="left" valign="middle" class="ordertableheaderAA" ><?php echo $Captions_arr['WISHLIST']['WISHLIST_ITEM']?></td>
					<td align="center" valign="middle" class="ordertableheaderBB"><?php echo $Captions_arr['WISHLIST']['WISHLIST_PRICE']?></td>
					<td align="center" valign="middle" class="ordertableheaderCC"><?php echo $Captions_arr['WISHLIST']['WISHLIST_ACTION']?></td>
		 	  </tr>
		  <?php
				if($db->num_rows($ret_select_wish))
				{  
				...
				// ###################################################################################	
					...
					{
						$vars_exists 	= true;
						$trmainclass		= 'shoppingcartcontent_noborderAA';
						$tdpriceBclass		= 'shoppingcartpriceB_noborderBB';
						$tdpriceAclass		= 'shoppingcartpriceA_noborderCC';
					}
					else
					{
						$trmainclass 		= 'shoppingcartcontentAA';	
						$tdpriceBclass		= 'shoppingcartpriceBB';
						$tdpriceAclass		= 'shoppingcartpriceAA';
					}	
				...
				$prod_sql_det = "SELECT product_id,product_name,product_webprice,product_discount,product_discount_enteredasval,product_bulkdiscount_allowed,
													product_total_preorder_allowed,product_variables_exists,product_variables_exists,product_variablesaddonprice_exists,product_variablecomboprice_allowed,
				...
				?>
				  ...
				  <?php
					...
					<td align="center" valign="middle" class="<?=$tdpriceBclass?>"  width="50%">
					<?php
			  ...
			  if($_REQUEST['fproduct_id']){?>
				...
				<td align="center" valign="middle" class="shoppingcartcontent_noborder">	
				<div class='wish_rightdiv'>
				<?
				if($custom_id) // ** Show the wishlist button only if logged in 
					{
			  ?>
						<input name="submit_wishlist" type="button" class="buttonblackbig_wish" id="submit_wishlist" value="<?php echo $Captions_arr['WISHLIST']['WISHLIST_BUYNOW'];?>"  onclick="return select_products();"  />
			  <?php
					}
					?>
				</div>	
