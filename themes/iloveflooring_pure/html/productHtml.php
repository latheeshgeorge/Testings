	...
	class product_Html
	{
		...
		function Show_ProductDetails($row_prod)
		{
			...
			if(in_array('mod_product_reviews',$inlineSiteComponents)) // Check whether the product review module is there for current site
			{
				...
				{
					$HTML_writerev = '<a href="'.url_link('writeproductreview'.$_REQUEST['product_id'].'.html',1).'" title="'.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_WRITEREVIEW']).'"></a>';
				}
				if($readreview_show == 1 or $writereview_show == 1)
				{
					$HTML_readrev = '<a href="'.url_link('readproductreview'.$_REQUEST['product_id'].'.html',1).'" title="'.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_READREVIEW']).'"> '.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_READREVIEW']).'</a>';
				}
			}
			if($email_show==1)
			{
				$HTML_email = '<a href="'.url_link('emailafriend'.$_REQUEST['product_id'].'.html',1).'" title="'.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_EMAILFRIEND']).'">'.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_EMAILFRIEND']).'</a>';
			}
			if($pdf_show==1) // Check whether the download pdf module is there for current site
			{ 
				$HTML_pdf = '<a href="javascript:download_pdf_common(\''.$_SERVER['HTTP_HOST'].'\',\''.$_SERVER['REQUEST_URI'].'\')" title="'.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_DOWNLOADPDF']).'"> '.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_DOWNLOADPDF']).'</a>';
			}
			$was_price = $cur_price = $sav_price = '';
			$price_arr = show_Price($row_prod,$price_class_arr,'prod_detail',false,5);
			//echo "<pre>";print_r($price_arr);
//			print_r($price_arr['price_with_captions']);
			if($price_arr['prince_without_captions']['discounted_price'])
			{
				...
				elseif($price_arr['price_with_captions']['disc_val'])
					$sav_price = $price_arr['price_with_captions']['disc_val'];
				else
			...
			if($in_combo==1 or $row_prod['product_show_pricepromise']==1)
			{
				...
				if($row_prod['product_show_pricepromise']==1)
				{
					...
					$HTML_promise_buttons .= '<a href="javascript:handle_price_promise()" title="Price Promise" class="priceprom_det_cls"><img src="'.url_site_image('price-promise.png',1).'" border="0"/></a>';
				}
			...
			if($db->num_rows($ret_tab))
			{
				...
				while ($row_tab = $db->fetch_array($ret_tab))
				{
					...
					{
						$tabs_content_arr[$row_tab['tab_id']]	= str_replace('font-size: medium','font-size: 12px',stripslashes($row_tab['tab_content']));
					}
					else
					{
							...
							if($db->num_rows($ret_comm_tab))
			                {
								...
								$tabs_content_arr[$row_tab['tab_id']]	= str_replace('font-size: medium','font-size: 12px',stripslashes($row_comm_tab['tab_content']));
							}
					}
			...
			if($writereview_show == 1 or $readreview_show == 1)
			{
				...
				if($db->num_rows($ret_prodreview))
				{
					...
					{
						$review_stars .='<img src="'.url_site_image('star-greenA.png',1).'"  />';
					}
			...
			{*/
				...
				if($db->num_rows($ret_attach))
				{
					...
					if(!$tab_id and count($tabs_arr)>1)
					{
						//$tab_id = -5;
					}	
				}	
				...
				if(how_displayed!='proddet_ajax_list') /* Code for ajax setting starts here */
				{
					...
					{ //alert(document.getElementById('ajax_div_holder').value);
						...
						{ //alert(document.getElementById('ajax_div_holder').value);
							...
							else if(document.getElementById('shoppingcart_ajax_sidecontainer'))
							{
								if(document.getElementById('ajax_div_holder').value=='shoppingcart_ajax_container')
								{
                ...
                <div class="det_price_otr">                    
                   ...
                   <div class="det_name_topA"><h1>
                    ...
                    <div class="det_name_bottom"></div>
                            ...
                            {
                            ?>
                            <div id="price_holder"><?=$HTML_price?></div>
                            <?php
                            }
                            ?>
                            </div>
						...
						if($enable_special_display==false) // Call special display function
						{
							$this->show_BulkDiscounts($row_prod,array());
								...
								?>
									...
									<li class="email"><?php echo $HTML_readrev; ?></li>
	...
	global $db,$ecomn_siteid,$ecom_hostname,$ecom_themename;	
	   ...
	   if ($_REQUEST['prod_curtab'] and $row_prod['product_variablecombocommon_image_allowed']=='N')
	  {
		...
		if ($show_normalimage)
		{
			...
			{ 
				//echo "return id $just_return_id,$from_ajax_call";
...
function show_more_images($row_prod,$exclude_tabid,$exclude_prodid)
{
	...
	if(count($prodimg_arr)>1)// Case if more than one image assigned to current product
	{
    ...
    <img id="<?php echo $k; ?>" src="<?php echo url_root_image($v['image_iconpath'],1);?>" />
  </a>
					</div>
				</div>
			<?php
...
?>
			...
			<div class='buyBinner_txt_det'><input type="text" class="det_qty_txt_det" name="qty"  value="<?php echo ($_REQUEST['qty'])?$_REQUEST['qty']:1?>" maxlength="2" /></div>
<?php
		}
	...
	{
?>
<div class="buyBinner_link_det"><a href="javascript:void(0);" class="det_buy_link_det" onclick="ajax_addto_cart_fromlist('add_prod_tocart_ajax','Prod_Addcart','frm_proddetails','<?php echo SITE_URL?>')"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS'][$caption_key]);?></a></div>
<?php
	}
	else
	{
?>
<div class="buyBinner_link_det"><a href="#" class="det_buy_link_det" onclick="show_wait_button(this,'Please Wait...');document.frm_proddetails.fpurpose.value='Prod_Addcart';document.frm_proddetails.submit();"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS'][$caption_key]);?></a></div>
<?php
	}
?>
</div>
	...
	if ($row_prod['product_stock_notification_required']=='Y' and $row_prod['product_alloworder_notinstock']=='N')
	{
		...
		if($show_notify) // Check whether notificataion link is to be shown
		{
			...
			?>
				...
				<input type="text"   name="stock_email" class="innerstock_txtbox"/>
			...
			?>	
				...
				<br />
				<span style="font-size:12px;font-weight:normal;color:#000;"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['PRODUCT_OUT_STOCK_EMAIL'])?><br />
