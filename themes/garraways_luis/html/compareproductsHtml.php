			...
			if ($num_products)
			{
					...
					if ($db->num_rows($ret_prod))
					{
							...
							if($db->num_rows($ret_var))
							{
								while ($row_var = $db->fetch_array($ret_var))
								{
									...
									if($db->num_rows($ret_varval))
									{
										...
										{
											$varval_arr[] = stripslash_normal($row_varval['var_value']);
										}
										$varval_str = implode(", ",$varval_arr);
									}
									if ($varval_str!='')
									{
										$var_arr[] = stripslash_normal($row_var['var_name']).': '.$varval_str;
										$var = 1;
									}
								}				
							}
						...
						if($Settings_arr['comp_showbulkdisc']==1)
						{
							...
							if (count($bulkdisc_details['qty']))
							{
								...
								{
								   $bulk_arr[] = $bulkdisc_details['qty'][$j].' '.stripslash_normal($Captions_arr['PROD_DETAILS']['BULK_FOR']).' '. print_price($bulkdisc_details['price'][$j]).' '.stripslash_normal($Captions_arr['PROD_DETAILS']['BULK_EACH']);
								}
						...
						if($Settings_arr['comp_showdesc']==1)
						{
							if($row_prod['product_averagerating'])
							{
								$rate = '';
									...
									{
										$rate .= '<img src="'.url_site_image('star.gif',1).'" border="0" alt="review image" />&nbsp;'; 
									}
						...
						if($Settings_arr['comp_showfreedelivery']==1)
						{					
							if($row_prod['product_freedelivery']==1)
							{
								
								$compare_arr['freedelivery'][] 	= 'Yes';
								$freedelivery = 1;
							}	
							else
							{
								$compare_arr['freedelivery'][]  =  'No';
							}	
						}
					}
				}
				...
				if(count($labelid_arr))
				{
					...
					if ($db->num_rows($ret_label))
					{
						...
						{
							$labelname_arr[$row_label['label_id']] = stripslash_normal($row_label['label_name']);
							$label = 1;
						}
					}
				}						
?>
				...
				<td align="left" valign="middle" width="15%">&nbsp;</td>
						...
						?>
							...
							<div class="prod_comparison_name"><a href="<?php url_product($compare_arr['main'][$i]['product_id'],"'".$compare_arr['main'][$i]['product_name']."'",-1)?>" title="<?php echo stripslash_normal($compare_arr['main'][$i]['product_name'])?>" class="prod_comparison_name"><?php echo stripslash_normal($compare_arr['main'][$i]['product_name'])?></a></div>
					          <div class="comp_imgdiv">
							<a href="<?php url_product($compare_arr['main'][$i]['product_id'],"'".$compare_arr['main'][$i]['product_name']."'",-1)?>" title="<?php echo stripslash_normal($compare_arr['main'][$i]['product_name'])?>">
									<?php 
								//	echo $prod_id_arr[$i];
								...
								<input type="hidden" name="pass_url" value="<?php echo $ecom_selfhttp.$ecom_hostname?>/compare_products.html" />
								<input type="hidden" name="fproduct_url" value="<?php url_product($compare_arr['main'][$i]['product_id'],$compare_arr['main'][$i]['product_name'])?>" />
											<?
												...
												$class_arr['QTY']               = ' ';
												show_addtocart($compare_arr['main'][$i],$class_arr,$frm_name,false,'','','normal_shlfA_pdt_buy_btn');
											?>
											</form>
											</div>
											<div class="normal_shlfA_pdt_info"><?php show_moreinfo($compare_arr['main'][$i],'')?></div>
											</div>
									</td>
						<?php
						}
					  ...
					  ?>
							  <tr>
								<td align="left" valign="middle" class="comparison_mainheadertop"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_COMPPRICE']);?></td>
								  <?php
									for ($i=0;$i<$num_products;$i++)
									{
									...
									?>		
										...
										<?php
												...
												$price_class_arr['normal_class'] 		= 'comparison_normalpriceA';
					  ...
					  ?>
						  <tr>
							<td align="left" valign="middle" class="comparison_mainheaderbottom"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_COMPSTOCK']);?></td>
							 <?php
								for ($i=0;$i<$num_products;$i++)
								{
									$cls = ($i==0)?'comparison_contentA':'comparison_contentB';
					 ...
					 ?> 
						  <tr>					
							<td colspan="<?php echo $num_products+1?>" align="left" valign="middle" class="comparison_sec_header"><div class="reg_shlf_hdr_outrC"><div class="reg_shlf_hdr_in"><span><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_COMPKEYFEATURE']);?></span></div></div></td>
						  </tr>
						  <?php 
						   $cnt_feat = 0;
						   $max_cnt = count($labelname_arr);
						  	foreach ($labelname_arr as $k=>$v)
							{$cnt_feat++;
							if($cnt_feat ==1)
							{
							   $clsc = 'comparison_mainheadertop';
							}
							else if($cnt_feat==$max_cnt)
							{
							  $clsc = 'comparison_mainheaderbottom';
							}
							else
							{
							  $clsc = 'comparison_mainheader';
							}
							
						  ?>
							  <tr>
								<td align="left" valign="middle" class="<?=$clsc?>"><?php echo $v?></td>
								<?php
									for ($i=0;$i<$num_products;$i++)
									{
										$cls = ($i==0)?'comparison_contentA':'comparison_contentB';
										...
										if ($db->num_rows($ret_check))
										{
											...
											if($row_check['is_textbox']==1) // case if type is textbox
											{
												if(trim($row_check['label_value']) != '')
													$curval = stripslash_normal($row_check['label_value']);					
											}	
											else // case if type is dropdown
											{
												...
												if ($db->num_rows($ret_labelval))
												{
													...
													$curval 			= stripslash_normal($row_labelval['label_value']);												
												}
											}	
										}						
											
								?>
										<td align="left" valign="middle" class="<?php echo $cls?>"><?php echo $curval?></td>
								<?php
									}
								?>
							  </tr>
						...
						if($desc==1 or $var==1 or $bulk==1 or $weight ==1 or $shipping ==1 or $manufact==1 or $model==1 or $bonus==1 or review==1 or $freedelivery==1 or $saleicon==1 or $newicon==1)
						{
						?>						  
						  <tr>
							<td colspan="<?php echo $num_products+1?>" align="left" valign="middle" class="comparison_sec_header"><div class="reg_shlf_hdr_outrC"><div class="reg_shlf_hdr_in"><span><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_COMPGENFEATURE']);?></span></div></div></td>
						  </tr>
						  <?php 
						 } 
						  	if($desc==1)
						  ...
						  ?>
							  <tr>
								  <td align="left" valign="middle" class="comparison_mainheadertop"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_COMPDESC']);?></td>
								  <?php
									for ($i=0;$i<$num_products;$i++)
									{
										$cls = ($i==0)?'comparison_contentA':'comparison_contentB';
									?>
										  <td align="left" valign="middle" class="<?php echo $cls?>"><?php echo stripslash_normal($compare_arr['description'][$i])?></td>
									<?php
									}
									?>	  
								</tr>
					  <?php
						...
						?>
							...
							<tr>
							<td align="left" valign="middle" class="comparison_mainheader"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_COMPVAR']);?></td>
							<?php
							for ($i=0;$i<$num_products;$i++)
							{
								$cls = ($i==0)?'comparison_contentA':'comparison_contentB';
					  ...
					  ?>
						   <tr>
							<td align="left" valign="middle" class="comparison_mainheader"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_COMPBULK']);?></td>
							 <?php
								for ($i=0;$i<$num_products;$i++)
								{
									$cls = ($i==0)?'comparison_contentA':'comparison_contentB';
					  ...
					  ?>
					  <tr>
						<td align="left" valign="middle" class="comparison_mainheader"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_COMPWEIGHT']);?></td>
						 <?php
							for ($i=0;$i<$num_products;$i++)
							{
								$cls = ($i==0)?'comparison_contentA':'comparison_contentB';
					  ...
					  ?>
					 <tr>
						<td align="left" valign="middle" class="comparison_mainheader"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_COMPSHIP']);?></td>
						 <?php
							for ($i=0;$i<$num_products;$i++)
							{
								$cls = ($i==0)?'comparison_contentA':'comparison_contentB';
					  ...
					  ?>
						  <tr>
							<td align="left" valign="middle" class="comparison_mainheader"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_COMPMANUFACT']);?></td>
							 <?php
								for ($i=0;$i<$num_products;$i++)
								{
									$cls = ($i==0)?'comparison_contentA':'comparison_contentB';
					  ...
					  ?>
							<tr>
							<td align="left" valign="middle" class="comparison_mainheader"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_COMPMODEL']);?></td>
							 <?php
								for ($i=0;$i<$num_products;$i++)
								{
									$cls = ($i==0)?'comparison_contentA':'comparison_contentB';
					  ...
					  ?>
						   <tr>
							<td align="left" valign="middle" class="comparison_mainheader"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_COMPPOINTS']);?></td>
							 <?php
								for ($i=0;$i<$num_products;$i++)
								{
									$cls = ($i==0)?'comparison_contentA':'comparison_contentB';
					  ...
					  // Check whether the free deliveryte

						if($freedelivery==1)
						{
						?>
					  <tr>
						<td align="left" valign="middle" class="comparison_mainheader"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_FREEDEL_CAPTION']);?></td>
						 <?php
							for ($i=0;$i<$num_products;$i++)
							{
								$cls = ($i==0)?'comparison_contentA':'comparison_contentB';
							?>	
								<td align="left" valign="middle" class="<?php echo $cls?>">
								<?php 
										echo $compare_arr['freedelivery'][$i];
								?>
								</td>
						<?php
							}	
					  ...
					  // Check whether the product review module is active for the site
						...
						?>
					  <tr>
						<td align="left" valign="middle" class="comparison_mainheaderbottom"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_REVIEW_CAPTION']);?></td>
						 <?php
							for ($i=0;$i<$num_products;$i++)
							{
								$cls = ($i==0)?'comparison_contentA':'comparison_contentB';
...
<?php 
	...
	?>
		<table width="100%" border="0" cellpadding="0" cellspacing="1" class="prod_comparison_table">
		<tr>
			<td align="center" valign="middle" class="prod_comparison_name"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_COMPNOPROD']);?></td>
		</tr>
		</table>	
	<?php	
	}
