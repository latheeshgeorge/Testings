...
class search_Html
{
	...
	function Show_Search($search_sql,$tot_cnt,$sql_relate)
	{
		...
		//$search_sql	.=  " AND product_subproduct=0 ";
		...
		if ($db->num_rows($ret_search))
		{
			...
			$HTML_topcontent .= '
								...
								.$HTML_tophiddenfor.$HTML_tophidden.$HTML_paging.'
								</div>';
								...
								$sql_ss = "SELECT product_bestsellericon_show FROM products where product_id = ".$row_prod['product_id']." LIMIT 1";
								$ret_ss = $db->query($sql_ss);
								if ($db->num_rows($ret_ss))
								{
									$row_ss = $db->fetch_array($ret_ss);
									if($row_ss['product_bestsellericon_show']==1)
									{
...
?>
				<form method="post" action="<?php url_link('manage_products.html')?>" name='<?php echo $frm_name?>' id="<?php echo $frm_name?>" class="frm_cls" onsubmit="return product_enterkey(this,<?php echo $row_prod['product_id']?>)">
						...
						?>
									...
									<?php /* <!--<a href="<?php url_product($row_prod['product_id'],$row_prod['product_name'])?>" title="<?php echo stripslashes($row_prod['product_name'])?>"><?php echo stripslash_normal($Captions_arr['COMMON']['COMMON_BUY_NOW'])?></a>--> */?>
