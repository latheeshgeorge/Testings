...
?>
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                ...
                                <tr>
                                    <td colspan="5" align="left" valign="top" class="cart_table_cnta"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                                        ...
                                        foreach ($cartData['products'] as $products_arr)
                                        {
                                            ...
                                            <td class="cart_td_discnt" valign="middle" align="left"> <?php 
                                        ...
                                        if (count($cartData['products'])!=0) // Done to show the message if no products in cart
                                        {
                                            ...
                                            <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                              ...
                                              <!--  <tr>
                                                ...
                                                ?>	
                                                    <tr>
                                                    <td  align="right" colspan="2" valign="middle" class="shoppingcartcontent_delivery" style="position:relative"><?php echo stripslash_normal($Captions_arr['CART']['CART_DELIVERY_LOC'])?><a name="a_deliv">&nbsp;</a> 
                    ...
                    ?>
                        ...
                        if($ecom_siteid != 65) // peterfield golf
                        {
							...
							?>
							<tr>
							<td align="" valign="middle" class="google_or" colspan="2">
								
							<div style="width:100px; float:right;padding-top: 21px;"><img src="<?php echo url_site_image('gateway_or.gif')?>" alt="Or" border="0" /></div>
		...
		function Show_Checkout()
		{
			...
			$replace_chckarr = false;
			if($ecom_siteid==105 or $ecom_siteid==95)
			{
				$str_spl_arr = array(); 
				if($_REQUEST['pret']==1) // case if coming back from PAYPAL with token.
				{
					if($_REQUEST['token'])
					{
						if($address['SHIPTONAME']!='')
						{
							...
							if(count($str_spl_arr)>2)
							{
								$saved_checkoutvals['checkout_fname'] = $str_spl_arr[1];
								$saved_checkoutvals['checkout_surname'] = $str_spl_arr[2];
							}
							else
							{
											...
											$sql_checkout = "SELECT field_det_id,field_key,field_name,field_req,field_error_msg   
														...
														{						
															...
															{		
																...
																if($cust_id)
																{	
																	//if($ecom_siteid==111) // case of puregusto local
																	if($ecom_siteid==105) // case of puregusto
																	{
																		if ($row_checkout['field_key']=='checkout_comp_name')
																		{
																			if(count($saved_checkoutvals))
																			{
																				if(trim($saved_checkoutvals['checkout_comp_name'])=='')
																				{
																					$obt_compname = '';
																					$sql_cnames = "SELECT customer_compname FROM customers WHERE customer_id = $cust_id AND sites_site_id = $ecom_siteid LIMIT 1";
																					$ret_cnames = $db->query($sql_cnames);
																					if($db->num_rows($ret_cnames))
																					{
																						$row_cnames = $db->fetch_array($ret_cnames);
																						$obt_compname = stripslashes($row_cnames['customer_compname']);
																					}
																					$saved_checkoutvals['checkout_comp_name'] = $obt_compname;
																				}
																			}
																		}
																	}
																...
																?>
																	<tr>
																		...
																		{
																			echo str_replace('size="30"','size="30" maxlength="20" ',get_Field_puregusto($row_checkout['field_key'],$saved_checkoutvals,$cartData['customer'],'',$class_array,$disable_field));		
																		}
																		else
																		{
																			echo get_Field_puregusto($row_checkout['field_key'],$saved_checkoutvals,$cartData['customer'],'',$class_array,$disable_field);		
																		}	
																		?>
																		</td>
																	</tr>
									 ...
									 // check whether billing and shipping address can be different in the site.
												  ...
												  ?>
													<tr>
														...
														<select name="<?php echo $selboxname;?>" onchange="handle_deliveryaddress_change(this)" <?php echo $disabled ?>>
													...
													<tr id="checkout_delivery_tr" <?php echo (($saved_checkoutvals['checkout_billing_same']=='Y') or ($saved_checkoutvals['checkout_billing_same']==''))?'style="display:none"':''?>>
														...
														<td colspan="2" align="left" class="checkout_hdr"><?php echo stripslash_normal($Captions_arr['CART']['CART_FILL_DELIVERY_ADDRESS'])?></td>
															...
															?>
																	...
																	<?php
																	$class_array['txtarea_cls'] = 'regiinput';
																	if($row_checkout['field_key']=='checkoutdelivery_fax')
																	{
																		echo str_replace('size="30"','size="30" maxlength="20" ',get_Field_puregusto($row_checkout['field_key'],$saved_checkoutvals,$cartData,'',$class_array,$disable_field));
																	}
																	else
																	{
												...
												<td colspan="6" align="right" valign="top" class="chk_td_price">
													<table width="100%" border="0" cellspacing="0" cellpadding="0">
														...
														if ($deliverydet_Arr['delivery_type'] != 'None') // Check whether any delivery method is selected for the site
														{
															...
															?>	
																		<tr>
																			<td  align="left" valign="top" class="cart_del_charge" colspan="2"><?php echo stripslash_normal($Captions_arr['CART']['CART_DELIVERY_LOC'])?>&nbsp;: &nbsp;<?php echo stripslash_normal($row_loc['location_name'])?></td>
																		</tr>
															<?php
																	}
																}
															...
															?>	
																		<tr>
																			<td  align="left" valign="top" class="cart_del_charge" colspan="2"><?php echo stripslash_normal($Captions_arr['CART']['CART_DELIVERY_OPT'])?>&nbsp;:&nbsp;<?php echo stripslash_normal($row_grp['delivery_group_name'])?></td>
																		</tr>
															<?php
																	}
																}	
										...
										<div class="chk_credit_outr">
											...
											<tr>
												<td  align="left" valign="middle" class="cartterms">			
													...
													{ 
													?>
														<br /><input class="shoppingcart_radio" type="checkbox" name="cart_terms" id="cart_terms" value="1" /><a href="<?php echo $ecom_selfhttp.$ecom_hostname; ?>/https://www.puregusto.co.uk/terms-of-business-pg50174.html" class="terms_link"><?php echo stripslash_normal($Captions_arr['CART']['CART_ACCEPT_TERMS_CONDITIONS'])?></a>
													<?php	
													}
													?>	
												<br /></td>
...
<?php /*?>		<tr>
			...
			*/
			if($ecom_isadword)
			{
				...
				?>
						...
						var tprice = <?php echo $total_price?>;
						if (tprice>0) 
						{
						...
						<img height="1" width="1" style="border-style:none;" alt="" src="//www.googleadservices.com/pagead/conversion/<?php echo $ecom_adword_conversionid?>/?value=<?=$total_price?>&amp;currency_code=GBP&amp;label=<?php echo $ecom_adword_conversionlabel?>&amp;guid=ON&amp;script=0"/>
						</div>
			...
			if ($order_id>0)
			{
				if($ecom_siteid == 105) // puregusto
				{
				?>
					<!-- Facebook Conversion Code for Checkouts - PureGusto -->
					<script>(function() {
					...
					window._fbq.push(['track', '6027113310072', {'value':'<?=$total_price?>','currency':'GBP'}]);
					</script>
					<noscript><img height="1" width="1" alt="" style="display:none" src="https://www.facebook.com/tr?ev=6027113310072&amp;cd[value]=<?=$total_price?>&amp;cd[currency]=GBP&amp;noscript=1" /></noscript>
					
					
					<?php /*
					<script type="text/javascript">
					var sayuClientId = 20549;
					var sayuConversionValue = <?=$total_price?>;
					var sayuTrackingHost = (("https:" == document.location.protocol) ?"https://www." : "http://www.");
					sayuTrackingHost += 'sayutracking.co.uk/conversion.js';document.write(unescape("%3Cscript src='" + sayuTrackingHost + "?client_id=" + sayuClientId + "&cv=" + sayuConversionValue + "' type='text/javascript'%3E%3C/script%3E"));
					</script>
					<noscript>
					<img height="1" width="1" border="0" src="https://www.sayutracking.co.uk/conversion.js?js=0&client_id=20549&cv=<?=$total_price?>" alt="" />
					</noscript>
		...
		?>
			...
			<div align="right" class="carterrormsg_span" ><a href="javascript: hide_instockmsg_div()"><img src="<?php url_site_image('close.gif')?>" border="0" alt="close" /></a></div>
...
function get_CheckoutValues_puregusto($pass_arr=array())
{
	...
	else
	{
		...
		{ 
			...
			if(count($shipname_arr)>2)
			{
				$pass_arr['FIRSTNAME'] = $shipname_arr[1];
				$pass_arr['LASTNAME']  = $shipname_arr[2];
			}
			else
			{
