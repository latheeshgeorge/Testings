<?php
		...
		function show_product_reviews($prod_id)
		{
			...
			<tr>
				...
				<div class="review_namebtn">
					<div class="review_btn"><div><a href="javascript:window.location='<?php url_link('writeproductreview'.$prod_id.'.html')?>'" class="det_buy_link"><?=stripslash_normal($Captions_arr['PRODUCT_REVIEWS']['READ_PRODUCT_ADDREVIEW_LINK'])?></a></div></div>
				</div>
			</tr>
			</table>
			<?php
	...
	function show_pricereload_detail($product_id,$var_arr=array())
	{
		...
		if ($db->num_rows($ret_prod))
		{
			...
			if(count($var_arr)==1)
			{
				      $sql_data_title = "SELECT google_feed_prod_title 
												FROM 
													product_variable_data 
												WHERE 
													var_value_id = ".$row_prod['cur_var_value_id']." 
												ORDER BY var_order  
												LIMIT 
													1";
						$ret_data_title = $db->query($sql_data_title);
						$row_data_title = $db->fetch_array($ret_data_title);
						//echo $row_data_title['google_feed_prod_title'];
						if($row_data_title['google_feed_prod_title']!='')
						{
						  ?>
						  <input type="hidden" name="google_feed_prod_title" id="google_feed_prod_title" value="<?php echo $row_data_title['google_feed_prod_title'];?>">
						  <?php
						}
						else
						{
							?>
						   <input type="hidden" name="google_feed_prod_title" id="google_feed_prod_title" value="<?php echo $row_prod['product_name'];?>">
						  <?php
