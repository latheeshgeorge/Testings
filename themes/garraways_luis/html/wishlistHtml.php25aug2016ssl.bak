		...
		$HTML_treemenu = '<div class="tree_menu_conA">
								  ...
								  <div class="tree_menu_midA">
									...
									<li><a href="'.url_link('',1).'">'.stripslash_normal($Captions_arr['COMMON']['TREE_MENU_HOME_LINK']).'</a></li>
									 <li>'.stripslash_normal($Captions_arr['WISHLIST']['WISHLIST_MAINHEADING']).'</li>
									</ul>
									  </div>
								  </div>
		...
		<form method="post" name="frm_wishlist" class="frm_cls" action="<?php url_link('wishlist.html')?>">
			...
			<div class="reg_shlf_outr">
			  ...
			  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="reg_table" >
				...
				?></td>
				<td align="right" valign="middle" colspan="3" class="">
              <? 
					...
					?>
						<div class="cart_shop_contA"><div>
										<input name="clearenq_button" type="button" class="inner_btn_red" id="clearenq_button" value="<?php echo stripslash_normal($Captions_arr['WISHLIST']['WISHLIST_CLEAR'])?>" onclick="if(confirm_message('<? echo stripslash_javascript($Captions_arr['WISHLIST']['WISHLIST_CLEARALLMSG'])?>')){document.frm_wishlist.wish_mod.value='clear_wishlist';document.frm_wishlist.submit();}" />
						</div></div>
				 ...
				 <tr>
				<td class="ordertableheader" width = "2%">&nbsp;</td>
				<td class="ordertableheader" >&nbsp;</td>
				<td class="ordertableheader"><?php echo stripslash_normal($Captions_arr['WISHLIST']['WISHLIST_ITEM'])?> </td>
				<td class="ordertableheader">&nbsp;</td>
				<td class="ordertableheader" align="center"><?php echo stripslash_normal($Captions_arr['WISHLIST']['WISHLIST_PRICE'])?> </td>
				<td class="ordertableheader" align="center"><?php echo stripslash_normal($Captions_arr['WISHLIST']['WISHLIST_ACTION'])?></td>
				<td class="ordertableheader" width = "2%">&nbsp;</td>
			  </tr>
		  ...
		  <?php
				...
				?>
				  <tr>
				  <td>&nbsp;</td>
				  <td align="left" valign="middle" class="cart_td_delete" width="8%" >
				  <input type="checkbox" name="wishlist_check[]" value="<?=$row_prod_det['product_id']?>" />
				  </td>
				   
					<td align="left" valign="middle" class="cart_td_img" width="28%" >
					<?php 
					...
					<td align="left" valign="middle" class="cart_td_name"  width="30%">
					...
					<td align="center" valign="middle" class="cart_td_price"  width="20%">
					<?php
			  ...
			  ?>
			  	<td align="center" valign="middle" colspan="4" class="">
			  <?
			  if($_REQUEST['fproduct_id']){?>
					<div class="cart_shop_contA"><div>
							<input name="back_Submit" class="inner_btn_red" value="Continue Shopping" type="button"  onclick="window.location='<?php echo $ps_url?>'">
					</div></div>
			 	<?php
				}
				?>
				</td>
				<td align="center" valign="middle" colspan="3" class="">
				<?
				if($custom_id) // ** Show the wishlist button only if logged in 
					{
			  ?>
					<div class="cart_shop_contA"><div>
							<input name="submit_wishlist" type="button" class="inner_btn_red" id="submit_wishlist" value="<?php echo stripslash_normal($Captions_arr['WISHLIST']['WISHLIST_BUYNOW']);?>"  onclick="return select_products();"  />
					</div></div>
			  <?php
					}
					?>
