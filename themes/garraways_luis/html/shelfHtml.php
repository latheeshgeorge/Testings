			...
			if (count($shelf_arr))
			{
				...
				{ 
					...
					if ($proceed)
					{
						...
						if($source_area=='sitemap')
						{
							$shelfData['shelf_currentstyle']= 'sitemap';
						}
						...
						global $special_for_category;
						if($special_for_category)
						{
							$Limit = ' LIMIT 6';
						}	
						...
						{ 
							...
							//echo 'iamhere'.$shelfData['shelf_currentstyle'];
							if($shelfData['shelf_currentstyle']=='sitemap') // case of normal design layout
							{
							?>
							<div class="featured_products" style="line-height:16px">
										<?php
										if ($cur_title)
										{
										?>
										<h4><?php  echo $cur_title?></h4>
										<?php
										}?>
										<?php
										...
										?>	<div class="featured_block">
											...
											{
											?>
                                            	<a href="<?php url_product($row_prod['product_id'],$row_prod['product_name'],-1)?>"  title="<?php echo stripslash_normal($row_prod['product_name'])?>" class="product_descr" style="text-decoration:none;"><span class="grey"><?php echo stripslash_normal($row_prod['product_name'])?></span></a>
											<?php
											}
									...
									<div class="spcl_shlf_showall_otr"> <a href="<?php  url_shelf_all($shelfData['shelf_id'],$shelfData['shelf_name'],-1)?>" class="spcl_shlf_showall" style="text-decoration:none;color:#FFF;" title="<?php echo stripslash_normal($Captions_arr['COMMON']['SHOW_ALL'])?>"><?php echo stripslash_normal($Captions_arr['COMMON']['SHOW_ALL'])?></a> </div>
									</div>
							<?php	
							}
						...
						?>
									...
									?>	<div class="detail_inside">
                                            ...
                                            <?php /*<!--<a href="<?php url_product($row_prod['product_id'],$row_prod['product_name'],-1)?>" title="<?php echo stripslash_normal($row_prod['product_name'])?>" class="cupid-green"><?php echo stripslash_normal($Captions_arr['COMMON']['COMMON_BUY_NOW'])?></a>--> */ ?>
                                            </div>
                                            <?php
											}
											?>
	...
	<?php
								      ...
								      if(isProductCompareEnabled()) {
									   if(in_array($row_prod['product_id'],$_SESSION['compare_products']))
												...
												<?php /* <!-- <div class="pdt_list_pdt_buy">
								   ...
								   ?>            
											<?php /* <!--<div class="pdt_list_pdt_more"><a class="" title="Love Meter T-Shirt Heart Lights" href="<?php url_product($row_prod['product_id'],$row_prod['product_name'])?>">More Info</a></div>--> */?>
												</div>   
                                                      </div>
							...
							{  
									 ...
									 switch($position)
									 {
									  ...
									  <div class="featured_products">
										...
										{
										?>
										<span class='featnewtitle'><?php  echo $cur_title?></span>
										<?php
										}?>
										<?php
										while($row_prod = $db->fetch_array($ret_prod))
										{
										?>	<div class="<?php echo ($special_for_category)?'featured_block_category':'featured_block'?>">
											<?php
											if($shelfData['shelf_showimage']==1) // whether image is to be displayed
											{
											?>
											<div class="featureimgqrap"><a href="<?php url_product($row_prod['product_id'],$row_prod['product_name'],-1)?>" title="<?php echo stripslash_normal(str_replace('&','&amp;',$row_prod['product_name']))?>" class="product_image">
											<?php
														//$pass_type = 'image_thumbcategorypath';
														$pass_type = 'image_thumbpath';
													...
													if(count($img_arr))
													{
														show_image(url_root_image($img_arr[0][$pass_type],1),str_replace('&','&amp;',$row_prod['product_name']),str_replace('&','&amp;',$row_prod['product_name']));
													}
													else
													{
														// calling the function to get the default image
											...
											{
											?>
                                            	<a href="<?php url_product($row_prod['product_id'],$row_prod['product_name'],-1)?>"  title="<?php echo stripslash_normal(str_replace('&','&amp;',$row_prod['product_name']))?>" class="product_descr"><h3><span class="grey"><?php echo stripslash_normal(str_replace('&','&amp;',$row_prod['product_name']))?></span></h3></a>
											<?php
											}
											if($shelfData['shelf_showdescription']==1)// whether title is to be displayed
											{
											...
											<div class="more_info_lu">			
													...
													<a href="<?php url_product($row_prod['product_id'],$row_prod['product_name'])?>" class="det_buy_link_more" title="<?php echo stripslashes(str_replace('&','&amp;',$row_prod['product_name']))?>"><?php echo stripslash_normal($Captions_arr['COMMON']['MORE_INFO'])?></a>
													</div>
                                            <form method="post" action="<?php url_link('manage_products.html')?>" name='<?php echo $frm_name?>' id="<?php echo $frm_name?>" class="frm_cls" onsubmit="return product_enterkey(this,<?php echo $row_prod['product_id']?>)">
                                                <input type="hidden" name="fpurpose" value="" />
                                                <input type="hidden" name="fproduct_id" value="" />
								...
								<table width="100%" border="0" cellspacing="0" cellpadding="0">
								  <tr>									  
									  <?php									  
										...
										if($shelfData['shelf_showtitle']==1)// whether title is to be displayed
										{
											$HTML_title = '<a href="'.url_product($row_prod['product_id'],$row_prod['product_name'],1).'" title="'.stripslash_normal(str_replace('&','&amp;',$row_prod['product_name'])).'">'.stripslash_normal(str_replace('&','&amp;',$row_prod['product_name'])).'</a>';
										}
										if ($shelfData['shelf_showimage']==1)// Check whether description is to be displayed
										{
											$HTML_image ='<a href="'.url_product($row_prod['product_id'],$row_prod['product_name'],1).'" title="'.stripslash_normal($row_prod['product_name']).'">';
