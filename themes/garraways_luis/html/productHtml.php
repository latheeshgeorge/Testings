	...
	class product_Html
	{
		...
		function Show_ProductDetails($row_prod)
		{
			global $Captions_arr,$ecom_siteid,$db,$ecom_hostname,$ecom_themename,$glob_qty_displayed,
					 $ecom_themeid,$default_layout,$inlineSiteComponents,$Settings_arr,$meta_arr,$ecom_twitteraccountId,$site_key;
			...
			$HTML_treemenu = '<div class="tree_menu_prdt"><ul class="tree_menu">'.generate_tree_menu(-1,$_REQUEST['product_id'],'','<li>','</li>').'</ul></div>';
			...
			if($row_prod['product_saleicon_show']==1)
			{
				...
				if($desc!='')
				{
					$HTML_saleicon = '<div class="deat_pdt_sale"><span>'.$desc.'</span></div>';
				}
			}
			if($row_prod['product_newicon_show']==1)
			{
				...
				if($desc!='')
				{
					$HTML_newicon = '<div class="deat_pdt_new"><span>'.$desc.'</span></div>';
				}
			}
			if($favourite_show==1) // Decide whether favorite option is to be displayed
			{
				...
				{ 
					...
					$HTML_fav = '<a href="#" onClick="if(confirm(\''.stripslash_javascript($Captions_arr['PROD_DETAILS']['PRODDET_ADD_CONFIRM']).'\')){document.frm_proddetails.fpurpose.value=\'add_favourite\';document.frm_proddetails.submit();}">'.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_FAVOURITE']).'</a>';
				}
				else
				{
					...
					$HTML_fav = '<a href="#" class="productdetailslink" onClick="if(confirm(\''.stripslash_javascript($Captions_arr['PROD_DETAILS']['PRODDET_REM_CONFIRM']).'\')){document.frm_proddetails.fpurpose.value=\'remove_favourite\';document.frm_proddetails.submit();}">'.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_REMFAVOURITE']).'</a>';
				}
			}
			...
			if($email_show==1)
			{
				$HTML_email = '<a href="'.url_link('emailafriend'.$_REQUEST['product_id'].'.html',1).'" title="'.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_EMAILFRIEND']).'"><img src="'.url_site_image('mail-icon.png',1).'" border="0" /> '.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_EMAILFRIEND']).'</a>';
			}
			if($pdf_show==1) // Check whether the download pdf module is there for current site
			{ 
				$HTML_pdf = '<a href="javascript:download_pdf_stream(\''.$_SERVER['HTTP_HOST'].'\',\''.$_SERVER['REQUEST_URI'].'\')" title="'.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_DOWNLOADPDF']).'"><img src="'.url_site_image('print.png',1).'" border="0" /> '.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_DOWNLOADPDF']).'</a>';
			}
			...
			if($in_combo==1 or $row_prod['product_show_pricepromise']==1)
			{
				...
				if($row_prod['product_show_pricepromise']==1)
				{
					...
					$HTML_promise_buttons .= '<a href="javascript:handle_price_promise()" title="Price Promise"><img src="'.url_site_image('price-promise.jpg',1).'" border="0" alt="Puregusto Price Promise"/></a>';
				}
			...
			{*/
				...
				if($db->num_rows($ret_attach))
				{
					...
					$tabs_content_arr[-5]	= '<div style="cursor:pointer" onclick="'."show_curtab_content('ultab_-5','".$docroot."',".$prodid.",'".$loading_gif."')".'">Click here to view the downloads</div>';
			...
			?>
			<?php/*<script type="text/javascript" src="<?php url_head_link("images/".$ecom_hostname."/scripts/tootip.js")?>"></script>*/?>
						...
						{ //alert(document.getElementById('ajax_div_holder').value);
							...
							if(document.getElementById('ajax_div_holder').value=='price_holder')
							{
								if(document.getElementById('ajax_changed_barcode_display'))
									...
									if(document.getElementById('google_feed_prod_title_prod') && document.getElementById('google_feed_prod_title'))
									{
										if(document.getElementById("google_feed_prod_title").value!='')
										{
											document.getElementById("google_feed_prod_title_prod").innerHTML = document.getElementById("google_feed_prod_title").value;
										}
									}
            ...
            <div class="det_cont_con" itemscope itemtype="https://schema.org/Product">
				<div class="det_otr_lu">
                ...
                <?=$HTML_loading?>
                    <div class="det_sub_imge">
                    	<div class="det_sub_imge_hdr"><img src="<?php url_site_image('zoom.gif')?>" border="0" alt="Click on Image to Zoom"></div>
                    ...
                    <?php /*<ul id="usefull_link_block" class="bordercolor">
                        <li class="email"><?php echo $HTML_email; ?></li>
                        <li class="print"><?php echo $HTML_pdf; ?></li>
                        <!--<li class="print"><?php echo $HTML_readrev; ?></li>
                        <li class="print"><?php echo $HTML_fav; ?></li>-->
                    </ul> */?>
                     <?php
            ...
            <div class="det_name_con">
                ...
                <div class="det_price_otr" >                    
                   ...
                   <div class="det_name_topA"><h1 itemprop="name">
						<span id="google_feed_prod_title_prod"><?php echo stripslash_normal($row_prod['product_name']); ?></span>
						<?php
                    ...
                    <div class="det_name_bottom"></div>
                          ...
                          <?php
								...
								?>
								<div class='req_info_new_button'><img id="data-modal-id" src="<?php url_site_image('request_info.png')?>" alt="Request Information" style="cursor:pointer"></div>
								<script type="text/javascript">
								...
								$ajax_j('#data-modal-id').click(function(e) {
									e.preventDefault();
									open_popupdiv();			
								
								});  

								$ajax_j('#data-modalleave-id').click(function(e) {
									e.preventDefault();
									open_popupdivleave();			
								...
								$ajax_j(".js-modal-close, .modal-overlay").click(function() {
								$ajax_j(".modal-box, .modal-overlay").fadeOut(500, function() {
								$ajax_j(".modal-overlay").remove();
								});

								});

								$ajax_j(window).resize(function() {
								$ajax_j(".modal-box").css({
								top: ($ajax_j(window).height() - $ajax_j(".modal-box").outerHeight()) / 2,
								left: ($ajax_j(window).width() - $ajax_j(".modal-box").outerWidth()) / 2 - 80
								});
								});

								//$ajax_j(window).resize();
								...
								function open_popupdiv(mod)
								{
									...
									$ajax_j("body").append(appendthis);
									$ajax_j(".modal-overlay").fadeTo(500, 0.7);
									
									$ajax_j('#popup1').fadeIn($ajax_j(this).data());
								...
								function open_popupdivleave(mod)
								{
									...
									$ajax_j('#popup2').fadeIn($ajax_j(this).data());
                            ...
                            <div class="deat_pdt_buy_left">
                                ...
                                <div class="deat_pdt_buyA"><?=$HTML_enquiry?></div>
                                 <?php
									...
									{
								?>
                                <div class="deat_pdt_buyA"><?=$HTML_wishlist?></div>
                                <?php
									} 
									if($HTML_fav != "")
									{
								?>
                                <div class="deat_pdt_buyA"><?=$HTML_fav?></div>
                                <?php
									}
								?>
                            </div>
            ...
            if($row_prod['product_longdesc']!='')
                        {
                        ?>
                        <div class="deat_desc_otr">
                        <?php echo stripslashes($row_prod['product_longdesc']);?>
                        </div>
                       <?php
						}
			//echo "tab id - ".$tab_id;
			...
			{		
				...
				$curtab = ($_REQUEST['prod_curtab'])?$_REQUEST['prod_curtab']:$tab_id;
			?>	
			...
			if($curtab==-3 or $curtab==-4 or $curtab==-5)
			{
			?> 
				<script type="text/javascript">
				/* $ajax_j(document).ready(function(){ 	
					show_curtab_content('ultab_<?php echo $curtab?>','<?php echo $docroot ?>','<?php echo $prodid ?>','<?php echo $loading_gif ?>');
				});	*/
				</script>
			<?php
			}
...
function Show_Image_Normal($row_prod,$just_return_id=false,$from_ajax_call=false)
{
	   ...
	   if ($_REQUEST['prod_curtab'] and $row_prod['product_variablecombocommon_image_allowed']=='N')
	  {
		...
		if ($show_normalimage)
		{
			...
			if ($_REQUEST['prodimgdet'])	
				...
				$showonly = 0;
			// Calling the function to get the type of image to shown for current 
			$pass_type = 'image_bigcategorypath';//get_default_imagetype('proddet');
			// Calling the function to get the image to be shown
...
function show_more_images($row_prod,$exclude_tabid,$exclude_prodid)
{
	...
	?>	
		...
		<div id="containerB" class="det_thumbimg_inner">
			...
			<div id="scroller_thumb" style="width:<?php echo (count($prodimg_arr)*100)?>px">
			<?php
		...
		<div class="det_thumbimg_nav"> <a href="#null" onmouseover="scrollDivLeft('containerB',<?php echo (count($prodimg_arr)*150)?>)" onmouseout="stopMe()"><img src="<?php url_site_image('shlf-arw-r.png')?>"></a></div>
		</div>
		</div>	
	<?php
	}
}
function show_ProductVariables($row_prod,$pos='column',$sizechart_heading,$HTML_saleicon='',$HTML_newicon='')
{
	...
	if($var_cnt==1)
	{
		$vardisp_type = 'FULL';//$row_prod['product_variable_display_type']; // take the display type from settings for current product
	}
	else 
  ...
  ?>
        ...
        <?php /* <div class="stock_detail">Stock</div>
        */?>
        
        <div class="var_detail">
                  ...
                  <tr>
					...
					<td align="left" valign="middle" class="varable_table_left">
						...
						if ($row_var['var_value_exists']==1)
						{
							...
							//$vardisp_type = 'NONE';// overriding the additional price display along with variable values
						...
						?>
									...
									<?php
									if($_REQUEST['var_'.$row_var['var_id']]) 
									$val_passid = $_REQUEST['var_'.$row_var['var_id']];
									else if($_REQUEST['tpx'])
									$val_passid = $_REQUEST['tpx'];
...
?>
			...
			<div class='buyBinner_txt'><input type="text" class="det_qty_txt" name="qty"  value="<?php echo ($_REQUEST['qty'])?$_REQUEST['qty']:1?>" maxlength="3" /></div>
<?php
		}
	...
	{
?>
<div class="buyBinner_link_new"><a href="javascript:void(0);" class="det_buy_link_newA" onclick="ajax_addto_cart_fromlist('add_prod_tocart_ajax','Prod_Addcart','frm_proddetails','<?php echo SITE_URL?>')"><img src="<?php url_site_image('add.jpg')?>" alt="Add To Cart"></a>
...
<?php
	...
	{
?>
<div class="buyBinner_link_new"><a href="#" class="det_buy_link_newA" onclick="show_wait_button(this,'Please Wait...');document.frm_proddetails.fpurpose.value='Prod_Addcart';document.frm_proddetails.submit();"><img src="<?php url_site_image('add.jpg')?>" alt="Add To Cart"></a></div>
<div class="det_buy_link_newAicon"><img src="<?php url_site_image('iconsnew.jpg')?>" alt="Pay By Card"></div>
<?php
	}
?>
 ...
 <div class="releated_pdt_con">
		...
		<div class="shlf_a_bottom">
            <div class="shlf_a_outer">
                <div class="shlf_a_nav"><a href="#null" onmouseover="scrollDivRight('containerA')" onmouseout="stopMe()"><img src="<?php url_site_image('shlf-arw-l.png')?>"></a></div>
			...
			<div class="shlf_a_pdt_top"></div>
				...
				<div class="shlf_a_pdt_r">
					...
					<? /*=$HTML_desc*/ ?>
							...
							<div class="shlf_a_pdt_buy1"><a href="<?php url_product($row_prod['product_id'],$row_prod['product_name'],-1)?>" class="det_buy_link"><?php echo stripslash_normal($Captions_arr['COMMON']['COMMON_BUY_NOW'])?></a></div>
								
							</div>
				   </div>
				   <?php
				   /*
					<div class="shlf_a_pdt_more1"><a href="<?php url_product($row_prod['product_id'],$row_prod['product_name'],-1)?>" class="det_buy_link" title="<?php echo stripslash_normal($row_prod['product_name'])?>" class="">More Info</a></div>
					*/?> 
			...
			<?
                ...
                <div class="shlf_a_nav"> <a href="#null" onmouseover="scrollDivLeft('containerA','<?php echo $div_width?>')" onmouseout="stopMe()"><img src="<?php url_site_image('shlf-arw-r.png')?>" /></a></div>
             </div>
        </div>
 </div>
<?php
	...
	if ($row_prod['product_stock_notification_required']=='Y' and $row_prod['product_alloworder_notinstock']=='N')
	{
		...
		if($show_notify) // Check whether notificataion link is to be shown
		{
			...
			<?php	
				...
				{
			?>	
				<div  class="alert_inner"><span><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['PRODUCT_OUT_STOCK'])?>.</span>
				<br />
				<span style="font-size:12px;font-weight:normal;color:#888;"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['PRODUCT_OUT_STOCK_EMAIL'])?><br />
...
?>
		...
		?>
			...
			<div class="donloadsleft"><span><a href="<?php echo $ecom_selfhttp.$ecom_hostname?>/product_download.php?attach_id=<?php echo $row_attach['attachment_id']?>" title="<?php echo $Captions_arr['PROD_DETAILS']['PRODDET_CLICKDOWNLOAD'];?>"><?php echo stripslashes($row_attach['attachment_title'])?></a></span></div>
			</li>
		<?php
        $cnts++;
		}
...
<?php
	...
	?>
	<div class="buyBinner_link"><input name="Submit_buy" type="button" class="det_buy_link" id="Submit_buy" value="<?php echo $Captions_arr['PROD_DETAILS'][$caption_key];?>"	onclick="ajax_addto_cart_fromlist('add_prod_tocart_ajax','Prod_Addcart','frm_proddetails_ajax','<?php echo SITE_URL?>')" /></div>
	<?php
	}
...
function show_ProductVariables_ajax($row_prod,$pos='column',$product_id,$enable_grid=false)
{
				...
				if($enable_grid==false)
					...
					$size_checkval = false;
						...
						{ 
							...
							{
							?>
							<div class="deat_pdt_sizechart_A"><div class="sizechartleft"><div><a href="javascript:showsizechartPopup('<?php echo $row_prod['product_id']?>','<?php echo $ecom_hostname?>','<?php echo $ecom_themename?>')" title="<?php echo stripslash_normal($sizechartmain_title)?>">Delivery Info<?php ?></a></div></div></div>
							<?php	
							}
							}
							if($sizechart_heading['size_Availvalues']==false && $row_prod['product_commonsizechart_link']!='')
							{
							?>
							<div class="deat_pdt_sizechart_A"><div class="sizechartleft"><div><a href="<?=$row_prod['product_commonsizechart_link']?>" title="Info" 	target="<?=$row_prod['produt_common_sizechart_target']?>">Delivery Info<?php ?></a></div></div></div>
							<?
							} 
...
// ** Function to display the additional price for product variables
	function Show_full_Additional_Price($prod_arr,$val_id)
	{
		...
		if($val_id!='')
		{
			// get the variable id for the give value id
			$sql_varget = "SELECT product_variables_var_id,var_addprice FROM product_variable_data WHERE var_value_id = ".$val_id." LIMIT 1";
			$ret_varget = $db->query($sql_varget);
			if($db->num_rows($ret_varget))
			{
				$row_varget = $db->fetch_array($ret_varget);
				$price = $row_varget['var_addprice'];
			}
		}
		...
		if ($PriceSettings_arr['price_variableprice_display']==1 and $prod_arr['product_variablecomboprice_allowed']=='N') // ** Check whether variable price is to be displayed
		{
			...
			{
				if($pric_arr['disc']['disc_percent_exists']==1) // case if discount is to be applied to variable and the discount is in %
				{
					if($pric_arr['disc']['direct_disc']>0)// direct discount
					{
						$price = $price - ($price * ($pric_arr['disc']['direct_disc']/100));
					}
					if($pric_arr['disc']['applicable_disc']>0)// direct discount
					{
						$price = $price - ($price * ($pric_arr['disc']['applicable_disc']/100));
					}
				}
	...
	function show_subproducts($prodid)
	{
		...
		{
		?>
			<table cellpadding="0" cellspacing="0" border="0" class="subproduct_maintable">
			<tr>
