<?php
	...
	class category_Html
	{
		...
		function Show_CategoryDetails($row_cat)
		{
			...
			if ($cat_desc!='' and $longdesc_exists==0)
			{
				$HTML_catdesc = '<div class="cate_content_mid">'.$cat_desc.'</div>';
			}
			elseif($cat_desc!='' and $longdesc_exists==1)
			{
				//$cat_desc = preg_replace('/<p [^>]*></p>/', $cat_desc);
				//$cat_desc = preg_replace('/<h2 [^>]*></h2>/', $cat_desc);
				$cat_desc = preg_replace("/<p[^>]*>[\s|&nbsp;]*<\/p>/", '', $cat_desc);
				$cat_desc = preg_replace("/<h2[^>]*>[\s|&nbsp;]*<\/h2>/", '', $cat_desc);
				
				$cat_shortdesc = nl2br(utf8_encode(stripslash_normal($row_cat['category_shortdescription'])));
				
				//$HTML_catdesc = '<div class="cate_content_mid">'.$cat_shortdesc.'</div>';
				$HTML_catdesc = '<div class="cate_content_mid" id="readmore_text" style="display:block;width:98%;overflow:hidden;">'.$cat_desc.'</div>';
				$HTML_catdesc .= '
				<div id="readmore_cap">Read More [...]</div>
				<style type="text/css">
					#readmore_cap{
						color:#FFF;
		...
		$j(document).ready(function(){
			...
			$j( "#readmore_cap").click(function() {
				if($j("#readmore_text").height()!=70)
				{
					$j( "#readmore_cap").html("Read More [...]");
					$j( "#readmore_text").height("70px");
				}
				else
				{
					$j( "#readmore_cap").html("<< Read Less");
					$j( "#readmore_text").height("100%");
				}
	  ...
	  function download_pdf() 
	  {
		...
		function Show_HomeCatgeory($title,$ret_category) 
		{
			...
			global $ecom_siteid,$db,$ecom_hostname,$Settings_arr,$Captions_arr,$body_dispcompid,$inlineSiteComponents,$disgthm_group_cat_array;
			if(in_array('mod_catimage',$inlineSiteComponents))
			{
				$img_support = true;
			}
		...
		{
			$more_conditions = " AND c.category_id IN ( ".implode(',',$disgthm_group_cat_array).") ";
		}
