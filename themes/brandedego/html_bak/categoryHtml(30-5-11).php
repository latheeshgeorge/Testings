<?php
	...
	class category_Html
	{
		...
		function Show_CategoryDetails($row_cat)
		{
			...
			{
				$HTML_icons .= '<a  href="javascript:download_pdf()" title="'.stripslash_normal($Captions_arr['CAT_DETAILS']['CATDET_DOWNLOADPDF']).'"><img src="'.url_site_image('pdf.gif',1).'" alt="'.stripslash_normal($Captions_arr['CAT_DETAILS']['CATDET_DOWNLOADPDF']).'" border="0" /></a>';
			}
	  ...
	  function download_pdf() 
	  {
    	...
    	<div class="sub_cat_hdr_icon"><?=$HTML_icons?><!--<img src="images/fav.gif" /><img src="images/rss.gif" /><img src="images/pdf.gif" />--></div>
   		</div>
			...
			if($alert)
			{
				...
				switch ($row_cat['category_subcatlistmethod'])
				{
					...
					?>
							...
							<div class="subcat_2row_pdt_name">-->
		...
		function Show_Products($ret_prod,$tot_cnt,$start_var,$cat_det,$def_orderfield,$def_orderby)
		{
			...
			<?php		
				break;
				case '4row': // case of four in a row for normal
					$HTML_comptitle = $HTML_maindesc = $HTML_paging = $HTML_showall = '';
				?>
					...
					?>
						...
						<!--<div class="normal_shlfA_pdt_price"></div>
						<?/*=$HTML_rating;*/?>	
						</div>-->
						</div> 
					<?
