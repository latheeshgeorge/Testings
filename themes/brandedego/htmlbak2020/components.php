...
<ul class="footlink">
								...
								<!--<li>
									<a href="<?php url_link('enquiry.html')?>" class="cart_linklf_txt" title="<?php echo stripslash_normal($Captions_arr['COMMON']['VIEW_ENQUIRY'])?>"><span class="enquiry_bottom"><?php echo stripslash_normal($Captions_arr['COMMON']['VIEW_ENQUIRY'])?></span></a></li>-->
					...
					elseif ($position=='seo-section')
					{
							...
							<?php		
								...
								if ($grpData['group_showfaqlink']==1)
								{
									?>
									<li>
									<a href="<?php url_link('faq.html')?>" class="general_links_div_link" title="<?php echo stripslash_normal($Captions_arr['STATIC_PAGES']['STAT_FAQ'])?>"><?php echo stripslash_normal($Captions_arr['STATIC_PAGES']['STAT_FAQ'])?></a>
									</li>
									<?php			
								}
									...
									if ($db->num_rows($ret_cat))
									{
										...
										if ($grpData['catgroup_show_subcat_indropdown']== 1 and $subcatdropdownsupport)// case if categories are to be shown in list menu
										{
											...
											?>
												...
												<ul id="nav">
												<li><a href="<?php url_link('')?>">HOME</a></li>
											...
											{	
												if($_REQUEST['req']=='')
																	{
																	  $clss_hme = "category_menuA";
																	}
																	else
																	{
																	  $clss_hme = "category_menu";
																	}											
										?>
											<div class="category_con">
												<div class="category_left"></div>
													<div class="category_mid">
														<ul  id="main_navigation"> 
															...
															<li>
																	<h2>
                                                 <a href="<?php url_link('')?>"  title="<?php echo stripslash_normal($Captions_arr['STATIC_PAGES']['STAT_HOME'])?>" class="<?php echo $clss_hme;?>">HOME</a>
                                                 </h2>
                                                 </li>
                                                 <?php
																$center_point = 2;
																...
																while ($row_cat = $db->fetch_array($ret_cat))
																{
																	...
																	?>
																	<li>
																	<h2><a href="<?php url_category($row_cat['category_id'],$row_cat['category_name'],-1)?>" class="<?php echo $clss;?>" title="<?php echo stripslash_normal($row_cat['category_name'])?>"><span><?php echo ucwords(stripslash_normal($row_cat['category_name']));?></span></a></h2>
																<?php
																	// Check whether there exists first level subcategories for current category
																	$sql_subcat = "SELECT category_id,category_name 
																					FROM 
																...
																?>
																			...
																			<ul class="subsubcat_clsA">
																			<li class="cat_subcatA"><h3><a href="<?php url_category($row_subcat['category_id'],$row_subcat['category_name'],-1)?>"><?php echo stripslashes($row_subcat['category_name'])?></a></h3></li>
																			<?php
																				...
																				if($num_subrows)
																				{
																					...
																					{
																						?>
																						<ul class="subsubcat_clsB">		
																						<li><span><a href="<?php url_category($row_subsubcat['category_id'],$row_subsubcat['category_name'],-1)?>"><?php echo stripslashes($row_subsubcat['category_name'])?></a></span></li>

																						<?php
																						// Check whether first level subcategories exists for current subcategory
																						$sql_subsubcat2 = "SELECT category_id,category_name 
																						FROM 
																						product_categories 
																						WHERE 
																						...
																						ORDER BY 
																						category_order";
																						$ret_subsubcat2 = $db->query($sql_subsubcat2);
																						$num_subrows2	= $db->num_rows($ret_subsubcat2);
																						if($num_subrows2)
																						{
																							while ($row_subsubcat2 = $db->fetch_array($ret_subsubcat2))
																							{
																							?>	
																							<li><a href="<?php url_category($row_subsubcat2['category_id'],$row_subsubcat2['category_name'],-1)?>"><?php echo stripslashes($row_subsubcat2['category_name'])?></a></li>
																							<?php
																							}
																						}
																						?>
...
<?php
		...
		function mod_quicksearch($title)
		{
			...
			if($position=='topband1')
			{
				 if($cust_id)
				  {
					  $outer = "search_inner_otrA";
					  $inner = "search_innerA";
				  }
				  else
				  {
				  	  $outer = "search_inner_otr";
					  $inner = "search_inner";
...
<?
		...
		function mod_shoppingcart($title)
		{
            ...
            <div class="<?php echo $btn?>">
				  ...
				  <div class="<?php echo $txt?>"><?php echo stripslash_normal($Captions_arr['COMMON']['ITEMS_IN_CART'])?>  <span><?php echo  (get_session_var('cart_total_items'))?get_session_var('cart_total_items'):0?> </span> <?php echo stripslash_normal($Captions_arr['COMMON']['CART_TOTAL'])?> <span><?php echo $cart_tot?></span></div>
...
<ul>
<li><span class="newstext">Sign up to our newsletter</span></li>
<li>
<span>For exclusive offers, events and news</span>
                      <form name="frm_newsletter" method="post" action="" class="frm_cls" onsubmit="return newsletter_validation(this)">
<input name="newsletter_email" type="text" class="top_news_con_input" id="emailSignupAddress" value="Newsletter Signup" onclick="javascript:document.frm_newsletter.newsletter_email.value=''" onblur="javascript:if(document.frm_newsletter.newsletter_email.value=='') document.frm_newsletter.newsletter_email.value='Newsletter SignUp'"  /> 
<input id="emailSignupSubmit" type="submit" value="Sign-Up">
...
<?php 
  ...
  <?php	
			...
			elseif ($position=='middleband') // show the advert only if position is bottomband
			{
				foreach ($advert_arr as $d=>$k)
					{
						// Do the following only if caching is not enabled or cache does not exists
				  ...
				  switch ($k['advert_type'])
                        {
										...
										<div id="<?php echo $advert_ul_id?>" class="sliderwrapper">
										<?php
										$adv_cnt = 0;
										while ($row_rotate = $db->fetch_array($ret_rotate))
										{
											$adv_cnt++;
											if($row_rotate['rotate_link']!='')
											{
												$onclick = "onclick='window.location=\"".$row_rotate['rotate_link']."\"'";
											}
											else
												$onclick = '';
										?>
										<div class="contentdiv" style="background-image: url('<?php echo url_root_image('rot/img/'.$row_rotate['rotate_image'],1) ?>'); display: block; z-index: 1; opacity: 1.0999999999999999; visibility: visible; background-repeat: no-repeat no-repeat;" <?php echo $onclick?>>
										</div>
										<?php
										}
										?>
										...
										<?php /*?><div id="<?php echo $pid ?>" class="pagination">

										<a href="#" class="toc">&nbsp;</a> <a href="#" class="toc anotherclass">&nbsp;</a> <a href="#" class="toc">&nbsp;</a> 
										<a href="#" class="toc">&nbsp;</a>
										</div><?php */?>
										
										<div id="<?php echo $pid ?>" class="pagination">
										<?php
										for($jj=0;$jj<$adv_cnt;$jj++)
										{
										?>
										<a href="#" class="toc">&nbsp;</a>
										<?php
										...
										id: "<?php echo $advert_ul_id?>",  //id of main slider DIV
