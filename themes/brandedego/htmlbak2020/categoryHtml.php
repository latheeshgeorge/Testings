<?php
	...
	class category_Html
	{
	  ...
	  function download_pdf() 
	  {
		...
		<?=$HTML_treemenu?>
        <div class="sub_cat_hdr_icon" style="float:right"><?=$HTML_icons?></div>
	  	...
	  	<?php
			...
			if($subcatshow)
			{
				...
				if ($cache_exists==false)
				{
					...
					$ret_subcat2 = $db->query($sql_subcat);

					if ($db->num_rows($ret_subcat))
					{
						?>
						<div class="categoryboxWrap">
						<?php
						while ($row_subcat = $db->fetch_array($ret_subcat))
						{
							$sql_subsubcat 	= "SELECT category_id,category_name,category_showimageofproduct,default_catgroup_id,subcategory_showimagetype,category_shortdescription   
								FROM
									product_categories 
								WHERE 
					      ...
					      $ret_subsubcat = $db->query($sql_subsubcat);
                       	  if ($db->num_rows($ret_subsubcat))
							{
								?>
								<div class="sub_cat_hdr_otr">
		<div class="sub_cat_hdr"><?php echo stripslashes($row_subcat['category_name'])?></div>
    	<?php /*<div class="sub_cat_hdr_icon"><?=$HTML_icons?></div>*/?>
   		</div>
								<?php

							$this->Show_Subcategories($ret_subsubcat,$url,$row_cat); // ** Calling the function to show the subcategories
							$subcat_exists = true;
							}
					    }
					    if($subcat_exists == false)
					    {
							?>
								<div class="sub_cat_hdr_otr">
    	...
    	<?php /*<div class="sub_cat_hdr_icon"><?=$HTML_icons?></div>*/?>
						  ...
						  $this->Show_Subcategories($ret_subcat2,$url,$row_cat); // ** Calling the function to show the subcategories
			...
			if($prodshow)// Checks whether products to be shown in the middle area
			{
				...
				{ 
					...
					if($_REQUEST['catdet_prodperpage']=='all')
					{
					  $_REQUEST['catdet_prodperpage'] =$tot_cnt;
					}
					...
					$this->Show_Products($ret_prod,$tot_cnt,$start_var,$row_cat,$base_sort_by,$prodsort_order,$url); // ** Calling function to show the products under current category
				}
				else
				{
		...
		function Show_Products($ret_prod,$tot_cnt,$start_var,$cat_det,$def_orderfield,$def_orderby,$url)
		{
			...
			$HTML_topcontent .= '
								...
								<div class=" page_nav_contBA">
								<a href="'.$url."?".$query_string.'&catdet_prodperpage=all">Show All</a>
								</div>
			...
			<div class="pdt_list_pdt_more"><a class="" title="Love Meter T-Shirt Heart Lights" href="<?php url_product($row_prod['product_id'],$row_prod['product_name'])?>">Details</a></div>
				</div>   
