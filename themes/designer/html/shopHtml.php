			...
			switch($shop_det['shopbrand_product_displaytype'])
			{
				...
				?>
					...
					while($row_prod = $db->fetch_array($ret_prod))
					{
						...
						{
							$HTML_title = '<div class="list_d_pdt_name_new"><a href="'.url_product($row_prod['product_id'],$row_prod['product_name'],1).'" title="'.stripslash_normal($row_prod['product_name']).'">'.stripslash_normal($row_prod['product_name']).'</a></div>';
						}
						...
						{
							$HTML_desc = '<div class="list_d_pdt_des">'.stripslash_normal($row_prod['product_shortdesc']).'</div>';
						}
