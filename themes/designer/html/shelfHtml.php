					...
					if ($proceed)
					{
						...
						{ 
							...
							if($shelfData['shelf_currentstyle']=='nor') // case of normal design layout
							{
                                    ...
                                    ?>
                                            ...
                                            <div class="shlf_a_nav"><a href="#null" onmouseover="scrollDivRight('<?php echo $divid;?>')" onmouseout="stopMe()"><img src="<?php url_site_image('shlf-arw-l.gif')?>" /></a></div>
                                                <div id="<?php echo $divid?>" class="shlf_thumb_outer">
                                                ...
                                                if($row_prod['product_saleicon_show']==1)
												{
													...
													//if($desc!='')
													{
														  $HTML_sale = '<div class="shlf_a_sale"></div>';
													}
												}
												if($row_prod['product_newicon_show']==1)
												{
													$desc = stripslash_normal(trim($row_prod['product_newicon_text']));
														  $HTML_new = '<div class="shlf_a_new"></div>';
												}
												echo $HTML_new ;
												echo $HTML_sale;
												?>
                                                <div class="shlf_a_pdt_top"></div>
                                                    <div class="shlf_a_pdt_mid">
                                                        ...
                                                        <?=$HTML_desc?>
                                                                ...
                                                                <div class="shlf_a_pdt_price"><?=$HTML_price?></div>
                                                                    ...
                                                                    $link 		= url_product($row_prod['product_id'],$row_prod['product_name'],1);s
                                                                    ?>
                                                                    </form>
                                                                    </div>
                                                                </div>
                                                         </div>
                                                        <div class="shlf_a_pdt_r"><?php echo $HTML_image;?></div>
                                                    </div>
                                                    <div class="shlf_a_pdt_bottom">
                                                    <div class="shlf_a_pdt_more"><a href="<?php echo $link?>" title="<?php echo $row_prod['product_name']?>" class=""><img src="<?php url_site_image('moreinfo-shlf.gif')?>" /></a></div></div>
                                                </div>
                                                <?
                                                }
                                                ?>
                                                ...
                                                <div class="shlf_a_nav"> <a href="#null" onmouseover="scrollDivLeft('<?php echo $divid;?>',<?php echo $div_width?>)" onmouseout="stopMe()"><img src="<?php url_site_image('shlf-arw-r.gif')?>" /></a></div>
                                            </div>
                                        </div>
                                    </div>
								<?php		
							...
							elseif($shelfData['shelf_currentstyle']=='listingall') // case of shelf to be displayed in inner pages
							{
								...
								{
									case '2row': // case of three in a row for inner pages
										...
										?>
                                           <?php //echo $HTML_comptitle;?>
											...
											<?
                                                ...
                                                <?=$HTML_title;?>
                                                <div class="list_d_pdt_des"><?php echo $HTML_desc;?></div>
                                                </div>
                                                <div class="list_d_pdt_img"><?=$HTML_image?></div>
                                                <div class="list_d_pdt_buy">
                                                
                                                <div class="list_d_pdt_buy_ba"> 
                                                <div class="list_d_pdt_price"> <?=$HTML_price?></div>
                                                <div class="list_d_pdt_buy_in"><a href="<?php url_product($row_prod['product_id'],$row_prod['product_name'])?>" title="<?php echo stripslashes($row_prod['product_name'])?>"><?php echo stripslash_normal($Captions_arr['COMMON']['COMMON_BUY_NOW'])?></a></div>
                                                </div>
                                                </div>
                                                <?
												...
												{
													echo "<div class='list_d_pdt_offer'>";
												}
													
													if($row_prod['product_freedelivery']==1)
													{
														echo $HTML_freedel = ' <img src="'.url_site_image('freedel.gif',1).'" title="Free Delivery"/>';
													}
													if($row_prod['product_bulkdiscount_allowed']=='Y')
													{
														echo $HTML_bulk = '<img src="'.url_site_image('bulk-d.gif',1).'" title="Multi Buy" />';
													}
												if($row_prod['product_bulkdiscount_allowed']=='Y' || $row_prod['product_freedelivery']==1)
												{
												   echo "</div>";
						...
						<?php
									...
									case '4row': // case of three in a row for normal
										...
										if($cur_title!='')
										{
										   $HTML_comptitle ='  <div class="shlf_c_hdr"> 
											<div class="shlf_chdr_top"> </div>
											<div class="shlf_chdr_bottom"><div class="shlf_chdr_l">'.stripslashes($cur_title).'</div></div>
											</div>';
										}
										$desc = trim($shelfData['shelf_description']);
										if($desc!='' and $desc!='&nbsp;')
										{
											 $desc = stripslashes($desc);
											 $HTML_maindesc = '<div class="shlf_c_des">'.$desc.'</div>';
										}
										if ($tot_cnt>0 and ($_REQUEST['req']!='') and $tot_cnt>$prodperpage)
										{
											$paging 		= paging_footer_advanced($path,$query_string,$tot_cnt,$start_var['pg'],$start_var['pages'],'',$pg_variable,'Products',$pageclass_arr);
									...
									?>
										...
										{ 
											...
											if($cur_col%2==0)
											{
											  $prclass1 	= 'shlf_c_pdt_buy_t';
											  $prclass12 	= 'shlf_c_pdt_buy_b';
											}
											else
											{
											  $prclass1 	= 'shlf_c_pdt_buy_ta';
											  $prclass12 	= 'shlf_c_pdt_buy_ba';
											}	
												?>
                                                    <div class="shlf_c_pdt_otr"> 
                                                        <?php echo $HTML_title;?>
                                                        <div class="<?php echo ($_REQUEST['req']=='')?'shlf_c_pdt_img_home':'shlf_c_pdt_img'?>"> <?php echo $HTML_image;?></div>
                                                        <div class="shlf_c_pdt_buy">
                                                            <div class="<?php echo $prclass1;?>"></div>
                                                            <div class="<?php echo $prclass12;?>"> 
                                                                <div class="shlf_c_pdt_price"><?php echo $HTML_price;?></div>
                                                                <div class="shlf_c_pdt_buy_btn"><a href="<?php url_product($row_prod['product_id'],$row_prod['product_name'])?>" title="<?php echo stripslashes($row_prod['product_name'])?>"><?php echo stripslash_normal($Captions_arr['COMMON']['COMMON_BUY_NOW'])?></a></div>
                                                            </div>
                                                        </div>
                                                    </div>
										<?php 
									...
									switch($shelfData['shelf_displaytype'])
									{
								     case '2row':
									  $HTML_comptitle = $HTML_maindesc = $HTML_paging = $HTML_showall = '';
										if($cur_title!='')
										{
											   $HTML_comptitle ='<div class="shlf_dhdr_l">'.stripslashes($cur_title).'</div>';
									 ...
									 ?>
                                       ...
                                       <div class="shlf_d_hdr"> 
                                        <div class="shlf_dhdr_top"> </div>
                                        <div class="shlf_dhdr_bottom">
											<?php echo  $HTML_comptitle;?>
                                        ...
                                        <?php echo $HTML_maindesc ; 
                                                    ...
                                                    ?>
                                                        ...
                                                        <?=$HTML_title;?>
                                                        <div class="shlf_d_pdt_img"><?=$HTML_image?></div>
                                                        <div class="shlf_d_pdt_buy">
                                                        <div class="shlf_d_pdt_buy_ba"> 
                                                        <div class="shlf_d_pdt_price"> <?=$HTML_price?></div>
									 ...
									 case '4row': 
										...
										if($cur_title!='')
										{
											   $HTML_comptitle ='  <div class="shlf_chdr_l">'.stripslashes($cur_title).'</div>';
										}
										$desc = trim($shelfData['shelf_description']);
										if($desc!='' and $desc!='&nbsp;')
										{
										...
										if($tot_cnt>0) // case of show all link
										{
										   $HTML_showall = "<div class='shlf_chdr_r'><a href='".url_shelf_all($shelfData['shelf_id'],$shelfData['shelf_name'],1)."' title='' class='shlfc-showall'>".$Captions_arr['COMMON']['SHOW_ALL']."</a></div>";
										}
									?>
                                        <div class="shlf_c_con"> 
                                            <div class="shlf_c_hdr"> 
                                                <div class="shlf_chdr_top"> </div>
                                                <div class="shlf_chdr_bottom">
                                                    <?php echo $HTML_comptitle;?>
                                                    <?php echo $HTML_showall;?>
                                                </div>
                                            </div>
