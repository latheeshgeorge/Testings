									...
									if ($db->num_rows($ret_cat))
									{
										...
										?>
                                           ...
                                           <?php
													...
													?>
													<li>
													<a href="<?php url_category($row_cat['category_id'],$row_cat['category_name'],-1)?>" class="cate_mid_ul_a" title="<?php echo stripslash_normal($row_cat['category_name'])?>"><span><?php echo ucwords(stripslash_normal($row_cat['category_name']));?></span></a>
													</li>
													<?php
													}
													?>
...
<?php
		...
		function mod_customerlogin($title)
		{
			...
			if (!$cust_id) // case customer is not logged in
			{
				...
				?>
                    <div class="email_otr">
                    <form name="frm_custlogin_top" id="frm_custlogin_top" method="post" action=""  class="frm_cls">
                        <div class="email_div"><input name="custlogin_uname" id="custlogin_uname" type="text"  class="email_input"/></div>
                        <div class="pass_div"><input name="custlogin_pass" id="custlogin_pass" type="password"  class="pass_input"/></div>
                        <div class="email_btn"> <!--<img src="<?php /*url_site_image('login.gif')*/?>" />-->
                       <a href="javascript:validate_login_top(document.frm_custlogin_top)"> 
                       <img src="<? url_site_image('login.gif')?>"  />
                       </a>
                        </div>
                        <input type="hidden" name="redirect_back" value="0" />
                        <input type="hidden" name="custologin_Submit" value="1" />
					</form>	
			 		</div>
<?
				}
...
<?
		...
		function mod_bestsellers($ret_main,$title,$display_id)
		{
			...
			{	
				if($ecom_siteid == 88) // only for skatesrus
				{
					/*
				?>
				<div class="category_lf_con">
                <a class="twitter-timeline"  href="https://twitter.com/skatesrus"  data-widget-id="307121011877351425">Tweets by @skatesrus</a>
				...
				*/ 
				...
				<?php
				/*include('includes/mytwit.inc.php');
				$twitter = new myTwit();
				$twitter->user = 'skatesrus'; 
				$twitter->cacheFile = ORG_DOCROOT.'/images/'.$ecom_hostname.'/twitter/twitcache.txt';
				$twitter->postLimit = 10;
				$twitter->initMyTwit();
				print $twitter->myTwitData;*/
				require('includes/mytwit.inc.php');
				$tFeed = new MyTwit;
				$tFeed->TwitterUser 						= 'skatesrus';
				$tFeed->cacheFile 							= ORG_DOCROOT.'/images/'.$ecom_hostname.'/twitter/twitcache.txt';
				$tFeed->TWITTER_CONSUMER_KEY 				= '3R6zjVGIWonWiat0e3Nw';
				$tFeed->TWITTER_CONSUMER_SECRET 			= 'tSODI98KH4qt8vAIq0S0RlqKowbJ0UtYN2WGx46Yk';
				$tFeed->TWITTER_OAUTH_ACCESS_TOKEN 			= '165758697-P2qB93VX8m7nGqos7egWiuaO0AQUFrHWLXQfk1fJ';
				$tFeed->TWITTER_OAUTH_ACCESS_TOKEN_SECRET 	= 'tp9mppvJgTk8QchsmMQZOGFU4lxZd9UfGYbKaM4N8';
				$tFeed->PostLimit 						= 20;
				...
				if ($tFeed->ErrorMessage) {
					...
					<ul class="MyTweets">';
					$srs_arr = array('Â£','Â','£','â€','â€');
					$rps_arr = array('','','&pound;','','');
					foreach ($tFeed->Tweets as $tweet) {
						echo '<li>'.
							str_replace($srs_arr,$rps_arr,$tweet['MyText']).' 
							</li>';
					}
					echo '</ul>';
				}
				...
				<div class="category_lf_con">
				<iframe src="//www.facebook.com/plugins/like.php?href=http%3A%2F%2Fwww.skatesrus.co.uk&amp;send=false&amp;layout=standard&amp;width=250&amp;show_faces=true&amp;font&amp;colorscheme=light&amp;action=like&amp;height=60" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:250px; height:60px;" allowTransparency="true"></iframe>
				</div>
				<?php
				}
                      ...
                      <div class="newsletter_bottom">
                        <div class="newsletter_bottom_otr">
                            ...
                            <form name="frm_newsletter" method="post" action="" class="frm_cls" onsubmit="return newsletter_validation(this)">
                                <div class="newsletter_input"> <input name="newsletter_email" type="text" class="newsletter_input_in" id="newsletter_email" onclick="javascript:document.frm_newsletter.newsletter_email.value=''"  /></div>
						...
						if($cache_exists!=true)
						{
							...
							{	
                                ...
                                <div class="shp_brnd_thumbimg_con">
                                  ...
                                  <div class="shp_brnd_nav"><a href="#" onmouseover="scrollDivLeft('<?=$divid?>',<?php echo $div_width?>)" onmouseout="stopMe()" o=""><img src="<?php url_site_image('shop-arrow-r.gif')?>"></a>
                                  </div>
                                </div>
                                </div>
                                </div>
...
<?php
			...
			elseif ($position=='topband') // show the advert only if position is topband
			{
				foreach ($advert_arr as $d=>$k)
					{
						// Do the following only if caching is not enabled or cache does not exists
