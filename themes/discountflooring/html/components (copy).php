	...
	class components
	{
		...
		function mod_shoppingcart($title)
		{
            ...
            <td align="right" valign="top" class="top_cart_x">
			  ...
			  <tr>
			  				<td><a href="#" class="cart_view_link" onclick="handle_checkout_submit('<?php echo $ecom_hostname?>',0,'<?php echo $pass_tot?>')"  title="<?php echo stripslash_normal($Captions_arr['COMMON']['CHECK_OUT'])?>"><img src="<?php url_site_image('chk-out.png') ?>"  /></a></td>
...
?> 
 ...
 $j(".div_clearcheck").children().each(function(n, i) {
  ...
  var starta;
  var enda;
  var minv = parseInt($j("#Minrange_"+ids).val());
  var maxv = parseInt($j("#Maxrange_"+ids).val());
...
function clearallslider(id)
{
			...
			if(!isNaN(ids))
              ...
              var suffix   =  $j("#Suffix_"+ids).val();
			   ...
			   $j("#Minrange_"+ids).val($j('#minprice').val());
				$j("#Maxrange_"+ids).val($j('#maxprice').val());
				$j( "#amount_"+ids ).val(prefix+ $j( "#slidervrange_"+ids ).slider( "values", 0 )+suffix +
            " - "+prefix+ $j( "#slidervrange_"+ids ).slider( "values", 1 )+suffix );
		   $j("#clear_"+ids).hide();
				}
			    ...
			    WHERE 
					...
					while($row_keyword=$db->fetch_array($ret_keyword))
					{
						...
						<h2 class="unitline"><?php echo $row_keyword['refine_caption']?></h2>
							...
							<input type="hidden" id="Minrange_<?php echo $rf_id ?>" name="Minrange_<?php echo $rf_id ?>" value="<?php echo $lowval?>" />
							<input type="hidden" id="Maxrange_<?php echo $rf_id ?>" name="Maxrange_<?php echo $rf_id ?>" value="<?php echo $highval?>" />
