...
<table width="100%" cellpadding="0" cellspacing="0" class="prod_details_tbl_l">
		...
		else
		{
			...
			{
			?><link rel="stylesheet" href="http://<?php echo $ecom_hostname?>/images/<?php echo $ecom_hostname?>/css/magnific-popup/magnific-popup.css"> 
				<script src="//ajax.googleapis.com/ajax/libs/jquery/1.9.1/jquery.min.js"></script> 
			<script type="text/javascript"	src="http://<?php echo $ecom_hostname?>/images/<?php echo $ecom_hostname?>/scripts/magnific-popup/jquery.magnific-popup.js"></script>
			<script type="text/javascript">
			jQuery.noConflict();
			var $j = jQuery; 
			...
			preload: [0,1] // Will preload 0 - before current, and 1 after the current image
				...
				<div class="prod_det_img" id="mainimage_holder">
...
?></div>
<script type="text/javascript">
			/*T$('testclick1').onclick = function(){TINY.box.show('http://<?php echo $ecom_hostname?>/images/<?php echo $ecom_hostname?>/otherfiles/vat-relief.html',1,600,400,1)}*/
		</script>
...
<?php
	...
	if($row_prod['product_newicon_show']==1)
		...
		{
			/*if($this->do_show_calculator($_REQUEST['product_id'])) // only for floorings online
					{    $hide_qty = true;
...
<?php
		...
		if(!$button_displayed )
		{
			$button_displayed=$this->show_buttons($row_prod,$hide_qty,$var_count);
		}	
...
$review_first = '';
$sql_prodreview	= "SELECT review_id,review_author,review_rating,review_details 
									FROM  
										 product_reviews 
									WHERE  
										sites_site_id = $ecom_siteid
...
if($db->num_rows($ret_prodreview))
{
	...
	$review_author = '<div class="review_auth_name">'.stripslashes($row_prodreview['review_author']).'</div>';
  ?>
 ...
 {
	$review_stars = '';	
	$review_first = stripslash_normal($Captions_arr['PRODUCT_REVIEWS']['NO_PRODUCT_DETREVIEW_FOUND']);
 }	
...
function show_ProductVariables($row_prod,$pos='column')
{
				...
				?>
				<script type="text/javascript">
				function handle_show_value_link(prodid,docroot,var_id,loading_gif)
				{ 	
					var varstr 	= '';
					var valid	= '';
					var qrystr = '';
					...
					document.getElementById('variable_linka').innerHTML = '<img src="<?php echo url_site_image('proddet_loading.gif',1)?>" alt="loading..." width="43px" height="11px;align:left">';
...
?>
								...
								while ($row_msg = $db->fetch_array($ret_msg))
									{
										...
										{
											if($this->do_show_calculator($_REQUEST['product_id'])) // case of flooringonline website
											{
											...
											?>
											<input type="text" name="varmsg_<?php echo $row_msg['message_id']?>" id="varmsg_<?php echo $row_msg['message_id']?>" value="<?php echo ($_REQUEST['varmsg_'.$row_msg['message_id']]!='')?$_REQUEST['varmsg_'.$row_msg['message_id']]:1?>" size="5" <?php echo $onchange_function?>" /> Mts &nbsp;&nbsp;&nbsp;<input type="button" name="refreshbutton" value="Refresh" style="background-color:#E97B15;color:#FFF;font-weight:bold;">
											<?php
										}
										else
										{
						...
						if($this->do_show_calculator($_REQUEST['product_id']))
						{
							if($showonchangeformsg)
							{
...
// ** Function to display the variables. This is written as function to avoid code repetation in various if conditions
function show_ProductVariables_special($row_prod,$pos='column',$mod='')
{
...
?>
						...
						if ($db->num_rows($ret_var) or $db->num_rows($ret_msg))
								...
								while ($row_var = $db->fetch_array($ret_var))
								{
									...
									?>	
											<tr>
											<td align="left" valign="top" class="productvariabletdB" colspan="2"><?php //echo $cur_qty_caption; :?>  </td>
											</tr>
											<tr>
											<td align="left" valign="middle" class="productvariabletdB" colspan="2"> 
											<?php
											//if($var_cnts_sp==0)
												$this->show_special_qty_box($row_prod,$mod);
											$var_cnts_sp++;	
											?>
											</td>
											</tr>
											...
											$var_cnts_sp++;
													...
													else
													{
														...
														{ 
															...
															if ($db->num_rows($ret_varaddprice_check))
															{
																$onchange_function      = " onchange='handle_normal_price_reload(\"".SITE_URL."\",\"".$_REQUEST['product_id']."\",\"".url_site_image('loading.gif',1)."\",\"".$mod."\")'";
																$onclick_var       		= '';
															}
															else
															{
...
function show_buttons($row_prod,$hide_qty=false,$var_count)
{
global $Captions_arr,$showqty,$Settings_arr,$ecom_hostname;
					...
					if ($caption_key!='') // Check whether the buynow or preorder button is to be displayed
					{
							...
							if($showqty==1 and $hide_qty!=true)// this decision is made in the main shop settings
							{
								if($row_prod['product_det_qty_type']=='NOR')
								{
									...
									<td align="left" valign="middle" class="details_buy_b"><input type="text" class="quainput_big" name="qty" value="<?php echo ($_REQUEST['qty'])?$_REQUEST['qty']:1?>" maxlength="4" <?php echo $onchange_function ?>/>
									</td>
									<?php
								}
					...
					if ($row_prod['product_show_enquirelink']==1 || $Settings_arr['proddet_showwishlist'])
					{
								...
								<?php
								if($this->do_show_calculator() and $var_count>0)
								{ 
									if($row_prod['product_variablecomboprice_allowed'] =='N' and  $row_prod['product_variablecombocommon_image_allowed']=='N')// and $row_prod['product_variablestock_allowed']=='N')
									{
...
function show_more_images($row_prod,$exclude_tabid,$exclude_prodid)
{
					...
					if(count($prodimg_arr)>0)// Case if more than one image assigned to current product
					{
						...
						{ 
							...
							*/?>
							<a href="<?php echo $extralarge_img?>" title="<?php echo $title;?>">
    <img id="<?php echo $k; ?>" src="<?php echo url_root_image($v['image_thumbcategorypath'],1);?>" title="<?php echo $title;?>" />
  </a>
							</li>
							<?php
						}
...
function Show_Image_Normal($row_prod)
					...
					*/ 
					...
					title="<?php echo $row_prod['product_name']?>" >
					<img id="zoom_03" src="<?php echo url_root_image($prodimg_arr[0]['image_bigpath'],1)?>" />
</a>
...
function show_buttons_ajax($row_prod,$hide_qty=false)
					{
					global $Captions_arr,$showqty,$Settings_arr,$ecom_hostname;
					$addto_cart_withajax =  $settings_arr['enable_ajax_in_site'];
					$cust_id 	= get_session_var("ecom_login_customer");
					...
					if($showqty==1 and $hide_qty!=true)// this decision is made in the main shop settings
					{
						if($this->do_show_calculator($row_prod['product_id']))
						{
							if($row_prod['product_variablecomboprice_allowed'] =='N' and  $row_prod['product_variablecombocommon_image_allowed']=='N')// and $row_prod['product_variablestock_allowed']=='N')
								$onchange_function      = " onChange='handle_normal_price_reload(\"".SITE_URL."\",\"".$row_prod['product_id']."\",\"".url_site_image('loading.gif',1)."\",\"ajax\")'";
							else
								$onchange_function		= "";
						}	
					if($row_prod['product_det_qty_type']=='NOR')
					...
					<div class="p_content_buy_B"><input type="text" class="quainput_big_ajax" name="qty" value="<?php echo ($_REQUEST['qty'])?$_REQUEST['qty']:1?>" maxlength="4"  <?php echo $onchange_function ?>/>
					</div>
					<?php
					}
					elseif($row_prod['product_det_qty_type']=='DROP')
...
function show_ProductVariables_ajax($row_prod,$pos='column',$product_id)
{
					...
					if ($row_msg['message_type']=='TXTBX')
					{
						if($this->do_show_calculator($_REQUEST['product_id'])) // case of flooringonline website
						{
							//if ($showonchangeformsg)
							//{
								$onchange_function      = " onchange='handle_normal_price_reload(\"".SITE_URL."\",\"".$_REQUEST['product_id']."\",\"".url_site_image('loading.gif',1)."\",\"ajax\")'";
								$onclick_function      = " onclick='handle_normal_price_reload(\"".SITE_URL."\",\"".$_REQUEST['product_id']."\",\"".url_site_image('loading.gif',1)."\",\"ajax\")'";
								$onclick_var       		= '';
					...
					?>
					<input type="text" name="varmsg_<?php echo $row_msg['message_id']?>" id="varmsg_<?php echo $row_msg['message_id']?>" value="<?php echo ($_REQUEST['varmsg_'.$row_msg['message_id']]!='')?$_REQUEST['varmsg_'.$row_msg['message_id']]:1?>" size="5" <?php echo $onchange_function?>" /> Mts &nbsp;&nbsp;&nbsp;<input type="button" name="refreshbutton" value="Refresh" style="background-color:#E97B15;color:#FFF;font-weight:bold;" <?php echo $onclick_function?>>
					<?php
					}
					else
					{
...
function show_special_qty_box($row_prod,$mod,$normal=false)
{
	...
	if($normal==false and $this->do_show_calculator($_REQUEST['product_id']))
	{
		if($row_prod['product_variablecomboprice_allowed'] =='N' and  $row_prod['product_variablecombocommon_image_allowed']=='N')// and $row_prod['product_variablestock_allowed']=='N')
			$onchange_function      = " onChange='handle_normal_price_reload(\"".SITE_URL."\",\"".$_REQUEST['product_id']."\",\"".url_site_image('loading.gif',1)."\",\"".$mod."\")'";
		else
	...
	if($showqty==1)// this decision is made in the main shop settings
	{
		...
		{
?>
<?php if($normal==true) { echo  $cur_qty_caption;}?>  <span class="refresh_txt"><? if ($normal==false) { echo $cur_qty_caption?></span></br>  <?php  /*X*/?> <?php }?><span class="qty_pro"><input type="text" class="quainput_big" name="qty"  value="<?php echo ($_REQUEST['qty'])?$_REQUEST['qty']:1?>" maxlength="4" <?php echo $onchange_function?> />  Mts &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span><span class="refresh_pro"><input type="button" name="refs" value="Refresh" style="background-color:#E33737;color:#FFF;"/></span>
<?php
		}
		elseif($row_prod['product_det_qty_type']=='DROP')
		{
...
function do_show_calculator($prodid=0)
{
	global $db,$ecom_siteid;
	$show_calc_return = false;
	// Check whether a variable message is set for this product, if yes then return true.
	$sql_msg = "SELECT message_id 
					FROM 
						product_variable_messages 
					WHERE 
						products_product_id = $prodid 
						AND message_hide = 0 
						AND message_type = 'TXTBX'
						LIMIT 1";
	$ret_msg = $db->query($sql_msg);
	if($db->num_rows($ret_msg))
	{
	...
	function old_do_show_calculator()
	{
		global $db,$ecom_siteid;
		$show_calc_return = false;
		//if($ecom_siteid==84)
