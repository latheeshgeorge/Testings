...
/* Start ---- Category array to handle the case of terms and conditions checkbox in cart page */
	...
	function show_Normal_price_detail($product_id,$var_arr=array(),$cur_qty=1)
	{
		...
		if(!trim($cur_qty))
			$cur_qty= 1;
		$cur_msgval=(trim($_REQUEST['passvarmsgval']))?trim($_REQUEST['passvarmsgval']):1;
		...
		if ($db->num_rows($ret_prod))
		{
			...
			if($price_arr['price_only']['discounted_price'])
			{
				...
				$cur_price = printcurr(($price_arr['price_only']['discounted_price'] * $cur_qty * $cur_msgval),$curr_sign);
			...
			else
			{
				...
				$cur_price = printcurr(($price_arr['price_only']['base_price'] * $cur_qty * $cur_msgval),$curr_sign);
				//$sav_price = '';
			}
			...
			if($was_price)
				$HTML_price .= '<li class="productdetstrikeprice">'.$normal_prefix.' '.$was_price.' + VAT</li>';
			if($cur_price)
				$HTML_price .= '<li class="productdetnormalprice">'.$normal_prefix.' '.$cur_price.' + VAT</li>';	
			if($sav_price)
				$HTML_price .= '
				 				<li class="productdetyousaveprice">'.$sav_price.'</li>
			 					';
			$HTML_price .= '</ul>';
