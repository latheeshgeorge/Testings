<?php
	...
	class cart_Html
	{
		...
		function Show_Cart()
		{
			...
			{
				$cart_alert_arr['err']=1;
				$cart_alert	 = $Captions_arr['CART'][$_REQUEST['hold_section_new']];
			}
        ...
        <div class="cart_cont_inner_top"></div>
          <div class="cart_cont_inner_cont">
			...
			<input type="hidden" name="hold_section" value="" />
				...
				$HTML_topdesc .='<div class="cart_top_outer">';
					...
					{ // Check whether logged in 
						$HTML_topdesc .= '<div class="cart_top_info">
										<div class="cart_top_info_hdr">'.stripslash_normal($Captions_arr['CART']['CART_MAINHEADING']).'</div>';
			  ...
			  <tr>
				...
				<td class="cart_table_header"><?php echo stripslash_normal($Captions_arr['CART']['CART_ITEM'])?></td>
				<td class="cart_table_header" <?php echo ($Settings_arr["product_show_instock"]==0)?'colspan="2"':''?>>&nbsp;</td>
				<?php
				...
				<td class="cart_table_header">  <?php echo stripslash_normal($Captions_arr['CART']['CART_DISCOUNT'])?> </td>
		  ...
		  else
		  {
			 $cur_indx = $all_prods_free_delivery_cnt = 0;		  
			  ...
			  ?>
			<tr>
				<td colspan="2" align="left" valign="top" class="cart_td_delete">
				<a href="#" class="update_link" onclick="if (confirm_message('<?php echo stripslash_normal($Captions_arr['CART']['CART_ITEM_REM_MSG'])?>')) {document.frm_cart.remcart_id.value='<?php echo $products_arr['cart_id']?>';handle_form_submit(document.frm_cart,'Remove_qty','#rem')}" title="<?php echo stripslash_normal($Captions_arr['CART']['CART_REMOVE'])?>"><img src='<?php url_site_image('delete-cart.gif')?>' border="0" alt="<?php echo stripslash_normal($Captions_arr['CART']['CART_REMOVE'])?>"></a>
				</td>
				<td align="left" valign="top" class="cart_td_qty">
					<?php
				...
				<td align="left" valign="middle" class="cart_td_name" <?php echo ($Settings_arr["product_show_instock"]==0)?'colspan="2"':''?>>
				...
				<td align="left" valign="middle" class="cart_td_price">
					<?php echo print_price($products_arr['product_webprice'],true)?>
				</td>
			<td align="left" valign="middle" class="cart_td_discnt"><span>
				<?php display_cart_discount($products_arr,$Captions_arr);?></span> </td>
			<td colspan="2" align="right" valign="middle" class="cart_td_total">
				<?php echo print_price($products_arr['final_price'],true)?>
			</td>
			</tr>
			<? 
			  ...
			  <tr>
				<td colspan="5" align="left" valign="top" class="cart_inner_left">
					<?php
					...
					{
				?>
				  		<div><div class="cart_butA"><div><input name="clearcart_button" type="button" class="clearcart_buttonA" id="clearcart_button" value="<?php echo stripslash_normal($Captions_arr['CART']['CART_CLEAR'])?>" onclick="if(confirm_message('Are you sure you want to clear all items in the cart?')){show_wait_button(this,'Please Wait...');document.frm_cart.cart_mod.value='clear_cart';document.frm_cart.submit();}" /></div></div></div>
			    <?php
				 	}
					...
					<tr id="giftwrap_details_tr" <?php echo ($row_cartdet['giftwrap_req'])?'':'style="display:none"'?>>
						...
						<table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
								...
								<td  align="left" valign="top"><?php echo $Captions_arr['CART']['CART_GIFTWRAP_MSG']?>(<?php echo print_price($row_gift['giftwrap_messageprice'],true)?>)</td>
			...
			<div class="cart_gift">
			 ...
			 <table width="100%" border="0" cellspacing="5" cellpadding="0"> 
			  ...
			  <tr>
				<td  align="right" valign="middle" ><?php echo stripslash_normal($Captions_arr['CART']['CART_GIFTWRAP_TOTAL'])?></td>
				<td align="right" valign="middle" ><?php echo print_price($cartData["totals"]["giftwrap"],true)?></td>
			  </tr>
			  </table>
			  </div>
				<div class="cart_gift_bottom"></div>
		  ...
		  if($promotional_proceed ==true)
		  {
		  ?>
		    <div class="prom_code">
		      <div class="prom_code_inner">
		  <?php
		  }
				if (($row_cartdet['promotionalcode_id']!=0 or $row_cartdet['voucher_id']!=0))
				{
		  ...
		  ?>
				 <table width="100%" border="0" cellspacing="0" cellpadding="0" class="prom_code_table">
			      <tr>
						  <td colspan="4" align="left" valign="middle" class="cart_td_normal"><a name="a_prom" href="#"></a>
						  <?php	
						  ...
						  <tr>
						      <td width="22%" align="left" valign="middle" class="cart_td_normal"><?php 
							  if($caption_req)
								  echo stripslash_normal($Captions_arr['CART']['CART_PROMO_CAP_2'])?></td>
						  <?php
							...
							?> <td align="right" valign="middle" colspan="2">
									<input name="cart_savepromotional" type="hidden" id="cart_savepromotional" value="0"/>	
								 	<input name="cancel_promotionalcode" type="button" class="cart_buttonA" id="cancel_promotionalcode" value="<?php echo stripslash_normal($Captions_arr['CART']['CART_PROMO_CANCEL_BUTTON'])?>" onclick="document.frm_cart.cart_savepromotional.value=1;handle_form_submit(document.frm_cart,'save_commondetails','#a_prom')" /></td>
						  <?php
						  	}
						  	elseif ($cartData["bonus"]['type']=='')
							{
				 ...
				 if($cust_id and $proceed_bonus ==false)
				 	$show_bonus_main_div = false;
						...
						if (is_Feature_exists('mod_bonuspoints'))
						{
							...
							if($show_bonus_main_div)
							{
						?>
							<div class="cart_bonus">
							<div class="cart_bonus_inner">
							<?php
							}
							if($proceed_bonus)
							{
						...
						<?php
							...
							if($cust_id) // case if logged in
							{
								...
								{  
								   ...
								   ?>
									 <tr>
										<td width="71%" class="cart_bonus_tdB"><a name="bonus">&nbsp;</a><?php echo stripslash_normal($Captions_arr['CART']['CART_SPENT_BONUS_POINTS'])?><br />(<? print (int)(($cartData["customer"]['customer_bonus']-$cartData["bonus"]["spending"])); ?> &nbsp;<?php echo stripslash_normal($Captions_arr['CART']['CART_REMAINING'])?>)&nbsp;
										</td>
										<td width="29%">
										<input type="hidden" name="maxBonusPoints"  class="input" value="<? print $cartData["customer"]["customer_bonus"]; ?>">
										<input type="text" name="spendBonusPoints" size="4" value="<? print $cartData["bonus"]["spending"]; ?>" class="cart_inputA"></td>
										<td ><input type="hidden" name="leftBonusPoints" value="<? print $cartData["bonus"]["left"] ?>">
		 ...
		 if(!$cust_id and count($cartData['products']))
		  ...
		  ?>
			  ...
			  <tr>
				<td align="right" valign="middle"><input type="button" name="button2" id="button2" value="Go"  class="cart_login_button" onclick="submit_cust_login('frm_cart');"/></td>
				</tr>
				<tr>
				...
				<td class="cart_login_td_right"><table width="100%" border="0" cellspacing="0" cellpadding="0" class="cartnewuser">
				  <tr>
				  ...
				  <tr>
					<td align="right" valign="middle"><input type="button" name="button3" id="button3" value="Sign up"  class="cart_newuser_button" onclick="window.location='<?php url_link('registration.html?pagetype=cart')?>'"/></td>
					</tr>
				</table></td>
			  </tr>
			</table>
		 ...
		 <?php
		  ...
		  ?>  
				...
				<td colspan="5" align="left" valign="top"class="cart_inner_right">
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
				...
				?>
				<tr>
				<td colspan="2" align="right" valign="middle" class="cart_dlvry">
				<input type="hidden" name="cart_delivery_id" id="cart_delivery_id" value="<?php echo $deliverydet_Arr['delivery_id']?>"  />	
					...
					?>	
					<tr>
					<td colspan="2" align="right" valign="middle" class="cart_td_delry"><?php echo stripslash_normal($Captions_arr['CART']['CART_DELIVERY_LOC'])?>
					<a name="a_deliv">&nbsp;</a> </td>
					</tr>
					<tr>
					<td align="right" valign="middle" >&nbsp;</td>
				...
				if($ecom_siteid == 	80)
			{
				...
				$sql_disable = "SELECT * FROM euro_zones ORDER BY sort_order";
				$ret_disable = $db->query($sql_disable);
				
				
				
					?>
			<input type="hidden" name="zone_msg_disp" id="zone_msg_disp" value="<?php echo $Captions_arr['CART']['ZONE_SELMSG']?>" />
		
			<tr>
				<td colspan="2" align="right" valign="middle" class="delivery_locAZ">
				 <table width="100%" border="0" cellspacing="1" cellpadding="1">
				...
				?>	
						 <tr>
						 <td align="right" class="shoppingcartcontent_delivery" colspan="2"><span class="shoppingcartpriceOtr"><?php echo $Captions_arr['CART']['ZONE']?></span> 
							<select name = "euro_zones" id = "euro_zones" >
							...
							{
								$id = $row_disable[id];
								$disable_arr[$id] =  $row_disable['zone'];
								?>
								<option value="<?php echo $id ?>" <?php if($row_cartdet['zone_id']==$id) { echo "selected";} ?>><?php echo $row_disable['zone']; ?></option>
								<?php
							}
							?>
				   ...
				   $sql_euvat = "SELECT * FROM euvat_values ORDER BY sort_order";
					$ret_euvat = $db->query($sql_euvat);
					if ($db->num_rows($ret_euvat)>0)
					{
						?>
						<tr>
						 <td align="right" class="shoppingcartcontent_delivery" colspan="2"><span class="shoppingcartpriceOtr"><?php echo $Captions_arr['CART']['EUV1']?></span> 
							<select name = "euvat_id" id = "euvat_id" onchange='handle_form_submit(document.frm_cart,"save_commondetails","#a_deliv")'>
							<option value="0">--Please Select--</option>

							<?php
							while($row_euvat = $db->fetch_array($ret_euvat))
							{
								$vatid = $row_euvat[id];
								?>
								<option value="<?php echo $vatid ?>" <?php if($row_cartdet['euvat_id']==$vatid) { echo "selected";} ?>><?php echo $row_euvat['euvat_text']; ?></option>
								<?php
							}
							?>								
							</select>
				...
				?>				  
				 ...
				 <?php
				 if($row_cartdet['euvat_id']==1)
				 {
				    $displ_comp = "display:''";				 
				 }
				 else
				 {
				   $displ_comp = "display:none";	
				 }
				 
				 //display:none
				 ?>
				 <tr style="<?php echo $displ_comp?>" id="comp_num">
				<td align="right" valign="middle" class="cart_td_normal" ><?php echo stripslash_normal($Captions_arr['CART']['CART_COMP_NUM'])?></td>
				<td align="right" valign="middle" class="cart_td_normal"><input type="text" class="cart_inputA" name="company_number" value="<?php echo $row_cartdet['company_number']?>"></td>
				</tr>		
			<?php	
		    }
...
// Show the payment type and payment method sections only if total amount > 0
 ...
 <?php
         $HTML_bottomdesc ='';
         $HTML_bottomdesc .='<div class="cart_bottom_links">';
		// Handling the case of login required before checkout.
		if (count($cartData['products'])>0)
		{
			if($hide_direct_checkout_button==false)// today start
			{
				if($show_checkoutbutton==true)	
				{
						 $HTML_bottomdesc .= '<div class="cart_shop_chkout"><div> <a href="#" onClick="handle_checkout_submit(\''.$ecom_hostname.'\',0,\''.$cartData["totals"]["bonus_price"].'\')">'.stripslash_normal($Captions_arr['CART']['CART_GO_CHKOUT']).'</a></div></div>';
				}
				else
				{
						$HTML_bottomdesc .= "<div class='cart_checkout_div' align='right'><span class='red_msg'>You need to login to continue to checkout</span></div>";
				}
			}// today start
		}
		if($ps_url) // show the continue shopping button only if ps_url have value
   ...
   <?php
    ...
    <div class="cart_cont_inner_bottom"></div>
		...
		function Show_Checkout()
		{
			...
			if($ecom_siteid==80)
			{
				/*if(!$row_cartdet['zone_id'])
				{
					$msg = 'CART_EURO_ZONE_REQ';
				...
				{
				    if($row_cartdet['euvat_id']==1 && $row_cartdet['company_number']=="")
					{
					$msg = 'CART_EUVAT_NUM_REQ';
					echo "<form method='post' action='http://$ecom_hostname/cart.html' id='cart_invalid_form' name='cart_invalid_form'><input type='hidden' name='hold_section_new' value='".$msg."' size='1'/><div style='position:absolute; left:0;top:0;padding:5px;background-color:#CC0000;color:#FFFFFF;font-size:12px;font-weight:bold'>Loading...</div></form><script type='text/javascript'>document.cart_invalid_form.submit();</script>";
					exit;
					}
				}
								...
								<div class="cart_top_outer">
									...
									<div class="cart_top_links">
										<div class="chk_back_btn">
											<div><a href="#" class="cart_blink" onclick="show_wait_button(this,'Please Wait...');gobackto_cart('<?php echo ($protectedUrl)? base64_encode($ecom_hostname):$ecom_hostname?>')"><?php echo stripslash_normal($Captions_arr['CART']['CART_BACK_CARTPAGE'])?></a></div>
										</div> 
									</div>
								</div>  
								<?php
									...
									<td align="right" valign="top"class="checkout_right">
										<table width="100%" border="0" cellspacing="0" cellpadding="0">
										  ...
										  $prod_cart_arr =array();
											  ...
											  ?>
												<tr class="cart_table">
													<td colspan="2" align="left" valign="top" class="cart_td_img">
														<?php 
														// Check whether thumb nail is to be shown here
														if ($Settings_arr['thumbnail_in_viewcart']==1)
														{
												...
												<td colspan="6" align="right" valign="top" class="chk_td_price">
													<table width="100%" border="0" cellspacing="0" cellpadding="0">
														...
														?>	
																	<tr>
																		<td  align="right" valign="middle" class="cart_td_normal"><?php echo stripslash_normal($Captions_arr['CART']['CART_GIFTWRAP_REQ'])?> (<?php echo print_price($row_gift['giftwrap_minprice'],true)?>)</td>
																		<td  align="right" valign="middle" class="cart_td_normal"><?php if ($row_cartdet['giftwrap_req']==1) echo 'Yes'?></td>
																	</tr>
																<?php
														...
														if ($deliverydet_Arr['delivery_type'] != 'None') // Check whether any delivery method is selected for the site
														{
															...
															// Check whether delivery is charged, then show the total after applying delivery charge
															if($row_cartdet["zone_id"]>0)
															{
																$sql_zone = "SELECT zone FROM euro_zones WHERE id =".$row_cartdet["zone_id"]." LIMIT 1";
																$ret_zone = $db->query($sql_zone);
																$row_zone = $db->fetch_array($ret_zone); 
															?>
															  <tr>
																<td align="right" valign="middle" class="cart_td_normal"><?php echo stripslash_normal($Captions_arr['CART']['ZONE'])?></td>
																<td align="right" valign="middle" class="cart_td_normal"><?php echo $row_zone['zone']?></td>
															  </tr>
													  <?php
															}
															// 
															if($row_cartdet["euvat_id"]>0)
															{
																$sql_vat = "SELECT euvat_text FROM euvat_values WHERE id =".$row_cartdet["euvat_id"]." LIMIT 1";
																$ret_vat = $db->query($sql_vat);
																$row_vat = $db->fetch_array($ret_vat); 
															?>
															  <tr>
																<td align="right" valign="middle" class="cart_td_normal"><?php echo stripslash_normal($Captions_arr['CART']['EUV11'])?></td>
																<td align="right" valign="middle" class="cart_td_normal"><?php echo $row_vat['euvat_text']?></td>
															  </tr>
													  <?php
															}
															if($row_cartdet["euvat_id"]==1)
															{
																if($row_cartdet["company_number"]!="")
																{
																	?>
																	<tr>
																	<td align="right" valign="middle" class="cart_td_normal"><?php echo stripslash_normal($Captions_arr['CART']['CART_COMP_NUM'])?></td>
																	<td align="right" valign="middle" class="cart_td_normal"><?php echo $row_cartdet['company_number']?></td>
																  </tr>
																	<?php																
																}
															}
										...
										<div class="<?php echo ($_REQUEST['pret']==1)?'chk_credit_outr1':'chk_credit_outr'?>">
											...
											if($cartData["payment"]["method"]['paymethod_takecarddetails']==1 and $_REQUEST['pret']!=1)
											{
											$checkout_caption = 'Checkout';
											?>
												<tr>
												...
												<?php
													...
													?>	
														...
														?>	
															...
															if($ecom_siteid==75 and $_REQUEST['pret']==1) // case of iloveflooring and returning from paypal
															{
															?>
																	<div class="chk_paymnt_btn1"><div> 
																	<input name="continue_checkout" type="submit" id="continue_checkout" value="<?php echo $checkout_caption?>" style="background-image:url(<?php url_site_image('confirmbtn_xpressCheckout.gif')?>); background-repeat:no-repeat; text-indent:-999px; height:42px; width:165px; border:0px;"/>		
																	</div></div>
															<?php
															}
															else
													...
													?>
													<div class="cart_top_links">
														<div class="chk_back_btn"><div><a href="#" class="cart_blink" onclick="<?php /*?>show_wait_button(this,'Please Wait...');<?php */?>gobackto_cart('<?php echo ($protectedUrl)? base64_encode($ecom_hostname):$ecom_hostname?>')"><?php echo stripslash_normal($Captions_arr['CART']['CART_BACK_CARTPAGE'])?></a></div></div> 
													</div>
													<?php
													}
													?>
		...
		order_gift_voucher_number,promotional_code_code_id,promotional_code_code_number,company_number,zone_id,euvat_id   
		FROM 
		orders 
		WHERE 
		...
		<div class="cart_top_outer">
		<div class="cart_top_header">
		<div class="cart_top_info_hdr"><?php echo stripslash_normal($Captions_arr['CART']['CART_CHECKOUTPREVIEWHEADING'])?></div>
		</div>
		...
		<td align="left" valign="middle" class="cart_td_name"><a href="<?php echo url_product($row_orddet['products_product_id'],$row_orddet['product_name'])?>" title="<?php echo $row_orddet['product_name']?>" class="shoppingcartprod_link"><?php echo $row_orddet['product_name']?></a></td>
		<td align="left" valign="middle" class="cart_td_qty"><label>
		<?php 
		...
		<td  align="right" valign="middle" class="cart_td_normal"><?php if ($row_ord['order_giftwrap']=='Y') echo 'Yes'?></td>
		</tr>
		<?php
		...
		<td align="right" valign="middle" class="cart_td_normal"><?php echo stripslash_normal($row_giftwrap_det['giftwrap_name'])?></td>
		</tr>
		<?php		
		}
		?>
		...
		if($row_ord["zone_id"]>0)
		{
			$sql_zone = "SELECT zone FROM euro_zones WHERE id =".$row_ord["zone_id"]." LIMIT 1";
			$ret_zone = $db->query($sql_zone);
			$row_zone = $db->fetch_array($ret_zone); 
		?>
		  <tr>
  ...
  <?php
		...
		if($row_ord["euvat_id"]>0)
		{
			$sql_vat = "SELECT euvat_text FROM euvat_values WHERE id =".$row_ord["euvat_id"]." LIMIT 1";
			$ret_vat = $db->query($sql_vat);
			$row_vat = $db->fetch_array($ret_vat); 
		?>
		  <tr>
  ...
  <?php
		...
		if($row_ord["euvat_id"]==1)
		{
			if($row_ord["company_number"]!="")
			{
				?>
				<tr>
				<td align="right" valign="middle" class="cart_td_normal"><?php echo stripslash_normal($Captions_arr['CART']['CART_COMP_NUM'])?></td>
				<td align="right" valign="middle" class="cart_td_normal"><?php echo $row_ord['company_number']?></td>
			  </tr>
				<?php																
			}
	    }
...
<?php /*?>		<tr>
		...
		function Show_CheckoutSuccess($order_id=0)
		{
			...
			if ($order_id>0)
			{
				...
				$sql_ord = "SELECT order_totalprice,order_paystatus,order_cpc_keyword, order_cpc_se_id, order_cpc_click_id,
								order_cpc_click_pm_id, order_cost_per_click_id,order_paymenttype   
								FROM 
									orders 
								WHERE 
									order_id=".$order_id." 
			...
			*/
			if($ecom_isadword)
			{
				if ($order_id>0)
				{
					...
					{
					?>
						<!-- Google Code for converted Conversion Page -->
						<script type="text/javascript">
						/* <![CDATA[ */
						var google_conversion_id = 991116047;
						var google_conversion_language = "en";
						var google_conversion_format = "2";
						var google_conversion_color = "ffffff";
						var google_conversion_label = "Hh65CKn5mwgQj_bM2AM";
						if (<?=$total_price?>) 
							{
						...
						src="//www.googleadservices.com/pagead/conversion/991116047/?value=1.000000&amp;label=Hh65CKn5mwgQj_bM2AM&amp;guid=ON&amp;script=0"/>
						</div>
						</noscript>
					<?php
					}
					elseif($ecom_siteid==83) //asll
					{
					?>
						
						<!-- Google Code for Google Conerted Clicks Conversion Page -->
						<script type="text/javascript">
						/* <![CDATA[ */
						var google_conversion_id = 1067201519;
						var google_conversion_language = "en";
						var google_conversion_format = "3";
						var google_conversion_color = "ffffff";
						var google_conversion_label = "gljJCM23pXEQ7-fw_AM";
						var google_conversion_value = <?=$total_price?>;
						...
						<noscript>
						<div style="display:inline;">
						<img height="1" width="1" style="border-style:none;" alt="" src="//www.googleadservices.com/pagead/conversion/1067201519/?value=<?=$total_price?>&amp;currency_code=GBP&amp;label=gljJCM23pXEQ7-fw_AM&amp;guid=ON&amp;script=0"/>
						</div>
						</noscript>  
		...
		?>
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="shoppingcarttable">
			  ...
			  <tr>
			  	<td align="left" class="regicontentA_msg">
				<?php
				$pay_type = $row_ord['order_paymenttype'];
				if($pay_type=='pay_on_phone' && $Captions_arr['CART']['CART_CHECKOUT_SUCCESS_MSG_PAYBYPHONE']!='')
				{
					echo stripslash_normal($Captions_arr['CART']['CART_CHECKOUT_SUCCESS_MSG_PAYBYPHONE']);
		...
		?>
			...
			?>
				<tr>
					<td align="left" class="regicontentA_msg_msg">
					<?php echo $msg?>
					</td>
				</tr>
			<?php
		...
		?>
			...
			<tr>
				<td align="left" valign="middle"></td>
					...
					<tr>
				<td align="center" class="regicontentA_msg">
				<br />
