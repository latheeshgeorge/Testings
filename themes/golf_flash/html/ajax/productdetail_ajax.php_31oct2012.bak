<?php
	...
	{
		global $db,$ecom_siteid,$Captions_arr,$PriceSettings_arr,$current_currency_details;
		...
		if ($db->num_rows($ret_prod))
		{
			...
			$HTML_price = '<div class="deat_price">';
			if($was_price)
				$HTML_price .= '<div class="deat_priceA">'.$normal_prefix.' '.$was_price.'</div>';
			if($cur_price)
				$HTML_price .= '<div class="deat_priceB">'.$normal_prefix.' '.$cur_price.'</div>';	
			if($sav_price)
				$HTML_price .= '<div class="deat_priceC">
				 				<div class="deat_priceCleft"><span>'.$sav_price.'</span></div>
			 					</div>';
