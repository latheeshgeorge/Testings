<?php
	...
	class combo_Html
	{
		...
		<div class="inner_contnt_hdr"><?php  echo $title;?></div>
	...
	function show_combo_product_details($row_prod,$keepselected=false)
	{
		...
		?>
			...
			<tr>
			<td class="shelfAprodname"><a href="<?php url_product($row_prod['product_id'],stripslashes($row_prod['product_name']),-1)?>" title="<?php echo stripslashes($row_prod['product_name'])?>" class="prod_infolink"><?php echo stripslashes($row_prod['product_name'])?></a></td>
			</tr>
			<tr>
		...
		<?php
			...
			if ($comb_id)
			{
				...
				?>
						<tr>
							<td align="left" valign="middle" class="shelfBtabletd" width="40%">
								<?php 
							...
							if ($row_var['var_value_exists']==1)
							{
								...
								{
							?>
									<input type="hidden" name="var<?=$row_prod['product_id']?>_<?php echo $row_var['var_id']?>" id="var<?=$row_prod['product_id']?>_<?php echo $row_var['var_id']?>" value="<?php echo $row_comb_det['var_value_id']?>" />
							<?php
								}
								else
								{
							...
							else
							{
								...
								{
							?>
									<input type="hidden" name="var<?=$row_prod['product_id']?>_<?php echo $row_var['var_id']?>" id="var<?=$row_prod['product_id']?>_<?php echo $row_var['var_id']?>" value="1" />
							<?php
								}
								else
								{
	...
	function Show_Combo_Multiple($ret_combos,$combprod_id,$deal_mod)
	{
		...
		while ($row_combos = $db->fetch_array($ret_combos))
		{
			...
			?>
							...
							<a href="<?php url_combo($row_combos['combo_id'],$row_combos['combo_name'],-1)?>"  title="<?php echo stripslash_normal($Captions_arr['COMMON']['SHOW_DEAL'])?>" class="pre-combodeal-showall"><?php echo stripslash_normal($Captions_arr['COMMON']['SHOW_DEAL'])?></a>
							</div>
							</div>	
						</td>
					</tr>
