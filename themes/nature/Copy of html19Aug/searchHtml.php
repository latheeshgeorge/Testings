		...
		if ($db->num_rows($ret_search))
		{
			...
			?>
				...
				?>
					...
					<?php echo stripslashes($top_content)?>
					</div>
				<?php
				}
				?>
	...
	function Show_Search_Category($search_sql,$tot_cnt)
	{
		...
		if ($db->num_rows($ret_search))
		{
			...
			while ($row_search = $db->fetch_array($ret_search))
			{
				...
				<div class="subcategoreynamelink"><a href="<?php url_category($row_search['category_id'],$row_search['category_name'],-1)?>" title="<?php echo stripslashes($row_search['category_name'])?>"><?php echo stripslashes($row_search['category_name'])?></a></div>
				</div>
				<?php
				if($cur_cnt>=$max_cnt)
				{
