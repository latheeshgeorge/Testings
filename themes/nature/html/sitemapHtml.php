...
?>
		...
		<div class="inner_contnt_hdr"><?=stripslash_normal($Captions_arr['SITEMAP']['SITEMAP_TREEMENU_TITLE'])?></div>
		...
		if(is_array($_SESSION['site_map_links']))
		{
				...
				foreach($_SESSION['site_map_links'] as $link_key => $linkval)
				{
					if($link_key > $_REQUEST['start_key'])
					{
						...
						?>
							...
							<a <?php echo $rel?> href="<?php url_static_page($pg_id,$linkval["'site_map_static_"."$pg_id'"])?>" class="sitemapcatlink" title="<?php echo stripslash_normal($linkval["'site_map_static_"."$pg_id'"])?>"><?=$linkval["'site_map_static_"."$pg_id'"]?>
							</a>
							</td>
						<?
							}
			...
			?>
				...
				<?php
				 ...
				 {
				?>
					<a href="<?=$ecom_selfhttp.$ecom_hostname?>/sitemap.html?start_key=<?=$prev_start?>" title="Prev"> &laquo; Prev  </a>&nbsp;&nbsp;
				<? 
				}
				if(is_array($_SESSION['site_map_links'][$next]))
				{
				?>
					<a href="<?=$ecom_selfhttp.$ecom_hostname?>/sitemap.html?start_key=<?=$link_key?>" title="Next"> Next &raquo;</a> 
				<? 
				}
				?>
				</td>
