<?php
	...
	class myorders_Html
	{
	  ...
	  {
	    global $insmsg, $ecom_siteid,$db,$ecom_hostname,$Settings_arr,$ecom_themeid,$default_layout,$Captions_arr,$inlineSiteComponents,$ecom_themename;
		...
		$sort_options 		= array('order_date' => 'Date','order_status'=>'Order Status');
		$sort_option_txt 	= generateselectbox('ord_sort_by',$sort_options,$sort_by,'','',0,'form-control');
		$sort_by_txt		= generateselectbox('ord_sort_order',array('ASC' => 'ASC','DESC' => 'DESC'),$sort_order,'','',0,'form-control');
	...
	if ($from_date or $to_date)
	{
		...
		{
			//$frm_arr 		= explode('/',$from_date);
			$mysql_fromdate = $frm_arr[2].'-'.$frm_arr[0].'-'.$frm_arr[1]; 
		}
		...
		{
			//$to_arr 		= explode('/',$to_date);
			$mysql_todate 	= $to_arr[2].'-'.$to_arr[0].'-'.$to_arr[1]; 
		}
		...
		$sql_count 			= "SELECT count(*) as cnt 
								FROM 
									orders  
									$where_conditions";//echo $sql_count;echo "<br>";
		...
		$ord_array = array();
			...
			//echo $HTML_treemenu;
			?>
					 <div class="container-fluid dateorder">
  ...
  $HTML_treemenu = '	<div class="breadcrump">
					<nav class="breadcrumb">
						 ...
						 <span class="breadcrumb-item active">'.stripslash_normal($Captions_arr['ORDER']['ORDER_MAINHEADING']).'</span>					</nav>
  ...
  */
  ?>
    ...
    <link rel="stylesheet" href="https://code.jquery.com/ui/jquery-ui-git.css">
		...
		<?php 
		/*
		if(!$_REQUEST['ord_fromdate'] && !$_REQUEST['ord_todate'])
		{
		?>
		<script type="text/javascript">
    ...
    ?>    
			...
			<?php
				if($Captions_arr['ORDER']['ORDER_MAINHEADING1']){
			?>		<div class="ord_list_title"><span><?=stripslash_normal($Captions_arr['ORDER']['ORDER_MAINHEADING'])?> </span></div>
			<?php }?>
			
			
			<div class="ord_listing">
				<table width="100%" class="search_areaTable table">
			
					<tr>
					
						<td class="ord_bet_date">
							<?php echo stripslash_normal($Captions_arr['ORDER']['ORDER_BETDATE'])?>
						</td>
						<td class="ord_from_date">
							<input name="ord_fromdate" id="datepicker" class="textfeild_ord form-control" type="text" size="10" value="<?php echo $_REQUEST['ord_fromdate']?>" />
						</td>
						<td class="ord_and">
							<span class="span_and">and</span>
						</td>
						<td  class="ord_to_date">
							<input name="ord_todate" id="datepicker1" class="textfeild_ord form-control"  type="text" size="10" value="<?php echo $_REQUEST['ord_todate']?>" />
						
						</td>
						<td class="ord_sort_option">
							<?php echo $sort_option_txt;?>
						</td>
						<td class="ord_sort_by">
							<?php echo $sort_by_txt;?>
						</td>
						<td  class="ord_go_button">
							<input name="Search_go" type="submit" class="redbutton" id="Search_go" value="Go" onclick="document.frm_orders.search_click.value=1" />
						</td>
						
					</tr>
				</table>
				<div class="new_post"></div><img src="<?php url_site_image('new_post.png')?>"   border="0" title="New" />
							&nbsp;<?php echo stripslash_normal($Captions_arr['ORDER']['ORDER_NEWPOSTS'])?>
				</div>
				<table width="100%" class="table table-bordered table-striped cart-page">
					<!--<tr>
						<td><img src="<?php url_site_image('new_post.png')?>"   border="0" title="New" />
							&nbsp;<?php echo stripslash_normal($Captions_arr['ORDER']['ORDER_NEWPOSTS'])?>
						</td>
					</tr>-->
					<tr>
						<td><?php echo stripslash_normal($Captions_arr['ORDER']['ORDER_ID'])?></td>
						<td><?php echo stripslash_normal($Captions_arr['ORDER']['ORDER_DATE'])?></td>
						<td><?php echo stripslash_normal($Captions_arr['ORDER']['ORDER_TOTAL'])?></td>
						<td><?php echo stripslash_normal($Captions_arr['ORDER']['ORDER_PAYSTAT'])?></td>
					</tr>
					<?php
					...
					{  
						...
						{ 
							...
							($cls=='ordertabletdcolorB')?$cls='ordertabletdcolorB':$cls='ordertabletdcolorB';
					?>
						<tr onclick="window.location='index.php?req=orders&reqtype=order_det&order_id=<?php echo $row_select_ord['order_id']?>'" style="cursor:pointer" class="edithreflink_tronmouse" title="View Details" onmouseover="this.className='edithreflink_trmouseout'" onmouseout="this.className='edithreflink_tronmouse'">
							<td class="<?=$cls?>"><?php echo $row_select_ord['order_id']; ?></td>
							<td class="<?=$cls?>"><?php echo $row_select_ord['order_dates']; ?></td>
							<td class="<?=$cls?>"><?php echo print_price_selected_currency($row_select_ord['order_totalprice'],$row_select_ord['order_currency_convertionrate'],$row_select_ord['order_currency_symbol'],true);  ?></td>
							<td class="<?=$cls?>"><?php echo getpaymentstatus_Name($row_select_ord['order_paystatus']); if($count_new==1){?> <img src="<?php url_site_image('new_post.png')?>"   border="0" title="New" /><? }?></td>
						</tr>
						<?php
						}	
						...
						{ ?>
							<tr>
								<td>
									...
									$paging 		= paging_footer_advanced_responsive($path,$query_string,$tot_cnt,$start_var['pg'],$start_var['pages'],'',$pg_variable,'Orders',$pageclass_arr);
...
if($insmsg)
	...
	$srno++;
	 ?>
	<tr>
	<td  align="left" valign="top" class="<?php echo $cls?>" nowrap="nowrap" >Order ID &amp; Date </td>
	<td  align="left" valign="top" class="userordercontent" nowrap="nowrap" colspan="3" ><?php echo $row_select_ord['order_id']?>&nbsp;&nbsp;(<?php echo dateFormat($row_select_ord['order_date'],'datetime');?>)</td>
	</tr>	
	<tr id="cancelId" style="display:none;">
	<td colspan="4" width="14%" align="left" valign="middle" nowrap="nowrap"  class="usermenucontentA"/>
	<textarea name="txt_cancel" class="regiinput" cols="15" rows="3"></textarea>
		<div style="margin-right:30%">
	<div class="cart_shop_contA"><div><input type="button" name="cancelSub" class="inner_btn_red" value=" Submit Cancel Reason " onclick="javascript:cancelSubmit(document.frm_orderdetail,'<?PHP echo $row_select_ord[order_id]; ?>')"/></div></div>
	</div>
	</td>
	</tr>
	<?php 
	...
	<td align="left" valign="middle" class="userordercontent" colspan="3"><?php echo getpaymenttype_Name($row_select_ord['order_paymenttype'])?></td>
	</tr>
	<?php
	...
	<td align="left" valign="middle" class="<?php echo $cls?>" colspan="3">
			<?php
			if($row_select_ord['order_paymentmethod']!='')
			{
	...
	<?php
			...
			?>
			<tr >
		<td colspan="7" class="prod_orderheaderA">Products Awaiting Despatch</td>
	  </tr>
	<tr>
	<td align="right" colspan="4" class="tdcolorgray_normal"><?php	
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="2">
		  <tr >
			<td align="left" class="<?php echo $cls?>" nowrap="nowrap"><div >Product</div></td>
			<?php
			/*
			...
			*/?>
			<td class="<?php echo $cls?>" align="center" nowrap="nowrap" colspan="2"><div >Qty</div></td>
		   <!-- <td class="<?php// echo $cls?>" align="center" nowrap="nowrap"><div align="center">Ord Qty</div></td> -->
			<td class="<?php echo $cls?>" align="right" nowrap="nowrap"><div >Net</div></td>
		  </tr>
		  ...
		  <?php
					...
					while ($row_prods = $db->fetch_array($ret_prods))
					{
						...
						if ($db->num_rows($ret_check))
						{
							...
							($cls=='ordertabletdcolorB')?$cls='ordertabletdcolorB':$cls='ordertabletdcolorB';
				?>
		  <tr onclick="window.location='<?php url_product($row_prods['products_product_id'],$row_prods['product_name'],-1)?>'">  
			...
			*/?> 
			<td width="8%"  class="<?PHP echo $cls;  ?>" align="center" colspan="2"><?php
	...
	<table width="100%" border="0">
	  <tr >
		<td colspan="7" class="prod_orderheaderA">Products Placed in Back Order</td>
	  </tr>
	  <tr>
		<td   class="<?PHP echo $cls;  ?>"><div align="left">Product</div></td>
		<td  class="<?PHP echo $cls;  ?>"><div align="center">Qty</div></td>
	...
	<table width="100%" border="0">
	  <tr >
		<td colspan="7" class="prod_orderheaderA">Despatched Products</td>
	  </tr>
	  <tr>
		<td   class="<?PHP echo $cls;  ?>"><div align="left">Product</div></td>
		<td  class="<?PHP echo $cls;  ?>"><div align="center">Qty</div></td>
	...
	<table width="100%" border="0">
	  <tr >
		<td colspan="7" class="prod_orderheaderA">Cancelled Products</td>
	  </tr>
	  <tr>
		<td   class="<?PHP echo $cls;  ?>"><div align="left">Product</div></td>
		<td  class="<?PHP echo $cls;  ?>"><div align="center">Qty</div></td>
	...
	<table width="100%" border="0">
	  <tr >
		<td colspan="5" class="prod_orderheaderA">Order Returns</td>
	  </tr>
	  <tr>
	...
	<tr onclick="handle_expansionall('refunddet_imgtag','bill','<?PHP echo $ecom_hostname; ?>')">
		<td   align="left"  colspan="4">
		<table width="100%" border="0" cellspacing="1" cellpadding="1">
	  <tr>
		<td width="2%" class="userorderheadernew" ><img id="refunddet_imgtag" src="<?PHP echo url_site_image('down_arrow.png'); ?>" border="0"  title="Click" style=""/></td>
		<td  align="left" class="userorderheadernew" colspan="3">Billing Details</td>
	  </tr>
	</table></td></tr>
	<tr id="billDetails" style="display:none;"><td  width="100%" colspan="4">
		<table width="100%" border="0" cellspacing="1" cellpadding="1" class="myorder_billtable">
		<?php
		$max_cols	= 1;
		$cur_col	= 0;
	...
	<tr onclick="handle_expansionall('billing_imgtag','delivery','<?PHP echo $ecom_hostname; ?>')">
	<td colspan="4" align="left" valign="bottom"><table width="100%" border="0" cellspacing="1" cellpadding="1" class="myorder_deltable">
	  <tr>
		<td width="2%" class="userorderheadernew"><img id="billing_imgtag" src="<?PHP echo url_site_image('down_arrow.png'); ?>" border="0"  title="Click" style=""/></td>
		<td  align="left" class="userorderheadernew">Delivery Details</td>
	  </tr>
	</table></td>
	</tr>
	...
	$cls = 'userordercontent';
	?>
		<table width="100%" border="0" cellspacing="1" cellpadding="1" id="deliverDetails" style="display:none;" class="myorder_deldettable"  >
		<tr>
		...
		<tr onclick="handle_expansionall('Enquiry_imgtag','enqDetails','<?PHP echo $ecom_hostname; ?>')"><td  width="100%" colspan="4">
	<table width="100%" border="0" class="myorder_enquirytable"> <tr>    
	<td width="2%" class="userorderheadernew"><img id="Enquiry_imgtag" src="<?PHP if(!$_REQUEST['ord_pg']) { echo url_site_image('down_arrow.png'); } else {  echo url_site_image('down_arrow.png'); } ?>" border="0"  title="Click" style=""/></td>
	 <td  align="left" class="userorderheadernew">Enquiry Details</td>
		</tr>
	</table>
	</td>
	</tr>
	<tr><td  width="100%" colspan="4" class="userorderheader">
	...
	<tr class="userorderheader">
	  <td colspan="6" align="right"><a href="javascript:newEnqury(document.frm_orderdetail)" class="edithreflink" > 
		<input type="hidden" name="hid_enq" /><input type="hidden" name="reqtype" />
		Add New  Enquiry </a></td>
	  </tr>
	<tr id="EnqId4" style="display:none;" >
		<td colspan="6" >
			<table border="0" cellspacing="0" cellpadding="0" width="100%" class="enq-formA">
				<tr>
	  <td colspan="6" class="userorderheader"><?php echo stripslash_normal($Captions_arr['ORDER']['ORDER_ENQ_PLACEENQ']); ?></td>
	  </tr>
	<tr id="EnqId1" style="display:none;" >
	  <td colspan="3">&nbsp;Enquiry Title </td>
	</tr>
	<tr id="EnqId1_1" style="display:none;" >  
	  <td colspan="3" ><input type="text" name="txt_enqtitle" class="form-control" /></td>
	</tr>
	<tr id="EnqId2" style="display:none;" >
	  <td colspan="3">&nbsp;Enquiry Content</td>
	 </tr>
	 <tr id="EnqId2_2" style="display:none;" > 
	  <td colspan="3"><textarea name="txt_enquiry" cols="15" rows="6" class="form-control"></textarea>       
	   &nbsp;&nbsp;<br/>        </td>
	  </tr>
	<tr id="EnqId3" style="display:none;"  >
	  <td class="" colspan="6" align="right" >	
						<div class="cart_shop_contA"><div>
										<input type="button" name="Submit" value="Add Enquiry" class="btn btn-default to-log-in" onclick="javascript:enqSub(document.frm_orderdetail)" />
	...
	<tr>
	  <td width="9%" class="ordertableheader" align="center" valign="top">#</td>
	  <td width="42%" class="ordertableheader" align="center" valign="top" colspan="2"><?php echo stripslash_normal($Captions_arr['ORDER']['ORDER_DATE'])?></td>
	  <td width="23%" class="ordertableheader" align="left" valign="top"><?php echo stripslash_normal($Captions_arr['ORDER']['ORDER_ENQ_SUB'])?></td>
	  <td width="12%" class="ordertableheader" align="center" valign="top">New</td>
	</tr>
	<?PHP if($num>0) {
	...
	?>
			<tr onclick="window.location='index.php?req=orders&reqtype=enqposts&enqid=<?PHP echo $row['query_id']; ?>&order_id=<?PHP echo $_REQUEST['order_id']; ?>'" style="cursor:pointer" class="edithreflink_tronmouse" title="View Details" onmouseover="this.className='edithreflink_trmouseout'" onmouseout="this.className='edithreflink_tronmouse'">
			  ...
			  <td class="<?PHP  echo $cls; ?>" align="center" valign="top" colspan="2"><?PHP echo $row['query_dates']; ?></td>
	...
	<?php
		...
		<tr>
			<td colspan="4" align="left" valign="bottom"><table width="100%" border="0" cellspacing="1" cellpadding="1">
			  <tr>
				<td width="3%" class="userorderheadernew"><img id="download_imgtag" src="<?PHP echo url_site_image('down_arrow.png'); ?>" border="0" onclick="handle_expansionall('download_imgtag','downloads_table','<?PHP echo $ecom_hostname; ?>')" title="Click"/></td>
...
function newEnqury(frm) { 
	...
	document.getElementById('EnqId1_1').style.display = 'none';
	document.getElementById('EnqId2').style.display = 'none';
	document.getElementById('EnqId2_2').style.display = 'none';
	...
	document.getElementById('EnqId1_1').style.display = '';
	document.getElementById('EnqId2').style.display = '';
	document.getElementById('EnqId2_2').style.display = '';
...
?>
  ...
  <tr><td class="userorderheader">
  <table width="100%" border="0" cellpadding="2" cellspacing="0" class="expandtable_cls">    
    <tr>
      <td colspan="5" align="left" class="usermenucontentbold">Subject:&nbsp;&nbsp;<?=$query_subject; ?></td>
    </tr>
    <tr>
      <td colspan="5" align="left" class="userordercontent">Enquiry:&nbsp;&nbsp;<?PHP echo nl2br($querycontent); ?></td>
    </tr>
    <tr>
    ...
    <tr id="postDet4" style="display:none;"  >
      <td colspan="2" align="left">&nbsp;<strong><?php echo stripslash_normal($Captions_arr['ORDER']['ORDER_ENQ_PLACEPOST'])?></strong></td>
      <td colspan="3">&nbsp;</td>
    </tr>
    <tr id="postDet1"   style="display:none;" >
      <td colspan="4"><textarea name="txt_post" cols="15" rows="6" class="form-control"></textarea>        &nbsp;</td>
    </tr>
    <tr id="postDet2" style="display:none;"  >
	     <td colspan="4" >
		<div><input type="button" name="Submit" value="Add New Post" onclick="javascript:postSub(document.frm_enqposts)" class="btn btn-default to-log-in" />	</div>
      ...
      <tr>
        <td colspan="5" class="userorderheader" align="left"> <?php echo stripslash_normal($Captions_arr['ORDER']['ORDER_ENQPOST_EXIST'])?> </td>
        </tr>
      <tr>
      <td width="9%" class="ordertableheader"><div align="center"><strong>#</strong></div></td>
      ...
      <td width="8%" class="ordertableheader"><div align="center"><strong>Details</strong></div></td>
    </tr>
	<?PHP
