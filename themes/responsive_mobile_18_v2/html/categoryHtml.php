<?php
	...
	class category_Html
	{
		...
		{
        global $Captions_arr,$ecom_siteid,$db,$ecom_hostname,$ecom_themename,$Settings_arr,$ecom_themeid,$default_layout,$inlineSiteComponents,$Settings_arr,$components,$position;
			...
			$HTML_treemenu = '	<div class="breadcrump">
					<nav class="breadcrumb">'.generate_tree_menu($_REQUEST['category_id'],-1,' ','','','breadcrumb-item').'</nav>
...
<div class="container-fluid mid-wrap product-list">
			...
			if ($cat_desc!='')
			{
				...
				$HTML_catdesc = '<div class="cat_desc_resp" id="desc_element">'.$cat_desc.'</div>';
				?>
		...
		$position = 'left';
			...
			?>	
					...
					<div class="col-md-8">
						<div class="container-fluid subCategory">
						<div class="row">     
					
				<div class="head-title-blue"><?php echo stripslashes($row_cat['category_name']);?>
						</div>
											
						<?php
						//$HTML_catdesc = str_replace("<p><span style=\"font-family: 'trebuchet ms', geneva; font-size: small;\">&nbsp;</span></p>", "", $HTML_catdesc);
						echo $HTML_catdesc;	 
						/*
						?>
				<script>
					//$("#desc_element1").removeAttr("style");
					$("div#desc_element").find("*").removeAttr("style");
				    $("#desc_element br").remove();
				    $("div#desc_element span").css({"font-family" : "","font-size" : ""});
					//$("div.desc_element").find("*").removeAttr("style");
					//$('div.desc_element *').removeAttr("style");
				</script>	
						<?php 
						*/ 
				...
				echo $HTML_alert = '<div class="red_msg">- '.$alert.' -</div>';
					...
					<?php
						...
						while ($row_subcat = $db->fetch_array($ret_subcat))
						{
							...
							{
						  		$HTML_subcatname = '<a class="" href="'.url_category($row_subcat['category_id'],$row_subcat['category_name'],1).'" title="'.stripslash_normal($row_subcat['category_name']).'">'.stripslash_normal($row_subcat['category_name']).'</a>';
							}
							if($row_cat['category_showimage']==1)
							{
								 $HTML_image = '<a href="'.url_category($row_subcat['category_id'],$row_subcat['category_name'],1).'" class="" title="'.stripslash_normal($row_subcat['category_name']).'">';
							 ...
							 //$HTML_image .='<a href="#">'.utf8_encode($HTML_subcatname).'</a>';
							if($row_cat['category_showshortdesc']==1)
							{
								$HTML_short_desc = stripslash_normal(replace_unwanted_quotes($row_subcat['category_shortdescription']));
							}
							?>
							<div class="col-lg-3 col-md-6 col-sm-12 subList-grid">
								<div class="pro-img-wrap">
									<div class="product-title">
										<?php
										echo utf8_encode($HTML_subcatname);
										?>
										</div>
										<?php
										echo $HTML_image;
		...
		{
			global $db,$Settings_arr,$Captions_arr,$inlineSiteComponents,$compare_button_displayed,$ecom_siteid,$default_layout,$position,$components;
           ...
           <?php
			if ($tot_cnt>0)
			{
				$prodperpage	= ($_REQUEST['catdet_prodperpage'])?$_REQUEST['catdet_prodperpage']:30;// product per page
       ...
       $HTML_topcontent = ' <div id="sort-by">
           ...
           unset($perpage_arr[$prodperpage]);
			...
			{   $cnt++;
				$HTML_topcontent .='<li><a class="sortno" href="#"  onclick="select_sort(\''.$select_sortby.'\','.$k.')" >'.$v.'</a></li>'; 
            }  
            $HTML_topcontent .='</ul></li></ul>';  
            $HTML_topcontent .='</div>';
           ...
           unset($perpage_arr[$prodperpage]);
            ...
            ?>
					...
					<div class="sorter">
						...
						<?php $sorting_html ='<select name="sort_order" id="sorter" data-role="sorter" class="sorter-options" onchange="select_sort(this.value,6)">';
							...
							$select_sortby = ($_REQUEST['catdet_sortby'])?$_REQUEST['catdet_sortby']:'custom';
							//echo $select_sortby;
						//$sorting_html .='<option value="'.$select_sortby.'" selected="selected" >Sort By</option>';
						//unset($selval_arr[$select_sortby]);
						
						//print_r($selval_arr);
						foreach($selval_arr as $k=>$v)
						{   $cnt++;
							if($_REQUEST['catdet_sortby'] == $k){
								 $select = 'selected="selected"';
							}else{
								$select = '';
							}
							$sorting_html .='<option value="'.$k.'" '.$select.'>'.$v.'</option>'; 
						}  
														   ...
														   ?>
															...
															<p class="price-details">
																...
																<span class="price"><?php
																	...
																	echo $base;
															?> <?php echo $curprice_tax_cap; ?></span>
															</p>
															</div>
							<?php //if($enable_special_display==false) // Call special display function
							{?>
							 <p><div id="bulkdisc_holder" class="bulkdisc_category"><?php show_BulkDiscounts($row_prod,array()); ?> </div></p>
							<?php }?>
							<div class="addwrap">
