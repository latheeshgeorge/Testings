...
<div class="product-zoom">
							...
							<script type="text/javascript">
								...
								$j("body").on("click", "#zoom_09", function(event){ 
								  var ez =   $j('#zoom_09').data('elevateZoom');	
	...
	<p><a href="javascript:handle_price_promise()" title="Price Promise">
		<img src="<?php echo url_site_image('banner-price-promise.jpg',1);?>" border="0"/></a></p>	
									...
									<div class="detail-products">
										...
										<p class="product-title-normal"><?php echo stripslash_normal($row_prod['product_name']); ?>
</p>

<p><?php echo stripslash_normal($row_prod['product_shortdesc']); ?></p>
 <meta itemprop="offers" itemscope itemtype="<?php echo $ecom_selfhttp?>schema.org/Offer">
								...
								if(!check_addprice_exists_new($row_prod['product_id']))
								{
									...
									//	$price_class_arr['class_type'] 		= 'div';
										...
										*/
										...
										$curprice_tax_cap = curprice_tax($price_arr);
										  ?>
										<p class="price-details"><span class="price"><?php
											if($disc>0)
													echo $disc;
											else
													echo $base;
...
if($Settings_arr['showsizechart_in_popup']==1 && $sizechart_heading['size_Availvalues']==true) // If size chart is set to show in a pop up window
	{
		...
		{
	?>
		<p><a class="text-links" href="javascript:showsizechartPopup('<?php echo $row_prod['product_id']?>','<?php echo $ecom_hostname?>','<?php echo $ecom_themename?>')" title="<?php echo stripslash_normal($sizechartmain_title)?>">Delivery Info<?php ?></a></p>
	<?php	
		}
	}
	...
	{
	?>
			<p><a class="text-links" href="<?=$row_prod['product_commonsizechart_link']?>" title="Info" 	target="<?=$row_prod['produt_common_sizechart_target']?>">Delivery Info<?php ?></a></p>
	 <?
	}
	?>
