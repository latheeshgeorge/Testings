        ...
        <tr>
          <td colspan="3" >
						...
						if($Settings_arr['turnoff_catimage']==0)
						{
				  			if ($img_support) // ** Support Category Image
							{
								...
								/*if($show_noimage)
								{
									...
									$no_img = get_noimage('prodcat','big'); 
									if ($no_img)
									{
										show_image($no_img,$row_cat['category_name'],$row_cat['category_name'],'imgwraptext');
									}	
								}*/
			 ...
			 <?php
					...
					if(count($catimg_arr)>0)// Case if more than one image assigned to category
					{	
					?>
						<tr>
