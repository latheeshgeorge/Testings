...
<div class="col-md-12 col-sm-12 content">
          ...
          <div class="<?php echo CONTAINER_CLASS?>">
			   <?php
				...
				{
			?>
					<div class="alert-success callback-success">
						- <?php echo $cart_alert?> -
					</div>
			<?php
				}
 ...
 <?php
                                                ...
                                                ?>												
												<a data-original-title="Delete" href="#" onclick="if (confirm_message('<?php echo $Captions_arr['CART']['CART_ITEM_REM_MSG']?>')) {document.frm_cart.remcart_id.value='<?php echo $products_arr['cart_id']?>';handle_form_submit(document.frm_cart,'Remove_qty','#rem')}" class="btn btn-primaryA" rel="tooltip" title=""><i class="fa fa-trash-o"></i></a>
											...
											if (count($cartData['products'])>0) // Done to show the message if no products in cart
											{
											  ...
											  ?>
													...
													?>	
													   ...
													   ?>
															<tr>
														 <td colspan="5"  class="text-right" ><?php echo $Captions_arr['CART']['CART_DELIVERY_OPT']?>  
					                              ...
					                              // If gift voucher or promotional code is valid
													...
													if ($Settings_arr['show_cart_promotional_voucher']==1)
													{
														if ($cartData["bonus"]['type']=='' or $cartData["bonus"]['type']=='promotional' or $cartData["bonus"]['type']=='voucher')
														{
															...
															?>
															<input name="cart_savepromotional" type="hidden" id="cart_savepromotional" value="0"/>
															<input name="cart_promotionalcode" type="text" id="cart_promotionalcode" size="15" class="form-control" />
															<button class="btn btn-default promogo" onclick="document.frm_cart.cart_savepromotional.value=1;handle_form_submit(document.frm_cart,'save_commondetails','')" data-original-title="" title=""><?php echo $Captions_arr['CART']['CART_PROMO_GO']?></button>
															<?php
															}
														   ?>
														   </td>
			...
			if (is_Feature_exists('mod_bonuspoints') || $proceed_bonus)
			{
				...
				*/ 
				 ...
				 $total_delivery_charge = $cartData["totals"]["extraShipping"] +  $cartData["totals"]["delivery"];
                                                ...
                                                ?>
                                                <tr>
                                                <td colspan="3"  class="text-right total_cap"><?php echo stripslash_normal($Captions_arr['CART']['CART_TOTAL_FINAL_COST'])?>&nbsp; </td>
          ...
          ?>										
                    ...
                    ?>
                    <tr>
                    <td colspan="4" align="left" valign="top" class="" >
                                    ...
                                    <tr>
                                        ...
                                        ?>
                                            <table width="100%" border="0" cellspacing="3" cellpadding="0" id="paymethod_id">
                                                <tr>
                                                <td colspan="<?php echo $pay_maxcnt?>" class="td"><?php echo stripslash_normal($Captions_arr['CART']['CART_SEL_PAYGATEWAY'])?>
                                                        <?php
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0" class="chk_butttable1" >
			<tr>
			<td>
		 <?php
        if($ps_url) // show the continue shopping button only if ps_url have value
         {
        ?>
		<a href="#" onClick="show_wait_button(this,'Please Wait...');window.location='<?php echo $ps_url ?>'" class="btn btn-shop btn-default"><?php echo stripslash_normal($Captions_arr['CART']['CART_CONT_SHOP']);?></a>
		<?php
		}
		if (count($cartData['products'])>0)
		{
			...
			{ 
				...
				{ 
					?>
					<a href="#" onClick="handle_checkout_submit('<?php echo $ecom_hostname ?>',0,'<?php echo $cartData["totals"]["bonus_price"]?>')" class="btn-primary-btA topcart-btA">checkout</a></div>
					<?php
				}
			}
		}
    ...
    <div class="<?php echo CONTAINER_CLASS?>">
      <div class="row">
        <div class="col-xs-12">
          <ul><li class="home"><a  href="'.url_link('',1).'" title="'.stripslash_normal($Captions_arr['COMMON']['TREE_MENU_HOME_LINK']).'">'.stripslash_normal($Captions_arr['COMMON']['TREE_MENU_HOME_LINK']).'</a><span>→ </span></li>
				 <li class="category13">'.stripslash_normal($Captions_arr['CART']['CART_CHECKOUTHEADING']).'</li>
				 </ul>
        </div>
      </div>
    </div>
		...
		?>	
				<div class="container-fluid">
					...
					if($_REQUEST['alertp']!='')
					{
						$whichtenant = $_REQUEST['alertp'];
					  $cart_alert .= "Invalid file type .Please upload JPEG,PNG OR GIF image for ".$whichtenant." !!!";
					}
								if($cart_alert and count($cartData['products'])>0)
								{
								?>
								<div class="cart_msg_outer">
									<div class="alert-success callback-success">- <?php echo $cart_alert?> -</div>
								</div> 
								<?php
								}
								?> 	
...
?>						<tr>						
			...
			?>
		  <tr>
				<td align="right" valign="middle" class="grandTotal" ></td>
				<td  align="right" valign="middle" class="grandTotalHead" colspan="2"><?php echo $Captions_arr['CART']['CART_TOTAL_FINAL_COST']?>&nbsp; </td>
				<td align="left" valign="middle" class="grandTotal" ><?php echo print_price($cartData["totals"]["bonus_price"],true)?></td>
			  </tr>
		  <?php
		  	}
			$rem_val = $cartData["totals"]["bonus_price"] - $str_reduceval;
					...
					<div class="cart_cont_outr">
						<div class="cart_cont_inner_cont_a">
							...
							<div class="chk_back_btn">
								<div><a href="#" class="to-log-in" onclick="show_wait_button(this,'Please Wait...');gobackto_cart('<?php echo ($protectedUrl)? base64_encode($ecom_hostname):$ecom_hostname?>')"><?php echo stripslash_normal($Captions_arr['CART']['CART_BACK_CARTPAGE'])?></a></div>
							</div> 
						</div>
						<form role="form" method="post" name="frm_checkout"  id="frm_checkout" class="registration-form" action="<?php echo $http?>" <?php/*onsubmit="return validate_checkout_fields(document.frm_checkout)" */?> enctype="multipart/form-data">
			...
			?>
														...
														{		
															...
															if($row_checkout['field_req']==1)
															{
																...
																$chkout_Req_Desc[]	= "'Enter ".$row_checkout['field_error_msg']."'";
																
																$chkout_Req_step[]		= "'".$row_checkout['field_key']."'";
																$chkout_Req_Desc_step[]	= "'Enter ".$row_checkout['field_error_msg']."'";			
															}
...
//				$cont_class = 'shoppingcartcontent_noborder';
				...
				include 'show_dynamic_fields.php';	
														?>
														<div class="form-group">		
														<div class="field_name_div"><label class="sr-onlyA">
														Upload passport*
														</label></div>
														<?php 
														$sess_id	= Get_session_Id_from();
														$sql_sel = "SELECT passport_filename,tenant_name FROM cart_passport_details WHERE session_id = '$sess_id' AND tenant_name='tenant1'" ;
														$ret_sel = $db->query($sql_sel);
														$row_sel = $db->fetch_array($ret_sel);
														$sel_file = $row_sel['passport_filename'];
														if($sel_file=='')
														{
														$chkout_Req_step[]		="'tenant1_passport'";
														$chkout_Req_Desc_step[]	= "'Upload passport'";
														}
														?>
														<div class="field_value_div">
														<input type="file" name="tenant1_passport" id="tenant1_passport"><span class="slected_file"><?php echo $sel_file;?></span>
														</div>
														</div>
														...
														{
														?>														
														<button type="button" id="chkbutton-bill" class="btn-primary-btA topcart-btA" onclick="validateStep1('submit');">Checkout</button>
													...
													if(($cartData['totals']['qty'])>1)
													{
														...
														{		
															...
															{ 	
																...
																$chkout_Req_Desc[]	= "'Enter".$row_checkout['field_error_msg']." For $tenantname'"; 			
																$chkout_Req_step[$cnt][]		= "'".$ext_name.$row_checkout['field_key']."'";
																$chkout_Req_Desc_step[$cnt][]	= "'Enter ".$row_checkout['field_error_msg']." For $tenantname'"; 			
															}
															?>
															<div class="form-group">
																<label class="sr-onlyA" for="form-<?php echo $row_checkout['field_key'];?>"><?php echo stripslash_normal($row_checkout['field_name']); if($row_checkout['field_req']==1) { echo '<span class="redtext">*</span>';}?></label>
														...
														$passport_name = $ext_name."passport";
															$tenantname = str_replace(' ', '', $tenantname);
														$sql_sel1 = "SELECT passport_filename,tenant_name FROM cart_passport_details WHERE session_id = '$sess_id' AND tenant_name='".strtolower($tenantname)."'" ;
														$ret_sel1 = $db->query($sql_sel1);
														$row_sel1 = $db->fetch_array($ret_sel1);
														$sel_file1 = $row_sel1['passport_filename'];
														$ten_file1 = $row_sel1['tenant_name'];
														if($sel_file1=='')
														{
														$chkout_Req_step[$cnt][]		="'".$passport_name."'";
														$chkout_Req_Desc_step[$cnt][]	= "'Upload passport $ten_file1'";
														}
														...
														<div class="field_name_div">
															<label class="sr-onlyA">
															Upload passport*</label>
															</div>
															<div class="field_value_div">
															<input type="file" name="<?php echo $ext_name?>passport" id="<?php echo $ext_name?>passport">
															</div>
															</div>
														<?php
														}	
										...
										<div class="chk_credit_outr">
											...
											<tr>
												<td  align="left" valign="middle" class="cartterms">			
													...
													{ 
													?>
														<br /><input class="shoppingcart_radio" type="checkbox" name="cart_terms" id="cart_terms" value="1" /><a href="<?php echo $ecom_selfhttp.$ecom_hostname; ?>/terms-conditions-pg50059.html" class="terms_link"><?php echo stripslash_normal($Captions_arr['CART']['CART_ACCEPT_TERMS_CONDITIONS'])?></a>
													<?php	
													}
													?>	
												<br /></td>
  ...
  echo $HTML_treemenu;	
		?>
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="shoppingcarttable">
			  ...
			  <tr>
			  	<td align="left" class="regicontentAA"><div class="alert-success callback-success">
				<?php 
