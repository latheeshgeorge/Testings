...
global $Captions_arr,$ecom_siteid,$db,$ecom_hostname,$ecom_themename,$Settings_arr,$ecom_themeid,$default_layout,$inlineSiteComponents,$Settings_arr;
        ...
        <div class="container-tree">
          <ul>
			...
			if ($cat_desc!='')
			{
				$HTML_catdesc = '<div class="col-md-12">'.$cat_desc.'</div>';
			}
			$row_cat['category_turnoff_mainimage'] =1; // overriding to hide category main image
		...
		<div class="<?php echo CONTAINER_CLASS?> list-item-cart">
					<div class="">

		<div class="new_title center">
        <h1 class="h1-header-cls"><?php echo stripslashes($row_cat['category_name']);?></h1>
      </div>
			...
			if($prodshow)// Checks whether products to be shown in the middle area
			{
				...
				{ 
					...
					*/ 
					...
					$prodsort_by = 'a.product_actualstock DESC, a.product_webprice ASC ';
					$prodsort_order = '';
		...
		function Show_Subcategories($ret_subcat,$url,$row_cat)
		{
			...
			if($alert)
			{
				echo $HTML_alert = '<div class="red_msg">- '.$alert.' -</div>';
					...
					?>
					<div class=""><div class="col-md-12 listitems">    <h2></h2>
					<?php
						$cur_col = 0;
...
';
		...
		function Show_Products($ret_prod,$tot_cnt,$start_var,$cat_det,$def_orderfield,$def_orderby)
		{
			...
			if ($tot_cnt>0)
			{
				...
				$query_string 	= "&amp;catdet_sortby=".$_REQUEST['catdet_sortby'].'&amp;catdet_sortorder='.$_REQUEST['catdet_sortorder'].'&amp;catdet_prodperpage='.$prodperpage;
				$mod = 'resp';
       ...
       $HTML_topcontent = ' <div id="sort-by">
           ...
           unset($selval_arr[$select_sortby]);
            ...
            $HTML_topcontent .='<div id="limiter">
                  <label>View: </label>';
           ...
           unset($perpage_arr[$prodperpage]);
			...
			<div class="row-container" >  
 <?php /*<h3><?php echo utf8_encode($cur_title);?></h3>*/?>
							...
							?>
								<div class="grid_1_of_4 images_1_of_4">
									<div class="product-grid">
										...
										?>	
													<a href="javascript:handletap('<?php url_product($row_prod['product_id'],$row_prod['product_name'],-1)?>','catdet<?php echo $row_prod['product_id']?>')" title="<?php echo stripslashes($row_prod['product_name'])?>" <?php //echo getmicrodata_producturl()?> >
																	<?php
																	// Calling the function to get the image to be shown
																	...
																	if(count($img_arr))
																	{
																		...
																		show_image(url_root_image($img_arr[0][$pass_type],1),$row_prod['product_name'],$row_prod['product_name'],'listscrollimg" data-imageid="catdet'.$row_prod['product_id'].'" data-tapped="0" data-immore="'.$moreimgstr);
																	}
																	else
																	{
								 ...
								 $mods['source'] = "shelf";
								 show_ProductLabels_Unipad($row_prod['product_id'],$mods); ?>
											</div>
