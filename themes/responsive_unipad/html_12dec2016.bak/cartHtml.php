...
//print_r($_REQUEST);
			...
			if($_REQUEST['alertp']==1)
			{
			  $cart_alert .= "Invalid passport field!!!";
			}
	 ...
	 <form method="post" name="frm_checkout"  id="frm_checkout" class="frm_cls" action="<?php echo $http?>" onsubmit="return validate_checkout_fields(document.frm_checkout)" enctype="multipart/form-data">
				...
				<td colspan="5" align="right" valign="middle">
					...
					?>
							<tr>
								...
								?>	
								isValid = check_validate_new(fieldRequired,fieldDescription,fieldEmail);	
								return isValid;
								}
								</script>
					<?php
					...
					$saved_checkoutvals_extrabilling = get_CheckoutValues_extrabilling($address);
									...
									if(($cartData['totals']['qty'])>1)
									{
									  ...
									  ?>
									  <div id="step-<?php echo $cnt?>">
								<h2 class="StepTitle">Step <?php echo $cnt?>: <?php echo "Please Fill In The Details Of Tenant # $cnt"; ?></h2>
								<table width="100%" cellpadding="1" cellspacing="1" border="0">
								<?php
								$chkout_Req_step = $chkout_Req_Desc_step = $chkout_Email_step = array();
									// Get the list of billing address static fields to be shown in the checkout out page in required order
									$sql_checkout = "SELECT field_det_id,field_key,field_name,field_req,field_error_msg   
														FROM 
															general_settings_site_checkoutfields 
												...
												{	$ext_nameEmail ="";					
													...
													{		
														...
														{ 	
															...
															$chkout_Req_step[]		= "'".$ext_name.$row_checkout['field_key']."'";
															$chkout_Req_Desc_step[]	= "'".$row_checkout['field_error_msg']." For $tenantname'"; 			
														}	
														?>
															<tr>
													...
													<?php
															...
															<script type="text/javascript">
															function validateStep<?php echo $cnt?>()
															{
																var isValid = true; 
																<?php
																	// Blank checking
...
<script type="text/javascript">
						function check_validate_new(fieldRequired,fieldDescription,fieldEmail)
						{
							if(fieldRequired.length)
							{
								...
								{
									var un = $j('#'+fieldRequired[xx]).val();
									 if(!un && un.length <= 0){
										 $j('#'+fieldRequired[xx]).focus();
										return false;
									}	
								}
							}
							if(fieldEmail.length)
							{
								...
								{
									var un = $j('#'+fieldEmail[xx]).val();
									if(!isValidEmailAddress(un)){
										alert("Invalid Emaid");
										 $j('#'+fieldEmail[xx]).focus();
										return false;
									}	
								}
							}
						...
						// Smart Wizard     
							...
							$j('#wizard').smartWizard({transitionEffect:'slideleft',onLeaveStep:leaveAStepCallback,onFinish:onFinishCallback,enableFinishButton:false});
						   ...
						   var isStepValid = true;
							...
							if(($cartData['totals']['qty'])>1)
							{
								for($i=1;$i<$cartData['totals']['qty'];$i++)
								{
									...
									$tenantname = "Tenant ".$cnt;
									?>
										if(validateStep<?php echo $cnt?>() == false){
										isStepValid = false;
										 
											$j('#wizard').smartWizard('setError',{stepnum:<?php echo $cnt?>,iserror:true});   
											$j('#wizard').smartWizard('showMessage','Please fill in the required fields in step '+<?php echo $cnt?>);      
										}else{
											$j('#wizard').smartWizard('setError',{stepnum:<?php echo $cnt?>,iserror:false});
										}	
									<?php
								}  
							}	
						...
						function validateSteps(step){
							var isStepValid = true;
							 if(step == 1){
								if(validateStep1() == false ){
								  isStepValid = false; 
								  $j('#wizard').smartWizard('showMessage','Please fill in the required fields in step '+step+ ' and click next.');
								  $j('#wizard').smartWizard('setError',{stepnum:step,iserror:true});         
								}else{
							...
							if(($cartData['totals']['qty'])>1)
							{
								for($i=1;$i<$cartData['totals']['qty'];$i++)
								{
									...
									$tenantname = "Tenant ".$cnt;
									?>
									if(step == <?php echo $cnt?>){
										if(validateStep<?php echo $cnt?>() == false ){
											isStepValid = false; 
											$j('#wizard').smartWizard('showMessage','Please correct the errors in step'+step+ ' and click next.');
											$j('#wizard').smartWizard('setError',{stepnum:step,iserror:true});         
										}else{
											$j('#wizard').smartWizard('hideMessage');
											$j('#wizard').smartWizard('setError',{stepnum:step,iserror:false});
