<?php
	...
	*/ 
		...
		{  
			...
			?>
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="cont-checkout">
	<tr>
	 ...
	 <tr>
		   <td class="image_tdA" colspan="2">
			   ...
			   <?php
				...
				{
				?>
					<a  href="<?php url_product($row_prod_det['product_id'],$row_prod_det['product_name'],-1)?>" title="<?php echo stripslash_normal($row_prod_det['product_name'])?>">					</a>
				...
				{
				?>
					<a href="<?php url_product($row_prod_det['product_id'],$row_prod_det['product_name'],-1)?>" class="image_tda" title="<?php echo stripslash_normal($row_prod_det['product_name'])?>">
					<?php
	...
	<div class="var_tddiv">
		 ...
		 <?php 
			...
			?>
		<label>
			<a href="#" onclick="if(confirm_message('Are you sure you want to Delete this Enquiry?')){document.frm_enquire.enq_mod.value='delete_enquiry';document.frm_enquire.enquiry_id.value='<?=$row_select_enq['enquiry_id']?>';document.frm_enquire.submit();}">                            <button class="btn btn-danger btn-sm" type="button" name="button" id="button3" value="<?php echo stripslash_normal($Captions_arr['CART']['CART_REMOVE'])?>"  /><i class="fa fa-trash-o"></i></button>
		  </a></label>
		  </div>
		</td> 
	</tr>
	<tr>
                        <td class="tdbgrys" align="left" colspan="2"><span class="fbig"><?php
					...
					$price_class_arr['discount_class'] 	= 'shlf_discountprice';
							//echo show_Price($row_prod_det,$price_class_arr,'other_3',3);
								...
								if($price_arr['discounted_price'])
									...
									echo $price_arr['base_price'];?></span></td>
                    </tr>
	 <tr>
		<td class="tdbgrys" colspan="2">			
			 
		  			<div><input type="button" name="button3" id="button3" value="Add Notes"  class="add-to-cart-in cart_margin" onclick="add_Enquire_notes(frm_enquire,'<?=$row_select_enq['enquiry_id']?>')"/></div>
	...
	<?php
					...
					{
					?>
						<div><input name="clearenq_button" type="buttonclear" class="add-to-cart-in cart_margin" id="clearenq_button" value="<?php echo stripslash_normal($Captions_arr['ENQUIRE']['ENQUIRE_CLEAR'])?>" onclick="if(confirm_message('Are you sure you want to clear all items in the Enquiry?')){show_wait_button(this,'Please Wait...');document.frm_enquire.enq_mod.value='clear_enquiry';document.frm_enquire.submit();}" /></div>
					<?php
					}
					?>						
			</td>
	...
	<tr id="add_note_tr_<?=$row_select_enq['enquiry_id']?>" style="display:none" >
			...
			<tr>
			<td  align="left" valign="middle"   class="regiconentA"  >Add Notes:<br /><textarea  name="enquirenote_<?=$row_select_enq['enquiry_id']?>" rows="3" cols="18" class="form-control" id="note_text_<?=$row_select_enq['enquiry_id']?>" ><? if($_REQUEST['enquirenote_'.$row_select_enq['enquiry_id']]) echo $_REQUEST['enquirenote_'.$row_select_enq['enquiry_id']];else echo $row_select_enq['enquiry_note'];?></textarea><br />
			<input name="buttonenqsaveone_<?=$row_select_enq['enquiry_id']?>" type="submit" class="add-to-cart-in cart_margin" id="buttonenqsaveone_<?=$row_select_enq['enquiry_id']?>" value="Save Note" /></td>
			</tr>
</table>
</td></tr>
			<? 		
...
?>
           ...
           <tr>
				...
				<td >
					...
					<a href="<?php url_link('fillenquiry.html')?>" > <input name="buttonenq1" type="button" class="btn btn-default topcart-bt" id="buttonenq1" value="Submit Enquiry" /></a>
				  ...
				  ?>
					<?php
						...
						{
							?>
						<a href="#" class="btn btn-default add-to-cart" onClick="show_wait_button(this,'Please Wait...');window.location='<?=$ps_url?>'"><?php echo stripslash_normal($Captions_arr['CART']['CART_CONT_SHOP'])?></a>
							<?php
						}
						else
							echo '&nbsp;';
...
<input name="txt_enqemail" class="form-control" value="<?=$row_cust['customer_email_7503']?>" type="text" maxlength="<?=$short?>" placeholder="<?=stripslash_normal($Captions_arr['ENQUIRE']['ENQUIRE_DET_EMAIL'])?>">
			...
			$table_class            = 'reg_table';			
				...
				include 'show_dynamic_fields.php';
			?>
			<input name="enquiry_submit" type="submit" class="btn btn-default topcart-bt" id="enquiry_submit" value="Send" />
			 </div>
			<?php	
				$cur_pos 				= 'Bottom';
				$section_typ			= 'enquire'; 
    ...
    function expand_date()
    {
		 ...
		 else if(document.getElementById('datetrtd').style.display=='none')
		 {
		    ...
		    document.getElementById("datetrtd").className += " ordoptionsA";
				...
				<div class="container containerEnq">
				...
				<tr>
					...
					<input name="clearenq_button" type="button" class="btn btn-default to-log-in" id="clearenq_button" value="Go" onclick="document.my_enquire.submit()" />
