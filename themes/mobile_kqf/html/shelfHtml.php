...
<div class="page_nav_content"><ul>';//.'';
								...
								//echo $shelfData['shelf_displaytype'];
								if($_REQUEST['req']=='')
								{ 
								...
								<?php							if ($shelfData['shelf_showprice']==1)// Check whether description is to be displayed
																{
																		...
																		$price_class_arr['ul_class'] 		= 'row2_price';
													$price_class_arr['normal_class'] 	= 'row2_price_a';
													$price_class_arr['strike_class'] 	= 'row2_price_a';
													$price_class_arr['yousave_class'] 	= 'row2_price_b';
													$price_class_arr['discount_class'] 	= 'row2_price_b';
												$price_arr =  show_Price($row_prod,$price_class_arr,'shelf',false,6);
																		//$price_arr =  show_Price($row_prod,array(),'shelf',false,6);
																		if($price_arr['discounted_price'] != "" || $price_arr['base_price'] != "")
																		{
								?>										<div class="shp_brnd_thumbimg_price">
								<?php									
																		//echo "<pre>";print_r($price_arr);die();
																		if($price_arr['discounted_price'])
																			echo $Captions_arr['COMMON']['LIST_PRICE']." ".print_price($price_arr['discounted_price']);
																		else
																			echo $Captions_arr['COMMON']['LIST_PRICE']." ".print_price($price_arr['base_price']);
																		/*if($price_arr['disc_percent'])
																			echo "<br>".$price_arr['disc_percent']."% ".$Captions_arr['COMMON']['FEAT_OFF'];*/
								/*?>										</div>
								<?php									}
																}
							...
							<?php									
								...
								<?php		}
											if ($shelfData['shelf_showprice']==1)// Check whether description is to be displayed
											{
												...
												if($price_arr['discounted_price'] != "" || $price_arr['base_price'] != "")
												{
								?>				<div class="shlf_table_2row_price">								<?php				
													...
													if($price_arr['disc_percent'])
														echo "<div class='row2_price_b'>".$price_arr['disc_percent']."% ".$Captions_arr['COMMON']['FEAT_OFF']."</div>";
								...
								<?php		break;
								  ...
								  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="ctable">
									...
									while($row_prod = $db->fetch_array($ret_prod))
									{
										...
										?>
									<tr>
									<td class="<?php echo $cls1;?>"><div class="<?php echo $cls; ?>">
									<a class="linkbold_s" href="<?php url_product($row_prod['product_id'],$row_prod['product_name'],-1)?>" title="<?=$row_prod['product_name']?>"><?=$row_prod['product_name']?></a></div></td>
									</tr>
									<?php 
									}
									?>
									<tr>
									<td class="shlf_shwall_c">
								...
								<?php				
													//echo "<pre>";print_r($price_arr);
													if($price_arr['base_price'])
														echo "<div class='row1_price_a'>".$Captions_arr['COMMON']['LIST_PRICE']." ".print_price($price_arr['base_price'])."</div>";
													if($price_arr['disc_percent'])
														echo "<div class='row1_price_b'>".$price_arr['disc_percent']."% ".$Captions_arr['COMMON']['FEAT_OFF']."</div>";
