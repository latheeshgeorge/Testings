	...
	class featured_Html
	{
		...
		/* Sony Jul 01, 2013 */
			...
			if(count($discthm_group_prod_array))
			{
				...
				if(!in_array($row_featured['product_id'],$discthm_group_prod_array))
					return;
			}
			/* Sony Jul 01, 2013 */
		...
		?>          <form method="post" action="<?php url_link('manage_products.html')?>" name='frm_featured' id="frm_featured" class="frm_cls" onsubmit="return product_enterkey(this,<?php echo $row_featured['product_id']?>)">
					...
					*/
					...
					<div class="ft_div_desc"><?php echo $row_featured['product_shortdesc'];?></div>
					<?php
					...
					$price_arr =  show_Price($row_featured,$price_class_arr,'featured',false,6);
					//echo "<pre>";print_r($price_arr);
					//echo print_price($price_arr['base_price']);echo "<br>";
					//echo "<pre>";print_r($Captions_arr);
					if($price_arr['base_price'])
					$HTML_price = '<div class="price_feat_a">'.$Captions_arr['COMMON']['LIST_PRICE']." ".print_price($price_arr['base_price']).'</div>';
					//if($price_arr['discounted_price'])
					//$HTML_price .= '<div class="priceb">'.$price_arr['discounted_price'].'</div>';
					if($price_arr['disc_percent'])
					$HTML_price .= '<div class="price_feat_b">'.$price_arr['disc_percent']."% ".$Captions_arr['COMMON']['FEAT_OFF'].'</div>';
					echo $HTML_price;
					?>
					</td>
					</tr>
