		...
		function Show_ProductDetails($row_prod)
		{
		   ...
		   $sql_prods = "SELECT product_sizechart_mainheading 
			...
			if ($Settings_arr['bonus_points_instock'] and $row_prod['product_bonuspoints']>0)
			{
				$sql 				= "SELECT bonus_point_details_content FROM general_settings_sites_common WHERE sites_site_id=".$ecom_siteid." LIMIT 1";
			 	...
			 	if($fetch_arr_admin['bonus_point_details_content']!='')
					$bonus_more_link = '<a href="'.url_link('bonuspoint_content.html',1).'" title=""><img src="'.url_site_image('bonusmoreinfo.gif',1).'" /></a>';
				else	
					$bonus_more_link = '';
				$HTML_bonus = '<div class="deat_bonus">
								...
								<div class="deat_bonusC">'.$bonus_more_link.'</div>
								</div>';
			}
			...
			if($db->num_rows($res))
			{
					...
					if($Settings_arr['showsizechart_in_popup']!=1 && $sizechart_heading['size_Availvalues']==true) // if size chart is set to show in current page itself
				 	{	
			...
			function ajax_return_productdetailscontents() 
			{
				...
				if(req.readyState==4)
				{
					if(req.status==200)
					{
						...
						{
							document.getElementById('moreimg_curpos').value = 0;
			...
			var interval_height = 50;
			function goup()
			{
				var totheight_arr = document.getElementById('innerdiv').style.height.split('px');
				var totheight = parseInt(totheight_arr[0]);
				curpos = parseInt(document.getElementById('moreimg_curpos').value);
				if(curpos<0 && curpos<=totheight)
				{
					var curval = curpos + interval_height;
					document.getElementById('innerdiv').style.top = curval+'px';
					curpos = curval;
					document.getElementById('moreimg_curpos').value = curpos;
				}
			}
			function godown()
			{
				var totheight_arr = document.getElementById('innerdiv').style.height.split('px');
				var totheight = parseInt(totheight_arr[0]);
				var temp_totheight = -1 * totheight;
				curpos = parseInt(document.getElementById('moreimg_curpos').value);
				if(curpos<=0 && (curpos)>temp_totheight)
				{
					var curval = curpos - interval_height;
					document.getElementById('innerdiv').style.top = curval+'px';
					curpos = curval;
			...
			<input type="hidden" name="moreimg_curpos" id="moreimg_curpos" value="0" />
			
			<div class="deat_outer">
			<div class="deat_top"></div>
			<div class="deat_cont">
			<?=$HTML_treemenu?>
			<div class="details_con_det">
			<div class="details_top_det"></div>
			<div class="details_mid_det">
			<div class="details_content">        
				<div class="deat_pdt_name"><?php echo stripslash_normal($row_prod['product_name']); 
			...
			<div class="deat_pdt_image">
			<div class="deat_pdt_image_inner"> 
			<div id="mainimage_holder">				
			<?php $this->Show_Image_Normal($row_prod);?>
			</div>
			<div class="deat_pdt_com_outr">
			<div class=" deat_compare_left"></div>
			<div class=" deat_compare_mid">            
			<?=$HTML_showstock?>				
			<div class="deat_pdt_button">
			<?=$HTML_video;?>
			</div>
			<div class="deat_pdt_com_com">
			<?=$HTML_compare?></div>
			</div>  
			<div class=" deat_compare_right"></div>
			</div>
			</div>
			...
			<div class="deat_cont_right">
			<div class="deat_cont_right_top"></div>
			<div class="deat_cont_right_mid">
			 <? 
			...
			<div class="deat_cont_right_bottom"></div>                    
			</div>                   
			</div>
			</div>
			</div>
			<div class="details_bottom_det"></div>
			</div>
...
function show_more_images($row_prod,$exclude_tabid,$exclude_prodid)
{
	...
	{
		$show_height = 50* count($prodimg_arr);
	?>	
		<div class="deat_pdt_thumbimg">
		<div class="move_div">
		<a href="javascript:goup()"><img src="<?php url_site_image('up_arrow.gif')?>" border="0" /></a></div>
		<div id="outerdiv" class="outerdiv_class">
		<div id="innerdiv" class="innerdiv_class" style="height:<?php echo $show_height?>px">
		<?php
		foreach ($prodimg_arr as $k=>$v)
		{ 
			$title = ($v['image_title'])?stripslash_normal($v['image_title']):$row_prod['product_name'];
		?>
		<div class="eachblock_class">
		...
		<div class="move_diva"><a href="javascript:godown()"><img src="<?php url_site_image('down_arrow.gif')?>" border="0" /></a></div>
		</div>
	<?php
	}
}
						...
						<?php
							  ...
							  else
							  {
									...
									while ($row_vals = $db->fetch_array($ret_vals))
									{
										...
										//global $image_icontype;
										$row_vals['image_icontype'] = 'image_thumbpath';
...
browser = new relatedBrowser( "relatedBrowser", "leftArrow", "rightArrow", 2 , 1, "http://<?=$ecom_hostname?>/images/<?=$ecom_hostname?>/site_images/" );
<?php
while($row_prod = $db->fetch_array($ret_prod))
	{  
	 $prod_array[] = $row_prod;
			...
			if(count($img_arr))
			{
				$img = url_root_image($img_arr[0][$pass_type],1);
			}
			else
			{
			 ...
			 $price_arr =  show_Price($row_prod,array(),'compshelf',false,4);
											if($price_arr['discounted_price'])
												$price =  $price_arr['discounted_price'];
											else
												$price =  $price_arr['base_price'];
		$url[]=url_product($row_prod['product_id'],$row_prod['product_name'],1);	
		$img_k[] = $img;
		$prod_name[] =$row_prod['product_name']; 
		$prod_desc[] = stripslashes($row_prod['product_shortdesc']);							
		$prod_price[] = $price;
	?>
		 browser.addItem( new relatedItem( "<?php url_product($row_prod['product_id'],$row_prod['product_name'],-1)?>" , "<? echo $img?>" , '' , '' ,"<h4><a  href='<?php url_product($row_prod['product_id'],$row_prod['product_name'],-1)?>'><? echo $row_prod['product_name']?></a></h4><?php echo stripslashes($row_prod['product_shortdesc'])?>" , "<?=$row_prod['product_name']?>" , "<?php url_product($row_prod['product_id'],$row_prod['product_name'],-1)?>" , false , "<?=$price ?>","http://<?=$ecom_hostname?>/images/<?=$ecom_hostname?>/site_images/" ) );
	 <?
	 }
		 ?>
}
		...
		<div class="link_pdt_top"></div>
			...
			<span id="leftArrow"></span><span id="rightArrow"></span>
			</div>
			<h4><?php echo stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_PEOPLE_INTERESTED']);?></h4>
			</div>
			<table id="relatedBrowser" border="0" cellpadding="0" cellspacing="0">
			<thead>
			<tr><th></th><th></th></tr>
			</thead>
			<tbody>
			...
			<div class="rProduct"><? if($prod_name[0]!='') {?>
			<div class="rImage"><a href="<?php echo $url[0] ?>"><img alt="<?=$prod_name[0]?>" src="<?=$img_k[0]?>" ></a></div><div><div class="rText"><h4><a title="<?=$prod_name[0]?>" href="<?=$url[0]?>"><?=$prod_name[0]?></a></h4> <?=$prod_desc[0]?></div></div><div class="rPrice"><?=$prod_price[0]?></div><div class="rMore"><a href="<?php echo $url[0]?>"><img alt="more info" src="<?php url_site_image('link-info.gif')?>"></a></div><? }?></div>
			
			</td><td>
			
			<div class="rProduct"><? if($prod_name[1]!='') {?><div class="rImage"><a href="<?php echo $url[1] ?>"><img alt="<?=$prod_name[1]?>" src="<?=$img_k[1]?>" ></a></div><div><div class="rText"><h4><a title="<?=$prod_name[1]?>" href="<?=$url[1]?>"><?=$prod_name[1]?></a></h4> <?=$prod_desc[1]?></div></div><div class="rPrice"><?=$prod_price[1]?></div><div class="rMore"><a href="<?php echo $url[1]?>"><img alt="more info" src="<?php url_site_image('link-info.gif')?>"></a></div><? }?>	</div>
