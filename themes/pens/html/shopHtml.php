<?php
	...
	class shop_Html
	{
	  ...
	  <?php
							...
							if(count($shopimg_arr))
								{
									...
									$HTML_image = '<div class="cat_main_image">'.show_image(url_root_image($shopimg_arr[0][$pass_type],1),$row_shop['shopbrand_name'],$row_shop['shopbrand_name'],'imgwraptext','',1).'</div>';
									$show_noimage 	= false;
								}
								else
									$show_noimage = true;
			...
			<div class="sub_cate_hdr"><?php echo stripslashes($row_shop['shopbrand_name'])?></div>
		       ...
		       if($_REQUEST['type_cat']=='sub_shop'){
				...
				$cur_col = 0;
					...
					?>
							...
							<div class="subcat_2row_pdt_name"><?=$HTML_subshopname?></div>
				...
				?>
					...
					while($row_prod = $db->fetch_array($ret_prod))
					{
						...
						if ($shop_det['shopbrand_product_showprice']==1)// Check whether description is to be displayed
						{
							...
							$HTML_price = show_Price($row_prod,$price_class_arr,'cat_detail_1');
						}
						if($row_prod['product_bulkdiscount_allowed']=='Y')
						{
							...
							$HTML_bulk = '&nbsp;';
						if($row_prod['product_bonuspoints']>0 and $shop_det['shopbrand_product_showbonuspoints']==1)// Check whether description is to be displayed)
						{
				...
				?>
					...
					while($row_prod = $db->fetch_array($ret_prod))
					{
						...
						if ($shop_det['shopbrand_product_showprice']==1)// Check whether description is to be displayed
						{
							...
							$price_class_arr['yousave_class']       = 'normal_shlfB_pdt_priceC';
							$price_class_arr['discount_class']      = 'normal_shlfB_pdt_priceC';
							$HTML_price = show_Price($row_prod,$price_class_arr,'cat_detail_3');
						}
						if($row_prod['product_bulkdiscount_allowed']=='Y')
						{
							$HTML_bulk = '<img src="'.url_site_image('multi-buyA.gif',1).'" />';
