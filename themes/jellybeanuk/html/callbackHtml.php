						...
						<div class="reg_shlf_cont_div">
							...
							<table width="100%" border="0" cellpadding="0" cellspacing="3"  class="reg_table">
								...
								<tr>
									...
									<?PHP 
									// Get default country id from general settings table
									$sql_genset = "SELECT default_country_id FROM general_settings_sites_common WHERE sites_site_id = $ecom_siteid LIMIT 1";
									$ret_genset = $db->query($sql_genset);
									if($db->num_rows($ret_genset))
									{
										$row_genset = $db->fetch_array($ret_genset);
										$def_countryid = $row_genset['default_country_id'];
									}
									...
									while($row = $db->fetch_array($res)) {
										if ($_POST['callback_country']=='')
											$selected = ($row['country_id']==$def_countryid)?'selected':'';
										else
											$selected = ($row['country_id']==$_POST['callback_country'])?'selected':'';	
