			  ...
			  <tr>
				...
				<?php 
					 ...
					 <?php
					  ...
					  {
					  ?>
					   <td class="cart_table_header" width="30%"><?php echo stripslash_normal($Captions_arr['CART']['CART_PRICE'])?>  </td>

					  <td class="cart_table_header"  width="10%"></td>
					  <?php
					  }
				?>	  
			...
			<tr>
				...
				<?php 
				     ...
				     <?php
					  ...
					  {
					  ?>
					  <td align="left" valign="middle" class="cart_td_discnt"></td>						  
					<?php						  
					   }
				     ?>
		  ...
		  ?>  
				...
				<input type="hidden" name="cart_delivery_id" id="cart_delivery_id" value="<?php echo $deliverydet_Arr['delivery_id']?>"  />	
					...
					if($row_cartdet['location_id'] and $ecom_site_delivery_location_country_map==1)
					{
						...
						{
					?>	    
							<div class="countrylist_image_div" onclick="handle_countrylist('<?php echo $row_cartdet['location_id']?>')"><img src="<?php url_site_image('countrylist.gif')?>" border="0" title="Click to view the country list"/></div>
							<div id='countrylist_div' class='countrylistshow_div' style='display:none;'>
							<table width="100%" cellpadding='0' cellspacing='1' border='0'>
							<tr>
								<td colspan="2" align='left' class='countrylist_header'>List of Countries in this Region</td>
							</tr>
							<?php
							$max_col = 2;
							$cur_col = 0;
					...
					<?php
					if($ecom_siteid == 87) //sumpandpump
					{
						if($Captions_arr['CART']['CART_BOTTOM_ADD_MSG']!='')
						{
					?>
							<tr>
								<td colspan="2" align="center" valign="middle" class=""><div><a href="<?php url_link('callback.html')?>" class="cart_additional_msg"><?php echo $Captions_arr['CART']['CART_BOTTOM_ADD_MSG']?></a></div></td>
							</tr>
					<?php
						}
					}
...
// Show the payment type and payment method sections only if total amount > 0
				...
				if($db->num_rows($ret_paytypes)==1)// Check whether there are more than 1 payment type. If no then dont show the payment option to user, just use hidden field
					{
						...
						{
							if($ecom_siteid==62) // case of eurolabels
							{
								if(!$row_cartdet['paytype_id'])
									$cc_exists = true;
								else
								{
 ...
 <?php
		...
		if (count($cartData['products'])>0)
		{
			if($hide_direct_checkout_button==false)// today start
			{
				if($show_checkoutbutton==true)	
				{
					if($ecom_siteid==62)
					{
						$cap = '<img src="'.url_site_image('credit_debit_card_new.jpg',1).'" border="0">';
						$outercls = 'cart_shop_chkout_euro';
					}	
					else
					{
						$cap = stripslash_normal($Captions_arr['CART']['CART_GO_CHKOUT']);
						$outercls = 'cart_shop_chkout';
					}	
					
						 $HTML_bottomdesc .= '<div class="'.$outercls.'"><div> <a href="#" onClick="handle_checkout_submit(\''.$ecom_hostname.'\',0,\''.$cartData["totals"]["bonus_price"].'\')">'.$cap.'</a></div></div>';
				}
				else
				{
		...
		if($ps_url and $ecom_siteid!=62) // show the continue shopping button only if ps_url have value
		{
    ...
    <div class="cart_cont_inner_bottom"></div>
		...
		function Show_Checkout()
		{
			global $ecom_siteid,$db,$ecom_hostname,$Settings_arr,$ecom_themeid,$default_layout,$Captions_arr,
					$inlineSiteComponents,$Settings_arr,$protectedUrl,$ecom_common_settings,
					$image_path,$ecom_site_delivery_location_country_map;
			$session_id 			= session_id();	// Get the session id for the current section
			$cust_id				= get_session_var("ecom_login_customer"); // Get the customer id from session
			$Captions_arr['CART'] 	= getCaptions('CART'); // Getting the captions to be used in this page
										  ...
										  $prod_cart_arr =array();
											  ...
											  ?>
												<tr class="cart_table">
													...
													<td align="left" valign="middle" class="cart_td_qty"><label><strong>
														<?php 
														if($products_arr['product_det_qty_type']=='DROP')
														{
															if (trim($products_arr['product_det_qty_drop_prefix'])!='')
										...
										<div class="chk_credit_outr">
											...
											<tr>
												<td  align="left" valign="middle" class="cartterms">			
													...
													{
														if($ecom_siteid==87)//for sumandpump
														{
															$terms = '<a href="http://sumpandpump.bshop4.co.uk/terms-and-conditions-pg49864.html" target ="_blank" class="">'.stripslash_normal($Captions_arr['CART']['CART_ACCEPT_TERMS_CONDITIONS']).'</a>';
														}
														else
														{
															$terms = stripslash_normal($Captions_arr['CART']['CART_ACCEPT_TERMS_CONDITIONS']);

														}
													?>
														<br /><div class="terms_link"><input class="shoppingcart_radio" type="checkbox" name="cart_terms" id="cart_terms" value="1" /><?php echo $terms ?></div>
													<?php	
													}
													?>	
												<br /></td>
												...
												<?php
													...
													?>	
														...
														?>	
															...
															{
																$divcls = 'cart_top_links_euro';
																$checkout_caption = "Continue Checkout";
															}	
															else
																$divcls = 'cart_top_links';
														?>
				...
				if($ecom_site_delivery_location_country_map==1)
				{
					if($row_cartdet['location_id']) // case if delivery location is selected
					{
						...
						echo '
								...
								{
									alert("Country selected in Delivery Address does not match with the Delivery location selected in cart page.\n\n Please select the correct country or go back to cart page to view the list of countries in the selected Delivery Location.");
									return false;	
								}
						';
					}
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="shoppingcarttable">
			  ...
			  <tr>
				...
				if ($ecom_siteid==87) // case of sumpandpump
				{
					if ($order_id>0)
					{
						// check whether the payment option selected is payonphone. If yes then show the special message otherwise show the normal message
						$sql_ordp = "SELECT order_paymenttype FROM orders WHERE order_id = $order_id LIMIT 1";
						$ret_ordp = $db->query($sql_ordp);
						if($db->num_rows($ret_ordp))
						{
							$row_ordp = $db->fetch_array($ret_ordp);
							if($row_ordp['order_paymenttype'] == 'invoice' or $row_ordp['order_paymenttype'] == 'pay_on_phone' )
								echo stripslash_normal($Captions_arr['CART']['CART_CHECKOUT_SUCCESS_MSG_SPECIAL']). '<br/><br/>'.stripslash_normal($Captions_arr['CART']['CART_CHECKOUT_SUCCESS_MSG_MORE']);	
							else
