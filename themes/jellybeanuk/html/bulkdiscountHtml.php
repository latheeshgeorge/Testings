		...
		if ($db->num_rows($ret_prod))
		{
			...
			if ($tot_cnt>0)
			{
				...
				$HTML_topcontent .= '
					...
					{
						$HTML_tophead .='<div class="normal_bulkA_hd"><span>'.str_replace('[tot_cnt]',$tot_cnt,stripslashes($Captions_arr['BULKDISC_PROD']['BULKDISC_PROD_TOPMSG'])).'</span></div>';	
					}	
			}	
			echo $HTML_treemenu;
			echo $HTML_tophead;
			...
			echo $HTML_paging;			
					...
					while($row_prod = $db->fetch_array($ret_prod))
					{
						...
						if($HTML_bonus!='' or $HTML_bulk !='' or $HTML_bulk!='')
						{
