		...
		function Show_Products($ret_prod,$tot_cnt,$start_var,$cat_det,$def_orderfield,$def_orderby)
		{
			...
			switch($cat_det['product_displaytype'])
			{
				case '3row': // case of three in a row for normal
					$HTML_comptitle = $HTML_maindesc = $HTML_paging1 = $HTML_showall = '';
				?>
			...
			$HTML_bottomcontent = 	'<div class="subcat_nav_content" >
								'.$HTML_totcnt.'
								<div class="subcat_nav_top"></div>
								<div class="subcat_nav_bottom">
								<div class=" page_nav_cont">
								...
								$selval_arr = array (
														...
														'price'			=> stripslash_normal($Captions_arr['CAT_DETAILS']['CATDET_PRICE']),
														'product_id'	=> stripslash_normal($Captions_arr['CAT_DETAILS']['SHOPDET_DATEADDED']));
			$HTML_bottomcontent .=	generateselectbox('catdet_sortbybottom',$selval_arr,$prodsort_by,'','',0,'',false,'catdet_sortbybottom');
								$selord_arr = array (
														'ASC'	=> stripslash_normal($Captions_arr['CAT_DETAILS']['CATDET_LOW2HIGH']),
														'DESC'	=> stripslash_normal($Captions_arr['CAT_DETAILS']['CATDET_HIGH2LOW']) 
													);
			$HTML_bottomcontent .=	generateselectbox('catdet_sortorderbottom',$selord_arr,$prodsort_order,'','',0,'',false,'catdet_sortorderbottom');
			$HTML_bottomcontent .=	'								
								</div>
								</div>
								<div class=" page_nav_contA">
			...
			for ($ii=$Settings_arr["productlist_interval"];$ii<=$Settings_arr["productlist_maxval"];$ii+=$Settings_arr["productlist_interval"])
				$perpage_arr[$ii] = $ii;
			$HTML_bottomcontent .=	generateselectbox('catdet_prodperpagebottom',$perpage_arr,$catdet_prodperpage,'','',0,'',false,'catdet_prodperpagebottom');
			$HTML_bottomcontent .= '
								</div>
								</div>
								<div class=" page_nav_contB">
								<input type="button" name="submit_Page" value="'.stripslash_normal($Captions_arr['CAT_DETAILS']['CATDET_GO']).'" class="nav_button" onclick="handle_categorydetailsdropdownval_sel(\''.url_category($_REQUEST['category_id'],stripslash_javascript($cat_det['category_name']),4,$_REQUEST['catgroup_id'],1).'\',\'catdet_sortbybottom\',\'catdet_sortorderbottom\',\'catdet_prodperpagebottom\')" />
								</div>
								</div>
								</div>';
			...
			switch ($row_group['catgroup_listtype'])
			{
				case 'Menu':
					...
					while ($row_subcat = $db->fetch_array($ret_category))
					{
						...
						if($ecom_siteid==62)
						{
							$pass_type = 'image_thumbcategorypath'; // case of eurolabels
						}	
						if ($row_subcat['category_showimageofproduct']==0) // Case to check for images directly assigned to category
						{
							// Calling the function to get the image to be shown
