<?php
	...
	class shop_Html
	{
	  ...
	  <?php
			...
			if($_REQUEST['shop_id'])
			  {
			$HTML_icons .= '<a  href="javascript:download_pdf()" title="'.stripslash_normal($Captions_arr['SHOP_DETAILS']['SHOPDET_DOWNLOADPDF']).'"><img src="'.url_site_image('pdfico.gif',1).'" alt="'.stripslash_normal($Captions_arr['SHOP_DETAILS']['SHOPDET_DOWNLOADPDF']).'" border="0" /></a>';
			}
			$SHOP_desc = stripslashes($row_shop['shopbrand_description']);
			if ($SHOP_desc!='')
			{
			$HTML_shopdesc = '<div class="cate_content_bottom">'.
			$SHOP_desc.'</div>';
			}
			$subshopshow 	    = false;
			if($_REQUEST['shop_id'])
			...
			<div class="sub_cate_hdr"><h1><?php echo stripslashes($row_shop['shopbrand_name'])?></h1></div>
			...
			if($prodshow)  // Checks whether products to be shown in the middle area
			{
				...
				if ($tot_cnt>0)
				{
					...
					$sql_prod = "SELECT a.manufacture_id,a.product_id,a.product_name,a.product_variablestock_allowed,a.product_show_cartlink,
			...
			if(in_array('mod_shopimage',$inlineSiteComponents))
			{
		       ...
		       if($_REQUEST['type_cat']=='sub_shop'){
				...
				$HTML_topdesc ='';	
					...
					{
					$HTML_topdesc .='
						<div class="cate_content_bottom" >'.$fetch_arr_admin['general_shopsall_topcontent'].'
							</div>		
						';
					}
				}
				...
				$cur_col = 0;
					...
					while ($row_subshop = $db->fetch_array($ret_subshop))
					{
						...
						$cur_col++;
						$HTML_subshopname = '<a href="'.url_shops($row_subshop['shopbrand_id'],$row_subshop['shopbrand_name'],1).'" title="'.stripslash_normal($row_subshop['shopbrand_name']).'">'.stripslash_normal($row_subshop['shopbrand_name']).'</a>';
						$HTML_short_desc = stripslashes($row_subshop['shopbrand_description']);
						$HTML_image = '<a href="'.url_shops($row_subshop['shopbrand_id'],$row_subshop['shopbrand_name'],1).'" class="subcategoreynamelink" title="'.stripslash_normal($row_subshop['shopbrand_name']).'">';
						$pass_type = 'image_thumbcategorypath';
						if ($row_subshop['shopbrand_showimageofproduct']==0) // Case to check for images directly assigned to shop
							{ 
								// Calling the function to get the image to be shown
								$img_arr = get_imagelist('prodshop',$row_subshop['shopbrand_id'],$pass_type,0,0,1);
								if(count($img_arr))
							{
								$HTML_image .=show_image(url_root_image($img_arr[0][$pass_type],1),$row_subshop['shopbrand_name'],$row_subshop['shopbrand_name'],'','',1);
								$show_noimage = false;
							}
							else
								$show_noimage = true;
							}
							else // Case of check for the first available image of any of the products under this shop
							{
								// Calling the function to get the id of products under current shop with image assigned to it
							$shop_id = $row_subshop['shopbrand_id'];
							$cur_prodid = find_AnyProductWithImageUnderShop($shop_id);
							if ($cur_prodid)// case if any product with image assigned to it under current shop exists
								{
									// Calling the function to get the image to be shown
									$img_arr = get_imagelist('prod',$cur_prodid,$pass_type,0,0,1);
									if(count($img_arr))
									{
										$HTML_image .=show_image(url_root_image($img_arr[0][$pass_type],1),$row_subshop['shopbrand_name'],$row_shop['shopbrand_name'],'','',1);
										$show_noimage = false;
									}
									else// case if no products exists under current shop with image assigned to it
									$show_noimage = true;
							...
							if ($show_noimage)
							{
								...
								if ($no_img)
								{
									$HTML_image .=show_image($no_img,$row_subshop['shopbrand_name'],$row_subshop['shopbrand_name'],'','',1);
								}	
							}
						$HTML_image .= '</a>';
					...
					{
						$HTML_bottomdesc .='
						<div class="cate_content_bottom" >'.$fetch_arr_admin['general_shopsall_bottomcontent'].'
							</div>		
						';
					}
					?>
							<div class="subcat_3row_pdt_outr">
							<div class="subcat_3row_pdt_name"><?=$HTML_subshopname?></div>
		...
		function Show_Products($ret_prod,$tot_cnt,$start_var,$shop_det)
		{
			...
			for ($ii=$Settings_arr["productlist_interval"];$ii<=$Settings_arr["productlist_maxval"];$ii+=$Settings_arr["productlist_interval"])
				$perpage_arr[$ii] = $ii;
			$HTML_topcontent .=	generateselectbox('shopdet_prodperpagetop',$perpage_arr,$catdet_prodperpage,'','',0,'',false,'shopdet_prodperpagetop');
			$HTML_topcontent .= '
								</div>
								</div>
				...
				?>
					...
					while($row_prod = $db->fetch_array($ret_prod))
					{
						...
						$HTML_price = $HTML_bulk= $HTML_bonus = $HTML_compare = $HTML_freedel= $HTML_bonus_bar = '';
						    ...
						    {
								if(in_array($_REQUEST['shop_id'],$chek_cat_arr))
								{
									$pname = $row_prod['manufacture_id'];
								}
								else
								{
									$pname = $row_prod['product_name'];
								}
							$HTML_title = '<div class="normal_shlf_listA_pdt_name"><a href="'.url_product($row_prod['product_id'],$row_prod['product_name'],1).'" title="'.stripslash_normal($pname).'">'.stripslash_normal($pname).'</a></div>';
							}
							if ($shop_det['shopbrand_product_showimage']==1)// Check whether description is to be displayed
							{
								...
								if(count($img_arr))
								{
									$HTML_image .= show_image(url_root_image($img_arr[0][$pass_type],1),$pname,$pname,'','',1);
								}
								else
								{
									...
									if ($no_img)
									{
										$HTML_image .= show_image($no_img,$pname,$pname,'','',1);
									}       
								}       
								$HTML_image .= '</a>';
							}
					...
					?>
						...
						<div class="normal_shlf_listA_pdt_moreinfo"><a href="<?php url_product($row_prod['product_id'],$row_prod['product_name'])?>">More Details</a></div>                                    
						</div>
					<?
						$cur_col++;
						if($cur_col>=$max_col)
					...
					<div class="normal_shlf_list">
						...
						while($row_prod = $db->fetch_array($ret_prod))
						{
							...
							$HTML_price = $HTML_bulk= $HTML_bonus = $HTML_compare = $HTML_freedel= $HTML_bonus_bar = '';
								...
								$HTML_title = '<div class="normal_shlf_listB_pdt_name"><a href="'.url_product($row_prod['product_id'],$row_prod['product_name'],1).'" title="'.stripslash_normal($pname).'">'.stripslash_normal($pname).'</a></div>';
								}
								if ($shop_det['shopbrand_product_showimage']==1)// Check whether description is to be displayed
								{
									$HTML_image ='<a href="'.url_product($row_prod['product_id'],$row_prod['product_name'],1).'" title="'.stripslash_normal($row_prod['product_name']).'">';
