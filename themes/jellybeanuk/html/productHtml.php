<?php
	...
	class product_Html
	{
		...
		function Show_ProductDetails($row_prod)
		{
			...
			{ 
				$HTML_pdf = '<div><a href="javascript:download_pdf_common(\''.$_SERVER['HTTP_HOST'].'\',\''.$_SERVER['REQUEST_URI'].'\')" title="'.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_DOWNLOADPDF']).'"><img src="'.url_site_image('det-icon_09.gif',1).'" border="0" /> '.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_DOWNLOADPDF']).'</a></div>';
			}
...
{
	global $Captions_arr,$showqty,$Settings_arr,$ecom_siteid,$db,$glob_qty_displayed,$ecom_hostname;
	$addto_cart_withajax =  $Settings_arr['enable_ajax_in_site'];
	$cust_id 	= get_session_var("ecom_login_customer");
	$showqty	= $Settings_arr['show_qty_box'];// show the qty box
	...
	if ($caption_key!='') // Check whether the buynow or preorder button is to be displayed
	{
			...
			<?php
			 ...
			 if($ecom_siteid == 62)
				 {
					 ?>
					 <?php if($Captions_arr['PROD_DETAILS']['PRICE_UK']!='')
					 {
					 ?>
					 <div class="deat_pdt_buy_uk"><?php echo str_replace('Please call +44 (0)1728 652120','<br/>Please call <strong>+44 (0)1728 652120</strong>',$Captions_arr['PROD_DETAILS']['PRICE_UK']); ?></div>
					 <?php
					 }
			     }
			    ?> 
			<div class="deat_pdt_buyB">
				<div class="deat_pdt_buyBinner">
					...
					if($addto_cart_withajax == 1)
					{
				?><a href="javascript:void(0);" class="det_buy_link" onclick="ajax_addto_cart_fromlist('add_prod_tocart_ajax','Prod_Addcart','frm_proddetails','<?php echo SITE_URL?>')"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS'][$caption_key]);?></a>
				<?php
					}
					else
					{
					?>
                    <a href="#" class="det_buy_link" onclick="show_wait_button(this,'Please Wait...');document.frm_proddetails.fpurpose.value='Prod_Addcart';document.frm_proddetails.submit();"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS'][$caption_key]);?></a>
                    <?php
					}
                    ?></div>
				</div>
			...
			?>
				<div class="deat_pdt_buyB">
					...
					<div class="buyBinner_link">
                    <?php
					$stok_video_exists = false;
					$sql_stok_prod = "SELECT product_stokke_exists,product_stokke_video_url FROM products WHERE product_id = ".$row_prod['product_id']." LIMIT 1";
					$ret_stok_prod = $db->query($sql_stok_prod);
					if($db->num_rows($ret_stok_prod))
					{
						$row_stok_prod = $db->fetch_array($ret_stok_prod);
						$stok_video_exists = ($row_stok_prod['product_stokke_exists']==1)?true:false;
						$stok_video_url = stripslashes($row_stok_prod['product_stokke_video_url']);
					}
					if($addto_cart_withajax == 1 and !$stok_video_exists)
					{
				?>		<a href="javascript:void(0);" class="det_buy_link" onclick="ajax_addto_cart_fromlist('add_prod_tocart_ajax','Prod_Addcart','frm_proddetails','<?php echo SITE_URL?>')"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS'][$caption_key]);?></a>
				<?php
					}
					...
					{
						if($stok_video_exists)
						{
							$stok_video_url .= "&RETURNSUCCESS=http://".$ecom_hostname."/stok_return.php";
							?>
							<script type="text/javascript">
							function stokke_win()
							{
								 window.open('<?php echo $stok_video_url?>', 'video_viewer','height=480,width=600,status=no,toolbar=no,menubar=no,location=no,scrollbars=yes');
							}
							</script>
							<a href="#" class="det_buy_link" onclick="stokke_win()"><?php echo stripslash_normal($Captions_arr['PROD_DETAILS'][$caption_key]);?></a>
							<?php
