										...
										if ($grpData['catgroup_show_subcat_indropdown']== 1 and $subcatdropdownsupport)// case if categories are to be shown in list menu
										{
											...
											<?php
												while ($row_cat = $db->fetch_array($ret_cat))
												{
													...
													{
													?>
													<li class="item1"><a href="<?php url_static_page(50475,$row_cat['category_name'],-1)?>" class="category_menu" title="<?php echo stripslash_normal($row_cat['category_name'])?>"><span><?php echo ucwords(stripslash_normal($row_cat['category_name']));?>1</span></a>
													<?php	
													}
													else
													{
			...
			?>
			 <form method="post" name="frm_quicksearch" class="frm_cls" action="<?php url_link('search.html')?>">
                   ...
                   <input type="hidden" name="rdo_suboption" id="rdo_suboption" value='title_desc'/>
						...
						<tr>
						<td  class="td_D" ><label>
							<input name="quick_search" type="text" class="input_s" id="quick_search"  value="SEARCH DINCWEAR" onFocus="if (this.value=='SEARCH DINCWEAR') this.value='';" onBlur="if (this.value=='') this.value='SEARCH DINCWEAR';"/>					
						</label></td>
