	...
	class mydownloads_Html
	{
		...
		function Show_Mydownloads($alert)
		{
			...
			$sql_download						= "SELECT a.ord_down_id,b.proddown_title,a.proddown_limited,a.proddown_limit,a.proddown_days_active,a.order_details_orderdet_id,
				...
				{
					case 1:
						$alert = $Captions_arr['MY_DOWNLOADS']['DOWN_LOGIN'];
					break;
					case 2:
						$alert = $Captions_arr['MY_DOWNLOADS']['DOWN_INV_CUST_ERR'];
					break;
					case 3:
						$alert = $Captions_arr['MY_DOWNLOADS']['DOWN_PAY_NOT_ERR'];
					break;
					case 4:
						$alert = $Captions_arr['MY_DOWNLOADS']['DOWN_CANCEL_ERR'];
					break;
					case 5:
						$alert 	= $Captions_arr['MY_DOWNLOADS']['DOWN_INV_INPUT_ERR'];
					break;
					case 6:
						$alert = $Captions_arr['MY_DOWNLOADS']['DOWN_DISABLED_ERR'];
					break;
					case 7:
						$alert = $Captions_arr['MY_DOWNLOADS']['DOWN_NO_AUTH'];
					break;
					default:
						$alert = '';
				};
?>
			<div class="treemenu"><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> >> <?=$Captions_arr['MY_DOWNLOADS']['DOWN_HEADER']?></div>
		<table width="100%" border="0" cellpadding="0" cellspacing="4" class="tbl_float">
		
		<tr>
			<td colspan="3" class="prod_orderheader"><?=$Captions_arr['MY_DOWNLOADS']['DOWN_HEADER']?></td>
		</tr>
		<tr>
		<td colspan="3">
			...
			?>
					<tr>
					<td  colspan="8" align="center" valign="middle" class="red_msg"><?php echo $alert?></td>
					</tr>
			<?php
				}
			?>
			...
			if($db->num_rows($ret_download))
			{
				...
				$query_string='';
				paging_footer($path,$query_string,$tot_cntdownload,$start_vardownload['pg'],$start_vardownload['pages'],'',$pg_variabledownload,$Captions_arr['MY_DOWNLOADS']['DOWN_DOWNLOAD'],$pageclass_arr); 
			}	
		?>			</td>
			</tr>
				<tr>
					<td align="left" width="3%" class="ordertableheader">
					<?=$Captions_arr['MY_DOWNLOADS']['DOWN_SLNO']?></td>
					<td align="left" width="10%" class="ordertableheader">
					<?=$Captions_arr['MY_DOWNLOADS']['DOWN_ORDID']?></td>
					<td align="left" width="24%" class="ordertableheader">
					<?=$Captions_arr['MY_DOWNLOADS']['DOWN_TITLE']?></td>
					<td align="center" width="14%" class="ordertableheader">
					<?=$Captions_arr['MY_DOWNLOADS']['DOWN_LIMIT']?></td>
					<td align="left" width="14%" class="ordertableheader">
					<?=$Captions_arr['MY_DOWNLOADS']['DOWN_START']?></td>
					<td align="left" width="23%" class="ordertableheader">
					<?=$Captions_arr['MY_DOWNLOADS']['DOWN_END']?></td>
			...
			{ ?>
			<tr>
					<td colspan="8" align="center" class="userorderheader">&nbsp;<?=$Captions_arr['MY_DOWNLOADS']['DOWN_NOT_FOUND']?></td>
			  </tr>
			<?PHP } else {
				$i=1;
				...
				?>
					<tr class="edithreflink_tronmouse" title="<?php echo $Captions_arr['MY_DOWNLOADS']['DOWN_DOWNLOAD']?>" onmouseover="this.className='edithreflink_trmouseout'" onmouseout="this.className='edithreflink_tronmouse'">
						...
						<a href="index.php?req=orders&reqtype=order_det&order_id=<?php echo $row_query['order_id']?>" class="favoriteprodlink" title="<?php echo $Captions_arr['MY_DOWNLOADS']['DOWN_VIEW_ORD']?>"><?php echo $row_query['order_id']?></a></td>
