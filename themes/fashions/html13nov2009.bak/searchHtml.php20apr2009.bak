											...
											while ($row_search = $db->fetch_array($ret_search))
											{
												...
												<td>
													...
													<div class="search_image">
																	...
																	{
																	?>															
																		<div class="compare_li"><?php dislplayCompareButton($row_search['product_id']);?></div>
																<?php
																	}
																	?>
													</div>
													<div class="search_content">
													<div class="search_name"><a href="<?php url_product($row_search['product_id'],$row_search['product_name'],-1)?>" title="<?php echo stripslashes($row_search['product_name'])?>"><?php echo stripslashes($row_search['product_name'])?></a></div>
													<div class="search_des"><?php echo stripslashes($row_search['product_shortdesc'])?></div>
													...
													<label><?php show_moreinfo($row_search,'product_info')?></label>
													...
													<input type="hidden" name="pass_url" value="<?php echo $_SERVER['REQUEST_URI']?>" />
														<?php
															...
															$prefix = '<div class="product_list_button_list">';
															$suffix = '</div>';
															show_addtocart($row_search,$class_arr,$frm_name,false,$prefix,$suffix);
														?>
													</form>
													</div>
							...
							<table width="100%" border="0" cellspacing="1" cellpadding="0">
									...
									<tr>
										...
										foreach ($search_rel as $k=>$search_values)
										{
											 $val++;
											 ?>
												<td align="center" class="search_relatedtd"><a href="<? url_link('s'.$search_values['search_id'].'/'.strip_url($search_values['search_keyword']).'.html')?>" class="link">
											 <? 
		...
		function Show_Search_Category($search_sql,$tot_cnt)
		{
			...
			$first_querystring=$query_string; //Assigning the initial string to the variable.
				        ...
				        if ($_REQUEST['req']!='')// LIMIT for products is applied only if not displayed in home page
						{
							...
							{
						  ?>
										<table border="0" cellpadding="0" cellspacing="0" class="tbl_float" width="100%">
										<tr>
												<td colspan="3" class="treemenu" align="left"><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a><? if($_REQUEST['quick_search']!=""){?> >> <? echo $_REQUEST['quick_search']; } ?> </td>
										</tr>
										<tr>
										...
										<?php
											...
											?>
													...
													<div class="category_name"><a href="<?php url_category($row_search['category_id'],$row_search['category_name'],-1)?>" title="<?php echo stripslashes($row_search['category_name'])?>"><?php echo stripslashes($row_search['category_name'])?></a></div>
													<div class="category_des"><?php echo stripslashes($row_search['category_shortdescription'])?></div>
													</div>
													</div>
													</td>
													</tr>
