...
var outInterval = 0;
var maxcnt = 0;
function handle_expand_curr_div(obj,totheight)
{
	maxcnt = totheight;
	if(curr_cur_cnt==20)
	{
		curr_cur_cnt =20;
		obj.className='normalscurrency_hover';
		intInterval = setInterval("increment_height()",50);
	}	
}
function increment_height()
{
	if (curr_cur_cnt <maxcnt)
	{
		curr_cur_cnt+=20;
		document.getElementById('curr_maindiv').style.height = curr_cur_cnt+'px';
	}
	else
	{
...
function handle_collapse_curr_div(obj)
{
	if(curr_cur_cnt>20)
	{
		obj.className='normalscurrency_hover';
		outInterval = setInterval("decrement_height()",50);
	}	
}
...
function handle_normal_curr_div()
{
	if (document.getElementById('curr_maindiv'))
	{
			clearInterval(intInterval);	
			document.getElementById('curr_maindiv').className='div-normal';
	}	
}
function handle_currency_select_link(vals)
{
	if (document.frm_topcurrency.cbo_selcurrency)
	{
		document.frm_topcurrency.cbo_selcurrency.value = vals;
		document.frm_topcurrency.submit();
	}
}
