<?php
	...
	if ($_REQUEST['product_id']) // If specific product is selected
	{
		$sql_grp = "SELECT a.group_id,a.group_name,group_hide,group_hidename,
							group_showhomelink,group_showsitemaplink,group_showhelplink,group_showsavedsearchlink,group_showxmlsitemaplink  
					FROM 
	...
	/*if (count($grp_array)==0)// to handle the case of not found in above queries
	{
		//Get the static page group with position = Top for this site and which is not hidden and also all_pages = 1
		$sql_grp = "SELECT group_id,group_name,group_hide,group_hidename,
							group_showhomelink,group_showsitemaplink,group_showhelplink,group_showsavedsearchlink,group_showxmlsitemaplink   
					FROM 
						static_pagegroup 
