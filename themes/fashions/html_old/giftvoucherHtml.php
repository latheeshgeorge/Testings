...
/*############################################################################
	...
	class giftvoucher_Html
	{
		...
		function Buy_Giftvoucher($alert='')
		{
			...
			if($protectedUrl)
				$http = url_protected('buy_voucher.html',1);
			else 	
							  ...
							  <div class="shoppaymentdiv">
								<table width="100%" border="0" cellspacing="0" cellpadding="0">
								  ...
								  ?>
										...
										<input class="shoppingcart_radio" type="radio" name="voucher_paytype" id="voucher_paytype" value="<?php echo $row_paytypes['paytype_id']?>" onclick="<?php echo $paytype_onclick?>" <?php echo ($_REQUEST['voucher_paytype']==$row_paytypes['paytype_id'])?'checked="checked"':''?> /><?php echo stripslashes($row_paytypes['paytype_name']).$add_text?>
										</td>
								<?php
										$pay_cnt++;
