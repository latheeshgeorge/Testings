		...
		?>
			...
			$count_sav=1;
				while ($row_search = $db->fetch_array($ret_search))
				{
				  $count_sav++;						
			?>					
					<td valign="middle" align="left" width="33%" class="sitemapcontents"> 
					<h1><a href="<?php url_savedsearch($row_search['search_id'],$row_search['search_keyword'])?>" title="<?=$row_search['search_keyword']?>" class="sitemapcatlink"><?=$row_search['search_keyword']?></a></h1> </td>
			<?php
