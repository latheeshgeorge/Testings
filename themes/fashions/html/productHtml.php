	...
	class product_Html
	{
		function Show_ProductDetails($row_prod)
		{
			global $Captions_arr,$ecom_siteid,$db,$ecom_hostname,$ecom_themename,
					 $ecom_themeid,$default_layout,$inlineSiteComponents,$Settings_arr,$components,$ecom_twitteraccountId;
			...
			<? 
				if($prod_img_show_type=='FLASH') // case of showing images in a flash container
				{
					...
					<script type="text/javascript">
						...
						<?php /*?>document.write('<param name="movie" value="http://<?php echo $ecom_hostname?>/themes/<?php echo $ecom_themename?>/swf/xml4.swf?filepath=http://<?php echo $ecom_hostname?>/getflashimage_images.php?prod_tab_id=<?=$prod_tab_id?>~ico" />');<?php */?>
						document.write('<param name="movie" value="http://<?php echo $ecom_hostname?>/images/<?php echo $ecom_hostname?>/swf/xml4.swf?filepath=http://<?php echo $ecom_hostname?>/getflashimage_images.php?prod_tab_id=<?=$prod_tab_id?>~ico" />');
		...
		if ($db->num_rows($ret_other))
		{
			...
			{
			?>
				<li class="det_stock"><?php echo $stk_det;?></li>
			<?php
			}
			...
			$var_listed = $this->show_ProductVariables($row_prod,'row');
					if($Settings_arr['showsizechart_in_popup']==1 and (is_array($heading))) // check whether size chart is set to show in a pop up window
					{
					?>
		...
		?>
			...
			{
			?>	
				<a href="<?php url_link('freedelivery'.$row_prod['product_id'].'.html')?>" title="Free Delivery"><img src="<?php url_site_image('free-delivery.gif')?>" border="0" /></a>
			<?php
			}
...
?>
		...
		if($Settings_arr['showsizechart_in_popup']!=1 and count($heading)) // check whether size chart is set to show in a pop up window
		{
			$this->show_Sizechart($heading,$cnt_hd,$sizevalue);
		}		
		?>	
		</form>
		<?php
...
<?php	
		...
		function show_ProductLabels1($pos='row')
		{
			global $db,$ecom_siteid,$Captions_arr;
							...
							if(count($label_arr))
							{
								...
								foreach ($label_arr as $k=>$v)
								{
									if($prev_grp!=$k)
									{
										...
										if($gname_arr[1]==0)
										{
											$ret_val .='<li class="keyfeatureHeading">'.$gname_arr[0].'</li>';
										}			
									}	
									if(is_array($v))
									{
  ...
  <?php
	...
	<?php	
						...
						<div class="content_product_images">
							<div >
								...
								?>
										...
										<script type="text/javascript">
		<?php /*?>var s1 = new SWFObject("http://<?php echo $ecom_hostname?>/themes/<?php echo $ecom_themename?>/swf/player.swf","ply","260","370","9","#FFFFFF");<?php */?>
		var s1 = new SWFObject("http://<?php echo $ecom_hostname?>/images/<?php echo $ecom_hostname?>/swf/player.swf","ply","260","370","9","#FFFFFF");
		...
		/*	function Show_StockNotify($id,$alert) 
			...
			function show_Sizechart($heading,$cnt_hd,$sizevalue)
			{
				global $db,$ecom_siteid,$ecom_hostname,$ecom_themename;	
				if(count($sizevalue))
				{
