			...
			?>
			  ...
			  <div class="checkou_div" align="right">
	            <ul class="topcartlink">
                	 ...
                	 <li class="check_out"><a href="#"  onclick="handle_checkout_submit('<?php echo $ecom_hostname?>',0,'<?php echo $pass_tot?>')" class="viewcart" title="<?php echo $Captions_arr['COMMON']['CHECK_OUT']?>"><?php echo $Captions_arr['COMMON']['CHECK_OUT']?></a></li>
			  ...
			  <?php
			  // get the list of currencies in the site
			  	$curr_arr 		= get_currency_list();
				$curr_cnt		= count($curr_arr);
				$max_height 	= 25+ $curr_cnt * 20  ;
				if($curr_cnt>1)
				{
			  ?>
				  <form method="post" id="frm_topcurrency" name="frm_topcurrency" class="frm_cls">
				  <input type="hidden" name="cbo_selcurrency" value="" />
				  <div class="div-normal"  id="curr_maindiv">
					<div style="float:left" onclick="handle_expand_curr_div(document.getElementById('curr_maindiv'),<?php echo $max_height?>)"><img src="<?php url_site_image('currency_change.gif')?>" alt="Select Currency" /></div>
					<?php
						foreach ($curr_arr as $k=>$v)
						{
					?>
							<div class="inner"><a href="javascript:handle_currency_select_link(<?php echo $k?>)" class="ch_cur"><?php echo stripslashes($v)?></a></div>
					<?php
						}					
					?>
				<div onclick="handle_collapse_curr_div(document.getElementById('curr_maindiv'))" class="curr_divclose"><img src="<?php url_site_image('currency_close.gif')?>" /></div>
				</div>
				</form>
		<?php
				}	
		...
		if($Settings_arr['shownewsletter_as_banner']==1) // Decide whether customer login is to be displayed as banner or in normal style
			{
				...
				<div class="nws_div">
				<a href="<?php url_link('newsletter.html')?>"><img src="<?php url_site_image('neweletter-banner1.gif')?>" border="0" />banner</a>
				</div>
				...
				if(count($grp_array))
				{
					...
					foreach ($grp_array as $k=>$groupData)
					{
						...
						if ($proceed)
						{
							...
							if ($db->num_rows($ret_shop))
							{
								if($groupData['shopbrandgroup_listtype'] =='Header')
								{
									$cache_type		= 'shop_left_dropdown';	
									$cache_exists 	= false;
									$cache_required	= false;
	...
	?>	
		...
		?>
			...
			<ul>
				...
				?>
				<li><a href="<?php url_link('mypricepromise.html')?>" class="userloginmenuytoplink"><?=$Captions_arr['LOGIN_MENU']['MY_PRICE_PROMISE']?></a></li>
		...
		function mod_currencyselector($title)
		{
	  	...
	  	?>
	  		...
	  		<div class="currside_maindiv"> 
			<?php
				if($title)
				{
			?>
					<div class="currside_title"><?php echo $title?></div> 
			<?php
				}
			?>	
			<div class="currside_select">
		<?php
		...
		function mod_searchfilter($title)
		{
			...
			global $ecom_siteid,$db,$ecom_hostname,$position,$Captions_arr,$Settings_arr,$sql_filter_search;
			...
			{ 
				...
				?>
			   <link type="text/css" href="../../testing_latheesh/jquery/css/ui-lightness/jquery-ui-1.7.2.custom.css" rel="stylesheet" />	
				<script type="text/javascript" src="../../testing_latheesh/jquery/js/jquery-1.3.2.min.js"></script>
				<script type="text/javascript" src="../../testing_latheesh/jquery/js/jquery_ajaxq.js"></script>
				<script type="text/javascript" src="../../testing_latheesh/jquery/js/jquery_slider.js"></script>
										<script type="text/javascript">
											...
											$("#frmProductSearch_filter input:checkbox").click(function( e ) {   SearchProducts('<?=$res_div?>')  });
											$(function() { 
												$("#slider-range").slider({
													range: true,
													min: <?=$min_price?>,
										...
										{ 
										    $("#" + container).html('loding');
											var myfrmval = $("#frmProductSearch_filter" /* Id of the form where the search controls are placed*/).serialize();
											$.ajaxSetup(
											{
												 type: "POST",
												timeout:100000,
												...
												error:function(xhr)
												{
													$("#" + container).html("<div>"+xhr.status+" : "+xhr.statusText+"</div>");
												}
											});
											      $.post("../../../includes/base_files/<?=$page_read?>",
													myfrmval,
													function(data)
													{	
														$("#" + container).html(data);
												...
												if(count($variables)>0)
												{
													...
													ORDER BY 
														...
														$rstAdvSearchVariablesOptions = $db->query($AdvSearchVariablesOptions);
															...
															<?php echo stripslashes($rowAdvSearch['var_value'])?>
															</td>
															<td width="20%">
																<input type="checkbox" id="<?php echo stripslashes($rowAdvSearch['var_value'])?>"  name="filtersearchVariableOption_<?=$key_var?>_<?=$rowAdvSearch['var_value_id']?>"  value="<?php echo stripslashes($rowAdvSearch['var_value']);?>" />
																</td></tr>
																<?php		
																}
															}
