	...
	function Show_Search($search_sql,$tot_cnt,$sql_relate)
	{
		...
		if ($db->num_rows($ret_search))
		{
			...
			$comp_active = isProductCompareEnabled();
				...
				echo $HTML_treemenu;
				    //$HTML_serach_header .= '<div class="serach_header">';
					//$HTML_serach_header .=	$Captions_arr['COMMON']['SEARCH'];	
					//$HTML_serach_header .='</div>';
					//$HTML_adserach_header .='<div class="search_advance"> <a href="'.url_link('advancedsearch.html',1).'" class="search_advance_lnk" title="'.$Captions_arr['COMMON']['ADVANCED_SEARCH'].'">'.$Captions_arr['COMMON']['ADVANCED_SEARCH'].'</a> </div>';
					//echo $HTML_serach_header;	
					//echo $HTML_adserach_header;	
			if($paging['total_cnt'])
			...
			for ($ii=$Settings_arr["productlist_interval"];$ii<=$Settings_arr["productlist_maxval"];$ii+=$Settings_arr["productlist_interval"])
				$perpage_arr[$ii] = $ii;
			$HTML_topcontent .=	generateselectbox('searchprod_prodperpagetop',$perpage_arr,$prodperpage,'','',0,'',false,'searchprod_prodperpagetop');
		    $HTML_tophiddenfor  = ''; 
			...
			$HTML_topcontent .= '
								...
								<input type="button" name="submit_Page" value="'.stripslash_normal($Captions_arr['SEARCH']['SEARCH_GO']).'" class="nav_button" onclick="handle_searchdropdownval_sel(\''.$ecom_hostname.'\',\'searchprod_sortbytop\',\'searchprod_sortordertop\',\'searchprod_prodperpagetop\')" />
								</div>
								</div>'
								.$HTML_tophiddenfor.$HTML_tophidden.'
								</div>';
		...
		if ($db->num_rows($ret_search))
		{
			...
			echo '
		              	...
		              	while ($row_search = $db->fetch_array($ret_search))
						{
							...
							$HTML_subcatname = stripslash_normal($row_search['category_name']);
							if($row_search['category_showimagetype']!='None' && ($img_support)) // ** Show sub category image only if catimage module is there for the site subcategoreynamelink
							{
									...
									if ($show_noimage)
									{
										...
										if ($no_img)
										{
											$HTML_image .= url_site_image($no_img,1);
										}	
									}

							}
					?>
							<div class="products_inner"><a href="<?php url_category($row_search['category_id'],$row_search['category_name'])?>" title="<?php echo stripslash_normal($row_search['category_name'])?>"><img src="<?php echo $HTML_image?>" onmouseover="tooltip.show('<?php echo stripslash_normal($row_search['category_name'])?>');" onmouseout="tooltip.hide();" /></a></div>
					<?php		
						}
		}
	}
