...
?>
	<div class="treemenu"><a href="<? url_link('');?>"><?=$Captions_arr['COMMON']['TREE_MENU_HOME_LINK'];?></a> >> <?=$Captions_arr['SITEMAP']['SITEMAP_TREEMENU_TITLE']?></div>
		...
		?>
			...
			$i++;
				if($rowcnt <= $max_rows) {
			?>
				<td align="left" valign="middle" class="sitemapcontents" width="50%"><a href="<?php if ($sitemap_static_pages['page_type']=='Page') url_static_page($sitemap_static_pages['page_id']); else echo $sitemap_static_pages['page_link'];?>" class="favoritecatlink" title="<?php echo stripslash_normal($sitemap_static_pages['title'])?>"><?=$sitemap_static_pages['pname']?>
				</a></td>
			<? 
					if($i%2== '0'){
						$rowcnt++;
	...
	echo "dfsd".$_SESSION['site_map_links'][0];
		...
		while($sitemap_products = $db->fetch_array($ret_sitemap_products)) {
			...
			if($rowcnt <= $max_rows) {
		?>
			<td align="left" valign="middle" class="sitemapcontents" width="50%"><?=$rowcnt?><a href="<?php url_product($sitemap_products['product_id'],$sitemap_products['product_name'],-1)?>" class="favoritecatlink" title="<?php echo stripslash_normal($sitemap_products['product_name'])?>"><?=$sitemap_products['product_name']?>
			</a></td>
		<? 
				if($i%2== '0') {
				$rowcnt++;
