	...
	class product_Html
	{
		...
		function Show_ProductDetails($row_prod)
		{
            ...
            <td class="det_table_name" colspan="2">
              ...
              <tr>
                <td class="td">
					...
					{
					?>	
					<div class="details_offer">
						<?php
						if($row_prod['product_bulkdiscount_allowed']=='Y')
						{
						?>
						
						<img src="<?php url_site_image('sss.png')?>"  />
						<?php
						}
						?>
						<?php
						...
						<img src="<?php url_site_image('free-deli.png')?>"  />
						<?php
						}
						?>
					</div>
			...
			?>
				...
				<div  id="cart_details_ajaxholderPop" ></div>
...
<?php
	...
	if($row_prod['product_longdesc']!='')
		$tabs_content_arr1[0]	= trim(stripslashes($row_prod['product_longdesc']));
	elseif ($row_prod['product_shortdesc'])
		$tabs_content_arr1[0]	= trim(stripslashes($row_prod['product_shortdesc']));
...
<tr class="details_expand_cont" style="display:none;" id="<?php echo str_replace(" ","-",strtolower($tabs_arr1['PRODDET_OVERVIEW']));?>-cont">
	<td><?php echo utf8_encode($tabs_content_arr1[0]);?></td>
</tr>
<?php
}
...
<tr class="details_expand_expand" id="<?php echo "tab-".$row_tab['tab_id'];?>" onclick="javascript: showTab('<?php echo "tab-".$row_tab['tab_id'];?>-cont',this.id);">
	<td><?php echo stripslashes(utf8_encode($row_tab['tab_title']));?></td>
</tr>
 ...
 ?>
  ...
  ?>
	...
	<ul class="coffee_str_ul">
		...
		{
		?>
			<li class="coffee_str_li"><img src="<?php url_site_image('coffee_str_active.png')?>"></li>
		<?php
		}
		for($str_i=($row_strgt['product_coffee_strength']+1);$str_i<=$max;$str_i++)
		{
		?>
			<li class="coffee_str_li"><img src="<?php url_site_image('coffee_str_inactive.png')?>"></li>
		<?php
		}
		?>	
	</ul>
...
function show_ProductVariables($row_prod,$pos='column',$sizechart_heading)
{
  ...
  ?>
	...
	?>
     <tr id="tr_summary">
         <td class="tda"><div class="deat_pdt_sizechart"><div class="sizechartleft"><div><a href="javascript:showsizechartPopup('<?php echo $row_prod['product_id']?>','<?php echo $ecom_hostname?>','<?php echo $ecom_themename?>')" title="<?php echo stripslash_normal($sizechartmain_title)?>">Delivery Info<?php ?></a></div></div></div>
         </td>
     </tr>
	<?php	
		}
	...
	?>
       <tr id="tr_summary">
         <td class="tda" style="padding-bottom:5px"> <div class="deat_pdt_sizechart"><div class="sizechartleft"><div><a href="<?=$row_prod['product_commonsizechart_link']?>" title="Delivery Info" 	target="<?=$row_prod['produt_common_sizechart_target']?>">Delivery Info<?php ?></a></div></div>&nbsp;</div>
         </td>
         </tr>
	 <?
	}
...
{ 
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="ftable_n">
		<tr>
                <td class="tdy" align="left" colspan="2" >
	<?php
	if ($caption_key!='') // Check whether the buynow or preorder button is to be displayed
	{
            ...
            <div class="class_qty">
                <?=$cur_qty_caption?></div>
                 <div class="class_qty_textA"><input type="text" class="det_qty_txt" name="qty"  value="<?php echo ($_REQUEST['qty'])?$_REQUEST['qty']:1?>" maxlength="5" />
                </div>
            <?php
			   $colspan = 2;
            }
            	...
            	if($addto_cart_withajax == 1)
							{
								?>
									...
									<div class="buyBinner_linkAB"><div class="buyBinner_link_in"><a href="#cart_details_ajaxholderPop"><input name="Buy Now" type="button" value="<?php echo stripslash_normal($Captions_arr['PROD_DETAILS'][$caption_key]);?>"  class="byellow_n" onclick="quick_view_prod('frm_proddetails','add_prod_tocart_ajax')"/></a></div></div>
...
function show_subproducts($prodid)
	{
		...
		{
		?>
			<table cellpadding="0" cellspacing="0" border="0" class="subproduct_maintable" style="margin-left:5px">
			<tr>
		...
		<?php	
			...
			{
				//$pname = stripslashes($row_sub['product_name']);
		...
		?>
				...
				else
				{
					...
					$sav_price = '';
					$cur_price_tax= '';
					if($holdcur_price and $row_proddets['product_applytax']=='Y')
					{
						$cur_price_tax = $holdcur_price + ($holdcur_price*20/100);
						//$cur_price_tax = sprintf('%0.2f',round($cur_price_tax,2));
						$cur_price_tax = sprintf('%0.2f',($cur_price_tax));
