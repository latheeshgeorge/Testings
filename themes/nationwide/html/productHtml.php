...
/*			// Check whether size chart details exists for current product
		   ...
		   $sql_prods = "SELECT product_sizechart_mainheading 
			...
			$HTML_treemenu = ' <div class="tree_menu_con_det">
								...
								<ul class="tree_menu">'.str_replace('&','&amp;',generate_tree_menu(-1,$_REQUEST['product_id'],'','<li>','</li>')).'
								</ul>
								</div>
								</div>
			...
			if($price_arr['price_with_captions']['discounted_price'])
			{
				...
				$pr = explode('(',$price_arr['price_with_captions']['discounted_price']);
				$rets =  getmicrodata_price($pr[0]);			
				
				$cur_price = str_replace($pr[0],$rets,$price_arr['price_with_captions']['discounted_price']);
			...
			else
			{
				...
				$pr = explode('(',$price_arr['price_with_captions']['base_price']);
				$rets =  getmicrodata_price($pr[0]);			
				
				$cur_price = str_replace($pr[0],$rets,$price_arr['price_with_captions']['base_price']);
				$sav_price = '';
			}
			...
			if ($Settings_arr['proddet_showwishlist'])
			{
				...
				$HTML_wishlist = '<div class="deat_pdt_buyA"><div title="'.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_ADDWISHLIST']).'" onclick="'.$wishlist_onclick.'">'.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_ADDWISHLIST']).'</div></div>';
			}			
			if ($row_prod['product_show_enquirelink']==1)
			{
				if($addto_cart_withajax==1)
				{
					$HTML_enquiry = '<div title="'.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_ENQUIRY']).'" onclick="ajax_addto_cart_fromlist(\'add_prod_tocart_ajax\',\'Prod_Enquire\',\'frm_proddetails\',\''.SITE_URL.'\')">'.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_ENQUIRY']).'</div>';
				}
				else
				{
					$HTML_enquiry = '<div title="'.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_ENQUIRY']).'" onclick="show_wait_button(this,\'Please Wait...\');document.frm_proddetails.fpurpose.value=\'Prod_Enquire\';document.frm_proddetails.submit();">'.stripslash_normal($Captions_arr['PROD_DETAILS']['PRODDET_ENQUIRY']).'</div>';
				}			
			}
			...
			{
				$tabs_arr 			= array (0=>'Product Details');
			...
			$HTML_loading = '<div class="proddet_loading_outer_div" style="height:15px"><div id="proddet_loading_div" style="display:none;padding:5px 0 0 0;">
							<img src="'.url_site_image('proddet_loading.gif',1).'" alt="loading..." style="width:43px;height:11px;text-align:left"/>
							</div></div>';	
			...
			<?php /*?><input type="hidden" name="ajax_div_holder" id="ajax_div_holder" value="" /><?php */?>
			...
			<div class="deat_img_outr">
				...
				<?//=$HTML_bookmark?>
			 	...
			 	<div class="deat_cont_outr" >
				<div class="deat_cont_top"></div>
				<div class="deat_cont_cont">
			   <div class="deat_pdt_icons">
			   </div>
			   <div class="deat_pdt_name" itemprop="name">
					<h1><?php echo stripslash_normal($row_prod['product_name']);?></h1>
				...
				<?php
				/*
				 * Section for check the book_now tag for the product
				 * */
				    $mystring  = $row_prod['product_longdesc'];
					$findme    = '[book_now]';
				...
				if($ecom_siteid == 70 && $book_now)
				{
				?>
					<div style="float:right;padding:0 0 3px 5px;"><a href="http://<?php echo $ecom_hostname?>/pg49918/book-now.html" class="" ><img src="<?php url_site_image('book_now_btn.gif')?>" alt="Book Now" border="0" /></a></div>
				<?php
				}
				?>
			...
			?>	
			<table style="width:100%" class="tab_class">
				<tr>
				<?
                 $cnt = 0;
				...
				?>		
					<td class="<?php echo $clssA?>"><?php echo $v_tabtitle?></td>
				<?php
				}
				?>
				</tr>
				...
				foreach ($tabs_content_arr as $k_tabcontid=>$v_tabcontent)
				{
					...
					if($book_now == true)
					{
						$v_tabcontent = str_replace("[book_now]"," ",$v_tabcontent);
				    }
					//$disp_content = str_replace($sr_arr,$rp_arr,$v_tabcontent);
					$disp_content = $v_tabcontent;
				?>
					 <td class="<?php echo $clss?>" style="vertical-align:top"><? echo $disp_content?></td>
				<?php
				}
				?>	
				 </tr>
...
function show_ProductVariables($row_prod,$pos='column',$sizechart_heading)
{
  ...
  ?>
		...
		<table class="deat_pdt_varable_table">
		<?php
		// Case of variables
		if ($db->num_rows($ret_var) or $db->num_rows($ret_msg))
		{
			while ($row_var = $db->fetch_array($ret_var))
			{
				...
				?>	
				  <tr>
					<td style="text-align:left;vertical-align:middle" class="varable_table_left"><?php echo stripslash_normal($row_var['var_name'])?> : </td></tr>
					<tr>
					<td style="text-align:left;vertical-align:middle" class="varable_table_right">
                        <?php
						...
						if ($row_var['var_value_exists']==1)
						{
							...
							if($var_disp_type == 'OTHER')
							{
							  ...
							  {
						?>
									<select style="width:100%" name="var_<?php echo $row_var['var_id']?>" id="var_<?php echo $row_var['var_id']?>" <?php echo $onchange_function?>>
									<?php 
...
function show_buttons($row_prod)
{
	...
	$cust_id 	= get_session_var("ecom_login_customer");
	$showqty	= 1;//$Settings_arr['show_qty_box'];// show the qty box
	...
	if($db->num_rows($ret_cats))
	{
		...
		if($db->num_rows($ret_grps))
		{
			...
			if($grp_nos)
			{
				...
				{
					$ret_val = '<table style="width:100%" class="keyfeature" id="proddet_var_table">';
					$i=1;
					$grp_cnt = 0;
					$label_arr = array();
							...
							if(is_array($v))
							{
								if(count($v))
								{
									for($i=0;$i<count($v);$i++)
									{
										...
										$ret_val .= '
													<tr>
														<td style="text-align:left;vertical-align:middle" class="'.$clss.'">'.$v[$i]['name'].'</td>
														<td style="text-align:left;vertical-align:middle" class="'.$clssA.'">:&nbsp;'.$v[$i]['val'].'</td>
													</tr>';	
									}
								}
							}
...
?>
		...
		<div class="det_link_nav"><a href="#null" onmouseover="scrollDivRight('containerA')" onmouseout="stopMe()"><img src="<?php url_site_image('/arrow-left.gif')?>" alt="scroll left" /></a></div>
		...
		{
			$row_prod['product_name'] = str_replace('&','&amp;',$row_prod['product_name']);
		?>
		...
		<div class="det_link_nav"> <a href="#null" onmouseover="scrollDivLeft('containerA','<?php echo $div_width?>')" onmouseout="stopMe()"><img src="<?php url_site_image('arrow-right.gif')?>" alt="scroll right" /></a></div>
		</div>
        </div>
