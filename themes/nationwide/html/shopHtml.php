		 ...
		 /* function download_pdf()
		  {
			...
			document.getElementById('bw').innerHTML = '<iframe src="http://www.web2pdfconvert.com/engine?curl=http://<?=$_SERVER['HTTP_HOST']?><?=$_SERVER['REQUEST_URI']?>&outputmode=service">';
			...
			if($_REQUEST['shop_id'])
			  {
			     ...
			     $HTML_icons .= '<a  href="javascript:download_pdf_stream(\''.$_SERVER['HTTP_HOST'].'\',\''.$_SERVER['REQUEST_URI'].'\')" title="'.stripslash_normal($Captions_arr['SHOP_DETAILS']['SHOPDET_DOWNLOADPDF']).'"><img src="'.url_site_image('pdfico.gif',1).'" alt="'.stripslash_normal($Captions_arr['SHOP_DETAILS']['SHOPDET_DOWNLOADPDF']).'" border="0" /></a>';
