<?php
	...
	class cart_Html
	{
		...
		function Show_Cart()
		{
			...
			// ==============================================================
			// Check whether any payment type is activated for the current website
			if($row_cartdet['paytype_id']==0)
			{
				// Check whether credit card payment type is active for current website
				$sql_pay_f_check = "SELECT paytype_id FROM payment_types WHERE paytype_code='credit_card' LIMIT 1";
				$ret_pay_f_check = $db->query($sql_pay_f_check);
				if($db->num_rows($ret_pay_f_check))
				{
					$row_pay_f_check = $db->fetch_array($ret_pay_f_check);
					$sql_pay_f2_check = "SELECT paytype_id FROM payment_types_forsites WHERE 
											sites_site_id = $ecom_siteid 
											AND paytype_id = ".$row_pay_f_check['paytype_id']."
											AND paytype_forsites_active = 1 
											AND paytype_forsites_userdisabled=0 
										LIMIT 
											1";
					$ret_pay_f2_check = $db->query($sql_pay_f2_check);
					if($db->num_rows($ret_pay_f2_check))
					{
						$update_pay_f2 = "UPDATE cart_supportdetails 
											SET 
												paytype_id = ".$row_pay_f_check['paytype_id']." 
											WHERE 
												session_id='".$session_id."'  
												AND sites_site_id = $ecom_siteid 
											LIMIT 
												1";
						$db->query($update_pay_f2);
					}					
				}
          ...
          <div class="cart_cont_inner_cont">
			...
			<input type="hidden" name="hold_section" value="" />
				...
				$HTML_topdesc .='<div class="cart_top_outer">';
					...
					{
						 $HTML_topdesc .= '<div class="cart_shop_cont" onClick="show_wait_button(this,\'Please Wait...\');window.location=\''.$ps_url.'\'"><div><span>'.stripslash_normal($Captions_arr['CART']['CART_CONT_SHOP']).'</span></div></div>';
					}
					else
						 $HTML_topdesc .= '&nbsp;';
				 	$HTML_topdesc .= '</div>';
 ...
 <?php
		...
		if (count($cartData['products'])>0)
		{
			if($hide_direct_checkout_button==false)// today start
			{
				if($show_checkoutbutton==true)	
				{
						 $HTML_bottomdesc .= '<div class="cart_shop_chkout" onClick="handle_checkout_submit(\''.$ecom_hostname.'\',0,\''.$cartData["totals"]["bonus_price"].'\')"><div> <span>'.stripslash_normal($Captions_arr['CART']['CART_GO_CHKOUT']).'</span></div></div>';
				}
				else
				{
		...
		if($ps_url) // show the continue shopping button only if ps_url have value
		{
			 $HTML_bottomdesc .= '<div class="cart_shop_cont" onClick="show_wait_button(this,\'Please Wait...\');window.location=\''.$ps_url.'\'"><div><span>'.stripslash_normal($Captions_arr['CART']['CART_CONT_SHOP']).'</span></div></div>';
		}
		else
			 $HTML_bottomdesc .= '&nbsp;';
		$HTML_bottomdesc .= '</div>';
  ...
  <tr>
   ...
   <?php
		...
		<?php
            ...
            <td colspan="10" align="right" valign="top" class="mainmiddle_carttdA">
				<table width="100%" border="0" cellpadding="0" cellspacing="0" class="shoppingcarttable">
					...
					<tr>
						<td align="right" valign="top" class="google_td" width="60%">
						<?php
				...
				?>
						...
						<td align="right" valign="middle" class="google_td">
						<input type='image' name='submit' src='<?php url_site_image('btn_xpressCheckout.gif')?>' border='0' align='top' alt='PayPal' onclick="handle_checkout_submit('<?php echo $ecom_hostname?>',2,'<?php echo $cartData["totals"]["bonus_price"]?>')"/>
						</td>
					</tr>	
				</table>
				</td>
    ...
    <div class="cart_cont_inner_bottom"></div>
			...
			if($_REQUEST['pret']==1) // case if coming back from PAYPAL with token.
			{
				if($_REQUEST['token'])
				{
					...
					if($ack == "SUCCESS" ) // case if address details obtained correctly
					{
						...
						echo "
							<script type='text/javascript'>
							alert('Please complete the field(s) marked red and checkout to complete your order.');
							</script>
						";
					}
						...
						?>
								...
								<div class="cart_top_outer">
									...
									<div class="cart_top_links">
										<div class="chk_back_btn" onclick="show_wait_button(this,'Please Wait...');gobackto_cart('<?php echo ($protectedUrl)? base64_encode($ecom_hostname):$ecom_hostname?>')">
											<div><span class="cart_blink"><?php echo stripslash_normal($Captions_arr['CART']['CART_BACK_CARTPAGE'])?></span></div>
										</div> 
									</div>
								</div>  
								<?php
									...
									<tr>
											...
											$sql_checkout = "SELECT field_det_id,field_key,field_name,field_req,field_error_msg   
														...
														{						
															...
															{		
																...
																?>
																	<tr>
																		<td align="left" width="50%" class="<?php echo ($row_checkout['field_req']==1)?'regiconent_redborder':'regiconent'?>">
									 ...
									 // check whether billing and shipping address can be different in the site.
												...
												<?php
													...
													?>
														<tr>
														<td align="center"  colspan="2" class="cartterms">
														<br /><input class="shoppingcart_radio" type="checkbox" name="cart_terms" id="cart_terms" value="1" checked="checked" /><strong></strong><?php echo stripslash_normal($Captions_arr['CART']['CART_ACCEPT_TERMS_CONDITIONS'])?></strong>
														</td>
														</tr>
													<?php	
													}
										...
										<div class="chk_credit_outr">
											...
											<tr>
												<td  align="right" valign="middle" class="cartterms">			
													...
													{
													?>
														<br /><input class="shoppingcart_radio" type="checkbox" name="cart_terms" id="cart_terms" value="1" /><strong></strong><?php echo stripslash_normal($Captions_arr['CART']['CART_ACCEPT_TERMS_CONDITIONS'])?></strong>
													<?php	
													}*/
													?>	
												<br /></td>
											...
											<tr>
												...
												<?php
													...
													?>	
														...
														?>
														<div class="chk_paymnt_btn_A"><div> <input name="continue_checkout" type="submit" class="buttonred_cart_A" id="continue_checkout" value="<?php echo $checkout_caption?>"/></div></div>
														<?php
														}
														else
														{
													...
													?>
													<div class="cart_top_links">
														<div class="chk_back_btn" onclick="gobackto_cart('<?php echo ($protectedUrl)? base64_encode($ecom_hostname):$ecom_hostname?>')"><div><span class="cart_blink"><?php echo stripslash_normal($Captions_arr['CART']['CART_BACK_CARTPAGE'])?></span></div></div> 
													</div>
													<?php
													if($cartData["payment"]["method"]['paymethod_key']=='PROTX_VSP')
													{
														echo "<div style='float:right;text-align:right;color:#FF0000'>You will be re-directed to the secure Sage Pay page to take your card payment</div>";
													}
													?>
												</td>
											</tr>
		...
		{
		?>
			<div class="chk_back_btn" onclick="show_wait_button(this,'Please Wait...');gobackto_cart('<?php echo ($protectedUrl)? base64_encode($ecom_hostname):$ecom_hostname?>')"><div><span class="cart_blink"><?php echo stripslash_normal($Captions_arr['CART']['CART_BACK_CARTPAGE'])?></span></div></div>
		<?php
		}
		?>
		</div>  
