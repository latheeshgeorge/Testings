...
?>
	...
	function maillinglist_onchange(obj)
	{
		...
		{
			var curlen = document.frm_registration.elements.length;
			for(i=0;i<curlen;i++)
			{
	  ...
	  ?>
		...
		?>
            ...
            <?php 
						...
						{
						?>
            <div>  <div> <input type="checkbox" name="newsletergroup[]" value="<?=$customer_grp['custgroup_id']?>" <?=($customer_grp['custgroup_id']==$customer_grp['selected_grp'])?"checked":""?> onchange="mailinglist_mainsel()"/></div><div><? echo str_replace('&','&amp;',stripslash_normal($customer_grp['custgroup_name']))?></div> </div> 
            <?php 
			}
			?>
            </div>
           ...
           <div class="reg_shlf_inner_cont">
				   ...
				   <table class="reg_shlf_inner_table" width="100%" border="0" cellpadding="2" cellspacing="0">
					...
					?>
					  ...
					  <tr>
					    <td align="left"><img src="<?php urlencode(url_verification_image('includes/vimg.php?size=4&amp;pass_vname=registration_Vimg&amp;bg=255 255 255&amp;brdr=255 255 255&amp;fnt=255 0 0&amp;circle=1'))?>" border="0" alt="Image Verification" class="captcha"/></td>
		...
		$jj(document).ready(function() {
			...
			$jj('#password-clear').show();
			$jj('#password-password').hide();
			$jj('#password-clear').focus(function() {
				$jj('#password-clear').hide();
				$jj('#password-password').show();
				$jj('#password-password').focus();
			});
			$jj('#password-password').blur(function() {
				if($jj('#password-password').val() == '') {
					$jj('#password-clear').show();
					$jj('#password-password').hide();
				}
			});
			$jj('.login_inner_input').each(function() {
	...
	<tr>
	<td><img src="<? url_site_image('lgoin-in-a.gif')?>" alt="login" /></td>
	<td class="login_inner_top"><img src="<? url_site_image('lgoin-inb.gif')?>" alt="login" /></td>
	</tr>
	<tr>
		...
		<?php
			...
			{
			?>
			<input id="password-clear" type="text" value="Password" <?php /*autocomplete="off"*/?>  class="login_inner_inputA"/>
			<?
			}
			?>
    		<input id="password-password" type="password" name="custlogin_pass" value="" <?php /*autocomplete="off"*/?>  class="login_inner_inputA"/>
		</div>
		<div> <a href="javascript:validatemiddle_login(document.frm_custlogin_middle)"><img src="<? url_site_image('loin-inbt.gif')?>" alt="login"  /></a></div>
		...
		<div> <a href="<?php url_link('registration.html')?>"><img src="<? url_site_image('login-btb.gif')?>" alt="login"  border="0" /></a></div>
		</td>
	</tr>
</table>
<div class="normal_shlfA_mid_bottom"></div> 
