<?php
	...
	class product_Html
	{
		...
		function Show_ProductDetails($row_prod)
		{
			...
			$HTML_price = '';
			if($was_price)
				$HTML_price .= '<div class="pricea">'.$was_price.'</div>';
			if($cur_price)
				$HTML_price .= '<div class="priceb">'.$cur_price.'</div>';	
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="ctable">
              <tr>
            <td class="curvea_top" colspan="2"><div class="white"><?php echo stripslash_normal($row_prod['product_name']); 
...
<td colspan="2">
              ...
              <tr>
                <td class="td"><div id="mainimage_holder"><?php $this->Show_Image_Normal($row_prod,'main');?></div></td>
                <td class="td"><?=$HTML_price?>
                </td>
              </tr>
              <tr>
                <td colspan="2" class="curve_bottom" ><a href="#" class="linkbold"><?php echo stripslash_normal($row_prod['product_name']); ?></a></td>
              </tr>
            </table>
       ...
       ?>
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="etable">
      <tr>
    <td class="tdyellow">
    
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td <?php echo $selA;?>><a href="#protabs"  class="linkbold" onclick="link_submit('101',0,'<?php url_product_tabs($row_prod['product_id'],$row_prod['product_name'],$prodimgdetid,'101','#protabs',-1)?>',0)">Summary</a></td>
    <td <?php echo $selB;?>><a href="#protabs"  class="linkbold" onclick="link_submit('102',0,'<?php url_product_tabs($row_prod['product_id'],$row_prod['product_name'],$prodimgdetid,'102','#protabs',-1)?>',0)">Picture</a></td>
    <td <?php echo $selC;?>><a href="#protabs" class="linkbold" onclick="link_submit('103',0,'<?php url_product_tabs($row_prod['product_id'],$row_prod['product_name'],$prodimgdetid,'103','#protabs',-1)?>',0)">Description</a></td>
  </tr>
</table>
    </td>
  </tr>
  ...
  if($curtab == '101')
  {
	  ...
	  if($tabs_content_arr[-1]!='')
	  {
	  ?>
      <tr id="tr_label">
    	 <td class="tda_label">
      <?php
	     echo $tabs_content_arr[-1];
		 ?>
         </td>
       </tr>
         <?php
  ...
  ?>
      <tr>
        <td class="td"><div id="mainimage_holder"><?php $return_arr = $this->Show_Image_Normal($row_prod,'main_tab'); 
		   $this->show_more_images($row_prod,$return_arr['exclude_tabid'],$return_arr['exclude_prodid']);?></div></td>
        </td>
      </tr>
...
function show_more_images($row_prod,$exclude_tabid,$exclude_prodid)
{
	...
	?>	
		...
		<div class="det_thumbimg_nav"><a href="#null" onmouseover="scrollDivRight('containerB')" onmouseout="stopMe()"><img src="<?php url_site_image('thmbarw.gif')?>">22</a></div>
		...
		<div class="det_thumbimg_nav"> <a href="#null" onmouseover="scrollDivLeft('containerB',<?php echo (count($prodimg_arr)*150)?>)" onmouseout="stopMe()"><img src="<?php url_site_image('thmbarwl.gif')?>">222</a></div>
		</div>
		</div>	
	<?php
	}
...
function show_buttons($row_prod)
{
	...
	?>	
		<table width="100%" border="0" cellspacing="0" cellpadding="0" class="ftable">
            ...
            ?>
                <tr>
                <td class="tdy" align="center"><?=$cur_qty_caption?></td>
                <td class="tdz" align="center"><input type="text" class="det_qty_txt" name="qty"  value="<?php echo ($_REQUEST['qty'])?$_REQUEST['qty']:1?>" maxlength="2" />
                </td>
                </tr>
            <?php
			   $colspan = 2;
            ...
            ?>
             <tr>
            <td class="td" align="center" colspan="<?php echo $colspan;?>">
            <div class="buyBinner_link"><a href="#" class="det_buy_link" onclick="show_wait_button(this,'Please Wait...');document.frm_proddetails.fpurpose.value='Prod_Addcart';document.frm_proddetails.submit();"><input name="Buy Now" type="button" value="<?php echo stripslash_normal($Captions_arr['PROD_DETAILS'][$caption_key]);?>"  class="byellow"/></a></div>
            </td>
      </tr>
      </table>
<?php
