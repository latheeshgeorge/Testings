	...
	class product_Html
	{
	 ...
	 <div class="pro_det_left_con">
		...
		if($row_prod['product_longdesc'])
			$tabs_content_arr[0]	= stripslash_normal($row_prod['product_longdesc']);
		elseif ($row_prod['product_shortdesc'])
			$tabs_content_arr[0]	= stripslash_normal($row_prod['product_shortdesc']);
		...
		if($db->num_rows($ret_tab))
		{
			while ($row_tab = $db->fetch_array($ret_tab))
			{
				...
				$tabs_content_arr[$row_tab['tab_id']]	= stripslash_normal($row_tab['tab_content']);
			}
		}
		?>
