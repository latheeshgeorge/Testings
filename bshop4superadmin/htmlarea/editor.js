...
//  ['custom1','custom2','custom3','separator'],
 //   ['mosimage','mospagebreak', 'separator'],
   ['popupeditor']
  ];
...
this.btnList = {
    ...
    "mosimage":           ['mosimage',       'Insert {mosimage}',    'editor_action(this.id)',  'ed_mos_image.gif'],
    "mospagebreak":      ['mospagebreak',    'Insert {mospagebreak}','editor_action(this.id)',  'ed_mos_pagebreak.gif'],
...
function editor_action(button_id) {
  ...
  else if (cmdID == 'mosimage') { // insert {mosimage}
    editor_insertHTML(objname, "{mosimage}");
  }

  else if (cmdID == 'mospagebreak') { // insert {mospagebreak}
    editor_insertHTML(objname, "{mospagebreak}");
  }
