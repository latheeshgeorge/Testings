...
*/
$document_explode_array = explode("/",$_SERVER['DOCUMENT_ROOT'].$_SERVER['PHP_SELF']);
for($i=0; $i<count($document_explode_array)-3; $i++)
{
	$document_client_implode_array[] = $document_explode_array[$i];
}
$mosConfig_absolute_path = implode("/",$document_client_implode_array);
//$mosConfig_absolute_path = 'c://apache//htdocs//edit';
...
// - Specify the fully-qualified path from the filesystem root in IMAGE_DIR.
// - Do NOT include "http://my.hostname.com" in IMAGE_URL; just the path from the
//   DocumentRoot of your webserver.

define("IMAGE_DIR", trim($mosConfig_absolute_path."/htmlarea/uploaded/"));
$url = "";// PATCH AJE:: Don't know what $url is doing????? this line is added
$root = $url.$_SERVER['PHP_SELF']; 

$root = str_replace("/editor","",$root);
$root = str_replace("/htmlarea","",$root);
$root = str_replace("/popups","",$root);
$root = str_replace("/viewer.php","/htmlarea/uploaded/",$root);
define("IMAGE_URL", trim($root));

// SCRIPT_DIR and SCRIPT_URL identify where these scripts reside (MUST end in "/")
// - Specify the fully-qualified path from the filesystem root in SCRIPT_DIR.
// - Do NOT include "http://my.hostname.com" in SCRIPT_URL; just the path from the
//   DocumentRoot of your webserver.
define("SCRIPT_DIR", trim($mosConfig_absolute_path."/htmlarea/popups/"));
$root = $url.$_SERVER['PHP_SELF'];
$root = str_replace("lister.php","",$root);
define("SCRIPT_URL", trim($root));

// SUPPORT_* identify optional features
define("SUPPORT_BITMAP", FALSE);  // supported by Internet Explorer ONLY!
define("SUPPORT_CREATE", TRUE);
define("SUPPORT_DELETE", TRUE);
define("SUPPORT_METAFILE", FALSE);// supported by Internet Explorer ONLY!
...
function imageURL($path, $encode = FALSE) {
   ...
   //return ($mosConfig_live_site . $url);
}
