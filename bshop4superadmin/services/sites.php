...
if($_REQUEST['fpurpose'] == '') {
	...
	if($_REQUEST['Addsite_submit'])
	{
		...
		if(!$alert) {
			...
			$insert_array['site_domain_alias']			= $_REQUEST['site_domain_alias'];
			...
			$insert_array['mobile_themes_theme_id']		= ($_REQUEST['mobile_theme_id']>0)?$_REQUEST['mobile_theme_id']:0;
			...
			$insert_array['site_activate_invoice']		= ($_REQUEST['site_activate_invoice'])?1:0;
			$insert_array['in_mobile_api']				= ($_REQUEST['in_mobile_api'])?1:0;
			
			$insert_array['site_grid_enable']			= ($_REQUEST['site_grid_enable'])?1:0;
     					
     		$insert_array['linked_product_cart']			= ($_REQUEST['linked_product_cart'])?1:0;

     		$insert_array['site_email_verification']		= ($_REQUEST['site_email_verification'])?1:0;

			$insert_array['site_footer_scripts']		= addslashes($_REQUEST['site_footer_scripts']);
			$insert_array['site_checkout_scripts']		= addslashes($_REQUEST['site_checkout_scripts']);
			...
			$insert_array['is_yahoometa_verificationcode']	= ($_REQUEST['is_yahoometa_verificationcode'])?1:0;
			if($_REQUEST['is_yahoometa_verificationcode'])
			{
				$insert_array['yahoo_meta_verificationcode']	= addslashes($_REQUEST['yahoo_meta_verificationcode']); 
			}
			$insert_array['is_msnmeta_verificationcode']	= ($_REQUEST['is_msnmeta_verificationcode'])?1:0;
			if($_REQUEST['is_msnmeta_verificationcode'])
			{
				$insert_array['msn_meta_verificationcode']	= addslashes($_REQUEST['msn_meta_verificationcode']); 
			}
			$insert_array['is_twitter_account']	= ($_REQUEST['is_twitter_account'])?1:0;
			if($_REQUEST['is_twitter_account'])
			{
				$insert_array['site_twitter_account_id']	= addslashes($_REQUEST['site_twitter_account_id']); 
			}
			...
			$webtracker_id = 0;
			/*if($_REQUEST['webtracker_account_id'])
			{
				if (is_numeric($_REQUEST['webtracker_account_id']))
				{
					$webtracker_id = $_REQUEST['webtracker_account_id'];
				}
			}*/
			$insert_array['webtracker_account_id']	                = $webtracker_id; 
			$insert_array['site_updation_notification_emailids']    = addslashes($_REQUEST['site_updation_notification_emailids']);
            $insert_array['template_product_layout'] 				= addslashes($_REQUEST['template_product_layout']);
			/*FB login script starts here*/
            $insert_array['site_fb_enable']							= ($_REQUEST['site_fb_enable'])?1:0; 
            $insert_array['site_fb_appid'] 							= addslashes($_REQUEST['site_fb_appid']);
            $insert_array['site_fb_secretkey'] 						= addslashes($_REQUEST['site_fb_secretkey']);
			/*FB login script ends here*/			
			
			$db->insert_from_array($insert_array, 'sites');
			$insert_siteid = $db->insert_id();//#Getting generated site id
			
			// Get the list of layouts available for current theme
			$layout_arr		= array();
			$sql_layouts 	= "SELECT layout_id,layout_code FROM themes_layouts WHERE themes_theme_id =".$_REQUEST['theme_id']." ORDER BY layout_order";
			...
			//##########################################################################################################
			//#theme static page position and path for the theme
			//##########################################################################################################
			$sql = "SELECT page_positions,themename FROM themes WHERE theme_id=".$_REQUEST['theme_id'];
			$res = $db->query($sql);
			$positions = $db->fetch_array($res);
			$position_array = explode(",",$positions['page_positions']);
			$theme_path		= $positions['themename'];
			...
			$insert_group_id = $db->insert_id();//#Getting generated Page Group id
				// Setting the static page menu to the top position
				// Get the id of the static page menu feature from features table
				$sql_feats = "SELECT feature_id 
								FROM 
									features 
								WHERE 
								...
								LIMIT 
									1";
				$ret_feats = $db->query($sql_feats);
				if($db->num_rows($ret_feats))
				{
					$row_feats	= $db->fetch_array($ret_feats);
					foreach ($layout_arr as $k=>$v)
					{
						$insert_array								= array();
						$insert_array['sites_site_id']				= $insert_siteid;
						$insert_array['features_feature_id']		= $row_feats['feature_id'];
						$insert_array['display_position']			= 'top';
						$insert_array['themes_layouts_layout_id']	= $k;
						$insert_array['layout_code']				= $v;
						$insert_array['display_title']				= 'Top';
						$insert_array['display_order']				= 0;
						$insert_array['display_component_id']		= $insert_group_id;
			...
			//##########################################################################################################
			//#Inserting 3 Static pages
			//##########################################################################################################
			$static_page_array = array('Home','About us','Contact us');
			...
			mkdir(IMAGE_ROOT_PATH."/".$_REQUEST['site_domain']."/settings_cache", 0777);
			chmod(IMAGE_ROOT_PATH."/".$_REQUEST['site_domain']."/settings_cache", 0777);
			mkdir(IMAGE_ROOT_PATH."/".$_REQUEST['site_domain']."/static", 0777);
			chmod(IMAGE_ROOT_PATH."/".$_REQUEST['site_domain']."/static", 0777);
			mkdir(IMAGE_ROOT_PATH."/".$_REQUEST['site_domain']."/swf", 0777);
			chmod(IMAGE_ROOT_PATH."/".$_REQUEST['site_domain']."/swf", 0777);
			mkdir(IMAGE_ROOT_PATH."/".$_REQUEST['site_domain']."/site_images/payment_methods_images", 0777);
			chmod(IMAGE_ROOT_PATH."/".$_REQUEST['site_domain']."/site_images/payment_methods_images", 0777);
			
	
			// Copying some default files required 
			$src_folder 	= IMAGE_ROOT_PATH."/default_files/otherfiles";
			$dest_folder 	= IMAGE_ROOT_PATH."/".$_REQUEST['site_domain']."/otherfiles";			
			cp_files($src_folder,$dest_folder);
			
			$src_folder 	= IMAGE_ROOT_PATH."/default_files/payment_methods_images";
			$dest_folder 	= IMAGE_ROOT_PATH."/".$_REQUEST['site_domain']."/site_images/payment_methods_images";			
			cp_files($src_folder,$dest_folder);
			
			$src_folder 	= IMAGE_ROOT_PATH."/default_files/ssl";
			$dest_folder 	= IMAGE_ROOT_PATH."/".$_REQUEST['site_domain']."/site_images";			
			cp_files($src_folder,$dest_folder);
			
			$src_folder 	= IMAGE_ROOT_PATH."/default_files/template";
			$dest_folder 	= IMAGE_ROOT_PATH."/".$_REQUEST['site_domain']."/static";			
			cp_files($src_folder,$dest_folder);
			if($_REQUEST['theme_id']>0)
			{
				$src_folder 	= $site_main_docroot."/themes/".$theme_path."/scripts";
				$dest_folder 	= IMAGE_ROOT_PATH."/".$_REQUEST['site_domain']."/scripts";			
				cp_files($src_folder,$dest_folder);
				
				$src_folder 	= $site_main_docroot."/themes/".$theme_path."/css";
				$dest_folder 	= IMAGE_ROOT_PATH."/".$_REQUEST['site_domain']."/css";			
				cp_files($src_folder,$dest_folder);
				
				$src_folder 	= $site_main_docroot."/themes/".$theme_path."/site_images";
				$dest_folder 	= IMAGE_ROOT_PATH."/".$_REQUEST['site_domain']."/site_images";			
				cp_files($src_folder,$dest_folder);
				
				$src_folder 	= $site_main_docroot."/themes/".$theme_path."/swf";
				$dest_folder 	= IMAGE_ROOT_PATH."/".$_REQUEST['site_domain']."/swf";			
				cp_files($src_folder,$dest_folder);
			}			
			// Copying the default newsletter template if any exists in table common_newsletter_template
			
			$sql_newslettertemp = "SELECT  letter_subject, letter_content, letter_product 
									FROM 
										common_newsletter_template";
			$ret_newslettertemp = $db->query($sql_newslettertemp);
			if ($db->num_rows($ret_newslettertemp))
			{
				$path = "http://".$_REQUEST['site_domain']."/images/".$_REQUEST['site_domain']."/static";
				while ($row_newslettertemp = $db->fetch_array($ret_newslettertemp))
				{
					$temp_subject 		= 	stripslashes($row_newslettertemp['letter_subject']);
					$temp_content		= str_replace('[path]',$path,stripslashes($row_newslettertemp['letter_content']));
					$temp_prod_content 	= str_replace('[path]',$path,stripslashes($row_newslettertemp['letter_product']));
					$insert_array 								= array();
					$insert_array['sites_site_id']				= $insert_siteid;
					$insert_array['newstemplate_name']			= addslashes($temp_subject);
					$insert_array['newstemplate_template']		= addslashes($temp_content);
					$insert_array['newstemplate_hide']			= 0;
					$insert_array['product_layout']				= addslashes($temp_prod_content);
			...
			if ($db->num_rows($ret_temp))
			{
				while ($row_temp = $db->fetch_array($ret_temp))
				{
					...
					$insert_array['lettertemplate_from']		= add_slash($_REQUEST['site_email']);
					...
					$insert_array['lettertemplate_contents']	= add_slash($row_temp['template_content'],false);
			...
			//Payment types
			//##########################################################################################################
			$sql = "SELECT paytype_id,paytype_name,paytype_code FROM payment_types ORDER BY paytype_id";
			$ret = $db->query($sql);
			if ($db->num_rows($ret))
			{
				while ($row = $db->fetch_array($ret))
				{
					...
					$insert_array['paytype_caption']							= $payname;
					$insert_array['sites_site_id']								= $insert_siteid;
					if($row['paytype_code'] =='credit_card' or $row['paytype_code'] =='invoice') // if payment type is credit card or invoice, then make it selected
						$insert_array['paytype_forsites_active']					= 1;
					else
						$insert_array['paytype_forsites_active']					= 0;	
			...
			*/
			...
			if ($db->num_rows($ret_common))
			{
				...
				foreach ($row_common as $k=>$v)
				{
					if ($k!=='listing_id' and $k!=='sites_site_id')
					{
						...
						if($k=='order_confirmationmail')
							$v = add_slash($_REQUEST['site_email']);
			...
			// ################################################################################################################
			// Inserting to delivery_methods by picking the ID of delivery method 'none'
			// ################################################################################################################
			$sql_delv = "SELECT deliverymethod_id 
									FROM 
										delivery_methods 
									WHERE 
										deliverymethod_text = 'None' 
									LIMIT 
										1";
			$ret_delv = $db->query($sql_delv);
			if ($db->num_rows($ret_delv))
			{
				$row_delv 													= $db->fetch_array($ret_delv);
				// Making an entry in general_settings_site_delivery table for current site 
				$insert_array												= array();
				$insert_array['sites_site_id']							= $insert_siteid;
				$insert_array['delivery_methods_delivery_id']	= $row_delv['deliverymethod_id'];
				$insert_array['charge_split_delivery']				= 'N';
				$db->insert_from_array($insert_array,'general_settings_site_delivery');
			}
			
			// ################################################################################################################
			// Inserting to payment_capture_types by picking the ID of paymentcapture_code 'PAYMENT'
			// ################################################################################################################
			$sql_paycap = "SELECT paymentcapture_id 
									FROM 
										payment_capture_types 
									WHERE 
										paymentcapture_code = 'PAYMENT' 
									LIMIT 
										1";
			$ret_paycap = $db->query($sql_paycap);
			if ($db->num_rows($ret_paycap))
			{
				$row_paycap 												= $db->fetch_array($ret_paycap);
				// Making an entry in general_settings_site_paymentcapture_type table for current site 
				$insert_array												= array();
				$insert_array['sites_site_id']								= $insert_siteid;
				$insert_array['payment_capture_types_paymentcapture_id']	= $row_paycap['paymentcapture_id'];
				$db->insert_from_array($insert_array,'general_settings_site_paymentcapture_type');
			}
				
			// ################################################################################################################
			// Setting the default payment methods for the site. By default only SELF will be made active
			// ################################################################################################################			
			
				// Get the paymethod_id for self
				$sql_pmethod 	= "SELECT paymethod_id,paymethod_name  
									FROM 
										payment_methods  
									WHERE 
										paymethod_key IN ('SELF')";
				$ret_pmethod 		= $db->query($sql_pmethod);
				if($db->num_rows($ret_pmethod))
				{
					while ($row_pmethod = $db->fetch_array($ret_pmethod))
					{
						// Making entry in payment_types_forsites table
						$insert_array									= array();
						$insert_array['payment_methods_paymethod_id']	= $row_pmethod['paymethod_id'];
						$insert_array['payment_method_sites_caption']	= addslashes(stripslashes($row_pmethod['paymethod_name']));
						$insert_array['sites_site_id']					= $insert_siteid;
						$db->insert_from_array($insert_array,'payment_methods_forsites');
					}
				}	
			// ################################################################################################################
			// Setting the Credit Card types supported by current site. By default all cards will be ticked
			// ################################################################################################################			
			
				// Get the cardtype_id for all cards
				$sql_card 	= "SELECT cardtype_id  
									FROM 
										payment_methods_supported_cards";
				...
				if($db->num_rows($ret_card))
				{
					...
					{
						// Making entry in payment_methods_sites_supported_cards  table
						$insert_array													= array();
						$insert_array['payment_methods_supported_cards_cardtype_id']	= $row_card['cardtype_id'];
						$insert_array['sites_site_id']									= $insert_siteid;
						$insert_array['supportcard_order']								= $card_cnt++;
						$db->insert_from_array($insert_array,'payment_methods_sites_supported_cards');
					}
				}	
									
			// Creating require cache files
			$base_path = IMAGE_ROOT_PATH."/".$_REQUEST['site_domain'];
			
			create_GeneralSettings_CacheFile($base_path,$insert_siteid); // general settings cache
			
			create_PriceDisplaySettings_CacheFile($base_path,$insert_siteid); // price display cache
			
			create_Captions_CacheFile_All($base_path,$insert_siteid); // captions cache
			
			create_SitemenModmenu_CacheFile($base_path,$insert_siteid); // site menu and mod menu cache
			
			create_Currency_CacheFile($base_path,$insert_siteid); // currency cache
			
			create_Tax_Delivery_Paytype_Paymethod_CacheFile($base_path,$insert_siteid); // tax, delivery. paytype, paymethod cache 
			
			if($_REQUEST['chk_sendmail']==1) // Check whether the details of current site is to be send to customer
			{
				require("classes/mime.php");
				// Preparing the content to be send as email
				$to 				= $_REQUEST['site_email'];
				$subject			= 'Business 1st: your new online shop details';
				ob_start();
				include("emails/siteadded.php");
				$content 			= ob_get_contents();
				ob_end_clean();
				headed_email($to, $subject, $content, array());
			}
			//##############################
			//#Success Msg		
			//##############################
			$alert = 'Successfully Added Site '.$_REQUEST['site_domain'];
	...
	if($_REQUEST['Updatesite_submit'])
	{
		...
		if(!$alert) {
			...
			$update_array['site_domain_alias']				= $_REQUEST['site_domain_alias'];
			...
			$update_array['mobile_themes_theme_id']			= ($_REQUEST['mobile_theme_id']>0)?$_REQUEST['mobile_theme_id']:0;
			...
			$update_array['site_activate_invoice']			= ($_REQUEST['site_activate_invoice'])?1:0;
			$update_array['in_mobile_api']					= ($_REQUEST['in_mobile_api'])?1:0;
			
			$update_array['site_grid_enable']				= ($_REQUEST['site_grid_enable'])?1:0;
     		$update_array['linked_product_cart']			= ($_REQUEST['linked_product_cart'])?1:0;
	
     		$update_array['site_email_verification']		= ($_REQUEST['site_email_verification'])?1:0;

      
			$update_array['site_footer_scripts']			= addslashes($_REQUEST['site_footer_scripts']);
			$update_array['site_checkout_scripts']			= addslashes($_REQUEST['site_checkout_scripts']);
			...
			$update_array['webtracker_account_id']                  = $webtracker_id;
			$update_array['site_updation_notification_emailids']    = addslashes($_REQUEST['site_updation_notification_emailids']);
			$update_array['template_product_layout']                = addslashes($_REQUEST['template_product_layout']);
			/*FB login script starts here*/
            $update_array['site_fb_enable']							= ($_REQUEST['site_fb_enable'])?1:0; 
            $update_array['site_fb_appid'] 							= addslashes($_REQUEST['site_fb_appid']);
            $update_array['site_fb_secretkey'] 						= addslashes($_REQUEST['site_fb_secretkey']);
			/*FB login script ends here*/
			...
			if ($db->num_rows($ret))
			{
				while ($row = $db->fetch_array($ret))
				{
					...
					if ($db->num_rows($ret_exists)==0)
					{
						$insert_array									= array();
						$insert_array['paytype_id']						= $payid;
						$insert_array['paytype_caption']				= addslashes(stripslashes($payname));
						$insert_array['sites_site_id']					= $_REQUEST['site_id'];
						$insert_array['paytype_forsites_active']		= 1;
						$insert_array['paytype_forsites_userdisabled']	= 0;
						$db->insert_from_array($insert_array,'payment_types_forsites');
			...
			create_SitemenModmenu_CacheFile($base_path,$_REQUEST['site_id']); // site menu and mod menu cache
			create_Currency_CacheFile($base_path,$_REQUEST['site_id']); // currency cache
			create_Tax_Delivery_Paytype_Paymethod_CacheFile($base_path,$_REQUEST['site_id']) ;// tax, delivery. paytype, paymethod cache 
			
			//create_Captions_CacheFile_All($base_path,$_REQUEST['site_id']); // captions cache
	...
	if($_REQUEST['assign_payment_methods'] == 'Save Selected Payment Methods') {
		...
		if(is_array($existingPymethod)){ // to delete an existing payment method from the already existing array
			foreach ($existingPymethod as $key => $val){
				if(!is_array($_REQUEST['paymt_method']))
				$paymt_method = $_REQUEST['paymt_method'] = array();
				if(!array_key_exists($val,$_REQUEST['paymt_method'])){
					$sql_sel = "SELECT payment_methods_forsites_id FROM payment_methods_forsites WHERE payment_methods_paymethod_id =$val AND sites_site_id =".$_REQUEST['site_id'];
					$ret_sel = $db->query($sql_sel);
					if ($db->num_rows($ret_sel))
					{
						while ($row_sel = $db->fetch_array($ret_sel))
						{
							$del = "DELETE FROM payment_methods_forsites_details WHERE payment_methods_forsites_id=".$row_sel['payment_methods_forsites_id']." 
											AND sites_site_id = ".$_REQUEST['site_id'];
							$db->query($del);				
						}
					}
		...
		if(is_array($_REQUEST['paymt_method'])){ // to insert new methods from the selected check box array
			foreach ($_REQUEST['paymt_method'] as $key => $val){
				$insert_array = array();
				if(!is_array($existingPymethod)) {
				$existingPymethod = array();
				}
				if(!in_array($key,$existingPymethod)){
					// Get the original name of payment method
					$sql_org = "SELECT paymethod_name 
									FROM 
										payment_methods 
									WHERE 
										paymethod_id = $key 
									LIMIT 
										1";
					$ret_org = $db->query($sql_org);
					if ($db->num_rows($ret_org))
					{
						$row_org = $db->fetch_array($ret_org);
					}
					$insert_array['payment_methods_paymethod_id']  = $key;
					$insert_array['payment_method_sites_caption']  = addslashes(stripslashes($row_org['paymethod_name']));
					$insert_array['sites_site_id']  = $_REQUEST['site_id'];
					$db->insert_from_array($insert_array, 'payment_methods_forsites');
				}
		...
		$row_dom 		= $db->fetch_array($ret_dom);
		$base_path 	= IMAGE_ROOT_PATH."/".$row_dom['site_domain'];
		create_Tax_Delivery_Paytype_Paymethod_CacheFile($base_path,$_REQUEST['site_id']);
		include("includes/sites/list_site_pymt_methods.php");
}
}
...
else if($_REQUEST['fpurpose'] == 'update_paymethod_values')
{
	...
	while($row_existing_values 	= $db->fetch_array($res_existing_values)){
		...
		if((in_array($row_existing_values['paydet_id'], $_REQUEST['payment_methods_details_id'])) && ($_REQUEST['details_values'][$row_existing_values[	'payment_methods_details_payment_method_details_id']] !='')) {
			...
			$update_array['payment_methods_forsites_details_values']	         = add_slash($_REQUEST['details_values'][$row_existing_values['payment_methods_details_payment_method_details_id']]);
	...
	if( ($_REQUEST['details_values'][$row_existing_values['payment_methods_details_payment_method_details_id']] =='') && ($row_existing_values['payment_methods_forsites_details_values'] !='')){
	...
	foreach($_REQUEST['payment_methods_details_id'] as $key => $val)
	{
		if (($val == '') && ($_REQUEST['details_values'][$key]) !='' )
		{ //checking whether a new value in the array passed from the form 
		...
		//echo "insert detail id=".$key;
			...
			$insert_array['payment_methods_forsites_details_values'] = add_slash($_REQUEST['details_values'][$key]);
...
//-------------------------------------------------------------------------------		
		...
		// Combo Table 
			...
			$sql_sel = "SELECT products_product_id FROM combo_products WHERE sites_site_id = $del_id";
			$ret_sel = $db->query($sql_sel);
			if($db->num_rows($ret_sel))
			{
				while ($row_sel = $db->fetch_array($ret_sel))
				{
					$sql_del = "DELETE FROM combo_products_variable_combination_map WHERE products_product_id=".$row_sel['products_product_id'];
					$db->query($sql_del);
					$sql_del = "DELETE FROM combo_products_variable_combination WHERE products_product_id=".$row_sel['products_product_id'];
					$db->query($sql_del);
				}
			}
		...
		$sql_del = "DELETE FROM customer_discount_group_categories_map WHERE sites_site_id = $del_id";
		...
		$sql_del = "DELETE FROM console_news WHERE sites_site_id = $del_id";
		$db->query($sql_del);
		
		$sql_del = "DELETE FROM console_suggestions WHERE sites_site_id = $del_id";
		$db->query($sql_del);
		
		//=================================
		
		$sql_del = "DELETE FROM costperclick_advertplacedon WHERE sites_site_id = $del_id";
		$db->query($sql_del);
		
		$sql_del = "DELETE FROM costperclick_adverturl WHERE sites_site_id = $del_id";
		$db->query($sql_del);

		$sql_del = "DELETE FROM costperclick_keywords WHERE sites_site_id = $del_id";
		$db->query($sql_del);

		$sql_del = "DELETE FROM costperclick_month WHERE sites_site_id = $del_id";
		$db->query($sql_del);
		
		$sql_del = "DELETE FROM costperclick_settings WHERE sites_site_id = $del_id";
		$db->query($sql_del);		
		
		$sql_del = "DELETE FROM costperclick_time WHERE sites_site_id = $del_id";
		$db->query($sql_del);
				
		
		//=================================
	
		$sql_del = "DELETE FROM custom_layout_spec_form WHERE sites_site_id = $del_id";
		$db->query($sql_del);
		...
		$sql_del = "DELETE FROM customer_email_notification WHERE sites_site_id = $del_id";
		...
		/*$sqldel = "SELECT cart_id 
						  FROM google_order_cart WHERE sites_site_id = $del_id";
		$resdel = $db->query($sqldel);	
		while($rowdel = $db->fetch_array($resdel)) {

			$cart_id = $rowdel['cart_id'];
			
			$sql_del = "DELETE FROM google_order_cart_details WHERE google_order_cart_cart_id = $voucher_id";
			$db->query($sql_del);
		}	
		
		$sql_del = "DELETE FROM google_order_cart WHERE sites_site_id = $del_id";
		...
		$sql_del = "DELETE FROM newsletter_template WHERE sites_site_id = $del_id";
		$db->query($sql_del);
		...
		while($rowdel = $db->fetch_array($resdel)) {
			...
			while($rowdel = $db->fetch_array($resdel)) {
					...
					$sql_del = "DELETE FROM order_details_backorder WHERE orderdet_id = $orderdet_id";
					$db->query($sql_del);

					$sql_del = "DELETE FROM order_details_cancelled WHERE orderdet_id = $orderdet_id";
					$db->query($sql_del);					
					...
					$sql_del = "DELETE FROM order_details_return WHERE orderdet_id = $orderdet_id";
					$db->query($sql_del);
			...
			$sql_del = "DELETE FROM order_payment_paypal WHERE orders_order_id = $order_id";
			$db->query($sql_del);
		...
		$sqldel = "SELECT pay_id 
						  FROM order_payonaccount_details WHERE sites_site_id = $del_id";
		$resdel = $db->query($sqldel);	
		while($resrow = $db->fetch_array($resdel)) {

			$pay_id = $resrow['pay_id'];

			$sql_del = "DELETE FROM order_payonaccount_cheque_details WHERE order_payaccount_cheque_pay_id = $pay_id";
			$db->query($sql_del);	
			
			$sql_del = "DELETE FROM order_payonaccount_payment WHERE order_payonaccount_pay_id = $pay_id";
			$db->query($sql_del);	
			
		}
		
		$sql_del = "DELETE FROM order_payonaccount_details WHERE sites_site_id = $del_id";
		$db->query($sql_del);
		
		
		$sqldel = "SELECT pendingpay_id 
						  FROM order_payonaccount_pending_details WHERE sites_site_id = $del_id";
		$resdel = $db->query($sqldel);	
		while($resrow = $db->fetch_array($resdel)) {

			$pendingpay_id = $resrow['pendingpay_id'];

			$sql_del = "DELETE FROM order_payonaccount_pending_details_cheque_details 
								WHERE order_payonaccount_pending_details_pending_id = $pendingpay_id";
			$db->query($sql_del);	
			
			$sql_del = "DELETE FROM order_payonaccount_pending_details_payment 
								WHERE order_payonaccount_pendingpay_id = $pendingpay_id";
			$db->query($sql_del);	
			
		}
		
		$sql_del = "DELETE FROM order_payonaccount_pending_details WHERE sites_site_id = $del_id";
		$db->query($sql_del);

		
		$sql_del = "DELETE FROM order_product_downloadable_products WHERE sites_site_id = $del_id";
		$db->query($sql_del);
		
		$sql_del = "DELETE FROM order_product_downloadable_products_customer_track WHERE sites_site_id = $del_id";
		$db->query($sql_del);
		...
		$sql_del = "DELETE FROM payonaccount_cartvalues WHERE sites_site_id = $del_id";
		$db->query($sql_del);
		...
		$resdel = $db->query($sqldel);	
			...
			while($rowdel = $db->fetch_array($resdel)) {
					...
					$sql_del = "DELETE FROM product_category_hit_count WHERE category_id = $category_id";
					$db->query($sql_del);
		...
		$sql_del = "DELETE FROM product_category_hit_count_totals WHERE sites_site_id = $del_id";
		$db->query($sql_del);
		
		$sql_del = "DELETE FROM product_downloadable_products WHERE sites_site_id = $del_id";
		$db->query($sql_del);
		...
		$sql_del = "DELETE FROM product_common_attachments WHERE sites_site_id = $del_id";
		
		$db->query($sql_del);
		
		$sql_del = "DELETE FROM product_common_tabs WHERE sites_site_id = $del_id";
		...
		$resdel = $db->query($sqldel);	
			...
			while($rowdel = $db->fetch_array($resdel)) {
					...
					$sql_del = "DELETE FROM product_hit_count WHERE product_id = $product_id";
					$db->query($sql_del);
					
					$sql_del = "DELETE FROM product_stock_request_details WHERE products_product_id = $product_id";
					$db->query($sql_del);
					
					$sql_del = "DELETE FROM product_stock_transfer_track WHERE product_id = $product_id";
					$db->query($sql_del);
		...
		$sqldel = "SELECT notify_id FROM product_stock_update_notification WHERE sites_site_id = $del_id";
		$resdel = $db->query($sqldel);
		while($row = $db->fetch_array($resdel)) {
		
			$notify_id = $row['notify_id'];
			
			$sql_del = "DELETE FROM product_stock_update_notification_messages WHERE product_stock_update_notification_notify_id = $notify_id";
			$db->query($sql_del);
			
			$sql_del = "DELETE FROM product_stock_update_notification_variables WHERE product_stock_update_notification_notify_id = $notify_id";
			$db->query($sql_del);
		
		}
		
		$sql_del = "DELETE FROM product_stock_update_notification WHERE sites_site_id = $del_id";
		$db->query($sql_del);
		
		$sql_del = "DELETE FROM product_hit_count_totals WHERE sites_site_id = $del_id";
		$db->query($sql_del);
		...
		$sql_sel = "SELECT pcode_det_id 
						FROM 
							promotional_code_product 
						WHERE 
							sites_site_id=".$del_id;
		$ret_sel = $db->query($sql_sel);
		if($db->num_rows($ret_sel))
		{
			while ($row_sel = $db->fetch_array($ret_sel))
			{
				$sql_com = "SELECT comb_id 
								FROM 
									promotional_code_products_variable_combination 
								WHERE 
									promotional_code_product_pcode_det_id=".$row_sel['pcode_det_id'];
				$ret_com = $db->query($sql_com);
				if($db->num_rows($ret_com))
				{
					while ($row_com = $db->fetch_array($ret_com))
					{
						$sql_del = "DELETE FROM 
										promotional_code_products_variable_combination_map  
									WHERE 
										promotional_code_products_variable_combination_comb_id=".$row_com['comb_id'];
						$db->query($sql_del);
					}
				}
				$sql_del = "DELETE FROM 
		...
		$sql_del = "DELETE FROM product_preset_variables WHERE sites_site_id = $del_id";
		$db->query($sql_del);
		
		$sql_del = "DELETE FROM product_preset_variable_data WHERE sites_site_id = $del_id";
		$db->query($sql_del);
		...
		$resdel = $db->query($sqldel);	
			...
			while($rowdel = $db->fetch_array($resdel)) {
					...
					$sql_del = "DELETE FROM quote_dynamicvalues WHERE quote_quote_id = $quote_id";
					$db->query($sql_del);
		...
		$sql_del = "DELETE FROM se_bestseller_titles WHERE sites_site_id = $del_id";
		$db->query($sql_del);
		...
		//======================= SEO Tables ===============================
		$sql_del = "DELETE FROM seo_revenue_keywords WHERE site_id = $del_id";
		$db->query($sql_del);
		
		$sqldel = "SELECT click_id FROM seo_revenue_total WHERE site_id = $del_id";
		$resdel = $db->query($sqldel);
		while($resrow = $db->fetch_array($resdel))
		{
			$click_id = $resrow['click_id'];
			
			$sql_del = "DELETE FROM seo_revenue_month WHERE click_id = $click_id";
			$db->query($sql_del);
		}
		
		$sql_del = "DELETE FROM seo_revenue_total WHERE site_id = $del_id";
		$db->query($sql_del);
...
$newsite = $sitedomain."_bak";
		...
		$alert_del = '<center><font color="red"><b> Site has been removed </b></font><br>';
		include("includes/sites/list_sites.php");
...
foreach($_REQUEST['shop_order'] as $key => $val){
	...
	$update_array['shop_order'] 		= $_REQUEST['shop_order'][$key];
	$update_array['shop_active'] 		= $_REQUEST['shop_active'][$key];
...
foreach($_REQUEST['ableto_order'] as $key => $val){
	$update_array = array();
	$update_array['det_order'] 		= $_REQUEST['ableto_order'][$key];
	$update_array['det_hidden'] 			= $_REQUEST['ableto_active'][$key];
...
function cp_files($src_folder,$dest_folder)
{
	if($src_folder != IMAGE_ROOT_PATH && $dest_folder != IMAGE_ROOT_PATH)
	{
		if ($handle = opendir($src_folder))
		{
			while (false !== ($file = readdir($handle)))
			{
				if ($file != "." && $file != "..")
				{
					...
					{	
						//Check whether the extension is exe
						$splt 	= explode(".",$file);
						$len 	= count($splt);
						if ($len>1)
						{
							if (strtolower($splt[$len-1])!='exe')
							{
								copy ($src_folder.'/'.$file,$dest_folder.'/'.$file);
							}	
						}
					}
				}
