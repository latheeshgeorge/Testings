...
<?php 
$filename = str_replace(" ", "_", $HTTP_POST_FILES['FCKeditor_File']['name']);
$savefile = "$upload_dir/" . $filename;
$temp_filename = str_replace(" ", "_", $HTTP_POST_FILES['FCKeditor_File']['tmp_name']);
...
elseif ($HTTP_POST_FILES['FCKeditor_File']['size']>301000)
{
	...
	if (is_uploaded_file($temp_filename)) {
		...
		/*//window.opener.setImage('<?php echo "/images/$ecom_hostname/static/" . $HTTP_POST_FILES['FCKeditor_File']['name'] ?>') ; window.close();</SCRIPT>
		<?php
