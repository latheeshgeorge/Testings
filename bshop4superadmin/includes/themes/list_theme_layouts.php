...
$sort_options = array('layout_name' => 'Layout name','layout_code' => 'Layout Code');
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
	<tr>
		...
		<table border="0" cellpadding="2" cellspacing="0" width="100%" align="left">
		  <tr> 
			<td class="menutabletoptd"><a href="home.php?request=themes&theme_name=<?=$_REQUEST['pass_theme_name']?>&themetype=<?=$_REQUEST['pass_themetype']?>&sort_by=<?=$_REQUEST['pass_sort_by']?>&sort_order=<?=$_REQUEST['pass_sort_order']?>&records_per_page=<?=$_REQUEST['pass_records_per_page']?>&pg=<?=$_REQUEST['pass_pg']?>" title="List Themes">List Themes</a><strong> <font size="1">>></font> List <?=$page_type?> for <?php echo $selthemename?>
    ...
    <tr  class="<?=$class_val;?>">
      <td height="20" align="center"><?=$count_no?>
        &nbsp;&nbsp;&nbsp;</td>
      <td height="20" align="center"><a href="home.php?request=themes&fpurpose=edit_layout&layout_id=<?=$row['layout_id']?>&pass_theme_id=<?=$_REQUEST['pass_theme_id']?>&theme_name=<?=$theme_name?>&sort_by=<?=$sort_by?>&sort_order=<?=$sort_order?>&records_per_page=<?=$records_per_page?>&pg=<?=$pg?>&pass_sort_by=<?=$_REQUEST['pass_sort_by']?>&pass_sort_order=<?=$_REQUEST['pass_sort_order']?>&pass_records_per_page=<?=$_REQUEST['pass_records_per_page']?>&pass_pg=<?=$_REQUEST['pass_pg']?>&pass_theme_name=<?=$_REQUEST['pass_theme_name']?>&pass_themetype=<?=$_REQUEST['pass_themetype']?>" title="Edit"><?php echo stripslashes($row['layout_name']); ?></a></td>
      ...
      <td width="16%" align="center" valign="middle"><a href="home.php?request=themes&fpurpose=edit_layout&layout_id=<?=$row['layout_id']?>&pass_theme_id=<?=$_REQUEST['pass_theme_id']?>&theme_name=<?=$theme_name?>&sort_by=<?=$sort_by?>&sort_order=<?=$sort_order?>&records_per_page=<?=$records_per_page?>&pg=<?=$pg?>&pass_sort_by=<?=$_REQUEST['pass_sort_by']?>&pass_sort_order=<?=$_REQUEST['pass_sort_order']?>&pass_records_per_page=<?=$_REQUEST['pass_records_per_page']?>&pass_pg=<?=$_REQUEST['pass_pg']?>&pass_theme_name=<?=$_REQUEST['pass_theme_name']?>&pass_themetype=<?=$_REQUEST['pass_themetype']?>" title="Edit"><img src="images/edit.gif" border="0" alt="Edit" /></a>&nbsp;
     <a href="#" onclick="if(delete_confirm()) { location.href='home.php?request=themes&fpurpose=delete_layout&layout_id=<?=$row['layout_id']?>&pass_theme_id=<?=$_REQUEST['pass_theme_id']?>&theme_name=<?=$theme_name?>&sort_by=<?=$sort_by?>&sort_order=<?=$sort_order?>&records_per_page=<?=$records_per_page?>&pg=<?=$pg?>&pass_sort_by=<?=$_REQUEST['pass_sort_by']?>&pass_sort_order=<?=$_REQUEST['pass_sort_order']?>&pass_records_per_page=<?=$_REQUEST['pass_records_per_page']?>&pass_pg=<?=$_REQUEST['pass_pg']?>&pass_theme_name=<?=$_REQUEST['pass_theme_name']?>&pass_themetype=<?=$_REQUEST['pass_themetype']?>';}" title="Delete"><img src="images/delete.gif"  border="0" /></a> </td>
    </tr>
    <?
	}
	}
	...
	?>
  <tr class="maininnertabletd1">
        <td align="center" valign="middle" class="maininnertabletd1" colspan="<?=$colspan?>"><input type="button" name="add_themes" value="Add <?=$page_type?>" onclick="location.href='home.php?request=themes&fpurpose=theme_layout_add&pass_theme_id=<?=$_REQUEST['pass_theme_id'];?>&theme_name=<?=$theme_name?>&sort_by=<?=$sort_by?>&sort_order=<?=$sort_order?>&records_per_page=<?=$records_per_page?>&pg=<?=$pg?>&pass_sort_by=<?=$_REQUEST['pass_sort_by']?>&pass_sort_order=<?=$_REQUEST['pass_sort_order']?>&pass_records_per_page=<?=$_REQUEST['pass_records_per_page']?>&pass_pg=<?=$_REQUEST['pass_pg']?>&pass_theme_name=<?=$_REQUEST['pass_theme_name']?>&pass_themetype=<?=$_REQUEST['pass_themetype']?>';" class="input-button"/></td>      
  </tr>
  <?php 
  $query_string .= "sort_by=$sort_by&sort_order=$sort_order&request=themes&fpurpose=theme_layout&pass_theme_id=".$_REQUEST['pass_theme_id'];
